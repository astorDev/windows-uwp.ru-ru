...
1. Откройте командную строку PowerShell, введите `python`, чтобы запустить интерпретатор Python 3. (В некоторых инструкциях предпочтительно использовать команду `py` или `python3`, они также должны работать). Вы узнаете, что вы завершили успешно, так как > > > выводится три символа больше, чем.
...
3. Распечатайте переменную с помощью: `print(variable)`. Отобразится текст "Hello World!".

4. Выясните, сколько символов используется строковой переменной: `len(variable)`. При этом будет показано, что используется 12 символов. (Обратите внимание, что пустое пространство, подсчитанное как символ в общей длине.)

5. Преобразуйте строковую переменную в прописные: `variable.upper()`. Теперь преобразуйте строковую переменную в буквы в нижнем регистре: `variable.lower()`.
...
2. Проверьте установку, выполнив пример игры: `python -m pygame.examples.aliens`.
