...
9. Убедитесь, что установлен Node. js и текущая версия по умолчанию: `node --version`. Затем убедитесь, что у вас также есть NPM: `npm --version` (можно также использовать `which node` или `which npm`, чтобы увидеть путь, используемый для версий по умолчанию).
