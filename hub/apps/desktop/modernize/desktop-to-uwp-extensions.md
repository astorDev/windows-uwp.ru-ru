...
|Категория | Всегда ``windows.fileTypeAssociation``.
|Имя |Имя сопоставления типа файла. Это имя можно использовать для упорядочения и группировки типов файлов. Имя должно содержать все символы нижнего регистра без пробелов. |
|Предикат |Имя, которое отображается в контекстном меню проводника. Эта строка может быть локализована с помощью ```ms-resource```.|
...
|Предикат |Имя, отображаемое в контекстном меню проводника для файлов, предоставляемых облачной службы. |
...
|Активировать |Указывает, включен или отключен начальный запуск задачи. Включенная задача будет запущена при следующем входе пользователя (если только пользователь не отключил ее). |
