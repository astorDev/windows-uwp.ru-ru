...
* Измените проект, чтобы использовать среда выполнения Windows API. Инструкции см. в [этой статье](desktop-to-uwp-enhance.md#set-up-your-project).

* Установите последний пакет NuGet [Microsoft. Toolkit. WPF. UI. Controls](https://www.nuget.org/packages/Microsoft.Toolkit.Wpf.UI.Controls) (для WPF) или [Microsoft. Toolkit. Forms. UI. Controls](https://www.nuget.org/packages/Microsoft.Toolkit.Forms.UI.Controls) (для Windows Forms) в проекте. Убедитесь, что установлена версия 6.0.0-Preview 6.4 или более поздняя версия пакета.
...
* Выполните одно из следующих действий.
    ...
    **Упакуйте приложение в пакет MSIX**. Упаковка приложения в [пакет MSIX](https://docs.microsoft.com/windows/msix/) предоставляет множество преимуществ при развертывании и запуске.
    1. Установите пакет SDK для Windows 10 версии 1903 (или более поздней версии).
    ...
    **Задайте значение maxversiontested укажите установленную в манифесте приложения**. Если вы не хотите упаковывать приложение в пакет MSIX, прежде чем использовать острова XAML, необходимо добавить [манифест приложения](https://docs.microsoft.com/windows/desktop/SbsCs/application-manifests) в проект и добавить в манифест элемент **maxversiontested укажите установленную** , чтобы указать, что приложение совместимо с Windows 10 версии 1903 или более поздней.
    ...
    2. В манифест приложения включите элемент **Compatibility** и дочерние элементы, показанные в следующем примере. Замените атрибут **ID** элемента **maxversiontested укажите установленную** на номер версии Windows 10 для целевой платформы (это должна быть windows 10, версия 1903 или более поздняя).
        ...
        > [!NOTE]
        > При добавлении элемента **maxversiontested укажите установленную** в манифест приложения может появиться следующее предупреждение сборки в проекте: `manifest authoring warning 81010002: Unrecognized Element "maxversiontested" in namespace "urn:schemas-microsoft-com:compatibility.v1"`. Это предупреждение не указывает на то, что в проекте ничего не так, и его можно пропустить.
