...
* **Проект приложения UWP, определяющий объект ксамлаппликатион**. Проект WPF (или Windows Forms) должен иметь доступ к экземпляру класса `Microsoft.Toolkit.Win32.UI.XamlHost.XamlApplication`, предоставляемого набором средств сообщества Windows. Этот объект выступает в качестве корневого поставщика метаданных для загрузки метаданных пользовательских типов универсального кода пользователя UWP в сборки в текущем каталоге приложения. Для этого рекомендуется добавить **пустой проект приложения (универсальное приложение Windows)** в то же решение, что и проект WPF (или Windows Forms), и исправить класс `App` по умолчанию в этом проекте.
  > [!NOTE]
...
4. Перед закрывающим элементом `</Project>` добавьте следующий XML-код, чтобы отключить несколько свойств, а затем сохраните файл проекта. Эти свойства должны быть включены для размещения пользовательского элемента управления UWP в приложении WPF (или Windows Forms).
...
7. В файле XAML для пользовательского элемента управления добавьте следующий `StackPanel` в качестве дочернего элемента для `Grid`по умолчанию. В этом примере добавляется элемент управления ``TextBlock``, а затем атрибут ``Text`` этого элемента управления привязывается к полю ``XamlIslandMessage``.
...
8. В файле кода программной части пользовательского элемента управления добавьте поле `XamlIslandMessage` в класс пользовательского элемента управления, как показано ниже.
...
3. В том же файле добавьте следующий элемент управления в элемент `<Grid>`. Измените атрибут `InitialTypeName` на полное имя пользовательского элемента управления в проекте библиотеки классов UWP.

    ```xml
    <xaml:WindowsXamlHost InitialTypeName="UWPClassLibrary.MyUserControl" ChildChanged="WindowsXamlHost_ChildChanged" />
    ```

4. Откройте файл кода программной части и добавьте следующий код в класс `Window`. Этот код определяет `ChildChanged` обработчик событий, который присваивает значение поля ``XamlIslandMessage`` пользовательского элемента управления UWP значению поля `WPFMessage` в приложении WPF. Измените `UWPClassLibrary.MyUserControl` на полное имя пользовательского элемента управления в проекте библиотеки классов UWP.
...
2. В файле App. XAML в этом проекте добавьте следующий дочерний элемент в элемент `<xaml:XamlApplication>`.
...
3. В проекте библиотеки классов UWP установите последнюю версию пакета NuGet [Microsoft. UI. XAML](https://www.nuget.org/packages/Microsoft.UI.Xaml) (той же версии, которая была установлена в проекте приложения UWP).

4. В том же проекте откройте файл XAML для пользовательского элемента управления и добавьте следующее объявление пространства имен в элемент `<UserControl>`.

    ```xml
    xmlns:winui="using:Microsoft.UI.Xaml.Controls"
    ```

5. В том же файле добавьте элемент `<winui:RatingControl />` в качестве дочернего элемента `<StackPanel>`. Этот элемент добавляет экземпляр класса [ратингконтрол](https://docs.microsoft.com/uwp/api/microsoft.ui.xaml.controls.ratingcontrol?view=winui-2.2) из библиотеки винуи. После добавления этого элемента `<StackPanel>` должен выглядеть примерно так, как показано далее.
...
4. Измените файл проекта WPF. В настоящее время эти изменения необходимы для упаковки приложений WPF, в которых размещаются настраиваемые элементы управления UWP.
    ...
    3. Нахождение последнего `</PropertyGroup>` закрывающего тега в файле и добавьте следующий XML сразу после этого тега.
