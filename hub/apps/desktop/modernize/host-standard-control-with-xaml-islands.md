...
* **Проект приложения UWP, определяющий класс корневого приложения, производного от ксамлаппликатион**. Проект WPF или Windows Forms должен иметь доступ к экземпляру класса [Microsoft. Toolkit. Win32. UI. ксамлхост. ксамлаппликатион](https://github.com/windows-toolkit/Microsoft.Toolkit.Win32/tree/master/Microsoft.Toolkit.Win32.UI.XamlApplication) , предоставляемого набором средств сообщества Windows. Для этого рекомендуется определить этот объект в отдельном проекте приложения UWP, который является частью решения для приложения WPF или Windows Forms. Этот объект выступает в качестве корневого поставщика метаданных для загрузки метаданных пользовательских типов универсального кода пользователя UWP в сборки в текущем каталоге приложения.

    > [!NOTE]
    > Хотя объект `XamlApplication` не требуется для размещения первого элемента управления UWP, приложению требуется этот объект для поддержки всего спектра сценариев остров XAML, включая размещение пользовательских элементов управления UWP. Поэтому рекомендуется всегда определять объект `XamlApplication` в любом решении, в котором используются острова XAML.
...
Затем добавьте проект приложения UWP в решение и измените класс `App` по умолчанию в этом проекте, чтобы он был производным от класса [Microsoft. Toolkit. Win32. UI. ксамлхост. ксамлаппликатион](https://github.com/windows-toolkit/Microsoft.Toolkit.Win32/tree/master/Microsoft.Toolkit.Win32.UI.XamlApplication) , предоставляемого набором средств сообщества Windows.

> [!NOTE]
> Хотя этот шаг не требуется для размещения первого элемента управления UWP, приложению требуется объект `XamlApplication` для поддержки всего спектра сценариев остров XAML, включая размещение пользовательских элементов управления UWP. Поэтому рекомендуется всегда определять объект `XamlApplication` в любом решении, в котором используются острова XAML.
...
Затем добавьте код в точку входа для приложения WPF, чтобы создать экземпляр класса `App`, который вы только что определили в проекте UWP (это класс, который теперь является производным от `XamlApplication`).
