...
Как правило все значения увеличивается или уменьшается отменить друг с другом. Тем не менее сцены уже была просмотру с обычной геометрией вызывает ошибку некоторые пикселей, чтобы тест z буфера как отображаемое shadow volume. Значения, оставшиеся в буфере трафарета, соответствуют пикселям в тени. Оставшееся содержимое буфера трафарета используется в качестве маски, чтобы выполнить альфа-смешение крупного, всеобъемлющего черного квартета со сценой. Если буфер трафарета используется в качестве маски, в результате затеняются пиксели, которые находятся в тени.
