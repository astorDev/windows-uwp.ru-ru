...
## <a name="span-idtrianglespanspan-idtrianglespanspan-idtrianglespantriangle-rasterization-rules-without-multisampling"></a><span id="Triangle"></span><span id="triangle"></span><span id="TRIANGLE"></span>Треугольник растеризации правила (без мультисэмплинга)
...
## <a name="span-idline1spanspan-idline1spanspan-idline1spanline-rasterization-rules-aliased-without-multisampling"></a><span id="Line_1"></span><span id="line_1"></span><span id="LINE_1"></span>Строки растеризации правила (без мультисэмплинга перенесен)
...
## <a name="span-idline2spanspan-idline2spanspan-idline2spanline-rasterization-rules-antialiased-without-multisampling"></a><span id="Line_2"></span><span id="line_2"></span><span id="LINE_2"></span>Строки растеризации правила (без мультисэмплинга несглаженные)
...
## <a name="span-idpointspanspan-idpointspanspan-idpointspanpoint-rasterization-rules-without-multisampling"></a><span id="Point"></span><span id="point"></span><span id="POINT"></span>Точка растеризации правила (без мультисэмплинга)
...
## <a name="span-idmultisamplespanspan-idmultisamplespanspan-idmultisamplespanmultisample-anti-aliasing-rasterization-rules"></a><span id="Multisample"></span><span id="multisample"></span><span id="MULTISAMPLE"></span>Правила мультисэмплинга растеризации сглаживания
...
Форматы множественной дискретизации могут использоваться в целях отрисовки, которые могут быть считаны в шейдеры с помощью [загрузки](https://docs.microsoft.com/windows/desktop/direct3dhlsl/dx-graphics-hlsl-to-load), так как разрешение для отдельных образцов, к которым обращается шейдер, не требуется. Форматы глубины не поддерживаются для ресурса множественной дискретизации, поэтому форматы глубины ограничены только целями прорисовки.

Нетипизированные форматы поддерживают множественную дискретизацию, чтобы представление ресурсов могло по-разному интерпретировать данные. Например, можно создать мультисэмплинга ресурса с помощью R8G8B8A8\_TYPELESS, отрисовки с помощью ресурса целевой объект прорисовки с R8G8B8A8\_целое число без знака форматирования, а затем разрешить содержимое на другой ресурс с R8G8B8A8\_UNORM формат данных.
...
-   Каждый формат, который поддерживает мультисэмплинга и имеет \_UNORM, \_SRGB, \_SNORM или \_форматирует число с плавающей запятой, также поддерживает разрешение.

### <a name="span-idcentroidsamplingspanspan-idcentroidsamplingspanspan-idcentroidsamplingspancentroid-sampling-of-attributes-when-multisample-antialiasing"></a><span id="Centroid_Sampling"></span><span id="centroid_sampling"></span><span id="CENTROID_SAMPLING"></span>Центроид выборки атрибутов при мультисэмплинга сглаживания
...
### <a name="span-idderivativecalculationsspanspan-idderivativecalculationsspanspan-idderivativecalculationsspanderivative-calculations-when-multisampling"></a><span id="Derivative_Calculations"></span><span id="derivative_calculations"></span><span id="DERIVATIVE_CALCULATIONS"></span>Класс, производный от вычислений, когда мультисэмплинга
