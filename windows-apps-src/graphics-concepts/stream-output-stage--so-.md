...
При полоса треугольник или строку привязан к сцене Assembler входные данные (IA), каждой из лент преобразуется в список перед их передаваемых в работе. Вершины всегда записываются как полный примитивы (например, 3 вершины во время для треугольники); Неполный примитивы никогда не отправляются в работе. Простые типы с соседство удалить данные соседство перед потоковая передача данных в работе.
