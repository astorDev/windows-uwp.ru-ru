...
Текущий пиксель записывается на целевую поверхность, если тест трафаретом пройден, в ином случае пиксель пропускается. Порядок сравнения по умолчанию является создание пикселя, независимо от того, как оказывается каждого битовой операции. Можно изменить это поведение, при изменении значения перечисленному типу для идентификации функции желаемую сравнения.
