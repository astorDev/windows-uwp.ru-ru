...
Сведения о режимах блоков формата BC7 доступны в статье [Справочник по режимам формата BC7](https://docs.microsoft.com/windows/desktop/direct3d11/bc7-format-mode-reference).

## <a name="span-idabout-bc7-dxgi-format-bc7spanspan-idabout-bc7-dxgi-format-bc7spanspan-idabout-bc7-dxgi-format-bc7spanabout-bc7dxgiformatbc7"></a><span id="About-BC7-DXGI-FORMAT-BC7"></span><span id="about-bc7-dxgi-format-bc7"></span><span id="ABOUT-BC7-DXGI-FORMAT-BC7"></span>О BC7/DXGI\_ФОРМАТ\_BC7


BC7 определяется следующий DXGI\_ФОРМАТ значений перечисления:

-   **DXGI\_ФОРМАТ\_BC7\_TYPELESS**.
-   **DXGI\_ФОРМАТ\_BC7\_UNORM**.
-   **DXGI\_ФОРМАТ\_BC7\_UNORM\_SRGB**.

Формат BC7 можно использовать для ресурсов текстуры [Texture2D](https://docs.microsoft.com/windows/desktop/direct3d10/d3d10-graphics-reference-resource-structures) (включая массивы), Texture3D или TextureCube (включая массивы). Аналогичным образом этот формат применяется к любым поверхностям MIP-карты, связанным с этими ресурсами.
...
Для блоков BC7 с отдельными компонентами цвета и альфа-компонентами блок состоит из битов режима, битов поворота, сжатых конечных точек, сжатых индексов и необязательного бита средства выбора индексов. Эти блоки имеют вектор действующие RGB \[R, G, B\] и скалярные альфа-канал \[A\] отдельно кодировке.
...
|---------------------------|-----------|---------------|--------------------|----------------|----------------------|----------|--------------------|
| только компоненты цвета     | обязательные  | Н/Д           | Н/Д                | обязательные       | обязательные             | необязательно | обязательные           |
| сочетание цвет + альфа    | обязательные  | Н/Д           | Н/Д                | необязательно       | обязательные             | необязательно | обязательные           |
| цвета и альфа по отдельности | обязательные  | обязательные      | необязательно           | Н/Д            | обязательные             | Н/Д      | обязательные           |
...
## <a name="span-iddecoding-the-bc7-formatspanspan-iddecoding-the-bc7-formatspanspan-iddecoding-the-bc7-formatspandecoding-the-bc7-format"></a><span id="Decoding-the-BC7-Format"></span><span id="decoding-the-bc7-format"></span><span id="DECODING-THE-BC7-FORMAT"></span>Декодирование BC7 формат
...
## <a name="span-idbc7-format-mode-referencespanspan-idbc7-format-mode-referencespanspan-idbc7-format-mode-referencespanbc7-format-mode-reference"></a><span id="BC7-format-mode-reference"></span><span id="bc7-format-mode-reference"></span><span id="BC7-FORMAT-MODE-REFERENCE"></span>Ссылка на режим BC7 формат
...
### <a name="span-idmode-7spanspan-idmode-7spanspan-idmode-7spanmode-7"></a><span id="Mode-7"></span><span id="mode-7"></span><span id="MODE-7"></span>Режим 7
...
### <a name="span-idremarksspanspan-idremarksspanspan-idremarksspanremarks"></a><span id="Remarks"></span><span id="remarks"></span><span id="REMARKS"></span>"Примечания"
...
-   Типы блоков с отдельными компонентами цвета и альфа-компонентами. В этих блоках значения цвета и альфа задаются по отдельности — каждый со своим собственным набором индексов. Таким образом, у них есть вектор эффективный и скалярные канал отдельно кодировать, где вектора обычно указывает цветовые каналы \[R, G, B\] альфа-канал с указанием скалярное \[объект\]. В поддержку такого подхода в кодировке предоставляется отдельное 2-битовое поле, которое позволяет задать отдельную кодировку канала в виде скалярного значения. В результате блок может иметь одно из следующих четырех представлений кодировки альфа (как указано в 2-битовом поле):
    ...
    После декодирования декодер снова упорядочивает каналы в порядке RGBA, поэтому внутренний формат блоков невидим разработчику. Блоки с отдельными компонентами цвета и альфа-компонентами также имеют два набора данных индекса: один — для векторного набора каналов, другой — для скалярного канала. (В случае режим 4, эти индексы являются различной ширины \[бит 2 или 3\]. Режим 4 также содержит 1-битовое средство выбора, указывающее, использует ли векторный или скалярный канал 3-битовые индексы.)

## <a name="span-idrelated-topicsspanrelated-topics"></a><span id="related-topics"></span>Связанные разделы


[Сжатие блока текстуры](texture-block-compression.md)
