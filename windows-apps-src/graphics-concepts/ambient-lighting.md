...
Внешним освещением = Cₐ\*\[Gₐ + сумма (Atten<sub>я</sub>\*плашечных<sub>я</sub>\*L<sub>ai</sub>)\]
...
-   color1 вершин, если AMBIENTMATERIALSOURCE = D3DMCS\_COLOR1 и первый цвет вершины, заданные в объявлении вершин.
-   цвет2 вершин, если AMBIENTMATERIALSOURCE = D3DMCS\_цвет2 и второй цвет вершины, заданные в объявление вершины.
-   внешний цвет материала.

**Примечание**    Если используется любой из вариантов AMBIENTMATERIALSOURCE и цвет вершины не задан, то используется материала окружающего цвета.

 

Чтобы использовать внешний цвет материала, используйте свойство SetMaterial, как показано в примере кода ниже.

Gₐ — это глобальный внешний цвет. Он устанавливается с помощью SetRenderState (D3DRS\_AMBIENT). В сцене Direct3D существует один глобальный внешний цвет. Этот параметр не связан с объектом света Direct3D.
