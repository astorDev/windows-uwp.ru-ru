...
## <a name="span-idbasicsspanspan-idbasicsspanspan-idbasicsspanhow-block-compression-works"></a><span id="Basics"></span><span id="basics"></span><span id="BASICS"></span>Инструкции по блокировке works сжатия
...
- [Хранение данных без сжатия](#storing-uncompressed-data)
- [Хранение сжатых данных](#storing-compressed-data)

### <a name="span-idstoringuncompresseddataspanspan-idstoringuncompresseddataspanspan-idstoringuncompresseddataspanspan-idstoring-uncompressed-dataspanstoring-uncompressed-data"></a><span id="Storing_Uncompressed_Data"></span><span id="storing_uncompressed_data"></span><span id="STORING_UNCOMPRESSED_DATA"></span><span id="storing-uncompressed-data"></span>Хранение данных без сжатия
...
## <a name="span-idusingblockcompressionspanspan-idusingblockcompressionspanspan-idusingblockcompressionspanusing-block-compression"></a><span id="Using_Block_Compression"></span><span id="using_block_compression"></span><span id="USING_BLOCK_COMPRESSION"></span>С помощью сжатие блока
...
В правой части схемы показаны размеры уровня MIP-карты, которые создаются для той же текстуры 60×40 со сжатием. Обратите внимание, что на втором и третьем уровнях используется заполнение памяти, чтобы сделать размеры кратными 4 на каждом уровне. Это необходимо, чтобы эти алгоритмы могли работать с блоками текселей 4×4. Это особенно очевидно, принимая во внимание уровней MIP-карт, которые меньше, чем 4 × 4; размер этих уровней mipmap очень мало будет округляться до ближайшего коэффициент, равный 4 при выделении памяти текстуры.
...
Используйте формат сжатия первого блока (BC1) (либо DXGI\_ФОРМАТ\_BC1\_TYPELESS, DXGI\_ФОРМАТ\_BC1\_UNORM или DXGI\_BC1\_UNORM \_SRGB) для хранения данных о цвете три компонента с помощью 5: цвет 6:5 (5 бит красный, зеленый 6 бит, 5 бит синий). Это актуально, даже если данные также содержат 1-битовый альфа-канал. Допустим, текстура 4×4 использует самый крупный из возможных форматов данных. В этом случае формат BC1 уменьшает объем необходимой памяти с 48 байтов (16 цветов × 3 компонента/цвет × 1 байт/компонент) до 8 байтов.

Этот алгоритм работает в блоках текселей 4×4. Вместо сохранения 16 цветов, алгоритм сохраняет 2 цветов (цвет\_0 и цвет\_1) и 16 бит цветовых индексов (блоки a – p), как показано на следующей схеме.

![структура для сжатия bc1](images/d3d10-compression-bc1.png)

Индексы цвета (a–p) используются для поиска первоначальных цветов в таблице цветов. Таблица цветов содержит 4 цвета. Первые два цвета — цвет\_0 и цвет\_1 — минимальным и максимальным цвета. Другие два цвета, цвета\_2 и цвет\_3, представляют собой промежуточные цвета, вычисляется с помощью линейной интерполяции.
...
Этот алгоритм пригоден для данных, которые также содержит 1-битовый альфа-канал. Единственное отличие заключается в этот цвет\_3 имеет значение 0 (представляющее прозрачный цвет) и цвет\_2 — это линейная цвет\_0 и цвет\_1.
...
Используйте формат BC2 (либо DXGI\_ФОРМАТ\_BC2\_TYPELESS, DXGI\_ФОРМАТ\_BC2\_UNORM или DXGI\_BC2\_UNORM\_SRGB) для хранение данных, содержащий данные, цвет и альфа-канал с низкой согласование (использовать [BC3](#bc3) для строго согласованных альфа-канала данных). Формат BC2 хранит данные RGB как цвет 5:6:5 (5 битов на красный, 6 битов на зеленый, 5 битов на синий), а альфа-канал — как отдельное 4-битовое значение. Допустим, текстура 4×4 использует самый крупный из возможных форматов данных. В этом случае эта техника сжатия уменьшает объем необходимой памяти с 64 байтов (16 цветов × 4 компонента/цвет × 1 байт/компонент) до 16 байтов памяти.
...
Используйте формат BC3 (либо DXGI\_ФОРМАТ\_BC3\_TYPELESS, DXGI\_ФОРМАТ\_BC3\_UNORM или DXGI\_BC3\_UNORM\_SRGB) для хранение данных строго согласованных цвет (использовать [BC2](#bc2) с менее согласовано альфа-канала данных). Формат BC3 сохраняет данные цвета с использованием цвета 5:6:5 (5 битов красного, 6 битов зеленого, 5 битов синего) и альфа-данные с использованием одного байта. Допустим, текстура 4×4 использует самый крупный из возможных форматов данных. В этом случае эта техника сжатия уменьшает объем необходимой памяти с 64 байтов (16 цветов × 4 компонента/цвет × 1 байт/компонент) до 16 байтов памяти.
...
Этот алгоритм работает в блоках текселей 4×4. Вместо сохранения 16 альфа-значения, алгоритм сохраняет 2 ссылку альфа (альфа-канал\_0 и альфа-канал\_1) и 16 бит 3 цветовых индексов (альфа-версия через p), как показано на следующей схеме.

![структура для сжатия bc3](images/d3d10-compression-bc3.png)

Формат BC3 использует альфа-индексы (a–p) для поиска первоначальных цветов в таблице поиска, которая содержит 8 значений. Первые два значения — альфа-канал\_0 и альфа-канал\_1 — — это минимальное и максимальное значения; другие шесть промежуточные значения вычисляются с помощью линейной интерполяции.

Алгоритм определяет количество интерполированных альфа-значений, анализируя два эталонных альфа-значения. Если альфа-\_0 больше, чем альфа-канал\_1, то BC3 интерполирует 6 альфа-значения; в противном случае он выполняет интерполяцию 4. Если BC3 интерполирует только 4 альфа-значения, он задает два дополнительных альфа-значения (0 для полностью прозрачного и 255 для полностью непрозрачного). BC3 сжимает альфа-значения в области текселей 4×4, сохраняя битовый код, соответствующий интерполированным альфа-значениям, которые наиболее полно соответствуют исходному альфа-значению для заданного текселя.
...
Используйте формат BC4 для хранения данных об однокомпонентном цвете с использованием 8 битов каждого цвета. В результате повышения точности (по сравнению с [BC1](#bc1)), BC4 идеально подходит для хранения данных с плавающей запятой в диапазоне от \[от 0 до 1\] с помощью DXGI\_ФОРМАТ\_BC4\_UNORM формат и \[-1 до + 1\] с помощью DXGI\_ФОРМАТ\_BC4\_SNORM формат. Допустим, текстура 4×4 использует самый крупный из возможных форматов данных. В этом случае эта техника сжатия уменьшает объем необходимой памяти с 16 байтов (16 цветов × 1 компонент/цвет × 1 байт/компонент) до 8 байтов.

Этот алгоритм работает в блоках текселей 4×4. Вместо сохранения 16 цветов, алгоритм сохраняет 2 цветов (red\_0 и красный\_1) и 16 индексов 3-разрядный цвет (красный до red p), как показано на следующей схеме.

![структура для сжатия bc4](images/d3d10-compression-bc4.png)

Алгоритм использует 3-битовые индексы для поиска цветов в таблице цветов, которая содержит 8 элементов. Первые два цвета — красный\_0 и красный\_1 — минимальное и максимальное цвета. Этот алгоритм вычисляет оставшиеся цвета с использованием линейной интерполяции.

Алгоритм определяет количество интерполированных значений цвета, анализируя два эталонных значения. Если red\_0 больше, чем red\_1, то BC4 интерполирует 6 значений цвета; в противном случае он выполняет интерполяцию 4. Если BC4 интерполирует только 4 значения цвета, задается два дополнительных значения цвета (0.0f для полностью прозрачного и 1.0f для полностью непрозрачного). BC4 сжимает альфа-значения в области текселей 4×4, сохраняя битовый код, соответствующий интерполированным альфа-значениям, которые наиболее полно соответствуют исходному альфа-значению для заданного текселя.
...
DXGI\_ФОРМАТ\_BC4\_SNORM остается прежним, за исключением того, что данные кодируются в диапазоне SNORM и если 4 значения цвета интерполируются. Следующий пример кода показывает, как выполняется интерполяция однокомпонентных данных.
...
Используйте формат BC5 для хранения данных о двухкомпонентном цвете с использованием 8 битов каждого цвета. В результате повышения точности (по сравнению с [BC1](#bc1)), BC5 идеально подходит для хранения данных с плавающей запятой в диапазоне от \[от 0 до 1\] с помощью DXGI\_ФОРМАТ\_BC5\_UNORM формат и \[-1 до + 1\] с помощью DXGI\_ФОРМАТ\_BC5\_SNORM формат. Допустим, текстура 4×4 использует самый крупный из возможных форматов данных. В этом случае эта техника сжатия уменьшает объем необходимой памяти с 32 байтов (16 цветов × 2 компонента/цвет × 1 байт/компонент) до 16 байтов.

- [BC5\_UNORM](#bc5-unorm)
- [BC5\_SNORM](#bc5-snorm)

Этот алгоритм работает в блоках текселей 4×4. Вместо сохранения 16 цветов для обоих компонентов, алгоритм сохраняет 2 цветов для каждого компонента (красный\_0, красный\_1, зеленый\_0, а зеленый\_1) и 16 бит 3 цветовых индексов для каждого компонента (красный через p красной и зеленой до зеленый p), как показано на следующей схеме.

![структура для сжатия bc5](images/d3d10-compression-bc5.png)

Алгоритм использует 3-битовые индексы для поиска цветов в таблице цветов, которая содержит 8 элементов. Первые два цвета — красный\_0 и красный\_1 (или зеленый\_0 и зеленый\_1) — минимальное и максимальное цвета. Этот алгоритм вычисляет оставшиеся цвета с использованием линейной интерполяции.

Алгоритм определяет количество интерполированных значений цвета, анализируя два эталонных значения. Если red\_0 больше, чем red\_1, то BC5 интерполирует 6 значений цвета; в противном случае он выполняет интерполяцию 4. Если BC5 интерполирует только 4 значения цвета, он задает остальные два значения цвета равными 0.0f и 1.0f.
...
DXGI\_ФОРМАТ\_BC5\_SNORM остается прежним, за исключением того, что данные кодируются в диапазоне SNORM и при интерполяции значений данных, 4, две дополнительные значения - 1.0f и 1.0f. Следующий пример кода показывает, как выполняется интерполяция однокомпонентных данных. Вычисления для зеленых компонентов похожи.
...
Чтобы повторно интерпретировать f как тип u, используйте функцию [memcpy](https://docs.microsoft.com/cpp/c-runtime-library/reference/memcpy-wmemcpy):

```cpp
memcpy( &u, &f, sizeof( f ) ); // 'u' becomes equal to 0x3F800000.
```

В предыдущем примере повторной интерпретации базовое значение данных не меняется; [memcpy](https://docs.microsoft.com/cpp/c-runtime-library/reference/memcpy-wmemcpy) повторно интерпретирует число с плавающей запятой как целое число без знака.
