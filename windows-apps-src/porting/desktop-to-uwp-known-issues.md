...
Если вы разработчик, вы можете запретить установку своих упакованных приложений в версиях Windows, в которых нет этого обновления. Обратите внимание, что при этом ваше приложение не будет доступно для пользователей, которые еще не установили обновление. Чтобы ограничить доступность вашего приложения пользователям, которые установили обновления, внесите в файл AppxManifest.xml следующим образом:
...
Запустите **certutil** из командной строки в файле PFX и скопируйте поле *темы* в выходных данных.
...
Это может произойти, если пакет содержит двоичный файл, который имеет поврежденные сертификат. Вот некоторые причины почему это может произойти.

* Меню "Пуск" сертификата не в конце изображения.  

* Размер сертификата не положительным.

* Меню "Пуск" сертификат не является после `IMAGE_NT_HEADERS32` структуру для 32-разрядный исполняемый файл или после `IMAGE_NT_HEADERS64` структуру для 64-разрядный исполняемый файл.

* Указатель сертификата не были правильно выровнены WIN_CERTIFICATE структуры.

Чтобы найти файлы, содержащие поврежденных PE сертификата, откройте **командную**строку и задать переменную среды с именем `APPXSIP_LOG` значение 1.
...
Сведения о том, файлы, содержащие поврежденных PE сертификата будут отображаться в **Окне консоли**. Пример.
