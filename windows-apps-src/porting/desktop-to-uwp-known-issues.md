...
Если вы разработчик, вы можете запретить установку своих упакованных приложений в версиях Windows, в которых нет этого обновления. Обратите внимание, что при этом ваше приложение не будет доступно пользователям, которые еще не установили обновление. Чтобы ограничить доступность вашего приложения пользователям, которые установили обновления, внесите в файл AppxManifest.xml следующим образом:
...
* Меню "Пуск" сертификат не является после `IMAGE_NT_HEADERS32` структуру для 32-разрядных исполняемого файла или после `IMAGE_NT_HEADERS64` структуру для 64-разрядный исполняемый файл.
...
Сведения о файлах, которые содержат поврежденных PE сертификат будет отображаться в **Окне консоли**. Пример.
