...
Рассмотрим упаковка вручную, если установить приложение с помощью команды xcopy или вы знакомы с изменениями, которые ваш установщик приложения вносит в систему и требуется более тщательный контроль над процессом.
...
Элемент [Properties](https://docs.microsoft.com/uwp/schemas/appxpackage/appxmanifestschema/element-properties) имеет три обязательных дочерних элемента. Вот пример узла **Properties** с замещающим текстом для элементов. **DisplayName** — имя приложения, которое вы резервируете в магазине для приложения загруженные в магазин.
...
Для классических приложений, которые вы создать пакет для, вам необходимо добавить ``runFullTrust`` возможность.
