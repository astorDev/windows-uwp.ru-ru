...
title: Упаковка приложения вручную (мост)
ms.author: normesta
ms.date: 05/18/2018
ms.topic: article
ms.prod: windows
...
ms.locfileid: "4268112"
---
# <a name="package-a-desktop-application-manually"></a>Вручную упаковки классического приложения
...
```
> [!NOTE]
> Если вы зарезервировали имя приложения в магазине Windows, можно получить имя и издателя с помощью информационной панели центра разработки для Windows. Если вы планируете неопубликованного приложения на другие системы, вы можете предоставить для этих атрибутов собственные имена, до тех пор, пока имя издателя, выбранного вами соответствует имени в сертификате, который используется для подписания приложения.

### <a name="properties"></a>Свойства

Элемент [Properties](https://docs.microsoft.com/uwp/schemas/appxpackage/appxmanifestschema/element-properties) имеет три обязательных дочерних элемента. Вот пример узла **Properties** с замещающим текстом для элементов. **DisplayName** — это имя приложения, которое вы резервируете в магазине, для приложений, которые отправляются в магазин.
...
> [!NOTE]
> Упакованное приложение всегда выполняется от имени текущего пользователя, а диск, который Установка упакованного приложения на должен быть отформатирован в NTFS.
