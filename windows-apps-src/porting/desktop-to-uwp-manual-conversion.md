...
Для классических приложений, которые можно создать пакет для необходимо добавить ``runFullTrust`` возможность.
...
Для классических приложений, которые можно создать пакет, ``EntryPoint`` атрибут элемента Application всегда имеет ``Windows.FullTrustApplication``.
...
Можно запустить приложение для проверки, локально не обязательно получать сертификат и подписывать его. Просто запустите этот командлет PowerShell:
...
> [!NOTE]
> Упакованное приложение всегда выполняется от имени текущего пользователя, а диск, который Установка упакованное приложение на должен быть отформатирован в NTFS.
