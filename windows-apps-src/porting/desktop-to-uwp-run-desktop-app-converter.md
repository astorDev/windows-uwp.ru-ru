...
: heavy_check_mark: Проверка приложения для упакованного приложения и требования к Microsoft Store.
...
``Sign`` Параметр создает сертификат и подписывает приложение с его помощью. Для запуска приложения вам придется установить этот сертификат. Дополнительные сведения см. в разделе [Запуск упакованного приложения](#run-app) данного руководства.
...
Другой способ — подписать приложение с помощью сертификата. Если вы используете ```sign``` параметра, Desktop App Converter будет создать для вас, а затем подпишите приложение с его помощью. Этот файл называется **auto-generated.cer** и находится в корневой папке вашего упакованного приложения.
