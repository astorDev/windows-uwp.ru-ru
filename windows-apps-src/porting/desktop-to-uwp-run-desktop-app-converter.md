...
Desktop App Converter (DAC) создает пакеты для классических приложений для интеграции с новейшие возможности Windows, включая распространения и обслуживания через Microsoft Store. Сюда входят приложения Win32 и приложения, созданные с помощью .NET 4.6.1.

![Значок DAC](images/desktop-to-uwp/dac.png)

Хотя в названии этого средства содержится термин "Преобразователь", на самом деле оно не преобразует ваше приложение. Ваше приложение остается неизменным. Однако это средство создает пакет приложения для Windows с идентификатором пакета. Также преобразователь обеспечивает возможность осуществлять вызов разнообразных API WinRT.
...
Вы можете пропустить к следующему разделу Если ваше приложение не имеет установщика.
...
В этом примере используется ``Installer`` параметр, чтобы указать корневую папку файлы приложения.
...
``Sign`` Параметр создает сертификат, а затем подписывает ваше приложение с ним. Для запуска приложения вам придется установить этот сертификат. Дополнительные сведения см. в разделе [Запуск упакованного приложения](#run-app) данного руководства.
