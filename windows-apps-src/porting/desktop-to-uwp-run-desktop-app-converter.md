...
Desktop App Converter (DAC) создает пакеты для классических приложений для интеграции с новейшие возможности Windows, в том числе распространения и обслуживания через Microsoft Store. Сюда входят приложения Win32 и приложения, созданные с помощью .NET 4.6.1.
...
Можно перейти к следующему разделу Если приложение не имеет установщика.
...
* [Упаковка приложения, подписание и подготовка для отправки в магазин](#optional-parameters)
...
> [!IMPORTANT]
> Существует два важных аспекта, о которых следует помнить. Во-первых, убедитесь, что ваш установщик находится в независимой папке и что в ней размещаются только файлы, связанные с этим установщиком. Преобразователь копирует все содержимое этой папки в изолированную среду Windows. <br> Во-вторых Если центр партнеров пакету назначен идентификатор, который начинается с цифры, убедитесь, что можно также передать в параметре <i>- AppId</i> и используется в качестве значения этого параметра только суффикс строки (после точки-разделителя).  
...
```
>[!IMPORTANT]
>Если центр партнеров пакету назначен идентификатор, который начинается с цифры, убедитесь, что можно также передать в параметре <i>- AppId</i> и используется в качестве значения этого параметра только суффикс строки (после точки-разделителя).
...
В этом примере используется ``Installer`` параметра для указания в корневой каталог файлов приложения.
...
Этот пример похож на первый одним исключением: Здесь описывается, как можно подписать приложение для локального тестирования и затем проверить приложения для упакованного приложения и требования к Microsoft Store.
...
Проверить можно приложения с помощью ``Verify`` параметр.
...
|-MakeMSIX [&lt;параметр-переключатель&gt;]  |Необязательно |Этот переключатель, он передает этот сценарий для упаковки выходные данные виде пакета MSIX. |
|<a id="identity-params" /><strong>Параметры идентификатора пакета</strong>||
|-PackageName &lt;String&gt; |Обязателен |Имя пакета вашего универсального приложения для Windows. Если центр партнеров пакету назначен идентификатор, который начинается с цифры, убедитесь, что можно также передать в параметре <i>- AppId</i> и используется в качестве значения этого параметра только суффикс строки (после точки-разделителя). |
...
|-AppExecutable &lt;String&gt; |Необязательно |Имя основного исполняемого файла вашего приложения (например, "MyApp.exe"). Этот параметр является обязательным для преобразования приложений, не имеющих установщика. |
|-AppFileTypes &lt;String&gt;|Необязательно |Список типов файлов, с которыми будет связано приложение. Типы файлов должны отделяться друг от друга запятыми. Пример использования: -AppFileTypes "'.md', '.markdown'".|
|-AppId &lt;String&gt; |Необязательный |Указывает значение, которое нужно задать идентификатору приложения в манифесте пакета приложения для Windows. Если параметр не указан, ему будет задано значение, передаваемое для *PackageName*. Во многих случаях достаточно использовать *PackageName*. Тем не менее если центр партнеров пакету назначен идентификатор, который начинается с цифры, убедитесь, что можно также передать в параметре <i>- AppId</i> и используется в качестве значения этого параметра только суффикс строки (после точки-разделителя). |
|-AppDisplayName &lt;String&gt;  |Необязательно |Указывает значение, которое нужно задать отображаемому имени приложения в манифесте пакета приложения для Windows. Если параметр не указан, ему будет задано значение, передаваемое для *PackageName*. |
|-AppDescription &lt;String&gt; |Необязательно |Указывает значение, которое нужно задать описанию приложения в манифесте пакета приложения для Windows. Если параметр не указан, ему будет задано значение, передаваемое для *PackageName*.|
|-PackageDisplayName &lt;String&gt; |Необязательно |Указывает значение, которое нужно задать отображаемому имени пакета в манифесте пакета приложения для Windows. Если параметр не указан, ему будет задано значение, передаваемое для *PackageName*. |
|-PackagePublisherDisplayName &lt;String&gt; |Необязательно |Указывает значение, которое нужно задать отображаемому имени издателя пакета в манифесте пакета приложения для Windows. Если параметр не указан, ему будет задано значение, передаваемое для *Publisher*. |
...
| -Sign [&lt;параметр-переключатель&gt;] |Необязательно |Сообщает этому сценарию, что требуется подписать выходной пакет приложения для Windows, используя созданный сертификат, для тестирования. Этот параметр следует использовать вместе с параметром ```-MakeAppx```. |
...
| -Verify [&lt;параметр-переключатель&gt;] |Необязательно |Этот переключатель, он передает DAC Команду проверить пакет приложения на соответствие упакованного приложения и требования к Microsoft Store. Результат — отчет о проверке "VerifyReport.xml", который лучше всего отображается в браузере. Этот параметр следует использовать вместе с параметром `-MakeAppx`. |
...
Другой способ — подписать приложение с помощью сертификата. Если вы используете ```sign``` параметр, Desktop App Converter будет создание сертификата, а затем подпишите приложение с ним. Этот файл называется **auto-generated.cer** и находится в корневой папке вашего упакованного приложения.
