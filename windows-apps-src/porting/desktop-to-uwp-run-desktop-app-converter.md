...
```
>[!IMPORTANT]
>Если центр партнеров пакету назначен идентификатор, который начинается с цифры, убедитесь, что вы также передавать в параметре <i>- AppId</i> и использовать в качестве значения этого параметра только суффикс строки (после точки-разделителя).
