...
ms.locfileid: "66358812"
---
# <a name="getting-started-getting-around-in-visual-studio"></a>Приступая к работе: Введение в Visual Studio
...
В отличие от Xcode, который скрывает информацию о макете и привязке в XIB-файле или файле раскадровки, Visual Studio предлагает вам изменять файлы XAML, используемые для хранения этих сведений, с помощью богатого, редактируемого, декларативного языка, похожего на язык XML. Подробнее о языке XAML см. в разделе [Обзор языка XAML](https://docs.microsoft.com/windows/uwp/xaml-platform/xaml-overview). Пока вам достаточно знать, что все, что отображается в области **Конструктор**, определяется в области **XAML**. Область **XAML** позволяет точнее управлять параметрами там, где это необходимо, а также ускорить разработку кода пользовательского интерфейса вручную. Пока же мы сосредоточимся на области **Конструктор** и окне **Свойства**.
...
Чтобы добавить код события нажатия кнопки, сначала выберите кнопку в области **Конструктор**. Далее нажмите кнопку с изображением молнии и дважды щелкните на пустом поле рядом с именем **Click**. Visual Studio добавляет событие «myButton\_нажмите кнопку» для **щелкните** , а затем добавляет и отображает соответствующий обработчик событий в файле MainPage.xaml.cs следующим образом.
...
Добавим код в событие нажатия кнопки. Чтобы сделать это, выберите файл MainPage.xaml.cs и добавьте следующий код к myButton\_обработчика события щелчка.
...
[Приступая к работе: Стандартные элементы управления](getting-started-common-controls.md)
