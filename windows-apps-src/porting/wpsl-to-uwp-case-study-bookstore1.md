...
description: В этом разделе представлен пример использования переноса очень простое приложение Windows Phone Silverlight в приложении 10 универсальных Windows платформы Windows (UWP).
title: Windows Phone Silverlight для универсальной платформы Windows практическом Bookstore1
ms.date: 02/08/2017
ms.topic: article
...
**Примечание**    при открытии Bookstore1Universal\_10 в Visual Studio, если вы видите сообщение «Требуется Visual Studio обновление», затем выполните действия для выбора из целевой платформы управление версиями в [ TargetPlatformVersion](wpsl-to-uwp-troubleshooting.md).

## <a name="downloads"></a>Загрузки

[Скачать Windows Phone Bookstore1WPSL8 приложение Silverlight](https://go.microsoft.com/fwlink/?linkid=517053).
...
Создание нового проекта в Visual Studio, копирование в него файлов Bookstore1WPSL8 и подключение скопированных файлов к новому проекту не займут много времени. Начните с создания нового проекта пустого приложения (Windows Universal). Назовите его Bookstore1Universal\_10. Они представляют собой файлы, чтобы скопировать из Bookstore1WPSL8 Bookstore1Universal\_10.
...
-   Изменить значение, возвращенное **Bookstore1Universal\_10.BookstoreViewModel.AppName** свойство от «BOOKSTORE1WPSL8» до «BOOKSTORE1UNIVERSAL».
...
По умолчанию все ориентации поддерживаются. Приложения Windows Phone Silverlight явно ограничивает сам только для книжной ориентации, для, так элементы задолженности \#1 и \#, перейдя в манифест пакета приложения в новом проекте и проверка окупилась 2 **книжной**под **поддерживаемые ориентации**.

Для этого приложения элемент \#3 не долг, так как в строке состояния (прежнее название панели задач) отображается по умолчанию. Для элементов \#4 и \#5, нам нужно найти четыре Windows платформы Универсальной **TextBlock** стили, которые соответствуют стили Windows Phone Silverlight, которые мы использовали. Можно запустить приложение Silverlight Windows Phone в эмуляторе и сравнить его рядом с приведенным в [текст](wpsl-to-uwp-porting-xaml-and-ui.md) раздел. Из этого и просмотре свойства стилей системы Windows Phone Silverlight мы можем сделать в этой таблице.
