...
Если вы используете экземпляры настраиваемых типов в представлениях (возможно, экземпляр модели представления или преобразователь значений), в вашей разметке XAML будут присутствовать объявления префикса пространства имен. Их синтаксис различен в WindowsPhone Silverlight и UWP. Вот несколько примеров:
...
Так как Windows10 приложения могут работать на потенциально широкий спектр устройств, каждый из которых имеет свой размер экрана и разрешение — вам потребуется выходит за пределы минимальных действий для переноса приложения и вам потребуется адаптировать ваш пользовательский Интерфейс ему наилучшего вида на этих устройствах. Вы можете использовать адаптивную функцию "Диспетчер визуальных состояний", чтобы динамически определять размер окна и изменять макет. Пример того, как это сделать, представлен в разделе [Адаптивный пользовательский интерфейс](wpsl-to-uwp-case-study-bookstore2.md) в примере Bookstore2.
...
В приложении WindowsPhone Silverlight прием — для использования альфа-маска (в формате растрового изображения) в качестве **OpacityMask** **прямоугольник** , заполненный с помощью кисти переднего плана:
...
WindowsPhone Silverlight и приложениями Windows10 отличаются тем, как они абстрагируют размер и расположение элементов пользовательского интерфейса от физических размеров и разрешения устройств. Приложение WindowsPhone Silverlight использует пиксели представления для этого. В Windows10 принцип пикселей представления был доработан эффективных пикселей. Вот описание этого термина, его значения и преимуществ.
...
Для приложения WindowsPhone Silverlight все экраны телефона являются строго 480 пикселей представления в ширину, без исключений, независимо от того, сколько физических пикселей в экране, или от плотности его пикселей или физического размера. Это означает, что элемент **изображения** с помощью `Width="48"` будет составлять ровно одну десятую часть ширины экрана любого телефона, которое может выполняться приложение WindowsPhone Silverlight.
...
Поскольку цифра 480 раньше была фиксированной шириной в представлении пикселей для экрана телефона и это значение теперь обычно меньше в эффективных пикселях, основным правилом является умножение любого размера в вашем WindowsPhone Silverlight на коэффициент 0,8.
