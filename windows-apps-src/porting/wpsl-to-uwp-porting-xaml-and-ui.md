...
Иногда правка императивного кода требует всего лишь изменения типа параметров. В других случаях необходимо будет использовать API-интерфейсы UWP вместо 8.x приложения .NET API-интерфейсы для среды выполнения Windows. Для идентификации поддерживаются какие интерфейсы API, используется оставшаяся часть переноса руководство в сочетании с [Общие сведения о приложениях 8.x .NET для среды выполнения Windows](https://msdn.microsoft.com/library/windows/apps/xaml/br230302.aspx) и [Справочник по среды выполнения Windows](https://msdn.microsoft.com/library/windows/apps/br211377).
...
| Panorama | Элемент управления Windows Phone Silverlight Панорама сопоставляет правила и [рекомендации для сервера-концентратора элементов управления в приложениях 8.x среды выполнения Windows](https://msdn.microsoft.com/library/windows/apps/dn449149) для элемента управления сервера-концентратора. <br/> Обратите внимание: элемент управления Panorama окружен текстом от последнего раздела до первого, а его фоновое изображение перемещается в параллаксе относительно разделам. Разделы [Hub](https://msdn.microsoft.com/library/windows/apps/dn251843) не создают обтекание текстом, и параллакс не используется. |
...
##  <a name="design-language-in-windows-10"></a>Язык дизайна в Windows10
...
Более общим способом, который подходит в случае различия между оформлением растрового изображения в светлой и темной теме, является использование двух ресурсов изображений—одного с темным передним планом (для светлой темы) и второго со светлым передним планом (для темной темы). Дополнительные сведения о том, как имя этого набора средств растровое изображение [варьировать ресурсов для языка, масштаба и других классификаторы](../app-resources/tailor-resources-lang-scale-contrast.md)см. После присвоения правильного имени набору файлов изображений вы можете ссылаться на них в общем с помощью их имени корневого элемента, например:
...
Стили системы TextBlock для приложений для Windows10
