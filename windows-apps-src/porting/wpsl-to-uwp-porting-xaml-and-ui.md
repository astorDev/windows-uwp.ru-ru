...
Более общий подход к поиску типа UWP, который соответствует типу WindowsPhone Silverlight можно ссылаться сопоставление [пространств имен и класса](wpsl-to-uwp-namespace-and-class-mappings.md).
...
Так как Windows10 приложения могут работать на потенциально широкий спектр устройств, каждый из которых имеет свой размер экрана и разрешение — вам потребуется выходит за пределы минимальных действий для переноса приложения и необходимо адаптировать пользовательский Интерфейс, чтобы ему наилучшего вида на этих устройствах. Вы можете использовать адаптивную функцию "Диспетчер визуальных состояний", чтобы динамически определять размер окна и изменять макет. Пример того, как это сделать, представлен в разделе [Адаптивный пользовательский интерфейс](wpsl-to-uwp-case-study-bookstore2.md) в примере Bookstore2.
...
В приложении WindowsPhone Silverlight все экраны телефона являются строго 480 пикселей представления в ширину, без исключения, независимо от того, сколько физических пикселей в экране, или от плотности его пикселей или физического размера. Это означает, что элемент **изображения** с `Width="48"` будет составлять ровно одну десятую часть ширины экрана любого телефона, можно запустить приложение WindowsPhone Silverlight.
