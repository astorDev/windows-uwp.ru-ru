...
Чтобы создать платформа поддержки пакетов, мы используются технологии [задержек](https://www.microsoft.com/en-us/research/project/detours) с открытым исходным кодом, разработанная по Microsoft Research (MSR) и помогает с перенаправлением API и перехвата.

Эту платформу — с открытым исходным кодом прост, и его можно использовать для решения проблем приложения быстро. Он также предоставляет следующие возможности проконсультироваться с сообществом по всему миру и в создании элементов поверх инвестиций других пользователей.
...
Когда пользователь запускает приложение, средство запуска оболочки — это первый исполняемый файл, запускающий. Считывает файл конфигурации и вставляет fix(s) среды выполнения и диспетчеру среды DLL в процессе приложения.

![Внедрение DLL Framework поддержки пакета](images/desktop-to-uwp/package-support-framework-2.png)

Диспетчеру среды применяется исправление, когда он необходим для выполнения внутри контейнера MSIX приложения.
