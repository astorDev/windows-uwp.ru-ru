...
ms.locfileid: "4531716"
---
# <a name="package-desktop-applications-desktop-bridge"></a>Упаковка классических приложений (моста для классических приложений)

Перенесите существующие Классические приложения и добавьте современных способов взаимодействия для пользователей Windows 10. После этого вы сможете охватить международные рынки путем распространения приложения через Microsoft Store. Приложение может дохода гораздо более простыми способами с помощью возможностей, в магазине. Конечно, использовать Store не обязательно. Вы можете использовать существующие каналы.
...
:heavy_check_mark: **Автоматическое обновление и лицензирование**. Приложения могут участвовать в Microsoft Store лицензирования и автоматического обновления средства. Автоматическое обновление— очень надежный и эффективный механизм, поскольку загружаются только измененные части файлов.
...
Во-первых Подготовьте свое приложение затем обрабатывает все проблемы, применимые к вашему приложению, до создания пакета приложения для Windows для сети, просматривая статьи [подготовки для упаковки классического приложения](desktop-to-uwp-prepare.md). Не может потребоваться пакет вносить никакие изменения в приложение. Однако существует несколько ситуаций, которые может потребоваться его предварительная оптимизация создания пакета для приложения.
...
Desktop App Converter переводит действия установщика и виртуальной файловой системы реестра, который будет использовать Упакованная версия вашего приложения. Desktop App Converter также выполняет некоторые дополнительные действия. Вот некоторые из них:
...
: heavy_check_mark: Проверка вашего приложения, упакованного классического приложения и требования к Microsoft Store.

— Это еще одна причина использовать Desktop App Converter, если сопровождение приложение в среде разработки, отличной от Visual Studio. Можно использовать Desktop App Converter, даже если приложение не имеет установщика.
...
Решение для упаковки Raynet, [RayPack Studio](https://raynet.de/Raynet-Products/RayPackStudio)поддерживает создание пакетов для классических приложений как один из нескольких возможных результатов эффективный и удобный для настройки преобразования и распаковки платформы.
...
После упаковки приложения вы можете улучшить его с помощью таких функций, как живые плитки и push-уведомления. Некоторые из этих возможностей могут значительно повысить уровень вовлечения вашего приложения, и они стоят немного времени на их добавление. Для некоторых возможностей требуется добавить небольшой код.

См. [Улучшение классического приложения для Windows 10](desktop-to-uwp-enhance.md).

## <a name="extend"></a>Расширение

Некоторые возможности Windows 10, такие как страницы с подходящим для сенсорного ввода интерфейсом, должны выполняться внутри современного контейнера приложения. Как правило, сначала необходимо определить, можно ли добавить ваше взаимодействие путем [усовершенствования](desktop-to-uwp-enhance.md) существующего классического приложения с помощью API-интерфейсов UWP. Если необходимо использовать компонент UWP для достижения результата, можно добавить в решение проект UWP и использования службы приложений для обмена данными между классического приложения и компоненты UWP.
...
| [Вручную упаковки классического приложения](desktop-to-uwp-manual-conversion.md) | Создание пакета приложения и манифеста для него вручную. |
