...
Мост для классических приложений представляет собой инфраструктуру, которую мы встроили в платформу, позволяющую эффективно распространять классические приложения и игры Win32, Windows Forms и WPF с помощью современных пакетов приложений для Windows.
...
:heavy_check_mark: **Автоматическое обновление и лицензирование**. Ваше приложение может взаимодействовать со встроенными средствами лицензирования и автоматического обновления Microsoft Store. Автоматическое обновление— очень надежный и эффективный механизм, поскольку загружаются только измененные части файлов.
...
Desktop App Converter переводит действия установщика в виртуальную файловую систему и виртуальный реестр, которые будет использовать упакованная версия вашего приложения. Desktop App Converter также выполняет некоторые дополнительные действия. Вот некоторые из них:
...
Если сопровождение приложение осуществляется в среде разработки, отличной от Visual Studio, это еще одна причина использовать Desktop App Converter. Можно использовать Desktop App Converter, даже если приложение не имеет установщика.

См. раздел [Упаковка приложения с помощью Desktop App Converter (мост для классических приложений)](desktop-to-uwp-run-desktop-app-converter.md)
...
:heavy_check_mark: Удобное назначение приложению идентификатора из имени, которое уже зарезервировано в информационной панели Центра разработки для Windows.

См. раздел [Упаковка приложения .NET с помощью Visual Studio (мост для классических приложений)](desktop-to-uwp-packaging-dot-net.md)

### <a name="third-party-installer"></a>Сторонний установщик

 Теперь мост для классических приложений поддерживает несколько известных продуктов сторонних компаний. Их можно использовать для создания установщиков MSI или пакетов упакованного приложения в несколько щелчков мышью. Поскольку мы не создаем документацию по использованию этих средств, посетите их веб-сайты, чтобы получить дополнительные сведения.
...
Install**Aware** имеет [репутацию](https://www.installaware.com/press-room.htm) быстрого принятия инноваций корпорации Майкрософт и обеспечивает создание [пакетов приложений Windows (мост для классических приложений)](https://www.installaware.com/appx-builder.htm), App-V (виртуализацию приложений), MSI (Windows Installer) и пакетов EXE (Native Code) из единого источника.
...
См. раздел [Упаковка приложения вручную (мост для классических приложений)](desktop-to-uwp-manual-conversion.md).
...
Чтобы максимально повысить шансы вашего приложения на публикацию в Microsoft Store или получение статуса [Сертифицировано для Windows](http://go.microsoft.com/fwlink/p/?LinkID=309666), проверьте и протестируйте приложение локально, прежде чем отправлять его на сертификацию.
...
Если вы используете Visual Studio, можно проверить ваше приложение из мастера **Создание пакетов приложения**. См. раздел [Создания файла пакета отправки приложения](../packaging/packaging-uwp-apps.md#create-an-app-package-upload-file).
...
| [Как работает мост для классических приложений (мост для классических приложений)](desktop-to-uwp-behind-the-scenes.md) | Узнайте, как устроен мост для классических приложений. |
| [Известные проблемы (мост для классических приложений)](desktop-to-uwp-known-issues.md) | Перечень известных проблем, касающихся моста для классических приложений. |
