...
description: В этом разделе представлен пример использования переноса работает головоломки peer-to-peer игр WinRT 8.1 пример приложения, чтобы приложение 10 универсальных Windows платформы Windows (UWP).
...
В этом разделе представлен пример использования переноса работает головоломки peer-to-peer игр WinRT 8.1 пример приложения, чтобы приложение 10 универсальных Windows платформы Windows (UWP).

Приложение Uwp 8.1, которое создает две версии одного приложения: один пакет приложения для Windows 8.1 и другого пакета приложения для Windows Phone 8.1. Версия QuizGame для WinRT 8.1 использует проект универсального приложения для Windows, но выбирает другой подход и выполняет сборку функционально отдельных приложений для двух платформ. Пакет приложения Windows 8.1 играет роль узла сеансов, игры тест, а пакет приложения Windows Phone 8.1 играет роль клиента к узлу. Две части игры-опроса связаны через одноранговую сеть.

Имеет смысл настроить эти части для использования на ПК и телефоне соответственно. Но было бы удобнее не запускать и узел, и клиент на каждом устройстве, не так ли? В этом случае исследования, мы будем порт оба приложения для Windows 10, где они будут каждой сборки в пакет одним приложением, пользователи могут устанавливать на широкий спектр устройств.
...
**Примечание**  в этом примере предполагается, что сеть настроена для отправки и получения настраиваемых UDP группе пакетов многоадресной рассылки (Большинство домашних сетей, несмотря на то, что может оказаться рабочей сети). В примере также отправляются и принимаются пакеты TCP.

 

**Примечание**    при открытии QuizGame10 в Visual Studio, если вы видите сообщение «Требуется Visual Studio обновление», затем выполните действия, описанные в [TargetPlatformVersion](w8x-to-uwp-troubleshooting.md).
...
-   QuizGame.Windows. Это проект, который создает пакет приложения для узла приложения, которая нацелена на Windows 8.1.
...
Исходя из этих вариантов, мы будет переносить QuizGame.Windows в новый проект Windows 10 с именем QuizGameHost. И мы будем переносить QuizGame.WindowsPhone в новый проект Windows 10 с именем QuizGameClient. Эти проекты предназначены для семейства универсальных устройств, поэтому они будут работать на любом устройстве. А исходные файлы QuizGame.Shared и пр. мы оставим в той папке, где они находятся, и будем взаимодействовать с этими общими файлами в двух новых проектах. Как и раньше, мы будем использовать одно решение и назовем его QuizGame10.
...
-   Скопируйте папки общего, модели, представления и модель представления из \\QuizGame.Shared\\ для \\QuizGame10\\.
...
-   Скопируйте файл App.xaml из \\QuizGame.Shared\\ для \\QuizGameHost\\ и изменить пространство имен для QuizGameHost.
...
-   В **свойства** &gt; **построения** &gt; **символы условной компиляции**, добавьте LOCALTESTMODEON.
...
-   Скопируйте файл App.xaml из \\QuizGame.Shared\\ для \\QuizGameClient\\ и изменить пространство имен для QuizGameClient.
-   В файле package.appxmanifest, измените имя возможности "internetClient" на "internetClientServer".
...
Приложения для QuizGameHost Windows 10 выглядит нормально, когда приложение выполняется в окне расширенных (которая возможна только на устройствах с большим экраном). Когда окно приложения узкое (это часто случается на небольших устройствах, но может также произойти и на большом устройстве), интерфейс сжимается настолько, что становится нечитабельным.

Можно использовать функцию адаптивной Диспетчер визуальных состояний исправить это, как мы объяснили в [практический пример: Bookstore2](w8x-to-uwp-case-study-bookstore2.md). Сначала задайте свойства визуальных элементов, так чтобы по умолчанию пользовательский интерфейс помещался в узком состоянии. Все эти изменения выполняются на \\представление\\HostView.xaml.
...
-   В `OptionContentControlStyle` измените значение **FontSize** метода задания на "20". Этот и предыдущий шаги обеспечивают возможность использования набора шрифтов, которые будут хорошо смотреться на всех устройствах. Это гораздо более гибкие размеры «30», мы использовали для приложения Windows 8.1.
...
Вы заметите, что в Windows 10, кнопки не имеют одного касания-целевого объекта заполнение в их шаблон. Это можно исправить двумя небольшими изменениями. Сначала добавьте эту разметку в файл app.xaml в QuizGameHost и в QuizGameClient.
...
А во-вторых, добавьте этот метод задания для `OptionButtonStyle` в \\представление\\ClientView.xaml.
...
Приложение, которое мы переносили в данном примере, было относительно сложным, поскольку включало несколько проектов, библиотеку классов и достаточно большой объем кода и пользовательского интерфейса. Даже так, перенос его был достаточно простым. Некоторые из простота переноса непосредственно от сходство между платформу разработки Windows 10 и платформы Windows 8.1 и Windows Phone 8.1. А частично — это заслуга способа разработки начального приложения, который обеспечил возможность отделения моделей, моделей представлений и представлений.
