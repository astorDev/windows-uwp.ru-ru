...
description: В этом разделе представлен практический пример переноса функционирующего однорангового для — игры WinRT 8.1 пример приложение в приложение универсальной платформы Windows (UWP) Windows10.
...
В этом разделе представлен практический переноса функционирующего для однорангового — игры WinRT 8.1 пример приложение в приложение Windows10Universal платформы Windows (UWP).

— Это универсальное приложение версии 8.1 создает две версии одного приложения: один пакет приложения для Windows8.1 и пакет другого приложения для Windows Phone 8.1. Версия QuizGame для WinRT8.1 использует проект универсального приложения для Windows, но выбирает другой подход и выполняет сборку функционально отдельных приложений для двух платформ. Пакет приложения Windows8.1 служит узлом для сеанса игры-опроса, а пакет приложения Windows Phone 8.1 играет роль клиента узла. Две части игры-опроса связаны через одноранговую сеть.

Имеет смысл настроить эти части для использования на ПК и телефоне соответственно. Но было бы удобнее не запускать и узел, и клиент на каждом устройстве, не так ли? В этом примере мы перенесем оба приложения в Windows10, где они будут собраны в один пакет приложения, пользователи могут установить на множестве различных устройств.
...
**Примечание**в этом примере предполагается, что ваша сеть настроена для отправки и получения настраиваемых UDP пакетов многоадресной рассылки группы (Большинство домашних сетей, несмотря на то, что ваша сеть может оказаться). В примере также отправляются и принимаются пакеты TCP.
...
[Скачать универсальное приложение QuizGame для версии 8.1](https://go.microsoft.com/fwlink/?linkid=532953). Это начальное состояние приложения до переноса. 

[Скачать приложение QuizGame10 для Windows10](https://go.microsoft.com/fwlink/?linkid=532954). Это состояние приложения сразу после переноса. 
...
-   QuizGame.Windows. Это проект создает пакет приложения для основного приложения, которое будет работать в Windows8.1.
...
-   В этом решении создайте новый проект библиотеки классов Windows10 (**Новый проект** &gt; **Универсальные** &gt; **Библиотека классов (Windows Universal)**) и назовите его P2PHelper.
...
-   В `OptionContentControlStyle` измените значение **FontSize** метода задания на "20". Этот и предыдущий шаги обеспечивают возможность использования набора шрифтов, которые будут хорошо смотреться на всех устройствах. Эти размеры гораздо более гибкие чем размер «30», мы использовали Windows8.1 приложения.
...
Приложение, которое мы переносили в данном примере, было относительно сложным, поскольку включало несколько проектов, библиотеку классов и достаточно большой объем кода и пользовательского интерфейса. Даже так, перенос его был достаточно простым. Частично простоту переноса непосредственно от сходство между платформой разработки Windows10 и платформы Windows8.1 и Windows Phone 8.1. А частично— это заслуга способа разработки начального приложения, который обеспечил возможность отделения моделей, моделей представлений и представлений.
