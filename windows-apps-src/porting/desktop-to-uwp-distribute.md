...
title: Публикация упакованного классического приложения в магазине Windows или загрузите его на одно или несколько устройств.
ms.author: normesta
ms.date: 05/18/2018
ms.topic: article
ms.prod: windows
...
ms.locfileid: "4533505"
---
# <a name="distribute-a-packaged-desktop-application"></a>Распространение упакованного классического приложения

Публикация упакованного классического приложения в магазине Windows или загрузите его на одно или несколько устройств.  
...
Публикация приложения в это хранилище для более широкую аудиторию. Кроме того корпоративные клиенты могут приобрести приложения, чтобы распространять его внутри своих организаций через [Магазин Майкрософт для бизнеса](https://www.microsoft.com/business-store).
...
Для распространения приложения на других устройствах без его размещения в магазине, необходимо получить сертификат, подписать приложение с помощью этого сертификата, а затем загружать неопубликованные приложения на этих устройствах.
...
Если вы собираетесь распространять приложения на устройствах под управлением Windows 10 S, приложение должно быть подписано Microsoft Store, поэтому вам необходимо пройти процесс отправки в магазин перед распространением приложения на этих устройствах.
...
* Сделать ваше упакованное приложение открывать определенные типы файлов по умолчанию.
...
Если вы собираетесь добавить код, который переносит пользовательские данные, лучше выполнить только при первом запуске приложения. Прежде чем переносить данные, отобразите пользователю диалоговое окно, в котором сообщите о том, что происходит, почему этим рекомендуется воспользоваться и что случится с их имеющимися данными.
