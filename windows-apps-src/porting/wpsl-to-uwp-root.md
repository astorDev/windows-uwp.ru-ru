---
description: Если вы — разработчик с приложением WindowsPhone Silverlight, вы можете достичь использовать свои навыки и исходный код при переходе в Windows10.
...
**Примечание**Windows10 поддерживает значительно большую часть .NET Framework, чем приложение магазина Windows Phone. Например Windows10 имеет несколько System.ServiceModel.\* пространств имен, а также System.Net, System.Net.NetworkInformation и пространства имен System.Net.Sockets. Таким образом теперь — это самое время перенести WindowsPhone Silverlight и код .NET просто скомпилировать и работать на новой платформе. См. раздел [Сопоставление пространства имен и класса](wpsl-to-uwp-namespace-and-class-mappings.md).
Еще одна причина перекомпилировать существующий исходный код .NET в приложение Windows10 —, что вы сможете воспользоваться возможностями .NET Native, что это технология вперед-компиляции, которая преобразует MSIL изначально встроенный готовый к запуску машинный код. Приложения .NET Native запускаются быстрее, используют меньше памяти и меньше заряда батареи, чем аналоги MSIL.
...
| Раздел| Описание|
|------|------------| 
| [Сопоставление пространства имен и класса](wpsl-to-uwp-namespace-and-class-mappings.md) | Этот раздел содержит подробно описывается сопоставление API-интерфейсы Silverlight WindowsPhone в их эквиваленты в UWP. |
| [Перенос проекта](wpsl-to-uwp-porting-to-a-uwp-project.md) | Начните процесс переноса с создания нового Windows10 проекта в Visual Studio и скопируйте в него свои файлы. |
