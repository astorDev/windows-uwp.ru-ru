...
# <a name="update-your-app-to-the-latest-microsoft-advertising-libraries"></a>Обновление приложения для использования последних библиотек Microsoft Advertising

Для показа в приложениях рекламных баннеров от Microsoft Advertising с помощью элементов управления **AdControl** или **AdMediatorControl** поддерживаются только следующие пакеты SDK:

* [Microsoft Store Services SDK](http://aka.ms/store-services-sdk) (для приложений UWP)
* [Microsoft Advertising SDK для Windows и Windows Phone 8.x](http://aka.ms/store-8-sdk) (для приложений Windows 8.1 и Windows Phone 8.x)

До этих SDK было несколько более старых выпусков пакетов SDK для показа рекламы в приложениях для Windows и Windows Phone. Эти устаревшие выпуски больше не поддерживаются. В будущем мы планируем прекратить показ рекламных баннеров в приложениях, в которых используются эти устаревшие SDK.

Если у вас есть приложение (уже опубликованное в Магазине или находящееся на стадии разработки), в котором рекламные баннеры отображаются с помощью элементов управления **AdControl** или **AdMediatorControl**, имеет смысл обновить его для использования последней версии рекламного SDK для вашей целевой платформы, чтобы ваше приложение продолжало получать рекламные баннеры в будущем. Следуйте инструкциям в этой статье, чтобы определить, затрагивает ли это изменение ваше приложение, и узнать, как его обновить.

Если это изменение затрагивает ваше приложение и вы не обновите его для использования последнего рекламного SDK, вы будете наблюдать следующее поведение, если мы прекратим показывать рекламные баннеры в приложениях с неподдерживаемыми выпусками рекламного SDK:
...
* При запросе новой рекламы элементом управления **AdControl** или **AdMediatorControl** в вашем приложении будет создаваться событие **ErrorOccurred** элемента управление, и свойство **ErrorCode** аргументов события будет иметь значение **NoAdAvailable**.

Некоторый дополнительный контекст об этом изменении: мы больше не поддерживаем старые выпуски рекламных SDK, которые не поддерживают минимальный набор возможностей, включая возможность показывать мультимедиа HTML5 по [спецификации Mobile Rich-media Ad Interface Definitions (MRAID) 1.0](http://www.iab.com/wp-content/uploads/2015/08/IAB_MRAID_VersionOne.pdf) Бюро интерактивной рекламы (IAB). Эти возможности интересуют многих из наших рекламодателей, поэтому это изменение поможет сделать нашу экосистему приложений более привлекательной для рекламодателей и в конечном итоге повысить ваши доходы.

В случае каких-либо проблем или если вам нужна помощь, [обратитесь в службу поддержки](http://go.microsoft.com/fwlink/?LinkId=393643).
