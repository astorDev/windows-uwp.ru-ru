...
description: "Использование этого метода в API покупок Магазина Windows для изменения состояния выставления счетов за подписку для пользователя."
...
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, uwp, API покупок Магазина Windows, подписки"
...
Использование этого метода в API покупок Магазина Windows для изменения состояния выставления счетов надстройки с подпиской для конкретного пользователя. Можно отменить, расширить, компенсировать или отключить автоматическое возобновление для подписки.
...
* Ключ идентификатора Магазина Windows, который представляет идентификатор пользователя, имеющего разрешение на подписку, которую вы хотите изменить.
...
|----------------|--------|-------------|-----------|
| recurrenceId | Строка | Идентификатор подписки, которую необходимо изменить. Чтобы получить этот ИД, вызовите метод [получение подписок для пользователя](get-subscriptions-for-a-user.md), определите запись текста ответа, представляющую надстройку изменяемой подписки, и используйте значение поля **Id** для записи.     | Да      |
...
|----------------|--------|---------------|----------|
| b2bKey         | Строка | [Ключ идентификатора Магазина Windows](view-and-grant-products-from-a-service.md#step-4), представляющий идентификатор пользователя, подписку которого требуется изменить.     | Да      |
...
В следующем примере показано, как использовать этот метод, чтобы расширить период подписки на 5 дней. Замените значение *b2bKey* [ключом идентификатора Магазина Windows](view-and-grant-products-from-a-service.md#step-4), представляющим идентификатор пользователя, подписку которого вы хотите изменить.
...
| productId | Строка | [Код продукта в Магазине](in-app-purchases-and-trials.md#store-ids) для [продукта](in-app-purchases-and-trials.md#products-skus-and-availabilities), который представляет надстройку подписки в каталоге Магазина Windows. Пример кода продукта в Магазине: 9NBLGGH42CFD.     |
| skuId | Строка |  [Код продукта в Магазине](in-app-purchases-and-trials.md#store-ids) для [номера SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities), который представляет надстройку подписки в каталоге Магазина Windows. Пример кода продукта в Магазине для номера SKU: 0010.    |
