...
|----------------|--------|-------------|-----------|
| recurrenceId | Строка | Идентификатор подписки, которую необходимо изменить. Чтобы получить этот идентификатор, вызовите [получить подписки пользователя](get-subscriptions-for-a-user.md) идентификации записи тело ответа, представляющий надстройку подписки, вы хотите изменить метод и использовать значение **идентификатор** поле для записи.     | Да      |
...
| changeType     | Строка |  Одна из следующих строк, которая определяет тип вносимого изменения:<ul><li>**Отмена**: Отменяет подписку.</li><li>**Расширить**: Расширяет подписки. Если задать это значение, необходимо также включить параметр *extensionTimeInDays*.</li><li>**Возврат**: Возмещения подписки для клиента.</li><li>**ToggleAutoRenew**: Отключает автоматическое продление подписки. Если автоматическое обновление в настоящее время отключено для подписки, это значение ни на что не влияет.</li></ul>   | Да      |
...
|---------------|--------|-----------------------------------------------|
| autoRenew | Boolean (Логическое) |  Указывает, настроено ли в подписке автоматическое продление в конце текущего периода.   |
...
| expirationTimeWithGrace | Строка | Дата и время действия подписки истекает включая льготный период, в формате ISO 8601. Это значение указывает, когда пользователь потеряет доступ к подписке, после сбоя автоматического возобновления подписки.    |
...
| isTrial | Boolean (Логическое) |  Указывает, является ли подписка пробной версией.     |
...
| productId | Строка | [Код продукта в Store](in-app-purchases-and-trials.md#store-ids) для [продукта](in-app-purchases-and-trials.md#products-skus-and-availabilities), который представляет надстройку подписки в каталоге Microsoft Store. Пример кода продукта в Магазине: 9NBLGGH42CFD.     |
| skuId | Строка |  [Код продукта в Store](in-app-purchases-and-trials.md#store-ids) для [номера SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities), который представляет надстройку подписки в каталоге Microsoft Store. Пример кода продукта в Магазине для номера SKU: 0010.    |
