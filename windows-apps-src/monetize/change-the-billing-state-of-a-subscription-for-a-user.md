...
|----------------|--------|-------------|-----------|
| recurrenceId | Строка | Идентификатор подписки, которую необходимо изменить. Для получения этого идентификатора, вызов метода [get подписки для пользователя](get-subscriptions-for-a-user.md) , определите запись текста ответа, который представляет надстройку подписки, которую вы хотите изменить и используйте значение поля **код** для записи.     | Да      |
...
| expirationTimeWithGrace | string | Дата и время истечения срока действия подписки будет льготный период, в том числе в формате ISO 8601. Это значение указывает, когда пользователь сможет получить доступ к подписки после сбоя для автоматического продления подписки.    |
