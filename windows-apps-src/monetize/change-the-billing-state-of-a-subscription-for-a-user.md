...
|----------------|--------|-------------|-----------|
| recurrenceId | Строка | Идентификатор подписки, которую необходимо изменить. Чтобы получить этот код, вызовите метод [Получение подписок для пользователя](get-subscriptions-for-a-user.md) , определение запись текста ответа, представляющий подписки надстройку, которую требуется изменить и использовать значение поля **код** для записи.     | Да      |
...
| expirationTimeWithGrace | string | Дата и время истечения срока действия подписки будет включая льготный период в формате ISO 8601. Это значение указывает, когда пользователь сможет получить доступ к подписки после сбоя подписки для автоматического обновления.    |
