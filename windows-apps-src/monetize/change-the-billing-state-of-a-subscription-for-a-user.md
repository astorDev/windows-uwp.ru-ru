...
| expirationTimeWithGrace | string | Дата и время истечения срока действия подписки будет льготный период, в том числе в формате ISO 8601. Это значение указывает, когда пользователи потеряют доступ к подписке, после сбоя для автоматического продления подписки.    |
