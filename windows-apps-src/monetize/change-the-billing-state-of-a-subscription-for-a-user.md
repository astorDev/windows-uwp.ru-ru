...
|----------------|--------|-------------|-----------|
| recurrenceId | Строка | Идентификатор подписки, которую необходимо изменить. Чтобы получить этот код, вызовите метод [получения подписки для пользователя](get-subscriptions-for-a-user.md) и идентификатора записи тело ответа, представляющий надстройку подписки, которую вы хотите изменить значение поля **идентификатора** используется для записи.     | Да      |
...
| expirationTimeWithGrace | string | Дата и время подписки истекает, включая льготный период, в формате ISO 8601. Это значение указывает, когда он потеряет доступ к подписке после сбоя подписки для автоматического обновления.    |
