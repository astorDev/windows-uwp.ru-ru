...
ms.locfileid: "7420078"
---
# <a name="get-daily-app-usage"></a>Получение сведений о ежедневном использовании приложения

Используйте этот метод в API аналитики для Microsoft Store, чтобы получить сводные данные (не включая многопользовательской игры Xbox) в формате JSON об использовании приложения во время в заданном диапазоне дат (за последние 90 дней только) и других дополнительных фильтров. Эта информация также доступна в [отчете об использовании](../publish/usage-report.md) в центре партнеров.
...
| monthlyActiveDevices      | long    | Количество устройств под управлением приложения определенный период времени, начиная с при запуске приложения (Начало процесса) и завершение завершением работы (завершение процесса) или после периода бездействия.                                      |
...
* [Получение ежемесячных ussage приложения](get-app-usage-monthly.md)
