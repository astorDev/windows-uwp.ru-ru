...
Используйте этот метод в API аналитики для Microsoft Store для получения сводных данных отчетности об ошибках для классического приложения, которое вы добавили в [программу для разработчиков классических приложений для Windows](https://msdn.microsoft.com/library/windows/desktop/mt826504). Этот метод можно получить только ошибки, возникшие за последние 30 дней. Эта информация также доступна в [отчете о работоспособности](https://msdn.microsoft.com/library/windows/desktop/mt826504) для классических приложений на информационной панели Центра разработки для Windows.
...
| startDate | дата | Начальная дата диапазона дат, для которого требуется получить данные отчетов об ошибках, в формате ```mm/dd/yyyy```. По умолчанию используется текущая дата.<p/><p/>**Примечание:**&nbsp;&nbsp;этот метод можно получить только ошибки, возникшие за последние 30 дней.  |  Нет  |
