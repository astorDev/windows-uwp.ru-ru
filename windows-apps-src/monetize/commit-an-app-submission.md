...
description: Используйте этот метод в интерфейсе API отправки Microsoft Store для сохранения Отправка нового или обновленного приложения в центре партнеров.
title: Подтверждение отправки приложения
ms.date: 04/17/2018
ms.topic: article
...
ms.locfileid: "66371997"
---
# <a name="commit-an-app-submission"></a>Подтверждение отправки приложения

Используйте этот метод в интерфейсе API отправки Microsoft Store для сохранения Отправка нового или обновленного приложения в центре партнеров. Фиксации действия оповещения центра партнеров, отправки будут переданы данные (включая все связанные пакеты и образы). В ответ центра партнеров фиксирует изменения данных отправки для приема и публикации. После успешного завершения операции фиксации изменения отправки отображаются в центре партнеров.
...
|--------|------------------------------------------------------------------|
| ПОМЕСТИТЬ    | `https://manage.devcenter.microsoft.com/v1.0/my/applications/{applicationId}/submissions/{submissionId}/commit` |
...
| Имя        | Тип   | Описание                                                                 |
|---------------|--------|-----------------------------------------------------------------------------|
| applicationId | строка | Обязательный. Код продукта в Магазине, принадлежащий приложению, которое содержит отправку для фиксации. Подробнее о коде продукта в Магазине см. в статье [Просмотр сведений об идентификации приложений](https://docs.microsoft.com/windows/uwp/publish/view-app-identity-details).  |
