...
ms.locfileid: "6464889"
---
# <a name="get-error-reporting-data-for-your-xbox-one-game"></a>Получение данных системы отчетов для Xbox One игры

Используйте этот метод в API аналитики Microsoft Store для получения сводные данные об ошибках для Xbox One, игры, которая была добавлена через портал разработчика Xbox (XDP) и доступны на информационной панели центра разработки аналитики XDP.
...
|---------------|--------|---------------|------|
| applicationId | string | КОД продукта для игры Xbox One, по которому запрашиваются данные отчетов об ошибках. Чтобы получить код продукта для игры, перейдите к игре на портале разработчиков для Xbox (XDP) и получите код продукта из URL-адреса. Кроме того Если загрузить данные работоспособности из аналитического отчета центра разработки для Windows, код продукта включается в TSV-файл. |  Да  |
...
В следующих примерах показано несколько запросов на получение Xbox One игры данные об ошибках. Замените значение *applicationId* код продукта для вашей игры.
...
| Значение           | Тип    | Описание        |
|-----------------|---------|---------------------|
| date            | строка  | Первая дата в диапазоне дат, для которого требуется получить данные об ошибках, в формате ```yyyy-mm-dd```. Если в запросе указывается один день, это значение равно дате, соответствующей тому дню. Если в запросе указывается более длительный диапазон дат, это значение равно первой дате в этом диапазоне дат. Для запросов, которые определяют значение *aggregationLevel* **час**, это значение также содержит значения времени в формате ```hh:mm:ss``` в местный часовой пояс, в котором произошла ошибка.  |
| applicationId   | string  | КОД продукта для игры Xbox One, для которого требуется получить данные об ошибках.   |
