...
description: Используйте эти методы в API отправки Microsoft Store для управления отправками надстроек для приложений, которые зарегистрированы в вашей учетной записи центра партнеров.
...
> [!IMPORTANT]
> Если вы используете API отправки Microsoft Store для создания отправки для надстройки, обязательно дополнительные изменения в отправку только с с помощью API, а не внесение изменений в центре партнеров. При использовании центра партнеров для изменения отправки, изначально созданной с помощью API, вы больше не сможете изменять или фиксировать эту отправку с помощью API. В некоторых случаях отправка может оставаться в состоянии ошибки, когда продолжить процесс отправки невозможно. В этом случае необходимо удалить отправку и создать новую.
...
Используйте следующие методы для получения, создания, обновления, фиксации и удаления отправки надстройки. Прежде чем использовать эти методы, надстройка должна уже существовать в вашей учетной записи центра партнеров. Вы можете создать надстройки в центре партнеров, [указав тип продукта и кода продукта](../publish/set-your-add-on-product-id.md) или с помощью методов API отправки Microsoft Store в описано в [надстройки](manage-add-ons.md).
...
1. Если еще не сделано, выполните необходимые условия, описанные в [Создание и управление отправками с помощью служб Microsoft Store](create-and-manage-submissions-using-windows-store-services.md), включая привязку приложения Azure AD с учетной записью центра партнеров и получение клиента идентификатор и ключ. Это необходимо сделать только один раз; после получения идентификатора клиента и ключа с их помощью можно в любой момент создать новый маркер доступа Azure AD.  
...
5. Подтвердите отправку, выполнив следующий метод. Таким образом оповещается центра партнеров, что Вы завершили отправка и что обновления теперь должен применяться к вашей учетной записи. Дополнительные сведения см. в разделе [Подтверждение отправки надстройки](commit-an-add-on-submission.md).
...
>    * После вызова [метода GET для получения отправки надстройки](get-an-add-on-submission.md) значение *sales* будет пустым. Вы можете продолжать использовать центр партнеров для получения данных продаж для отправки надстройки.
>    * При вызове [метода PUT для обновления отправки надстройки](update-an-add-on-submission.md) сведения в значении *sales* будут игнорироваться. Вы можете продолжать использовать центр партнеров для изменения данных продаж для отправки надстройки.
...
* [Надстроек в центре партнеров](https://msdn.microsoft.com/windows/uwp/publish/iap-submissions)
