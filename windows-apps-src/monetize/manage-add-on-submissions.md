...
description: Эти методы можно используете в интерфейсе API отправки Microsoft Store для управление отправкой надстройки для приложений, зарегистрированных для учетной записи центра партнеров.
...
> [!IMPORTANT]
> Если вы используете API отправки Microsoft Store для создания отправки для надстройки, не забудьте вносить дальнейшие изменения для отправки только с помощью API, а внесение изменений в центре партнеров. При использовании центра партнеров, чтобы изменить такие данные, изначально созданные с помощью API, вы больше не сможете изменить или фиксации, отправки с помощью API. В некоторых случаях отправка может оставаться в состоянии ошибки, когда продолжить процесс отправки невозможно. В этом случае необходимо удалить отправку и создать новую.
...
Используйте следующие методы для получения, создания, обновления, фиксации и удаления отправки надстройки. Прежде чем использовать эти методы, надстройка должна существовать в вашей учетной записи центра партнеров. Чтобы создать надстройку в центре партнеров по [определения его тип продукта и код продукта](../publish/set-your-add-on-product-id.md) или с помощью методов API отправки в Microsoft Store, описанных в [Управление надстройками](manage-add-ons.md).
...
<td align="left"><a href="get-an-add-on-submission.md">Получение существующего отправки надстройки</a></td>
</tr>
<tr>
<td align="left">GET</td>
...
<td align="left">https://manage.devcenter.microsoft.com/v1.0/my/inappproducts/{id}/submissions/{submissionId}</td>
<td align="left"><a href="delete-an-add-on-submission.md">Удалить надстройку отправки</a></td>
</tr>
</tbody>
</table>
...
1. Если еще не сделано, полный предварительные требования описано в разделе [Создание и управление отправкой, с помощью служб Microsoft Store](create-and-manage-submissions-using-windows-store-services.md), в том числе сопоставления для приложения Azure AD с вашей учетной записи центра партнеров и получение вашей Идентификатор клиента и ключ. Это необходимо сделать только один раз; после получения идентификатора клиента и ключа с их помощью можно в любой момент создать новый маркер доступа Azure AD.  
...
| keywords           | array  | Массив строк, содержащих до 10 [ключевых слов](../publish/enter-add-on-properties.md#keywords) для надстройки. Приложение может запрашивать надстройки с помощью этих ключевых слов.   |
...
|  sales               |   array      |  **Не рекомендуется**. Массив [ресурсов продажи](#sale-object), содержащих сведения о продажах для надстройки.     |     
...
>    * После вызова [метода GET для получения отправки надстройки](get-an-add-on-submission.md) значение *sales* будет пустым. Можно продолжать использовать корпорации Майкрософт для получения данных продаж для отправки надстройки.
>    * При вызове [метода PUT для обновления отправки надстройки](update-an-add-on-submission.md) сведения в значении *sales* будут игнорироваться. Вы можете использовать центр партнеров для изменения данных продаж для отправки надстройки.
