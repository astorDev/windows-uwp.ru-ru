...
* Создайте эксперимент [проекта](run-app-experiments-with-a-b-testing.md#terms) в центре партнеров, которая определяет несколько удаленных переменных, отвечающих за текст и цвет кнопки приложения.
* Создание приложения с помощью кода, получающий значений удаленных переменных, использовать эти данные для изменения фонового цвета кнопки и записывает представления преобразования событий и центр партнеров.
...
Чтобы пройти это пошаговое руководство, вам потребуется учетная запись центра партнеров и необходимо настроить компьютер разработчика, как описано в [Проведение экспериментов с A / b-тестирования](run-app-experiments-with-a-b-testing.md).
...
2. Если у вас уже есть приложение в центре партнеров, которые вы хотите использовать для создания эксперимента, выберите это приложение в центре партнеров. Если вы еще не приложения в центре партнеров, [Создание приложения посредством резервирования имени](../publish/create-your-app-by-reserving-a-name.md) и выберите этого приложения в центре партнеров.
...
8. Замените все содержимое файла с кодом следующим кодом. Назначьте ```projectId``` переменной значение [идентификатор проекта](run-app-experiments-with-a-b-testing.md#terms) , который вы получили из центра партнеров в предыдущем разделе.
...
1. Вернитесь на страницу проекта **Эксперименты щелкните кнопку** в центре партнеров.
...
3. Убедитесь, что результаты в разделах **Сводка по результатам** и **Подробные сведения о результатах** соответствуют вашим ожиданиям. Дополнительные сведения об этих разделах см. в разделе [Управление экспериментом в центре партнеров](manage-your-experiment.md#review-the-results-of-your-experiment).
    > [!NOTE]
