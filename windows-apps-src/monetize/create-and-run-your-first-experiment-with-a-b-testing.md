...
* Создание службы "Экспериментирование" [проекта](run-app-experiments-with-a-b-testing.md#terms) в центре партнеров, который определяет несколько удаленных переменных, которые представляют текст и цвет кнопки приложения.
* Создание приложения с кодом, который получает удаленный значения переменных, эти данные используются, чтобы изменить цвет фона кнопки и представление журналов и преобразования данных события обратно в центре партнеров.
...
* Просмотрите результаты эксперимента в центре партнеров, выберите вариант, чтобы включить для всех пользователей приложения и завершите эксперимента.
...
## <a name="create-a-project-with-remote-variables-in-partner-center"></a>Создайте проект с удаленных переменных в центре партнеров

1. Войдите в [Центр партнеров](https://partner.microsoft.com/dashboard).
2. Если у вас уже есть приложение в центре партнеров, который вы хотите использовать для создания эксперимента, выберите это приложение в центре партнеров. Если у вас еще нет приложения в центре партнеров, [создать новое приложение, резервируя имя](../publish/create-your-app-by-reserving-a-name.md) и выберите приложение в центре партнеров.
...
1. Вернитесь к **эксперименты, щелкните кнопку** странице проекта в центре партнеров.
...
> [!NOTE]
> Как только вы активируете эксперимент, центра партнеров немедленно запускает сбор данных из любого приложения, подготовленные для ведения журнала данных для своего эксперимента. Тем не менее может занять несколько часов для эксперимента для отображения данных в центре партнеров.
...
3. Убедитесь, что результаты в разделах **Сводка по результатам** и **Подробные сведения о результатах** соответствуют вашим ожиданиям. Дополнительные сведения об этих разделов см. в разделе [управление эксперимента в центре партнеров](manage-your-experiment.md#review-the-results-of-your-experiment).
    > [!NOTE]
