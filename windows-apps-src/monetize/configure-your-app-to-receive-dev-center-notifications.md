...
Чтобы зарегистрировать приложение для получения целевых Push-уведомления из центра партнеров.
...
## <a name="notify-partner-center-of-your-app-launch"></a>Уведомление центра партнеров запуске вашего приложения

При выборе параметра **Отслеживание скорости запуска приложений** для вашего целевого Push-уведомления в центре партнеров, вызовите метод [ParseArgumentsAndTrackAppLaunch](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesengagementmanager.parseargumentsandtrackapplaunch) из соответствующей точки входа в вашем приложении, чтобы уведомить центр партнеров, который был вашего приложения запущено в ответ на Push-уведомление.

Этот метод также возвращает исходные аргументы запуска для приложения. При выборе для отслеживания запусков приложения для Push-уведомления, непрозрачный отслеживающий идентификатор добавляется в аргументы запуска для отслеживания его запуска в центре партнеров. Необходимо передать аргументы запуска вашего приложения в метод [ParseArgumentsAndTrackAppLaunch](https://docs.microsoft.com/uwp/api/microsoft.services.store.engagement.storeservicesengagementmanager.parseargumentsandtrackapplaunch) , и этот метод отправит отслеживающий идентификатор в центр партнеров, удалит его из аргументов запуска и вернет исходные аргументы запуска для вашего код.
...
Обратите внимание, что этот метод делает недействительным канал, который используется для уведомлений, поэтому приложение больше не будет получать push-уведомления *ни от каких* служб. После его закрытия, канала невозможно использовать ни для каких служб, включая целевые Push-уведомления из центра партнеров и другие уведомления с помощью WNS. Чтобы возобновить отправку push-уведомлений для этого приложения, приложение должно будет запросить новый канал.
