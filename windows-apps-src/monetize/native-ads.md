...
2.  В центре партнеров, перейдите в раздел [рекламы в приложении](../publish/in-app-ads.md) страницы и [создать единицу ad](set-up-ad-units-in-your-app.md#live-ad-units). В качестве типа группы объявлений укажите **Собственная**. Запишите идентификатор рекламного блока и идентификатор приложения.
    > [!NOTE]
...
4.  В коде, замените значения модульного тестирования ad (т. е *applicationId* и *adUnitId* параметры [NativeAdsManagerV2](https://docs.microsoft.com/uwp/api/microsoft.advertising.winrt.ui.nativeadsmanagerv2.-ctor) конструктор) с помощью динамического значения создан в центре партнеров.
