...
|----------------|--------|-------------------------------------------------------------------------------------------------------|
| Authorization  | строка | Обязательный. Маркер доступа Azure AD в виде *токена*&gt; **носителя** &lt;.                           |
...
|-------------------|--------------|---------------------|----------|
| beneficiaries     | List&lt;UserIdentity & gt | Список объектов UserIdentity, представляющих пользователей, которые запрашиваются для продуктов. Дополнительные сведения см. в таблице ниже.    | Да      |
...
| productTypes      | Строка&lt;списка&gt;       | Указывает, какие типы продуктов должны возвращаться в результатах запроса. Поддерживаемые типы продуктов: **Application**, **Durable** и **UnmanagedConsumable**.     | Да       |
...
| productType          | строка             | Один из следующих типов продуктов: **Application**, **долговечность**и **унманажедконсумабле**.        | Да      |
