...
2.  [Сопоставление идентификатора приложения Azure AD со своим приложением в панели мониторинга Центр разработчиков Windows](#step-2).
3.  В своей службе [создайте маркеры доступа Azure AD](#step-3), представляющие ваше удостоверение издателя.
4.  В приложении Windows client [создания ключа Microsoft Store ID](#step-4) , который представляет идентификатор текущего пользователя и передайте этот ключ резервного к службе.
...
Этот процесс начала до конца состоит из двух компонентов программного обеспечения, которые выполнения различных задач:

* **Службы**. Это приложение, на котором выполняется безопасно в контексте бизнес-среде, и можно реализовать с помощью любого платформы разработки. Служба отвечает за создание маркеры доступа Azure AD необходимые для сценария, а также для вызова коды URI REST для набор хранилищ Microsoft интерфейсов API и покупки.
* **Приложение Windows client**. Это приложение, для которого требуется получить доступ к информации и управление клиента обслуживания (включая надстройки для приложения). Это приложение отвечает за создание клавиш Microsoft Store ID, которые вам нужно вызвать набор хранилищ Microsoft API и Приобретите API от службы.

<span id="step-1"/>

## <a name="step-1-configure-an-application-in-azure-ad"></a>Шаг 1: Настройте приложения в Azure AD

Прежде чем использовать набор хранилищ Microsoft API или приобрести API, необходимо создать Azure AD веб-приложения, получение клиентом идентификатор и идентификатор приложения для приложения и создание ключа. Azure AD веб-приложения, представляющий службы, из которой нужно вызвать набор хранилищ Microsoft API поставщика или приобретите API. Требуется идентификатор клиента, код приложения и ключ для создания маркеры доступа Azure AD, которые необходимо принять вызов API.

> [!NOTE]
> Описанные в этом разделе задачи необходимо выполнить только один раз. После того как обновить манифест приложения Azure AD и у вас есть идентификатор клиента, приложение кода и секрета клиента, можно повторно использовать эти значения любое время, вам необходимо создать новый маркер доступа Azure AD.

1.  Если вы еще не сделано, следуйте инструкциям в [Интеграции приложений с помощью Azure Active Directory](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications) для регистрации **Web app и API** приложений с Azure AD.
    > [!NOTE]
    > При регистрации приложения необходимо выбрать **Web app и API** как приложения введите таким образом, вы можете получить ключ (также называемая *секрет клиента*) для вашего приложения. В последнем шаге для вызова API коллекции Microsoft Store или API покупок в Microsoft Store необходимо предоставить секрет клиента при запросе маркера доступа от Azure AD.

2.  В [Портала управления Azure](https://portal.azure.com/)перейдите к **Azure Active Directory**. Выберите каталог, нажмите кнопку **регистрации приложения** в левой панели переходов и выберите приложение.
3.  Будет выполнен переход к странице главного регистрации приложения. На этой странице скопируйте значение **Идентификатора приложения** для использования в дальнейшем.
4.  Создание ключа, который требуется более поздняя версия (это все называется *секрет клиента*). В левой области нажмите кнопку **Параметры** , а затем **ключи**. На этой странице выполните действия для [создания ключа](https://docs.microsoft.com/azure/active-directory/develop/active-directory-integrating-applications#to-add-application-credentials-or-permissions-to-access-web-apis). Скопируйте этот ключ для дальнейшего использования.
5.  Добавьте несколько необходимых аудиторию URIs вашей [манифест приложения](https://docs.microsoft.com/azure/active-directory/develop/active-directory-application-manifest). В левой области щелкните **манифест**. Нажмите кнопку **Изменить**, замените `"identifierUris"` раздела с следующий текст и нажмите кнопку **Сохранить**.
...
## <a name="step-2-associate-your-azure-ad-application-id-with-your-client-app-in-windows-dev-center"></a>Шаг 2: Сопоставление идентификатора приложения Azure AD с приложением клиента в центр разработчиков Windows

Перед можно использовать набор хранилищ Microsoft API или приобретите API для настройки прав владельца и приобретений для приложения или дополнительный компонент, необходимо связать код приложения Azure AD с помощью приложения (или приложение, которое содержит надстройки) в панели мониторинга центра разработки.
...
2.  Последовательно выберите пункты **служб** &gt; **семейства продуктов и покупки** страницы и введите код приложения Azure AD в одну из доступные поля **Идентификатор клиента** .
...
* Параметров *client\_id* и *client\_secret* укажите идентификатор приложения и секрет клиента для приложения, который извлекается из [Портала управления Azure](http://manage.windowsazure.com). Оба эти параметра являются обязательными для создания маркера доступа с уровнем проверки подлинности, необходимым для API коллекции Microsoft Store или API покупки Microsoft Store.
...
2.  В коде приложения вызовите один из следующих методов, чтобы извлечь ключ идентификатора Microsoft Store:
  ...
  * Если в вашем приложении для управления покупками из приложения используется класс [CurrentApp](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Store.CurrentApp) из пространства имен [Windows.ApplicationModel.Store](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store), используйте метод [CurrentApp.GetCustomerCollectionsIdAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store.currentapp.getcustomercollectionsidasync).
    ...
    Передайте маркер доступа Azure AD в параметр *serviceTicket* метода. Если ведение анонимного пользователя идентификаторы в контексте служб, которыми вы управляете как publisher текущего приложения, можно также передать идентификатор пользователя параметр *publisherUserId* для связи с новым ключом Microsoft Store ID (идентификатор пользователя будет длинное текущего пользователя bedded в разделе). В противном случае если вам не нужно связать с ключом Microsoft Store ID идентификатор пользователя, можно передать любое строковое значение для параметра *publisherUserId* .
...
На следующей схеме показана процесс создания ключа Microsoft Store ID.

  ![Создание ключа код магазина Windows](images/b2b-1.png)
...
Ниже описывается процесс вызова метода в набор хранилищ Microsoft API или Покупка API из службы.

  ![Позвонить семейств сайтов или puchase API](images/b2b-2.png)
