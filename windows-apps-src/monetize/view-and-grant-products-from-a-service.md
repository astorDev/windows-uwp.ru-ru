...
2.  [Связать свой идентификатор приложения Azure AD с приложением в центре партнеров](#step-2).
...
* **Ваша служба**. Это приложение, которое безопасно выполняется в контексте бизнес-среде, и он может осуществляться с помощью любого платформы разработки, который вы выбираете. Ваша служба отвечает, для создания маркеры доступа Azure AD требуется для сценария, а также для вызова коды URI REST для коллекции Microsoft Store, API и API покупок.
* **Windows свое клиентское приложение**. Это приложение, для которого вы хотите получить доступ к информации и управление клиентом права (включая надстройки для приложения). Это приложение отвечает за создание ключей идентификатора Microsoft Store, вам нужно вызывать API коллекции Microsoft Store и API покупки из службы.

<span id="step-1"/>

## <a name="step-1-configure-an-application-in-azure-ad"></a>Шаг 1: Настройка приложения в Azure AD

Прежде чем использовать API коллекции Microsoft Store или API покупки, необходимо создать Azure AD веб-приложения, извлечь идентификатор владельца и идентификатор приложения для приложения и создать ключ. Azure AD веб-приложение представляет службу, из которого вы хотите вызывать API коллекции Microsoft Store или API покупки. Требуется идентификатор владельца, идентификатор и ключ для создания маркеры доступа Azure AD, которые необходимо вызвать API.

> [!NOTE]
> Описанные в этом разделе задачи необходимо выполнить только один раз. После того как вы обновите манифест приложения Azure AD и у вас есть идентификатор клиента, секрет клиента и идентификатор приложения, вы можете повторно использовать эти значения любое время, вам необходимо создать новый маркер доступа Azure AD.
...
5.  Добавьте несколько требуется аудиторий, ваш [манифест приложения](https://docs.microsoft.com/azure/active-directory/develop/active-directory-application-manifest). На левой панели щелкните **манифест**. Нажмите кнопку **Изменить**, замените `"identifierUris"` разделе следующим текстом, а затем нажмите кнопку **Сохранить**.
...
## <a name="step-2-associate-your-azure-ad-application-id-with-your-client-app-in-partner-center"></a>Шаг 2: Привязка идентификатор приложения Azure AD с приложением клиента в центре партнеров

Прежде чем использовать API коллекции Microsoft Store или API покупки для настройки право собственности и покупок из приложения или надстройки, вы должны связать свой идентификатор приложения Azure AD с приложением (или приложения, которое содержит надстройку) в центре партнеров.

> [!NOTE]
> Эту операцию необходимо выполнить только один раз.

1.  Войдите [Центр партнеров](https://partner.microsoft.com/dashboard) и выберите свое приложение.
