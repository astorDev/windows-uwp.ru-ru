...
* **Ваша служба**. Это приложение, которое безопасно выполняется в контексте рабочей среды, и он может осуществляться с помощью любого платформы разработки, который вы выбираете. Ваша служба отвечает для создания маркера доступа Azure AD необходимые для сценария, а также для вызова REST идентификаторы URI для коллекции Microsoft Store, API и API покупок.
...
Прежде чем использовать API коллекции Microsoft Store или API покупки для настройки принадлежности и покупок из приложения или надстройки, вы должны связать свой идентификатор приложения Azure AD с приложением (или приложения, которое содержит надстройку) на информационной панели центра разработки.
