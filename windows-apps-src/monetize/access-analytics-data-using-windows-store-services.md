...
3.  [Вызов интерфейса API аналитики Магазина Windows](#call-the-windows-store-analytics-api).
...
После привязки приложения Azure AD к учетной записи в Центре разработки и получения кода клиента и ключа для приложения эти данные можно использовать, чтобы получить токен доступа Azure AD. Маркер доступа необходим для вызова любых методов API аналитики для Магазина Windows. После создания маркера доступа у вас будет 60 минут, чтобы использовать его до окончания его срока действия.
