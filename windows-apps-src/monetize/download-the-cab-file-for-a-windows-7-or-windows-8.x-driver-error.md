...
description: Используйте этот метод в API аналитики для Microsoft Store, чтобы скачать CAB-файл для ошибки драйвера для Windows7 или Windows8.x. Этот метод предназначен только для независимых поставщиков оборудования.
...
Используйте этот метод в API аналитики для Microsoft Store, чтобы скачать CAB-файл, связанный с определенной ошибкой драйвера для Windows7 или Windows8.x. Перед применением этого метода сначала воспользуйтесь методом [получения сведений об ошибке драйвера для Windows7 или Windows 8.x](get-details-for-a-windows-7-or-windows-8.x-driver-error.md), чтобы извлечь идентификатор требуемого CAB-файла.

Чтобы получить другие сведения об ошибках драйверов для Windows7 или Windows8.x, воспользуйтесь методами [получения данных системы отчетов для драйверов для Windows7 и Windows8.x](get-error-reporting-data-for-windows-7-and-windows-8.x-drivers.md) и [получения сведений об ошибке драйвера для Windows7 или Windows 8.x](get-details-for-a-windows-7-or-windows-8.x-driver-error.md) в API аналитики для Microsoft Store.
