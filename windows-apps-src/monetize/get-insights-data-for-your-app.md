...
|---------------|--------|---------------|------|
| applicationId | string | [Код продукта в магазине](in-app-purchases-and-trials.md#store-ids) для приложения, для которого требуется получить данные о. Если этот параметр не задан, тело ответа будет содержать данные аналитические данные для всех приложений, зарегистрированных в вашей учетной записи.  |  Нет  |
| startDate | date | Начальная дата диапазона дат данных аналитические данные для получения. По умолчанию используется текущая дата минус 30 дней. |  Нет  |
| endDate | date | Конечная дата диапазона дат данных аналитические данные для извлечения. По умолчанию используется текущая дата. |  Нет  |
...
В следующем примере показано запроса на получение данных аналитические данные. Замените значение *applicationId* кодом продукта в Магазине для вашего приложения.
...
|------------|--------|-------------------------------------------------------|
| Значение      | array  | Массив объектов, содержащих данные аналитические данные для приложения. Дополнительные сведения о данных в каждом объекте см. ниже раздел " [представление значения](#insight-values) ".                                                                                                                      |
| TotalCount | int    | Общее количество строк в результирующих данных для запроса.                 |


### <a name="insight-values"></a>Представление значения

Элементы в массиве *Value* содержат следующие значения.

| Значение               | Тип   | Описание                           |
|---------------------|--------|-------------------------------------------|
| applicationId       | string | Код продукта в магазине для приложения, по которой запрашиваются данные аналитические данные.     |
| insightDate                | string | Дата, на котором мы определили изменение определенного метрики. До этой даты окончания недели, в котором обнаружено значительное повышение или уменьшение показатель, по сравнению с недели до этого. |
...
| SubDimensions         | array |  Один или несколько объектов, описывающих один метрики для понимания.   |
| PercentChange            | string |  Процент измененного метрики для своей базы всем клиентам.  |
| Имя измерения           | string |  Имя метрики, описанные в текущем измерения. Примеры включают **EventType**, **рынка**, **DeviceType**, **PackageVersion**, **AcquisitionType**, **AgeGroup** и **Пол**.   |
...
| Дата              | строка |  Дата, на котором мы определили изменения, связанные с текущее представление или текущего измерения.   |
