...
ms.locfileid: "3115196"
---
# <a name="get-insights-data"></a>Получение данных о

Используйте этот метод в API аналитики Microsoft Store для получения данных аналитические сведения, связанные с приобретения, работоспособности и использования метрик для приложения во время в заданном диапазоне дат и других дополнительных фильтров. Эта информация также доступна в [аналитический отчет](../publish/insights-report.md) в информационной панели центра разработки для Windows.
...
|---------------|--------|---------------|------|
| applicationId | string | [Код продукта в магазине](in-app-purchases-and-trials.md#store-ids) для приложения, для которой требуется получить данные этих сведений. Если этот параметр не задан, тело ответа будет содержать данные сведения для всех приложений, зарегистрированных в вашей учетной записи.  |  Нет  |
...
|------------|--------|-------------------------------------------------------|
| Значение      | array  | Массив объектов, содержащих данные о для приложения. Дополнительные сведения о данных в каждом объекте см. Далее в разделе [Insight значения](#insight-values) .                                                                                                                      |
...
| Значение               | Тип   | Описание                           |
|---------------------|--------|-------------------------------------------|
| applicationId       | string | Код продукта в магазине для приложения, по которой запрашиваются данные этих сведений.     |
| insightDate                | string | Дата, на котором мы определили изменение в определенных метрики. Эту дату представляет собой конце недели, в котором обнаружено значительное увеличение или уменьшение недели до этого по сравнению с метрика. |
...
| insightDetail          | array | Одно или несколько [InsightDetail значения](#insightdetail-values) , представляющие сведения о текущей аналитические сведения.    |
...
| DimensionValue              | string | Значение показателя, описанное в текущее измерение. Например если **имя измерения** **EventType**, **DimensionValue** может быть **сбоя** или **зависания**.   |
| FactValue     | string | Абсолютное значение метрики на выбранную дату обнаружен аналитические сведения.  |
| Direction | string |  Направление изменение (**положительное** или **отрицательное**).   |
| Дата              | строка |  Дата, на котором мы определили изменения, связанные с текущей insight или текущего измерения.   |
