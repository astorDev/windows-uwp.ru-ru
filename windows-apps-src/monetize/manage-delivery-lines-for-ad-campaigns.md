...
* Если вы еще не сделали этого, выполните все [необходимые условия](run-ad-campaigns-using-windows-store-services.md#prerequisites) для API рекламных акций Microsoft Store.
  ...
  > [!NOTE]
  > Как часть необходимых компонентов убедитесь, что вы [создали по крайней мере одну платную рекламную кампанию в центре партнеров](../publish/create-an-ad-campaign-for-your-app.md) и что вы добавить по крайней мере одно средство платежа для рекламной кампании в центре партнеров. Каналами доставки, созданные с помощью этого API, будут автоматически выставлять счет на оплату платежному средству по умолчанию, выбранному на странице **рекламные кампании** в центре партнеров.
