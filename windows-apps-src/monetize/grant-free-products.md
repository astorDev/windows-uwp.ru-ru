...
| Content-Type   | string | Указывает тип запросов и ответов. На данный момент единственным поддерживаемым значением является **application/json**. |
...
| b2bKey         | string | Ключ идентификатора Магазина Windows, представляющий личность пользователя.                                                                                                                                                                                                                                                        | Да      |
| devOfferId     | string | Идентификатор определенного разработчиком предложения, который появится в элементе коллекции после покупки.                                                                                                                                                                                                                                 | Нет       |
...
| productId      | string | Код продукта в Магазине из каталога Магазина Windows. Для приложения код продукта в Магазине доступен на [странице удостоверения приложения](../publish/view-app-identity-details.md) информационной панели Центра разработки. Для надстройки код продукта в Магазине указан в URL-адресе страницы обзора для надстройки на информационной панели Центра разработки для Windows. Пример кода продукта в Магазине: 9WZDNCRFJ3Q8. | Да      |
...
| productId               | string         | Код продукта в Магазине для позиции.                                                               | Да      |
...
| skuId                   | string         | Код SKU Магазина Windows для позиции.                                                                   | Да      |
