...
description: "Используйте этот метод в API покупок Магазина Windows, чтобы предоставить бесплатное приложение или надстройку определенному пользователю."
...
ms.prod: windows
ms.technology: uwp
keywords: "windows 10, uwp, API покупок Магазина Windows, предоставление продуктов"
...
Используйте этот метод в API покупок Магазина Windows, чтобы предоставить бесплатное приложение или надстройку (также называется внутренним продуктом приложения или IAP) определенному пользователю.
...
* Маркер доступа Azure AD, созданный с помощью универсального кода ресурса (URI) аудитории `https://onestore.microsoft.com`;
* Ключ ИД Магазина Windows, представляющий удостоверение пользователя, которому требуется предоставить бесплатный продукт.
...
|----------------|--------|---------------------|----------|
| availabilityId | строка | ИД доступности предоставляемого продукта из каталога Магазина Windows.         | Да      |
| b2bKey         | строка | [Ключ ИД Магазина Windows](view-and-grant-products-from-a-service.md#step-4), представляющий удостоверение пользователя, которому требуется предоставить продукт.    | Да      |
...
| productId      | строка | [Код [продукта](in-app-purchases-and-trials.md#products-skus-and-availabilities) в Магазине](in-app-purchases-and-trials.md#store-ids) в каталоге Магазина Windows. Пример кода продукта в Магазине — 9NBLGGH42CFD. | Да      |
| quantity       | целое число    | Объем покупки. На данный момент единственным поддерживаемым значением является "1". Если оно не указано, по умолчанию используется значение "1".   | Нет       |
| skuId          | строка | [Код [SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities) продукта в Магазине](in-app-purchases-and-trials.md#store-ids) в каталоге Магазина Windows. Пример кода продукта в Магазине для SKU — 0010.     | Да      |
...
| friendlyName              | string                      | Понятное имя для заказа. Недоступно для заказов, осуществляемых с помощью API покупок Магазина Windows. | Да      |
...
| availabilityId          | string         | Идентификатора доступности приобретаемого продукта из каталога Магазина Windows.                           | Да      |
...
| productId               | строка         | [Код [продукта](in-app-purchases-and-trials.md#products-skus-and-availabilities) в Магазине](in-app-purchases-and-trials.md#store-ids), представляющий позицию в каталоге Магазина Windows. Пример кода продукта в Магазине — 9NBLGGH42CFD.   | Да      |
...
| skuId                   | строка         | [Код [SKU](in-app-purchases-and-trials.md#products-skus-and-availabilities) продукта в Магазине](in-app-purchases-and-trials.md#store-ids) позиции в каталоге Магазина Windows. Пример кода продукта в Магазине для SKU — 0010.                                                                   | Да      |
| taxAmount               | десятичная дробь        | Сумма налогов для позиции.                                                                            | Да      |
...
| 400  | BadRequest   | InvalidParameter           | Подробности содержат информацию о тексте запроса и полях с недопустимым значением.           |
