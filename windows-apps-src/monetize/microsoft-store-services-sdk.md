...
> [!NOTE]
> Для отображения рекламы в приложениях UWP используйте [Microsoft Advertising SDK](http://aka.ms/ads-sdk-uwp) вместо Microsoft Store Services SDK. Дополнительные сведения см. в статье [Показ рекламы в приложениях](display-ads-in-your-app.md).

## <a name="scenarios-supported-by-the-microsoft-store-services-sdk"></a>Сценарии, поддерживаемые Microsoft Store Services SDK

Microsoft Store Services SDK в настоящее время поддерживает следующие сценарии для приложений UWP. Справочную документацию по API см. в [справочнике по API пакета Microsoft Store Services SDK](https://msdn.microsoft.com/library/windows/apps/mt691886.aspx).
...
> [!NOTE]
> Для отображения рекламы в приложении используйте [Microsoft Advertising SDK](http://aka.ms/ads-sdk-uwp) вместо Microsoft Store Services SDK. Дополнительные сведения см. в статье [Показ рекламы в приложениях](display-ads-in-your-app.md).
...
* **Пакет NuGet.**&nbsp;&nbsp;Можно установить библиотеки SDK для конкретного проекта UWP с использованием NuGet. В этом случае библиотеки SDK устанавливаются только для проекта, в котором установлен пакет NuGet.
...
5. В поле поиска введите **Microsoft.Services.Store.SDK** и установите пакет Microsoft.Services.Store.SDK. Когда установка пакета завершится, сохраните решение.
    > [!NOTE]
...
3. В **Диспетчере ссылок** разверните **универсальные для Windows**, нажмите **Расширения** и затем установите флажок рядом с пунктом **Microsoft Engagement Framework**. Это позволяет использовать API в пространстве имен [Microsoft.Services.Store.Engagement](https://msdn.microsoft.com/library/windows/apps/microsoft.services.store.engagement.aspx) для таких сценариев привлечения клиентов, как [проведение A/B-экспериментов](run-app-experiments-with-a-b-testing.md), [запуск Центра отзывов](launch-feedback-hub-from-your-app.md), [получение целевых push-уведомлений из Центра разработки](configure-your-app-to-receive-dev-center-notifications.md) и [ведение журнала пользовательских событий в Центре разработки](log-custom-events-for-dev-center.md).

3. Нажмите кнопку **OK**.

> [!NOTE]
> Если вы установили библиотеки пакета SDK через NuGet, проект будет содержать ссылку **Microsoft.Services.Store.SDK**. Ссылка на **Microsoft.Services.Store.SDK** представляет пакет NuGet (а не библиотеки в нем), и вы можете проигнорировать ее.

<span id="framework" />
## <a name="understanding-framework-packages-in-the-sdk"></a>Понимание пакетов платформы в SDK

Библиотека Microsoft.Services.Store.Engagement.dll в Microsoft Store Services SDK настроена как *пакет платформы*. Эта библиотека содержит API в пространстве имен [Microsoft.Services.Store.Engagement](https://msdn.microsoft.com/library/windows/apps/microsoft.services.store.engagement.aspx).
