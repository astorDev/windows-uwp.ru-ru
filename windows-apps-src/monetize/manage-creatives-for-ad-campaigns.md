...
|  landingUrl   |  Строка   |  Если вы используете службы отслеживания кампаний, такие как Kochava, AppsFlyer или Tune, для анализа установок вашего приложения, укажите в этом поле URL-адрес отслеживания при вызове метода POST (это значение должно иметь допустимый URI). Если вы не используете службы отслеживания кампании, опустите это значение при вызове метода POST (в данном случае этот URL-адрес будет создан автоматически).   |  Нет    |     |   Да    |       
