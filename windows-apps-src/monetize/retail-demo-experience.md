...
description: Подготовка приложения для демонстрации коммерческий режим, что хорошее на полу розничной продажи.
...
ms.locfileid: "4531696"
---
# <a name="add-retail-demo-rdx-features-to-your-app"></a>Добавление функций розничных магазинах (RDX) в приложение

Включите режим розничных магазинах в приложении для Windows, таким образом пользователей, компьютеров и устройств на продажу низ можно перейти непосредственно к.

Когда клиенты в розничном магазине, они ожидают, что возможность опробовать демонстрации компьютеров и устройств. Они часто проводят немало времени, Экспериментируя с приложениями с помощью [возникают демонстрации в розничных магазинах (RDX)](https://docs.microsoft.com/windows-hardware/customize/desktop/retail-demo-experience).

Можно настроить приложение для предоставления различных возможностей в _обычной_ или _розничной торговли_ . Например если приложение запускается с процессом установки, может пропустить в коммерческом режиме и предустановить приложения с помощью параметров образцов данных и по умолчанию, поэтому они могут перейти в.

С точки зрения наших клиентов существует только в одном приложении. Чтобы помочь им различать два режима, мы рекомендуем, во время работы приложения в коммерческом режиме, он показывает слово «Retail» важны в строке заголовка или другом заметном месте.
...
* **Показать максимально**. Используйте демонстрационный для покупателю понять, почему вашего приложения. Это первый раз ваш клиент увидит вашего приложения, поэтому покажите выгодной!
...
Приложениям с определением RDX, которые не соответствуют этим требованиям должны изучить исправление немедленно. Если сразу же исправить приложение не удается, возможно, оно будет удалено со всех демонстрационных устройств для розничных магазинов.

* **Автономный возникают Memorable**. Ваше приложение должно предусматривать комфортную работу в автономном режиме, поскольку около 50% от устройства не подключены к в расположениях розничной торговли. Так покупатели, взаимодействующие с вашим приложением, все равно смогут получить положительные впечатления от работы с ним.

* **Обновленные взаимодействие**. Ваше приложение не должно быть запрос обновлений при подключении к сети. Если обновления, они должны выполняться без вмешательства пользователя.
...
* **Возраст соответствующего содержимого**. Все содержимое приложения должно быть ниже возрастную категорию или «для подростков». Дополнительные сведения, см. в разделе [Получение приложения с оценкой by IARC](https://www.globalratings.com/for-developers.aspx) и [ESRB ratings](https://www.esrb.org/ratings/ratings_guide.aspx).
...
* **Соответствие требованиям к размеру приложений магазина розничной торговли**. Размер приложения не должен превышать 800МБ. Обратитесь в службу розничного магазина Windows непосредственно для обсудить этот вопрос, если RDX-учитывать приложение не соответствует требованиям размер.
...
Свойство [**IsDemoModeEnabled**](https://docs.microsoft.com/uwp/api/windows.system.profile.retailinfo.isdemomodeenabled) в классе служебную программу [**RetailInfo**](https://docs.microsoft.com/uwp/api/Windows.System.Profile.RetailInfo) , который входит в пространство имен [Windows.System.Profile](https://docs.microsoft.com/uwp/api/windows.system.profile) в Windows 10 SDK, используется в качестве логического индикатора для указания какому пути кода работает приложение — обычный _ _режима или режима _розничной торговли_ .
...
Очистка начинается двух минут после остановки покупатель взаимодействует с устройством. Воспроизведение демонстрации в розничных магазинах, и Windows начнется, сбросе любой демонстрационные данные в контакты, фотографий и других приложений. В зависимости от устройства это может занять 1-5 минут, чтобы полностью восстановить все обычный. Это гарантирует, что каждый пользователь в розничном магазине можно к устройству и быть одинаковым при взаимодействии с устройством.
...
Для хранения данных между сеансами пользователя, можно хранить сведения в __ApplicationData.Current.TemporaryFolder__ , поскольку процесс очистки по умолчанию автоматически не удаляет данные в этой папке. Обратите внимание, что в процессе очистки удаляется информация, хранимая с помощью *LocalState* .
...
Сценарии, где требуется логику пользовательского очистки включает в себя многочисленные настройкой, скачивание и кэширование данных или не хотят *LocalState* данных.
