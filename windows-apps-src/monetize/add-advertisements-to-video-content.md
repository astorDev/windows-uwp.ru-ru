...
5.  Откройте файл index.html (или другой html-файл, если это необходимо для вашего проекта). В разделе `<head>` после ссылок на JavaScript проекта из default.css и main.js добавьте ссылку на ad.js и adscheduler.js.
...
Это свойство Возвращает или задает количество миллисекунд ожидания для запроса отклик на рекламу до истечения времени ожидания. Значение 0 сообщает системе, что время ожидания не ограничено. Значение по умолчанию — 30000 мс (30 секунд).
...
* **Ход выполнения**: Состояние воспроизведения ad (один из **MediaProgress** значений перечисления, определенных в AdScheduler.js).
* **клипов**: Видеоклип, это действие выполняется. Данный объект не предназначен для использования в коде.
* **adPackage**: Объект, представляющий часть полезных данных ad, соответствующий в AD, который играет. Данный объект не предназначен для использования в коде.
...
* **remainingAdTime**: Количество секунд, оставшийся для текущего рекламы.
* **remainingPodTime**: Количество секунд, оставшийся для текущего модуля.
...
* **время начала**: Время начала pod, в секундах.
* **POD**: Объект, представляющий pod. Данный объект не предназначен для использования в коде.
