...
| startDate | date | Начальная дата диапазона дат, для которого требуется получить подробные данные об ошибках. По умолчанию используется текущая дата минус 30 дней.<p/><p/>**Примечание:**&nbsp;&nbsp;этот метод только можно получить подробные сведения об ошибках, произошедших за последние 30 дней. |  Нет  |
