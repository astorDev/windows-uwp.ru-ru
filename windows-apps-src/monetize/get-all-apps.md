...
ms.locfileid: "8968568"
---
# <a name="get-all-apps"></a>Получение всех приложений


Используйте этот метод в API отправки Microsoft Store для получения данных для приложений, которые зарегистрированы в вашей учетной записи центра партнеров.
...
Для данного метода все параметры запроса являются необязательными. При вызове этого метода без параметров ответ содержит данные для первых 10 приложений, зарегистрированных в вашей учетной записи.
...
В следующем примере демонстрируется способ извлечения информации о всех приложениях, которые зарегистрированы в вашей учетной записи. Во-первых получите первые 10 приложений:
...
Затем рекурсивно вызов `GET https://manage.devcenter.microsoft.com/v1.0/my/{@nextLink}` до `{@nextlink}` имеет значение null или не существует в ответе. Пример.
...
Если вы уже знаете общее число приложений, которые имеются в вашей учетной записи, можно просто передать этот номер в параметре **верхней** для получения информации о всех ваших приложений.
...
| 409  | Приложения используют функции центра партнеров, которые [в настоящее время не поддерживается API отправки в Microsoft Store](create-and-manage-submissions-using-windows-store-services.md#not_supported).  |
