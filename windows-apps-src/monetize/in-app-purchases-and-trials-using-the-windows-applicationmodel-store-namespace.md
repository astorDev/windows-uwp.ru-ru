...
Файл WindowsStoreProxy.xml создается по умолчанию по следующему адресу: %UserProfile%\AppData\Local\Packages\\&lt;папки пакета приложения&gt;\LocalState\Microsoft\Windows Store\ApiData. Вы можете изменить этот файл, чтобы определить сценарий, который требуется смоделировать в свойствах **CurrentAppSimulator**.
...
После этого изменения, внесенные в WindowsStoreProxy.xml, будут зависеть от схемы. Дополнительные сведения см. в разделе [как: Выбрать схему XML для использования](https://go.microsoft.com/fwlink/p/?LinkId=403014).
...
|  Элемент  |  Обязательно  |  Количество  |  Описание   |
|-------------|------------|--------|--------|
|  [Приложение](#app-child-of-listinginformation)  |    Да   |  1   |    Предоставляет данные о приложении.         |
...
|  Элемент  |  Обязательно  |  Количество  | Описание   |
|-------------|------------|--------|--------|
|  **Идентификатор приложения**  |    Да   |  1   |   GUID, определяющий приложение в Магазине. Это может быть любой GUID для тестирования.        |
...
|  **AgeRating**  |    Да  |  1   |     Целое число, представляющее минимальную возрастную категорию приложения. Это то же значение, которое следует указать в центре партнеров при отправке приложения. Доступны следующие значения, используемые Store. 3, 7, 12 и 16. Подробнее об этих категориях см. в разделе [Возрастные категории](../publish/age-ratings.md).        |
...
|  **Тег**  |    Нет  |   0 или 1   |      [Пользовательские данные, определенные разработчиком](../publish/enter-add-on-properties.md#custom-developer-data) (также называются тегом) для надстройки.       |
...
|  Элемент  |  Обязательно  |  Количество  | Описание   |
|-------------|------------|--------|--------|
|  [Приложение](#app-child-of-licenseinformation)  |    Да   |  1   |    Описывает лицензию приложения.         |
...
|  **Значение HResult**  |     Да       |   Назначьте этот атрибут одному из значений перечисления, показанных для типа **ResponseCodes** в [схеме](#schema). Каждое из этих значений перечисления представляет код ошибки, который требуется вернуть для метода, назначенного атрибуту **MethodName** для этого элемента **DefaultResponse**.           |
