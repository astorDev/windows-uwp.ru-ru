...
* Подготовка приложения для использования с API отправки в Microsoft Store.
  ...
  * Если приложение еще не существует в центре партнеров, вы должны [создать приложение, резервируя его имя в центре партнеров](https://msdn.microsoft.com/windows/uwp/publish/create-your-app-by-reserving-a-name). Нельзя использовать API отправки Microsoft Store для создания приложения в центре партнеров; необходимо работать в центре партнеров для его создания, и затем после этого можно использовать API для доступа к приложению и программным способом создания отправки для него. Однако API можно использовать для программного создания надстроек и тестовых пакетов, прежде чем вы начнете создавать для них отправки.

  * Перед созданием отправки для заданного приложения с помощью этого API, необходимо сначала [Создание Отправка приложения в центре партнеров](https://msdn.microsoft.com/windows/uwp/publish/app-submissions), включая ответы на [age оценки](https://msdn.microsoft.com/windows/uwp/publish/age-ratings) анкеты. После этого можно будет программным образом создавать новые отправки для этого приложения с помощью API. Необходимость в создании отправки надстройки или отправки тестового пакета перед началом использования API для этих типов отправок отсутствует.
