...
| 0x803F6107   | Непредвиденный произошла неизвестная ошибка при обработке текущей операции.                                             |
...
| 0x803F6109   | Выполнение потребляемой надстройки не может быть завершена, так как указанное количество больше, чем оставшийся баланс.        |
...
|  Код ошибки  |  Описание  |
|--------------|---------------|
| 0x803F700C   | Устройство находится в автономном режиме. Использовать это приложение, когда устройство находится в автономном режиме, откройте параметров магазина и включить настройку **Автономного разрешения** .            |
...
| 0x803F9006    |  Операция не может быть завершена, так как пользователь, который имеет право на этот продукт не выполнил вход под своей учетной записи Майкрософт.            |
| 0x803F9008<br/>0x803F9009    |  Устройство находится в автономном режиме. Устройство должно подключение к Интернету для работы с продуктом.            |
...
| 0x803F6206   | Для загрузки и установки пакета обновления, так как скачивание будет выполняться на измеряемого сетевого подключения требуется согласие пользователя.     |
