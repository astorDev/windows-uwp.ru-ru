...
|---------------|--------|---------------|------|
| applicationId | string | КОД продукта для классического приложения, для которого требуется получить аналитических данных. Чтобы получить код продукта для классического приложения, откройте любой [аналитический отчет для классического приложения в центре партнеров](https://msdn.microsoft.com/library/windows/desktop/mt826504) (например, **отчет о работоспособности**) и получите код продукта из URL-адреса. Если этот параметр не задан, тело ответа будет содержать аналитических данных для всех приложений, зарегистрированных в вашей учетной записи.  |  Нет  |
...
| SubDimensions         | array |  Один или несколько объектов, описывающие один метрики для аналитические сведения.   |
...
| DimensionValue              | string | Значение метрики, описанное в текущего измерения. Например если **имя измерения** **EventType**, **DimensionValue** может быть **Сбой** или **зависание**.   |
| FactValue     | string | Абсолютное значение метрики в аналитические был обнаружен дату.  |
