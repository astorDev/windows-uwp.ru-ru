...
Для проверки подлинности квитанции серверная система (веб-служба или аналогичный объект) должна проверить подпись на квитанции с помощью открытого сертификата. Чтобы получить этот сертификат, используйте URL- ```https://lic.apps.microsoft.com/licensing/certificateserver/?cid=CertificateId%60%60%60, where ```адрес CertificateId ' ' ' в качестве значения **CertificateId** в уведомлении.
...
|  [ProductReceipt](#productreceipt)  |     нет       |  0 или больше    |   Содержит сведения о покупке из приложения для текущего приложения.     |
|  Подпись  |      Да      |  1   |   Этот элемент — стандартная [конструкция XML-DSIG](https://www.w3.org/TR/xmldsig-core/). Он содержит элемент **SignatureValue** с подписью, который можно использовать для проверки квитанции, а также элемент **SignedInfo**.      |
...
|  **ИД**  |     Значение имени семейства пакетов, используемое ОС для приложения.           |
...
|-------------|-------------------|
|  **Id**  |    Идентифицирует покупку.           |
|  **ИД**  |     Определяет приложение, через которое пользователь совершил покупку.           |
|  **Кодом**  |     Определяет приобретенный продукт.           |
|  **продукттипе**  |    Определяет тип продукта. В настоящее время поддерживает только значение **Durable**.          |  
