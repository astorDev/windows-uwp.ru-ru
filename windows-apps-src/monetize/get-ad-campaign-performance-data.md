...
Этот метод возвращает те же данные, предоставляемые [отчет о кампании Ad](../publish/app-install-ads-reports.md) в центре партнеров. Дополнительные сведения о рекламных кампаниях см. в разделе [Создание рекламной кампании для вашего приложения](../publish/create-an-ad-campaign-for-your-app.md).
...
|  groupby  |  строка   |  <p>Оператор, который применяет агрегирование данных только к указанным полям. Можно указать следующие поля:</p><ul><li><strong>campaignId</strong></li><li><strong>Идентификатор приложения</strong></li><li><strong>date</strong></li><li><strong>currencyCode</strong></li></ul><p>Параметр <em>groupby</em> можно использовать вместе с параметром <em>aggregationLevel</em>. Например: <em>&amp;groupby=applicationId&amp;aggregationLevel=week</em></p>   |   Нет    |
...
* [Создайте рекламную кампанию для своего приложения](https://docs.microsoft.com/windows/uwp/publish/create-an-ad-campaign-for-your-app)
