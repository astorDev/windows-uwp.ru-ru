...
<span id="Automation_peer_classes__"></span><span id="automation_peer_classes__"></span><span id="AUTOMATION_PEER_CLASSES__"></span>Одноранговые классы автоматизации
-----------------------------------------------------------------------------------------------------------------------------------------------------------
...
### <span id="Guidance_for_how_to_implement_control_patterns"></span><span id="guidance_for_how_to_implement_control_patterns"></span><span id="GUIDANCE_FOR_HOW_TO_IMPLEMENT_CONTROL_PATTERNS"></span>Рекомендации по реализации шаблонов элементов управления
...
<span id="Built-in_automation_peer_classes"></span><span id="built-in_automation_peer_classes"></span><span id="BUILT-IN_AUTOMATION_PEER_CLASSES"></span>Встроенные одноранговые классы автоматизации
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
<span id="UI_Automation_and_UWP_process_boundaries"></span><span id="ui_automation_and_uwp_process_boundaries"></span><span id="UI_AUTOMATION_AND_UWP_PROCESS_BOUNDARIES"></span>Ограничения процесса модели автоматизации пользовательского интерфейса и платформы UWP
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
### <span id="Choosing_the_correct_peer_base_class"></span><span id="choosing_the_correct_peer_base_class"></span><span id="CHOOSING_THE_CORRECT_PEER_BASE_CLASS"></span>Выбор нужного базового однорангового класса
...
В случае наследования непосредственно от [**ContentControl**](https://msdn.microsoft.com/library/windows/apps/BR209365) у класса не будет поведения однорангового элемента автоматизации по умолчанию, так как отсутствует реализация [**OnCreateAutomationPeer**](https://msdn.microsoft.com/library/windows/apps/BR208911_oncreateautomationpeer), которая ссылается на одноранговый класс. Поэтому следует реализовать **OnCreateAutomationPeer** для использования собственного однорангового элемента или использовать [**FrameworkElementAutomationPeer**](https://msdn.microsoft.com/library/windows/apps/BR242472) в качестве однорангового элемента, если такой уровень поддержки специальных возможностей достаточен для вашего элемента управления.

**Примечание.** В большинстве случаев следует проводить наследование не от [**AutomationPeer**](https://msdn.microsoft.com/library/windows/apps/BR209185), а от [**FrameworkElementAutomationPeer**](https://msdn.microsoft.com/library/windows/apps/BR242472). В случае наследования непосредственно от **AutomationPeer** вам потребуется дублировать поддержку основных специальных возможностей, которые уже содержатся в **FrameworkElementAutomationPeer**.

 

<span id="Initialization_of_a_custom_peer_class"></span><span id="initialization_of_a_custom_peer_class"></span><span id="INITIALIZATION_OF_A_CUSTOM_PEER_CLASS"></span>Инициализация пользовательского однорангового класса
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
<span id="Core_methods_of_AutomationPeer"></span><span id="core_methods_of_automationpeer"></span><span id="CORE_METHODS_OF_AUTOMATIONPEER"></span>Основные (Core) методы AutomationPeer
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Как минимум, при каждом определении нового однорангового класса следует реализовать метод [**GetClassNameCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getclassnamecore), как показано в следующем примере.
...
**Примечание.** Вы можете сохранить строки в виде констант, а не непосредственно в теле метода. Решение остается за вами. Для [**GetClassNameCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getclassnamecore) локализовывать эту строку не нужно. Всякий раз, когда локализованная строка требуется клиенту модели автоматизации пользовательского интерфейса, используется свойство **LocalizedControlType**, а не **ClassName**.
...
### <span id="Forwarding_patterns_from_subelements"></span><span id="forwarding_patterns_from_subelements"></span><span id="FORWARDING_PATTERNS_FROM_SUBELEMENTS"></span>Пересылка шаблонов из подэлементов

Реализация метода [**GetPatternCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getpatterncore) может также указывать подэлемент или часть в качестве поставщика шаблона для своего основного элемента. В этом примере имитируется передача элементом [**ItemsControl**](https://msdn.microsoft.com/library/windows/apps/BR242803) обработки шаблона прокрутки одноранговому элементу внутреннего элемента управления [**ScrollViewer**](https://msdn.microsoft.com/library/windows/apps/BR209527). Чтобы указать подэлемент для обработки шаблона, этот код получает объект подэлемента, создает для подэлемента одноранговый элемент с помощью метода [**FrameworkElement.CreatePeerForElement**](https://msdn.microsoft.com/library/windows/apps/BR242472_createpeerforelement) и возвращает новый одноранговый элемент.
...
-   [**GetBoundingRectangleCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getboundingrectanglecore): возвращает структуру [**Rect**](https://msdn.microsoft.com/library/windows/apps/BR225994) на основании известных характеристик макета. Возвращает нулевое значение **Rect**, если [**IsOffscreen**](https://msdn.microsoft.com/library/windows/apps/BR209185_isoffscreen) имеет значение **true**.
-   [**GetClickablePointCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getclickablepointcore): возвращает структуру [**Point**](https://msdn.microsoft.com/library/windows/apps/BR225870) на основании известных характеристик макета, пока **BoundingRectangle** имеет значение, не равное 0.
-   [**GetNameCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getnamecore): более комплексное поведение, которое сложно описать здесь; подробное описание см. здесь: [**GetNameCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getnamecore). Метод, в сущности, делает попытку строкового преобразования любого известного содержимого [**ContentControl**](https://msdn.microsoft.com/library/windows/apps/BR209365) или связанных классов с содержимым. Кроме того, если у [**LabeledBy**](https://msdn.microsoft.com/library/windows/apps/Hh759769) есть значение, значение **Name** этого элемента используется как **Name**.
-   [**HasKeyboardFocusCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_haskeyboardfocuscore): оценивается на основании свойств владельца [**FocusState**](https://msdn.microsoft.com/library/windows/apps/BR209390_focusstate) и [**IsEnabled**](https://msdn.microsoft.com/library/windows/apps/BR209390_isenabled). Элементы, не являющиеся элементами управления, всегда возвращают **false**.
-   [**IsEnabledCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_isenabledcore): оценивается на основании свойства [**IsEnabled**](https://msdn.microsoft.com/library/windows/apps/BR209390_isenabled) владельца, если это [**Control**](https://msdn.microsoft.com/library/windows/apps/BR209390). Элементы, не являющиеся элементами управления, всегда возвращают **true**. Это не значит, что владелец активен в том смысле, что с ним можно взаимодействовать. Это значит, что одноранговый элемент активен несмотря на то, что владелец не имеет свойства **IsEnabled**.
-   [**IsKeyboardFocusableCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_iskeyboardfocusablecore): возвращает **true**, если владелец — [**Control**](https://msdn.microsoft.com/library/windows/apps/BR209390); в противном случае возвращает **false**.
-   [**IsOffscreenCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_isoffscreencore): [**Visibility**](https://msdn.microsoft.com/library/windows/apps/BR208911_visibility) со значением [**Hidden**](https://msdn.microsoft.com/library/windows/apps/BR209006) в элементе владельца или любом из его родительских элементов приравнивается к значению**true** для [**IsOffscreen**](https://msdn.microsoft.com/library/windows/apps/BR209185_isoffscreen). Исключение: объект [**Popup**](https://msdn.microsoft.com/library/windows/apps/BR227842) может быть видимым, даже если родительские элементы его владельца скрыты.
-   [**SetFocusCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_setfocuscore): вызывает [**Focus**](https://msdn.microsoft.com/library/windows/apps/BR209390_focus).
-   [**GetParent**](https://msdn.microsoft.com/library/windows/apps/BR209185_getparent): вызывает [**FrameworkElement.Parent**](https://msdn.microsoft.com/library/windows/apps/BR208739) владельца и ищет подходящий одноранговый элемент. Эта пара не переопределяется с помощью метода Core, поэтому вы не сможете изменить ее поведение.

**Примечание.** Одноранговые элементы UWP по умолчанию реализуют поведение с помощью внутреннего собственного кода, который реализует UWP (не обязательно с помощью фактического кода UWP). Вы не сможете увидеть код или логику реализации с помощью отражения среды CLR или других приемов. Вы также не сможете увидеть конкретные справочные страницы для переопределений подклассов реакций базовых одноранговых элементов на события. Например, может существовать дополнительное поведение для метода [**GetNameCore**](https://msdn.microsoft.com/library/windows/apps/BR209185_getnamecore) элемента [**TextBoxAutomationPeer**](https://msdn.microsoft.com/library/windows/apps/BR242550), которое не будет описано на справочной странице **AutomationPeer.GetNameCore**, а справочная страница для **TextBoxAutomationPeer.GetNameCore** отсутствует. Нет даже справочной страницы **TextBoxAutomationPeer.GetNameCore**. Вместо этого изучите справочный раздел по классу ближайшего однорангового элемента и найдите примечания о реализации в разделе «Примечания».

 

<span id="Peers_and_AutomationProperties"></span><span id="peers_and_automationproperties"></span><span id="PEERS_AND_AUTOMATIONPROPERTIES"></span>Одноранговые элементы и AutomationProperties
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Типичная реализация подразумевает, что API поставщика сначала вызывают [**Owner**](https://msdn.microsoft.com/library/windows/apps/BR242472_owner) для получения доступа к экземпляру элемента управления во время выполнения. Затем на этом объекте можно вызывать необходимый метод поведения.
