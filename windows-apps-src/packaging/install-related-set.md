...
Если основной пакет приложения является файлом .appxbundle, используйте `<MainBundle>`, как показано ниже. Если основной пакет приложения является файлом .appx file, используйте `<MainPackage>` вместо `<MainBundle>` в фрагменте кода. 
...
В файле Установщика приложений можно также указать параметр обновления, чтобы связанные наборы автоматически обновлялись при публикации нового файла Установщика приложений. Элемент **<UpdateSettings>** необязателен. 
``` xml
<?xml version="1.0" encoding="utf-8"?>
<AppInstaller
    xmlns="http://schemas.microsoft.com/appx/appinstaller/2017"
