...
После создания пакета зависимостей настроить шаги 1 – 4, вы можете продолжить разработку обычным образом. Если вы хотите загрузки кода из дополнительного пакета в главный пакет, необходимо создать связанный набор. Дополнительные сведения в разделе [задает соответствующие](#related_sets) .
...
Теперь каждый раз, когда ввод F5 или Создание проекта с помощью дополнительного пакета, Visual Studio создаст сначала проект основного пакета. Это позволит гарантировать, что основного проекта и дополнительных проектов будут синхронизированы.
...
3. В `Bundle.Mapping.txt` файл, укажите относительные пути к любой дополнительный пакет проекты или внешние пакеты. Этот пример `Bundle.Mapping.txt` файл должен выглядеть примерно следующим образом:
...
Обратите внимание, что как и дополнительные пакеты `Bundle.Mapping.txt` файл для связанных наборов будет работать только на Windows 10 версии 1703. Кроме того Минимальная версия вашего приложения целевой платформы необходимо установить значение 10.0.15063.0.
