...
9. Если приложение успешно упаковано, вы увидите это диалоговое окно и отправки файла пакета приложения можно получить из указанное выходное расположение. На этом этапе вы можете [проверка пакета приложения на локальном компьютере или удаленном компьютере](#validate-your-app-package) и [автоматизировать отправка в магазин](#automate-store-submissions).
...
### <a name="validate-your-app-package-locally"></a>Проверка пакета приложения локально
...
1. В окончательном **создание пакетов завершено** странице **создание пакетов приложения** мастера оставьте **локального компьютера** установленным и нажмите кнопку **запуска Комплект сертификации приложений для Windows**. Подробнее о тестировании приложений с помощью комплекта сертификации приложений для Windows см. в разделе [Комплект сертификации приложений для Windows](https://docs.microsoft.com/windows/uwp/debug-test-perf/windows-app-certification-kit).
    ...
    Комплект сертификации приложений Windows (WACK) выполняет различные тесты и возвращает результаты. Подробные сведения см. в разделе [Тесты из комплекта сертификации приложений для Windows](https://docs.microsoft.com/windows/uwp/debug-test-perf/windows-app-certification-kit-tests).
...
1. Включите устройство Windows 10 для разработки, выполнив [включить устройство для разработки](https://docs.microsoft.com/windows/uwp/get-started/enable-your-device-for-development) инструкции.
    >[!IMPORTANT]
...
## <a name="automate-store-submissions"></a>Автоматизация отправки Store

Начиная с Visual Studio 2019 г., вы можете отправить созданный appxupload-файл Microsoft Store прямо из IDE, выбрав **автоматически отправить в Microsoft Store после проверки комплект сертификации приложений для Windows** параметр в конце [мастера создания пакетов приложения](#create-your-app-package-upload-file-using-visual-studio). Эта функция использует Azure Active Directory для доступа к сведения учетной записи центра партнеров, необходимые для публикации приложения. Чтобы использовать эту функцию, вы должны связать Azure Active Directory с помощью учетной записи центра партнеров и получить несколько учетные данные, необходимые для отправки.
...
Чтобы вы могли получать учетные данные, которые требуются для автоматической отправки Store, сначала выполните следующие действия в [мониторинга центра партнеров](https://partner.microsoft.com/dashboard) Если вы еще не сделали.

1. [Связать учетную запись центра партнеров с вашей организации Azure Active Directory](https://docs.microsoft.com/windows/uwp/publish/associate-azure-ad-with-partner-center). Если ваша организация уже использует Office 365 или другие бизнес-службы от Майкрософт, то у вас уже есть Azure AD. В противном случае можно создать новый клиент Azure AD из в центре партнеров без дополнительной платы.

2. [Добавление приложения Azure AD для вашей учетной записи центра партнеров](https://docs.microsoft.com/windows/uwp/publish/add-users-groups-and-azure-ad-applications#add-azure-ad-applications-to-your-partner-center-account). Это приложение Azure AD представляет приложения или службы, который будет использоваться для доступа к отправленные данные для учетной записи центра разработчиков. Необходимо назначить этому приложению **Manager** роли. Если приложение уже есть в каталоге Azure AD, вы можете выбрать его на странице **Добавление приложений Azure AD**, чтобы добавить его к учетной записи Центра разработки. Также можно создать приложение Azure AD на странице **Добавление приложений Azure AD**.

### <a name="retrieve-the-credentials-required-for-submissions"></a>Получить учетные данные, необходимые для отправки

После этого можно извлечь учетные данные центра партнеров, необходимые для отправки: **идентификатор клиента Azure**, **идентификатор клиента** и **ключ клиента**.

1. Перейдите к [мониторинга центра партнеров](https://partner.microsoft.com/dashboard) и войдите, используя свои учетные данные Azure AD.

2. На панели мониторинга центра партнеров выберите значок колокольчика (правом верхнем углу панели мониторинга), а затем выберите **параметров разработчика**.

3. В **параметры** меню в левой области щелкните **пользователей**.

4. Щелкните имя приложения Azure AD, чтобы перейти к параметрам приложения. На этой странице скопируйте **Арендатора** и **идентификатор клиента** значения.

5. В **ключи** щелкните **добавить новый ключ**. На следующем экране скопируйте **ключ** значение, которое соответствует секрету клиента. Вы не сможете снова получить доступ к этой информации после закрыть эту страницу, поэтому убедитесь, что не потерять его. Дополнительные сведения см. в разделе [Управление ключами для приложения Azure AD](https://docs.microsoft.com/windows/uwp/publish/add-users-groups-and-azure-ad-applications#manage-keys-for-an-azure-ad-application).

### <a name="configure-automatic-store-submissions-in-visual-studio"></a>Настройка автоматической отправки Store в Visual Studio

После выполнения предыдущих шагов, можно настроить автоматическое Store отправки в Visual Studio 2019.

1. В конце [мастера создания пакетов приложения](#create-your-app-package-upload-file-using-visual-studio)выберите **автоматически отправить в Microsoft Store после проверки комплект сертификации приложений для Windows** и нажмите кнопку **перенастроить**.

2. В **параметры настройки отправки Microsoft Store** диалоговое окно, введите ключ клиента Azure идентификатор, идентификатор клиента и клиента.

    ![Настроить параметры отправки Microsoft Store](images/packaging-screen8.jpg)

    > [!Important]
    > Учетные данные могут сохраняться в свой профиль для использования в будущих отправок

3. Нажмите кнопку **ОК**.

Отправка начнется после завершения теста WACK. Можно отслеживать ход выполнения отправки в **проверить и опубликовать** окна.

![Проверка и выполнение публикации](images/packaging-screen9.jpg)
...
Представленный в Windows 10 Anniversary Update (Windows 10, версия 1607), пакеты приложения могут устанавливаться просто дважды щелкнув файл пакета приложения. Для этого перейдите в пакет приложения или файл пакета приложения и дважды щелкните его. [Установщик приложения](https://docs.microsoft.com/windows/msix/app-installer/app-installer-root) запускает и предоставляет сведения базовое приложение, а также кнопку "установить", индикатор выполнения установки и все соответствующие сообщения об ошибках.

![Окно установщика приложения при установке образца приложения под названием Contoso](images/appinstaller-screen.png)

> [!NOTE]
> Установщик приложения предполагается, что приложение является доверенным для устройства. Если вы являетесь разрабатываете собственное или корпоративное приложение без его публикации, вам потребуется установить сертификат подписи в хранилище центров сертификации "Доверенные люди" или "Доверенные издатели" на устройстве. Если вы не знаете, как это сделать, см. раздел [Установка тестовых сертификатов](https://docs.microsoft.com/windows-hardware/drivers/install/installing-test-certificates).
