...
9. Если приложение успешно упаковано, вы увидите это диалоговое окно и отправки файла пакета приложения можно получить из указанное выходное расположение. На этом этапе вы можете [проверка пакета приложения на локальном компьютере или удаленном компьютере](#validate-your-app-package) и [автоматизации отправки в магазин](#automate-store-submission).
...
1. В окончательном **создание пакетов завершено** странице **создание пакетов приложения** мастера оставьте **локального компьютера** установленным и нажмите кнопку **запуска Комплект сертификации приложений для Windows**. Подробнее о тестировании приложений с помощью комплекта сертификации приложений для Windows см. в разделе [Комплект сертификации приложений для Windows](https://docs.microsoft.com/windows/uwp/debug-test-perf/windows-app-certification-kit).
    ...
    Комплект сертификации приложений для Windows выполняет различные тесты и отображает их результаты. Подробные сведения см. в разделе [Тесты из комплекта сертификации приложений для Windows](https://docs.microsoft.com/windows/uwp/debug-test-perf/windows-app-certification-kit-tests).
...
### <a name="automate-store-submission"></a>Автоматизация отправки в магазин

Начиная с Visual Studio 2019 г., вы можете отправить созданный appxupload-файл Microsoft Store прямо из IDE, выбрав **автоматически отправить в Microsoft Store после проверки комплект сертификации приложений для Windows** параметр. Необходимо указать учетные данные, используемые для передачи. Эти учетные данные состоят из трех частей: **идентификатор клиента Azure**, **идентификатор клиента** и **секрет клиента**. Выполните следующие действия для их получения.

1. В центре партнеров, перейдите к вашей **параметров разработчика**, нажмите кнопку **пользователей**и свяжите учетную запись центра разработки для вашей организации в каталоге Azure AD вашей организации. Подробные инструкции см. в разделе [Управление учетными записями пользователей](https://docs.microsoft.com/windows/uwp/publish/manage-account-users).
2. В **пользователей** щелкните **добавить приложения Azure AD**добавьте приложение Azure AD, которое представляет приложение или служба будет использовать для доступа к отправленные данные для учетной записи центра разработчиков и назначьте его Роль Manager. Если это приложение уже существует в каталоге Azure AD, его можно выбрать на странице приложения добавить Azure AD, чтобы добавить его в учетной записи центра разработчиков. Также можно создать приложение Azure AD на странице **Добавление приложений Azure AD**. Дополнительные сведения см. в разделе [добавьте пользователей, группы и приложения Azure AD для вашей учетной записи центра партнеров](/windows/uwp/publish/add-users-groups-and-azure-ad-applications).
3. Вернитесь к **Управление пользователями** щелкните имя приложения Azure AD, чтобы перейти на параметры приложения и скопируйте **Арендатора** и **идентификатор клиента** значения.
4. Нажмите кнопку **Добавить новый ключ**. На следующем экране скопируйте значение ключа, которое соответствует секрету клиента. Вы не сможете снова получить доступ к этой информации после закрыть эту страницу, поэтому убедитесь, что не потерять его. Дополнительные сведения см. сведения об управлении ключами в [добавьте пользователей, группы и приложения Azure AD для вашей учетной записи центра партнеров](/windows/uwp/publish/add-users-groups-and-azure-ad-applications).
    ![Проверка и выполнение публикации](images/packaging-screen8.jpg)
    > [!Important] 
