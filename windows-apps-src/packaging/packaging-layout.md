...
Эта структура упаковки создает один плоский файл .appxbundle с пакетом архитектуры x64 и пакетом активов "Мультимедиа". 
...
Элемент **PrebuiltPackage** служит для добавления пакетов, которые не определены во включаемой структуре упаковки и ссылки на которые нет в создаваемых файлах .appxbundle. В этом случае сюда будет включен другой дополнительный пакет DLC, чтобы основной файл .appxbundle мог на него сослаться и сделать его частью соответствующего набора.
