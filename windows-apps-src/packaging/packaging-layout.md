...
Этот макет упаковки будет создан файл пакета плоского приложение с одним с x x64 пакете с архитектурой и пакет средств «Мультимедиа». 
...
**PrebuiltPackage** элемент используется для добавления пакетов, которые не определены в макете упаковки нужно включить или ссылка в файле пакета приложения для построения. В этом случае другой дополнительный пакет DLC включено здесь, чтобы файл основного приложения пакета можно сослаться на него, который будет входить в соответствующем наборе.
