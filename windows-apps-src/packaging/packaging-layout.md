...
Эта структура упаковки создает файл одно приложение плоский пакет с x x64 пакет архитектуры и пакетом активов «Мультимедиа». 
...
Элемент **PrebuiltPackage** используется для добавления пакетов, которые не определены в структуре упаковки и ссылки на файлы пакета приложения необходимо создать. В этом случае другой дополнительный пакет DLC включается здесь, чтобы файл пакета основного приложения могут ссылаться на него и сделать его частью соответствующего набора.
