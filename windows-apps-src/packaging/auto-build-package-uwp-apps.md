...
**Добавление сертификата проекта в репозитории исходного кода**
...
|AppxBundle|Всегда|Создает пакет appxbundle с APPX-файлами для указанной платформы.|
...
|**Тип сборки**|**Артефакт**|**Рекомендуемые частота**|**Описание**|
|-----------------|------------|-------------------------|---------------|
...
>[!NOTE]
> Если в одном пакете решения объединены несколько приложений, может возникнуть ошибка. В следующем разделе для помощи в устранении этой ошибки: [Устраните ошибки, которые отображаются при создании набора сразу несколько приложений в одном решении.](#bundle-errors)
...
Поскольку сборка CI не развертывается для пользователей, рекомендуется применять разные номера версий во избежание путаницы со сборками непрерывного развертывания (CD). Пример: `$(BuildDefinitionName)_0.0.$(DayOfYear)$(Rev:.r)`
...
Модульные тесты UWP выполняются в контексте данного APPXRECIPE-файла, поэтому использовать созданный пакет невозможно. Кроме того, вам необходимо указать путь к APPXRECIPE-файлу для конкретной платформы. Например:

```ps
$(Build.ArtifactStagingDirectory)\AppxPackages\MyUWPApp.UnitTest\x86\MyUWPApp.UnitTest_$(AppxVersion)_x86.appxrecipe
```

Чтобы выполнить тесты, параметр консоли потребуется добавить в файл vstest.console.exe. Этот параметр можно указать через: **Параметры выполнения = > другие параметры консоли**. Добавьте следующий параметр:
...
>[!NOTE]
> Используйте следующую команду для выполнения модульных тестов локально из командной строки: `"%ProgramFiles(x86)%\Microsoft Visual Studio 14.0\Common7\IDE\CommonExtensions\Microsoft\TestWindow\vstest.console.exe"`
...
После завершения сборок этого типа пользователи могут скачать файл пакета приложения из раздела артефакты страницы результатов сборки.
...
#### <a name="optional-integrate-with-hockeyapp"></a>Дополнительно Интеграция с HockeyApp
...
Затем настройте подключение HockeyApp с помощью этого руководства: [Сведения об использовании HockeyApp с Visual Studio Team Services (VSTS) или Team Foundation Server (TFS).](https://support.hockeyapp.net/kb/third-party-bug-trackers-services-and-webhooks/how-to-use-hockeyapp-with-visual-studio-team-services-vsts-or-team-foundation-server-tfs)
Для настройки учетной записи HockeyApp вы можете воспользоваться учетной записью Майкрософт, учетной записью социальных сетей или электронным адресом. Бесплатный тарифный план предоставляет два приложения и одного владельца без ограничения данных.

Затем приложение HockeyApp можно создать вручную, или загрузить существующий файл пакета приложения. Дополнительные сведения см. в разделе [Создание приложения.](https://support.hockeyapp.net/kb/app-management-2/how-to-create-a-new-app)

Чтобы использовать существующий файл пакета приложения, добавьте шаг сборки и задайте для параметра двоичные путь к файлу шага построения.
...
Несмотря на то, что задача HockeyApp можно указать путь к файлу символов, рекомендуется включить символы с пакетом.
...
Если у вас нет доступа к учетной записи центра партнеров, который использовался для публикации приложения, необходимо выполнить инструкции в этом документе: [Сборка приложения для третьих сторон? Упаковка приложения Store](https://blogs.windows.com/buildingapps/2015/12/15/building-an-app-for-a-3rd-party-how-to-package-their-store-app/#e35YzR5aRG6uaBqK.97).
...
Это создаст файл загрузки, который можно отправить в Store.
...
Необходимо подключить свою учетную запись центра партнеров с Azure Active Directory (AD), а затем создайте приложение в службе AD для проверки подлинности запросов. Для этого следуйте рекомендациям на странице расширения.

После настройки расширения, можно добавить задачу сборки и настроить его, используя идентификатор приложения и расположение файла для отправки.
...
Используйте сценарий PowerShell `Add-AppDevPackage.ps1` для установки приложений. Этот сценарий будет добавить сертификат в разделе доверенного корневого центра сертификации для локального компьютера и затем установить или обновить файл пакета приложения.
...
В юбилейном обновлении Windows 10 можно дважды щелкните файл пакета приложения и установить приложения в диалоговом окне "нажмите кнопку" установить ".
...
Если вы хотите распространить пакеты приложений Windows с веб-сайта, таких как VSTS или HockeyApp, необходимо добавить в список надежных сайтов в браузере. В противном случае Windows отметит файл как заблокированный.
...
|**Машины**|**Использование**|**Сертификат**|**Сертификат Store**|
|-----------|---------|---------------|---------------------|
|Компьютер разработчика или построения|Подпись сборок|MyCert.PFX|Хранилище текущего пользователя или личное хранилище|
|Компьютер разработчика или построения|Выполните следующую команду:|MyCert.cer|Локальный компьютер/доверенные лица|
|Пользователь|Выполните следующую команду:|MyCert.cer|Локальный компьютер/доверенные лица|

>Примечание. Можно также использовать корпоративный сертификат, который уже является доверенным для пользователей.
...
- [Упаковка приложений универсальной платформы Windows](https://msdn.microsoft.com/windows/uwp/packaging/packaging-uwp-apps)
