...
> [!NOTE]
> Дополнительные пакеты, содержащего исполняемый код должен быть частью [связанный набор](https://docs.microsoft.com/windows/uwp/packaging/optional-packages#related-sets). Вы не сможете для выполнения кода из дополнительного пакета, если он не является частью связанный набор.
