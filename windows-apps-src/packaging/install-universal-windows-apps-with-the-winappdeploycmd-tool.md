...
description: Развертывание приложения Windows (WinAppDeployCmd.exe) — это средство командной строки, можно использовать для развертывания приложения универсальной платформы Windows (UWP) на Компьютере с Windows 10 на любом устройстве Windows 10.
ms.date: 09/30/2018
ms.topic: article
...
Развертывание приложения Windows (WinAppDeployCmd.exe) — это средство командной строки, можно использовать для развертывания приложения универсальной платформы Windows (UWP) на Компьютере с Windows 10 на любом устройстве Windows 10. Это средство можно использовать для развертывания пакета приложения, когда устройство Windows 10 подключенные по USB или на той же подсети, без необходимости в Microsoft Visual Studio или решение для этого приложения. Вы также можете развернуть приложение без его предварительной упаковки на удаленном компьютере или консоли Xbox One. В этой статье описывается, как устанавливать приложения UWP с помощью этого средства.
...
Во-первых подключение устройства Windows 10 к той же подсети или непосредственно к компьютеру Windows 10 с помощью USB-подключение. Затем используйте следующий синтаксис и примеры данной команды, описанные далее в этой статье, чтобы выполнить развертывание приложения UWP.
...
Устанавливает приложение из пакета MyApp.appx, который находится в каталоге файлы для загрузки компьютера, на устройство Windows 10 с IP-адресом 192.168.0.1, с ПИН-код кодом A1B2C3 для установления соединения с устройством
...
Обновляет приложение, которое уже установлено на устройстве Windows 10 с IP-адресом 192.168.0.1, используя указанный пакет приложения.
