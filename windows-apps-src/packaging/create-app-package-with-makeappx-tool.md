...
ms.locfileid: "57634659"
---
# <a name="create-an-app-package-with-the-makeappxexe-tool"></a>Создание пакета приложений с помощью средства MakeAppx.exe


**MakeAppx.exe** создает пакеты приложений (.msix или AppX) и приложения группы пакетов (.msixbundle или appxbundle). **MakeAppx.exe** также извлекает файлы из пакета приложения или пакета приложений и шифрует или расшифровывает пакеты приложения и пакеты приложений. Это средство входит в состав пакета SDK Windows 10, и его можно использовать через командную строку или файл сценария.

> [!IMPORTANT]
> Если для разработки приложения использовали Visual Studio, рекомендуется применять мастер Visual Studio для создания пакета приложения. Дополнительные сведения см. в разделе [Упаковка приложения UWP с помощью Visual Studio](packaging-uwp-apps.md).

> [!IMPORTANT]
> Обратите внимание, что **MakeAppx.exe** не приводит к созданию [файл отправки пакета приложения (.appxupload или .msixupload)](packaging-uwp-apps.md#types-of-app-packages), это рекомендуемый тип пакета приложений допустимым для [отправки в центр партнеров ](../publish/upload-app-packages.md). Отправка файла пакета приложения обычно является [создана как часть процесса упаковки Visual Studio](packaging-uwp-apps.md#create-an-app-package-upload-file), несмотря на то, что его можно также создать вручную.
...
- x86: C:\Program файлы (x86) \Windows Kits\10\bin\\&lt;номер сборки&gt;\x86\makeappx.exe
- x64: C:\Program файлы (x86) \Windows Kits\10\bin\\&lt;номер сборки&gt;\x64\makeappx.exe
...
| **Аргумент**                          | **Описание**                       |
|---------------------------------------|---------------------------------------|
| &lt;output package name&gt;           | Имя созданного пакета. Это имя файла, дополненный .msix или AppX. |
| &lt;encrypted output package name&gt; | Имя созданного зашифрованного пакета. Это имя файла, дополненный .emsix или .eappx. |
| &lt;input package name&gt;            | Имя пакета. Это имя файла, дополненный .msix или AppX. |
| &lt;encrypted input package name&gt;  | Имя зашифрованного пакета. Это имя файла, дополненный .emsix или .eappx. |
| &lt;output bundle name&gt;            | Имя созданного пакета приложений. Это имя файла, дополненный .msixbundle или appxbundle. |
| &lt;encrypted output bundle name&gt;  | Имя созданного зашифрованного пакета приложений. Это имя файла, дополненный .emsixbundle или .eappxbundle. |
| &lt;input bundle name&gt;             | Имя пакета приложений. Это имя файла, дополненный .msixbundle или appxbundle. |
| &lt;encrypted input bundle name&gt;   | Имя зашифрованного пакета приложений. Это имя файла, дополненный .emsixbundle или .eappxbundle. |
...
| /h            | Указывает хэш-алгоритм, используемый при создании сопоставления блоков. Этот параметр можно использовать только с командой pack. Включить допустимые алгоритмы: SHA256 (по умолчанию), SHA384, SHA512. |
...
| **Параметр**    | **Описание**                       |
|---------------|---------------------------------------|
| /bv           | Указывает номер версии пакета. Номер версии должен быть в четырех частей, разделенных точками, в форме: &lt;Основные&gt;.&lt; Дополнительный номер&gt;.&lt; Построение&gt;.&lt; Версия&gt;. |
| /f            | Указывает файл сопоставления.           |
...
- Приложение не регистрирует запрещенное протокола из этого списка: SMB, ФАЙЛ, MS-WWA-WEB, MS-WWA.
