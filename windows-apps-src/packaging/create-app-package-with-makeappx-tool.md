...
> [!IMPORTANT]
> Обратите внимание, что **MakeAppx.exe** создает [пакет приложения для передачи файла (.appxupload или .msixupload)](packaging-uwp-apps.md#types-of-app-packages), который является рекомендуемым тип допустимого пакета приложения для [отправки в центр партнеров](../publish/upload-app-packages.md). Файл отправки пакета приложения обычно является, [как часть процесса упаковки Visual Studio](packaging-uwp-apps.md#create-an-app-package-upload-file), несмотря на то, что его также можно создать вручную.
