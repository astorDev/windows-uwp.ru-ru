...
По умолчанию плоские пакеты будут ссылаться файлы пакета приложения в той же папке, сами. Однако эту ссылку можно изменить, указав другие пути (относительные пути, сетевые папки и расположения http). Для этого необходимо напрямую указать **BundleManifest** при создании плоского пакета. 
