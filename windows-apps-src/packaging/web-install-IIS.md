...
Чтобы успешно выполнить с помощью этого учебника, вам понадобится следующее:
...
> [!NOTE]
> Установите все флажки в разделе службы IIS не требуется. Подходят только выбранные при проверке **Службы IIS** .

Вам также потребуется установить ASP.NET 4.5 или более поздней версии. Чтобы установить ее, найдите **службы IIS "веб->" службы -> средства разработки приложений**. Выберите версию ASP.NET, которое больше или равно ASP.NET 4.5.

![Установка ASP.NET](images/install-asp.png)

## <a name="step-2---install-visual-studio-2017-and-web-development-tools"></a>Шаг 2 - установка Visual Studio 2017 и средства веб-разработка 
...
Запустите Visual Studio 2017 от имени **администратора** и создайте новый проект **Visual C# веб-приложения** с шаблоном **пустой** проект. 
...
В свойствах приложения web выберите вкладку **Web** . В разделе " **серверы** " выберите **Локальные службы IIS** из раскрывающегося меню и нажмите кнопку **Создать виртуальный каталог**. 
...
Включите следующий код HTML на веб-странице. Ключ для успешного вызова установщика приложений — использовать настраиваемую схему, установщик приложений регистрируется ОС: `ms-appinstaller:?source=`. См. в примере кода ниже, для получения дополнительных сведений.
...
Из-за сетевой изоляции, приложениях UWP, таких как установщик приложений ограничены IP-адресам замыкания на себя как http://localhost/. При использовании локального сервера IIS, установщик приложений необходимо добавить в список исключение замыкания на себя. 
