...
description: В этом руководстве показано, как настроить сервер IIS, убедитесь, что ваше веб-приложение можно размещать пакеты приложений и вызова и эффективное использование установщика приложений.
ms.author: cdon
ms.date: 05/30/2018
ms.topic: article
ms.prod: windows
...
ms.locfileid: "4264685"
---
# <a name="install-a-uwp-app-from-an-iis-server"></a>Установка приложения UWP с сервера IIS

В этом руководстве показано, как настроить сервер IIS, убедитесь, что ваше веб-приложение можно размещать пакеты приложений и вызова и эффективное использование установщика приложений.
...
2. Средствах веб-разработки и IIS 
...
[Службы IIS](https://www.iis.net/) — это функция Windows, которая может быть установлена через меню "Пуск". В **меню "Пуск"** поиск **Включение или отключение компонентов**.

Найдите и выберите **Службы IIS** для установки служб IIS.

> [!NOTE]
> Установите все флажки в разделе службы IIS не требуется. Только выбранные при проверке **Службы IIS** , достаточно.

Также необходимо установить ASP.NET 4.5 или более поздней версии. Чтобы установить ее, найдите **веб -> службы IIS служб -> средства разработки приложений**. Выберите версию ASP.NET, которое больше или равно ASP.NET 4.5.

![Установка ASP.NET](images/install-asp.png)

## <a name="step-2---install-visual-studio-2017-and-web-development-tools"></a>Шаг 2 - установка Visual Studio 2017 и средства веб-разработки 

[Установка Visual Studio 2017](https://docs.microsoft.com/visualstudio/install/install-visual-studio) , если вы еще не установлено его. Если у вас уже есть Visual Studio 2017, убедитесь, что установлены нагрузками. Если рабочих нагрузок, не представлены в вашей установке, следуйте с помощью установщика Visual Studio (найти из меню "Пуск").  
...
Этот пример веб-приложение использует простой HTML-код. Вы, кстати для создания веб-приложения при необходимости на ваших потребностей. 
...
Дважды щелкните файл HTML, чтобы открыть его в окне редактора кода. В этом руководстве будут использоваться только для элементов в требуемом на веб-странице, чтобы вызвать установщик приложений успешно для установки приложений Windows 10. 
...
Из-за сетевой изоляции приложений UWP, например установщик приложений ограничены использовать IP-адреса замыкания на себя как http://localhost/. При использовании локального сервера IIS, установщик приложений необходимо добавить в список исключение замыкания на себя. 

Чтобы сделать это, откройте окно **командной строки** с правами **администратора** и введите следующую команду: '' "CheckNetIsolation.exe LoopbackExempt командной строки --n=microsoft.desktopappinstaller_8wekyb3d8bbwe
```
...
После завершения локальной проверки установки приложений через установщик приложений вы можете удалить замыкания, который был добавлен на этом этапе по:
