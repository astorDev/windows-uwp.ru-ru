...
description: В этом руководстве показано, как настроить сервер IIS, убедитесь, что ваше веб-приложение может размещать пакеты приложений и вызывать функцию и эффективное использование установщика приложений.
ms.author: cdon
ms.date: 05/30/2018
ms.topic: article
ms.prod: windows
...
ms.locfileid: "4469523"
---
# <a name="install-a-uwp-app-from-an-iis-server"></a>Установка приложения UWP с сервера IIS

В этом руководстве показано, как настроить сервер IIS, убедитесь, что ваше веб-приложение может размещать пакеты приложений и вызывать функцию и эффективное использование установщика приложений.
...
> [!NOTE]
> Установите все флажки в разделе службы IIS не нужно. Подходят только выбранные при проверке **Службы IIS** .

Также потребуется установить ASP.NET 4.5 или более поздней версии. Чтобы установить ее, найдите **веб -> службы IIS служб -> средства разработки приложений**. Выбор версии ASP.NET, больше или равно ASP.NET 4.5.
...
Включите следующий код HTML на веб-странице. Ключ для успешного вызова установщика приложений — использовать настраиваемую схему, установщик приложений регистрируется операционной системы: `ms-appinstaller:?source=`. См. в примере кода ниже, для получения дополнительных сведений.
...
После завершения локальной проверки установки приложения с помощью установщика приложения вы можете удалить замыкания, который был добавлен на этом этапе по:
