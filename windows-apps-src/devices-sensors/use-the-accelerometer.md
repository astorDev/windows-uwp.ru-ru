...
-   [**акселерометр**](https://docs.microsoft.com/uwp/api/Windows.Devices.Sensors.Accelerometer)
...
Узнайте, как использовать акселерометр для реагирования на движения пользователя.
...
Первую часть имени класса из предыдущего фрагмента нужно заменить на пространство имен вашего приложения. Например, если вы создали проект под названием **AccelerometerCS**, необходимо заменить `x:Class="App1.MainPage"` на `x:Class="AccelerometerCS.MainPage"`. Кроме того, необходимо заменить `xmlns:local="using:App1"` на `xmlns:local="using:AccelerometerCS"`.

-   Нажмите клавишу F5 или выберите **Отладка** &gt; **начать отладку** для построения, развертывания и запуска приложения.

После запуска приложения можно изменить значения акселерометра, перемещая устройство или используя средства эмулятора.

-   Остановить приложение, Возвращаясь к Visual Studio и нажав клавиши Shift + F5 или выберите **Отладка** &gt; **остановить отладку** остановить приложение.
