...
Узнайте, как добавить функцию трехмерной печати в универсальное приложение для Windows. В этом разделе рассматривается запуск диалогового окна трехмерной печати после проверки формата и доступности для печати трехмерной модели.

## Настройка класса


Добавьте пространство имен [Windows.Graphics.Printing3D](https://msdn.microsoft.com/library/windows/apps/dn998169) в класс, где требуется реализовать функцию трехмерной печати.
...
В этом примере используется три пользовательских элемента управления: кнопка загрузки (для переноса файла в память программы), кнопка исправления (для изменения файла в соответствии с потребностями) и кнопка печати (для запуска задания печати). Следующий код позволяет создать эти кнопки (с соответствующими обработчиками события нажатия) в XAML-файле вашего класса.
...
## Использование конструктора 3D Builder для преобразования в формат создания 3D (.3mf)
...
Необходимо преобразовать файл трехмерных данных, чтобы реализовать тип [IRandomAccessStream](https://msdn.microsoft.com/library/windows/apps/br241731), который можно использовать для создания объекта [Printing3DModel](https://msdn.microsoft.com/library/windows/apps/mt203679).
...
Возвращенный объект **Print3DTask** назначается объявленной в начале переменной класса. Теперь (необязательно) можно использовать эту ссылку для обработки определенных событий, создаваемых этой задачей.
