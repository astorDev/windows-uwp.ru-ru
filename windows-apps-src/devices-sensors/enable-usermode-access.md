...
description: В этом руководстве описано, как предоставить доступ в пользовательском режиме к GPIO, I2C, SPI и UART в Windows 10.
ms.date: 02/08/2017
ms.topic: article
...
Давайте рассмотрим объявление узла устройств rhproxy в Raspberry Pi 2. Сначала создайте объявление устройства ACPI в области @no__t -0 _SB.
...
В этом разделе описывается, как предоставить ресурсы мультиплексирования клиентским драйверам. Обратите внимание, что для компиляции таблиц, содержащих ресурсы `MsftFunctionConfig()`, потребуется Microsoft ASL Compiler, сборка 14327 и выше. `MsftFunctionConfig()` ресурсов предоставляются для закрепления клиентов мультиплексированием как аппаратных ресурсов. `MsftFunctionConfig()` ресурсов следует предоставлять драйверам, требующим изменения ПИН-кода мультиплексированием, которые обычно SPB и драйверы последовательного контроллера, но не должны передаваться в драйверы SPB и Serial периферийных устройств, так как драйвер контроллера обрабатывает конфигурацию мультиплексированием.
...
В дополнение к ресурсам памяти и прерывания, которые обычно требуются драйверу контроллера, также необходимо задать ресурс `MsftFunctionConfig()`. Этот ресурс позволяет драйверу контроллера I2C размещать контакты 2 и 3, управляемые узлом устройства, по адресу @no__t -0 _SB. GPIO0 — в функции 4 с включенным резистором по запросу.
...
1. Компиляция и загрузка узла рхпрокси с помощью `ACPITABL.dat`
...
2. Скачайте [WDK](https://docs.microsoft.com/windows-hardware/drivers/download-the-wdk) и найдите `asl.exe` в `C:\Program Files (x86)\Windows Kits\10\Tools\x64\ACPIVerify`
...
### <a name="verify-that-the-expected-devices-are-exposed-to-usermode"></a>Убедитесь, что ожидаемые устройства доступны для пользовательского режима.
...
Клонировать репозиторий [https://github.com/ms-iot/samples](https://github.com/ms-iot/samples) и собрать образцы `GpioTestTool`, `I2cTestTool`, `SpiTestTool` и `Mincomm`. Скопируйте средства на свое устройство в тестовую папку и используйте следующие команды для перечисления устройств.
...
* [Несуществующие тесты подчиненных адресов I2C WinRT](https://docs.microsoft.com/windows-hardware/test/hlk/testref/2746ad72-fe5c-4412-8231-f7ed53d95e71)
...
* [Проверочные тесты частоты синхронизации по часам WinRT (требуется mbed LPC1768)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/50cf9ccc-bbd3-4514-979f-b0499cb18ed8)
* [Тесты перемещения операций ввода-вывода SPI WinRT (требуется mbed LPC1768)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/00c892e8-c226-4c71-9c2a-68349fed7113)
* [Тесты для проверки STRIDE WinRT](https://docs.microsoft.com/windows-hardware/test/hlk/testref/20c6b079-62f7-4067-953f-e252bd271938)
* [Тесты обнаружения зазоров при переносе SPI WinRT (требуется mbed LPC1768)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/6da79d04-940b-4c49-8f00-333bf0cfbb19)
...
Схема контактов заголовка: https://developer.microsoft.com/windows/iot/samples/PinMappingsRPi2
...
Схема контактов заголовка: https://developer.microsoft.com/windows/iot/samples/PinMappingsMBM
