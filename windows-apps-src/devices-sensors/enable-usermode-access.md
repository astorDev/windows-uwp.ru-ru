...
Давайте рассмотрим объявление узла устройств rhproxy в Raspberry Pi 2. Во-первых, создайте объявление устройства ACPI в \\_SB области.
...
В этом разделе описывается, как предоставить ресурсы мультиплексирования клиентским драйверам. Обратите внимание, что для компиляции таблиц, содержащих ресурсы `MsftFunctionConfig()`, потребуется Microsoft ASL Compiler, сборка 14327 и выше. `MsftFunctionConfig()` ресурсы предоставляются клиентам muxing ПИН-код как ресурсы оборудования. `MsftFunctionConfig()` ресурсов должен быть введен для драйверы, которые требуют изменений muxing ПИН-кода, которые обычно являются SPB и последовательного контроллера, но не следует указывать SPB и последовательного периферийного драйверы, с момента конфигурацию muxing дескрипторы драйверов контроллера.
...
В дополнение к ресурсам памяти и прерывания, которые обычно требуются драйверу контроллера, также необходимо задать ресурс `MsftFunctionConfig()`. Этот ресурс позволяет поместить ПИН-кодов, 2 и 3 — под управлением узла устройства в драйверу I2C контроллера \\_SB. GPIO0 — в функции 4 с резистор подключается включена.

## <a name="supporting-muxing-support-in-gpioclx-client-drivers"></a>Поддержка мультиплексирования в драйверах клиента GpioClx

`GpioClx` имеет встроенную поддержку muxing ПИН-код. Драйверы мини-портов GpioClx (драйверы клиента GpioClx), оборудование контроллера GPIO. Начиная с Windows 10, сборка 14327, драйверы мини-портов GpioClx могут добавлять поддержку мультиплексирования контактов, реализуя два новых DDI:
...
1. Компиляция и загрузка с помощью rhproxy узла `ACPITABL.dat`
...
2. Скачайте [WDK](https://docs.microsoft.com/windows-hardware/drivers/download-the-wdk) и найти `asl.exe` в `C:\Program Files (x86)\Windows Kits\10\Tools\x64\ACPIVerify`
...
Клон [ https://github.com/ms-iot/samples ](https://github.com/ms-iot/samples) репозиторий и построения `GpioTestTool`, `I2cTestTool`, `SpiTestTool`, и `Mincomm` примеры. Скопируйте средства на свое устройство в тестовую папку и используйте следующие команды для перечисления устройств.
...
* [GPIO WinRT функциональные и нагрузочные тесты](https://docs.microsoft.com/windows-hardware/test/hlk/testref/f1fc0922-1186-48bd-bfcd-c7385a2f6f96)
* [I2C WinRT записи тестов (требуется EEPROM)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/2ab0df1b-3369-4aaf-a4d5-d157cb7bf578)
* [Тесты I2C чтения WinRT (EEPROM требуется)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/ca91c2d2-4615-4a1b-928e-587ab2b69b04)
* [Тесты адрес несуществующего подчиненного WinRT I2C](https://docs.microsoft.com/windows-hardware/test/hlk/testref/2746ad72-fe5c-4412-8231-f7ed53d95e71)
* [I2C WinRT дополнительные функциональные тесты (mbed LPC1768 требуется)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/a60f5a94-12b2-4905-8416-e9774f539f1d)
* [Тесты SPI WinRT часы частота проверки (mbed LPC1768 требуется)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/50cf9ccc-bbd3-4514-979f-b0499cb18ed8)
* [SPI WinRT операций ввода-ВЫВОДА передачи тесты (mbed LPC1768 требуется)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/00c892e8-c226-4c71-9c2a-68349fed7113)
* [Тесты проверки SPI WinRT Stride](https://docs.microsoft.com/windows-hardware/test/hlk/testref/20c6b079-62f7-4067-953f-e252bd271938)
* [Тесты SPI WinRT передачи Gap обнаружения (mbed LPC1768 требуется)](https://docs.microsoft.com/windows-hardware/test/hlk/testref/6da79d04-940b-4c49-8f00-333bf0cfbb19)
