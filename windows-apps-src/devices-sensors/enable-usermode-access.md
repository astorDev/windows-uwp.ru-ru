...
---
# Включение доступа в режиме пользователя в Windows 10 IoT Базовая
...
Затем объявляем ресурсы GPIO и SPB, которые должны быть предоставлены пользовательскому режиму. Порядок объявления ресурсов важен, потому что индексированные ресурсы используются для связи свойств с ресурсами. Если предоставлено несколько шин I2C или SPI, первая объявленная шина считается шиной по умолчанию для этого типа и представляет собой экземпляр, возвращаемый методами `GetDefaultAsync()` [Windows.Devices.I2c.I2cController](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.i2c.i2ccontroller.aspx) и [Windows.Devices.Spi.SpiController](https://msdn.microsoft.com/en-us/library/windows/apps/windows.devices.spi.spicontroller.aspx). 
...
## Мультиплексирование контактов в среде выполнения 
...
### Архитектура мультиплексирования контактов 
...
* Встроенное ПО ACPI — определяет конфигурацию мультиплексирования с ресурсами `FunctionConfig()`. Ресурсы FunctionConfig показывают, какие контакты и в каких конфигурациях мультиплексирования необходимы клиенту. Ресурсы FunctionConfig содержат номер функции, конфигурацию запроса и список номеров контактов. Ресурсы FunctionConfig предоставляются клиентам мультиплексирования контактов в качестве аппаратных; их получают драйверы в ходе обратного вызова PrepareHardware аналогично ресурсам подключения GPIO и SPB. Клиенты получают ИД центра ресурсов, который можно использовать для открытия дескриптора ресурса. 
...
1.  Клиент получает ресурсы FunctionConfig от встроенного ПО ACPI в ходе обратного вызова [EvtDevicePrepareHardware()](https://msdn.microsoft.com/library/windows/hardware/ff540880.aspx).
...
### Описание протокола для клиентов мультиплексирования контактов
...
Драйвер WDF получает ресурсы `FunctionConfig()` в рамках процедуры [EvtDevicePrepareHardware()](https://msdn.microsoft.com/library/windows/hardware/ff540880.aspx). Ресурсы FunctionConfig можно идентифицировать по следующим полям.
...
Процедура `EvtDevicePrepareHardware()` может извлекать ресурсы FunctionConfig следующим образом.
...
Если клиент желает выполнить мультиплексирование контактов, он резервирует и фиксирует ресурс FunctionConfig. В следующем примере показано, как клиент может зарезервировать и зафиксировать ресурсы FunctionConfig.
...
### Описание протокол для серверов мультиплексирования контактов
...
Обратите внимание, что режим общего доступа входящих запросов должен браться из дескриптора FunctionConfig, а не из [IrpSp->Parameters.Create.ShareAccess](https://msdn.microsoft.com/library/windows/hardware/ff548630.aspx).

####    Обработка запросов IOCTL_GPIO_COMMIT_FUNCTION_CONFIG_PINS

После того как клиент успешно резервирует ресурс FunctionConfig, открывая дескриптор, можно отправить *IOCTL_GPIO_COMMIT_FUNCTION_CONFIG_PINS*, чтобы отправить на сервер запрос о выполнении фактической операции аппаратного мультиплексирования. Когда сервер получает запрос *IOCTL_GPIO_COMMIT_FUNCTION_CONFIG_PINS*, для каждого контакта в списке необходимо выполнить следующие действия: 
...
Значение FunctionNumber определяется сервером; исходят из того, что дескриптор FunctionConfig был создан специалистом, который знает, как сервер интерпретирует это поле.
...
### Инструкции по созданию таблиц ACPI

В этом разделе описывается, как предоставить ресурсы мультиплексирования клиентским драйверам. Обратите внимание, что для компиляции таблиц, содержащих ресурсы `FunctionConfig()`, потребуется Microsoft ASL Compiler, сборка 14327 и выше. `FunctionConfig()` ресурсы предоставляются клиентам мультиплексирования контактов в качестве аппаратных. `FunctionConfig()` ресурсы необходимо предоставлять драйверам, которые требуют изменений мультиплексирования контактов; как правило, это драйверы контроллеров SPB и последовательных контроллеров; не следует предоставлять ресурсы периферийным драйверам SPB и последовательным периферийным драйверам, поскольку драйвер контроллера обрабатывает конфигурацию мультиплексирования.
Макрос ACPI `FunctionConfig()` определяется следующим образом.
...
В следующем примере показано, как предоставить ресурс FunctionConfig() драйверу контроллера I2C. 
...
В дополнение к ресурсам памяти и прерывания, которые обычно требуются драйверу контроллера, также необходимо задать ресурс `FunctionConfig()`. Этот ресурс позволяет драйверу контроллера I2C помещать контакты 2 и 3 (управляемые узлом устройств в \\_SB.GPIO0) в функцию 4 с включенным повышающим резистором. 

### Поддержка мультиплексирования в драйверах клиента GpioClx 
...
### Поддержка мультиплексирования в дисках контроллеров SpbCx и SerCx 
...
Во время инициализации устройства платформы `SpbCx` и `SerCx` выполняют синтаксический разбор всех ресурсов `FunctionConfig()`, предоставленных устройству в качестве аппаратных. Затем SpbCx/SerCx по требованию приобретают и предоставляют ресурсы мультиплексирования контактов.
...
`SerCx` функционируют аналогично. `SerCx` приобретает все ресурсы `FunctionConfig()` в обработчике *IRP_MJ_CREATE* до обратного вызова [EvtSerCx2FileOpen()](https://msdn.microsoft.com/library/windows/hardware/dn265209.aspx) драйвера контроллера и отдает все ресурсы в обработчике IRP_MJ_CLOSE сразу после обратного вызова метода [EvtSerCx2FileClose](https://msdn.microsoft.com/library/windows/hardware/dn265208.aspx) драйвера контроллера.
...
### Выполните компиляцию и загрузку ASL с использованием файлов ACPITABL.dat 
...
### Приложение А. Списки ASL в Raspberry Pi

Вывод заголовка: https://developer.microsoft.com/en-us/windows/iot/win10/samples/PinMappingsRPi2
...
### Приложение B. Списки ASL в MinnowBoardMax

Вывод заголовка: https://developer.microsoft.com/en-us/windows/iot/win10/samples/PinMappingsMBM
