...
Доступ в пользовательском режиме к шинам низкого уровня в Windows осуществляется через существующие платформы `GpioClx` и `SpbCx`. Новый драйвер под названием *рхпрокси*, доступный в Windows IOT базовая и Windows Enterprise, предоставляет `GpioClx` и `SpbCx` ресурсы в пользовательском режиме. Чтобы включить API, узел устройства для рхпрокси должен быть объявлен в таблицах ACPI с каждым из ресурсов GPIO и SPB, которые должны быть доступны в пользовательском режиме. Этот документ содержит пошаговое руководство по созданию и проверке ASL.
...
Чтобы предоставить пользовательский режим `SerCx`UART в пользовательском режиме, объявите ресурс `UARTSerialBus` следующим образом.
...
> Необходимо передать `/MsftInternal` параметр командной строки `asl.exe` , чтобы собрать ASL-файлы, содержащие дескрипторы `MsftFunctionConfig()`, поскольку эти дескрипторы в данный момент рассматриваются рабочим комитетом ACPI. Пример: `asl.exe /MsftInternal dsdt.asl`
...
В этом разделе описывается, как предоставить ресурсы мультиплексирования клиентским драйверам. Обратите внимание, что для компиляции таблиц, содержащих ресурсы `MsftFunctionConfig()`, потребуется Microsoft ASL Compiler, сборка 14327 и выше. `MsftFunctionConfig()`ные ресурсы предоставляются для закрепления клиентов мультиплексированием как аппаратных ресурсов. `MsftFunctionConfig()`ные ресурсы должны предоставляться драйверам, требующим изменения ПИН-кода мультиплексированием, которые обычно SPB и драйверы последовательного контроллера, но не должны предоставляться для SPB и последовательных драйверов периферийных устройств, так как драйвер контроллера обрабатывает конфигурацию мультиплексированием.
...
Клонировать репозиторий [https://github.com/ms-iot/samples](https://github.com/ms-iot/samples) и собрать примеры `GpioTestTool`, `I2cTestTool`, `SpiTestTool`и `Mincomm`. Скопируйте средства на свое устройство в тестовую папку и используйте следующие команды для перечисления устройств.
