...
ms.locfileid: "57644589"
---
# <a name="create-an-nfc-smart-card-app"></a>Создание приложения для смарт-карты NFC


**Важные**  в этом разделе применяется только к Windows 10 Mobile.

ОС Windows Phone 8.1 поддерживала приложения эмуляции для карты NFC с помощью защищенного элемента на основе SIM-карты, но для этой модели требовалось, чтобы приложения для безопасной оплаты были тесно связаны с операторами мобильных сетей (ОМС). Это ограничивало выбор возможных способов оплаты для продавцов и разработчиков, не связанных с ОМС. В Windows 10 Mobile мы представили новую карту эмуляции технологию, эмуляции карты узла (HCE). Технология HCE позволяет вашему приложению устанавливать прямую связь с устройством чтения карт NFC. В этом разделе показано, как эмуляции карты узла (HCE) работает на устройствах Windows 10 Mobile и как можно разрабатывать приложения HCE, позволяя заказчикам обеспечить доступ к службам через свой телефон вместо использования физических карт без сотрудничества с MNO.

## <a name="what-you-need-to-develop-an-hce-app"></a>Необходимые условия для разработки приложения с поддержкой HCE


Для разработки приложений эмуляции карты на основе HCE для Windows 10 Mobile, необходимо будет получить настройки среды разработки. Получить настройкой, установив Visual Studio 2015 Microsoft, включая средства для разработчиков Windows и эмулятор Windows 10 Mobile с поддержкой NFC эмуляции. Подробнее о настройке см. в разделе [Настройка](https://msdn.microsoft.com/library/windows/apps/Dn726766)

При необходимости Если вы хотите протестировать с помощью реального устройства Windows 10 Mobile вместо включен эмулятор Windows 10 Mobile, необходимо также следующие элементы.

-   Устройство Windows 10 Mobile с поддержкой NFC HCE. В настоящее время приложения для NFC на основе HCE поддерживаются на телефонах Lumia 730, 830, 640 и 640 XL.
-   Терминал устройства считывания, поддерживающий протоколы ISO/IEC 14443-4 и ISO/IEC 7816-4

Windows 10 Mobile реализует это HCE служба, которая предоставляет следующие функциональные возможности.
...
Windows 10 поддерживает эмуляцию смарт-карт, которые основаны на ISO DEP (ISO-IEC 14443-4) и взаимодействует с помощью APDUs, как определено в ISO-IEC спецификации 7816-4. Windows 10 поддерживает ISO/IEC технология 14443-4 типа A для HCE приложений. Технологии типов B и F, а также технологии, отличные от ISO-DEP (например, MIFARE), по умолчанию направляются на SIM-карту.

С помощью функции эмуляции карты включены только устройства Windows 10 Mobile. Эмуляция SIM-HCE основе и карты недоступна в других версиях Windows 10.
...
**Сосуществование с помощью диспетчера установки на основе приложений NFC**

В Windows 10 Mobile система устанавливает таблице маршрутизации контроллера NFC, который используется для выполнения решений маршрутизации на уровне контроллера. Эта таблица содержит сведения о маршрутизации для перечисленных ниже элементов.
...
Windows 10 Mobile предоставляет меню параметр «SIM-карта» на странице параметров NFC для продолжения использования устаревших приложений на базе Windows Phone 8.1 SIM, которые не регистрируют свои средства с системой. Если пользователь выберет параметр "SIM-карта" в качестве карты для оплаты по умолчанию, маршрут ISO-DEP получит значение UICC. В случае всех остальных вариантов выбора в раскрывающемся меню маршрут ISO-DEP будет указывать на узел.

ISO-DEP маршрут имеет значение «SIM-карта» для устройств что SE включили SIM-карты при загрузке устройства в первый раз с Windows 10 Mobile. Если пользователь установит приложение, поддерживающее HCE, и это приложение включит какие-либо регистрации группы AID с HCE, маршрут ISO-DEP будет указывать на узел. Новым приложениям на основе SIM-карты необходимо зарегистрировать идентификаторы AID на SIM-карте, чтобы в таблицу маршрутизации контроллера были включены конкретные маршруты AID.
...
Функция эмуляции NFC смарт-карты действует только на Windows 10 Mobile, поэтому попытка использовать эмулятор смарт-карты интерфейсов API в других версиях Windows 10, приводит к ошибкам. Вы можете проверить поддержку API смарт-карты в следующем фрагменте кода.
...
Поддержка маршрутизации UICC на основе HCE и AID доступна только на недавно появившихся устройствах, в частности на Lumia 730, 830, 640 и 640 XL. Все новые NFC поддерживает устройства под управлением Windows 10 Mobile и после того, как должны поддерживать HCE. Ваше приложение может проверить наличие поддержки HCE следующим образом.
