...
-   [**Inclinometer (инклинометр)**](https://msdn.microsoft.com/library/windows/apps/BR225766)
...
Первую часть имени класса из предыдущего фрагмента нужно заменить на пространство имен вашего приложения. Например, если вы создали проект под названием **InclinometerCS**, необходимо поменять `x:Class="App1.MainPage"` на `x:Class="InclinometerCS.MainPage"`. Кроме того, необходимо заменить `xmlns:local="using:App1"` на `xmlns:local="using:InclinometerCS"`.
