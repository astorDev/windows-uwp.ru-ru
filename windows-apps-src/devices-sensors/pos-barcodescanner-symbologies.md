...
В следующем примере Получает список поддерживаемых символик сканер штрихкодов и отображает их в текстовом блоке:
...
Некоторым символикам, например Code 39 и Code 128, свойственна переменная длина.  Штрихкоды эти символик могут находиться рядом друг с другом содержащий различные данные, зачастую конкретной длины. Задание конкретной длины данных, которые вы хотите получить, помогает предотвратить недействительные считывания.
...
* **Диапазон**: декодирования длины между **DecodeLength1** и **DecodeLength2** символов. Порядок **DecodeLength1** и **DecodeLength2** буквально несколько (либо может быть выше или ниже, чем другой).
...
### <a name="check-digit-validation"></a>Проверьте проверки цифр
