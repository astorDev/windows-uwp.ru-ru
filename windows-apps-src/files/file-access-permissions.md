...
|----------|------------|---------------------|
| Все файлы, к которым у пользователя имеется доступ. Например: документы, изображения, фотографии, файлы для загрузки, рабочий стол, OneDrive и т. д. | broadFileSystemAccess<br><br>Это ограниченная возможность. Доступ настраивается в **параметрах** > **конфиденциальности** > **файловой системы**. Так как пользователи могут предоставить разрешение или отказать любое время в **параметрах**, необходимо убедиться, что ваше приложение будет устойчивым к эти изменения. Если вы обнаружите, что ваше приложение не имеет доступа, можно предложить пользователям изменять этот параметр, предоставляя ссылку на статью [доступ к файловой системе Windows 10 и конфиденциальность](https://privacy.microsoft.com/en-US/windows-10-file-system-access-and-privacy) . Обратите внимание, что пользователь должен закрыть приложение переключите параметр и перезапустите приложение. Если они переключите параметр во время работы приложения, платформа приостановит ваше приложение таким образом, вы можете сохранить состояние, а затем принудительно завершить работу приложения, чтобы применить новые параметры. В обновлении апреля 2018 г. по умолчанию для разрешения включен. В обновлении октября 2018 г. по умолчанию отключено.<br /><br />Если приложение отправляется в магазин Store, объявляющий эту возможность, потребуется дополнительно обосновать, зачем приложению нужна эта возможность и как планируется ее использовать.<br>Эта возможность работает для API-интерфейсы в пространстве имен [**Windows.Storage**](https://msdn.microsoft.com/library/windows/apps/BR227346) . См. в разделе " **Пример** " в конце этой статьи, пример того, как включить эту возможность в вашем приложении. | Неприменимо |
