...
## <a name="net"></a>.NET
Языки .NET распознают перегруженные версии в C + +/ CX класс "ref" так же, как и в любой класс .NET Framework.
...
При передаче значения DateTime из C + +/ CX в код JavaScript, JavaScript принимает его как объекта Date и отображает его по умолчанию как строку даты в полном формате.
...
Языках .NET обращение к свойствам собственные C + +/ CX объект так же, как бы это был объект .NET Framework.
...
Если вы удалите C + +/ проект компонента среды выполнения Windows CX с помощью решения, необходимо также вручную удалить ссылку на проект из проекта JavaScript. Невыполнение этого требования приведет к невозможности последующей отладки и сборки. При необходимости можно добавить ссылку на сборку в библиотеку DLL.
