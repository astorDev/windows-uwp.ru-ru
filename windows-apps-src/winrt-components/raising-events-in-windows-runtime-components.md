...
Функция makeToast необходимо подключили к кнопке. Обновление default.html кнопки и имеется пространство для вывода результатов, что всплывающее уведомление:
...
Первый шаг в создании прокси и заглушку для компонента является добавление интерфейсы уникальный идентификатор или идентификатор GUID. Однако формат GUID для использования отличается в зависимости от ли программирования C#, Visual Basic или другом языке .NET или C++.
...
Вернитесь к определению интерфейса, а затем вставьте новый идентификатор GUID перед интерфейс IToaster, как показано в следующем примере. (Не используйте GUID в примере. Каждый уникальный интерфейс должен быть собственный GUID.)
...
Теперь у интерфейсы уникальные идентификаторы, мы создайте файл IDL, смешивают файла .winmd в средство командной строки winmdidl и затем генерации исходного кода C для прокси и заглушки, смешивают этого файла IDL в средство командной строки MIDL. Visual Studio выполнять для нас, если мы создаем события после построения, как показано на следующих этапах.
...
Теперь у вас есть необходимые файлы для устройств, можно скомпилировать их для получения DLL, которая представляет собой файл C++. Чтобы сделать это максимально, добавьте новый проект для поддержки создания прокси-серверы. Откройте контекстное меню решения ToasterApplication и выберите **Добавить > Новый проект**. В левой области диалогового окна « **Новый проект** » разверните **Visual C++ &gt; Windows &gt; Windows перестанут**, а затем на центральной панели выберите **DLL (приложения UWP)**. (Обратите внимание, что это не проект компонента среды выполнения Windows C++). Назовите проект прокси-серверы, а затем нажмите кнопку « **ОК** ». Эти файлы будет обновляться с помощью событий после построения при каких-либо изменений в классе C#.

По умолчанию в проект прокси-серверы создает заголовок файлы .h и .cpp C++. Так как библиотеки DLL создается из файлов, созданные в MIDL, файлы .h и .cpp не являются обязательными. В обозревателе решений откройте контекстное меню для них, выберите **Удалить**и затем подтвердить удаление.

Теперь, когда проект пуст, можно добавить обратно создаваемые MIDL файлы. Откройте контекстное меню проекта прокси-серверы, а затем выберите **Добавить > существующий элемент.** В диалоговом окне перейдите в каталог проекта ToasterComponent и выберите эти файлы: файлы ToasterComponent.h, ToasterComponent_i.c, ToasterComponent_p.c и dlldata.c. Нажмите кнопку « **Добавить** ».

В проекте прокси-серверы создайте DEF-файл для определения экспорта библиотеки DLL, описанные в dlldata.c. Откройте контекстное меню проекта, а затем выберите **Добавить > новый элемент**. В левой области диалогового окна выберите код, а затем на центральной панели выберите файл определения модуля. Назовите proxies.def файла и нажмите кнопку **Добавить** . Откройте этот DEF-файл и изменить его для включения экспорт, которые определены в dlldata.c:
...
Сделан эти изменения, нажмите кнопку « **ОК** » в диалоговом окне **Страницы свойств** .

Затем выполните зависимости в проект ToasterComponent. Это гарантирует, что тостера описывается создание перед сборки проекта прокси-сервера. Это необходимо, поскольку проекта тостер отвечает за создание файлов для создания прокси-сервера.
...
В проект ToasterApplication откройте контекстное меню для package.appxmanifest и выберите **Открыть с помощью**. В диалоговом окне Открыть с помощью выберите **Текстовый редактор XML** и затем нажмите кнопку « **ОК** ». Мы добавим вставьте в некоторых XML, в котором содержатся регистрацию расширения windows.activatableClass.proxyStub и который основаны на GUID в прокси-сервера. Чтобы найти идентификаторы GUID для использования в файле .appxmanifest, откройте ToasterComponent_i.c. Найти записи, которые позволяют те в следующем примере. Также Обратите внимание, определения для IToast, IToaster и третий интерфейса — типизированным событием обработчик, который имеет два параметра: тостера и всплывающих уведомлений. Это соответствует событию, которое определяется в классе тостер. Обратите внимание, что идентификаторы GUID для IToast и IToaster соответствует GUID, которые определены в интерфейсы в файле C#. Так как интерфейс обработчика типизированным событием, автоматически созданных GUID для этого интерфейса также является автоматически созданного.
...
-   Идентификатор класса ProxyStub устанавливается первый идентификатор GUID в файле ToasterComponent\_i.c. Используйте первый GUID, который определяется в этот файл на идентификатор класса. (Это может быть так же, как GUID для ITypedEventHandler2.)
-   Путь — это относительный путь пакета двоичный прокси-сервера. (В этом пошаговом руководстве proxies.dll находится в той же папке, что ToasterApplication.winmd.)
...
-   Имена интерфейсов являются уникальными в манифесте. Так как они не используются в системе, вы можете выбрать значения. Рекомендуется выбрать имена интерфейсов, которые четко соответствуют интерфейсы, которые вы определили. Созданный интерфейсов имена должны быть указывает созданный интерфейсов. Можно использовать файл ToasterComponent\_i.c помогают создавать имена интерфейсов.
...
Теперь необходимо создавать проект. Запустите проект и убедитесь, что вы можете сделать всплывающего уведомления.
