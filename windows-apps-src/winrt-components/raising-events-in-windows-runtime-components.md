...
description: Как событие типа делегата, определяемого пользователем в фоновом потоке, таким образом, чтобы JavaScript получит событие.
ms.author: misatran
ms.date: 07/19/2018
ms.topic: article
ms.prod: windows
...
Функции makeToast необходимо подключить к кнопке. Обновление default.html кнопки и место для вывода результатов, что всплывающее уведомление:
...
Если мы не TypedEventHandler, мы теперь сможет запустить приложение на локальном компьютере и нажмите кнопку, чтобы сделать всплывающего уведомления. Но в наше приложение ничего не происходит. Чтобы узнать, почему, давайте отладка управляемого кода, который запускает ToastCompletedEvent. Остановите проект, а затем в строке меню выберите **отладки &gt; свойства приложения тостер**. Измените **Тип отладчика** **только управляемый код**. Еще раз в строке меню выберите **отладки &gt; исключения**, а затем выберите **Исключения среды CLR**.

Теперь запустите приложение и нажмите кнопку сделать всплывающего. Отладчик перехватывает исключение недопустимого приведения. Несмотря на то, что он не очевидно из его сообщения, это исключение происходит из-за отсутствия для этого интерфейса прокси-серверы.

![отсутствует прокси-сервера](./images/debuggererrormissingproxy.png)

Первый шаг в создании прокси и заглушку для компонента, является добавление интерфейсы уникальный идентификатор или идентификатор GUID. Тем не менее формат GUID для использования отличается в зависимости от ли программирования в C#, Visual Basic или другом языке .NET или C++.

## <a name="to-generate-guids-for-the-components-interfaces-c-and-other-net-languages"></a>Чтобы создать GUID для интерфейсов компонентов (C# и другие языки .NET)

В строке меню выберите средства &gt; создать GUID. В диалоговом окне выберите 5. \[GUID ("xxxxxxxx-xxxx … xxxx) \]. Нажмите кнопку новый идентификатор GUID и затем нажмите кнопку «Копировать».

![генератора GUID](./images/guidgeneratortool.png)

Вернитесь в определении интерфейса, а затем вставьте новый GUID перед интерфейс IToaster, как показано в следующем примере. (Не используйте GUID в примере. Каждый уникальный интерфейс должен быть собственный GUID.)
...
Добавить с помощью директивы для пространства имен System.Runtime.InteropServices.
...
В строке меню выберите средства &gt; создать GUID. В диалоговом окне выберите 3. статические const структуры GUID = {...}. Нажмите кнопку новый идентификатор GUID и затем нажмите кнопку «Копировать».
...
```
Добавить с помощью директивы для Windows.Foundation.Metadata добавить GuidAttribute в область действия.
...
Теперь у интерфейсы уникальные идентификаторы, мы можно создать файл IDL путем передачи файла .winmd в средство командной строки winmdidl и затем создания исходного кода C для прокси и заглушки, смешивают этого файла IDL в средство командной строки MIDL. Visual Studio этого мы создадим события после построения, как показано на следующих этапах.

## <a name="to-generate-the-proxy-and-stub-source-code"></a>Чтобы создать прокси и заглушки исходного кода

Чтобы добавить пользовательское событие после построения, в обозревателе решений, откройте контекстное меню в проект ToasterComponent и выберите свойства. В левой области страницы свойств выберите события сборки и затем нажмите кнопку изменить после сборки. Добавьте следующие команды в командной строке после сборки. (Пакетный файл должен вызываться сначала задать переменные среды, чтобы найти средство winmdidl.)
...
Чтобы убедиться, что файл IDL генерируется при каждом изменении файла .winmd, измените **Выполнить событие после построения** для **при построении обновляет выходные данные проекта.**
...
По умолчанию в проект прокси-серверы создает файлы заголовков .h и .cpp C++. Так как библиотеки DLL создается из файлов, созданные в MIDL, файлы .h и .cpp не требуются. В обозревателе решений откройте контекстное меню для них, выберите **Удалить**и затем подтвердить удаление.

Теперь, когда проект пуст, вы можете добавить обратно файлов созданными MIDL. Откройте контекстное меню проекта прокси-серверов, а затем выберите **Добавить > существующий элемент.** В диалоговом окне перейдите в папку проекта ToasterComponent и выберите следующие файлы: файлы ToasterComponent.h, ToasterComponent_i.c, ToasterComponent_p.c и dlldata.c. Нажмите кнопку « **Добавить** ».

В проекте прокси-серверы создайте DEF-файл для определения экспорта библиотеки DLL, описанные в dlldata.c. Откройте контекстное меню проекта, а затем выберите **Добавить > новый элемент**. В левой области диалогового окна выберите код, а затем на центральной панели выберите файл определения модуля. Имя файла proxies.def, а затем нажмите кнопку **Добавить** . Откройте этот DEF-файл и измените его, чтобы включить экспорт, которые определены в dlldata.c:
...
При создании проекта теперь она завершится ошибкой. Для правильной компиляции проекта, вам придется вносить изменения компиляции и связанного проекта. В обозревателе решений откройте контекстное меню проекта прокси-серверы и выберите **Свойства**. Измените на страницах свойств следующим образом.
...
Затем выполните зависимости в проект ToasterComponent. Это гарантирует, что тостер будет создан перед сборки проекта прокси-сервера. Это необходимо, так как в проект тостер отвечает за создание файлов для создания прокси-сервера.

Откройте контекстное меню проекта прокси-серверов, а затем выберите зависимости проекта. Установите флажки, чтобы указать, что проект прокси-серверы, зависит от проект ToasterComponent, чтобы убедиться, что Visual Studio создает их в правильном порядке.
...
В проект ToasterApplication откройте контекстное меню для package.appxmanifest и выберите **Открыть с помощью**. В диалоговом окне Открыть с помощью выберите **Текстовый редактор XML** и затем нажмите кнопку « **ОК** ». Мы создадим для вставки в некоторых XML, в котором содержатся регистрацию расширения windows.activatableClass.proxyStub и который основаны на GUID в прокси-сервера. Чтобы найти идентификаторы GUID для использования в файле .appxmanifest, откройте ToasterComponent_i.c. Найти записи, которые позволяют те в следующем примере. Также Обратите внимание, определения IToast, IToaster и третий интерфейса — типизированным событием обработчик, который имеет два параметра: тостера и всплывающих уведомлений. Это соответствует событию, которое определяется в классе тостер. Обратите внимание, что идентификаторы GUID для IToast и IToaster соответствуют идентификаторы GUID, которые определены в интерфейсы в файле C#. Так как интерфейс обработчика типизированным событием автоматически, идентификатор GUID для этого интерфейса также является автоматически.
...
Теперь мы копируем идентификаторы GUID, вставьте их в файле package.appxmanifest в узел, который мы добавляем и имя расширения, а затем переформатировать их. Запись манифеста напоминает следующий пример — но опять же, не забудьте задать собственные идентификаторы GUID. Обратите внимание, что идентификатор класса GUID в XML-ФАЙЛ так же, как ITypedEventHandler2. Это связано с тем, GUID — это первый из них, перечисленных в ToasterComponent_i.c. GUID здесь без учета регистра. Вместо вручную переформатирование GUID для IToast и IToaster, можно вернуться в определения интерфейса и получите значение GuidAttribute, который имеет правильный формат. В C++ существует GUID правильно отформатированы в комментарии. В любом случае необходимо вручную переформатировать GUID, который используется для идентификатор класса и обработчик событий.
...
Перед переходом, очень важно, чтобы:

-   Идентификатор класса ProxyStub устанавливается первый идентификатор GUID в файле ToasterComponent\_i.c. Используйте первый GUID, который определяется в этот файл на идентификатор класса. (Это может быть так же, как идентификатор GUID для ITypedEventHandler2.)
...
-   Идентификаторы GUID, в нужном формате. (Это можно легко получить неверный.)
-   Интерфейс идентификаторы в манифесте соответствует идентификаторы IID в файле ToasterComponent\_i.c.
...
При попытке запустить решение, вы получите сообщение об ошибке, proxies.dll не является частью полезных данных. Откройте контекстное меню для **ссылки на** папку в проект ToasterApplication, а затем выберите **Добавить ссылку**. Установите флажок рядом с проектом прокси-серверы. Кроме того убедитесь, что установлен флажок рядом с ToasterComponent. Нажмите кнопку **ОК**.

Теперь необходимо создавать проекта. Запустите проект и убедитесь, что вы можете сделать всплывающего уведомления.
