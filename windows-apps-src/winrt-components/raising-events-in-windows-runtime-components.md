...
Теперь ручное преобразование const GUID GuidAttribute таким образом, чтобы он отформатирован, как показано в следующем примере. Обратите внимание, что фигурные скобки заменяются скобки и скобки и удаляется конечные точки с запятой.
```cpp
...
Чтобы убедиться, что файл IDL генерируется при каждом изменении файла .winmd, измените, чтобы **Выполнить событие после построения** **при обновлении выходной файл проекта построения.**
...
Не вставлять эти библиотеки непосредственно в строке списка. Используйте поле **Изменить** для того, чтобы MSBuild в Visual Studio будет правильно Дополнительные зависимости.

После внесения этих изменений, нажмите кнопку « **ОК** » в диалоговом окне **Страницы свойств** .

Затем выполните зависимости в проект ToasterComponent. Это гарантирует, что тостера описывается создание перед сборки проекта прокси-сервера. Это необходимо, поскольку проект тостер отвечает за создание файлов для создания прокси-сервера.
...
В проект ToasterApplication откройте контекстное меню для package.appxmanifest и выберите **Открыть с помощью**. В диалоговом окне Открыть с помощью выберите **Текстовый редактор XML** и затем нажмите кнопку « **ОК** ». Мы добавим вставьте в некоторых XML, в котором содержатся регистрацию расширения windows.activatableClass.proxyStub и который основаны на GUID в прокси-сервера. Чтобы найти идентификаторы GUID для использования в файле .appxmanifest, откройте ToasterComponent_i.c. Поиск записи, похожие те в следующем примере. Также Обратите внимание, определения IToast, IToaster и третий интерфейса — типизированным событием обработчик, который имеет два параметра: тостера и всплывающих уведомлений. Это соответствует событию, которое определяется в классе тостер. Обратите внимание, что идентификаторы GUID для IToast и IToaster соответствует GUID, которые определены в интерфейсы в файле C#. Так как интерфейс обработчика типизированным событием, автоматически созданных GUID для этого интерфейса также является автоматически созданного.
...
Вставьте узел расширения XML в качестве прямого дочерний узел пакет и однорангового элемента, например, ресурсов узла.
