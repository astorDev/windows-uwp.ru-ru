...
description: В этой статье ориентированная предприятия поддерживаются Windows10, что позволяет использовать существующий код, отвечающий за ключевые бизнес-операции приложения .NET с поддержкой сенсорного ввода.
ms.date: 02/08/2017
ms.topic: article
...
>**Примечание**загрузку без публикации можно включить в параметры -&gt; обновление безопасности & -&gt; для разработчиков.
...
После того как ссылки настроены надлежащим образом, следующая задача— реализовать функциональность сервера. См. в разделе MSDN[Советы и рекомендации по взаимодействию с компонентами среды выполнения Windows (приложения UWP на C\ #/ VB/C++ и XAML)](https://msdn.microsoft.com/library/windows/apps/hh750311.aspx).
...
Для установки приложения, скопируйте реализации**winmd**в правильный каталог, указанный в манифесте связанные неопубликованного приложения: <ActivatableClassAttribute>Value = «путь». Также скопируйте все связанные вспомогательные файлы и DLL-файл прокси/заглушки (о прокси и заглушках см. далее). Не удается скопировать реализации**winmd**сервера directory обращенные все неопубликованные приложения вызовы новые RuntimeClass вызовет ошибку «класс не зарегистрирован». Если не установить (или не зарегистрировать) прокси/заглушку, ни один вызов не сможет вернуть значение. Последняя ошибка часто**не**являетсясвязана с видимыми исключениями.
...
**Шаг 6:** Добавьте def-файла в проект «MyWinRTProxy»**(проект > Добавление нового элемента > кода > файл определения модуля**) и обновления содержимого будет:
...
-   Возврат*списка<T>* где*T*— объект из асинхронной операции или извлеченное свойство, потребует много межпроцессных. Например, предположим, вы вернете*списка&lt;люди&gt;* объектов. Каждая итерация будет межпроцессным вызовом. Каждый*людей*объекта, возвращенного представляется прокси-сервера и каждый вызов метода или свойства этого отдельного объекта приведет к межпроцессному вызову. Таким образом «безобидный»*списка&lt;люди&gt;* объект где*количество*большой приведет к большое количество вызовов медленно. Массовая передача структур содержимого в массиве обеспечивает более высокую производительность. Пример.
...
-   [NorthwindRT (пример компонента WinRT, работающего через брокер)](https://go.microsoft.com/fwlink/p/?LinkID=397349)

-   [Создание надежных и заслуживающих доверия приложений Microsoft Store](https://go.microsoft.com/fwlink/p/?LinkID=393644)
...
-   [Развертывание приложений UWP для бизнеса](https://go.microsoft.com/fwlink/p/?LinkID=264770)
