...
description: В этом документе рассматриваются функции в предназначенном для предприятия, поддерживается в Windows 10, который позволяет приложениям .NET сенсорные дисплеи использовать существующий код, отвечающий за критически важных бизнес-операции с ключами.
ms.date: 02/08/2017
ms.topic: article
...
В этой статье рассматриваются функции в предназначенном для предприятия, поддерживается в Windows 10, который позволяет приложениям .NET сенсорные дисплеи использовать существующий код, отвечающий за критически важных бизнес-операции с ключами.

## <a name="introduction"></a>Введение

>**Примечание**  пример кода, сопровождающий этот документ можно загрузить для [Visual Studio 2015 и 2017](https://aka.ms/brokeredsample). Шаблон Microsoft Visual Studio для построения через посредника компонентов среды выполнения Windows можно загрузить здесь: [Шаблон Visual Studio 2015, предназначенные для универсальной Windows приложений для Windows 10](https://visualstudiogallery.msdn.microsoft.com/10be07b3-67ef-4e02-9243-01b78cd27935)
...
Наконец, учитывая непреодолимой уязвимости среды выполнения .NET и C\# языка в корпоративной разработки, эта функция было разработано с акцентом на использование .NET для приложений универсальной платформы Windows и сторон настольного компонента. Несмотря на то, возможны другие языки и среды выполнения для приложения универсальной платформы Windows, в соответствующем примере только иллюстрирует C\#и исключительно предназначен только для среды выполнения .NET.

## <a name="application-components"></a>Компоненты приложения

>**Примечание**  эта функция предназначена исключительно для использования платформы .NET. И клиентское приложение, и классический компонент должны быть созданы с использованием .NET.
...
**Компонент настольной системы**

Классический компонент в этой функциональности представляет собой приложение нового типа. Этот компонент настольной системы может быть написан только на C\# и необходимо ориентироваться на платформу .NET 4.6 или более поздней версии для Windows 10. Соответствующий тип проекта представляет собой гибрид со средой CLR, ориентированной на UWP, так как формат межпроцессного взаимодействия охватывает типы и классы UWP, тогда как классический компонент способен вызывать любые элементы библиотеки классов времени выполнения .NET. То, как это влияет на проект Visual Studio, будет подробно рассмотрено далее. Эта гибридная конфигурация обеспечивает маршалинг типов UWP между приложениями на базе классических компонентов. При этом она позволяет вызывать код классической среды CLR внутри реализации классического компонента.

**Контракт**

Контракт между неопубликованным приложением и классическим компонентом описывается в понятиях системы типов UWP. Это включает в себя один или несколько C объявления\# классы, которые могут представлять UWP. См. в разделе MSDN [создание компонентов среды выполнения Windows на языке C\# и Visual Basic](https://msdn.microsoft.com/library/br230301.aspx) определенные требования для создания класса среды выполнения Windows, с помощью c#\#.

>**Примечание**  перечисления не поддерживаются в контракте компоненты среды выполнения Windows между компонент настольной системы и неопубликованных приложений в настоящее время.
...
>**Примечание** загрузки неопубликованных приложений можно включить в параметры -&gt; обновление и безопасность -&gt; для разработчиков.
...
Неопубликованные приложения могут быть 64-разрядными (при условии, что зарегистрированы 64- и 32-разрядные прокси), но это будет нетипичная ситуация. Сборка приложения на языке C неопубликованных\# естественным образом с помощью нормальная конфигурация «neutral» и значение по умолчанию «предпочтительно 32-разр.» создает 32-разрядных неопубликованных приложений.

**Создание экземпляров сервера и домены приложений**
...
К счастью, это легко объявить с помощью c#\# классы. При определении этих контрактов, однако, необходимо учитывать некоторые связанные с производительностью моменты; они будут рассмотрены в одном из следующих разделов.
...
**Шаг 2.** Измените файл проекта вручную, чтобы изменить тип выходных данных проекта в компонент среды выполнения Windows
...
В открытом файле найдите \<OutputType\> теги и измените его значение на «winmdobj».

**Шаг 3.** Создание правила построения, создающий Windows «ссылку» файл метаданных (winmd-файл). т. е. не имеет реализации.

**Шаг 4.** Создайте правило сборки, которое создает файл метаданных Windows «реализация», т. е. имеет те же сведения метаданных, но также включает реализацию.
...
Один раз ссылки **winmd** создается (в папке «ссылки» в целевой папке проекта), это вручную, находящимися (Копировать) для каждого проекта неопубликованных приложений и ссылки на. Это будет описано в следующем разделе. Структура проекта, воплощенные в выше правила построения убедитесь, что реализация и ссылка **winmd** находятся в каталогах четко отделенные друг от друга в иерархии сборки, чтобы избежать путаницы.

## <a name="side-loaded-applications-in-detail"></a>Подробнее о неопубликованных приложениях
Как уже упоминалось, неопубликованное приложение создается так же, как любое другое приложение UWP, за исключением одной особенности: объявления доступности классов RuntimeClass в манифесте неопубликованного приложения. Это позволяет приложению просто создать новый объект для доступа к функциональности классического компонента. Новая запись манифеста в разделе <Extension> описывает RuntimeClass, реализованный в классическом компоненте, и предоставляет данные о его расположении. Содержимое этого объявления в манифесте приложения одинаковое для всех приложений, нацеленных на Windows 10. Например:
...
В качестве категории выбрана inProcessServer, так как в категории outOfProcessServer есть несколько записей, неприменимых к этой конфигурации приложения. Обратите внимание, что <Path> компонент всегда должен содержать clrhost.dll (тем не менее это **не** принудительно и указав другое значение не определено способами).

Раздел <ActivatableClass> совпадает с настоящим RuntimeClass, который используется внутри процесса и предпочтителен для компонента среды выполнения Windows в пакете приложения. <ActivatableClassAttribute> новый элемент и атрибуты Name = «DesktopApplicationPath» и Type = «string» обязательные и инвариантным. Атрибут Value указывает расположение WINMD-файла реализации классического компонента (дополнительную информацию о нем см. в следующем разделе). Каждый RuntimeClass, предпочтительный для классического компонента, должен иметь собственное дерево элементов <ActivatableClass>. ActivatableClassId должен соответствовать полному имени пространства имен RuntimeClass.

Как указывалось в разделе "Определение контракта", ссылка на проект должна быть включена в ссылочный WINMD-файл классического компонента. Система проекта Visual Studio обычно создает двухуровневую структуру каталогов с тем же именем. В примере это EnterpriseIPCApplication\\EnterpriseIPCApplication. Ссылка **winmd** вручную копируется в этом каталоге второго уровня, а затем ссылки проекта, используется диалоговое окно (щелкните **Обзор … **  кнопка) для поиска и ссылки на эти **winmd**. После этого принадлежащее классическому компоненту пространство имен верхнего уровня (например, Fabrikam) должно появиться в качестве узла верхнего уровня в части проекта "Ссылки".

>**Примечание** очень важно использовать **ссылки winmd** в приложении неопубликованных. Если вы случайно переносятся **реализацию winmd** в каталог неопубликованных приложений и ссылки, скорее всего вы получите ошибки, связанной с «не удается найти IStringable». Это один вход, конечно неправильным **winmd** была сделана ссылка. Правила после сборки в IPC серверное приложение (см. в следующем разделе) тщательно разделить эти два **winmd** в разных каталогах.

Переменные среды (особенно % ProgramFiles %) можно использовать в <ActivatableClassAttribute Value="path"> . Как отмечалось ранее, компонент Service Broker приложение поддерживает только 32-разрядной, % ProgramFiles % будет решать задачи C:\\Program Files (x86) при запуске приложения в 64-разрядной операционной системе.

## <a name="desktop-ipc-server-detail"></a>Дополнительная информация о классическом сервере IPC

Двух предыдущих разделах описываются объявление класса и механизм передачи ссылки **winmd** проект неопубликованных приложений. Основная часть оставшейся работы с классическим компонентом заключается в реализации. Так как суть классического компонента заключается в способности вызывать код классического приложения (обычно для повторного использования существующих ресурсов кода), проект необходимо настроить особым образом.
...
Как правило, указываются только прочие ссылки между проектами. Но проект классического компонента имеет очень специализированный набор ссылок. Он запускается как «классический рабочий стол\\библиотека классов» проекта и, следовательно, проект для настольной системы. Поэтому явные ссылки на API среды выполнения Windows (с использованием ссылок на **winmd** файлы) должны выполняться. Добавьте необходимые ссылки, как показано ниже.
...
После того как ссылки настроены надлежащим образом, следующая задача — реализовать функциональность сервера. См. в разделе MSDN [советы и рекомендации для обеспечения взаимодействия с компонентами среды выполнения Windows (приложений универсальной платформы Windows, с помощью c#\#/VB/C++ и XAML)](https://msdn.microsoft.com/library/windows/apps/hh750311.aspx).
...
Чтобы установить приложение, скопируйте реализация **winmd** в нужный каталог, указанный в связанной неопубликованных манифест приложения: <ActivatableClassAttribute>от значения = «path». Также скопируйте все связанные вспомогательные файлы и DLL-файл прокси/заглушки (о прокси и заглушках см. далее). Не удается скопировать реализация **winmd** на сервер каталога приведет все неопубликованных вызовов приложения новое в RuntimeClass будет выдавать ошибку «класс не зарегистрирован». Если не установить (или не зарегистрировать) прокси/заглушку, ни один вызов не сможет вернуть значение. Эта последняя ошибка часто является **не** связанных с исключениями видимым.
Если эта ошибка конфигурации вызывает исключения, их могут отнести к "недопустимому приведению".

**Рекомендации по реализации Server**
...
**Async среды выполнения Windows на сервере**
...
>**Примечание** При написании реализации обычно ожидаются некоторые другие потенциально длительные операции. Если Да, **Task.Run** кода должно быть объявлено как:
...
**Вызов функции сервера из фонового потока приложения**

Так как обычно код для клиента и сервера пишется в одной организации, можно взять за обыкновение программировать выполнение всех вызовов сервера в неопубликованных приложениях в фоновом потоке. Прямой вызов, собирающий один или несколько пакетов данных с сервера, можно выполнять из фонового потока. Если результаты получены полностью, пакет данных, находящийся в памяти процесса приложения, обычно можно извлечь непосредственно из потока пользовательского интерфейса. C\# объекты являются естественным образом agile между фоновых потоках и потоках пользовательского интерфейса таким образом особенно полезны для такого рода шаблон вызова.
...
**Создание прокси-сервера в Visual Studio**

Процесс создания и регистрации, прокси и заглушки для использования внутри обычный пакет приложения универсальной платформы Windows описаны в разделе [создание событий в компонентах среды выполнения Windows](https://msdn.microsoft.com/library/windows/apps/dn169426.aspx).
...
**Шаг 1.** С помощью решения для проекта настольного компонента, создайте проект заглушки/прокси-сервера в Visual Studio:

**Решение > Добавить > проект > Visual C++ > DLL-Библиотеки Win32 консоли выберите параметр.**

Приведенные ниже предполагается, компонент сервера называется **MyWinRTComponent**.

**Шаг 3.** Удаление всех файлов CPP или H из проекта.

**Шаг 4.** Предыдущем разделе «Определение контракта» содержит команду после сборки, которая запускается **winmdidl.exe**, **midl.exe**, **mdmerge.exe**, и т. д. В результате работы midl.exe в рамках этой послесборочной команды создается четыре выходных файла.

а) Dlldata.c

б) файл заголовка (например, MyWinRTComponent.h)

c) объект \* \_i.c файла (например MyWinRTComponent\_i.c)

(г) объект \* \_p.c файла (например MyWinRTComponent\_p.c)

**Шаг 5.** Добавьте четыре созданные файлы в проект «MyWinRTProxy».

**Шаг 6.** Добавьте файл def «MyWinRTProxy» проект **(проект > Добавить новый элемент > код > файла определения модуля**) и обновите содержимое, чтобы быть:
...
**Шаг 7.** Откройте окно свойств для проекта «MyWinRTProxy»:

**Свойства конфигурация > Общие > имя целевой:**

MyWinRTComponent.Proxies

**C/C++ > определения препроцессора > Добавить**

«WIN32; \_WINDOWS; ЗАРЕГИСТРИРОВАТЬ\_ПРОКСИ\_DLL»

**C/C++ > предкомпилированного заголовка: Выберите «Отказ от использования предкомпилированного заголовка»**

**Компоновщик > Общие > Пропустить библиотеку импорта: Выберите «Да»**

**Компоновщик > ввод > Дополнительные зависимости: Добавить rpcrt4.lib;runtimeobject.lib**

**Компоновщик > метаданных Windows > создавать метаданные Windows: Нажмите кнопку «Нет»**

**Шаг 8.** Выполните сборку проекта «MyWinRTProxy».

**Развертывание прокси-сервера**

Прокси необходимо зарегистрировать глобально. Проще всего это сделать с помощью вызова DllRegisterServer из proxy dll в процессе установки. Обратите внимание, что, поскольку этот компонент поддерживает только серверы для архитектуры x86 (т. е. не поддерживает 64-разрядные системы), то в простейшей конфигурации используется 32-разрядный сервер, 32-разрядный прокси и 32-разрядное неопубликованное приложение. Прокси-сервер обычно располагается наряду с реализацией **winmd** для компонент настольной системы.

Необходимо выполнить дополнительный шаг настройки. Чтобы процесс загрузки без публикации загружал и выполнял прокси, необходимо присвоить каталогу отметку "чтение/выполнение" для ALL_APPLICATION_PACKAGES. Это делается с помощью **icacls.exe** средство командной строки. Эта команда должна выполняться в этом каталоге где реализация **winmd** и находится библиотека dll посредника заглушки:
...
-   Возвращение *списка<T>* где *T* — это объект из асинхронной операции или свойства выборки, приведет к массу число вызовов между процессами. Предположим, например, можно вернуть*списка&lt;людей&gt;* объектов. Каждая итерация будет межпроцессным вызовом. Каждый *людей* возвращенный объект представляется прокси-сервер и каждый вызов метода или свойства для этого отдельного объекта приведет к межпроцессного вызова. Поэтому «невинный» *списка&lt;людей&gt;* объекта где *число* большой приведет к большим числом медленные вызовы. Массовая передача структур содержимого в массиве обеспечивает более высокую производительность. Например:
...
Затем вернитесь* PersonStruct\[\]* вместо *списка&lt;PersonObject&gt;*.
В этом случае все данные будут получены за один сеанс межпроцессного взаимодействия.

Как всегда в вопросах производительности, большую роль играют измерение и тестирование. В идеале телеметрию нужно включать в разные операции, чтобы определить их длительность. Очень важно для измерения по целому: например, как фактически через какое время неиспользование всех *людей* объектов для конкретного запроса в приложении неопубликованных?
...
Процесс сервера можно найти и завершить с помощью диспетчера задач или других, сторонних приложений. Средство командной строки **TaskList.exe **также включен и имеет гибкий синтаксис, например:
...
Должны быть перечислены в списке модулей для сервера посредника *clrhost.dll* в свой список загруженных модулей.

## <a name="resources"></a>Ресурсы

-   [Управляемый компонент WinRT шаблоны проектов для Windows 10 и Visual STUDIO 2015](https://visualstudiogallery.msdn.microsoft.com/10be07b3-67ef-4e02-9243-01b78cd27935)

-   [Образец NorthwindRT через посредника компонента WinRT](https://go.microsoft.com/fwlink/p/?LinkID=397349)

-   [Доставки надежных и надежности приложений Microsoft Store](https://go.microsoft.com/fwlink/p/?LinkID=393644)

-   [Контракты и расширения (приложения для Windows Store) приложений](https://msdn.microsoft.com/library/windows/apps/hh464906.aspx)

-   [Как для загрузки неопубликованных приложений в Windows 10](https://msdn.microsoft.com/windows/uwp/get-started/enable-your-device-for-development#GroupPolicy)

-   [Развертывание приложений универсальной платформы Windows для бизнеса](https://go.microsoft.com/fwlink/p/?LinkID=264770)
