...
Если вы создаете компонент для использования только в приложениях UWP, написанных на Visual Basic или C#, и он не содержит элементы управления UWP, затем onsider, используя шаблон **Библиотека классов** вместо шаблона проекта **Компонента среды выполнения Windows** в Microsoft Visual Studio. У простой библиотеки классов меньше ограничений.
...
- Открытые классы и интерфейсы могут содержать методы, свойства и события. Можно объявить делегаты для событий или использовать **EventHandler&lt;T&gt; ** делегат. Открытые классы и интерфейсы не могут:
    - быть универсальными;
    - Реализации интерфейса, который не является интерфейс среды выполнения Windows (тем не менее, можно создать собственные интерфейсы среды выполнения Windows и реализовать их).
...
Если ваше приложение UWP и компонент построены с использованием управляемого кода, можно отлаживать оба одновременно.

При тестировании компонента как часть приложения UWP на C++, в то же время можно отлаживать управляемый и машинный код. По умолчанию может отлаживаться только машинный код.
...
При тестировании компонента как части приложения UWP, с помощью JavaScript, решения по умолчанию находится в режиме отладки JavaScript. В Visual Studio невозможно отлаживать JavaScript и управляемый код одновременно.
...
В среде выполнения Windows **IMap&lt;K, V&gt; ** и **IMapView&lt;K, V&gt; ** осуществляется с помощью IKeyValuePair. При их передаче управляемого кода, они отображаются в виде **IDictionary&lt;TKey, TValue&gt; ** и **IReadOnlyDictionary&lt;TKey, TValue&gt;**, поэтому использование **System.Collections.Generic.KeyValuePair&lt;TKey, TValue&gt; ** перечислить их.
...
-   Нельзя скрывать реализацию **IStringable** от базовых классов с помощью определения метода, например следующие:
...
> [!NOTE]
> В самых разных случаях вызовы из машинного кода в управляемый тип, реализующий **IStringable** или скрывающий свою реализацию **ToString** может привести к непредвиденному поведению.
