...
> **Обратите внимание** , сообщения об ошибках, ссылающиеся на среда выполнения Windows, используют старую терминологию. Теперь эта среда называется универсальной платформой Windows (UWP). Например, типы среды выполнения Windows теперь называются типами UWP.
...
> **Обратите внимание** , в сообщении об ошибке оператор определяется по имени метаданных, например Op\_сложение, Op\_умножение, Op\_Exclusive, Op\_неявное (неявное преобразование) и т. д.
...
| Номер ошибки | Текст сообщения                                                                                                                                                                      |
|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1059      | Несколько перегрузок параметра {0}"{1}.{2}"снабжены Windows. Foundation. Metadata. DefaultOverloadAttribute.                                                            |
| WME1085      | Перегрузки {0}параметра {1}.{2} должен иметь только один метод, указанный в качестве перегрузки по умолчанию, путем его оформления в Windows. Foundation. Metadata. DefaultOverloadAttribute. |
...
| WME1068      | Тип "{0}" не может иметь то же имя, что и пространство имен "{1}".                                                                                                                                                                                                                                                                                                 |
...
<td align="left">В сигнатуре метода "{0}" имеется параметр типа "{1}". Хотя этот тип не является допустимым типом среды выполнения Windows, он реализует интерфейсы, являющиеся допустимыми типами среды выполнения Windows. Попробуйте изменить сигнатуру метода с использованием одного из следующих типов: "{2}".</td>
</tr>
<tr class="odd">
...
> **Примечание** .  Для {2}Winmdexp. exe добавляет список альтернативных вариантов, например "рассмотрите возможность изменения типа System. Collections. Generic. List&lt;T&gt;" в сигнатуре метода на один из следующих типов: "System. Collections. Generic. IList&lt;T&gt;, System. Collections. Generic. Иреадонлилист&lt;T&gt;, System. Collections. Generic. IEnumerable&lt;T&gt;".
</td>
</tr>
<tr class="even">
<td align="left">WME1040</td>
<td align="left">В сигнатуре метода "{0}" имеется параметр типа "{1}". Вместо управляемого типа Task используйте Windows.Foundation.IAsyncAction, Windows.Foundation.IAsyncOperation или один из асинхронных интерфейсов среды выполнения Windows. Стандартный шаблон ожидания .NET также применим к этим интерфейсам. Дополнительные сведения о преобразовании объектов управляемых задач в асинхронные интерфейсы среды выполнения Windows вы можете найти в разделе System.Runtime.InteropServices.WindowsRuntime.AsyncInfo.</td>
</tr>
</tbody>
</table>
...
| Номер ошибки | Текст сообщения                                                                                                                                                                                                                                                            |
|--------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1060      | В структуре "{0}" есть поле "{1}" типа "{2}". "{2}" не является допустимым типом поля среды выполнения Windows. Каждое поле в структуре среды выполнения Windows должно иметь тип UInt8, Int16, UInt16, Int32, UInt32, Int64, UInt64, Single, Double, Boolean, String, Enum или само быть структурой. |
...
> **Обратите внимание** , это ограничение не применяется к массивам, которые вы используете внутри реализации.

 

| Номер ошибки | Текст сообщения                                                                                                                                                     |
|--------------|--------------------|
| WME1034      | Метод "{0}" имеет массив типа "{1}" с ненулевой нижней границей в сигнатуре. Нижняя граница массивов в сигнатурах методов среды выполнения Windows должна равняться нулю. |
...
| WME1036      | В сигнатуре метода "{0}" имеется вложенный массив типа "{1}". Массивы в подписях методов среды выполнения Windows не могут быть вложенными.                                    |
...
В UWP параметры должны быть доступны только для чтения или только для записи. Параметры нельзя пометить модификатором **ref** (**ByRef** без атрибута [OutAttribute](https://docs.microsoft.com/dotnet/api/system.runtime.interopservices.outattribute) в Visual Basic). Это относится к содержимому массивов, поэтому параметры массивов должны указывать, доступно ли содержимое массива только для чтения или только для записи. Направление четко определено для параметров **out** (параметр **ByRef** с атрибутом OutAttribute в Visual Basic), но параметры массивов, передаваемые по значению (ByVal в Visual Basic), должны быть помечены. Ознакомьтесь с разделом [Передача массивов в компонент среды выполнения Windows](passing-arrays-to-a-windows-runtime-component.md).

| Номер ошибки | Текст сообщения         |
|--------------|----------------------|
| WME1101      | Метод "{0}" имеет параметр "{1}", который является массивом и содержит как {2}, так и {3}. В среде выполнения Windows содержимое параметров-массивов должно быть доступным для чтения или записи. Удалите один из атрибутов из "{1}".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| WME1103      | Метод "{0}" имеет параметр "{1}", который является массивом и имеет либо атрибут System. Runtime. InteropServices. InAttribute, либо System. Runtime. InteropServices. OutAttribute. В среде выполнения Windows у параметров массива должен быть атрибут {3} или {3}. Удалите эти атрибуты или при необходимости замените их соответствующими атрибутами среды выполнения Windows.                                                                                                                                                                                                                                                                                                                                                                                          |
| WME1104      | Метод "{0}" имеет параметр "{1}", который не является массивом и содержит либо {2}, либо {3}. Среда выполнения Windows не поддерживает использование {3} и {3} для пометки параметров, не являющихся массивами.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| WME1106      | Метод "{0}" имеет параметр "{1}", который является массивом. В среде выполнения Windows содержимое параметров-массивов должно быть доступным для чтения или записи. Примените к "{1}" {1} или {1}.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
