...
> **Примечание** сообщения об ошибках, относящиеся к среде выполнения Windows использовать старый терминологию. Теперь эта среда называется универсальной платформой Windows (UWP). Например, типы среды выполнения Windows теперь называются типами UWP.
...
Эта проблема возникает только при использовании средства Winmdexp.exe через командную строку. Мы рекомендуем использовать параметр/reference, чтобы включить ссылки на библиотеки mscorlib.dll и System.Runtime.dll — из основных ссылочных сборок .NET Framework, которые находятся в «% ProgramFiles(x86) %\\ссылочные сборки\\Microsoft\\Framework\\. NETCore\\v4.5» ("% ProgramFiles %\\...» на 32-разрядном компьютере).
...
> **Примечание** в сообщении об ошибке оператор определяется по имени метаданных, например op\_сложение, op\_Multiply op\_ExclusiveOr, op\_неявные (неявное преобразование) и т. д.
...
| Номер ошибки | Текст сообщения                                                                                                                                            |
|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1099      | Тип "{0}«имеет несколько конструкторов с»{1}" аргументами. Типы среды выполнения Windows не могут иметь несколько конструкторов с одинаковым числом аргументов. |
...
| Номер ошибки | Текст сообщения                                                                                                                                                                      |
|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1059      | Несколько {0}-перегрузок "{1}.{2}", обозначаются атрибутом Windows.Foundation.Metadata.DefaultOverloadAttribute.                                                            |
| WME1085      | {0}-Перегрузок {1}.{2} должен быть задан как перегрузка по умолчанию, дополнив его Windows.Foundation.Metadata.DefaultOverloadAttribute ровно один метод. |
...
> **Примечание**  эти ограничения применяются только к открытым типам, а не к закрытым типам, используемым в вашей реализации.
...
> **Внимание**  при непосредственном вызове Winmdexp.exe и не используйте параметр/out. Чтобы указать имя для компонента среды выполнения Windows, Winmdexp.exe пытается сформировать имя, включающее все пространства имен в компоненте. Переименование пространств имен может изменить имя компонента.

 

| Номер ошибки | Текст сообщения                                                                                                                                                                                                                                                                                                                                             |
|--------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME0006      | "{0}" не является именем файла winmd допустимым для этой сборки. Все типы в файле метаданных Windows должны находиться во вложенном пространстве имен пространства имен, заданного именем файла. Типы, отсутствующие в этом вложенном пространстве имен, невозможно найти во время выполнения. В этой сборке наименьшее общее пространство имен, которое можно использовать в качестве имени файла, — "{1}". |
...
<td align="left">Метод "{0}«имеет параметр типа»{1}" сигнатура. Хотя этот тип не является допустимым типом среды выполнения Windows, он реализует интерфейсы, являющиеся допустимыми типами среды выполнения Windows. Попробуйте изменить сигнатуру метода с использованием одного из следующих типов: "{2}".</td>
</tr>
<tr class="odd">
...
> **Примечание**  для {2}, Winmdexp.exe добавляет список альтернативных вариантов, например «попробуйте изменить тип "System.Collections.Generic.List&lt;T&gt;" в сигнатуре метода на один из следующих типов Вместо этого: &lt;T&gt;, System.Collections.Generic.IReadOnlyList&lt;T&gt;, System.Collections.Generic.IEnumerable&lt;T&gt;".»
</td>
</tr>
<tr class="even">
...
| Номер ошибки | Текст сообщения                                                                                                                                                     |
|--------------|--------------------|
| WME1034      | Метод "{0}«имеет массив объектов типа»{1}" с ненулевой нижней границей его подпись. Нижняя граница массивов в сигнатурах методов среды выполнения Windows должна равняться нулю. |
...
| Номер ошибки | Текст сообщения         |
|--------------|----------------------|
| WME1101      | Метод "{0}«имеет параметр»{1}" который является массивом и содержит оба {2} и {3}. В среде выполнения Windows содержимое параметров-массивов должно быть доступным для чтения или записи. Удалите один из атрибутов из "{1}".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| WME1102      | Метод "{0}«имеется выходной параметр»{1}" который является массивом, но содержит {2}. В среде выполнения Windows содержимое выходных массивов доступно для записи. Удалите атрибут из "{1}".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| WME1103      | Метод "{0}«имеет параметр»{1}" который является массивом и содержит с атрибутом System.Runtime.InteropServices.InAttribute или System.Runtime.InteropServices.OutAttribute. В среде выполнения Windows у параметров массива должен быть атрибут {2} или {3}. Удалите эти атрибуты или при необходимости замените их соответствующими атрибутами среды выполнения Windows.                                                                                                                                                                                                                                                                                                                                                                                          |
| WME1104      | Метод "{0}«имеет параметр»{1}" которого не является массивом и содержит либо {2} или {3}. Среда выполнения Windows не поддерживает использование {2} и {3} для пометки параметров, не являющихся массивами.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Номер ошибки | Текст сообщения |
|--------------|--------------|
| WME1091 | Метод "\{0}" имеется возвращаемое значение с именем "\{1}" которого совпадает с именем параметра. В среде выполнения Windows параметры методов и возвращаемые значения должны иметь уникальные имена. |
| WME1092 | Метод "\{0}" имеет параметр с именем "\{1}" которого совпадает со значением по умолчанию именем возвращаемого значения. Попробуйте использовать другое имя параметра или с помощью атрибута System.Runtime.InteropServices.WindowsRuntime.ReturnValueNameAttribute явно укажите имя возвращаемого значения. |
