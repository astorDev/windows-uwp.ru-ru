...
> **Примечание** The сообщения об ошибках, которые ссылаются на среда выполнения Windows, используют старую терминологию. Теперь эта среда называется универсальной платформой Windows (UWP). Например, типы среды выполнения Windows теперь называются типами UWP.
...
> **Примечание** in это сообщение об ошибке, оператор определяется по имени метаданных, например Op @ No__t-2Addition, Op @ No__t-3Multiply, Op @ No__t-4ExclusiveOr, Op @ No__t-5Implicit (неявное преобразование) и т. д.
...
| Номер ошибки | Текст сообщения                                                                                                                                                                      |
|--------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| WME1059      | Несколько @no__t -0 перегрузки параметров "{1}. {2}" дополнены Windows. Foundation. Metadata. DefaultOverloadAttribute.                                                            |
| WME1085      | @No__t перегрузок -0-Parameter {1}. {2} должен иметь ровно один метод, заданный в качестве перегрузки по умолчанию, путем его оформления в Windows. Foundation. Metadata. DefaultOverloadAttribute. |
...
> **Примечание**@no__t — ограничение 1This не применяется к массивам, которые вы используете внутри вашей реализации.
...
| WME1035      | Метод "{0}" имеет многомерный массив типа "{1}" в его сигнатуре. Массивы в сигнатурах методов среды выполнения Windows должны быть одномерными.                  |
...
| Номер ошибки | Текст сообщения         |
|--------------|----------------------|
| WME1101      | Метод "{0}" имеет параметр "{1}", который является массивом и имеет как {2}, так и {3}. В среде выполнения Windows содержимое параметров-массивов должно быть доступным для чтения или записи. Удалите один из атрибутов из "{1}".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| WME1104      | Метод "{0}" имеет параметр "{1}", который не является массивом и имеет либо {2}, либо {3}. Среда выполнения Windows не поддерживает использование {2} и {3} для пометки параметров, не являющихся массивами.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
