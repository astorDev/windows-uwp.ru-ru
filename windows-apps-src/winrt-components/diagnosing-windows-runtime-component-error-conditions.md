...
| WME1068      | Тип "{0}«не может быть совпадает с именем пространства имен»{1}".                                                                                                                                                                                                                                                                                                 |
...
<td align="left">Метод "{0}«имеет параметр типа»{1}" сигнатура. Хотя этот тип не является допустимым типом среды выполнения Windows, он реализует интерфейсы, являющиеся допустимыми типами среды выполнения Windows. Попробуйте изменить сигнатуру метода, чтобы вместо этого используйте один из следующих типов: "{2}".</td>
</tr>
<tr class="odd">
...
> **Примечание**при изменении имени возвращаемого значения, а новое имя конфликтует с именем другого параметра, возникает ошибка WME1091.
