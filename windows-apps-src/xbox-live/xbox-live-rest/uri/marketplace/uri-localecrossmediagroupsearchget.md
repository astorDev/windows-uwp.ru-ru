...
ms.locfileid: "57658399"
---
# <a name="get-mediamarketplaceidcrossmediagroupsearch"></a>GET (/media/{marketplaceId}/crossMediaGroupSearch)
Получает элементы из нескольких групп другой носитель. Доменом для таких URI является `eds.xboxlive.com`.
...
API между группами позволяет клиентам выполнять поиск элементов из нескольких групп другие средства передачи. Этот API требует использования маркер продолжения только вперед для результатов по страницам. Этот API принимает уточнения запроса.
...
| Параметр| Тип| Описание| 
| --- | --- | --- | --- | --- | --- | 
| continuationToken| Строка| Необязательно. См. параметр ContinuationToken.| 
...
Приведенный ниже код JSON: в ответ на вызов `/media/en-us/crossMediaGroupSearch?q=vector&maxItems=25&fields=all`.
