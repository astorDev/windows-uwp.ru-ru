...
-   [**Географическое расположение**](https://msdn.microsoft.com/library/windows/apps/br225603)
...
-   Очищайте кэшированные данные о расположении и освобождайте объект [**Geolocator**](https://msdn.microsoft.com/library/windows/apps/br225534), когда пользователь отключает доступ к сведениям о расположении.
    ...
    Освобождайте объект [**Geolocator**](https://msdn.microsoft.com/library/windows/apps/br225534), если пользователь отключает доступ к сведениям о расположении через "Параметры". Приложение будет получать **доступа\_отказано в ДОСТУПЕ** результаты для каких-либо вызовов API определения местоположения. Если приложение сохраняет или кэширует данные о расположении, очищайте все кэшированные данные, когда пользователь отключает доступ к данным о расположении. Предоставьте альтернативный способ ввода данных расположения вручную на случай, если они окажутся недоступны для служб определения расположения.

-   Предусмотрите пользовательский интерфейс для повторного включения определения расположения. Например, предоставить кнопка обновления, заново создается [ **Geolocator** ](https://msdn.microsoft.com/library/windows/apps/br225534) объекта и пытается получить данные о расположении еще раз.
...
-   Чтобы определить, когда пользователь отключает или вновь включает передачу данных о расположении:
    ...
    -   Проверьте коды ошибок, возвращаемые методом [**GetGeopositionAsync**](https://msdn.microsoft.com/library/windows/apps/hh973536). Если пользователь отключил службы определения местоположения, вызовы **GetGeopositionAsync** завершится ошибкой и **доступа\_отказано в ДОСТУПЕ** ошибки и [ **LocationStatus** ](https://msdn.microsoft.com/library/windows/apps/br225538) свойство имеет значение **отключено**.
