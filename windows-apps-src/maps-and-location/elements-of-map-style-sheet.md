...
Чтобы области воды отображались красным можно использовать указанный ниже JSON, отображаться метки воды — зеленым цветом, а области Земли синего цвета:
...
| >>> indigenousPeoplesReserve | [MapElement](#mapelement) |  ✔   |  ✔   |  ✔   |  ✔   | Области, которые охватывают коренному люди резервирует. |
...
| >>> neighborhood             | [MapElement](#mapelement) |  ✔   |  ✔   |  ✔   |  ✔   | Метки область окружение. |
...
| >>>> reserve                 | [MapElement](#mapelement) |  ✔   |  ✔   |  ✔   |  ✔   | Оставляет за собой областей, которые охватывают характер. |
...
| >>>>> essentialServicePoint  | [PointStyle](#pointstyle) |      |  ✔   |  ✔   |  ✔   | Значки, которые представляют основные службы, такие как парковки, банки, газа и т. д. |
...
| >>>> transitBuilding         | [MapElement](#mapelement) |  ✔   |  ✔   |  ✔   |  ✔   | Используется для передачи, например аэропортах зданий. |
...
| rasterRegionsVisible         | Логический    |  ✔   |  ✔   |  ✔   |  ✔   | Флаг, который указывает, следует ли рисовать растровые регионы, где они имеют представление лучше, чем векторов (Япония и Корея). |
...
| borderWidthScale             | Плавающий   |  ✔   |  ✔   |  ✔   |  ✔   | Сумма, которую границ масштабируется. Например, используйте значение *1* для стандартного размера и значение *2* для вдвое большего размера. |
