...
Обратите внимание, что мы также передаем строку "consumers" необязательному параметру *authority*. Это связано с тем, что Майкрософт предоставляет два различных типа проверки подлинности: учетные записи Майкрософт (MSA) для "потребителей" и Azure Active Directory (AAD) для "организаций". Сертификат "потребители" позволяет поставщику понять, что нас интересует первый вариант.

Если вы разрабатываете корпоративное приложение, можно использовать конечную точку AAD. См. полный [пример кода WebAccountManagement на GitHub](http://go.microsoft.com/fwlink/p/?LinkId=620621) и документацию Azure для получения дополнительных сведений о том, как это сделать. 
