...
- **Блокировка намерения сигнала**: Позволяет пользователю показывать его цель для разблокировки по, например, при нажатии кнопки в Windows Hello сопутствующих устройства. Сигнал намерения должен поступать c **устройства-компаньона Windows Hello**.
- **Сигнал присутствия пользователя**: Подтверждает наличие пользователя. Устройство-компаньон Windows Hello может, например, запросить PIN-код, прежде чем его можно будет использовать для разблокировки компьютера (не путать с PIN-кодом компьютера), либо запросить нажатие кнопки.
- **Устранение неоднозначности сигнала**: Устраняет неоднозначность какие Windows 10 desktop, пользователю необходимо разблокировать при Windows Hello устройство-компаньон доступно несколько вариантов.
...
- Служба фильтрации для предприятия: Предприятия можно ограничить набор Windows Hello дополнительных устройств, которые могут работать в своих средах и select лишь немногие из конкретного поставщика. Например, компания Contoso может заказать 10 000 сопутствующих устройств модели Y от поставщика X и разрешить только этим устройствам работать на домене Contoso (запрет также распространяется на все остальные модели устройств от поставщика X).
- Инвентаризация.  Предприятия можно определить список существующих устройств-компаньонов, используемые в корпоративной среде.
- Отзыва в режиме реального времени: Если сотрудник сообщает, что его устройство-компаньон потерян или украден, веб-службы можно отозвать это устройство.
- Перемещаемые: Пользователь имеет только один раз зарегистрировать свои устройства-компаньона и работает на всех его рабочих столов Windows 10 и мобильных устройств.
...
3. Ожидание нужного события фоновой задачи
    * WaitingForUserConfirmation: Ожидания для этого события, если действие/жест пользователя на стороне устройства Windows Hello companion необходим для запуска потока проверки подлинности
    * CollectingCredential: Ожидания для этого события, если Windows Hello устройство-компаньон зависит от действия/жест пользователя на стороне ПК для запуска потока проверки подлинности (например, нажав клавишу ПРОБЕЛ)
    * Другой триггер, таких как смарт-карты: Убедитесь в том, что запрашивать текущее состояние проверки подлинности для вызова API-интерфейсы справа.
...
Регистрации требуется два вызова API Companion службы проверки подлинности: RequestStartRegisteringDeviceAsync и FinishRegisteringDeviceAsync.
