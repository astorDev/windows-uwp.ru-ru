...
description: Устройство-компаньон Windows Hello — это устройство, которое может взаимодействовать с настольным компьютером Windows 10 для повышения удобства процедуры проверки подлинности пользователя. С помощью платформы устройств, сопровождающей Windows Hello, вспомогательное устройство может обеспечить широкие возможности Windows Hello, даже если они недоступны (например, если на рабочем столе Windows 10 отсутствует Камера для проверки подлинности лиц или устройства чтения отпечатков пальцев). , например).
ms.date: 02/08/2017
ms.topic: article
...
> **Примечание** . Корпорация Майкрософт будет использовать API для платформы устройств Windows Hello в первой половине 2020.
...
> Видеообзор см. в презентации [Разблокировка Windows с помощью устройств IoT](https://channel9.msdn.com/Events/Build/2016/P491), представленной на конференции Build 2016 на веб-сайте Channel 9.
...
- **Сигнал намерения**: Позволяет пользователю показывать свое намерение разблокировать, например, нажав кнопку на устройстве Windows Hello Companion. Сигнал намерения должен поступать c **устройства-компаньона Windows Hello**.
...
- **Сигнал устранения неоднозначности**: Различения Windows 10 Desktop (Рабочий стол), который пользователь хочет разблокировать, если для устройства Windows Hello доступно несколько параметров.
...
- Мы рекомендуем выполнять обнаружение, загрузку и запуск приложения Windows Hello Companion, а также, если это возможно, автоматизировать (например, приложение можно загрузить при касании устройства Windows Hello на устройстве NFC-сканера в Windows 10 Desktop Side). Однако за выполнение этой процедуры отвечают устройство-компаньон Windows Hello и его приложение.
...
Приложение устройства-компаньона Windows Hello управляет всеми этапами процесса на стороне устройства-компаньона Windows Hello. Платформа сопутствующих устройств Windows Hello не управляет этой частью процесса взаимодействия с пользователем. В частности, сопутствующий поставщик проверки подлинности информирует приложение-помощник Windows Hello (с помощью своего фонового приложения) об изменениях состояния в пользовательском интерфейсе входа (например, на экране блокировки, а также на экране блокировки пользователем с помощью клавиши пробел). Кроме того, ответственность за то, чтобы создать такую работу (например, при нажатии пользователем клавиши пробела и на экране разблокировки для расшифровки), лежит в приложении Windows Hello Companion, начните искать устройство по USB.
...
Поставщик или изготовитель оборудования сопутствующего устройства может реализовать веб-службу для перемещения состояния регистрации между настольными компьютерами или мобильными устройствами с Windows 10. Дополнительные сведения см. в разделе "Служба роуминга, отзыва и фильтрации".
...
- Служба фильтрации для предприятия: Предприятие может ограничить набор вспомогательных устройств Windows Hello, которые могут работать в своей среде, до нескольких выбранных от конкретного поставщика. Например, компания Contoso может заказать 10 000 сопутствующих устройств модели Y от поставщика X и разрешить только этим устройствам работать на домене Contoso (запрет также распространяется на все остальные модели устройств от поставщика X).
- Товары  Предприятие может определить список существующих сопутствующих устройств, используемых в корпоративной среде.
- Отзыв в реальном времени: Если сотрудник сообщает о том, что его сопутствующее устройство потеряно или украдено, то для отмены этого устройства можно использовать веб-службу.
- Роуминге Пользователь должен только зарегистрировать свое вспомогательное устройство и работать со всеми его настольными компьютерами с Windows 10 и мобильными устройствами.
...
3. Ожидание нужного события фоновой задачи
    * Ваитингфорусерконфирматион: Дождитесь этого события, если для запуска потока проверки подлинности требуется действие или жест пользователя на стороне устройства "помощник Windows Hello"
    * Коллектингкредентиал: Дождитесь этого события, если устройство-компаньон Windows Hello использует действие или жест пользователя на стороне компьютера для запуска потока проверки подлинности (например, нажатием клавиши пробел).
    * Другой триггер, например смарт-карта: Обязательно выполните запрос текущего состояния проверки подлинности, чтобы вызвать правильные API.
...
Для регистрации требуется два вызова API сопровождающей службы проверки подлинности: Рекуестстартрегистерингдевицеасинк и Финишрегистерингдевицеасинк.
...
Для проверки подлинности требуется два вызова API сопровождающей службы проверки подлинности: Стартаусентикатионасинк и Финишаусенкатионасинк.
...
| State                         | Описание                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
|----------------------------   |-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------    |
| WaitingForUserConfirmation    | Это событие уведомления об изменении состояния срабатывает, когда появляется экран блокировки (например, пользователь нажал на Windows + L). Не рекомендуется запрашивать сообщения об ошибках в случае возникновения проблем с поиском устройства при этом состоянии. Как правило, рекомендуется отображать сообщения только при наличии сигнала намерения. Приложение для устройства "помощник Windows Hello" должно сделать первый вызов API для проверки подлинности в этом состоянии, если сопутствующее устройство собирает сигнал намерения (например, коснуться NFC-сканера, нажать кнопку на сопровождающем устройстве или специальный жест, например аплодисменты ), и фоновая задача приложения для устройства Windows Hello получает указание с устройства, на котором был обнаружен сигнал намерения. В противном случае, если при запуске процесса проверки подлинности приложению устройства-компаньона Windows Hello требуется компьютер (пользователю необходимо провести пальцем вверх по экрану блокировки или нажать клавишу "ПРОБЕЛ"), приложение устройства-компаньона Windows Hello должно дождаться возникновения следующего состояния (CollectingCredential).   |
| CollectingCredential          | Это событие уведомления об изменении состояния активируется, если пользователь открывает крышку ноутбука, нажимает любую клавишу на клавиатуре или проводит пальцем вверх по экрану блокировки. Если устройство-компаньон Windows Hello использует описанные выше действия для начала сбора сигнала о намерениях, то приложение-помощник Windows Hello должно начать его сбор (например, через всплывающее устройство с вопросом, нужно ли пользователю разблокировать компьютер). . Это подходящий момент для отображения сообщений об ошибках, если приложению устройства-компаньона Windows Hello требуется, чтобы пользователь отправил сигнал своего присутствия с устройства-компаньона (например, путем ввода PIN-кода на этом устройстве).                                                                                                                                                                                                                                                                                                                                             |
