...
Если сопутствующее устройство поддерживает биометрию, платформу сопутствующих устройств можно использовать, только если устройство соответствует одному из следующих условий:

- На нем есть пользовательский интерфейс и операционная система, отличная от Windows, с помощью которых пользователь может зарегистрировать свои биометрические данные и управлять ими (например, телефон с поддержкой биометрии). Если сопутствующее устройство работает на операционной системе Windows и поддерживает биометрию (распознавание лиц и отпечатков пальцев), оно должно соответствовать [биометрической платформе Windows](https://msdn.microsoft.com/en-us/library/windows/hardware/mt608302(v=vs.85).aspx). 
- Оно лишено пользовательского интерфейса (аппаратный USB-ключ) и не поддерживает распознавание лиц, радужной оболочки глаза или отпечатков пальцев. 

Если сопутствующее устройство не соответствует ни одному из этих условий и поддерживает биометрию, следуйте рекомендациям для [сценариев приложений изготовителей оборудования и независимых поставщиков оборудования с поддержкой Windows Hello](https://msdn.microsoft.com/en-us/library/windows/hardware/mt608302(v=vs.85).aspx). 
...
- Приложение [универсальной платформы Windows (UWP)](https://msdn.microsoft.com/en-us/windows/uwp/get-started/universal-application-platform-guide) для сопутствующего устройства, скачанное из магазина приложений для Windows. 
...
Перед подключением сопутствующего устройства к платформе сопутствующих устройств его необходимо зарегистрировать на этой платформе. Приложение сопутствующего устройства управляет всеми этапами процесса регистрации. Платформа сопутствующих устройств не накладывает каких-либо ограничений на этот процесс взаимодействия с пользователем. 
...
- Возникла проблема. Войдите с помощью ПИН-кода или пароля, а затем снова настройте *имя устройства*.
...
- Повторите попытку
- Произнесите пароль в микрофон *имя устройства*
