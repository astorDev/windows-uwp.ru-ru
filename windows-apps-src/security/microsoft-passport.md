...
ms.locfileid: "67320589"
---
# <a name="windows-hello"></a>Windows Hello

В этой статье описывается новая Windows Hello технология, которая поставляется как часть операционной системы Windows 10 и обсуждается, как разработчики могут реализовать эту технологию для защиты своих приложений универсальной платформы Windows (UWP) и серверных служб. Здесь рассматриваются определенные возможности этих технологий, помогающие снизить угрозы от использования стандартных учетных данных, и содержит рекомендации по разработке и развертыванию этих технологий в рамках внедрения Windows 10.

Учтите, что эта статья посвящена разработке приложений. Дополнительные сведения об архитектуре и подробности реализации Windows Hello см. в разделе [Руководство по Windows Hello на веб-сайте TechNet](https://docs.microsoft.com/windows/keep-secure/microsoft-passport-guide).

Полный пример кода см. в [образце кода Windows Hello на GitHub](https://go.microsoft.com/fwlink/?LinkID=717812).
...
Если пользователь задал PIN-код, приложение создаст [**KeyCredential**](https://docs.microsoft.com/uwp/api/Windows.Security.Credentials.KeyCredential) пользователя. Приложение также получает дополнительные сведения об аттестации ключа для получения криптографического подтверждения, что данный ключ создан в модуле TPM. Созданный открытый ключ и, при необходимости, аттестация, отправляются на внутренний сервер для регистрации используемого устройства. Каждая пара ключей, создаваемая на каждом устройстве, будет уникальной.

Код для создания [**KeyCredential**](https://docs.microsoft.com/uwp/api/Windows.Security.Credentials.KeyCredential) выглядит следующим образом.
...
- Подпись на большом двоичном объекте [**KeyAttestation**](https://docs.microsoft.com/uwp/api/Windows.Security.Cryptography.Certificates.KeyAttestationHelper) использует открытый ключ AIK.
- Открытый ключ, включенный в большой двоичный объект [**KeyAttestation**](https://docs.microsoft.com/uwp/api/Windows.Security.Cryptography.Certificates.KeyAttestationHelper), соответствует открытому ключу RSA, который клиент отправил наряду с оператором аттестации.
...
Первая строка, [**KeyCredentialManager.OpenAsync**](https://docs.microsoft.com/uwp/api/windows.security.credentials.keycredentialmanager.openasync), отправит запрос операционной системе на открытие дескриптора ключа. Если он будет выполнен успешно, вы сможете подписать сообщение запроса с помощью метода [**KeyCredential.RequestSignAsync**](https://docs.microsoft.com/uwp/api/windows.security.credentials.keycredential.requestsignasync), который заставит операционную систему запросить PIN-код или биометрические данные пользователя с помощью Windows Hello. Ни в один момент времени у разработчика не будет доступа к закрытому ключу пользователя. Они все защищаются с помощью API.
...
Далее сервер должен проверить подпись. При запросе открытый ключ и отправить его на сервер для будущих проверки, это в большом двоичном объекте publicKeyInfo кодировке ASN.1. Если взглянуть на [Windows Hello пример кода на GitHub](https://go.microsoft.com/fwlink/?LinkID=717812), вы увидите, что существуют вспомогательные классы для создания оболочки функция Crypt32 функции для преобразования в кодировке ASN.1 большого двоичного объекта в большой двоичный объект CNG, который используется чаще. Большой двоичный объект содержит алгоритм открытого ключа, который является RSA, и открытый ключ RSA.

В этом примере мы преобразуем большой двоичный объект кодировке ASN.1 в большой двоичный объект CNG связано с тем, чтобы он мог быть используется только с CNG (/ windows/desktop/SecCNG/cng портал) и BCrypt API. Если взглянуть CNG большой двоичный объект, она направит вас на связанную [BCRYPT_KEY_BLOB структуры](/windows/desktop/api/bcrypt/ns-bcrypt-_bcrypt_key_blob). Этой области API может использоваться для проверки подлинности и шифрования в приложениях Windows. ASN.1 — это документированных стандарт для взаимодействия структур данных, которые могут быть сериализованы, и она часто используется в криптографии с открытым ключом и с помощью сертификатов. Именно таким образом возвращаются сведения об открытом ключе. Открытый ключ является ключ RSA; и это алгоритм, который Windows Hello используется при подписи данных.
...
- [Обзор Windows Hello](https://support.microsoft.com/help/17215)
- [Сведения о реализации для Windows Hello](https://docs.microsoft.com/windows/keep-secure/microsoft-passport-guide)
- [Windows Hello пример кода на GitHub](https://go.microsoft.com/fwlink/?LinkID=717812)
...
| PKI | Инфраструктура открытых ключей, обычно используемая для указания среды, размещенной самой организацией и ответственной за создание ключей, отзыв ключей и т. д. |
...
| Аттестация ключей доверенного платформенного модуля (TPM Key) Attestation | Протокол, который криптографически подтверждает, что этот ключ привязан к TPM. Этот тип аттестации можно использовать, чтобы гарантировать, что определенная криптографическая операция произошла в TPM конкретного компьютера. |
