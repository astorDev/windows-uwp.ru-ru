...
В базовой реализации видеозахвата, описанной в статье [Фото- и видеозахват с помощью MediaCapture](capture-photos-and-video-with-mediacapture.md), объект профиля кодирования мультимедиа назначается локальной переменной, поскольку не используется в других местах кода. Для этого сценария следует назначать объект переменной-члену, чтобы можно было получить к нему доступ позже.
