...
Начиная c Windows 10 (версия 1607) класс **AdvancedPhotoCapture** можно использовать для съемки фото с применением встроенного алгоритма, который повышает качество фото, сделанных в условиях низкой освещенности.

> [!NOTE] 
> Одновременная запись видео и захват фотографий с помощью **AdvancedPhotoCapture** не поддерживается.

> [!NOTE] 
> Если свойство [**FlashControl.Enabled**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Devices.FlashControl.Enabled) имеет значение true, оно переопределит параметры **AdvancedPhotoCapture** и обычное фото будет сделано со вспышкой. Если параметр [**Auto**](https://msdn.microsoft.com/library/windows/apps/Windows.Media.Devices.FlashControl.Auto) имеет значение true, класс **AdvancedPhotoCapture** будет использоваться как настроенный, вспышка включаться не будет.
