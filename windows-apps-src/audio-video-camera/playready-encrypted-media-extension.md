...
description: В этом разделе описывается изменение PlayReady веб-приложения для поддержки изменения, внесенные в предыдущей версии Windows 8.1 до версии Windows 10.
...
В этом разделе описывается изменение PlayReady веб-приложения для поддержки изменения, внесенные в предыдущей версии Windows 8.1 до версии Windows 10.
...
Этот раздел содержит список изменений, внесенных для PlayReady зашифрованных носителей расширения (EME), чтобы включить защиту содержимого PlayReady в Windows 10.

Ниже перечислены новые функции и изменения, внесенные в PlayReady Encrypted Media расширения для Windows 10.
...
-   Обеспечивает получение нескольких лицензий в одном сообщении.
    ...
    Можно использовать объект PlayReady с несколькими идентификаторами ключей (KeyIDs) как в Windows 8.1, или использовать [данные модели расшифровки (CDMData) содержимого](https://go.microsoft.com/fwlink/p/?LinkID=626819) с несколькими KeyIDs.

    > [!NOTE]
    > В Windows 10, поддерживаются несколько идентификаторов ключей &lt;KeyID&gt; в CDMData.
...
> [!NOTE]
> Их безопасной остановки `<SessionID>B64 encoded session ID</SessionID>` в приведенном выше примере может быть символ звездочки (\*), который является символом подстановки для всех записанных сеансов безопасной остановки. То есть **SessionID** тега может быть определенный сеанс или подстановочного знака (\*) для выбора всех сеансов безопасной остановки.
...
В этом разделе перечислены вопросы программирования, которые следует учитывать при создании PlayReady с поддержкой веб-приложения для Windows 10.

Объекты **MSMediaKeys** и **MSMediaKeySession**, созданные приложением, должны оставаться активными, пока приложение не будет закрыто. Один из способов поддержания этих объектов в активном состоянии заключается в назначении их в качестве глобальных переменных (если переменные будут объявлены локальными переменными внутри функции, они окажутся вне диапазона и могут быть удалены в процессе сборки мусора). Например, следующий пример присваивает переменные *g\_msMediaKeys* и *g\_mediaKeySession* глобальными переменными, который затем назначаются **MSMediaKeys** и **MSMediaKeySession** объектов в функции.
