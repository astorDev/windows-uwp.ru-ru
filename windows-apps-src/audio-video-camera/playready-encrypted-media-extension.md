...
В данном разделе содержится список изменений, внесенных в расширение зашифрованного мультимедиа PlayReady (EME) и обеспечивающих защиту содержимого PlayReady в Windows 10.
...
-   Обеспечивает получение нескольких лицензий в одном сообщении.
    ...
    Вы можете использовать либо объект PlayReady с несколькими идентификаторами ключей, как в Windows 8.1 (KeyID), либо [данные модели расшифровки содержимого](https://docs.microsoft.com/previous-versions/windows/apps/dn457361(v=ieb.10)?redirectedfrom=MSDN) с несколькими идентификаторами ключей.

    > [!NOTE]
    > В Windows 10 несколько идентификаторов ключей поддерживаются в разделе &lt;KeyID&gt; в данных модели расшифровки содержимого.
...
> [!NOTE]
> `<SessionID>B64 encoded session ID</SessionID>` В примере выше может быть звездочка (\*), которая является подстановочным знаком для всех записанных сеансов безопасной регистрации. Это значит, что тег **SessionID** может быть конкретным сеансом или подстановочным знаком (\*) для выбора всех сеансов безопасных прерываний.
...
Дополнительные сведения см. в разделе [примеры приложений](https://github.com/microsoft/Windows-universal-samples/tree/master/Samples/PlayReady).
