...
В данной статье содержится краткий обзор того, как в Windows10 реализована доверенная среда выполнения.
