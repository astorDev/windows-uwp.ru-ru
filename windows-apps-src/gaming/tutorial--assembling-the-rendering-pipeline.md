...
ms.locfileid: "66367733"
---
# <a name="rendering-framework-i-intro-to-rendering"></a>Платформу для отображения инструкции: Введение в отрисовку

В предыдущих статьях мы рассмотрели, как структурировать игру универсальной платформы Windows (UWP) для работы в среде выполнения Windows, а также как определить конечный автомат для обработки потока игры. Теперь пора поговорить о построении платформы отрисовки. Давайте взглянем на прорисовку игр сцены с помощью Direct3D 11 (обычно называется DirectX 11) с помощью образца игры.
...
[Платформу для отображения II: Отрисовки игр](tutorial-game-rendering.md) охватывает шаг 2; как настроить платформу для отображения и как подготовки данных до отрисовки может произойти.
...
__Сделать\_разделяемая функция__, как показано [ниже](#appinitialize-method), используется для создания __общего\_ptr__ для [ __DX::D eviceResources__](#dxdeviceresources), который также предоставляет доступ к устройству. 
...
При запуске игры необходимо отрисовать игровую сцену. Инструкции по отрисовке запускаются в методе [__GameMain::Run__](#gamemainrun-method), как показано ниже.
...
Если включена [стереоскопическая отрисовка](#stereo-rendering), проходов отрисовки два: один для правого глаза, второй для левого. В каждом прохода отрисовки выполняется привязка целевого объекта отрисовки и представление трафарета глубины на устройстве. После этого мы также очищаем представление трафарета глубины.
...
Далее мы устанавливаем Direct3D контекст, используемый буферы констант, определенных в более ранних версиях, которые используются [вершинный построитель текстуры](#vertex-shaders-and-pixel-shaders) этапа конвейера и [текстуры](#vertex-shaders-and-pixel-shaders) этапа конвейера. 

> [!Note]
> См. в разделе [платформу для отображения II: Отрисовки игр](tutorial-game-rendering.md) Дополнительные сведения об определении буферов констант.
...
* Обновить буфер констант (__m\_constantBufferChangesEveryPrim__) с помощью модели [матрицу мирового преобразования](#world-transform-matrix) и данные по спецификации.
...
* Вызывается метод рисования для рисования индексированного, не являющегося экземпляром примитива. Метод __GameObject::Render__ обновляет [буфер констант](#constant-buffer-or-shader-constant-buffer), добавляя данные, характерные для конкретного примитива. В результате происходит вызов метода __DrawIndexed__ в контексте для рисования геометрии каждого примитива. В частности, этот вызов метода рисования помещает команды и данные в очередь графического процессора (GPU) с параметрами, заданными данными буфера констант. Каждый вызов draw выполняется вершинный построитель текстуры один раз на вершину, а затем [текстуры](#vertex-shaders-and-pixel-shaders) один раз для каждого пикселя каждого треугольника в примитиве. Текстуры являются частью состояния, которое построитель текстуры использует для прорисовки.

Почему буферов констант несколько?
    * Игра использует несколько буферов констант, но обновлять эти буферы ей нужно только по одному разу для каждого примитива. Как уже говорилось, буферы констант можно рассматривать как входные данные для шейдеров, которые выполняются для каждого примитива. Некоторые данные являются статическими (__m\_constantBufferNeverChanges__); некоторые данные испытывает кадр (__m\_constantBufferChangesEveryFrame__), такие как положение камеры. и некоторые данные относится только к примитивом, например его цвета и текстуры (__m\_constantBufferChangesEveryPrim__)
    * Обработчик игры разделяет эти входные данные по разным буферам констант, чтобы оптимизировать пропускную способность памяти, используемую ЦП и GPU. Этот подход также помогает свести к минимуму количество данных, которые должен отслеживать GPU. У GPU большая очередь команд, и каждый раз, когда игра вызывает метод __Draw__, эта команда добавляется в очередь вместе со связанными с ней данными. Когда игра обновляет буфер констант примитива и выдает следующую команду __Draw__, графический драйвер добавляет эту команду и связанные с ней данные в очередь. Если игра рисует 100 примитивов, в очереди может накопиться 100 копий данных буфера констант. Чтобы свести к минимуму количество данных, которое игра отправляет GPU, игра использует отдельный буфер констант примитива, который содержит только обновления для каждого примитива.
...
* Метод __HandleDeviceLost__ используется для обработки сценария, когда [устройство](#device) удаляется. Если устройство было удалено путем отключения или обновления драйвера, необходимо заново создать все ресурсы устройства. Подробнее: [Обработка сценариев удаления устройства в Direct3D 11](handling-device-lost-scenarios.md).
...
В этой статье была рассмотрена отрисовка графики на дисплее, с кратким описанием некоторых терминов, используемых применительно к отрисовке. Дополнительные сведения о подготовке к просмотру в [платформу для отображения II: Отрисовки игр](tutorial-game-rendering.md) статью и узнайте, как подготовить данные, необходимые перед отрисовкой.
...
* [Шейдеры](#Shaders)
* [Вершинные построители текстуры и шейдеры](#vertext-shaders-pixel-shaders)
* [Этапов шейдера](#shader-stages)
* [Форматы файлов шейдера](#various-shader-file-formats)

Подробнее: [Общие сведения о конвейере отрисовки Direct3D 11](https://docs.microsoft.com/windows/desktop/direct3dgetstarted/understand-the-directx-11-2-graphics-pipeline) и [Графический конвейер](https://docs.microsoft.com/windows/desktop/direct3d11/overviews-direct3d-11-graphics-pipeline).

#### <a name="hlsl"></a>HLSL

HLSL — это высокоуровневый язык шейдеров (High Level Shading Language) для DirectX. Используя HLSL, можно создавать C-подобные программируемые шейдеры для конвейера Direct3D. Подробнее: [HLSL](https://docs.microsoft.com/windows/desktop/direct3dhlsl/dx-graphics-hlsl).
...
Дополнительные сведения см. в следующих разделах: [Представление трафарета глубины](../graphics-concepts/depth-stencil-view--dsv-.md), [буфер глубины](../graphics-concepts/depth-buffers.md), и [буфер шаблона](../graphics-concepts/stencil-buffers.md).

#### <a name="render-target"></a>Целевой объект отрисовки

Целевой объект отрисовки — это ресурс, в который производится запись данных по окончании этапа отрисовки. Обычно он создается с помощью метода [ID3D11Device::CreateRenderTargetView](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11device-createrendertargetview) с использованием заднего буфера цепочки буферов (который также является ресурсом) в качестве входного параметра. 

Каждый целевой объект отрисовки также должен иметь соответствующее представление трафарета глубины, потому что мы используем метод [OMSetRenderTargets](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-omsetrendertargets) для задания целевого объекта отрисовки, прежде чем использовать его, и этому методу требуется в том числе представление трафарета глубины. Доступ к ресурсу целевого объекта отрисовки осуществляется через представление целевого объекта отрисовки, реализуемого с помощью интерфейса [ID3D11RenderTargetView](https://docs.microsoft.com/windows/desktop/api/d3d11/nn-d3d11-id3d11rendertargetview). 
...
Устройство представляется интерфейсом [ID3D11Device](https://docs.microsoft.com/windows/desktop/api/d3d11/nn-d3d11-id3d11device). Иными словами, интерфейс ID3D11Device представляет виртуальный видеоадаптер и используется для создания ресурсов, которые принадлежат устройству. 

Обратите внимание, что существуют различные версии ID3D11Device. [ID3D11Device5](https://docs.microsoft.com/windows/desktop/api/d3d11_4/nn-d3d11_4-id3d11device5) — это последняя версия, в которой добавлены новые методы относительно ID3D11Device4. Подробнее о том, как Direct3D взаимодействует с оборудованием, см. в статье [Архитектура модели драйверов устройств с Windows (WDDM)](https://docs.microsoft.com/windows-hardware/drivers/display/windows-vista-and-later-display-driver-model-architecture).

К каждого приложения должно быть хотя бы одно устройство; большинство приложений создают только одно устройство. Создание устройства для одного из драйверов оборудования, установленных на компьютере путем вызова __D3D11CreateDevice__ или __D3D11CreateDeviceAndSwapChain__ и указав тип драйвера с D3D\_ ДРАЙВЕР\_флаг ТИПА. Каждое устройство можно использовать один или несколько контекстов устройств, в зависимости от требуемой функциональности. Подробнее: [Функция D3D11CreateDevice](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-d3d11createdevice).
...
Direct3D 11 реализует два типа контекста устройств: один для немедленной отрисовки и другой для отложенной отрисовки; оба контекста представляются с помощью интерфейса [ID3D11DeviceContext](https://docs.microsoft.com/windows/desktop/api/d3d11/nn-d3d11-id3d11devicecontext).  
...
Примечание. __ID3D11DeviceContext4__ впервые появился в Windows 10 Creators Update и — последняя версия __ID3D11DeviceContext__ интерфейс. В приложениях, предназначенных для Windows 10 Creators Update, следует использовать этот интерфейс, а не более ранние версии. Подробнее: [ID3D11DeviceContext4](https://docs.microsoft.com/windows/desktop/api/d3d11_3/nn-d3d11_3-id3d11devicecontext4).
...
Подробнее: [Вводные сведения о буферах в Direct3D 11](https://docs.microsoft.com/windows/desktop/direct3d11/overviews-direct3d-11-resources-buffers-intro).

### <a name="dxgi"></a>DXGI

Microsoft DirectX Graphics Infrastructure (DXGI) — это новая подсистема, которая впервые появилась в Windows Vista, которая инкапсулирует некоторые низкоуровневые задачи, которые необходимы на Direct3D 10, 10.1, 11 и 11.1. В многопоточных приложениях использовать DXGI необходимо с особой осторожностью во избежание возникновения взаимоблокировок. Дополнительные сведения см. в разделе [DirectX Graphics Infrastructure (DXGI): Лучшие методики-многопоточности](https://docs.microsoft.com/windows/desktop/direct3darticles/dxgi-best-practices)
...
Используя уровень компонентов, при создании устройства вы можете попытаться создать устройство для уровня компонентов, который вы хотите запросить. Если создать устройство удалось, этот уровень компонентов существует; если нет, оборудование не поддерживает этот уровень компонентов. Можно либо попробовать заново создать устройство на более низком уровне компонентов, либо закрыть приложение. Например, 12\_0 функциональный уровень требует Direct3D 11.3 или Direct3D 12 и модель шейдера 5.1. Дополнительные сведения см. в разделе [функциональные уровни Direct3D: Общие сведения для каждого уровня функция](https://docs.microsoft.com/windows/desktop/direct3d11/overviews-direct3d-11-devices-downlevel-intro).

Используя функциональные уровни, можно разработать приложение Direct3D 9, Microsoft Direct3D 10 или Direct3D 11 и затем запустите его на 9, 10 или 11 оборудования (за некоторыми исключениями). Подробнее: [Уровни компонентов Direct3D](https://docs.microsoft.com/windows/desktop/direct3d11/overviews-direct3d-11-devices-downlevel-intro).
...
Где: 
* M(модель-мир) — матрица преобразования координат модели в мировые координаты, также называемая [матрицей мирового преобразования](#world-transform-matrix). Обеспечивается примитивом.
* M(мир-представление) — матрица преобразования мировых координат в координаты представления, также называемая [матрицей видового преобразования](#view-transform-matrix).
    * Обеспечивается матрицей представления камеры. Определяется положением камеры и векторами взгляда (вектор "взгляд на", который направлен из камеры прямо на сцену, и вектор "взгляд вверх", который направлен вверх, перпендикулярно ему).
    * В образец игры __m\_viewMatrix__ матрицы преобразования представления и вычисляется с помощью __Camera::SetViewParams__ 
* M(представление-устройство) — матрица преобразования координат представления в координаты устройства, также называемая [матрицей проекционного преобразования](#projection-transform-matrix).
    * Обеспечивается проекцией камеры. Содержит информацию о том, какая часть этот пространства фактически видна в окончательной сцене. Матрица проекционного преобразования определяется полем зрения, пропорциями и плоскостями обрезки.
    * В образец игры __m\_projectionMatrix__ определяет преобразование к значениям координат проекции, вычисляемое с использованием __Camera::SetProjParams__ (стерео проекции, используются два проекция матрицы: один для представления каждого глаза.) 
