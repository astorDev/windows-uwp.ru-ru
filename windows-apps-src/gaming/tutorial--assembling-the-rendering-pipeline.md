...
Примечание. __ID3D11DeviceContext4__ впервые появился в Windows 10 Creators Update и — последняя версия __ID3D11DeviceContext__ интерфейс. В приложениях, предназначенных для Windows 10 Creators Update, следует использовать этот интерфейс, а не более ранние версии. Подробнее: [ID3D11DeviceContext4](https://msdn.microsoft.com/library/windows/desktop/mt492481.aspx).
...
Используя уровень компонентов, при создании устройства вы можете попытаться создать устройство для уровня компонентов, который вы хотите запросить. Если создать устройство удалось, этот уровень компонентов существует; если нет, оборудование не поддерживает этот уровень компонентов. Можно либо попробовать заново создать устройство на более низком уровне компонентов, либо закрыть приложение. Например, 12\_0 функциональный уровень требует Direct3D 11.3 или Direct3D 12 и модель шейдера 5.1. Дополнительные сведения см. в разделе [функциональные уровни Direct3D: Общие сведения для каждого уровня функция](https://msdn.microsoft.com/library/windows/desktop/ff476876.aspx#Overview).

Используя функциональные уровни, можно разработать приложение Direct3D 9, Microsoft Direct3D 10 или Direct3D 11 и затем запустите его на 9, 10 или 11 оборудования (за некоторыми исключениями). Подробнее: [Уровни компонентов Direct3D](https://msdn.microsoft.com/library/windows/desktop/ff476876.aspx).
