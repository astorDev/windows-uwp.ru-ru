...
Если включена [стереоскопическая отрисовка](#stereo-rendering), проходов отрисовки два: один для правого глаза, второй для левого. На каждом проходе отрисовки мы привязываем целевой объект отрисовки и представление трафарета глубины к устройству. После этого мы также очищаем представление трафарета глубины.
...
Следующий шаг— настроить контекст Direct3D на использование ранее определенных буферов констант, используемых на двух этапах конвейера: [вершинными шейдерами](#vertex-shaders-and-pixel-shaders) и [построителями текстур](#vertex-shaders-and-pixel-shaders). 
