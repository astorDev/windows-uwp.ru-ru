...
В Windows8.1 в инфраструктуру DXGI был добавлен флаг **DXGI\_SWAP\_CHAIN\_FLAG\_FRAME\_LATENCY\_WAITABLE\_OBJECT** для цепочки буферов, что позволяет приложениям легко сократить задержку без необходимости использовать эвристику для очистки очереди Present. Цепочки буферов с таким флагом называются цепочками буферов с поддержкой ожидания. На рисунке2 представлены примерный жизненный цикл и реакция на событие ввода для приложений, использующих цепочки буферов с поддержкой ожидания.
