...
-   [Сравнение OpenGL ES 2.0 и Direct3D 11](#compare)
-   [Перенос переменных GLSL в HLSL](#variables)
-   [Перенос типов GLSL в HLSL](#types)
-   [Перенос предварительно определенных глобальных переменных GLSL в HLSL](#porting_glsl_pre-defined_global_variables_to_hlsl)
-   [Пример переноса переменных GLSL в HLSL](#example1)
    -   [Неизменная переменная, атрибут и меняющаяся переменная в GLSL](#uniform___attribute__and_varying_in_glsl)
    -   [Буферы констант и передача данных в HLSL](#constant_buffers_and_data_transfers_in_hlsl)
-   [Примеры переноса кода отрисовки OpenGL в Direct3D](#example2)
...
Вы используете семантику, чтобы указать положение, цвет и прочие сведения для входных данных вершинного шейдера и построителя текстуры. Значения семантики в макете входных данных должны соответствовать входным данным вершинного шейдера. Например, см. раздел [Примеры переноса переменных GLSL в HLSL](#example1). Подробнее о семантике HLSL: [Семантика](https://msdn.microsoft.com/library/windows/desktop/bb509647).
...
* [Перенос из OpenGL ES 2.0 в Direct3D 11](port-from-opengl-es-2-0-to-directx-11-1.md)
