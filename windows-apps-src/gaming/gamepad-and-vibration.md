...
> [!NOTE]
> `Windows.Gaming.Input.Gamepad` также поддерживает геймпады Xbox 360, которые имеют один и тот же макет элемента управления, как и стандартные геймпады Xbox One.
...
> [!NOTE]
> Геймпады Xbox 360 не имеют с _импульсных триггеров_.
...
Контроллер навигации в пользовательском Интерфейсе геймпадов сопоставлять [Обязательный набор](ui-navigation-controller.md#required-set) команд навигации для левого джойстика, Крестовина, **представления**, **меню**, **A**и **B** кнопки.
...
При добавлении или удалении геймпада, [GamepadAdded][] и [GamepadRemoved][] возникают. Можно зарегистрировать обработчики для этих событий, чтобы отслеживать подключенные в данный момент геймпады.
...
Следующий пример кода останавливает отслеживание удаленного геймпада. Также необходимо обработать, что происходит с геймпадов, вы отслеживаете после их удаления; Например, этот код только отслеживает входные данные с одного геймпада, а просто задает для него `nullptr` при удалении. Необходимо проверить каждого кадра, если активно вашего игрового контроллера и обновления, какие геймпада входных данных сбор, когда контроллеров подключения и отключения.
...
Все кнопки геймпада&mdash;— четыре направления крестовины, левый и правый бамперы, нажатие левого и правого аналогового стика, **A**, **B**, **X**, **Y**, **представления**и **меню**&mdash;предоставляет цифровые данные, Указывает, имеет ли он нажата (вниз) или отпущена (вверх). Для повышения эффективности показания кнопок не указываются в виде отдельных логических значений; Вместо этого все они упаковываются в одно битовое поле, представляемое перечислением [GamepadButtons][] .
...
Несмотря на то что члены `Gamepad.Vibration` свойство можно изменить напрямую, рекомендуется инициализации отдельного `GamepadVibration` экземпляр значения, вы хотите, а затем скопировать его в `Gamepad.Vibration` свойства одновременно изменить фактические моторов.
