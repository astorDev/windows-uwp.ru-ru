...
* как игровые панели будут рассматриваться в качестве пользовательского интерфейса навигации устройств
...
Один Xbox игровые панели оборудованных направления набора (или набора D); **A**, **B**, **X**, **Y**, **представления**и кнопки **меню** ; thumbsticks влево и вправо, нему и триггеров; и всего четыре моторов вибрации. Оба аналоговых стика обеспечивают двойное считывание аналоговых данных по осям X и Y, а также выступают как кнопки при нажатии вниз. Каждый триггер содержит аналоговый чтения, представляющий, насколько он извлекается обратно.
...
> [!NOTE]
> `Windows.Gaming.Input.Gamepad` также поддерживает игровые планшеты Xbox 360, что стандартных игровые панели Xbox один и тот же макет элемента управления.
...
> [!NOTE]
> Игровые панели Xbox 360 не снабжены _импульсных триггеров_.
...
Как контроллер пользовательский Интерфейс навигации игровые панели сопоставить [необходимый набор](ui-navigation-controller.md#required-set) команд навигации слева thumbstick, D-набора, **представления**, **меню**, **A**и **B** кнопок.
...
Класс [Gamepad][] имеет статическое свойство [Gamepads][] — доступный только для чтения список подключенных геймпадов. Так как только будет интересно в некоторых подключенных игровые панели, рекомендуется поддерживать коллекции вместо доступ к их с помощью `Gamepads` свойство.

В следующем примере кода выполняется копирование всех подключенных геймпадов в новую коллекцию. Обратите внимание, так как другие потоки в фоновом режиме будут иметь доступ к этой коллекции (в события [GamepadAdded][] и [GamepadRemoved][] ), необходимо заблокировать любой код, который считывает и обновляет коллекцию.
...
В следующем примере останавливается, отслеживание игровой, который был удален. Также необходимо обрабатывать, что происходит с игровые панели, отслеживаются при их удалением; к примеру, этот код отслеживает только входные данные из одного игровой и просто задает для него значение `nullptr` при удалении. Вам потребуется проверка каждые frame, если ваш игровой активен и обновления, какие игровой ввода данных в случае сбора, когда контроллеры подключения и отключения.
...
Для получения дополнительных сведений в разделе [рекомендации входных данных для игр](input-practices-for-games.md) .
...
Каждый стик позволяет считывать с него аналоговые данные в диапазоне от -1,0 до +1,0 по осям X и Y. На оси X значение -1,0 соответствует крайнему левому положению стика, значение +1,0 — крайнему правому. На оси Y значение -1,0 соответствует крайнему нижнему положению стика, а значение +1,0 — крайнему верхнему. В обоих осей значение составляет примерно 0.0 упрощенный находится в положение центра, когда он является нормальной точное значение для изменения, даже между последующих показания; в этом разделе позднее обсуждаются стратегии по смягчению этого варианта.
...
Кнопки игровой&mdash;четыре указаниям из набора D, нему влево и вправо, нажмите клавишу thumbstick влево и вправо, **A**, **B**, **X**, **Y**, **представления**и **меню**&mdash;предоставляет цифровыми, чтение Указывает, имеет ли оно нажата (вниз) или выпущен (вверх). Для повышения эффективности показания кнопка не представленные в виде отдельных значений типа boolean. Вместо этого они лучше все защищены в одной битами, представленным в перечислении [GamepadButtons][] .
...
В некоторых случаях может потребоваться определить, когда кнопка переходит из пользователь нажимает, чтобы выпущенной или выпущен, чтобы нажатия, являются ли несколько кнопок нажата или выпущен или набор кнопок объединены определенным образом&mdash;некоторые нажата, некоторые не. Сведения о том, как определить каждое из этих условий, см. в разделах [Определение положений кнопки](input-practices-for-games.md#detecting-button-transitions) и [Определение сложных схем положений кнопок](input-practices-for-games.md#detecting-complex-button-arrangements).
...
Хотя члены `Gamepad.Vibration` свойства можно изменять напрямую, рекомендуется инициализации отдельных `GamepadVibration` экземпляра для значений будет, а затем скопируйте его в `Gamepad.Vibration` свойства для изменения фактических motor интенсивности за один раз.
