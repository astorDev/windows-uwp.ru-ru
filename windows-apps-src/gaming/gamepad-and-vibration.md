...
Контроллер навигации в пользовательском Интерфейсе считанных сопоставлять [необходимый набор](ui-navigation-controller.md#required-set) команд навигации для левого джойстика, Крестовина, **представления**, **меню**, **A**и **B** кнопки.
...
Следующий пример кода останавливает отслеживание удаленного геймпада. Вам также понадобится для обработки действий на геймпады, который вы отслеживаете после их удаления; Например, этот код только отслеживает входные данные с одного геймпада, а просто задает для него `nullptr` при удалении. Вам потребуется проверить каждый кадр, если активно вашего игрового контроллера и обновления, какой игрового контроллера ввод от сбор, когда контроллеров подключения и отключения.
...
Несмотря на то что члены `Gamepad.Vibration` свойство можно изменить напрямую, рекомендуется инициализации отдельного `GamepadVibration` экземпляр значения требуется, а затем скопировать его в `Gamepad.Vibration` свойства для изменения фактического моторов все сразу.
