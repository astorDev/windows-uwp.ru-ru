...
description: Используйте API контроллера навигации по пользовательскому интерфейсу Windows.Gaming.Input для обнаружения и прочтения различных устройств ввода для навигации по пользовательскому интерфейсу.
...
Команды направления предназначены для базовой [навигации по осям XY](../design/input/gamepad-and-remote-interactions.md#xy-focus-navigation-and-interaction) между отдельными элементами пользовательского интерфейса. Команды просмотра и меню предназначены для отображения информации об игре (часто кратковременной и иногда модальной) и для переключения между игрой и контекстами меню соответственно. Команды принятия и отмены предназначены для утвердительных (да) и отрицательных (нет) ответов соответственно.
...
| Команда навигации | Ввод с геймпада          | Ввод с аркадного джойстика | Ввод с гоночного руля    |
| ------------------:| ---------------------- | ------------------ | --------------------- |
|             PageUp | Левый триггер           | _не поддерживается_    | _зависит от_              |
|           PageDown | Правый триггер          | _не поддерживается_    | _зависит от_              |
|           PageLeft | Левый бампер            | _не поддерживается_    | _зависит от_              |
|          PageRight | Правый бампер           | _не поддерживается_    | _зависит от_              |
|           ScrollUp | Правый аналоговый стик вверх    | _не поддерживается_    | _зависит от_              |
|         ScrollDown | Правый аналоговый стик вниз  | _не поддерживается_    | _зависит от_              |
|         ScrollLeft | Правый аналоговый стик влево  | _не поддерживается_    | _зависит от_              |
|        ScrollRight | Правый аналоговый стик вправо | _не поддерживается_    | _зависит от_              |
|           Context1 | Кнопка х               | _не поддерживается_    | Кнопка X (_часто_) |
|           Context2 | Кнопка Y               | _не поддерживается_    | Кнопка Y (_часто_) |
|           Context3 | Нажатие левого аналогового стика  | _не поддерживается_    | _зависит от_              |
|           Context4 | Нажатие правого аналогового стика | _не поддерживается_    | _зависит от_              |
...
[Пример InputInterfacingUWP _(github)_](https://github.com/Microsoft/Xbox-ATG-Samples/tree/master/UWPSamples/System/InputInterfacingUWP) показывает, как разные устройства ввода ведут себя в качестве контроллеров навигации по пользовательскому интерфейсу.
