...
description: Здесь мы обсудим рекомендации по обработке поворота экрана в приложении DirectX UWP, чтобы обеспечить эффективное и эффективное использование графического оборудования устройства Windows 10.
...
ms.locfileid: "74258518"
---
# <a name="supporting-screen-orientation-directx-and-c"></a>Поддержка ориентации экрана (DirectX и C++)



Приложение универсальной платформы Windows (UWP) может поддерживать разные варианты ориентации экрана при обработке события [**DisplayInformation::OrientationChanged**](https://docs.microsoft.com/uwp/api/windows.graphics.display.displayinformation.orientationchanged). Здесь мы обсудим рекомендации по обработке поворота экрана в приложении DirectX UWP, чтобы обеспечить эффективное и эффективное использование графического оборудования устройства Windows 10.

Вначале запомните, что графическое оборудование всегда выводит данные о пикселях одинаковым образом независимо от ориентации устройства. Устройства Windows 10 могут определить текущую ориентацию экрана (с некоторым датчиком или с помощью переключателя программного обеспечения) и разрешить пользователям изменять параметры отображения. По этой причине Windows 10 обрабатывает поворот изображений, чтобы убедиться, что они расположены в вертикальном порядке, исходя из ориентации устройства. По умолчанию ваше приложение получает уведомление о том, что в ориентации что-то изменилось (например, размер окна). В этом случае Windows 10 немедленно поворачивает образ для окончательного вывода. Для трех из четырех конкретных ориентаций экрана (обсуждаемых ниже) в Windows 10 используются дополнительные графические ресурсы и вычисления для отображения окончательного изображения.
...
В Windows 10 определено четыре режима ориентации экрана:

-   Альбомная — ориентация экрана по умолчанию для Windows 10, которая считается основанием для вращения (0 градусов).
...
Когда экран поворачивается с одной ориентации на другую, Windows 10 внутренне выполняет операцию вращения, чтобы выстроить изображение с новой ориентацией, и пользователь видит на экране вертикальное изображение.

Кроме того, Windows 10 отображает анимацию автоматического перехода, чтобы обеспечить плавное взаимодействие с пользователем при сдвиге от одной ориентации к другой. При изменении ориентации экрана для пользователя эти переходы выглядят как фиксированная анимация с увеличением и поворотом изображения на экране. Время выделяется Windows 10 приложению для макета в новой ориентации.
...
2.  Уведомить Windows 10 о ориентации цепочки буферов с помощью [**IDXGISwapChain1:: сетротатион**](https://docs.microsoft.com/windows/desktop/api/dxgi1_2/nf-dxgi1_2-idxgiswapchain1-setrotation).
...
Кстати, координаты [**CoreWindow**](https://docs.microsoft.com/uwp/api/Windows.UI.Core.CoreWindow) всегда определяются с использованием значений DIP. Для Windows 10 и более ранних версий Windows DIP определяется как 1/1/96 дюйма и соответствует определению *ОС.* Когда ориентация экрана после поворота становится книжной, приложение меняет местами ширину и высоту **CoreWindow**. При этом размер однобуферной прорисовки (границ) должен измениться соответствующим образом. Поскольку координаты Direct3D всегда представлены в виде физических пикселей, требуется преобразовать значения DIP для **CoreWindow** в целочисленные пиксельные значения перед передачей их в Direct3D для настройки цепочки буферов.
...
-   После этого задайте подходящее преобразование двухмерной или трехмерной матрицы, которое будет применяться соответственно к пикселям или вершинам в конвейере графики во время их рендеринга в цепочку буферов. У нас есть 4 возможных матрицы вращения:
    ...
    -   Альбомная (режим\_DXGI\_ВРАЩЕНИЕ\_УДОСТОВЕРЕНие)
    -   Книжная (режим\_DXGI\_ВРАЩЕНИЕ\_ROTATE270)
    -   альбомная, зеркально отраженная (режим\_DXGI\_ВРАЩЕНИЕ\_ROTATE180)
    -   Книжная, зеркально отраженная (режим\_DXGI\_ВРАЩЕНИЕ\_ROTATE90)

    Правильная матрица выбирается на основе данных, предоставляемых Windows 10 (например, результатов [**DisplayInformation:: OrientationChanged**](https://docs.microsoft.com/uwp/api/windows.graphics.display.displayinformation.orientationchanged)), для определения ориентации экрана, и она будет умножена на координаты каждого пиксела (Direct2D) или вершины (Direct3D) в сцене, эффективно поворачивая их в соответствии с ориентацией экрана. Учтите, что в Direct2D в качестве начала координат экрана определен верхний левый угол, тогда как в Direct3D начало координат определено как логический центр окна.
...
Когда пользователь поворачивает ориентацию дисплея, Windows 10 отображает анимацию, независимую от приложения, в качестве отзыва пользователю. Анимация состоит из трех частей, расположенных в следующем порядке.

-   Windows 10 сжимает исходный образ.
-   Windows 10 содержит образ на время, необходимое для перестроения нового макета. Это тот период времени, который вы хотели бы сократить, поскольку, возможно, вашему приложению весь он не нужен.
...
Как было предложено в третьем маркере, когда приложение вызывает [**нотифилайауткомплетед**](https://docs.microsoft.com/uwp/api/windows.ui.core.corewindowresizemanager.notifylayoutcompleted), Windows 10 останавливает окно время ожидания, завершает анимацию вращения и возвращает управление приложению, которое теперь рисуется на новой ориентации экрана. Общий эффект состоит в том, что теперь ваше приложение стало чуть более легким на подъем и отзывчивым. И работает оно немного эффективней!
...
Возможно, вы заметили в примера кода из раздела [Изменение размера цепочки буферов и предварительный поворот ее содержимого](#resizing-the-swap-chain-and-pre-rotating-its-contents) (и в [примере поворота цепочки буферов DXGI](https://code.msdn.microsoft.com/windowsapps/DXGI-swap-chain-rotation-21d13d71)), что у нас были отдельные матрицы поворота для вывода Direct2D и вывода Direct3D. Давайте сначала рассмотрим двухмерные матрицы.
...
В примере кода раздела [Изменение размера цепочки буферов и предварительный поворот ее содержимого](#resizing-the-swap-chain-and-pre-rotating-its-contents) (и в [пример поворота цепочки буферов DXGI](https://code.msdn.microsoft.com/windowsapps/DXGI-swap-chain-rotation-21d13d71)) мы определили особую матрицу преобразования для каждой из возможных ориентаций экрана. Теперь давайте взглянем на матрицы для поворота трехмерных сцен. Как и раньше, вы создаете набор матриц для каждой из четырех возможных ориентаций. Во избежание ошибок при округлении, вызывающих небольшие искажения графики, явно объявите эти матрицы в своем коде.

Матрицы трехмерного вращения настраиваются следующим образом. Матрицы, указанные в приведенном ниже примере кода, относятся к категории стандартных матриц вращения для поворотов на 0, 90, 180 и 270 градусов вершин, определяющих точки в трехмерном пространстве сцены камеры. Значение координаты \[x, y, z\] вершины в сцене умножается на эту матрицу вращения при вычислении двухмерной проекции сцены.
