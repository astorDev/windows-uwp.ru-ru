...
-   Используйте **приложение DirectX 11 (универсальных Windows - C++/CX)** шаблона в Visual Studio для создания игры DirectX универсальной платформы Windows.
...
Когда мы создавали проект Visual Studio для Marble Maze, мы начинали с существующего проекта. Тем не менее, если у вас еще нет существующего проекта, который предоставляет основные функциональные возможности, необходимые для вашей игры DirectX универсальной платформы Windows, мы рекомендуем создать проект на основе Visual Studio **приложение DirectX 11 (универсальных Windows - C++/CX)** шаблона, так как она предоставляет простое рабочее приложение 3D. Для этого выполните следующие действия:

1. В Visual Studio 2019 г., выберите **файл > Создать > проект...**

2. В **создайте новый проект** выберите **приложение DirectX 11 (универсальных Windows - C++/CX)** . Если этот параметр не отображается, отсутствуют необходимые компоненты установлены&mdash;см. в разделе [изменение Visual Studio 2019 г. путем добавления или удаления рабочих нагрузок и компонентов](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) сведения об установке дополнительных компонентов .

![Новый проект](images/vs2019-marble-maze-sample-fundamentals-1.png)

3. Выберите **Далее**, а затем введите **имя_проекта**, **расположение** для файлов для сохранения и **имя решения**и выберите  **Создание**.



Один важный параметр в **приложение DirectX 11 (универсальных Windows - C++/CX)** шаблон **/ZW** параметр, который позволяет программе использовать расширения языка среды выполнения Windows. При использовании шаблона Visual Studio этот параметр включен по умолчанию. В разделе [Настройка параметров компилятора](https://docs.microsoft.com/cpp/build/reference/setting-compiler-options) представлены дополнительные сведения о том, как настроить параметры компилятора в Visual Studio.
