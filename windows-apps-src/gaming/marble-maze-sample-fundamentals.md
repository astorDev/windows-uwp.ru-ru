...
-   При вызове процедуры DirectX, которая возвращает **HRESULT**, используйте функцию **DX::ThrowIfFailed**. Эта функция определена в [DirectXHelper.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/master/C%2B%2B/Shared/DirectXHelper.h). **ThrowIfFailed** выдает исключение, если переданный **HRESULT** является кодом ошибки. Например **E\_УКАЗАТЕЛЬ** вызывает **ThrowIfFailed** исключение [Platform::NullReferenceException](https://msdn.microsoft.com/library/windows/apps/hh755823.aspx).
