...
description: В этом документе описываются основные характеристики мрамор создается проект; к примеру как использует Visual C++ в среду выполнения Windows, как он создается и структурирован и его построение.
...
-   Используйте [примечания SAL](https://docs.microsoft.com/visualstudio/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects) вместе с средства анализа кода для обнаружения ошибок в приложении.

## <a name="creating-the-visual-studio-project"></a>Создание проекта Visual Studio


Если загружена и извлекаться примера, файл **MarbleMaze_VS2017.sln** (в папке **C++** ) можно открыть в Visual Studio и вам придется кода перед собой.
...
2. В окне **Создание проекта** в левой боковой панели выберите **Installed > Шаблоны > Visual C++**.

3. Выберите **Приложение 11 DirectX (универсальные Windows)** в среднем списке. Если этот параметр не отображается, может не иметь все необходимые компоненты,&mdash;подробные сведения об установке дополнительных компонентов [Изменение Visual Studio 2017 путем добавления или удаления рабочих нагрузок и компоненты](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) .

4. Присвоить проекту **имя** **расположения** для файлов должны храниться и **имя решения**и нажмите **кнопку ОК**.

![Создание проекта](images/marble-maze-sample-fundamentals-1.png)
...
Marble Maze использует **^** совместно с **Microsoft::WRL::ComPtr**, чтобы управлять динамически выделенными объектами и свести к минимуму потери памяти. Мы рекомендуем использовать ^ для управления жизненным циклом переменных среды выполнения Windows **ComPtr** для управления жизненным циклом переменных COM (например, при использовании DirectX) и **std::shared\_ptr** или **std::unique\_ptr** для управления жизненным циклом для всех других Выделенная кучи C++ объекты.
...
-   При вызове процедуры DirectX, которая возвращает **HRESULT**, используйте функцию **DX::ThrowIfFailed**. Эта функция определяется в [DirectXHelper.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/master/C%2B%2B/Shared/DirectXHelper.h). **ThrowIfFailed** создает исключение, если предоставленное **значение HRESULT** является код ошибки. Например, **E\_POINTER** заставляет **ThrowIfFailed** вызвать [Platform::NullReferenceException](https://msdn.microsoft.com/library/windows/apps/hh755823.aspx).
...
-   Хотя мы рекомендуем не использование **HRESULT** для непредвиденные ошибки, более важна отказаться от использования обработку исключений для управления выполнением кода. Поэтому, когда возникает необходимость управлять потоком кода, предпочтительнее использовать значение, возвращаемое **HRESULT**.
...
Рассмотрим метод **BasicLoader::LoadMesh** , который объявляется в [BasicLoader.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/e62d68a85499e208d591d2caefbd9df62af86809/C%2B%2B/Shared/BasicLoader.h). Этот метод использует `_In_` для указания *имени файла* является входного параметра (и поэтому будут только считываться), `_Out_` для указания, что *vertexBuffer* и *indexBuffer* являются выходные параметры (и поэтому будут только для записи) и `_Out_opt_` для указания, что *vertexCount* и *indexCount* являются необязательными выходные параметры (и могут быть записаны в). Поскольку *vertexCount* и *indexCount* являются необязательными выходными параметрами, они могут принимать значение **nullptr**. Средство C/C++ Code Analysis проверяет вызовы этого метода, чтобы убедиться, что параметры, передаваемые им, удовлетворяют этим критериям.
