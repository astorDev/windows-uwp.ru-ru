...
-   Используйте шаблон **приложения DirectX 11 (универсальный Windows- C++/CX)** в Visual Studio для создания игры DirectX UWP.
...
-   Используйте ссылки на объекты с символом "Hat" (^) для управления временем существования переменных среда выполнения Windows, [Microsoft:: WRL:: ComPtr](https://docs.microsoft.com/cpp/windows/comptr-class) для управления временем существования объектов COM, а [std:: Shared\_PTR](https://docs.microsoft.com/cpp/standard-library/shared-ptr-class) или [std:: Unique\_PTR](https://docs.microsoft.com/cpp/standard-library/unique-ptr-class) для управления временем существования всех C++ остальных объектов, выделенных в куче.
...
Когда мы создавали проект Visual Studio для Marble Maze, мы начинали с существующего проекта. Однако если у вас еще нет проекта, который предоставляет базовые функциональные возможности, необходимые для игры DirectX UWP, мы рекомендуем создать проект на основе шаблона приложения Visual Studio **DirectX 11 (универсальное приложение Windows- C++/CX)** , поскольку он предоставляет базовое рабочее приложение. Для этого выполните следующие действия:

1. В Visual Studio 2019 выберите **файл > создать > проект...**

2. В окне **Создание нового проекта** выберите **приложение DirectX 11 (универсальный Windows- C++/CX)** . Если этот параметр не отображается, то возможно, что необходимые компоненты не установлены&mdash;см. статью [изменение Visual Studio 2019 путем добавления или удаления рабочих нагрузок и компонентов](https://docs.microsoft.com/visualstudio/install/modify-visual-studio) для получения сведений о том, как устанавливать дополнительные компоненты.

![Новый проект](images/vs2019-marble-maze-sample-fundamentals-1.png)

3. Нажмите кнопку **Далее**, введите **имя проекта**, **Расположение** сохраняемых файлов и **имя решения**, а затем нажмите кнопку **создать**.



Одним из важных параметров проекта в шаблоне **приложения DirectX 11 (Universal C++Windows-/CX)** является параметр **/ZW** , который позволяет программе использовать расширения языка среда выполнения Windows. При использовании шаблона Visual Studio этот параметр включен по умолчанию. В разделе [Настройка параметров компилятора](https://docs.microsoft.com/cpp/build/reference/setting-compiler-options) представлены дополнительные сведения о том, как настроить параметры компилятора в Visual Studio.

> **Внимание!**    параметр **/ZW** несовместим с параметрами, такими как **/CLR**. В случае **/clr** это означает, что вы не сможете настроить работу и .NET Framework, и среды выполнения Windows с одним и тем же проектом Visual C++.

 

Каждое приложение UWP, полученное от Microsoft Store, входит в форму пакета приложения. Пакет приложения включает в себя манифест пакета, содержащий информацию о вашем приложении. Например, вы можете указать возможности (то есть требуемый доступ к защищенным системным ресурсам или данным пользователя) вашего приложения. Если вы знаете, что вашему приложению необходимы определенные возможности, используйте манифест пакета для объявления требуемых возможностей. Манифест позволяет также задать свойства проекта, такие как поддержка поворота устройств, изображения плиток и экран приветствия. Изменить манифест можно, открыв файл **Package.appxmanifest** в проекте. Подробнее о пакетах приложений см. в разделе [Создание пакетов приложений](https://docs.microsoft.com/windows/uwp/packaging/index).
...
В Marble Maze используется символ **^** совместно с **Microsoft::WRL::ComPtr**, чтобы управлять динамически выделенными объектами и свести к минимуму потери памяти. Рекомендуется использовать ^ для управления жизненным циклом среда выполнения Windows переменных, **ComPtr** для управления временем существования переменных COM (например, при использовании DirectX) и **std:: Shared\_PTR** или **std:: Unique\_PTR** для управления временем существования всех остальных объектов, выделенных C++ в куче.
...
-   При вызове процедуры DirectX, которая возвращает **HRESULT**, используйте функцию **DX::ThrowIfFailed**. Эта функция определена в [DirectXHelper.h](https://github.com/Microsoft/Windows-appsample-marble-maze/blob/master/C%2B%2B/Shared/DirectXHelper.h). **ThrowIfFailed** выдает исключение, если переданный **HRESULT** является кодом ошибки. Например, **E\_указатель** приводит к тому, что **Сровиффаилед** создает исключение [Platform:: NullReferenceException](https://docs.microsoft.com/cpp/cppcx/platform-nullreferenceexception-class).
