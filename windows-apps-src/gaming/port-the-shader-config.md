...
> **Примечание.** В следующем примере выполняется загрузка шейдера и асинхронная компиляция с помощью ключевого слова **auto** и лямбда-синтаксиса. Метод ReadDataAsync() реализован для шаблона, который читает CSO-файл как массив байтовых данных (fileData).
