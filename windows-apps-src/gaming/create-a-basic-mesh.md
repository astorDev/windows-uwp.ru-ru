...
> **Важные**    в примере кода включены здесь использует типы (такие как DirectX::XMFLOAT3 и DirectX::XMFLOAT4X4) и встроенные методы, объявленные в DirectXMath.h. Если вы являетесь вырезания и вставки этого кода \#включают &lt;DirectXMath.h&gt; в проекте.
...
### <a name="step-1-construct-the-mesh-for-the-model"></a>Шаг 1. Конструкция сети для модели
...
-   **ПОЗИЦИЯ**: Это семантика позиции данных шейдеру HLSL. В данном коде это DirectX::XMFLOAT3 или, точнее, структура с тремя 32-разрядными величинами с плавающей запятой, которые соответствуют трехмерной координате (X, Y, Z). Можно также использовать float4, если вы указали однородных координат «w», и в этом случае указать DXGI\_ФОРМАТ\_R32G32B32A32\_число с плавающей запятой. Будете ли вы использовать DirectX::XMFLOAT3 или float4, зависит от специфических потребностей вашей игры. Обязательно убедитесь, что данные вершин для вашей сетки соответствуют используемому формату.
...
-   **ЦВЕТ**: Это HLSL семантической для данных о цвете. Аналогично **POSITION**, она содержит три 32-разрядных значения с плавающей запятой (DirectX::XMFLOAT3). Каждое значение содержит компонент, отвечающий за цвет: красный (r), синий (b) или зеленый (g), выраженный числом с плавающей запятой в диапазоне от 0 до 1.
...
### <a name="step-4-populate-the-index-buffers"></a>Шаг 4. Заполните буферы индексов
...
В нашем примере у куба 8 вершин, которые образуют 6 квадратов для сторон. Вы делите квадраты на треугольники (в общей сложности на 12 треугольников), в которых используются наши 8 вершин. При 3 вершинах на треугольник у вас будет 36 записей в буфере индексов. В нашем примере этот шаблон индекса называется списком треугольник и указывает его Direct3D как **D3D11\_ПРИМИТИВНЫЙ\_ТОПОЛОГИИ\_TRIANGLELIST** при задании топология примитивов.
...
В топологии полоса или вентилятор заказать вершины способом, который устраняет многие избыточных сторон во время обхода (например, на стороне от индекса 0 для индекса 2 на рисунке.) Для крупных сетей это значительно сокращает количество вершинный построитель текстуры запускается и значительно повышает производительность. Однако остановимся на таком простом варианте, как список треугольников.
...
> **Примечание**    в примере кода, представленные здесь используется столбец матрицы на основе. Строка матрицы на основе можно использовать вместо этого с помощью **строки\_основных** ключевое слово в HLSL и обеспечении исходные данные матрицы также строкам. DirectXMath использует строки матрицы на основе и может использоваться непосредственно с матрицами HLSL, определенные с помощью **строки\_основных** ключевое слово.
...
> **Примечание**  обычно объявляется матрица проекции при настройке устройства определенным ресурсам, так как результаты умножения с ним должен соответствовать текущие параметры размер окна просмотра 2-D (которые часто соответствуют пикселя Высота и ширина дисплея). Если эти параметры меняются, необходимо соответственно устанавливать пропорции значений координат X и Y.
...
### <a name="step-6-process-the-mesh-with-the-vertex-shader"></a>Шаг 6. Процесс с числом вершинный построитель текстуры
...
### <a name="step-7-passing-the-mesh-through-the-pixel-shader"></a>Шаг 7. Передача сетки через шейдер пикселей

Обычно на данном этапе графического конвейера выполняются попиксельные операции на видимых проецируемых поверхностях ваших объектов. (Как и текстуры сотрудников). В рамках примера Однако вам достаточно передать ей на этом этапе.
