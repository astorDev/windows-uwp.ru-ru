...
-   [Стадия средства программной прорисовки](https://docs.microsoft.com/windows/desktop/direct3d11/d3d10-graphics-programming-guide-rasterizer-stage). Средство программной прорисовки обрезает примитивы, готовит их для построителя текстуры и определяет, как вызывать построители текстуры. Можно отключить растрирование, указав конвейеру, что в нем нет построителя текстуры (задайте для этапа построителя текстуры значение NULL с помощью [**ссылку ID3D11DeviceContext::P ссетшадер**](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-pssetshader)) и отключите тестирование глубины и трафарета (установите Депсенабле и стенЦиленабле в значение false в [**D3D11\_depth\_трафарет\_DESC**](https://docs.microsoft.com/windows/desktop/api/d3d11/ns-d3d11-d3d11_depth_stencil_desc)). После отключения не будут обновляться связанные с растеризацией счетчики конвейера.
...
В Direct3D ресурсы шейдера не создаются до компиляции и их загрузки. Ресурсы создаются, когда загружается HLSL. Таким образом, в Глкреатешадер нет прямой аналогичной функции, которая создает инициализированный ресурс шейдера определенного типа (например, GL\_ВЕРШИНный\_ШЕЙДЕР или\_ФРАГМЕНТа\_ШЕЙДЕР). Вместо этого шейдеры создаются после загрузки HLSL с конкретными функциями, такими как [**ID3D11Device1::CreateVertexShader**](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11device-createvertexshader) и [**ID3D11Device1::CreatePixelShader**](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11device-createpixelshader), которые принимают тип и компилированный HLSL в качестве параметров.
...
| OpenGL ES 2.0 | Direct3D 11                                                                                                                                  |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------------|
| glViewport    | [**Окно просмотра\_CD3D11**](https://docs.microsoft.com/previous-versions/windows/desktop/legacy/jj151722(v=vs.85)), [ **ссылку ID3D11DeviceContext:: рссетвиевпортс**](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11devicecontext-rssetviewports) |
...
| GLSL ES для OpenGL ES 2.0 | 1.40. Другие (более старые) реализации GLSL ES для OpenGL ES 2.0 могут использовать версии от 1.10 до 1.30. Проверьте исходный код с помощью Глжетстринг (\_ШТРИХОВКИ\_язык\_версия) или Глжетстринг (ЗАТЕНЕНие\_язык\_версии), чтобы определить его. | ~SM 2.0               |
...
Семантика с префиксом "SV\_" — это семантика системных значений, записываемая программой шейдера; само приложение (работающее на ЦП) не может изменить их. Обычно они содержат значения, которые являются входными или выходными для другой стадии шейдера в конвейере графики или полностью генерируются ЦП.

Кроме того, при использовании для указания входных данных или выходных данных на этапе шейдера семантика в ОКП\_ имеет разную поведений. Например, ОКП\_ПОЛОЖЕНИЕМ (Output) содержит данные вершин, преобразованные на этапе шейдера вершин, а ОКП\_ПОЛОЖЕНИЕМ (input) содержит значения позиций в пикселах, которые интерполируются во время растрирования.
...
|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Расположение\_GL        | POSITION(n) для данных буфера вершин. Значение ОКП\_определяет расположение пиксельного шейдера в пикселях и не может быть записано приложением.                                        |
| Обычная\_ГК          | NORMAL(n) для данных нормали, предоставленных буфером вершин.                                                                                                                 |
| GL\_Текскурд\[n\]   | TEXCOORD(n) для данных UV-координат (ST в некоторой документации OpenGL) текстуры, которые предоставляются шейдером.                                                                       |
| Фрагколор GL\_       | COLOR(n) для данных цветов RGBA, которые предоставляются шейдеру. Отметим, что они обрабатываются аналогично данным координат. Семантика просто помогает понять, что они являются данными цветов. |
| GL\_Фрагдата\[n\]   | ОКП\_целевой\[n\] для записи из шейдера пикселей в целевую текстуру или другой буфер пикселей.                                                                               |
...
В этом случае объект ОКП\_TARGET — это расположение целевого объекта отрисовки, в который записывается цвет пикселя (определенный вектором с четырьмя значениями типа float), когда шейдер завершает выполнение.
