...
-   Файлы ресурсов хранятся в папках с произвольными именами, но имена файлов задаются с общей меткой, которая добавляется к строкам, зарезервированным системой, для обозначения языка или других квалификаторов. В частности, строки квалификаторов прикреплены к обобщенному имени файла после подчеркивания ("\_"). Например, \\Assets\\Menu\_параметр1\_Ланг-Ен-ус. PNG, \\Assets\\Menu\_параметр1\_Scale-140. PNG, \\Assets\\кулсигн\_dxfl-DX11. DDS. Вы также можете объединять эти строки. Например, \\активы\\меню\_параметр1\_Scale-140\_Ланг-Ен-ус. png.
    > **Примечание** .   при использовании в имени файла, а не на отдельном каталоге, квалификатор языка должен иметь форму "lang-<tag>", например "lang-en-US", как описано в разделе [адаптация ресурсов для языка, масштаба и других квалификаторов](../app-resources/tailor-resources-lang-scale-contrast.md).

     

Имена каталогов можно комбинировать, чтобы упаковка ресурсов стала более избирательной. Однако они не должны быть слишком длинными. Например, \\меню en-US\\\_параметр1\_Ланг-Ен-ус. png является избыточным.

Вы можете указывать любые незарезервированные имена подкаталогов, которые должны быть включены в каталог ресурсов, если структуры всех каталогов ресурсов совпадают. Например, \\dxfl-содержимым DX10\\активы\\текстуры\\кулсигн. DDS. Когда вы загружаете ресурс или ссылаетесь на него, имя пути должно быть универсальным, без каких-либо квалификаторов языка, масштаба или уровня компонентов DirectX, вне зависимости от того, входят они в узлы папок или же в имена файлов. Например, для ссылки в коде на ресурс, для которого один из вариантов — \\dxfl-содержимым DX10\\активы\\текстуры\\кулсигн. DDS, используйте \\активы\\текстуры\\кулсигн. DDS. Аналогично, чтобы сослаться на ресурс с вариантом \\образы\\Background\_Scale-140. PNG, используйте \\изображения\\Background. png.
...
|------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Локализованные ресурсы             | Все возможные языки, сочетания языков и языковых стандартов для Windows 10. (Префикс квалификатора «lang-» в имени папки не обязателен.) | "\_", за которым следует языковой, языковой стандарт или описатель языкового стандарта языка. Например, "\_EN", "\_US" или "\_en-US" соответственно. |
...
| Ресурсы уровня компонентов DirectX | dxfl-dx9, dxfl-dx10 и dxfl-dx11. Эти ресурсы относятся к уровням компонентов DirectX 9, 10 и 11 соответственно.                                     | "\_", за которым следует "dxfl-DX9", "dxfl-содержимым DX10" или "dxfl-DX11".                                                                     |
...
-   Убедитесь, что каждый ресурс или файл строкового ресурса (RESW-файл) одинаково называется в каждом каталоге. Например, меню\_параметр1. png должно иметь то же имя в каталогах \\en-US и \\JP-JP, даже если содержимое файла предназначено для другого языка. В этом случае вы увидите, что они отображаются в меню \\en-US\\\_параметр1. png и \\JP-JP\\\_параметр1. png.
    > **Обратите внимание** ,   при необходимости можно добавить языковой стандарт к имени файла и сохранить их в одном каталоге. Например, \\Assets\\Menu\_параметр1\_Ланг-Ен-ус. PNG, \\Assets\\Menu\_параметр1\_Ланг-ЖП-ЖП. png.
...
-   Убедитесь, что каждый ресурс одинаково называется в каждом каталоге. Например, меню\_параметр1. png должно иметь одно и то же имя в каталогах \\Scale-100 и \\Scale-180, даже если содержимое файла отличается. В этом случае они появятся в меню \\Scale-100\\\_параметр1. png и \\Scale-140\\\_параметр1. png.
    > **Обратите внимание** ,   опять же, при необходимости можно добавить суффикс коэффициента масштабирования к имени файла и сохранить его в одном каталоге. Например, \\Assets\\Menu\_параметр1\_Scale-100. PNG, \\Assets\\Menu\_параметр1\_Scale-140. png.
...
Механизм ресурсов в основном имеет дело с форматами текстур, поддерживаемыми ресурсами. Поэтому он поддерживает только 3 общих функциональных уровня. Если вам нужны отдельные шейдеры для подуровней (версии с точками), такие как DX9\_1 vs DX9\_3, управление активами и код визуализации должны напрямую их обрабатывались.
...
-   Используйте API в [**Windows.ApplicationModel.Resources.Core**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Resources.Core) для загрузки ресурсов. Ссылки на ресурсы должны быть обобщенными (без суффиксов), без уровня компонентов. Однако в отличие от языка и масштаба, система не определяет автоматически оптимальный уровень компонентов для определенного экрана. Это должны определить вы сами на основе логики кода. После этого используйте API, чтобы информировать ОС о предпочитаемом уровне компонентов. Тогда система сможет извлечь подходящий ресурс на основе этого предпочтения. Вот пример кода, который показывает, как сообщить приложению о текущем уровне компонентов DirectX для платформы:
    ...
    > **Обратите внимание** ,  в коде загружайте текстуру непосредственно по имени (или по пути под каталогом уровня компонентов). Не включайте имя каталога или суффикс уровня компонентов. Например, загрузите "текстуры\\кулсигн. DDS", а не "dxfl-DX11\\текстуры\\кулсигн. DDS" или "текстуры\\кулсигн\_dxfl-DX11. DDS".
