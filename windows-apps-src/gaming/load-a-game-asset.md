...
<tr class="even">
<td align="left"><p><a href="complete-code-for-basicreaderwriter.md">Полный код для BasicReaderWriter</a></p></td>
...
<tr class="odd">
<td align="left"><p><a href="complete-code-for-ddstextureloader.md">Полный код для DDSTextureLoader</a></p></td>
...
Дополнительные сведения см. в разделе [Асинхронное программирование на языке C++](https://docs.microsoft.com/windows/uwp/threading-async/asynchronous-programming-in-cpp-universal-windows-platform-apps).
...
Если ваша игра не слишком сложная, загружайте ресурсы с помощью подобного метода при запуске игры. Это можно сделать до запуска основного цикла игры в какой-либо момент последовательности вызовов вашей реализации [**IFrameworkView::Run**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.core.iframeworkview.run). Точно так же, вызов методов загрузки ресурсов может выполняться асинхронно, чтобы игра быстрее запускалась и пользователю игры не приходилось ждать окончания загрузки до выполнения каких-либо действий на ранних этапах.
...
Ниже приводится пример, в котором асинхронные методы, определенные в файле BasicLoader.cpp, используются для загрузки шейдеров, mesh-объектов и текстур при запуске игры. Обратите внимание, что установит определенного поля в объекте игр **m\_loadingComplete**, после всех методов загрузки.
...
Давайте получим байтовые данные из файла mesh-объекта. Предполагается, что формат в нашем примере является пользовательским форматом с расширением .vbo. (Опять же, этот формат не является таким же, как OpenGL VBO формат.) Сопоставляется с каждой вершины, сам **BasicVertex** тип, который является структурой, определенный в коде для obj2vbo преобразователь. Ниже показана примерная структура данных вершин в файле .vbo:

-   Первые 32 бита (4 байта) потока данных содержат число вершин в mesh-объекте (numVertices) в виде значения uint32.
-   Следующие 32 бита (4 байта) потока данных содержат число индексов в mesh-объекте (numIndices) в виде значения uint32.
-   После этого последующие (numVertices \* sizeof (**BasicVertex**)) биты содержат данные вершин.
-   Последний (numIndices \* 16) битов данных содержат данные индекса, представленное в виде последовательности значений uint16.

Резюме: необходимо знать структуру данных загружаемого mesh-объекта на битовом уровне. Кроме того, необходимо согласовать адресацию объекта. Все платформы Windows 8 — с прямым порядком байтов.
...
**CreateMesh** интерпретирует байтовые данные, загруженные из файла и создает буфер вершин и буфер индексов для сетки, передав списки вершин и индексов, соответственно, чтобы [ **ID3D11Device::CreateBuffer** ](https://docs.microsoft.com/windows/desktop/api/d3d11/nf-d3d11-id3d11device-createbuffer) и указания либо D3D11\_ПРИВЯЗАТЬ\_ВЕРШИН\_БУФЕР или D3D11\_ПРИВЯЗАТЬ\_индекс\_БУФЕРА. Код, который использовался в **BasicLoader**:
...
-   Описание данных, содержащихся в файле.
    ...
    Данные описываются с помощью заголовка описание [ **DDS\_заголовок**](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-header); формат пикселей определяется с помощью [ **DDS\_PIXELFORMAT** ](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-pixelformat). Обратите внимание, что **DDS\_заголовок** и **DDS\_PIXELFORMAT** структур замените устаревшие DDSURFACEDESC2 "," DDSCAPS2 "и" DDPIXELFORMAT DirectDraw 7 структуры. **DDS\_заголовок** двоичных эквивалентен DDSURFACEDESC2 и DDSCAPS2. **DDS\_PIXELFORMAT** эквивалентен двоичный DDPIXELFORMAT.
    ...
    Если значение **dwFlags** в [ **DDS\_PIXELFORMAT** ](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-pixelformat) присваивается DDPF\_FOURCC и **dwFourCC** имеет значение «Содержимым DX10» дополнительных [ **DDS\_заголовок\_DXT10** ](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-header-dxt10) структура будет присутствовать в соответствии с массивы текстур или DXGI форматы, которые не могут быть выражены в качестве значения RGB формат пикселей например форматы с плавающей запятой с точки, sRGB форматов и т.д. Когда **DDS\_заголовок\_DXT10** Структура присутствует, то описание данных выглядит следующим образом.
...
-   Указатель на массив байтов, содержащий сведения об остальных плоскостях, например об уровнях MIP, гранях в кубической текстуре, глубин в объемной текстуре. В статьях, ссылки на которые приведены ниже, представлены дополнительные сведения о структуре DDS-файлов для [текстуры](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-file-layout-for-textures), [кубической текстуры](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-file-layout-for-cubic-environment-maps) или [объемной текстуры](https://docs.microsoft.com/windows/desktop/direct3ddds/dds-file-layout-for-volume-textures).

    ```cpp
    BYTE bdata2[]
    ```

Экспорт в формат DDS можно выполнить при помощи многих программных средств. Если у вас нет средства для экспорта вашей текстуры в этот формат, подумайте о его создании. Дополнительные сведения о формате DDS и о работе с ним в коде вашей программы см. в статье [Руководство по программированию для DDS](https://docs.microsoft.com/windows/desktop/direct3ddds/dx-graphics-dds-pguide). В нашем примере мы будем использовать DDS.
...
По завершении работы кода в памяти остается текстура [**Texture2D**](https://docs.microsoft.com/windows/desktop/api/d3d11/nn-d3d11-id3d11texture2d), загруженная из файла изображения. Скорее всего, в вашей игре во всех сценах используется множество текстур (как и mesh-объектов). Если игре необходим регулярный доступ к определенным текстурам, рекомендуется создать кэши для конкретных сцен или уровней вместо того, чтобы загружать все текстуры в момент запуска игры или любого уровня.
...
Эти элементы входного макета для вершины называются [семантическим элементами HLSL](https://docs.microsoft.com/windows/desktop/direct3dhlsl/dx-graphics-hlsl-semantics). Они представляют собой набор определенных регистров, которые будут использоваться для передачи данных от скомпилированного объекта шейдера и к этому объекту. Ваш графический конвейер выполняет шейдер вершин один раз для каждой вершины в загружаемом mesh-объекте. Семантические элементы определяют входные (и выходные) данные шейдера вершин по мере его выполнения и предоставляют эти данные для расчета каждой вершины в HLSL-коде построителя текстуры.
...
* [Пример загрузки ресурсов Direct3D]( https://go.microsoft.com/fwlink/p/?LinkID=265132)
* [Полный код для BasicLoader](complete-code-for-basicloader.md)
* [Полный код для BasicReaderWriter](complete-code-for-basicreaderwriter.md)
* [Полный код для DDSTextureLoader](complete-code-for-ddstextureloader.md)
