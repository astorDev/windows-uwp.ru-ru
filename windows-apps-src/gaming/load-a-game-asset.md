...
**CreateMesh** интерпретирует байтовые данные, загруженные из файла и создает буфер вершин и буфер индексов для сетки, передав списки вершин и индексов, соответственно, чтобы [ **ID3D11Device::CreateBuffer** ](https://msdn.microsoft.com/library/windows/desktop/ff476501) и указания либо D3D11\_ПРИВЯЗАТЬ\_ВЕРШИН\_БУФЕР или D3D11\_ПРИВЯЗАТЬ\_индекс\_БУФЕРА. Код, который использовался в **BasicLoader**:
...
-   Описание данных, содержащихся в файле.
    ...
    Данные описываются с помощью заголовка описание [ **DDS\_заголовок**](https://msdn.microsoft.com/library/windows/desktop/bb943982); формат пикселей определяется с помощью [ **DDS\_PIXELFORMAT** ](https://msdn.microsoft.com/library/windows/desktop/bb943984). Обратите внимание, что **DDS\_заголовок** и **DDS\_PIXELFORMAT** структур замените устаревшие DDSURFACEDESC2 "," DDSCAPS2 "и" DDPIXELFORMAT DirectDraw 7 структуры. **DDS\_заголовок** двоичных эквивалентен DDSURFACEDESC2 и DDSCAPS2. **DDS\_PIXELFORMAT** эквивалентен двоичный DDPIXELFORMAT.
    ...
    Если значение **dwFlags** в [ **DDS\_PIXELFORMAT** ](https://msdn.microsoft.com/library/windows/desktop/bb943984) присваивается DDPF\_FOURCC и **dwFourCC** имеет значение «Содержимым DX10» дополнительных [ **DDS\_заголовок\_DXT10** ](https://msdn.microsoft.com/library/windows/desktop/bb943983) структура будет присутствовать в соответствии с массивы текстур или DXGI форматы, которые не могут быть выражены в качестве значения RGB формат пикселей например форматы с плавающей запятой с точки, sRGB форматов и т.д. Когда **DDS\_заголовок\_DXT10** Структура присутствует, то описание данных выглядит следующим образом.
