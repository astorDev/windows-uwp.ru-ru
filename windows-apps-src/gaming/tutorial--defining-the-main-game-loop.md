...
Узнайте, как применять основные методы разработки базовые реализации игры правила и Механика игры UWP DirectX.

## <a name="main-game-object"></a>Основной игровой объект

В этом примере игры __Simple3DGame__ является класс главного объекта игры. Метод __App::Load__ создается экземпляр объекта __Simple3DGame__ .
...
* Содержит методы, которые обмениваются данными:
    ...
    * Сведения для обновления пользовательского интерфейса (наложения и отображения экранных элементов) игры, анимации и физики (динамики).
    ...
    >[!Note]
    >Обновление графики обрабатывается классом __GameRenderer__ , который содержит методы для получения и использования ресурсы графического устройства, используемых в игре. Дополнительные сведения см. в разделе [платформа отрисовки I: введение в отрисовку](tutorial--assembling-the-rendering-pipeline.md).

* Служит контейнером для данных, который определяет игровой сеанс, уровень или время существования в зависимости от того, как определить игру на высоком уровне. В этом случае данные состояния игры обозначают время существования игры и инициализируется один раз, когда пользователь запускает игры.
...
* Создается и активируется объект проигрывателя. Мы используем объект-сферу для определения приближения игрока к стенам и препятствиям и запретить получение помещается в положение, которое может разрушить погружения камеры.
...
Большая часть сложной работы выполняется уровня при выполнении построения выполняется в файлах __Level.h/.cpp__ , находящийся в папке __GameLevels__ примера решения. Так как этот раздел посвящен особые реализации, мы не раскрываются их здесь. Важно лишь знать, что код каждого уровня выполняется как отдельный объект __LevelN__. Если вы хотите расширить игру, можно создать объект **уровня** , который принимает назначенным номером в качестве параметра и случайным образом помещает препятствиями и целями. Или требуется, чтобы загружать данные конфигурации уровня из файла ресурсов или даже из Интернета.

## <a name="define-the-game-play"></a>Определение игрового процесса

Теперь у нас есть все компоненты для создания игры. Уровни берется память из примитивы и все готово для проигрывателя, чтобы начать взаимодействие с.

Лучшие игры языке незамедлительно реагировать на данные, вводимые пользователем, выдавая мгновенную обратную связь. Это справедливо для любого типа, игры, из twitch действие, в режиме реального времени шутеры от первого лица захватывающих глубоко, соблюдать очередь.
...
*  Метод обновляет таймер, который отсчитывает секунды, пока игрок не завершит прохождение уровня, и проверяет, не истекло ли время прохождения. Это одно из правил игры: Если время истекло и всех конечных объектов не были снимок, ее.
...
В цикле __UpdateDynamics__ вызывать методы, которые используются для определения игрового мира в движение, независимо от игрока ввода, создавать современные методы взаимодействия, игры и сделать уровень *ожила*. Сюда входят графики, которая должна быть и запущена анимация циклы преобразовывать о живого, изменчивый мир, даже в том случае, когда нет игрока. Например swaying на ветру, волны cresting вдоль Берег, волны курения и инопланетные монстры растяжение и передвигать деревья. Динамика также включает в себя взаимодействие между объектами, включая столкновения сферы игрока с окружающим миром или летящих выстрелов с препятствиями и целями.

Цикл игры всегда должно поддерживать обновление игрового мира, является ли он основан на игровой логики, физических алгоритмах или будь то просто случайные, когда игра специально приостановлена. 
...
### <a name="review-the-flow"></a>Ознакомьтесь с процессом

Теперь, когда мы обновить все объекты в сцене и рассчитывается все столкновения, необходимо использовать эти сведения для соответствующего визуально. 

После завершения __GameMain::Update()__ текущей итерации игрового цикла игра немедленно вызывает __Render()__ для новых данных объектов и создания новой сцены для отображения для игрока, как показано ниже. Теперь давайте рассмотрим отрисовку.
...
>[!Note]
>Существует два состояния пример наложенное изображение Direct2D: один которой игра отображает игровую информацию наложения, содержащий растровое изображение меню приостановки, а где игра отображает обрезные прямоугольники для сенсорного экрана перемещением и обзором контроллер. Набранные очки отображаются в обоих состояниях. Подробнее: [Платформа отрисовки I: введение в отрисовку](tutorial--assembling-the-rendering-pipeline.md).
...
Доступны следующие методы и данные, определенные в классе объект __Simple3DGame__ .
...
-   **FinalizeLoadLevel**: завершает любую работу загрузку уровня, которая должна выполняться в основном потоке. Включает любые вызовы контекстных ([**ID3D11DeviceContext**](https://msdn.microsoft.com/library/windows/desktop/ff476385)) методов устройства в Direct3D 11.
...
-   **UpdateDynamics**: это важный метод, поскольку все игровые объекты, в зависимости от подпрограммами анимации, физики и дистанционного управления. Он является основой взаимодействия всех элементов игры. Это рассматривается в разделе [обновление игрового мира](#update-the-game-world) .
...
-   **GameRenderer** объект: представляет обработчик Direct3D 11, производные от класса **DirectXBase** , который управляет всеми объектами конкретного устройства и их прорисовкой. Дополнительные сведения см. в разделе [платформа отрисовки I](tutorial--assembling-the-rendering-pipeline.md).
