...
ms.locfileid: "8942445"
---
# <a name="define-the-main-game-object"></a>Определение основного игрового объекта

После того, как размещены базовая структура примера игры и реализован конечный автомат, обрабатывающий высокоуровневые пользователя и поведение системы, необходимо проверить правила и механизмы, включить образец игры в игру. Рассмотрим сведения о основного объекта образца игры, а также механизм взаимодействия с игрового мира игры правила.
...
Узнайте, как применить методы разработки для реализации правил игры и Механика игры UWP DirectX.
...
* Содержит методы, которые обмениваются данными:
    * Изменения состояния игры автомату, определенному в структуре приложения.
    ...
    * Сведения для обновления пользовательского интерфейса (наложения и отображения экранных элементов) игры, анимации и физики (динамики).
    ...
    >[!Note]
    >Обновление графики обрабатывается классом __GameRenderer__ , который содержит методы для получения и использовать ресурсы графического устройства, используемых в игре. Дополнительные сведения см. в разделе [платформа отрисовки I: введение в отрисовку](tutorial--assembling-the-rendering-pipeline.md).
...
* Создается и активируется объект проигрывателя. Мы используем объект-сферу для определения приближения игрока к стенам и препятствиям и запретить получение помещаются в положение, которое может разрушить погружения камеры.
...
Большая часть сложной работы выполняется уровня при выполнении построения выполняется в файлах __Level.h/.cpp__ , находящийся в папке __GameLevels__ примера решения. Так как этот раздел посвящен особые реализации, мы не раскрываются их здесь. Важно лишь знать, что код каждого уровня выполняется как отдельный объект __LevelN__. Если вы хотите расширить игру, можно создать объект **уровня** , который принимает назначенным номером в качестве параметра и случайным образом помещает препятствиями и целями. Или, если требуется, чтобы загружать данные конфигурации уровня из файла ресурсов или даже из Интернета.

## <a name="define-the-game-play"></a>Определение игрового процесса

Теперь у нас есть все компоненты для создания игры. Уровни создан в память из примитивы и готовы для проигрывателя, чтобы начать взаимодействие с.

Лучшие игры языке незамедлительно реагировать на данные, вводимые пользователем, выдавая мгновенную обратную связь. Это справедливо для любого типа игры, twitch-действий, в режиме реального времени шутеры от первого лица захватывающих сложный, соблюдать очередь.
...
*  Метод обновляет таймер, который отсчитывает секунды, пока игрок не завершит прохождение уровня, и проверяет, не истекло ли время прохождения. Это одно из правил игры: Если время истекло, все целевые объекты не были снимок ее.
...
-   **LoadLevelAsync**: запускает асинхронную задачу (Если вы не знакомы с асинхронные задачи, см. в разделе [Библиотеки параллельных шаблонов](https://docs.microsoft.com/cpp/parallel/concrt/parallel-patterns-library-ppl)) для инициализации уровня, а затем вызывает асинхронную задачу на обработчике для загрузки ресурсов устройства, характерных уровня. Данный метод выполняется в отдельном потоке; в результате из данного потока могут вызываться только методы [**ID3D11Device**](https://msdn.microsoft.com/library/windows/desktop/ff476379) (в противоположность методам [**ID3D11DeviceContext**](https://msdn.microsoft.com/library/windows/desktop/ff476385)). Все контекстные методы устройства вызываются в методе **FinalizeLoadLevel**.
...
-   **UpdateDynamics**: это метод играет важную, поскольку все игровые объекты, в зависимости от подпрограммами анимации, физики и дистанционного управления. Он является основой взаимодействия всех элементов игры. Такие случаи описаны в разделе " [обновление игрового мира](#update-the-game-world) ".
...
-   **GameRenderer** объект: представляет обработчик Direct3D 11, производные от класса **DirectXBase** , все объекты конкретного устройства и их прорисовкой. Дополнительные сведения см. в разделе [платформа отрисовки I](tutorial--assembling-the-rendering-pipeline.md).
