...
Метаданные, которые вы пишете с помощью **AppCaptureMetadataWriter**, кэшируются системой до записи в соответствующий мультимедийный поток. Система определяет ограничения размера для кэша метаданных каждого приложения. После достижения предельного размера кэша система начнет очистку кэшированных метаданных. Система удалит метаданных, который был записан с **[AppCaptureMetadataPriority.Informational](https://docs.microsoft.com/uwp/api/windows.media.capture.appcapturemetadatapriority)** значение приоритета перед удалением метаданных с **[ AppCaptureMetadataPriority.Important](https://docs.microsoft.com/uwp/api/windows.media.capture.appcapturemetadatapriority)** приоритет.
...
Можно зарегистрировать для получения уведомления, когда системе начинает очистку метаданных для приложения, зарегистрировав обработчик для **[MetadataPurged](https://docs.microsoft.com/uwp/api/windows.media.capture.appcapturemetadatawriter.MetadataPurged)** событий.
