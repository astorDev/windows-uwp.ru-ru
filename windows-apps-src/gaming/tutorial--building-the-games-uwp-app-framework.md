...
* [__Устанавливает__](#initialize-the-view-provider)
* [__сетвиндов__](#configure-the-window-and-display-behaviors)
* [__Загрузить__](#load-method-of-the-view-provider)
* [__Запуска__](#run-method-of-the-view-provider)
...
* Создайте контроллер, чтобы он мог считывать данные с контроллера (сенсорный, мышь или адаптер Xbox). Подробнее: [Добавление элементов управления](tutorial--adding-controls.md).
...
Образец кода переходит в одно из двух состояний автомата состояний модуля игры.
    * __Deactivated__: окно игры деактивировано (потеряло фокус) или прикреплено. Когда этот происходит, игра приостанавливает обработку событий и ожидает, когда окно получит фокус или будет откреплено.
...
В Windows 10 закрытие окна приложения не приводит к завершению процесса приложения, а записывает состояние одноэлементного приложения в память. Если необходимо, чтобы при освобождении системой этой области памяти выполнялась какая-либо особая операция, включая особое удаление ресурсов, вставьте в этот метод код для такого удаления.
