...
В Windows10 закрытие окна приложения не процесса приложения, прекращению вместо этого состояние Singleton-объект приложения в память. Если необходимо, чтобы при освобождении системой этой области памяти выполнялась какая-либо особая операция, включая особое удаление ресурсов, вставьте в этот метод код для такого удаления.
