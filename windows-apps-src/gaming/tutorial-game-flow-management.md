...
2. __Ожидание press__: Игровой цикл повторяется в ожидании конкретного ввода от пользователя. Ввод представляет действие пользователя для загрузки игры, начала прохождения уровня или продолжения уровня. Образец игры ссылается на эти промежуточные состояния как на перечисляемые значения __PressResultState__.
3. В __Dynamics__: Игровой цикл выполняется с участием пользователя. При нахождении пользователя в игровом процессе игра проверяет 3 состояния, в которые она может перейти: 
...
* [Определить основной объект игр](tutorial--defining-the-main-game-loop.md)
* [Платформу для отображения инструкции: Общие сведения о подготовке к просмотру](tutorial--assembling-the-rendering-pipeline.md)
* [Платформу для отображения II: Отрисовки игр](tutorial-game-rendering.md)
