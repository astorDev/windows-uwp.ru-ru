...
description: В этом учебном руководстве описывается добавление в игру DirectX традиционных элементов управления перемещением и обзором с помощью мыши и клавиатуры.
...
ms.locfileid: "66367857"
---
# <a name="span-iddevgamingtutorialaddingmove-lookcontrolstoyourdirectxgamespanmove-look-controls-for-games"></a><span id="dev_gaming.tutorial__adding_move-look_controls_to_your_directx_game"></span>Перемещение внешний вид элементов управления для игр



В этом учебном руководстве описывается добавление в игру DirectX традиционных элементов управления перемещением и обзором с помощью мыши и клавиатуры.
...
-   **m\_позиции** — это положение камеры (и, следовательно viewplane) в трехмерной сцене, с помощью координат сцены.
-   **m\_pitch** является pitch камеры или вверх вниз вращения вокруг оси x viewplane, в радианах.
-   **m\_Поворот** является нутации камеры или слева направо вращения вокруг оси y viewplane, в радианах.
...
-   **m\_moveInUse** показывает, используется ли контроллер перемещения.
-   **m\_movePointerID** уникальный идентификатор для текущего перемещения указателя. Этот идентификатор используется, чтобы различать указатель обзора и указатель перемещения при проверке значения идентификатора указателя.
-   **m\_moveFirstDown** представляет собой точку на экране, где игрок первом касании области указатель перемещения контроллера. Позднее с помощью этого значения зададим "мертвую" зону для предотвращения дрожания плоскости отображения из-за небольших перемещений.
-   **m\_movePointerPosition** представляет собой точку на экране, игрок в настоящее время был перемещен указатель. Мы используем чтобы определить направление проигрыватель нужно было перенести путем проверки его относительно **m\_moveFirstDown**.
-   **m\_moveCommand** — это последняя команда вычисляемых для перемещения контроллера: (вперед), счетчик (назад), влево или вправо.

Теперь определим поля, предназначенные для устройства управления обзором, — с возможностью управления мышью и касанием.

-   **m\_lookInUse** показывает, используется ли внешний вид элемента управления.
-   **m\_lookPointerID** уникальный идентификатор для текущего вида указателя. Этот идентификатор используется, чтобы различать указатель обзора и указатель перемещения при проверке значения идентификатора указателя.
-   **m\_lookLastPoint** является последней точкой, в координатах сцены, которые были собраны в предыдущем фрейме.
-   **m\_lookLastDelta** вычисляемый разница между текущим **m\_позиции** и **m\_lookLastPoint**.
...
-   **m\_вперед**, **m\_обратно**, **m\_левой**, **m\_правой**, **m\_вверх** и **m\_вниз**.
...
-   **Initialize**. Приложение вызывает этот обработчик событий для инициализации элементов управления и присоединения их к объекту [**CoreWindow**](https://docs.microsoft.com/uwp/api/Windows.UI.Core.CoreWindow), который описывает окно отображения.
...
-   **получить\_позиции**. С помощью этого свойства приложение получает доступ к текущему расположению камеры в пространстве сцены. Это свойство используется для связи текущего расположения камеры с приложением.
-   **получить\_LookPoint**. С помощью этого свойства приложение получает текущую точку, к которой обращено устройство управления камерой.
...
Сначала добавим обработчики событий мыши и указателя касания. В первом обработчике событий **OnPointerPressed()** для получения координат X-Y указателя используется тип [**CoreWindow**](https://docs.microsoft.com/uwp/api/Windows.UI.Core.CoreWindow), который управляет экраном, когда пользователь нажимает кнопку мыши или касается экрана в области устройства управления обзором.
...
Этот обработчик событий проверяет, не является ли указатель указателем мыши (для этого образца, который поддерживает мышь и касания) и не находится ли он в области устройства управления перемещением. Если оба условия имеют значение true, проверяет ли указатель был только что нажата, в частности, ли щелкните не связан с предыдущим переместить или найдите input, проверяя, **m\_moveInUse** имеет значение false. Если таким образом, обработчик захватывает точку в области перемещения контроллера, где произошло press и задает **m\_moveInUse** значение true, таким образом, когда этот обработчик вызывается снова, он не перезаписывает начальное положение перемещения взаимодействие входного контроллера. Обработчик также присваивает идентификатору указателя устройства управления перемещением значение идентификатора текущего указателя.

Если используется указатель мыши или если указатель касания не находится в области устройства управления перемещением, он должен находиться в области устройства управления обзором. Он задает **m\_lookLastPoint** к текущей позиции, когда пользователь нажимает кнопку мыши или затронутых и нажата, сбрасывает дельта и обновляет код контроллера внешний вид указателя для текущего идентификатора указатель. Он также задает активное состояние устройства управления обзором.
...
В завершение необходимо отключить поведение устройства управления перемещением или обзором, когда игрок перестает двигать мышь или касаться экрана. Мы используем **OnPointerReleased**, который мы называем при [ **PointerReleased** ](https://docs.microsoft.com/uwp/api/windows.ui.core.corewindow.pointerreleased) инициируется, чтобы задать **m\_moveInUse** или **m\_lookInUse** значение FALSE, и отключить перемещение сдвиг камеры и к нулю out указатель на идентификатор.
...
Теперь мы выполнять вычисления, которые преобразуют данные координат указателя, отслеживаются в **m\_movePointerPosition** в новые сведения о координатах соответствующих наш мир системы координат. Приложение вызывает этот метод каждый раз при обновлении основного цикла приложения. Здесь мы вычисляем новое расположение точки обзора, которое необходимо передать в приложение для обновления матрицы обзора перед проецированием в окно просмотра.
...
Последняя позиция точки внешний вид проигрывателя является последней позиции, а также вычисляемые скорость, и это, что считываются с помощью модуля подготовки отчетов, при вызове **получить\_позиции** (скорее всего, во время установки для каждого метода кадр). После этого команде перемещения присваивается значение 0.

## <a name="updating-the-view-matrix-with-the-new-camera-position"></a>Обновление матрицы просмотра с новым положением камеры


Мы можем получить координаты пространства сцены, на которых сфокусирована наша камера. Эти координаты обновляются с интервалом, заданным в приложении (например, каждые 60 секунд в основном цикле приложения). Следующий псевдокод предлагает реакции вызова, которые вы можете реализовать.
