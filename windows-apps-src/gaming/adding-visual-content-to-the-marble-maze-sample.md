...
3.  Убедитесь, что двух- и трехмерной правильной загрузки и отображения ресурсов в игре.
...
-   Сначала обычно рисуются объекты трехмерной сцены, и затем все 2D объекты, которые отображаются перед сценой.
...
> [!NOTE]
> Термин *представление* имеет разные значения в среде выполнения Windows, чем ранее в Direct3D. В среде выполнения Windows представление — это набор параметров пользовательского интерфейса для приложения, в том числе область отображения и возможные реакции на ввод, а также поток, используемый для обработки. При создании представления задаются нужные параметры и конфигурация. Процедура настройки представления приложения описана в разделе [Структура приложения Marble Maze](marble-maze-application-structure.md).
...
-   Текст, который считывает **приостановлено** , когда игра приостановлена.

Marble Maze определяет элементы пользовательского интерфейса игры в **файле UserInterface.h**. В Marble Maze класс **ElementBase** определяется в качестве базового типа для всех элементов пользовательского интерфейса. В классе **ElementBase** определены такие атрибуты элемента пользовательского интерфейса, как размер, положение, выравнивание и видимость. Он также управляет обновлением и отрисовкой элементов.
...
После рисования объектов трехмерной сцены, Marble Maze рисует двухмерные элементы пользовательского интерфейса отображаются перед сценой.
...
[IDXGISwapChain::Present](https://msdn.microsoft.com/library/windows/desktop/bb174576) метод возвращает код ошибки, который указывает, что устройство была удалена или в противном случае не удалось. В этом случае Marble Maze повторно инициализирует устройство.
