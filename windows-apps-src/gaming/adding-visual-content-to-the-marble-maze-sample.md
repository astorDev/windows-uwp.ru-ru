...
description: Документ иллюстрирует, как Marble Maze использует Direct3D и Direct2D в приложении UWP. Вы освоите приемы работы и сможете адаптировать их для своей игры.
...
В Marble Maze с помощью Direct3D 11.1 выполняется отрисовка трехмерных игровых объектов: шарика и лабиринта. В Marble Maze также используются Direct2D, DirectWrite и компонент обработки изображений Windows (WIC) для рисования двумерных игровых ресурсов: меню, таймера и т. п. Наконец, Marble Maze использует XAML, чтобы предоставить панель приложения, и позволяет вам добавлять элементы управления XAML.
...
### Привязка Direct3D к представлению
...
Чтобы обеспечить хороший отклик приложения, Marble Maze загружает ресурсы сцены асинхронно, в фоновом режиме. Пока ресурсы загружаются в фоновом режиме, игра может отвечать на события окон. Этот процесс подробно описан в разделе [Загрузка ресурсов игры в фоновом режиме](marble-maze-application-structure.md#loading_game_assets) данного руководства.

###  Загрузка двухмерного наложения и пользовательского интерфейса
...
### Подготовка вершинного шейдера и шейдеров пикселей
...
### Рисование пользовательского интерфейса и наложения
