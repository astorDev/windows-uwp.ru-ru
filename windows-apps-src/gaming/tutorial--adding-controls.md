...
После перехода в состояние **WaitForInput** игра прекращает обработку практических всех событий ввода игрового процесса, пока не вернется в состояние **Active**. Исключением является кнопка "Пауза", при нажатии которой игра возвращается в активное состояние. Кроме "Пауза" чтобы игра возвращается в **активное** состояние игрок может выбрать пункт меню. 
...
> [!NOTE]
> Приложения UWP не может получить входные данные с контроллером Xbox One, пока приложение не находится в фокусе.
