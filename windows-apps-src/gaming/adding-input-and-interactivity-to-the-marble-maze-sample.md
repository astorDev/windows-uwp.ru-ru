...
Класс **MarbleMazeMain** также создает объект **std::map** для хранения событий касания. Ключом для этого назначенного объекта является значение, уникально идентифицирующее указатель ввода. Каждый ключ сопоставляется с расстоянием между каждой точкой касания и центром экрана. Позже Marble Maze использует эти значения для расчета степени наклона лабиринта.
...
##  <a name="controlling-game-play"></a>Управление ходом игры
