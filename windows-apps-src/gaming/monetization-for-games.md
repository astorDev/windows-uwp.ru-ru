...
* Публикация игры в Магазине Windows— защищенном интернет-магазине для [всемирного распространения](#worldwide-distribution-channel) программных продуктов. Пользователи по всему миру смогут купить вашу игру в Интернете по [установленной вами цене](#set-a-price-for-your-game).
...
* Использование API-интерфейсов в [Microsoft Store Services SDK](https://visualstudiogallery.msdn.microsoft.com/229b7858-2c6a-4073-886e-cbb79e851211) для показа рекламы из рекламных сетей. Вы можете [показывать рекламу в своей игре](#display-ads-in-your-game) и предоставлять игрокам возможность просматривать видеорекламу в обмен на какие-либо бонусы в игре.
...
Опубликованные в Магазине Windows игры доступны для скачивания в более чем 200 странах и регионах по всему миру, с поддержкой различных способов оплаты, включая такие как Visa, MasterCard и PayPal. Полный список стран и регионов см. в разделе [Рынки и особые цены](https://msdn.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection#markets-and-custom-prices).

## Установка цены для игры 
...
Базовая цена игры определяет, как классифицируется ваша игра: как _платная_ или как _бесплатная_. Настроить базовую цену в зависимости от страны и регионы можно на [информационной панели Центра разработки](https://developer.microsoft.com/windows). При определении цены необходимо учитывать [налогообложение при продаже приложений в разные страны](https://msdn.microsoft.com/windows/uwp/publish/tax-details-for-paid-apps) и [ценовые соображения для конкретных рынков](https://msdn.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection#price-considerations-for-specific-markets). Также можно [установить особые цены для конкретных рынков](https://msdn.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection#markets-and-custom-prices). Подробнее см. в статье [Определение цены и выбор рынка](https://msdn.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection).
...
Покупки из игры— это продукты, приобретаемые внутри игры. Более общее название для них— _покупки из приложения_. В Магазине Windows такие продукты называются _надстройками_. [Надстройки публикуются](https://msdn.microsoft.com/windows/uwp/publish/add-on-submissions) через информационную панель Центра разработки для Windows. Вам также понадобится включить надстройки в коде игры.
...
При создании потребляемых надстроек нужно решить, как вы хотите их отслеживать &mdash; т.е. будут ли они _управляемыми разработчиком_ или _управляемыми Магазином_ (эта возможность доступна начиная с Windows 10 версии 1607). В случае потребляемой настройки, управляемой разработчиком, за отслеживание остатка надстройки у игрока отвечаете вы; в случае управляемой Магазином надстройки Магазин Windows будет делать это за вас. Подробнее см. в разделе [Обзор потребляемых надстроек](https://msdn.microsoft.com/windows/uwp/monetize/enable-consumable-add-on-purchases#overview-of-consumable-add-ons).

### Создание покупок из игры
...
### Тестирование покупок из игры в процессе разработки игры
...
3. В ответ на вопрос __Вы хотите выполнить сборку пакетов для отправки в Магазин Windows?__ выберите __Да__.
...
#### Публикация игры как скрытой игры
...
4. Настройте эту новую отправку. На странице отправки: 
    * Щелкните __Цены и доступность__. В разделе __Видимость__ выберите __Скрыть приложение и остановить продажу...__, чтобы доступ к игре был только у вашей команды разработчиков. Подробнее см. в статье [Распространение и видимость](https://msdn.microsoft.com/windows/uwp/publish/set-app-pricing-and-availability#distribution-and-visibility).
...
#### Связывание решения игры с Магазином
...
Если в игре много надстроек, их можно создать программным способом с помощью __API отправки Магазина Windows__. Подробнее см. в статье [Создание отправок и управление ими с помощью служб Магазина Windows](https://msdn.microsoft.com/windows/uwp/monetize/create-and-manage-submissions-using-windows-store-services).

## Показ рекламы в игре

Библиотеки и средства, входящие в состав Microsoft Store Services SDK, позволяют настроить в игре службу для получения рекламы из рекламной сети. Вашим игроки будет показываться динамическая реклама, и вы будете получать деньги от рекламодателей всякий раз, когда игрок просматривает рекламу или взаимодействует с ней. Подробнее см. в статье [Процессы создания приложений с рекламой](https://msdn.microsoft.com/windows/uwp/monetize/workflows-for-creating-apps-with-ads).

### Форматы рекламы

С помощью Microsoft Store Services SDK можно показывать рекламу двух типов:
...
### Какая реклама будет показываться?

При использовании Microsoft Store Services SDK реклама для показа в настоящее время поступает из наших партнерских сетей. Подробнее о предложениях, имеющихся на данный момент, см. статье [Получайте доход от своих приложений за счет рекламы](https://developer.microsoft.com/store/monetize/ads-in-apps).
Если вы используете для показа рекламы элемент управления AdControl, вы можете разрешить показ [рекламы от аффилированных лиц](https://msdn.microsoft.com/windows/uwp/publish/about-affiliate-ads), тем самым расширив набор рекламы в вашей игре.

### На каких рынках разрешено показывать рекламу?

Рекламные баннеры и промежуточную видеорекламу можно показывать пользователям не во всех странах. Полный список стран и регионов, которые поддерживают рекламу, см. в статье [Поддерживаемые рынки для Microsoft Advertising](https://msdn.microsoft.com/windows/uwp/monetize/supported-markets-for-microsoft-advertising).

### API-интерфейсы для показа рекламы

Для показа рекламы в играх используются классы [AdControl](https://msdn.microsoft.com/library/windows/apps/microsoft.advertising.winrt.ui.adcontrol.aspx) и [InterstitialAd](https://msdn.microsoft.com/library/windows/apps/microsoft.advertising.winrt.ui.interstitialad.aspx) в пакете Microsoft Store Services SDK, который входит в пространство имен [Microsoft.Advertising.WinRT.UI](https://msdn.microsoft.com/library/windows/apps/microsoft.advertising.winrt.ui.aspx).

Для начала работы с ними скачайте и установите [Microsoft Store Services SDK](http://aka.ms/store-em-sdk) с Visual Studio 2015. Подробнее см. в статье [Возможности, доступные в SDK](https://msdn.microsoft.com/windows/uwp/monetize/microsoft-store-services-sdk#features-available-in-the-sdk).

#### Пошаговые руководства

В этих руководствах показано, как реализовать рекламу с использованием классов __AdControl__ и __InterstitialAd__:

* [Создание рекламных баннеров с использованием класса AdControl в XAML и .NET](https://msdn.microsoft.com/windows/uwp/monetize/adcontrol-in-xaml-and--net)
* [Создание рекламных баннеров с использованием класса AdControl в HTML5 и JavaScript](https://msdn.microsoft.com/windows/uwp/monetize/adcontrol-in-html-5-and-javascript)
* [Создание промежуточной видеорекламы с использованием класса InterstitialAd](https://msdn.microsoft.com/windows/uwp/monetize/interstitial-ads)

В процессе разработки можно использовать следующие тестовые значения, чтобы увидеть, как отображается реклама. Эти же решения используются в пошаговых руководствах выше.

|AdType             | AdUnitId  | AppId                              |
|-------------------|-----------|------------------------------------|
|Рекламные баннеры         |10865270   |3f83fe91-d6be-434d-a0ae-7351c5a997f1|
|Промежуточная видеореклама   |11389925   |d25517cb-12d4-4699-8bdc-52040c712cab|

Ниже приведены некоторые рекомендации, которые помогут вам в процессе проектирования и реализации.

* [Рекомендации по реализации рекламных баннеров с использованием класса AdControl](https://msdn.microsoft.com/windows/uwp/monetize/ui-and-user-experience-guidelines)
* [Рекомендации по реализации промежуточной видеорекламы с использованием класса InterstitialAd](https://msdn.microsoft.com/windows/uwp/monetize/ui-and-user-experience-guidelines#interstitialbestpractices10)
...
### Подготовка к выпуску путем замены тестовых значений рекламных блоков

Когда вы будете готовы переходить к тестированию в реальных условиях или к показу рекламы в опубликованных играх, вы должны заменить тестовые значения рекламных блоков на реальные значения, предоставленные для вашей игры.
О том, как создать рекламные блоки для вашей игры, см. в статье [Настройка рекламных блоков в приложении](https://msdn.microsoft.com/windows/uwp/monetize/set-up-ad-units-in-your-app).

### Другие рекламные сети

Существуют и другие рекламные сети, поддерживающие показ рекламы в приложениях и играх UWP.
...
## Увеличение потенциала вашей игры посредством рекламных кампаний
