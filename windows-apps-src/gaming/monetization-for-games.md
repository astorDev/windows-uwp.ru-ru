...
* Разместите игру в Microsoft Store, который является защищенным Интернет-магазином по [всему миру](#worldwide-distribution-channel). Пользователи по всему миру смогут купить вашу игру в Интернете по [установленной вами цене](#set-a-price-for-your-game).
...
* Использование API-интерфейсов в пакете [Microsoft Advertising SDK](https://marketplace.visualstudio.com/items?itemName=AdMediator.MicrosoftAdvertisingSDK) для показа рекламы из рекламных сетей. Вы можете [показывать рекламу в своей игре](#display-ads-in-your-game) и предоставлять игрокам возможность просматривать видеорекламу в обмен на какие-либо бонусы в игре.
...
Microsoft Store может сделать игру доступной для загрузки более 200 стран и регионов по всему миру с поддержкой выставления счетов с помощью различных форм оплаты, включая Visa, MasterCard и PayPal. Полный список стран и регионов см. в разделе [Определение выбора рынка](https://docs.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection).
...
Базовая цена игры определяет, как классифицируется ваша игра: как _платная_ или как _бесплатная_. [Центр партнеров](https://partner.microsoft.com/dashboard) можно использовать для настройки базовой цены на основе страны и региона.
При определении цены необходимо учитывать [налогообложение при продаже приложений в разные страны](https://docs.microsoft.com/windows/uwp/publish/tax-details-for-paid-apps) и [ценовые соображения для конкретных рынков](https://docs.microsoft.com/windows/uwp/publish/define-pricing-and-market-selection). Также можно [установить особые цены для конкретных рынков](../publish/set-and-schedule-app-pricing.md#override-base-price-for-specific-markets).
...
Кампании-распродажи можно планировать заранее, указывая начальную и конечную дату распродажи. Более подробную информацию см. в статье [Выставление на продажу приложений и надстроек](https://docs.microsoft.com/windows/uwp/publish/put-apps-and-add-ons-on-sale).

## <a name="in-game-purchases"></a>Покупки из игры

Покупки из игры — это продукты, приобретаемые внутри игры. Более общее название для них — _покупки из приложения_. В Microsoft Store эти продукты называются дополнительными _компонентами_. [Надстройки публикуются](https://docs.microsoft.com/windows/uwp/publish/add-on-submissions) в центре партнеров. Вам также понадобится включить надстройки в коде игры.
...
При создании потребляемых надстроек нужно решить, как вы хотите их отслеживать &mdash; т. е. будут ли они _управляемыми разработчиком_ или _управляемыми Магазином_ (эта возможность доступна начиная с Windows 10 версии 1607). С помощью управляемой разработчиком системы вы несете ответственность за отслеживание баланса элемента для игроков; используя управляемое хранилищем, Microsoft Store отслеживает баланс товара. Подробнее см. в разделе [Обзор потребляемых надстроек](https://docs.microsoft.com/windows/uwp/monetize/enable-consumable-add-on-purchases).

### <a name="create-in-game-purchases"></a>Создание покупок из игры

Последние API-интерфейсы для покупок из приложения и лицензионной информации входят в состав пространства имен [Windows.Services.Store](https://docs.microsoft.com/uwp/api/windows.services.store) в Windows SDK (начиная с Windows 10 версии 1607). Если вы разрабатываете новую игру, предназначенную для версии 1607 или более поздней, мы рекомендуем использовать пространство имен __Windows.Services.Store__, потому что оно поддерживает последние типы надстроек и обеспечивает более высокую производительность.
Он также разработан для совместимости с будущими типами продуктов и функций, поддерживаемых центром партнеров и магазином. При разработке игр для предыдущих версий Windows 10 используйте вместо него пространство имен [Windows.ApplicationModel.Store](https://docs.microsoft.com/uwp/api/windows.applicationmodel.store).

Подробнее см. в статье [Покупки из приложения и пробные версии](https://docs.microsoft.com/windows/uwp/monetize/in-app-purchases-and-trials).
...
|--------------------------------------------------------------------------|----------------------------------------|
|Игрок входит в магазин. Появляется меню магазина с доступными надстройками и их ценами. |  Игра [получает информацию о продукте](https://docs.microsoft.com/windows/uwp/monetize/get-product-info-for-apps-and-add-ons) для надстроек, [определяет, есть ли у надстроек надлежащая лицензия](https://docs.microsoft.com/windows/uwp/monetize/get-license-info-for-apps-and-add-ons) и отображает надстройки, доступные для приобретения игроком, в меню магазина.                           |
|Игрок нажимает кнопку __Купить__, чтобы приобрести надстройку             |Действие __Купить__ отправляет запрос на покупку надстройки и инициирует процесс оплаты для ее приобретения. Реализация зависит от типа надстройки. Если это [надстройка длительного пользования (однократно приобретаемая)](https://docs.microsoft.com/windows/uwp/monetize/enable-in-app-purchases-of-apps-and-add-ons), игрок может владеть только одним ее экземпляром, пока не истечет его срок действия. Если надстройка является [потребляемой](https://docs.microsoft.com/windows/uwp/monetize/enable-consumable-add-on-purchases), игрок может владеть одним или несколькими ее экземплярами. |
...
2. [Публикация игры как скрытой](#publish-the-game-as-hidden)
3. [Связывание решения игры в Visual Studio с магазином](#associate-your-game-solution-with-the-store)
...
3. Чтобы __вы хотите собрать пакеты для отправки в Microsoft Store?__ выберите __Да__.
4. Войдите в учетную запись разработчика [центра партнеров](https://partner.microsoft.com/dashboard) . Если у вас нет учетной записи разработчика, [зарегистрируйтесь](https://developer.microsoft.com/store/register), чтобы получить ее.
5. Выберите приложение, для которого требуется создать пакет отправки. Если вы еще не создали отправку приложения, укажите новое имя приложения для создания новой отправки. Подробнее см. в статье [Создание приложения посредством резервирования имени](https://docs.microsoft.com/windows/uwp/publish/create-your-app-by-reserving-a-name).
...
1. Перейдите в [Центр партнеров](https://partner.microsoft.com/dashboard) и войдите в систему.
...
4. Настройте эту новую отправку. На странице отправки:
    * Щелкните __Цены и доступность__. В разделе __видимость__ выберите "__скрыть это приложение и запретить получение...__ ", чтобы предоставить доступ к игре только команде разработчиков. Подробнее см. в статье [Распространение и видимость](https://docs.microsoft.com/windows/uwp/publish/set-app-pricing-and-availability).
...
Подробнее см. в статье [Отправка приложений](https://docs.microsoft.com/windows/uwp/publish/app-submissions).

После отправки игры в Магазин игра поступает на [сертификацию приложения](https://docs.microsoft.com/windows/uwp/publish/the-app-certification-process). Процесс сертификации может занять до 16 часов; только после этого игра появится в Магазине.
...
2. Войдите в учетную запись разработчика центра партнеров и выберите имя приложения, с которым связывается это решение.
...
Если вы связали решение с опубликованной игрой, активной и доступной в Магазине, у вашего решения будет активная лицензия; это значит, что вы на один шаг ближе к созданию надстроек для своей игры. Дополнительные сведения см. в разделе [Формирование пакетов приложений](https://docs.microsoft.com/windows/uwp/packaging/index).

#### <a name="create-an-add-on-in-the-store"></a>Создание надстройки в Магазине

При создании надстроек следите за тем, чтобы они связывались с правильной отправкой игры. Подробнее о том, как настроить связанную с надстройкой информацию, см. в статье [Отправка надстроек](https://docs.microsoft.com/windows/uwp/publish/add-on-submissions).
...
* [Свойства](https://docs.microsoft.com/windows/uwp/publish/enter-add-on-properties)
* [Цены и доступность](https://docs.microsoft.com/windows/uwp/publish/set-add-on-pricing-and-availability)
* [Список магазинов](https://docs.microsoft.com/windows/uwp/publish/create-add-on-store-listings)

Если у вашей игры много надстроек, их можно создать программно с помощью __API отправки Microsoft Store__. Дополнительные сведения см. в статье [Создание отправок и управление ими с помощью служб Microsoft Store Services](https://docs.microsoft.com/windows/uwp/monetize/create-and-manage-submissions-using-windows-store-services).
...
Классы [AdControl](https://docs.microsoft.com/uwp/api/microsoft.advertising.winrt.ui.adcontrol), [InterstitialAd](https://docs.microsoft.com/uwp/api/microsoft.advertising.winrt.ui.interstitialad) и [NativeAd](https://docs.microsoft.com/uwp/api/microsoft.advertising.winrt.ui.nativead) в Microsoft Advertising SDK позволяют отображать рекламу в играх.

Для начала работы с ними скачайте и установите [Microsoft Advertising SDK](https://marketplace.visualstudio.com/items?itemName=AdMediator.MicrosoftAdvertisingSDK) с Visual Studio 2015 или более поздней версией. Подробнее см. в статье об [установке Microsoft Advertising SDK](../monetize/install-the-microsoft-advertising-libraries.md).
...
* [Создание баннерных объявлений в XAML и .NET](https://docs.microsoft.com/windows/uwp/monetize/adcontrol-in-xaml-and--net)
* [Создание баннерных объявлений в HTML5 и JavaScript](https://docs.microsoft.com/windows/uwp/monetize/adcontrol-in-html-5-and-javascript)
* [Создание рекламных объявлений внутреннего](https://docs.microsoft.com/windows/uwp/monetize/interstitial-ads)
* [Создание собственных рекламных объявлений](https://docs.microsoft.com/windows/uwp/monetize/native-ads)
...
* [Рекомендации по баннерам](https://docs.microsoft.com/windows/uwp/monetize/ui-and-user-experience-guidelines)
* [Рекомендации для промежуточной видеорекламы](https://docs.microsoft.com/windows/uwp/monetize/ui-and-user-experience-guidelines)

Способы решения распространенных проблем, возникающих при разработке (реклама не отображается, «черный ящик» моргает и исчезает, реклама не обновляется и т. п.) см. в [руководствах по устранению неполадок](https://docs.microsoft.com/windows/uwp/monetize/troubleshooting-guides).
...
SDK Vungle для Windows обеспечивает возможность показа видеорекламы в приложениях и играх. Загрузить этот SDK можно [здесь](https://publisher.vungle.com/sdk/).
...
* Рекламные баннеры: [Windows 10 SDK for XAML](https://adduplex.zendesk.com/hc/articles/204849031-Windows-10-SDK-for-XAML-apps-installation-and-usage)
* Промежуточная реклама: [Windows 10 XAML AdDuplex Interstitial Ad Installation and Usage](https://adduplex.zendesk.com/hc/articles/204849091-Windows-10-XAML-AdDuplex-Interstitial-Ad-Installation-and-Usage)

Подробнее об интеграции SDK AdDuplex в игры UWP для Windows 10, созданные с помощью Unity см. в статье [Windows 10 SDK for Unity apps installation and usage](https://adduplex.zendesk.com/hc/articles/207279435-Windows-10-SDK-for-Unity-apps-installation-and-usage).
...
Сделайте следующий шаг в продвижении своей игры — используйте рекламу. Когда вы [создаете рекламную кампанию](https://docs.microsoft.com/windows/uwp/publish/create-an-ad-campaign-for-your-app) для своей игры, в других приложениях и играх будет отображаться реклама вашей игры.
...
|--------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|Paid                      |В приложениях, соответствующих устройству или категории вашей игры.                                                                                                                                                   |
|Бесплатная от сообщества            |В приложениях, опубликованных другими разработчиками, которые также согласились участвовать в рекламных кампаниях сообщества. Подробнее см. в статье [О рекламе от сообщества](https://docs.microsoft.com/windows/uwp/publish/about-community-ads).|
|Бесплатная собственная                |Только в приложениях, которые опубликовали вы сами. Подробнее см. в статье [О собственной рекламе](https://docs.microsoft.com/windows/uwp/publish/about-house-ads).                                                            |

## <a name="related-links"></a>Дополнительные ссылки

* [Получение платежей](https://docs.microsoft.com/windows/uwp/publish/getting-paid-apps)
* [Типы учетных записей, местоположения и сборы](https://docs.microsoft.com/windows/uwp/publish/account-types-locations-and-fees)
* [Аналитика](https://docs.microsoft.com/windows/uwp/publish/analytics)
* [Глобализация и локализация](https://docs.microsoft.com/windows/uwp/globalizing/globalizing-portal)
* [Реализация пробной версии приложения](https://docs.microsoft.com/windows/uwp/monetize/implement-a-trial-version-of-your-app)
* [Выполнение экспериментов приложения с помощью тестирования A/B](https://docs.microsoft.com/windows/uwp/monetize/run-app-experiments-with-a-b-testing)
