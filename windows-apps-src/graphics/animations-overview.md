...
Библиотека анимации не предоставляет анимаций для всех возможных сценариев. В некоторых случаях вам может понадобиться создать собственную анимацию на XAML. Дополнительные сведения об этом см. в статье [Раскадрованные анимации](storyboarded-animations.md).
...
-   *Анимации темы* — это анимации одного или нескольких свойств, относящихся к готовым типам пользовательского интерфейса XAML из среды выполнения Windows. Анимации тем отличаются от переходов темы, так как первые ориентированы на один конкретный элемент и существуют в конкретных визуальных состояниях элемента управления, а переходы темы назначаются свойствам элемента управления, существующим вне визуальных состояний и влияющим на переходы между этими состояниями. Многие элементы управления среды выполнения Windows на XAML содержат анимации тем в раскадровках, входящих в состав шаблона данного элемента управления, причем анимации запускаются визуальными состояниями. Если вы не вносите изменений в шаблоны, то такие встроенные анимации тем доступны для элементов управления пользовательского интерфейса. Однако в случае замены шаблонов происходит удаление и встроенных анимаций тем элементов управления. Чтобы вернуть их, необходимо определить раскадровку, которая включает анимации тем в составе набора визуальных состояний элемента управления. Можно также запускать анимации тем из раскадровок, не относящихся к визуальным состояниям, используя для этого метод [**Begin**](https://msdn.microsoft.com/library/windows/apps/BR210491), однако такой способ применяется редко. Все анимации темы входят в библиотеку анимации.
-   *Визуальные переходы* применяются, когда элемент управления переходит из одного из заданных визуальных состояний в другое. Эти анимации создаются пользователем и обычно связываются с настраиваемым шаблоном, который создается для элемента управления, и определениями визуальных состояний в этом шаблоне. Анимация выполняется только во время между состояниями. Обычно это малое время, не более нескольких секунд. Подробнее см. в [подразделе "VisualTransition" раздела "Раскадрованные анимации для визуальных состояний"](https://msdn.microsoft.com/library/windows/apps/xaml/JJ819808#VisualTransition).
-   *Раскадрованные анимации* применяются, если они явно запускаются из кода приложения или из кода, который сопровождает включенный компонент (например, элемент управления). Раскадрованные анимации могут постепенно менять значение любого свойства зависимости в среде выполнения Windows. Раскадрованные анимации применяются не только для сценариев пользовательского интерфейса. Очень удобно рассматривать их в качестве методики "машины состояний", если вам знакома эта концепция. Раскадрованная анимация не ограничивается временем перехода между визуальными состояниями. Она может выполняться в любое время независимо от изменения состояния элемента управления, в том числе выполняться постоянно. Дополнительные сведения об этом см. в статье [Раскадрованные анимации](storyboarded-animations.md). Подробнее о свойствах зависимостей и их размещении: [Общие сведения о свойствах зависимостей](https://msdn.microsoft.com/library/windows/apps/Mt185583).
...
-   [Касание/отрыв указателя](./animations-overview.md#pointer-up-down): дает визуальную реакцию на касание или щелчок плитки.
...
-   [Жесты прокрутки](./animations-overview.md#swipe-gestures): показывает, что плитка поддерживает взаимодействие прокрутки, или указывает на выбранное прокруткой состояние.

### Переход между содержимым и переход во время первой загрузки
...
В библиотеке анимаций XAML отсутствует концепция анимации, применимая к странице в целом при ее загрузке, однако имеется отдельный переход ([**EntranceThemeTransition**](https://msdn.microsoft.com/library/windows/apps/BR210288)) который можно применять для содержимого при первой загрузке страницы с содержимым и преобразовании этого содержимого. Таким образом, реакция на первое представление содержимого и на его изменение может быть разной.

### Появление или исчезновение и плавный переход между элементами
...
Также существует отдельный класс [**ReorderThemeTransition**](https://msdn.microsoft.com/library/windows/apps/BR210409), который применяется, если элемент изменяет положение в списке. Он анимируется не так, как удаление элемента или добавление элемента в другое расположение.

### Перетаскивание

Используйте анимацию перетаскивания ([**DragItemThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/BR243173), [**DragOverThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/BR243177)) и анимацию отпускания ([**DropTargetItemThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/BR243185)), чтобы отобразить визуальную реакцию на перетаскивание элемента пользователем.
...
Используйте анимацию [**SwipeHintThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/Hh702489), чтобы показать, что плитка поддерживает взаимодействие прокрутки. Пользователи могут прокрутить вниз, чтобы выбрать плитку. Если пользователь не знает, можно ли прокручивать плитку, жест нажатия и удерживания плитки выведет анимацию подсказки прокрутки, сообщающую, что с плиткой можно взаимодействовать через прокрутку.

Используйте анимацию [**SwipeBackThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/Hh702477) для отображения выбора плитки и возврата плитки в исходное положение.
...
Посмотрите видео, демонстрирующее эту анимацию:
...
На этом видео демонстрируется анимация, выполняемая для удаляемых прямоугольников:
...
| API | Описание |
|-----|-------------|
| [**AddDeleteThemeTransition**](https://msdn.microsoft.com/library/windows/apps/BR243047) | Обеспечивает анимированный переход в ситуациях, когда элементы управления добавляют или удаляют дочерние объекты или содержимое. Обычно элемент управления представляет собой контейнер элементов. |
...
| [**SplitCloseThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/BR210454) | Анимация с заданными настройками, которая скрывает целевой пользовательский интерфейс с помощью разделенной анимации. |
| [**SplitOpenThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/BR210472) | Анимация с заданными настройками, которая выводит конечный пользовательский интерфейс с помощью разделенной анимации. |
| [**SwipeBackThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/Hh702477) | Анимация с заданными настройками, применяемая к элементам управления, которые возвращаются со скольжением на свое штатное место в макете после взаимодействия с жестом прокрутки. |
| [**SwipeHintThemeAnimation**](https://msdn.microsoft.com/library/windows/apps/Hh702489) | Предварительно настроенная анимация, указывающая на то, что теперь можно выполнить жест прокрутки. |
...
Самая большая область определения пользовательского интерфейса приложения в XAML, где вы определяете пользовательские раскадрованные анимации, — это определение визуальных состояний элементов управления в XAML. Это делается при создании нового класса элементов управления или при изменении шаблона существующего элемента управления, имеющего визуальные состояния в шаблоне. Дополнительные сведения об этом см. в статье [Раскадрованные анимации для визуальных состояний](https://msdn.microsoft.com/library/windows/apps/xaml/JJ819808). Эти анимации обычно представляют собой не постепенные переходы, а мгновенные изменения, и в большей степени являются методом задания набора изменений свойств для состояния. Они не всегда применяют визуальную анимацию к пользовательскому интерфейсу, хотя зачастую визуальные состояния для элементов управления включают сами анимации библиотеки. В этом случае в анимациях тем используются постепенные переходы, хотя обычно они достаточно кратковременные.
