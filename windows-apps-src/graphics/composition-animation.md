...
##Типы анимаций композиции
            ...
            **Анимация с помощью выражений**— это новый тип анимации, реализованный в визуальном уровне в ноябрьском обновлении Windows 10 (сборка 10586). Коротко говоря, анимация с помощью выражений позволяет разработчикам создавать математические связи между свойствами объектов класса Visual и дискретными значениями, которые будут анализироваться и обновляться для каждого кадра. Разработчики могут создавать ссылки на свойства объектов композиции и наборы свойств, использовать вспомогательные математические функции и даже ссылаться на входные данные для запуска этих математических связей. Выражения могут использоваться для описания более сложных видов взаимодействия с пользователем (например, закрепленных заголовков и параллакса) на платформе Windows.  

##Преимущества анимаций композиции
**Производительность**  
 При создании универсальных приложений для Windows большая часть кода разработчика выполняется в потоке пользовательского интерфейса. Таким образом, чтобы обеспечить плавное выполнение анимаций на разных категориях устройств, система выполняет вычисление анимированных значений и обработку на независимом потоке для сохранения частоты обновления в 60кадр/с. Это значит, что разработчики могут не беспокоится о плавности выполнения анимаций, так как за это будет отвечать система, в то время как приложения будут выполнять другие сложные операции для реализации сложных видов взаимодействия с пользователем.    
 
**Возможности**  
Целью использования анимаций композиции в визуальном уровне является создание красивого пользовательского интерфейса. Мы хотим предоставить разработчикам необходимую гибкость и разные типы анимаций, чтобы они могли воплощать свои невероятные замыслы и расширять возможности работы с UWP
...
*Если требуется анимировать все свойство TransformMatrix в качестве типа Matrix4x4, для этого необходимо использовать анимацию на основе выражений. Или же можно выбрать отдельные ячейки матрицы и применить к ним анимацию по ключевым кадрам либо анимацию на основе выражений.  

###Объекты класса InsetClip
...
##Свойства подканала объекта класса Visual
Помимо возможности анимации свойств объектов класса Visual для применения анимации также можно выбирать *подканальные* компоненты этих свойств. Предположим, вы просто хотите анимировать смещение по оси X (X Offset) объекта класса Visual, а не все смещение (Offset) полностью. Анимацию можно применить либо к свойству Vector3 Offset, либо к компоненту Scalar X свойства Offset. Вдобавок к возможности выбирать отдельный подканальный компонент свойства вы можете выбрать несколько компонентов. Например, вы можете выбрать компоненты X и Y свойства Scale.
...
##Наборы свойств и эффекты
Помимо возможности анимации свойств объектов класса Visual и InsetClip композиции можно также анимировать свойства в наборе свойств (PropertySet) или эффекты. При работе с наборами свойств выполняется определение свойства, которое помещается на хранение в набор свойств композиции. В дальнейшем это свойство можно будет выбрать для анимации (а также одновременно с этим установить на него ссылку из другого свойства). Более подробно об этом будет рассказано в разделах ниже.  
...
##Краткое руководство: начало работы с анимациями композиции
...
4.  Запустите анимацию объекта Visual, свойство которого требуется анимировать: вызовите функцию StartAnimation и добавьте в качестве параметров имя анимируемого свойства (в строковом формате) и анимируемый объект.  
...
###Создание собственной анимации и определение ключевых кадров
...
1.  Функция StartAnimation прикрепляется к объекту Visual.Brush вместо объекта Visual, где **Color** является параметром свойства, которое необходимо анимировать.
2.  Компонент "value" ключевого кадра определяется объектом Colors пространства имен Windows.UI.
3.  Вы можете определить цветовое пространство для интерполяции, задав свойство InterpolationColorSpace. Возможные значения: а)  CompositionColorSpace.Rgb б)  CompositionColorSpace.Hsl


##Свойства анимации по ключевым кадрам
...
После того как вы определили анимацию и ключевые кадры, вы можете подключить анимацию. При запуске анимации необходимо указать объект Visual, который необходимо анимировать, целевое свойство для анимации и ссылку на анимацию. Для этого необходимо вызвать функцию StartAnimation(). Следует помнить, что вызов функции StartAnimation() для свойства приведет к отключению и удалению любых ранее запущенных анимаций.  

            **Примечание.** Ссылка на анимируемое свойство имеет строковый формат.  

В фрагменте кода ниже выполняется настройка и запуск анимации свойства Offset объекта Visual.  
```cs
...
В следующем фрагменте кода выполняется остановка анимации свойства Offset объекта Visual.    
```cs
...
##Создание и прикрепление анимации с помощью выражений
...
###Параметры свойств
Одной из сильных сторон языка выражений являются параметры свойств. В строку выражения можно добавлять ссылки на значения свойств от других объектов, таких как визуальный объект композиции, набор свойств композиции (Composition Visual, Composition Property Set) или других объектов C#.   
...
В следующем фрагменте кода выполняется создание анимации на основе выражений, которая будет ссылаться на свойство Offset двух других визуальных объектов композиции и базовый объект Vector3 System.Numerics.  
```cs
...
В примере ниже выполняется установка ссылок на подканал Х свойств Offset от двух объектов Visual— одна непосредственно в самой строке выражения и другая при создании параметра ссылки.
...
###Вспомогательные функции выражений и конструкторы
...
##Анимация с помощью наборов свойств
...
##Набор свойств ManipulationPropertySet
...
##Приложение
###Функции выражения по типу структуры
###Scalar (Скаляр)  
...
|Sin(Float value1)||
|Asin(Float value2)|    Возвращает синус или арксинус параметра Float|
|Cos(Float value1)||
|ACos(Float value2)|    Возвращает косинус или арккосинус параметра Float|
|Tan(Float value1)||
|ATan(Float value2)|    Возвращает тангенс или арктангенс параметра Float|
...
|Lerp(Matrix3x2 value1, Matrix3x2 value2, Float progress)|  Возвращает Matrix3x2, которая представляет собой результат вычисления линейной интерполяции между двумя значениями Matrix3x2 с учетом значения состояния изменения ключевого кадра (Примечание. Значения от 0,0 до 1,0)|
...
|Lerp(Matrix4x4 value1, Matrix4x4 value2, Float progress)|  Возвращает Matrix4x4, которая представляет собой результат вычисления линейной интерполяции между двумя значениями Matrix4x4 с учетом значения состояния изменения ключевого кадра (Примечание. Значения от 0,0 до 1,0)|
