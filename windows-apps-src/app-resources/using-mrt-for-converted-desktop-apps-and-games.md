...
1. Чтобы создать пакет приложения, выполните следующую команду, заменив `contoso_demo.appx` именем файла MSIX/AppX, который необходимо создать, и выберите другой каталог для файла (в этом примере используется родительский каталог). он может находиться в любом месте, но **не** должен быть каталогом проекта).
