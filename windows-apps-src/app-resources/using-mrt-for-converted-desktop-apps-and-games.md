...
**Этап 2. Использовать MRT для определения и поиска ресурсов**
...
**Этап 4. Перенос в форматы MRT ресурсов и API-интерфейсы**
...
Если вы хотите использовать [Desktop App Converter](https://aka.ms/converter), обратитесь к [ **рабочего стола для UWP Bridge: Desktop App Converter** в MSDN](https://aka.ms/converterdocs) Дополнительные сведения о процессе преобразования. Полный набор примеров Desktop Converter можно найти в [репозитории GitHub **Примеры кода для преобразования классических приложений в приложения UWP**](https://github.com/Microsoft/DesktopBridgeToUWP-Samples).
...
### <a name="step-11-update-strings--assets-in-the-appxmanifest"></a>Шаг 1.1. Обновите строки и средств в манифест AppXManifest
...
[!Note] Существуют ограничения на длину некоторые из этих строк. Дополнительные сведения см. в разделе [VisualElements](/uwp/schemas/appxpackage/appxmanifestschema/element-visualelements?branch=live).
...
0. Создание `Strings\en-us` папке (или другом языке, соответствующим образом) в проекте и добавьте **новый элемент** в корневую папку проекта, используя имя по умолчанию `resources.resw`
...
**Обновить манифест для ссылки на ресурсы**
...
*Примечание: опущены многие элементы манифеста, этот фрагмент кода — что-нибудь не удалять!*
...
### <a name="step-12-build-pri-file-make-an-appx-package-and-verify-its-working"></a>Шаг 1.2. Построение PRI-файл, внести пакета AppX и проверить его работы
 ...
 * `/o` Задает его, чтобы перезаписать выходной файл, если он существует
0. После создания пакета его необходимо подписать. Самый простой способ получить сертификат для подписи, — путем создания пустой проект универсальной Windows в Visual Studio и копирования `.pfx` файла, он создает, но его можно создать вручную с помощью `MakeCert` и `Pvk2Pfx` служебные программы, как описано в <a href="https://msdn.microsoft.com/en-us/library/windows/desktop/jj835832.aspx"> Создание сертификата для подписи пакета приложения</a>, в этой статье MSDN. 
 * **Важно!** Если вы вручную создать сертификат для подписи, убедитесь, что поместите файлы в каталоге, отличном от исходного проекта или источника пакета, в противном случае он может получить входит в состав пакета, включая закрытый ключ!
...
0. Принять запроса на повышение прав контроля учетных записей администратора, в том случае, если она находится и нажмите кнопку `Next`
0. Введите пароль для закрытого ключа, если таковой имеется и нажмите кнопку `Next`
0. Выберите `Place all certificates in the following store`
0. Нажмите кнопку `Browse` и выберите папку `Trusted People` (**не** "Доверенные издатели")
0. Нажмите кнопку `Next` и затем `Finish`
...
**Создайте дополнительные локализованные ресурсы**
...
**Обновление манифеста AppX для поддерживаемых языков**
...
**Повторное построение с локализованные значения**
...
**Тестирование с помощью локализованные значения**
...
0. Перейти к `Time & language`
0. Перейти к `Region & language`
0. Нажмите кнопку `Add a language`
...
0. Нажмите кнопку `Set as default`
...
### <a name="step-14-localizing-more-parts-of-the-appx-manifest-optional"></a>Шаг 1.4. Локализация большему количеству частей элемента манифеста AppX (необязательно)
...
**Загрузка ресурсов многоязыкового интерфейса в Win32**
...
**С помощью инструмента создания пакета**
