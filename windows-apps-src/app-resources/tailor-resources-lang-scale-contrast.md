...
При добавлении нескольких квалификаторов в имя папки они разделяются символом подчеркивания. `<qualifier1>[_<qualifier2>...]` является форматом.
...
Целевая система или приложение должны предоставить значение, с которым сопоставляются квалификаторы `alternateform`. При указании своих значений квалификатора `msft-` не добавляйте префикс `alternateform`.
...
Квалификатор `homeregion` соответствует параметру страны или региона пользователя. Он представляет домашнее расположение пользователя. Его значением может быть любой действительный [тег региона BCP-47](https://tools.ietf.org/html/bcp47). то есть любой двухбуквенный код региона **ISO 3166-1 alpha-2** или трехзначный географический код для составных регионов **ISO 3166-1 numeric** (см. [Состав кодов регионов подразделения статистики ООН M49](https://unstats.un.org/unsd/methods/m49/m49regin.htm)). Коды выбранных экономических и других групп недействительны.

## <a name="language"></a>Язык

Квалификатор `language` соответствует параметру языка интерфейса. Его значением может быть любой действительный [тег языка BCP-47](https://tools.ietf.org/html/bcp47). Список языков см. в [реестре подтегов языков IANA](https://www.iana.org/assignments/language-subtag-registry).
...
## <a name="shell-light-theme-and-unplated-resources"></a>Непредусмотренная тема оболочки и недоступные ресурсы
*Обновление Windows 10 2019 может привести* к появилось новой «светлой» темой оболочки Windows. В результате некоторые активы приложения, которые ранее отображались на темном фоне, теперь будут отображаться на светлом фоне. Для приложений, которые предоставили алтформ ресурсы для панели задач и окна Переключатели (ALT + TAB, представление задач и т. д.), следует убедиться, что они имеют приемлемую контрастность на светлом фоне.

### <a name="providing-light-theme-specific-assets"></a>Предоставление тематических активов
Приложения, которым требуется предоставить специализированный ресурс для светлой темы оболочки, могут использовать новый квалификатор ресурса альтернативной формы: `altform-lightunplated`. Этот квалификатор отражает существующий квалификатор алтформ-unplate. 

### <a name="downlevel-considerations"></a>Рекомендации по предыдущим уровням
Приложения не должны использовать квалификатор `theme-light` с квалификатором `altform-unplated`. Это приведет к непредсказуемому поведению в RS5 и более ранних версиях Windows из-за способа загрузки ресурсов для панели задач. В более ранних версиях Windows может использоваться неправильная версия темы. Квалификатор `altform-lightunplated` позволяет избежать этой проблемы. 

### <a name="compatibility-behavior"></a>Поведение совместимости
Для обеспечения обратной совместимости Windows включает логику для обнаружения монохромных значков и проверяет, отличается ли он от предполагаемого фона. Если значок не соответствует требованиям к контрастам, Windows будет искать версию ресурса с контрастной белой версией. Если это недоступно, Windows вернется к использованию размещенной в нем версии ресурса.
...
* [Статистическое деление Соединенных стран M49 композиция кодов регионов](https://unstats.un.org/unsd/methods/m49/m49regin.htm)
