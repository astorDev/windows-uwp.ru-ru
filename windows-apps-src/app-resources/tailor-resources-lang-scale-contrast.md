...
Квалификатор `targetsize` используется в основном для указания [значков сопоставления типов файлов](https://docs.microsoft.com/windows/desktop/shell/how-to-assign-a-custom-icon-to-a-file-type) или [значков протоколов](https://docs.microsoft.com/windows/desktop/search/-search-3x-wds-ph-ui-extensions), отображаемых в проводнике. Значение этого квалификатора представляет длину стороны квадратного изображения в физических (необработанных) пикселях. Загружается ресурс, значение которого соответствует параметру "Представление" в проводнике, или следующий ресурс с наибольшим значением при отсутствии точного соответствия.
...
## <a name="shell-light-theme-and-unplated-resources"></a>Светлая тема оболочки и без покрытия ресурсы
*Windows 10 может обновлять 2019* представил новую тему «light» для оболочки Windows. В результате некоторые ресурсы приложения, которые ранее отображались на темном фоне теперь отображается на светлым фоном. Для приложений, приложениям, который предоставлять без покрытия altform активы для задач и окно Переключатели (Alt + Tab, представление задач, и т.д.), следует убедиться, что они имеют допустимые контрастности на светлым фоном.

### <a name="providing-light-theme-specific-assets"></a>Предоставляя конкретные ресурсы "светлой" теме
Приложения, которые хотите предоставить собственные ресурсы для "светлой" теме оболочки можно использовать новый квалификатор ресурсов альтернативный способ: `altform-lightunplated`. Этот квалификатор отражает существующие без покрытия altform квалификатор. 

### <a name="downlevel-considerations"></a>Рекомендации по нижнего уровня
Приложения не должны использовать `theme-light` квалификатор с `altform-unplated` квалификатор. Это приведет к непредсказуемому поведению в RS5 и более ранних версиях Windows из-за ресурсов способом загружаются для панели задач. В более ранних версиях windows версия темы light может использоваться неправильно. `altform-lightunplated` Квалификатор позволяет избежать этой проблемы. 

### <a name="compatibility-behavior"></a>Поведение совместимости
Для обеспечения обратной совместимости, Windows включает логику для обнаружения монохромное значки и проверьте ли они отличаются от предполагаемого фона. Если значок не соответствует требования к контрастности элементов, Windows будет искать контрастность белого версию ресурса. Если это не доступны, Windows будет переключиться на использование покрытием версию ресурса.
...
* [Подготовка к локализации](https://docs.microsoft.com/previous-versions/windows/apps/hh967762(v=win.10))
