...
description: Используйте встроенные тесты Bridge рабочего стола для убедитесь, что ваше приложение оптимизирован для ее преобразования в приложении UWP.
ms.author: pafarley
ms.date: 12/18/2017
ms.topic: article
ms.prod: windows
...
ms.locfileid: "2820230"
---
# <a name="windows-desktop-bridge-app-tests"></a>Тесты приложений моста для классических приложений

[Мост настольных приложений](https://docs.microsoft.com/en-us/windows/uwp/porting/desktop-to-uwp-root) , настольных приложений Windows преобразуется в универсальные платформы Windows (UWP) приложений через [Мост рабочего стола](https://developer.microsoft.com/en-us/windows/bridges/desktop). После преобразования классическое приложение для Windows упаковывается, обслуживается и развертывается в виде пакета приложения UWP (APPX или APPXBUNDLE), предназначенного для классической версии Windows 10.

## <a name="required-versus-optional-tests"></a>Обязательные и необязательные тесты
Необязательный тесты для приложений Bridge рабочий стол Windows информационные только и не будет использоваться для проверки приложения во время хранилища Майкрософт адаптация новых сотрудников. Мы рекомендуем исследование результаты для создания улучшенных приложений качества тестов. Общие критерии допуска приложения к выставлению в магазине определяются обязательными тестами, результаты необязательных тестов при этом не учитываются.
...
### <a name="2-app-manifest-resources-tests"></a>2. Тесты ресурсов манифеста приложения 
...
* Проверяет, что каждый управляемых двоичный файл в пакете приложения не будет иметь зависимости на Win32 API, который не поддерживается для разработки приложений UWP, проверив таблицы адресов импорта двоичного файла.
...
> [!NOTE]
> Построение отладки приложения завершится ошибкой этот тест, даже в том случае, если приложение использует только [API-интерфейсы для UWP приложений](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx). Просмотр сообщений об ошибках для идентификации API этот параметр указан, не разрешенные API для UWP приложений. 

> [!NOTE]
> Приложения C++, встроенных в конфигурации отладки завершится ошибкой этот тест, даже в том случае, если конфигурацию только используются интерфейсы API из пакета SDK Windows для UWP приложений. Для получения дополнительных сведений обратитесь [к API -интерфейсы Windows в приложениях UWP альтернативы](https://msdn.microsoft.com/library/windows/apps/hh464945.aspx) .
...
**Сведения о тесте**  
Приложения не могут запрашивать повышение прав администратора или UIAccess каждой политики хранения Microsoft. Повышение разрешений безопасности не поддерживается. 
...
**Корректирующие действия**  
В таком случае нужно используйте последнюю версию пакета [Bing Maps SDK](http://go.microsoft.com/fwlink/p/?linkid=614880) для UWP приложений.
