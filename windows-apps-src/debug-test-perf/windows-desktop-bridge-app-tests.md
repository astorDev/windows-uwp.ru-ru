...
description: Используйте встроенные тесты Desktop Bridge для оптимизации классического приложения для ее преобразования в приложение UWP.
ms.author: pafarley
ms.date: 12/18/2017
ms.topic: article
...
> [!NOTE]
> Отладочная сборка приложения не пройдет этот тест, даже если приложение использует только [API для приложений UWP](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx). Просмотрите сообщения об ошибках для идентификации API-Интерфейс присутствует, не разрешенных API для приложений UWP. 

> [!NOTE]
> Приложения C++, которые встроены в конфигурации отладки не пройдет этот тест, даже если в конфигурации используются только API из пакета Windows SDK для приложений UWP. Дополнительные сведения см. [альтернативы API Windows в приложениях UWP](https://msdn.microsoft.com/library/windows/apps/hh464945.aspx) .
