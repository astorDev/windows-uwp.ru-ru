...
**Корректирующие действия**  
Объяснение этих расширений и описание их надлежащего использования см. в статье [Мост переноса классических приложений на UWP: расширение приложений](https://docs.microsoft.com/windows/apps/desktop/modernize/desktop-to-uwp-extensions). 

### <a name="3-debug-configuration-test"></a>3. Тест отладочной конфигурации
Этот тест проверяет, что msix или appx не является отладочной сборкой.
...
**Сведения о тесте**  
Проверяет манифест приложения, чтобы подтвердить правильность его содержимого в соответствии с разделом [Требования к пакетам приложений](https://docs.microsoft.com/windows/uwp/publish/app-package-requirements). В этом тесте выполняются следующие проверки:
...
* Проверяет, что каждый управляемый двоичный файл в пакете приложения не зависит от API Win32, который не поддерживается для разработки приложений UWP, путем проверки таблицы адресов импорта двоичного файла.
...
> [!NOTE]
> Отладочная сборка приложения не будет выполнять этот тест, даже если приложение использует только [API для приложений UWP](https://docs.microsoft.com/uwp/). Проверьте сообщения об ошибках, чтобы найти API, который не является разрешенным API для приложений UWP. 

> [!NOTE]
> C++приложения, созданные в конфигурации отладки, не будут выполнять этот тест, даже если в конфигурации используются только интерфейсы API из Windows SDK для приложений UWP. Дополнительные сведения см. в разделе [альтернативы интерфейсам API Windows в приложениях UWP](https://docs.microsoft.com/uwp/win32-and-com/win32-and-com-for-uwp-apps) .
...
**Сведения о тесте**  
Приложение не может запросить повышение прав администратора или UIAccess на политику Microsoft Store. Повышение разрешений безопасности не поддерживается. 

**Корректирующие действия**  
Приложения должны запускаться от имени текущего пользователя. Более подробную информацию см. в разделе [Обзор системы безопасности модели автоматизации пользовательского интерфейса](https://docs.microsoft.com/dotnet/framework/ui-automation/ui-automation-security-overview?redirectedfrom=MSDN).
...
* **Имя типа — чувствительность к регистру**  
...
* *Bing. Maps. Жаваскрипт\жс\веапикоре.ЖС*  
...
**Корректирующие действия**  
Чтобы исправить это, используйте последнюю версию [пакета SDK для карт Bing](https://www.bingmapsportal.com/) для приложений UWP.
