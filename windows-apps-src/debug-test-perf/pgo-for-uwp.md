...
description: Пошаговое руководство по применению профильной оптимизации (PGO) для приложений универсальной платформы Windows (UWP).
ms.date: 02/08/2017
...
Снимки экрана в этом руководстве основаны на следующий новый проект: ![Диалоговое окно нового проекта](images/pgo-001.png)
...
7. Выполняемая в настоящее время сборка является инструментируемой; она создает данные PGO. На данном этапе необходимо запустить приложение для некоторых самых распространенных сценариев, которые планируется оптимизировать. После прогона предполагаемых сценариев программой найдите инструмент pgosweep.exe, расположенный в той же папке, что и соответствующая версия `pgort140.dll`. Кроме того, в командной строке Visual Studio (x86/x64) Native Tools соответствующая версия уже будет записана в пути. Для сбора данных PGO выполните следующую команду, пока приложение работает, чтобы создать PGC-файл, который будет содержать данные профилирования.
  ...
  Дальнейшие сбор может также быть `App1!CoreScenario.pgc`, `App1!UseCase5.pgc`и т. д. Если PGC-файлы называются, таким образом и в местоположения выходных данных сборки вместе с .pgd, они будут автоматически объединены при связывании на шаге 9.
 
8. НЕОБЯЗАТЕЛЬНО: По умолчанию всех файлов с именем, как указано в шаге 7 и помещено рядом с .pgd будут объединены при связывания и взвешенных одинаково, но также может иметь больше управления для всех выполнений, каким образом конкретные снабжаются весовыми коэффициентами. Для этого необходимо использовать инструмент **pgomgr.exe**, который находится в той же папке, где впервые был обнаружен экземпляр `pgort140.dll`. Например, чтобы объединить `CoreScenario`, запускаемый с тройным по сравнению с другими запусками приоритетом, можно использовать следующую команду:
