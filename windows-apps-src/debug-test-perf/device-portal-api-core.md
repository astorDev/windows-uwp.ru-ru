...
Весь портал устройств Windows построен на REST API, которые можно использовать для получения доступа к данным и программного управления устройством.
...
Список установленных на компьютере устройств можно получить с помощью следующего формата запроса.
...
Создать сеанс трассировки событий Windows в режиме реального времени можно с помощью следующего формата запроса. Процесс будет управляться по веб-сокету.  События трассировки событий Windows упаковываются на сервере и отправляются клиенту раз в секунду. 
...
Список работающих в настоящее время процессов можно получить с помощью следующего формата запроса.  Его также можно обновить до подключения WebSocket с теми же данными JSON, отправляемыми клиенту раз в секунду. 
...
Данные о текущем состоянии аккумулятора можно получить с помощью следующего формата запроса.
...
Перезапустить целевой компьютер можно с помощью следующего формата запроса.
...
Текущую конфигурацию IP можно получить с помощью следующего формата запроса.
...
Передать профиль WPR и запустить трассировку с использованием этого профиля можно с помощью следующего формата запроса.  Одновременно может выполняться только одна трассировка. Профиль не останется на устройстве. 
