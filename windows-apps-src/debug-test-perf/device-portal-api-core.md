...
---
### <a name="get-a-list-of-paired-bluetooth-devices"></a>Получение списка связанными устройствами Bluetooth

**Запрос**

Список в настоящее время связанными устройствами Bluetooth можно получить, используя следующий формат запроса. Это можно обновить до соединения WebSocket с теми же данными JSON. В течение времени существования соединения WebSocket можно изменить список устройств. Полный список устройств будет отправляться через соединение WebSocket каждый раз, когда происходит обновление.
...
Ответ содержит массив JSON устройств Bluetooth, которые в настоящее время являются парными.
```json
{"PairedDevices": [
    {
        "Name" : string,
...
```
*AudioConnectionStatus* поля будут присутствовать в том случае, если устройство может использоваться для аудио в этой системе. (Политики и дополнительных компонентов может повлиять на это.) *AudioConnectionStatus* будет «Подключено» или «Отключено».
...
Можно получить список доступных устройств Bluetooth для связывания, используя следующий формат запроса. Это можно обновить до соединения WebSocket с теми же данными JSON. В течение времени существования соединения WebSocket можно изменить список устройств. Полный список устройств будет отправляться через соединение WebSocket каждый раз, когда происходит обновление.
...
Будет подключиться к устройству, если устройство может использоваться для аудио в этой системе. (Политики и дополнительных компонентов может повлиять на это.)

| Метод       | Универсальный код ресурса (URI) запроса           |
| :---         | :---                  |
| ПОМЕСТИТЬ         | /API/BT/connectdevice |

**Параметры URI**

| Параметр универсального кода ресурса (URI) | Описание |
| :---          | :--- |
| ID            | (**требуется**) идентификатор конечной точки ассоциации для устройства Bluetooth и должно быть в кодировке base 64. |
...
Будет отключить устройство, если устройство может использоваться для аудио в этой системе. (Политики и дополнительных компонентов может повлиять на это.)

| Метод       | Универсальный код ресурса (URI) запроса              |
| :---         | :---                     |
| ПОМЕСТИТЬ         | /API/BT/disconnectdevice |
...
## <a name="remote-control"></a>Пульт ДУ
...
### <a name="set-a-static-ip-address-ipv4-configuration"></a>Задать статический IP-адрес (IPV4-конфигурация)
