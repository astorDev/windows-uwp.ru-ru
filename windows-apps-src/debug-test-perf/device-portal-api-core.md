...
| :------     | :----- |
| ПОМЕСТИТЬ | /api/app/packagemanager/package |
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/app/packagemanager/networkapp |
...
Список установленных на компьютере Bluetooth-устройств можно получить, используя следующий формат запросов. Это можно обновить до соединения WebSocket, с теми же данными JSON.
...
<hr>
### Включение или отключение Bluetooth-устройства.
...
| Параметр универсального кода ресурса (URI) | Описание |
| :------          | :------ |
| ID            | (**необходимо**) Идентификатор устройства для Bluetooth-устройства, должен быть в кодировке base 64. |
| Состояние         | (**требуется**) это может быть `"On"` или `"Off"`. |
...
<hr>
### Получение данных на подключенных USB-устройствах/концентраторах
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/debug/dump/usermode/crashcontrol |
...
1. Полный дамп памяти (собирает все используемой памяти)

2. Дамп памяти ядра (не учитывает памяти в режиме пользователя)

3. Минидамп ограниченной ядра
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/debug/dump/kernel/crashcontrol |
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/power/cfg/*<power scheme path>* |
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/control/shutdown |
...
<hr>
### Завершение процесса посредством PID
...
<hr>
### Задать статический IP-адрес (IPV4-конфигурация)

**Запрос**

Задает конфигурацию IPV4, статические IP-адрес и DNS. Если статический IP-адрес не указан, он позволяет DHCP. Если указан статический IP-адрес, затем DNS должны быть также указаны.
...
| Параметр универсального кода ресурса (URI) | Описание |
| :---          | :--- |
| AdapterName | (**требуется**) сетевой интерфейс GUID. |
| IPAddress | Статический IP-адрес для задания. |
| Маска подсети | (**требуется** Если *IPAddress* не равно null) маску подсети статический. |
| Шлюз по умолчанию | (**требуется** Если *IPAddress* не равно null) статический шлюз. |
| PrimaryDNS | (**требуется** Если *IPAddress* не равно null) статический основной DNS-сервер для задания. |
...
| ключ   | (**необходимо, если *op* == connect и необходима проверка подлинности сети**) Общий ключ. |
...
-  Нет.  **Примечание.** Это длительная операция.  Значение возвращается, после того как ETL завершит запись на диск.
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/filesystem/apps/rename |
...
| Метод      | Универсальный код ресурса (URI) запроса |
| :------     | :----- |
| ПОМЕСТИТЬ | /api/filesystem/apps/file |
