...
Портал устройств Windows — это веб-сервер на своем устройстве, которому можно подключаться из веб-браузера на ПК. Если устройство оснащено веб-браузера, можно также подключаться локально из браузера на этом устройстве.
...
2. Подключите устройство к Компьютеру через локальную сеть или USB.
...
* **Развертывание приложений**: развертывание упакованные приложения из локальной сети или веб-узлов и зарегистрировать свободных файлов из сетевой папки.
* **Установленные приложения**: используйте в раскрывающемся меню для удаления или запуск приложений, установленных на устройстве.
* **Запуск приложений**: получение сведений о приложениях, которые в настоящее время выполняются и закройте их при необходимости.

#### <a name="install-sideload-an-app"></a>Установка приложения (загрузки неопубликованных)
...
2. В портале устройств Windows перейдите на страницу **Диспетчер приложений** .

3. В разделе **Развертывание приложений** выберите **Локального хранилища**.

4. В списке **Выберите пакет приложения**выберите **Выбрать файл** и найдите пакет приложения, который требуется загрузить неопубликованное.

5. В списке **выберите файл сертификата (.cer), используемый для подписи пакета приложения**выберите **Выбрать файл** и перейдите к сертификату, связанные с помощью этого пакета приложения.

6. Проверьте соответствующих полях, если вы хотите установить дополнительные или пакеты платформы вместе с установки приложения и выберите **Далее** выбирать их.

7. Выберите **установки** для запуска установки.

8. Если устройство работает под управлением Windows 10 в S-режиме, и его в первый раз, или сертификат был установлен на устройстве, перезагрузите устройство.

#### <a name="install-a-certificate"></a>Установка сертификата

Кроме того можно установить сертификат через портал устройств Windows и установить приложение с помощью других средств:

1. В портале устройств Windows перейдите на страницу **Диспетчер приложений** .

2. В разделе **Развертывание приложений** выберите **Установить сертификат**.

3. В списке **выберите файл сертификата (.cer), используемый для подписывания пакета приложения**выберите **Выбрать файл** и найдите сертификат, связанный с пакет приложения, который требуется загрузить неопубликованное.

4. Выберите **установки** для запуска установки.

5. Если устройство работает под управлением Windows 10 в S-режиме, и его в первый раз, или сертификат был установлен на устройстве, перезагрузите устройство.

#### <a name="uninstall-an-app"></a>Удаление приложения
1.  Убедитесь, что приложение не работает. 
2.  Если Да, перейдите на **Запуск приложений** и закройте его. Если вы попытаетесь удалить работающее приложение, это приведет к возникновению проблем при попытке переустановить приложение. 
...
На этой странице можно просматривать и обрабатывать файлы неопубликованных приложений. См. в разделе [с помощью проводника приложения](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/) записи блога для получения дополнительных сведений о проводнике и как его использовать. 
...
- **Ввод-вывод**: чтение и запись объемов данных
- **Сеть**: получение и отправка данных
- **Графический Процессор**: процент общее доступных GPU модуля использования


![Страница "производительность портала устройства"](images/device-portal/mob-device-portal-perf.png)
...
- **Фильтров или события**: В разделе **события** перечислены события Трассировки событий Windows выбранных поставщиков в формате таблицы. Таблица обновляется в режиме реального времени. Используйте меню « **фильтры** » для настройки пользовательские фильтры, для которых будут отображаться события. Нажмите кнопку « **Очистить** », чтобы удалить все события трассировки событий Windows из таблицы. Это не приводит к отключению каких-либо поставщиков. Вы можете щелкнуть **Сохранить файл** экспортировать собранные события Трассировки событий Windows для локального CSV-файла.

Дополнительные сведения об использовании ведения журнала трассировки событий Windows см. в статье в блоге [Использования портала устройств для просмотра журналов отладки](https://blogs.windows.com/buildingapps/2016/06/10/using-device-portal-to-view-debug-logs-for-uwp/) . 
...
![Страница "производительность трассировки портала устройства"](images/device-portal/mob-device-portal-perf-tracing.png)
...
Захват. ETL-файлы можно открыть для анализа в [Windows Performance Analyzer](https://msdn.microsoft.com/library/windows/desktop/hh448170.aspx).

### <a name="device-manager"></a>Диспетчер устройств

На странице диспетчера устройств перечисляет все периферийные устройства, подключенные к устройству. Вы можете щелкнуть значков параметры, чтобы просмотреть свойства каждого из них.

![Страницы портала устройства диспетчера устройств](images/device-portal/mob-device-portal-devices.png)

### <a name="networking"></a>Сеть

На странице сеть Управление сетевыми подключениями на устройстве. Если вы не подключены к порталу устройства через USB, изменение этих параметров будет скорее всего приводит к отключению от портала устройств.
- **Доступные сети**: показано сети WiFi, доступные на устройстве. Щелкните или нажмите сеть, чтобы подключиться к ней, при необходимости предоставьте ключ доступа. Портал устройства еще не поддерживает Корпоративная проверка подлинности. Также можно использовать в раскрывающемся списке **профилей** попытается подключиться к любому из профилей Wi-Fi, известные на устройстве.
