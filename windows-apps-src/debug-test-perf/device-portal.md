...
Функциональные возможности портал устройств Windows реализуется с помощью [Интерфейсов API REST](device-portal-api-core.md) , который можно использовать непосредственно для доступа к данным и программного управления устройством.
...
2.  В раздел Диспетчер приложений портала устройств нажмите кнопку **Добавить** и выберите **установлен пакет приложения из локального хранилища**.
...
4.  Проверьте соответствующих полях, если вы хотите установить дополнительные или пакеты платформы, а также установки приложения. При наличии нескольких зависимостей добавляйте каждую по отдельности.     
5.  Нажмите кнопку **Далее** для перехода к на следующем шаге и **установите** для запуска установки. 

#### <a name="uninstall-an-app"></a>Удаление приложения
1.  Убедитесь, что приложение не работает. 
2.  Если это, перейдите на **Запуск приложений** и закройте его. Если вы попытаетесь удалить работающее приложение, это приведет к возникновению проблем при попытке переустановить приложение. 
...
На этой странице позволяет просматривать и обрабатывать файлы неопубликованных приложений. См. в разделе блога [с помощью проводника приложения](https://blogs.windows.com/buildingapps/2016/06/08/using-the-app-file-explorer-to-see-your-app-data/) для получения дополнительных сведений о проводнике и как его использовать. 
...
- **Графический Процессор**: процентов общее доступных GPU модуля использования
...
На странице трассировки производительности позволяет для представления трассировки [Windows Performance Recorder (WPR)](https://msdn.microsoft.com/library/hh448205.aspx) от устройства узла.
...
Страница диспетчера устройств перечисляет все периферийные устройства, подключенные к устройству. Вы можете щелкнуть значков параметры, чтобы просмотреть свойства каждого из них.
