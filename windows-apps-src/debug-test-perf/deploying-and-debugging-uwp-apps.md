...
В Visual Studio 2019 г. можно передать аргументы командной строки отладки при запуске отладки приложения UWP. Можно получить доступ к аргументам отладки командной строки из параметра *args* метода **OnLaunched** класса [**Application**](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.application). Чтобы указать аргументы отладки командной строки, откройте свойства проекта и перейдите к вкладке **Отладка**.
...
- **Не запускать, но отлаживать мой код при открытии / Launch приложение**
