...
# <a name="deploying-and-debugging-universal-windows-platform-uwp-apps"></a>Развертывание и отладка приложений универсальной платформы Windows (UWP)
...
-   **Эмулятор.** Позволяет загрузить и развернуть приложение на эмуляторе, конфигурация которого указана в его названии. Эмуляторы доступны только на компьютерах с Windows 8.1 или более старшей версии с включенным Hyper-V.
...
Дополнительные сведения см. на странице скачивания [инструментов удаленной отладки для Visual Studio](http://go.microsoft.com/fwlink/p/?LinkId=717039).
...
- **Универсальный (незашифрованный протокол)**. Используйте этот режим проверки подлинности при развертывании на удаленном устройстве (настольных компьютерах и ноутбуках) под управлением ОС, отличной от Windows. На данный момент он используется для устройств IoT, Xbox и HoloLens. Универсальный (незашифрованный протокол) следует использовать только в доверенных сетях. Подключение для отладки уязвимо для злонамеренных пользователей, которые могут перехватывать и изменить данные, передаваемые между компьютером разработчика и удаленным компьютером.
- **Windows**. Этот режим проверки подлинности предназначен только для развертывания на удаленных компьютерах (настольных компьютерах и ноутбуках). Используйте этот режим проверки подлинности при наличии доступа к учетным данным пользователя, от лица которого выполнен вход на целевом компьютере. Это наиболее безопасный режим удаленного развертывания.
- **Нет**. Этот режим проверки подлинности предназначен только для развертывания на удаленных компьютерах (настольных компьютерах и ноутбуках). Используйте этот режим проверки подлинности, если тестовый компьютер работает в среде, в которой вход выполнен под тестовой учетной записью, и если вы не можете ввести учетные данные. Убедитесь, что параметры удаленного отладчика позволяют отказаться от проверки подлинности.

## <a name="advanced-remote-deployment-options"></a>Расширенные параметры удаленного развертывания
С выпуском обновления 3 для Visual Studio 2015 и юбилейного обновления Windows 10 появились новые расширенные параметры удаленного развертывания для некоторых устройств с Windows 10. Эти расширенные параметры удаленного развертывания можно найти в меню **Отладка** со свойствами проекта.
...
* Установить обновление 3 для Visual Studio 2015 со средствами для Windows 10 версии 1.4.1 (которые включают пакет SDK юбилейного обновления Windows 10)
* Выбор удаленного устройства Xbox с юбилейным обновлением Windows 10 в качестве целевого
...
> [!NOTE]
> Функция **копирования файлов на устройство** на данный момент поддерживается на Xbox с юбилейным обновлением Windows 10.

На удаленном устройстве макет копируется в следующее расположение по умолчанию в зависимости от семейства устройств:
...
> [!NOTE]
> Функция **регистрации макета через сеть** на данный момент поддерживается на Xbox с юбилейным обновлением Windows 10.

На удаленном устройстве макет регистрируется в следующем расположении по умолчанию в зависимости от семейства устройств:
...
- **Разрешение на использование локального петлевого адреса**
  ...
  -   На странице свойств **Отладка** проектов C# и Visual Basic снимите флажок **Разрешить локальный петлевой адрес**.
