...
2. Выполните [жест запуска](https://docs.microsoft.com/hololens/hololens2-basic-usage#start-gesture) или жест [раскрытия](https://developer.microsoft.com/mixed-reality#Bloom) для HoloLens (1-го поколения), чтобы запустить главное меню.
3. Взгляните на плитку **Параметры** и выполните жест [касания](https://developer.microsoft.com/mixed-reality#Press_and_release) на hololens (1-й общий) или выберите его в hololens 2, [касаясь его или используя руки](https://docs.microsoft.com/hololens/hololens2-basic-usage). Приложение параметров будет запущено после его выбора.
...
2. Найдите IP-адрес вашего устройства. Найдите IP-адрес на устройстве в разделе **параметры > сеть & интернет > Wi-Fi > свойства оборудования**. Можно также спросить: «Привет, Кортана! Какой у меня IP-адрес»?
...
> [!IMPORTANT]
> Если компьютеру не удается найти устройство, попробуйте использовать реальный сетевой IP-адрес устройства HoloLens, а не `http://127.0.0.1:10080`.
...
- Раздел **Настройки** содержит следующие параметры:
    - **IPD**: задает межзрачковое расстояние, то есть расстояние в миллиметрах между центрами зрачков смотрящего прямо пользователя. Этот параметр незамедлительно вступает в силу. Значение по умолчанию было вычислено автоматически при настройке устройства. **Является допустимым только для HoloLens (только для 1-го поколения), то есть в Hololens 2.** 
