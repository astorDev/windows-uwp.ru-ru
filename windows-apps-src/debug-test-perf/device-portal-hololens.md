...
2. Выполните жест [цветение](https://developer.microsoft.com/mixed-reality#Bloom), чтобы запустить главное меню.
3. Посмотрите на плитку **Параметры** и выполните жест [воздушное касание](https://developer.microsoft.com/mixed-reality#Press_and_release). Выполните второе воздушное касание, чтобы разместить приложение в вашей среде. Приложение «Параметры» запустится после его размещения.
...
2. Поиск IP-адрес вашего устройства. Найти IP-адрес на устройстве под **параметры > сеть и Интернет > Wi-Fi > Свойства оборудования**. Можно также спросить: «Привет, Кортана! Какой у меня IP-адрес»?
...
> [!IMPORTANT]
> Если компьютер не сможет найти устройство, попробуйте воспользоваться реальной сети IP-адрес устройства, HoloLens, а не `http://127.0.0.1:10080`.
...
- Откройте портал устройств: Открывает в эмуляторе портал устройств Windows для HoloLens OS.
...
3. Установка сертификата в «доверенные корневые центры сертификации» хранить на вашем ПК. — в меню Windows, тип: Управление сертификатами компьютеров и запустить приложение.
...
- Раздел **Настройки** содержит следующие параметры:
    ...
    - **Имя устройства**: Присвоить имя HoloLens. Необходимо перезапустить устройство после изменения этого параметра. После нажатия «Сохранить» появится диалоговое окно с вопросом о немедленной перезагрузке устройства или отложенной перезагрузке.
...
- **Параметры отслеживания**: Включите непрерывного visual отслеживания, установив visual отслеживания Force. Пауза останавливает визуальное отслеживание.
- **Просмотр параметров**: Задание параметров на объемного вида: - отслеживания: Указывает, активен ли visual отслеживания.
...
- **Показать сетку**: Отображение сетки поверхности сопоставления, которые представляет обстановке.
...
- Параметры: Управления мультимедиа, которые регистрируются, установив следующие параметры:-голограммы: Захватывает holographic содержимое в поток видео. Голограммы обрабатываются в режиме моно, а не стерео.
...
- **Запись места**: Используется для загрузки файла имитации места, содержащего пространственное сопоставление сети для окружения пользователя. Назовите комнату, затем щелкните «Захват» для сохранения данных на ПК в виде XEF-файла. Этот файл комнаты может быть загружен в эмулятор HoloLens.
- **Запись**: Проверьте потоки для записи, имя записи и щелкните запись, чтобы начать запись. Выполняйте действия с помощью устройства HoloLens, затем щелкните «Стоп», чтобы сохранить данные на ПК в виде XEF-файла. Этот файл может быть загружен в эмулятор или на устройство HoloLens.
- **Воспроизведение**: Щелкните или коснитесь передачи, записи, чтобы выбрать файл xef с вашего компьютера и отправить данные в HoloLens.
- **Режим управления**: Выберите из раскрывающегося списка, по умолчанию или моделирования и щелкните или коснитесь кнопка "задать", чтобы выбрать режим на HoloLens. Выбор «Симуляторы» отключает датчики на устройстве HoloLens и использует вместо них переданные смоделированные данные. Если переключиться на «Симуляторы», устройство HoloLens не будет реагировать на пользователя, пока не переключитесь обратно на значение «По умолчанию».
