...
- *Конечный объект*. Версия пакета SDK, Visual Studio компилирует код приложения и запустите все средства от. Все API и ресурсы в этой версии SDK доступны в коде приложения во время компиляции.
- *Минимальные требования* Версия пакета SDK, поддерживающий версии операционной системы, которую приложение может выполняться на (и будет развертываться на хранилище) и версии, Visual Studio компилирует код разметки приложения от. 
...
> [!NOTE]
> Если у вас есть Предварительная версия Windows пакет средств разработки (SDK) установлен, еще не описана, сведения о поддержке контракта API можно найти в файле «Platform.xml», расположенный в папке установки пакета SDK в "\(Program Files (x86)) \ Windows Kits\10\Platforms\<платформы >\<SDK version > \Platform.xml ".
