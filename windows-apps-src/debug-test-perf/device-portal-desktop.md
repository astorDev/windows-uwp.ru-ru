...
### <a name="windowsupdatesearch-returns-invalid-number-of-updates-0x800f0950-cbseinvalidwindowsupdatecount"></a>WindowsUpdateSearch возвращает недопустимый количество обновлений (0x800f0950 CBS_E_INVALID_WINDOWS_UPDATE_COUNT)

Эта ошибка может возникнуть при попытке установить пакеты разработчика в предварительной сборке Windows 10. Эти пакеты компонентов по требованию (FoD) размещены в центре обновления Windows, и загружать их на предварительные сборки требуется осуществить фокус-тестирования. Если установки не разрешенный фокус-тестирования для сочетания круг и правый сборки, полезных данных загрузки не будет. Еще раз проверьте следующее:

1. Перейдите к **> параметры > обновления & безопасности программы предварительной оценки Windows** и убедитесь, что разделе **учетная запись программы предварительной оценки Windows** есть правильные сведения о вашем. Если вы не видите этот раздел, выберите **связать учетную запись программы предварительной оценки Windows**, добавление учетной записи электронной почты и убедитесь, что оно отображается под заголовком **учетная запись программы предварительной оценки Windows** (может потребоваться выбрать **связать учетную запись программы предварительной оценки Windows** вторично для фактически свяжите учетную запись только что добавленный).
 
2. В разделе **какой тип содержимого, вы хотите получать?**, убедитесь, что выбран **активной разработке Windows** .
 
3. В разделе **какие темпе вы хотите получать новые сборки?**, убедитесь, что выбран **Fast программы предварительной оценки Windows** .
 
4. Теперь можно будет установить fod-файлы. Если вы убедились, что вы на программы предварительной оценки Windows быстрое и по-прежнему не удается установить fod-файлы, см обратной связи и присоединить файлы журналов в разделе **C:\Windows\Logs\CBS**.

### <a name="sc-startservice-openservice-failed-1060-the-specified-service-does-not-exist-as-an-installed-service"></a>[SC] StartService: OpenService сбой 1060: указанной службы не существует как установленной службы

Эта ошибка может возникнуть, если устанавливаются пакеты разработчика. Без пакетов разработчика нет веб-служба управления не. Попробуйте снова установить пакеты разработчика.

### <a name="cbs-cannot-start-download-because-the-system-is-on-metered-network-cbsemeterednetwork"></a>CBS не удается запустить загрузки, поскольку система измеряемого сети (CBS_E_METERED_NETWORK)

Эта ошибка может возникнуть при наличии лимитное подключение к Интернету. Вы не сможете загрузить пакеты разработчика на лимитное подключение.
