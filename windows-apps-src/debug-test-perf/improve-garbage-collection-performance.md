...
**Примечание**  необходимости вмешаться в сборщик мусора по умолчанию строго признаком проблем с общей памятью с вашим приложением. Дополнительные сведения об этом см. в статье [Средство использования памяти при отладке в Visual Studio 2015](https://blogs.msdn.microsoft.com/devops/2014/11/13/memory-usage-tool-while-debugging-in-visual-studio-2015/). Эта статья касается только языков C# и Visual Basic.
...
Инициировать сборку мусора определенного поколения можно при помощи метода [**GC.Collect(n)** ](https://docs.microsoft.com/dotnet/api/system.gc.collect?redirectedfrom=MSDN#System_GC_Collect_System_Int32_), где n — номер поколения (0, 1 или 2).

**Примечание**  рекомендуется не выполнять принудительную сбор мусора в приложении, так как сборщик мусора использует много эвристику для определения наиболее подходящее время для выполнения сбора и принудительно коллекции является во многих случаях излишнего использования ЦП. Однако если вам известно, что в приложении есть множество неиспользуемых объектов, и вы хотите восстановить занимаемую ими память, то, возможно, стоит выполнить принудительную сборку мусора. Например, можно инициировать сборку мусора по окончании процедуры загрузки в игре, чтобы освободить память до начала игры.
 
Чтобы случайно не запустить слишком много процессов сборки мусора, можно задать для параметра [**GCCollectionMode**](https://docs.microsoft.com/dotnet/api/system.gccollectionmode?redirectedfrom=MSDN) значение **Optimized**. Это дает указание сборщику мусора начинать сборку, только если он определяет, что улучшение производительности будет оправданным.
