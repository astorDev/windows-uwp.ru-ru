...
Принимаемые вами решения о расположении элементов управления пользовательского интерфейса повлияют на количество создаваемых при запуске приложения элементов пользовательского интерфейса. Более подробные сведения об оптимизации макета см. в разделе [Оптимизация макета XAML](optimize-your-xaml-layout.md).
...
У панелей макета есть свойство [Background](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.panel.background), поэтому нет необходимости использовать перед ними класс [Rectangle](/uwp/api/Windows.UI.Xaml.Shapes.Rectangle), только чтобы изменить цвет панели.
...
Панели макета также имеют встроенные свойства границ, поэтому вокруг панели макета не нужно помещать элемент [Border](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.border). Дополнительные сведения и примеры доступны в разделе [Оптимизация макета XAML](optimize-your-xaml-layout.md).
...
Как правило, [словари ресурсов](../design/controls-and-patterns/resourcedictionary-and-xaml-resource-references.md) используются для сохранения на определенном глобальном уровне ресурсов, на которые вы хотите ссылаться в нескольких местах вашего приложения. Например, стили, кисти, шаблоны и т. д.
...
Платформа XAML пытается кэшировать часто используемые объекты, чтобы их можно было применять повторно как можно чаще. Но XAML не может определить, одинаковы ли две кисти, объявленные в разных частях разметки. В этом примере класс [SolidColorBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.SolidColorBrush) использован для демонстрации, но гораздо важнее применять этот способ к классу [GradientBrush](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.GradientBrush). Также проверьте кисти, использующие предопределенные цвета, например `"Orange"` и `"#FFFFA500"` — это один и тот же цвет.
...
Если следует выполнить проверку нажатия объекта класса [Grid](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Grid), присвойте ему прозрачный фон.
...
Другая причина перерисовки — использование формы, созданной из большого количества перекрывающихся элементов. Если вы присвоили [CacheMode](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.CacheMode) значение **BitmapCache** в классе [UIElement](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.UIElement), который содержит составную форму, платформа обработает элемент как растровое изображение, а затем будет использовать это изображение в каждом кадре вместо того, чтобы перерисовывать объекты.
...
Обратите внимание на использование класса [CacheMode](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.CacheMode). Не используйте этот прием, если какая-либо из подформ является анимированной, так как, вероятно, потребуется повторная генерация кэша в каждом кадре, что неэффективно.
...
- [Инструменты для профилирования и повышения производительности](tools-for-profiling-and-performance.md)
