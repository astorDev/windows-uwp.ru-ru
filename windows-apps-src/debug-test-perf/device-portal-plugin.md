...
Существует два события, которые должны обрабатываться приложением для завершения цикла обработки запросов: **Закрыто**, для каждый раз, когда служба портала устройств завершает работу, и [**RequestReceived**](https://docs.microsoft.com/en-us/uwp/api/windows.system.diagnostics.deviceportal.deviceportalconnectionrequestreceivedeventargs), который поверхности Входящие HTTP запросы и предоставляет основной функциональные возможности поставщика портала устройств. 
