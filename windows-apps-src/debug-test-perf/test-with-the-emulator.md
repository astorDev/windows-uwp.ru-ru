...
title: Тестирование с помощью эмулятора устройства с Windows 10 Mobile (Майкрософт)
...
ЦП (эти параметры, если доступны, можно включить в BIOS)

-   Виртуализация с аппаратной поддержкой ([Проверка совместимости](https://www.microsoft.com/download/details.aspx?id=592)).
...
Для Microsoft Emulator для Windows 10 Mobile требуется Visual Studio 2015 или более поздней версии. Он не имеет обратной совместимости с более ранними версиями Visual Studio.
...
Эмулятор Microsoft для Windows 10 Mobile поставляется в составе Windows 10 SDK. Windows 10 SDK и эмулятор можно установить как часть Visual Studio. См. [страницу скачивания Visual Studio](https://developer.microsoft.com/windows/downloads).
...
Чтобы запустить приложение в эмуляторе, просто выберите эмуляцию устройства в раскрывающемся меню **Запуск** в Visual Studio.
...
Аппаратные клавиши компьютера (сборка эмулятора 10.0.14295 и более ранние) | Аппаратные клавиши компьютера (сборка эмулятора 10.0.14332 и более новые) | Аппаратная кнопка Windows Phone | Заметки
--------------------- | ------------------------- | ----------------------------- | -----
...
> **Важно** .  при первом запуске средства касания вы получите запрос брандмауэра Windows. Вам НЕОБХОДИМО установить ВСЕ 3 флажка и разрешить средство в брандмауэре или же оно не будет запущено без какого-либо уведомления.
...
2.  -   Выберите тип ввода.
        ...
        **Запуск моделирования с помощью Live input**
    ...
    -   **Запуск моделирования с записанными входными данными**
...
| Ранний доступ        |                          |                  |Проход по маршруту на скорости, превышающей указанную предельную скорость. | 
...
**Дополнительные сведения о симуляторе расположения**
...
> **Обратите внимание** ,  поскольку SD-карта, используемая телефоном, форматируется с помощью файловой системы FAT32, максимальный размер — 32 ГБ.
...
2.  **Выберите приложение для получения уведомлений.**
...
6.  **При необходимости измените полезные данные уведомления.**
...
| Flash              | Нет                       | Да                       |
| Zoom               | 2x (цифровая или оптическая)  | 2x (цифровая или оптическая)   |
...
**Описание счетчиков частоты кадров**
...
В некоторых ситуациях, в том числе после обновления до нового рейса Windows 10, коммутатор виртуальной сети, связанный с эмулятором, может перейти в состояние, в котором его нельзя удалить с помощью пользовательского интерфейса.
...
**Обратите внимание** ,  эта команда удалит все сетевые устройства, а не только те, которые связаны с эмулятором. При следующем запуске компьютера все аппаратные сетевые устройства будут обнаружены автоматически.
 
...
> **Обратите внимание** ,  ксдеклеануп. exe удаляет все виртуальные машины Hyper-V, относящиеся к эмулятору, а также все контрольные точки виртуальных машин или сохраненные состояния.

### <a name="uninstall-windows-10-for-mobile-image"></a>Удаление образа Windows 10 Mobile

При установке эмулятора устанавливается VHD-образ Windows 10 Mobile, который добавляется в список **Программы и компоненты** панели управления. Если вы хотите удалить образ, найдите **Windows 10 для мобильного образа —\<версии\>** в списке установленных программ, щелкните ее правой кнопкой мыши и выберите **Удалить**.

В текущем выпуске после этого необходимо удалить VHD-файл для эмулятора вручную. Если вы установили эмулятор в путь по умолчанию, VHD-файл находится на диске C:\\Program Files (x86)\\комплектах Windows\\10\\Emulator\\Mobile\\\<версия\>\\Flash. VHD.
...
* [Запуск Windows Phone приложений в эмуляторе](https://docs.microsoft.com/visualstudio/debugger/run-windows-phone-apps-in-the-emulator?view=vs-2015)
