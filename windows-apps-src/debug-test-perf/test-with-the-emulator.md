...
ЦП (эти параметры, если он доступен, можно включить в BIOS)

-   Аппаратная поддержка виртуализации ([проверить совместимость](https://www.microsoft.com/download/details.aspx?id=592)).
...
Эмулятор Microsoft для Windows 10 Mobile требуется Visual Studio 2015 или более поздней версии; он не имеет обратной совместимости с предыдущими версиями Visual Studio.
...
Эмулятор Microsoft для Windows 10 Mobile поставляется в составе Windows 10 SDK. Windows 10 SDK и эмулятор можно установить как часть Visual Studio. См. [страницу скачивания Visual Studio](https://go.microsoft.com/fwlink/p/?LinkId=534785).
...
### <a name="uninstalling"></a>При удалении
...
Чтобы запустить приложение в эмуляторе, просто выберите эмулируемого устройства из **запуска** раскрывающееся меню в Visual Studio.
...
> **Важные**  при первом запуске средство tapper, вы получите брандмауэр Windows prompt. Вам НЕОБХОДИМО установить ВСЕ 3 флажка и разрешить средство в брандмауэре или же оно не будет запущено без какого-либо уведомления.
...
2.  -   Выберите тип ввода.
        ...
        **Для запуска моделирования с использованием live ввода**
    ...
    -   **Для запуска моделирования с использованием записи входных данных**
...
**Дополнительные сведения о симулятор расположения**
...
> **Примечание**  поскольку SD-карты, используемые на телефоне отформатирован в файловой системе FAT32, 32 ГБ — максимальный размер.
...
2.  **Выберите приложение, чтобы получать уведомления.**
...
**Описания счетчики частоты кадров**
...
**Включение и отключение счетчики частоты кадров**
...
В определенных ситуациях, включая после обновления до нового рейса Windows 10 виртуальный сетевой коммутатор, связанные с эмулятором можно перевести в состояние, где не может быть удален через пользовательский интерфейс.
...
**Примечание**  эта команда приведет к удалению всех сетевых устройствах, не только элементами, связанными с эмулятором. При следующем запуске компьютера все аппаратные сетевые устройства будут обнаружены автоматически.
 
...
> **Примечание**  XDECleanup.exe удаляет все эмулятор определенные виртуальные машины Hyper-V и она также удаляет все контрольные точки виртуальной Машины или сохраненных состояний.
...
В текущем выпуске после этого необходимо удалить VHD-файл для эмулятора вручную. Если вы установили эмулятора на путь по умолчанию, файл виртуального жесткого диска находится в C:\\Program Files (x86)\\комплекты Windows\\10\\эмуляции\\Mobile\\ <version> \\ Flash.VHD.
...
**Примечание.** После установки этого значения реестра, необходимо удалить виртуальную машину в диспетчере Hyper-V для конфигурации, которая запускается в Visual Studio и повторно запустите эмулятор с графикой, подготовке к просмотру программного обеспечения.
...
* [Запуск приложений Windows Phone в эмуляторе](https://msdn.microsoft.com/library/windows/apps/xaml/dn632391.aspx)
