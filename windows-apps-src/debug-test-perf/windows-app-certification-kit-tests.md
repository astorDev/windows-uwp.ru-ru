...
Для компонентов ОС (включая Trident, WWAHost и т. п.) накладывается внутреннее ограничение на длину путей файловой системы (не более MAX\_PATH), поэтому такие компоненты не будут работать в случае превышения длины.
...
> **Примечание.**  Эта статья адресована разработчикам приложений UWP для Windows 10. В случае разработки приложений для Windows 8.x или Windows Phone 8.x см. раздел [архивной документации](http://go.microsoft.com/fwlink/p/?linkid=619132).
