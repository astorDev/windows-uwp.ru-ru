...
> 
            **Примечание.**  Отладочная сборка приложения не пройдет этот тест, даже если в приложении используются только [API для приложений Магазина Windows](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx).

Проверьте сообщения об ошибках, чтобы выявить используемый в программе API, не относящийся к [API для приложений Магазина Windows](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx).
...
Для всех универсальных приложений Магазина Windows, использующих Direct3D, должна выполняться правильная прорисовка или корректная обработка сбоя на уровне компонентов9\-1.
...
> 
            **Примечание.**  Этот тест применяется только к приложениям Магазина Windows, разработанным для Windows8.1 и более поздних версий.
