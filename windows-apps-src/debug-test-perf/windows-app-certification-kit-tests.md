...
Комплект сертификации приложений для Windows содержит ряд тестов, позволяющих убедиться, что ваше приложение готово к публикации в Магазине Windows.
...
Использование сведений о версии операционной системы для Магазина Windows ограничено. Эти сведения часто неправильно использовались приложениями для проверки версии ОС и включения функций, соответствующих этой версии.
...
## Проверка совместимости с манифестом приложения
...
-   **Тест межпроцессного взаимодействия**
    ...
    Этот тест проверяет требования, по которым приложения Магазина Windows не могут передавать данные за пределы контейнера приложения компонентам рабочего стола. Межпроцессное взаимодействие предназначено только для параллельно загружаемых приложений. Приложения, в которых задан атрибут [**ActivatableClassAttribute**](https://msdn.microsoft.com/library/windows/apps/BR211414) с именем "DesktopApplicationPath", не пройдут этот тест.
...
Проверяет безопасность приложения, запуская средство [BinScope Binary Analyzer](#binscope).
...
Тесты [BinScope Binary Analyzer](http://go.microsoft.com/fwlink/p/?linkid=257276) проверяют способы программирования и сборки двоичных файлов приложения, снижающие его уязвимость перед атаками и возможность использования его в качестве вектора атаки.
...
**Сообщение об ошибке комплекта сертификации приложений для Windows:** не пройден тест APTCACheck.
...
### <span id="binscope-2"></span>Защита с использованием обработчика исключений /SafeSEH
...
### <span id="binscope-3"></span>Предотвращение выполнения данных

**Сообщение об ошибке комплекта сертификации приложений для Windows:** не пройден тест NXCheck.
...
### <span id="binscope-4"></span>Случайный выбор расположения адресного пространства
...
Этот тест выполняется только для приложений, написанных с использованием управляемого кода, например C# или .NET Framework.

### <span id="binscope-5"></span>Общий раздел PE чтения/записи
...
Для сертификации в Магазине Windows в приложениях необходимо использовать API для приложений Магазина Windows (API среды выполнения Windows или поддерживаемые API Win32). Этот тест также позволяет выявить ситуации, в которых управляемый двоичный файл использует зависимость от функции вне утвержденного профиля.

### Сведения о тесте

-   Позволяет убедиться, что ни один двоичный файл в пакете приложения не использует зависимость от API Win32, который не поддерживается для разработки приложений Магазина Windows. Для этого проверяется таблица адресов импорта для двоичного файла.
...
> **Примечание.**  Отладочная сборка приложения не пройдет этот тест, даже если в приложении используются только [API для приложений Магазина Windows](https://msdn.microsoft.com/en-us/library/windows/apps/xaml/bg124285.aspx).

Проверьте сообщения об ошибках, чтобы выявить используемый в программе API, не относящийся к [API для приложений Магазина Windows](https://msdn.microsoft.com/en-us/library/windows/apps/xaml/bg124285.aspx).

> **Примечание.**  Приложения на C++ в конфигурации отладки не пройдут этот тест, даже если в конфигурации используются только API из пакета Windows SDK для приложений Магазина Windows. Дополнительные сведения об этом см. в статье [Альтернативы API Windows в приложениях Магазина Windows](http://go.microsoft.com/fwlink/p/?LinkID=244022).
...
<p>Дополнительные сведения см. в разделах <a href="https://msdn.microsoft.com/en-us/library/windows/apps/xaml/dn958435.aspx">Гибкое оформление 101 для приложений UWP</a> и <a href="https://msdn.microsoft.com/en-us/library/windows/apps/xaml/hh465241.aspx">Руководство по ресурсам приложений</a>.</p>
</td></tr>
<tr><td>
...
<p>MakePRI.exe поддерживает параметр с названием <strong>AutoMerge</strong>. Значение <strong>AutoMerge</strong> по умолчанию — <strong>off</strong>. Когда параметр <strong>AutoMerge</strong> включен, ресурсы языкового пакета приложения объединяются в единый файл resources.pri во время выполнения. Мы не рекомендуем использовать этот вариант в приложениях, предназначенных для распространения через Магазин Windows. Файл resources.pri приложения, распространяемого через Магазин Windows, должен находиться в корневом каталоге пакета приложения и содержать все настройки языка, поддерживаемые приложением.</p>
</td></tr>
<tr><td>
<p>Строка {string} не удовлетворяет ограничению по длине в {number} символов.</p>
</td><td>
<p>См. раздел <a href="https://msdn.microsoft.com/en-us/library/windows/apps/xaml/mt148525.aspx">Требования к пакетам приложения</a>.</p>
...
<p>Дополнительные сведения см. в разделе <a href="https://msdn.microsoft.com/en-us/library/windows/apps/xaml/mt148525.aspx">Требования к пакетам приложения</a>.</p>
</td></tr>
<tr><td>
<p>В файле resources.pri не указываются ресурсы по умолчанию.</p>
</td><td>
<p>Дополнительные сведения см. в разделе <a href="https://msdn.microsoft.com/en-us/library/windows/apps/xaml/hh465241.aspx">Рекомендации по ресурсам приложений</a>.</p>
...
Приложения Магазина Windows должны быть законченными и полностью работоспособными. Приложения, использующие изображения по умолчанию (из шаблонов или примеров SDK), непривлекательны для пользователей, и их сложно найти в каталоге Магазина.
...
Чтобы пройти сертификацию для Магазина Windows, программы не должны быть скомпилированы для отладки и не должны ссылаться на отладочные версии исполняемого файла. Для прохождения этого теста вам также необходимо создать для своего приложения собственный оптимизированный код.
...
-   Выполните сборку рабочего выпуска приложения, прежде чем отправлять его в Магазин Windows.
...
Для всех универсальных приложений Магазина Windows, использующих Direct3D, должна выполняться правильная прорисовка или корректная обработка сбоя на уровне компонентов 9\-1.
...
Проверьте правильность прорисовки приложения на уровне компонентов 9\-1 Direct3D, даже если вы планируете использовать его при более высоком уровне компонентов. Дополнительные сведения см. в статье [Разработка для различных уровней компонентов Direct3D](http://go.microsoft.com/fwlink/p/?LinkID=253575).

### Вызов API Trim Direct3D после приостановки

> **Примечание.**  Этот тест применяется только к приложениям Магазина Windows, разработанным для Windows 8.1 и более поздних версий.
...
Приложения, устанавливающие смешанные двоичные файлы, могут аварийно завершить работу или работать неправильно в зависимости от архитектуры процессора пользователя.
...
Тест фоновых задач WinJS позволяет убедиться, что приложения на JavaScript имеют надлежащие операторы Close и, следовательно, не расходуют энергию батареи.
...
> **Примечание.**  Эта статья адресована разработчикам приложений UWP для Windows 10. При разработке приложений для Windows 8.x или Windows Phone 8.x см. раздел [archived documentation](http://go.microsoft.com/fwlink/p/?linkid=619132).
