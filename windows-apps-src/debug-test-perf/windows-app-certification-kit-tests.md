...
> **Примечание**отладочная сборка приложения не пройдет этот тест, даже если приложение использует только [API для приложений UWP](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx).
...
> **Примечание**приложений на C++, которые встроены в конфигурации отладки не пройдет этот тест, даже если в конфигурации используются только API из пакета Windows SDK для приложений UWP. См. в разделе, [альтернативы API Windows в приложениях UWP](http://go.microsoft.com/fwlink/p/?LinkID=244022) Дополнительные сведения.
...
Приложения UWP должны быть законченными и полностью работоспособными. Приложения, использующие изображения по умолчанию (из шаблонов или примеров SDK), непривлекательны для пользователей, и их сложно найти в каталоге Магазина.
...
Microsoft Store требует всех приложений, использующих Direct3D правильная прорисовка или компонентов уровня 9\-1 корректная обработка сбоя на.
