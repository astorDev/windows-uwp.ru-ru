...
description: Комплект сертификации приложений для Windows содержит ряд тестов, которые могут помочь гарантировать, что приложение будет готово для публикации в Microsoft Store.
ms.date: 02/08/2017
ms.topic: article
...
[Комплект сертификации приложений для Windows](windows-app-certification-kit.md) содержит ряд тестов, которые помогают обеспечить приложение будет готово для публикации в Microsoft Store. Тесты перечислены ниже с условиями сведения и рекомендуемые действия в случае сбоя.
...
Приложения не должны список библиотек DLL, загружаемых в HKEY\-ЛОКАЛЬНОГО\-МАШИНЫ\\программного обеспечения\\Microsoft\\Windows NT\\CurrentVersion\\Windows\\инициализации приложений \-Раздел реестра для библиотек DLL.
...
Сведения о версии операционной системы ограниченное использование для Microsoft Store. Эти сведения часто неправильно использовались приложениями для проверки версии ОС и включения функций, соответствующих этой версии.
...
-   [Рандомизация компоновки адресного пространства](#binscope-4)
-   [Чтение и запись общий раздел PE-файла](#binscope-5)
...
**Комплект сертификации приложений для Windows сообщение об ошибке:** APTCACheck тест не пройден
...
**Комплект сертификации приложений для Windows сообщение об ошибке:** SafeSEHCheck тест не пройден
...
**Комплект сертификации приложений для Windows сообщение об ошибке:** NXCheck тест не пройден
...
### <a name="span-idbinscope-4spanaddress-space-layout-randomization"></a><span id="binscope-4"></span>Рандомизация компоновки адресного пространства

**Комплект сертификации приложений для Windows сообщение об ошибке:** DBCheck тест не пройден
...
### <a name="span-idbinscope-5spanreadwrite-shared-pe-section"></a><span id="binscope-5"></span>Чтение и запись общий раздел PE-файла
...
**Комплект сертификации приложений для Windows сообщение об ошибке:** AppContainerCheck тест не пройден.
...
Если управляемый исполняемый файл не прошли проверку, убедитесь, что использовать последнюю версию компилятора и компоновщика, такие как Microsoft Visual Studio, создать приложение универсальной платформы Windows.
...
**Комплект сертификации приложений для Windows сообщение об ошибке:** ExecutableImportsCheck тест не пройден.
...
**Комплект сертификации приложений для Windows сообщение об ошибке:** WXCheck тест не пройден.

Этот тест помогает убедиться, что двоичный файл не содержит страниц, которые сопоставлены как записываемые и исполняемые. Это может произойти, если двоичный файл содержит раздел для записи и исполняемый файл или двоичный файл *SectionAlignment* — меньше, чем *страницы\-размер*.

**Что делать, если приложение не пройдет проверку**

Убедитесь, что двоичный файл не имеет раздел для записи или могут быть выполнены и что двоичного файла *SectionAlignment* имеет по крайней мере значение его *страницы\-размер*.
...
*СТРАНИЦА\-размер* используется по умолчанию *SectionAlignment* для исполняемых файлов.
...
Приложения должны использовать API-интерфейсы для приложений универсальной платформы Windows (среда выполнения Windows или поддерживаемые API Win32), быть сертифицированным для Microsoft Store. Этот тест также позволяет выявить ситуации, в которых управляемый двоичный файл использует зависимость от функции вне утвержденного профиля.

### <a name="test-details"></a>Сведения о тесте

-   Проверяет, что каждый двоичный файл в пакет приложения не зависят от API Win32, который не поддерживается для разработки приложений UWP, проверяя таблицу адресов импорта двоичного файла.
...
<tr><td>
<p>Файл изображения {filename} должен быть меньше, чем 204800 байт.\*\*</p>
</td><td>
...
<tr><td colspan="2">
<p>\*\* Указывает, что тест был добавлен в 3,3 комплект сертификации приложений Windows для Windows 8.1 и применяется, только если используется этот пакет или более поздней версии.</p>
</td></tr>
</table>
...
Приложения универсальной платформы Windows должны быть завершено и полностью функционально. Приложения, использующие изображения по умолчанию (из шаблонов или примеров SDK), непривлекательны для пользователей, и их сложно найти в каталоге Магазина.
...
Microsoft Store требуется уровень 9 признаков всех приложений, использующих Direct3D для отображаться должным образом или вызвать сбой корректно на\-1 графические платы.

Так как пользователи могут изменять графического оборудования в устройстве после его установки, если выбран минимальный уровень компонента выше, чем 9\-1, приложение должно определять при запуске ли текущее оборудование соответствует минимальным требованиям. Если минимальные требования не соблюдаются, приложение должно предоставлять пользователю подробные сведения о требованиях к Direct3D. Кроме того, если приложение загружено на устройство, не соответствующее требованиям, этот факт должен быть выявлен приложением на этапе запуска, при этом должно отобразиться сообщение для клиента с информацией о требованиях.

### <a name="test-details"></a>Сведения о тесте

Тест пройдет проверку, если приложения визуализируют точно на функциональном уровне 9\-1.

### <a name="corrective-action"></a>Корректирующее действие

Убедитесь, что приложение правильно отображается на уровне компонентов Direct3D 9\-1, даже в том случае, если ожидается, что его для запуска на более высоком уровне компонента. Дополнительные сведения см. в статье [Разработка для различных уровней компонентов Direct3D](https://go.microsoft.com/fwlink/p/?LinkID=253575).

### <a name="direct3d-trim-after-suspend"></a>Вызов API Trim Direct3D после приостановки

> **Примечание**  этот тест применим только для приложений универсальной платформы Windows, разработанных для Windows 8.1 и более поздних версий.
...
-   **ExclusiveTo проверки атрибута:** Убедитесь, что классы UWP не реализовывать интерфейсы, которые помечены как ExclusiveTo другого класса.
-   **Проверка расположения типа:** Убедитесь, что метаданные для всех типов для универсальной платформы Windows находится в файле winmd, который имеет имя самой длинной сопоставления пространства имен в пакете приложения.
-   **Проверка учета регистра имя типа:** Убедитесь, что в пакете приложения все типы UWP имеют уникальные имена без учета регистра. Также убедитесь, что ни одно имя типа UWP не используется одновременно в качестве имени пространства имен в пакете приложения.
-   **Проверка правильности имени типа:** Убедитесь, что в глобальном пространстве имен или в пространстве имен Windows верхнего уровня отсутствуют типы UWP.
-   **Проверка правильности общие метаданных:** Убедитесь, что компилятор, который используется для создания типов в актуальном состоянии с характеристиками универсальной платформы Windows.
...
Проверяет, что приложения не создаете подкаталогов в процессе установки, превышает МАКСИМУМ\-путь.

### <a name="background"></a>Фон

Компоненты операционной системы (включая Trident, WWAHost и т. д.) ограничены внутренне MAX\-путь для файла путей к системным и не будет работать для длинных путей.

### <a name="test-details"></a>Сведения о тесте

Проверяет, что путь в каталоге установки приложения не превышает МАКСИМАЛЬНЫЙ\-путь.
