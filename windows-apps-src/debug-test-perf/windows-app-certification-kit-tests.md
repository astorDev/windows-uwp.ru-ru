...
description: Комплект сертификации приложений для Windows содержит ряд тестов, которые могут помочь убедитесь, что ваше приложение готово для публикации в магазине Майкрософт.
ms.author: pafarley
ms.date: 02/08/2017
ms.topic: article
ms.prod: windows
...
ms.locfileid: "2802045"
---
# <a name="windows-app-certification-kit-tests"></a>Тесты комплекта сертификации приложений для Windows


[Комплект сертификации приложений для Windows](windows-app-certification-kit.md) содержит ряд тестов, гарантировать, что ваше приложение готово к опубликовать в магазине Майкрософт. Тесты перечислены ниже с их условия, сведения и рекомендуемые действия в случае сбоя.
...
Сведений о версии операционной системы с ограниченными об использовании для хранения Майкрософт. Эти сведения часто неправильно использовались приложениями для проверки версии ОС и включения функций, соответствующих этой версии.
...
В случае сбоя проверки управляемого исполняемого файла убедитесь в том, используемые последние компилятора и компоновщика, таких как Microsoft Visual Studio для создания приложения UWP.
...
Приложения необходимо использовать API-интерфейсы для приложения UWP (среды выполнения Windows или поддерживаемые интерфейсы API Win32), чтобы иметь разрешения на хранилище Microsoft. Этот тест также позволяет выявить ситуации, в которых управляемый двоичный файл использует зависимость от функции вне утвержденного профиля.

### <a name="test-details"></a>Сведения о тесте

-   Проверяет, что каждый двоичный файл в пакете приложения не будет иметь зависимости на Win32 API, который не поддерживается для разработки приложений UWP, проверив таблицы адресов импорта двоичного файла.
...
> **Примечание**  Построение отладки приложения завершится ошибкой этот тест, даже в том случае, если приложение использует только [API-интерфейсы для UWP приложений](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx).

Просмотрите сообщения об ошибках для идентификации API использует приложения, которые не интерфейса [API для UWP приложений](https://msdn.microsoft.com/library/windows/apps/xaml/bg124285.aspx).

> **Примечание**  Приложения C++, встроенных в конфигурации отладки завершится ошибкой этот тест, даже в том случае, если конфигурацию только используются интерфейсы API из пакета SDK Windows для UWP приложений. Отображается, [альтернативы Windows API -интерфейсы в приложениях UWP](http://go.microsoft.com/fwlink/p/?LinkID=244022) для получения дополнительных сведений.
...
<p>MakePRI.exe поддерживает параметр с названием <strong>AutoMerge</strong>. Значение <strong>AutoMerge</strong> по умолчанию — <strong>off</strong>. Когда параметр <strong>AutoMerge</strong> включен, ресурсы языкового пакета приложения объединяются в единый файл resources.pri во время выполнения. Мы не рекомендуем для приложений, которые вы планируете распространение через хранилища Майкрософт. Resources.pri приложения, которое распространяется через Microsoft Store должен быть в корне пакета приложения и содержать все ссылки языка, поддерживаемых этим приложением.</p>
</td></tr>
<tr><td>
...
UWP приложений должны быть законченной и полностью функциональным. Приложения, использующие изображения по умолчанию (из шаблонов или примеров SDK), непривлекательны для пользователей, и их сложно найти в каталоге Магазина.
...
Сертификации Майкрософт хранилища, приложения должны компилироваться не для отладки и они не должны ссылаться отладочные версии исполняемого файла. Для прохождения этого теста вам также необходимо создать для своего приложения собственный оптимизированный код.
...
-   Построение приложения как выпуска перед отправкой хранилища Майкрософт.
...
Microsoft Store требуется все приложения, использующие Direct3D обработку должным образом или корректного сбоя на компонент уровня 9\ 1 графические карты.
...
> **Примечание**  Этот тест применяется только к UWP приложений, разработанных для Windows 8.1 и более поздних версий.
