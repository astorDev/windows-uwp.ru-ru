...
Приложения не должны перечислять библиотеки DLL для загрузки в раздел реестра HKEY @ no__t-0LOCAL @ no__t-1MACHINE @ no__t-2Software @ no__t-3Microsoft @ no__t-4Windows NT @ no__t-5CurrentVersion @ no__t-6Windows.
...
1.  Откройте файл eventvwr. exe и перейдите к папке Application и Services log @ no__t-0Microsoft @ no__t-1Windows @ no__t-2Immersive-Shell.
2.  Фильтрация представления для отображения идентификаторов событий: 5900-6000.
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Апткачекк
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Сафесехчекк
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Нксчекк
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Дбчекк
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Шаредсектионсчекк.
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Аппконтаинерчекк.
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Ексекутаблеимпортсчекк.
...
**Сообщение об ошибке набора сертификации приложений Windows:** Сбой теста Вксчекк.

Этот тест помогает убедиться, что двоичный файл не содержит страниц, которые сопоставлены как записываемые и исполняемые. Это может произойти, если двоичный файл содержит доступный для записи и исполняемый раздел или если *выравнивание раздела* двоичного файла меньше, чем *Page @ no__t-2SIZE*.

**Что делать, если приложение не проходит проверку**

Убедитесь, что двоичный файл не имеет подраздела для записи или исполняемого файла и что значение *выравнивание раздела* двоичного файла по крайней мере равно его *странице @ no__t-2SIZE*.
...
*Page @ no__t-1SIZE* — это *выравнивание раздела* по умолчанию для исполняемых файлов.
...
> **Примечание**  The отладочная сборка приложения не будет выполнять этот тест, даже если приложение использует только [API для приложений UWP](https://docs.microsoft.com/uwp/).
...
<tr><td>
<p>Размер файла образа {filename} не должен превышать 204800 байт. \* @ no__t-1</p>
</td><td>
<p>Уменьшите размер указанных изображений.</p>
</td></tr>
<tr><td>
<p>Файл {filename} не должен содержать раздел с обратным отображением. \* @ no__t-1</p>
</td><td>
...
<tr><td colspan="2">
<p>\* @ no__t-1 указывает, что тест был добавлен в набор сертификации приложений Windows 3,3 для Windows 8.1 и применим только при использовании этой версии пакета или более поздней.</p>
</td></tr>
</table>
...
Microsoft Store требует, чтобы все приложения, использующие Direct3D, отображались должным образом или правильно завершаться неправильно на графических картах уровня функции 9 @ no__t-01.

Поскольку пользователи могут изменить графическое оборудование на устройстве после установки приложения, при выборе минимального уровня функций выше 9 @ no__t-01 приложение должно определить, соответствует ли текущее оборудование минимальным требованиям. Если минимальные требования не соблюдаются, приложение должно предоставлять пользователю подробные сведения о требованиях к Direct3D. Кроме того, если приложение загружено на устройство, не соответствующее требованиям, этот факт должен быть выявлен приложением на этапе запуска, при этом должно отобразиться сообщение для клиента с информацией о требованиях.

### <a name="test-details"></a>Сведения о тесте

Тест проверит, правильно ли отображаются приложения на уровне функций 9 @ no__t-01.

### <a name="corrective-action"></a>Корректирующее действие

Убедитесь, что приложение правильно отображается на уровне функций Direct3D 9 @ no__t-01, даже если предполагается, что он будет работать на более высоком уровне функций. Дополнительные сведения см. в статье [Разработка для различных уровней компонентов Direct3D](https://go.microsoft.com/fwlink/p/?LinkID=253575).

### <a name="direct3d-trim-after-suspend"></a>Вызов API Trim Direct3D после приостановки

> **Примечание**@no__t — тест 1This применяется только к приложениям UWP, разработанным для Windows 8.1 и более поздних версий.
...
-   **Проверка атрибута Ексклусивето:** Убедитесь, что классы UWP не реализуют интерфейсы, помеченные как Ексклусивето другим классом.
-   **Проверка расположения типов:** Убедитесь, что метаданные для всех типов UWP находятся в файле WinMD с самым длинным именем, соответствующим пространству имен в пакете приложения.
-   **Проверка чувствительности к регистру имени типа:** Убедитесь, что в пакете приложения все типы UWP имеют уникальные имена без учета регистра. Также убедитесь, что ни одно имя типа UWP не используется одновременно в качестве имени пространства имен в пакете приложения.
...
-   **Проверка правильности метаданных:** Убедитесь, что компилятор, который вы используете для создания типов, обновлен с использованием спецификаций UWP.
...
Проверяет, что приложения не создаются подкаталоги в процессе установки, длина которых превышает максимальную @ no__t-0PATH.

### <a name="background"></a>Фон

Компоненты ОС (включая Trident, WWAHost и т. д.) внутренне ограничиваются МАКСИМУМом @ no__t-0PATH для путей файловой системы и не будут правильно работать для более длинных путей.

### <a name="test-details"></a>Сведения о тесте

Проверяет, превышает ли путь в каталоге установки приложения значение MAX @ no__t-0PATH.
