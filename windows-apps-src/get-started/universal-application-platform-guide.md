...
## <a name="how-the-universal-windows-platform-relates-to-windows-runtime-apis"></a>Как универсальные платформы Windows относится к API среды выполнения Windows
При создании приложения универсальные платформы Windows (UWP), можно получить много расстояния и удобства из него обработке термины «универсальные Windows платформы (UWP)» и «Среды выполнения Windows (WinRT)» в качестве тем или иным синонимами. Но он *является* возможность поиска в разделе Описание технологии и определение только что разница между их. Если вы хотите узнать, который, в этом последнем разделе — для вас.

Среды выполнения Windows и интерфейсы API WinRT — это эволюция API-интерфейсов Windows. Изначально Windows был запрограммирован с помощью плоский API Win32 В стиле. Теми были добавлены COM API-интерфейсы (которые наиболее важные пример[DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) ). Windows Forms, WPF, .NET и управляемые языки подключить собственные способом написания приложения для Windows и собственные вида технологии API. Среда выполнения Windows — на внутреннем следующий этап COM. На уровне приложения двоичный интерфейс (ABI) его корни COM становятся видимыми. Однако среды выполнения Windows может быть вызван из замечательных диапазон разные языки программирования. И вызываемую, чтобы естественным для каждого из этих языков. С этой целью доступа для среды выполнения Windows доступны через что называются проекции языка. Существует проекции языка среды выполнения Windows в C#, в Visual Basic, в стандартный C++, JavaScript и т. д. Кроме того, один раз упакованных надлежащим образом (смотрите [Bridge рабочий стол](/windows/uwp/porting/desktop-to-uwp-root)), WinRT API-интерфейсы можно вызывать из приложений, созданных в одной из замечательных диапазон модели приложений: Win32, .NET, WinForms и WPF.

И, конечно, может вызывать интерфейсы API WinRT из вашего приложения UWP. UWP — это модель приложений, созданных на основе среды выполнения Windows. Технически UWP модели приложения на основе [CoreApplication](/uwp/api/windows.applicationmodel.core.coreapplication), несмотря на то, что с подробным описанием могут быть скрыты от вас, в зависимости от выбранного языка программирования. Как описано в этом разделе, с значение предложение точка зрения, UWP используется для записи одного двоичного файла, можно следует выбрать, опубликовать в магазине Майкрософт и запустить на любом из замечательных диапазона устройств. Reach устройства UWP приложения зависит от подмножество интерфейсов API UWP ограничить ваше приложение для вызова или условно вызове.

Надеюсь в этом разделе было выполнено успешно при описании различие между базовым API среды выполнения Windows, а механизм и экономическую выгоду универсальные платформы Windows технологии.
