...
ms.locfileid: "6969519"
---
# <a name="whats-a-universal-windows-platform-uwp-app"></a>Что такое приложение универсальной платформы Windows (UWP)?

![Приложения универсальной платформы Windows работать на различных устройствах, поддерживают адаптивный пользовательский интерфейс, естественный пользовательский ввод, один магазин, центр партнеров и облачные службы](images/universalapps-overview.png)
...
Если вы разрабатываете приложение универсальной платформы Windows (UWP), вы можете получить много расстояния и удобства из рассмотрения термины «универсальной платформы Windows (UWP)» и «Среды выполнения Windows (WinRT)» в качестве более или менее синонимами. Но он *является* возможность выглядеть рассматривается технологии и определить только разницу между эти идеи. Если вам интересно, этого последнего раздела документ для вас.

Среда выполнения Windows и API-интерфейсов WinRT, являются развитием API-интерфейсы Windows. Изначально Windows был запрограммирован через плоский, API-интерфейсы Win32 заканчивающуюся символом. Для тех, были добавлены API модели COM (что четкой примере[DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) ). Windows Forms, WPF, .NET и управляемых языках выведено собственный способ написания приложений для Windows и собственные вида технологии API. Среда выполнения Windows является рассматривается, на следующем этапе работы COM. На уровне двоичного интерфейса (ABI) фактическое приложение его корней в модели COM становятся видимыми. Но среды выполнения Windows была разработана для вызываться из отличный диапазона разных языках программирования. И библиотекой способом, который является очень естественным для каждого из этих языков. С этой целью доступ к среде выполнения Windows становится доступным через новые, известны как языковые проекции. Существует проекция языка среды выполнения Windows в C#, в Visual Basic, в стандартной версии C++, JavaScript и т. д. Кроме того, один раз упакованных соответствующим образом (см. в разделе [Моста для классических приложений](/windows/uwp/porting/desktop-to-uwp-root)), могут вызывать API WinRT из приложения, созданного в одном из отличный диапазон модели приложений: Win32, .NET, WinForms и WPF.

И, конечно, вы можете вызывать API-интерфейсов WinRT из приложения UWP. UWP — это модель приложений, создана на основе среды выполнения Windows. С технической точки зрения модели приложения UWP основана на [CoreApplication](/uwp/api/windows.applicationmodel.core.coreapplication), несмотря на то, что с подробным описанием могут быть скрыты от вас, в зависимости от выбранного вами языка программирования. Как описано в этом разделе, с значение предложение точки зрения, UWP пригоден для написания одного двоичного файла, который можно Если вы решите, опубликованных в Microsoft Store и запустить на любой из отличный различных форм-факторов. Устройство аудиторию вашего приложения UWP, зависит от подмножество API-интерфейсы UWP ограничить приложение для вызова, или что вызвать условно.

Следует надеяться, что в этом разделе, была успешной при описании разницу между технологии базовый API среды выполнения Windows и механизм и значение универсальной платформы Windows для бизнеса.
