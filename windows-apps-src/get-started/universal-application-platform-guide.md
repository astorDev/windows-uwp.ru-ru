...
ms.locfileid: "8737668"
---
# <a name="whats-a-universal-windows-platform-uwp-app"></a>Что такое приложение универсальной платформы Windows (UWP)?

![Приложения универсальной платформы Windows работать на различных устройствах, поддерживают адаптивный пользовательский интерфейс, естественный пользовательский ввод, один магазин, центра партнеров и облачные службы](images/universalapps-overview.png)
...
Windows10 представляет универсальной платформы Windows (UWP), которая предоставляет общую платформу приложений на любом устройстве под управлением Windows10. Основные API UWP являются одинаковыми на всех устройствах с Windows. Если ваше приложение использует только основные API, оно будет работать на любом устройстве Windows10 независимо от того, является ли вы настольных ПК, Xbox, гарнитуру смешанной реальности и т. д.

Приложение UWP, написанное на C++ /WinRT или C++ /CX, имеет доступ к API-интерфейсам Win32, которые входят в состав UWP. Эти API-интерфейсы Win32 реализуются всеми устройствами Windows10.
...
Благодаря единому магазину делает приложение доступным в Windows10 устройства, такие как ПК, планшет, Xbox, HoloLens, Surface Hub и устройства Интернета вещей (IoT). Вы можете отправить свое приложение в магазин и сделать его доступным для всех типов устройств или только некоторых из них. Отправка всех ваших приложений для устройств с Windows и управление ими осуществляются централизованно. У вас есть классическое приложение на C++, которое вы хотите модернизировать за счет возможностей UWP и продавать в Microsoft Store? Пожалуйста!
...
### <a name="submit-your-app-to-the-store"></a>Отправьте приложение в магазин

[Центр партнеров](https://partner.microsoft.com/dashboard) позволяет управлять приложениями и отправлять все ваши приложения для устройств с Windows в одном месте. См. в разделе [Windows публикации приложений и игр,](../publish/index.md) чтобы узнать, как отправлять приложения в Microsoft Store для публикации.
...
Если вы разрабатываете приложение универсальной платформы Windows (UWP), вы можете получить много расстояния и удобство из рассмотрения термины «универсальной платформы Windows (UWP)» и «Среды выполнения Windows (WinRT)» в качестве более или менее синонимами. Но он *является* возможность выглядеть рассматривается технологии и определите только разницу между этими идеями. Если вам интересно, этого последнего раздела документ для вас.

Среда выполнения Windows и API-интерфейсов WinRT, являются развитием API-интерфейсы Windows. Изначально Windows был запрограммирован через плоский API Win32 В стиле. Для тех, были добавлены API модели COM (что четкой примере[DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) ). Windows Forms, WPF, .NET и управляемых языках выведено собственный способ написания приложений для Windows и собственные вида технологии API. Среда выполнения Windows является за кулисами следующий этап COM. На уровне двоичного интерфейса (ABI) фактического приложений его корней в модели COM становятся видимыми. Однако среда выполнения Windows была разработана для вызываться из отличный диапазона разных языках программирования. И вызываемые способом, который является очень естественным для каждого из этих языков. Для этого нужно доступ к среде выполнения Windows становятся доступными через то, что известно как языковые проекции. Существует проекция языка среды выполнения Windows в C#, в Visual Basic, в стандартной версии C++, JavaScript и т. д. Кроме того, один раз упакованных соответствующим образом (см. в разделе [Моста для классических приложений](/windows/uwp/porting/desktop-to-uwp-root)), могут вызывать API WinRT из приложения, созданного в одном из отличный диапазон модели приложений: Win32, .NET, WinForms и WPF.

И, конечно, могут вызывать API WinRT из приложения UWP. UWP — это модель приложений, создана на основе среды выполнения Windows. С технической точки зрения модели приложения UWP основана на [CoreApplication](/uwp/api/windows.applicationmodel.core.coreapplication), несмотря на то, что с подробным описанием могут быть скрыты от пользователя, в зависимости от выбранного вами языка программирования. Как описано в этом разделе, с значение предложение точки зрения, UWP пригоден для написания одного двоичного файла, который можно Если вы решите, опубликованных в Microsoft Store и запустить на любой из отличный различных форм-факторов. Устройство аудиторию вашего приложения UWP зависит от подмножество API-интерфейсы UWP ограничить приложение для вызова или что вызвать определенных условиях.

Следует надеяться, что в этом разделе было выполнено успешно при описании разницу между технологии базовый API среды выполнения Windows и механизм и значение универсальной платформы Windows для бизнеса.
