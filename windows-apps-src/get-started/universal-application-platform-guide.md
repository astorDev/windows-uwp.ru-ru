...
Благодаря единому магазину делает приложение доступным в Windows10 устройства, такие как ПК, планшет, Xbox, HoloLens, Surface Hub и устройств Интернета вещей (IoT). Вы можете отправить свое приложение в магазин и сделать его доступным для всех типов устройств или только некоторых из них. Отправка всех ваших приложений для устройств с Windows и управление ими осуществляются централизованно. У вас есть классическое приложение на C++, которое вы хотите модернизировать за счет возможностей UWP и продавать в Microsoft Store? Пожалуйста!
...
- [Компиляция приложения с помощью .NET Native](https://docs.microsoft.com/dotnet/framework/net-native/)
...
Если вы разрабатываете приложение универсальной платформы Windows (UWP), вы можете получить много расстояния и удобство из рассмотрения термины «универсальной платформы Windows (UWP)» и «Среды выполнения Windows (WinRT)» в качестве большую или меньшую синонимами. Но он *является* возможность поиска рассматривается технологии и определите только разницу между этими идеи. Если вам интересно, этого последнего раздела документ для вас.

Среда выполнения Windows и API-интерфейсов WinRT, являются развитием API-интерфейсы Windows. Изначально Windows был запрограммирован через плоский, API-интерфейсы Win32 заканчивающуюся символом. Для тех, были добавлены API модели COM (что четкой пример[DirectX](https://msdn.microsoft.com/library/windows/desktop/ee663274) ). Windows Forms, WPF, .NET и управляемых языках выведено собственный способ написания приложений для Windows и собственные вида технологии API. Среда выполнения Windows является рассматривается, на следующем этапе работы COM. На уровне приложения двоичный интерфейс (ABI) его корней в модели COM становятся видимыми. Однако среда выполнения Windows была разработана для вызываться из отличный диапазона разных языках программирования. И библиотекой способом, который является очень естественным для каждого из этих языков. С этой целью доступ к среде выполнения Windows становятся доступными через то, что известно как языковые проекции. Существует проекция языка среды выполнения Windows в C#, в Visual Basic, в стандартной версии C++, JavaScript и т. д. Кроме того, один раз упакованных соответствующим образом (см. в разделе [Моста для классических приложений](/windows/uwp/porting/desktop-to-uwp-root)), можно вызвать API-интерфейсов WinRT из приложения, созданного в одном из отличный диапазон модели приложений: Win32, .NET, WinForms и WPF.

И, конечно, вы можете вызывать API-интерфейсов WinRT из приложения UWP. UWP — это модель приложений, создана на основе среды выполнения Windows. С технической точки зрения модели приложения UWP основана на [CoreApplication](/uwp/api/windows.applicationmodel.core.coreapplication), несмотря на то, что с подробным описанием могут быть скрыты от пользователя, в зависимости от выбранного вами языка программирования. Как описано в этом разделе, с значение выделиться точки зрения, UWP пригоден для написания одного двоичного файла, который можно Если вы решите, опубликованных в Microsoft Store и запустить на любой из отличный различных форм-факторов. Устройство аудиторию вашего приложения UWP зависит от подмножество API UWP ограничить приложение для вызова, или что вызвать определенных условиях.

Следует надеяться, что в этом разделе было выполнено успешно при описании разница между технологии базовый API среды выполнения Windows и механизм и значение универсальной платформы Windows для бизнеса.
