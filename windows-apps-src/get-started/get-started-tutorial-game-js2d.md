...
**Примечание:** Этот пример не завершенной (или хорошей) игрой. она предназначена для демонстрации с помощью JavaScript и третий сторонних библиотеки создать приложение, готовое к публикации в Microsoft Store.
...
Если вы запустили игру с помощью клавиши F5, наверняка вы задаетесь вопросом, что же сейчас происходит. А «не много», происходит совсем немного как значительная часть кода — это сейчас закомментирована. Итак вы увидите всего лишь динозавра и предложение требования к созданию клавишу ПРОБЕЛ. 
...
EaselJS предоставляет нам несколько различных типов графических объектов. Мы можем создавать простые фигуры (такие, как синий прямоугольник— небо), растровые изображения (например, облака, которые мы собираемся добавить), текстовые объекты и спрайты. Используется лист спрайтов (SpriteSheet) [http://createjs.com/docs/easeljs/classes/SpriteSheet.html]: — единое растровое изображение, содержащее множество рисунков. Например, этот SpriteSheet нужен для хранения разных кадров анимации с динозавром:
...
Теперь у вас есть приложение UWP, его можно опубликовать в Microsoft Store (при условии, что вы сначала Усовершенствуйте его!) 
