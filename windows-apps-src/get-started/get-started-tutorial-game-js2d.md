...
description: Простой UWP игру для хранилища Майкрософт, написанных на языке JavaScript и CreateJS
...
Публикацией приложения означает хранилища Майкрософт можно совместно использовать его (или Осуществите продажу!) с миллионы людей на различных устройствах.  

Чтобы опубликовать приложение в магазин корпорации Майкрософт, должны быть записаны в качестве приложения UWP (универсальные платформы Windows). Однако UWP— очень гибкая платформа и поддерживает множество языков и технологий. Мы продемонстрируем это на примере простой игры, которая написана на языке JavaScript и использует несколько библиотек CreateJS. Также в примере показано, как рисовать спрайты, создавать игровой цикл, настраивать поддержку клавиатуры и мыши и адаптацию к различным размерам экрана.
...
**Примечание:** Не игру полный (или хороший!); оно предназначено для демонстрации использования библиотеки JavaScript и третьей стороны, чтобы сделать приложение готово для публикации в хранилище Microsoft.
...
В этом проекте используется инфраструктура CreateJS JavaScript. CreateJS— это бесплатный набор инструментов, выпущенный по лицензии MIT, который упрощает создание игр на основе спрайтов. Библиотеки CreateJS уже присутствуют в проекте (ищите *js/easeljs-0.8.2.min.js* и *js/preloadjs-0.6.2.min.js* в представлении обозревателя решений). Подробнее о CreateJS можно узнать на [домашней странице CreateJS](http://www.createjs.com).
...
Если вы запустили игру с помощью клавиши F5, наверняка вы задаетесь вопросом, что же сейчас происходит. И ответ — «не много», как большой объем кода в настоящее время комментариев для масштабирования. На данный момент все, что вы увидите — динозавр и ineffectual требования к созданию клавиатуры пространства. 
...
EaselJS предоставляет нам несколько различных типов графических объектов. Мы можем создавать простые фигуры (такие, как синий прямоугольник— небо), растровые изображения (например, облака, которые мы собираемся добавить), текстовые объекты и спрайты. Используйте спрайтов (SpriteSheet) [http://createjs.com/docs/easeljs/classes/SpriteSheet.html]: один растровое изображение, содержащий несколько изображений. Например, этот SpriteSheet нужен для хранения разных кадров анимации с динозавром:
...
Gamestate— конструктивный шаблон, который широко используется в играх. Все события происходят в функции **gameLoop()**, которая вызывается таймером функции ticker. GameLoop() с помощью переменной отслеживает состояния игры, определенные разработчиком: запущена, завершена, готова к запуску или другое. Эта переменная состояния проверяется в операторе switch, который определяет, какие другие функции нужно вызвать. Если состояние игры— "запущена", будут вызваны функции, которые отвечают за прыжки динозавра и движение бочек. Если динозавр погиб, переменная gamestate обретет значение "игра завершена", и на экране появится сообщение "Конец игры". Если вас интересуют конструктивные шаблоны, вам поможет книга [Game Programming Patterns](http://gameprogrammingpatterns.com/).
...
## <a name="publishing-to-the-microsoft-store"></a>Публикация в хранилище Майкрософт

Теперь у вас есть UWP приложения, его можно публиковать в хранилище Microsoft (если он повысили сначала!) 
...
Для получения дополнительных сведений см [публикуете его UWP](https://developer.microsoft.com/en-us/store/publish-apps).
...
1. Подключите звуковые эффекты. Библиотека CreateJS включает поддержку звука через библиотеку под названием [SoundJS](http://www.createjs.com/soundjs).
...
* [Выбор игрового движка HTML/JS](https://html5gameengine.com/)
