...
3.  Выберите **Blank App (Universal Windows) JavaScript** (Пустое приложение (универсальное приложение для Windows) JavaScript) и нажмите кнопку **Далее**.
    ...
    Если универсальные шаблоны не отображаются, возможно, у вас нет компонентов для создания приложений UWP. Вы можете повторить процесс установки и добавить поддержку UWP. Для этого щелкните **Открыть установщик Visual Studio** в диалоговом окне **Создание проекта**. См. раздел [Подготовка](get-set-up.md).

4.  В диалоговом окне **Настройка нового проекта** введите **HelloWorld** в качестве **имени проекта** и выберите **Создать**.

> [!NOTE]
> Если вы используете Visual Studio впервые, может открыться диалоговое окно с запросом включить параметр **Режим разработчика**. Режим разработчика — это специальный параметр, включающий определенные функции, например разрешение на непосредственный запуск приложений, а не только через Store. Дополнительные сведения см. в разделе [Подготовка устройства к разработке](enable-your-device-for-development.md). Чтобы продолжить работу с этим руководством, выберите **Режим разработчика**, щелкните **Да** и закройте диалоговое окно.
...
Щелкните файл **index.html**, чтобы выбрать его в редакторе, и измените содержащийся в нем код HTML.
...
Теперь добавим код JavaScript. Щелкните файл **main.js**, чтобы выбрать его, и добавьте в него код.
...
Теперь вы можете запустить приложение с помощью клавиши F5. Приложение загрузится, и отобразится веб-страница. Нажмите кнопку. Откроется окно сообщения.
