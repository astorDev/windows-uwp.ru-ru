...
description: Простая игра UWP для Microsoft Store, написанная на языках C# и MonoGame
ms.date: 03/06/2017
ms.topic: article
...
Мы хотим, чтобы брокколи появлялись где-то за пределами экрана и двигались по направлению к игровому персонажу, который должен будет от них уклоняться. Для этого добавьте этот метод в класс **Game1.cs** :
...
Затем нам понадобится новый метод для обработки ввода с клавиатуры. Добавьте этот метод **Game1.cs**:
...
Благодаря третьей динозавр прыгает через изменение свойства **dY**, если нажата клавиша **ПРОБЕЛ**. Обратите внимание, что персонаж не может прыгнуть, если они находятся на «Земле» (dino.y = screenHeight * SKYRATIO), а также не элементы перемещаются Если вместо нажаты только один раз удерживается клавишу ПРОБЕЛ. Это позволяет избежать прыжка сразу после начала игры (ведь такому нажатию уже присвоена функция запуска игры).
...
В MonoGame новые вызовы **spriteBatch.Draw** будут рисоваться поверх любых предыдущих вызовов. Это означает, что спрайта динозавр и брокколи будут нарисованы поверх существующего спрайта травы, поэтому они могут никогда не будут за ним независимо от своего положения.

Попробуйте запустить игру и передвигать динозавра с помощью клавиш со стрелками и клавиши ПРОБЕЛ. Если вы следовали инструкциям выше, вы сможете управлять движениями в окне игры персонажа, а брокколи должны появляться с возрастающей скоростью.
...
Наконец мы прорисовываем каждую строку. Чтобы выровнять текст по горизонтали, внесения значение **X** вектор позиции равным **screenWidth / 2 - textSize.X / 2**.
...
### <a name="1-get-the-textures"></a>1. Получение текстур
Последний образ, что нам нужно, — это один для «игру заново». [Щелкните здесь, чтобы загрузить изображение.](https://github.com/Microsoft/Windows-appsample-get-started-mg2d/blob/master/MonoGame2D/Content/game-over.png)

Так же, как перед с зеленым прямоугольником ninja-cat и брокколи изображений, добавьте этот образ **Content.mgcb** через **MonoGame Pipeline**, присвоение ему «игра over.png».
...
Этот метод обнаруживает столкновение двух прямоугольных объектов. Этот алгоритм работает, тестируя ли разрыва между какие-либо из сторон прямоугольников. Если есть зазор, то столкновения нет. Если нет зазора, значит, должно быть столкновение.
...
### <a name="4-implement-game-over-logic"></a>4. Реализация логики «игра завершена»
...
Добавьте этот код в метод **KeyboardHandler** , чтобы разрешить пользователю сбросить игры, если они нажмите клавишу ВВОД:
...
### <a name="6-draw-game-over-splash-and-text"></a>6. игры отображение экрана-заставки и текста
...
Дополнительные сведения см. в разделе [Публикация приложения UWP](https://developer.microsoft.com/en-us/store/publish-apps).
