...
description: Простые игры для Microsoft Store, написанных на UWP C# и MonoGame
ms.date: 03/06/2017
ms.topic: article
...
+   Windows 10 и Microsoft Visual Studio 2017.  [Щелкните здесь, чтобы узнать, как подготовиться к работе с Visual Studio](https://docs.microsoft.com/en-us/windows/uwp/get-started/get-set-up).
+ Платформа разработки классических приложений .NET. Если у вас еще не установлена эта платформа, ее можно установить, повторно запустив установщик Visual Studio и изменив установку Visual Studio 2017.
...
**Примечание.** Игра, создаваемый в этом образце не должен быть полный (или любой развлекательные вообще). Его единственная цель — продемонстрировать основные понятия разработки 2D в MonoGame. Вы можете использовать этот код, чтобы сделать что-то намного лучшее. Или же, освоив основы, создайте свою игру с чистого листа!
...
4. В разделе c шаблонами проектов Visual C# выберите **MonoGame** и **MonoGame Windows 10 Universal Project**.
...
**защищенный override void обновления (игра игра)** этот метод вызывается один раз для каждого цикла цикл игры. Здесь мы обновляем состояния любого объекта или переменной, используемых в игре. Сюда входят такие свойства объектов, как положение, скорость или цвет. Это также где обрабатывается ввод данных пользователем. Иными словами, этот метод обрабатывает всю игровую логику, кроме рисования объектов на экране.
...
### <a name="1-download-a-texture"></a>1. Скачайте текстуры
...
### <a name="2-add-the-texture-to-the-content-folder"></a>2. Добавьте в папку Content текстуры
...
### <a name="3-add-class-variables"></a>3. Добавить переменные класса
...
### <a name="4-initialize-class-variables-and-set-window-size"></a>4. Инициализировать переменные класса и размер окна
...
### <a name="5-load-the-texture"></a>5. Загрузите текстуру
...
### <a name="6-draw-the-sprite"></a>6. Прорисовку спрайта
...
### <a name="1-create-a-new-class"></a>1. Создайте новый класс
...
### <a name="2-add-class-variables"></a>2. Добавить переменные класса
...
### <a name="3-create-a-constructor"></a>3. Создайте конструктор
...
Теперь, когда у нас есть класс SpriteClass, мы используем его для создания двух новых игровых объектов. Первый из них — игровой персонаж, которым игрок может управлять с помощью клавиш со стрелками и клавиши ПРОБЕЛ. Второй — это объект, необходимо избегать проигрыватель.

### <a name="1-get-the-textures"></a>1. Получить текстуры
...
### <a name="4-construct-spriteclasses"></a>4. Конструкция SpriteClasses
...
### <a name="5-program-obstacle-behaviour"></a>5. Поведение программы препятствие
Мы хотим, чтобы брокколи появлялись где-то за пределами экрана и двигались по направлению к игровому персонажу, который должен будет от них уклоняться. Для этого добавьте следующий метод для **Game1.cs** класса:
...
### <a name="6-program-game-starting-state"></a>6. Игры начальное состояние программы
...
Дальше нам понадобится новый метод для обработки ввода с клавиатуры. Добавьте следующий метод для **Game1.cs**:
...
Благодаря третьей динозавр прыгает через изменение свойства **dY**, если нажата клавиша **ПРОБЕЛ**. Обратите внимание, что игрок не может переходить, пока не будут на землю «» (dino.y = устройств * SKYRATIO) и будет также прыгает Если вместо нажатия один раз удерживается клавиша ПРОБЕЛ. Это позволяет избежать прыжка сразу после начала игры (ведь такому нажатию уже присвоена функция запуска игры).
...
### <a name="8-add-logic-to-the-update-method"></a>8. Добавить логику метода Update
...
В MonoGame новые вызовы **spriteBatch.Draw** будут рисоваться поверх любых предыдущих вызовов. Это означает, что broccoli и спрайт Дино получаются через существующие grass спрайта, поэтому они могут никогда не скрыт за его независимо от их расположения.

Попробуйте запустить игру и передвигать динозавра с помощью клавиш со стрелками и клавиши ПРОБЕЛ. Если вы выполнили описанные выше действия, вы сможете сделать свой аватар перемещать в пределах окна игры и broccoli должен породить скоростью чаще.
...
### <a name="1-create-spritefont-description-files"></a>1. Создание файлов SpriteFont описание
...
### <a name="3-load-spritefonts"></a>3. Загрузить SpriteFonts
...
### <a name="4-draw-the-score"></a>4. Рисование оценка
...
### <a name="5-draw-horizontally-centered-text"></a>5. Рисование текста по горизонтали по центру
...
Наконец мы прорисовываем каждую строку. Текст по центру по горизонтали, мы внести **X** значение вектора его положение равным **screenWidth / 2 - textSize.X / 2**.
...
### <a name="1-get-the-textures"></a>1. Получить текстуры
Последнего образа, которые необходимы, — один для «игру заново». [Щелкните здесь, чтобы загрузить изображение.](https://github.com/Microsoft/Windows-appsample-get-started-mg2d/blob/master/MonoGame2D/Content/game-over.png)

Как и раньше с зеленый прямоугольник, ninja cat и broccoli изображения, добавьте этот образ для **Content.mgcb** через **MonoGame конвейера**, назовите его «игра over.png».

### <a name="2-rectangular-collision"></a>2. Прямоугольная конфликтов
...
Этот метод обнаруживает столкновение двух прямоугольных объектов. Алгоритм работает путем тестирования, чтобы выяснить, имеется несоответствие между любой из сторон прямоугольника. Если есть зазор, то столкновения нет. Если нет зазора, значит, должно быть столкновение.

### <a name="3-load-new-textures"></a>3. Загрузить новый текстуры
...
### <a name="4-implement-game-over-logic"></a>4. Реализовать логику «взломанным»
...
### <a name="5-add-user-input-for-resetting-the-game"></a>5. Добавить входные данные пользователя для сброса настроек игры
Добавьте следующий код в **KeyboardHandler** метод, позволяющий пользователю для сброса игры в том случае, если они нажмите клавишу ВВОД:
...
### <a name="6-draw-game-over-splash-and-text"></a>6. Создайте игру за заставки и текста
...
Так как мы создали эту игру, как приложения UWP, можно опубликовать этот проект в Microsoft Store. Для этого нужно выполнить несколько шагов.
