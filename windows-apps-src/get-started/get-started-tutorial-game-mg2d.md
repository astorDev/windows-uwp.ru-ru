...
Благодаря третьей динозавр прыгает через изменение свойства **dY**, если нажата клавиша **ПРОБЕЛ**. Обратите внимание, что персонаж не может прыгнуть, если они находятся на «Земле» (dino.y = screenHeight * SKYRATIO), а также не элементы перемещаются Если вместо нажаты только один раз удерживается клавиша ПРОБЕЛ. Это позволяет избежать прыжка сразу после начала игры (ведь такому нажатию уже присвоена функция запуска игры).
...
В MonoGame новые вызовы **spriteBatch.Draw** будут рисоваться поверх любых предыдущих вызовов. Это означает, что брокколи и спрайта динозавра будут нарисованы поверх существующего спрайта травы, поэтому можно никогда не будут за ним независимо от своего положения.
