...
-   **Выражение с учетом жесты**  пользователи могут подключить устройства с другими пользователями, находящихся в непосредственной близости, физически коснувшись устройств друг с другом (многопользовательские игры). Узнайте подробнее о [бесконтактном и контактном взаимодействии](https://msdn.microsoft.com/library/windows/apps/hh465229).
...
-   **Географическое расположение**  используйте geolocation сведения из данных стандартные веб-сайтов или датчики географического положения, чтобы помочь пользователям обойти, найти их положение на карте или получить уведомления ближайших пользователей, действий и назначения. Дополнительные сведения о [географическом положении](https://msdn.microsoft.com/library/windows/apps/hh465139).
...
-   **Если невозможно использовать содержимое.** Разместите команды на одной из этих поверхностей интерфейса, если не получается использовать содержимое:
    ...
    -   В [панели команд](https://msdn.microsoft.com/library/windows/apps/hh465302): Большинство команд следует помещать на панели команд, который обычно является скрытым, пока пользователь касается, чтобы сделать его видимым.
    ...
    -   В [контекстное меню](https://msdn.microsoft.com/windows/uwp/controls-and-patterns/menus): Вы можете использовать контекстные меню для действия с буфером обмена (например, Вырезать, копировать и вставить), или для команды, применимые для содержимого, которое будет недоступен (например, добавление канцелярской кнопки в расположение на карте).
