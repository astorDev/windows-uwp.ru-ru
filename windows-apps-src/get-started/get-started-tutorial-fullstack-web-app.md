...
Файл Swagger (config\swagger.json) предписывает нашему серверу как обрабатывать различные HTTP-запросы клиента путем сопоставления каждого пути URL-адреса, которые он определяет для файла обработчика (в \handlers), а также каждого метода, определенного для этого пути (например, **GET**, **POST**) для `operationId` (функция) в рамках этого файла обработчика.
...
Ниже перечислено несколько полезных ресурсов для дальнейшей работы:
 ...
 - [Документация Azure по Интернету и мобильным устройствам](https://docs.microsoft.com/en-us/azure/#pivot=services&panel=web)
...
Теперь давайте посмотрим на этот код в действии, создав пользовательский интерфейс. В этом руководстве мы используем модуль создания шаблонов [Pug](https://pugjs.org/) (ранее Jade).  *Pug* является чистым синтаксисом, учитывающим пробелы, для написания HTML. Рассмотрим пример. 
...
2. Теперь добавьте следующие свойства в класс `.cards` в style.css. `.cards` `div` является элементом, который фактически будет осуществлять анимацию переворота, отображая лицевую или оборотную сторону карты. 
...
Ниже приведены основные шаги для публикации в Microsoft Store.
 ...
 2. Заполните [контрольный список](https://docs.microsoft.com/en-us/windows/uwp/publish/app-submissions) для отправки приложения.
