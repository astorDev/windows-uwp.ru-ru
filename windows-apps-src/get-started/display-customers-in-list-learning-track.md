...
* [**x: Bind** ](../xaml-platform/x-bind-markup-extension.md) является строго типизированным и создает код во время компиляции для повышения производительности. x:Bind по умолчанию используется для единовременной привязки, которая оптимизирована для быстрого отображения данных только для чтения, которые не изменяются.
* [**Привязка** ](../xaml-platform/binding-markup-extension.md) является слабо типизированным и собирать во время выполнения. Это приводит к более низкой производительности по сравнению с x:Bind. В большинстве случаев следует использовать x:Bind вместо Binding. Тем не менее вы скорее встретите этот объект в старом коде. По умолчанию Binding используется для односторонней передачи данных, оптимизированной для данных только для чтения, которые могут измениться в источнике.
...
* *Одноразовый*: Эта привязка данных включается только один раз и не реагировать на изменения.
* *Одностороннее*: Эта привязка данных пользовательского интерфейса замените любые изменения, внесенные в источник данных.
* *Двустороннее*: Эта привязка данных обновления UI при любые изменения, внесенные в источник данных и также обновить данные с любого изменения, внесенные в пользовательском Интерфейсе.
...
| [Привязки](../xaml-platform/binding-markup-extension.md) | Документация по старому расширению разметки Binding. |
...
| [Элемент управления ItemsControl](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ItemsControl) | Свойство **ItemsSource** этого класса позволяет привязать ListView к источнику данных. |
