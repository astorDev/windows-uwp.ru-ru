...
ms.locfileid: "2858290"
---
# <a name="package-flights"></a>Тестовые пакеты

Пакет перелетов можно использовать для распространения определенных пакетов для ограниченной группы инженеров-испытателей. Пакеты, которые уже опубликован в хранилище будет использоваться для других клиентов, поэтому их работы не будет прервана.

С помощью пакета перелетов отличаются только пакеты; Магазина подробные сведения будут одинаково для всех пользователей. Всем пользователям в группе полетов будет получать пакеты, которые включены в пакет полета, хотя пользователи, которые не входят в группу полетов предоставляться регулярных пакеты (не flighted).  Если позднее вы решите, что требуется пакеты из пакета полетов делаются доступными для всех клиентов, можно легко используйте те же пакеты в не flighted отправки. Обратите внимание, что перелетов пакета необходимо использовать [процесс сертификации](the-app-certification-process.md), точно так же, как любой отправки.

При установке пакета перелетов можно указать пользователей, которые должны получить определенных пакетов, добавив их **Известные группы пользователей** (иногда называемую полетов). Любой участник тестовой группы, который использует устройство с версией Windows 10, поддерживающей тестовые пакеты (Windows.Desktop сборки 10586 или более поздней; Windows.Mobile сборки 10586.63 или более поздней или Xbox One), получит тестовые пакеты, предназначенные для этой группы. (Тестовые группы могут содержать пакеты для любой версии ОС, в том числе Windows 8.1 или Windows Phone 8.1 или более ранних версий.) Пользователи, не добавленные ни в одну из тестовых групп или использующие устройства, не поддерживающие тестовые пакеты, получат пакеты из обычной отправки.
...
> [!NOTE]
> Если у вас больше одного тестового пакета, необходимо присвоить каждому из них приоритет. Дополнительные сведения в разделе [Add и ранжирования дополнительный пакет перелетов](#add-and-rank-additional-package-flights) ниже.
...
У вас есть возможность выбрать пакеты, связанные с предыдущей опубликованной отправкой (обычной отправкой или другим тестовым пакетом, если у вас их несколько). Если вам требуется отправка новых пакетов для этой полетов пакета, их можно загрузить здесь (с использованием [аналогичен процессу при отправке пакетов приложений для регулярного отправки не flighted](upload-app-packages.md)). Нажмите **Сохранить**, как только укажете все пакеты, которые требуется включить в этот тестовый пакет.
...
> [!IMPORTANT]
> Постепенное развертывание пакетов в пакете полетов, пользователей, которые не включены в процентах, который получает новые пакеты получите пакеты из предыдущей отправки полетов пакета (если им доступен более высоким рангом полетов).
...
Для создания новой отправки для полетов пакета, уже опубликованы, рядом с именем полетов на странице Общие сведения о приложении нажмите кнопку **Обновить** . Вы сможете добавить новые пакеты (и удалить ненужные), как и при обычной отправке. Внесите остальные необходимые изменения и щелкните **Отправить в Магазин** для отправки обновленного тестового пакета на прохождение [процедуры сертификации приложения](the-app-certification-process.md).

Чтобы изменить существующий тестовый пакет, не создавая и не отправляя обновление, щелкните **Изменить** рядом с именем пакета. Это позволяет изменять такие сведения, как группы, название и приоритет тестового пакета, без необходимости повторного прохождения тестовым пакетом процесса сертификации. Обратите внимание, что при наличии обновления в процессе выполнения или если еще не опубликован вашей полетов пакета, вы не увидите « **Изменить** ». 
...
Обратите внимание, что сообщение не flighted всегда ранжирования низшего (#1). Это означает, что пользователи, не состоящие ни в одной тестовой группе, могут получать через Магазин только пакеты из обычной отправки. Пользователи в группе полетов всегда будет пакеты из доступных полета наивысшим рангом пакета для их (но никогда не отправки не flighted, так как она имела низкий ранг). Это дает вам гибкость при распределении пакетов среди пользователей, которые могут состоять в нескольких тестовых группах.
