...
ms.locfileid: "4390268"
---
# <a name="package-flights"></a>Тестовые пакеты

Тестовые пакеты можно использовать для распространения конкретных пакетов только группе тест-инженеров. Пакеты, которые вы уже опубликовали в магазин будет использоваться для других пользователей, чтобы их опыте не будет прервана.

С помощью тестовых отличаются только пакеты; Описание приложения в магазине будет одинаковым для всех пользователей. Любой участник тестовой группы получат пакеты, которые включены в тестового пакета пользователи, которые не входят в тестовую группу по-прежнему получать регулярные пакетов (обычные).  Если в дальнейшем вы решите, что вы хотите сделать доступным пакеты из тестового пакета всем пользователям, вы можете легко используйте те же пакеты в обычную отправку. Обратите внимание, тестовые пакеты должны пройти точно так же, как любой отправки [процесса сертификации](the-app-certification-process.md).
...
Обратите внимание, что и обычные отправки всегда значится на низкий (#1). Это означает, что пользователи, не состоящие ни в одной тестовой группе, могут получать через Магазин только пакеты из обычной отправки. Участники тестовых групп всегда будут получать пакеты из тестового пакета с самым высоким приоритетом доступны для них (но никогда не обычную отправку, поскольку он имеет низкий приоритет). Это дает вам гибкость при распределении пакетов среди пользователей, которые могут состоять в нескольких тестовых группах.
