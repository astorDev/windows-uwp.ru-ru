...
ms.locfileid: "2918715"
---
# <a name="package-flights"></a>Тестовые пакеты

Тестовые пакеты можно использовать для распространения конкретных пакетов ограниченной группе тест-инженеров. Пакеты, которые вы уже опубликовали в магазине будут использоваться для других пользователей, поэтому их опыте не будет прервана.

С помощью тестовых отличаются только пакеты; Описание приложения в магазине будет одинаковым для всех пользователей. Любой участник тестовой группы получат пакеты, которые включены в тестовый пакет, пользователи, которые не входят в тестовую группу по-прежнему получать регулярные пакетов (обычные).  Если в дальнейшем вы решите, что вы хотите сделать доступным пакеты из тестового пакета всем пользователям, вы можете легко используйте те же пакеты в обычную отправку. Обратите внимание на то, тестовые пакеты должны пройти точно так же, как любой отправки [процесса сертификации](the-app-certification-process.md).
...
> [!IMPORTANT]
> При постепенном выпуске пакетов в составе тестового, пользователи, не включенные в ту часть, которая получает ваши новые пакеты будут получать пакеты из предыдущей отправки тестового пакета (если им доступна тестового высокого ранга).
...
Обратите внимание, что и обычные отправки всегда значится низкий (1). Это означает, что пользователи, не состоящие ни в одной тестовой группе, могут получать через Магазин только пакеты из обычной отправки. Участники тестовых групп всегда получают пакеты из тестового пакета с самым высоким приоритетом доступны для них (но никогда не обычную отправку, поскольку он имеет низкий приоритет). Это дает вам гибкость при распределении пакетов среди пользователей, которые могут состоять в нескольких тестовых группах.
