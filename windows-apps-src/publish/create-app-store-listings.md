...
## <a name="add-and-edit-store-listing-info"></a>Добавление и изменение хранилища, сведения о решении

Чтобы изменить магазина, выберите имя языка отправки "Обзор".
...
В этом поле со списком позволяет указать, какое имя следует использовать в списке хранилище (если зарезервированным более одного имени для приложения).

Если отправки пакеты на одном языке, как описание приложения в магазине, на котором вы работаете, имя, используемое в пакеты, будет установлен. Если требуется [Переименовать приложения](manage-app-names.md#rename-an-app-that-has-already-been-published) после его уже была опубликована, можно выбрать разные зарезервированное имя, при создании новой отправки с помощью пакетов, использующих новое имя.

Если вы еще не загружен пакетов для языка вы работаете и зарезервировано более одного имени, вам потребуется выбрать одно из имен зарезервированные приложения, поскольку на этом языке, из которого извлекается имя еще нет связанного пакета.

> [!NOTE]
> **Имя продукта** , выберите только применяется в списке магазина на языке работе. Он не влияет на имя, отображаемое при установке потребителем приложения; Это имя, поступающие из манифеста, устанавливаемый пакет. Чтобы избежать путаницы, мы рекомендуем использовать то же имя каждого языковые пакеты и описание приложения в магазине.
...
Для достижения наилучших результатов оставьте краткое описание в разделе 270 символов. Поле ограничен 500 символов, но в некоторых представлениях будут отображаться только первые 270 символов (со ссылкой, доступные для просмотра остальных краткого описания).
