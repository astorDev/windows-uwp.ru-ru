...
> [!NOTE]
> При создании описания в Store на языке, который не поддерживается вашими пакетами, необходимо указать, какое из зарезервированных вами имен приложения должно отображаться в этом описании в Store, поскольку отсутствует связанный пакет на этом языке, из которого можно было бы получить имя. Имя, которое вы здесь выберете, относится только к описанию в Store для этого языка и не влияет на имя, отображаемое при установке приложения пользователем.

Чтобы изменить описание в Store, щелкните имя языка на странице общих сведений об отправке.
