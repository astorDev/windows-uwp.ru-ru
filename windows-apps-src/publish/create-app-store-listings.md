...
> [!TIP]
> Вы можете [импортировать и экспортировать описания в магазине](import-and-export-store-listings.md) , если вы хотите ввести описания в автономном режиме в CSV-файл, вместо предоставления сведения и передачи файлов непосредственно в центре партнеров. Использование параметров импорта и экспорта может быть особенно удобным, если у вас есть описания на многих языках, так как они позволяют выполнять несколько обновлений одновременно. 

Если ранее опубликованное приложение поддерживает Windows 8.x и/или Windows Phone 8.x или более ранних версий, вы можете [Создать описания в магазине для определенной платформы](create-platform-specific-store-listings.md) для отображения для этих пользователей. 
...
Чтобы изменить описание в магазине, выберите имя языка на странице общих сведений отправки. Необходимо изменить каждого языка по отдельности, пока не будет выбран экспорта описаний в магазине и работать в автономном режиме, а затем импортировать все данные описания за один раз. Дополнительные сведения о том, как это работает, см. в разделе [импортировать и экспортировать описания в магазине](import-and-export-store-listings.md).

Доступные поля описаны ниже.

## <a name="product-name"></a>Название продукта

Этот флажок раскрывающийся список позволяет указать, какое имя следует использовать в описании в магазине (Если вы уже были зарезервированы несколько имен для приложения).

Если вы отправили пакеты на одном языке, как вы описания в магазине работаете, имя, используемое в эти пакеты будут выбраны. Если вам нужно [Переименовать приложение](manage-app-names.md#rename-an-app-that-has-already-been-published) после его уже были опубликованы, зарезервированное имя, которое отличается здесь можно выбрать при создании новой отправки, после вы отправили пакеты, которые используют новое имя.
...
> [!NOTE]
> Выберите только **Название продукта** относится к описания в магазине на языке вы работаете в. Он не влияет на имя, отображаемое, когда пользователь устанавливает приложение; Это имя берется из манифеста пакета, который получает установлен. Чтобы избежать путаницы, рекомендуется использовать то же имя каждого языка пакеты и описание в магазине.
...
Если это первая отправка вашего приложения, оставьте это поле пустым. Для обновления существующего приложения это где вы можете разрешить пользователям, что изменилось в последнем выпуске клиентам. Длина этого поля ограничена 1500 символами. (Ранее это поле называлось **Заметки о выпуске**).

## <a name="product-features"></a>Возможности продукта
...
> [!NOTE]
> Эти возможности отобразятся в виде маркированного списка в описании в магазине, поэтому не добавляйте собственные маркеры.
...
Вы можете отправить дополнительные ресурсы для вашего продукта, в том числе видео трейлеры и рекламные изображения. Это все необязательно, но мы рекомендуем выложить их как можно больше. Эти изображения помогут сформировать у клиентов лучшее представление о вашем продукте и сделать его описание более привлекательным.

Дополнительные сведения см. в разделе [трейлеры и дополнительные ресурсы](app-screenshots-and-images.md#trailers-and-additional-assets).
...
Для получения наилучших результатов актуальности вашего краткое описание в разделе 270 символов. Поля ограничена 500 символами, но в некоторых представлениях будут отображаться только первые 270 символы (с помощью ссылки, доступных для просмотра остальная часть краткое описание).
