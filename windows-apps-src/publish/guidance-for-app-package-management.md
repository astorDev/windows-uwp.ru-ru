...
Как правило, более поздние версии ОС могут запускать пакеты, предназначенные для предыдущих версий ОС для того же семейства устройств. Windows10 устройств можно запустить всех ранее поддерживаемых версий ОС (для каждого семейства устройств). Настольные устройства Windows10 можно запускать приложения, которые были созданы для Windows8.1 или Windows8; Мобильные устройства Windows10 можно запускать приложения, которые были созданы для Windows Phone 8.1, WindowsPhone8 и даже Windows Phone 7.x. Тем не менее пользователи Windows 10 только будут получать эти пакеты, если приложение не содержит пакеты UWP, предназначенных для применимых семейства.
...
## <a name="adding-packages-for-windows10-to-a-previously-published-app"></a>Добавление пакетов для Windows10 в ранее опубликованное приложение
...
> [!NOTE]
> Как только пользователь Windows10 получит ваш пакет UWP, не может вернуть данного пользователя к использованию пакета для предыдущей версии операционной системы. 

Обратите внимание, что номер версии пакетов Windows10 должен быть больше, чем пакеты Windows8, Windows8.1 и Windows Phone 8.1, которые вы использовали. Дополнительные сведения см. в разделе [Нумерация версий пакета](package-version-numbering.md).
