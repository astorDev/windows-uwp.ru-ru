...
Если вы удалите все пакеты для определенного [семейства устройств](https://docs.microsoft.com/uwp/extension-sdks/device-families-overview) , ваше приложение ранее поддерживается, вам будет предложено подтвердить, что это ваше намерение, прежде чем вы сможете сохранить внесенные изменения на странице " **пакеты** ".

При публикации отправки, удаляющей все пакеты, которые могут выполняться на семейство устройств, ранее поддерживавшегося вашим приложением, новые клиенты не смогут приобрести приложение на семейство устройств. В дальнейшем вы можете в любое время опубликовать еще одно обновление, что вернуть пакеты для этого семейства устройств.
...
Если ваше приложение для Windows8.x и/или WindowsPhone8.x опубликовано в Магазине и вы хотите обновить его для Windows10, создайте новую отправку и добавьте пакеты .appxupload UWP на этапе [Пакеты](upload-app-packages.md). После ваше приложение должно пройти сертификацию, пользователи, которые уже было приложение и, теперь представлены в Windows 10 смогут получить пакет UWP виде обновления из магазина. Пакет UWP также будет доступен для новых приобретений пользователями, использующими Windows 10.
...
> 
> Это происходит, даже если вы исключили определенное семейство на шаге [доступность семейства устройств](device-family-availability.md) отправки, поскольку что раздел относится только к новым приобретениям. Если вы не хотите, чтобы любой пользователь предыдущих версий ОС получил ваш универсальный пакет Windows 10, обязательно обновите элемент [**TargetDeviceFamily**](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-targetdevicefamily) в манифесте APPX для включения только определенного семейства устройств, которое нужно поддерживать.
> 
> Предположим, вы хотите, которые обновились до настольном компьютере для получения нового приложения UWP пользователи Windows 8 и Windows 8.1, но вы хотите, чтобы пользователи Windows Phone, которые на устройствах Windows 10 Mobile для сохранения пакетов, которые вы бы использовали ранее осуществляются доступен e (предназначенных для Windows Phone 8 или Windows Phone 8.1). Чтобы сделать это, необходимо обновить [**TargetDeviceFamily**](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-targetdevicefamily) в манифесте appx для включения только **Windows.Desktop** (для семейства настольных устройств), а не оставлять значение **Windows.Universal** (для семейства универсальных устройств) что Microsoft Visual Studio включает в манифест по умолчанию. Не отправляйте пакеты UWP, ориентированные на семейства универсальных или мобильных устройств (**Windows.Universal** или **Windows.Universal**). Таким образом ваши пользователи Windows 10 Mobile не получат пакетов UWP.
