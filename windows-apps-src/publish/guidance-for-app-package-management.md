...
Как правило, более поздние версии ОС могут запускать пакеты, предназначенные для предыдущих версий ОС для того же семейства устройств. Тем не менее клиенты получите только те пакеты, если приложение не включает пакет, предназначенный для их текущей версии операционной системы.
...
В примере приложения 4 любое устройство под управлением Windows10 может получить это приложение, но оно не будет доступно клиентам, использующим любые предыдущие версии ОС. Так как пакет UWP относится семейства универсальные устройства, оно будет доступно на любом устройстве Windows 10 (на [выбранных элементов семейства доступности устройства](device-family-availability.md)).
...
После удаления всех пакетов для определенного [семейства устройств](https://docs.microsoft.com/uwp/extension-sdks/device-families-overview) , ваше приложение ранее поддерживается, будет выведен запрос, чтобы убедиться, что вы собираетесь перед сохранением изменений на странице **пакетов** .

При публикации отправки, который удаляет все пакеты, которые могут выполняться на семейство устройства, который ваше приложение ранее поддерживается новых клиентов не сможет получить приложение для данного семейства устройств. В дальнейшем вы можете в любое время опубликовать еще одно обновление, что вернуть пакеты для этого семейства устройств.
...
Если ваше приложение для Windows8.x и/или WindowsPhone8.x опубликовано в Магазине и вы хотите обновить его для Windows10, создайте новую отправку и добавьте пакеты .appxupload UWP на этапе [Пакеты](upload-app-packages.md). После вашего приложения проходит через процесс сертификации, пользователей, которые уже были вашего приложения и теперь находятся на Windows 10 получить пакет UWP как обновление из хранилища. Пакет UWP также будет доступен для новых приобретений пользователями, использующими Windows 10.
...
> 
> Это происходит даже в том случае, если исключенные семейства определенное устройство на этапе [доступности семейства устройств](device-family-availability.md) отправке, начиная с, что раздела относятся только к новой приобретений. Если вы не хотите, чтобы любой пользователь предыдущих версий ОС получил ваш универсальный пакет Windows 10, обязательно обновите элемент [**TargetDeviceFamily**](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-targetdevicefamily) в манифесте APPX для включения только определенного семейства устройств, которое нужно поддерживать.
> 
> Например, Windows 8 и Windows 8.1 пользователей, которым требуется обновление до Windows 10 рабочего стола устройства для получения нового приложения UWP, но разрешить клиентов Windows Phone, теперь на устройствах Windows 10 Mobile для хранения пакетов, как ранее выполняются доступен e (ориентация Windows Phone 8 или Windows Phone 8.1). Для этого необходимо обновить [**TargetDeviceFamily**](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-targetdevicefamily) в манифесте appx включать только **Windows.Desktop** (для настольных систем устройства семейства), а не на основе оставив его как значение **Windows.Universal** (для операционных систем семейства универсальные устройства) по умолчанию, Microsoft Visual Studio включает в манифесте. Не отправляйте пакеты UWP, ориентированные на семейства универсальных или мобильных устройств (**Windows.Universal** или **Windows.Universal**). Таким образом ваши пользователи Windows 10 Mobile не получат пакетов UWP.
