...
Если вы используете пользовательские кампании для вашего приложения, вы можете отслеживать относительную эффективность каждой кампании, создав разные URL-адреса приложения для каждой пользовательской кампании, где каждый URL-адрес содержит собственный *идентификатор кампании*. При Windows10 пользователь щелкает URL-адрес, содержащий идентификатор кампании, Майкрософт связывает щелчка с соответствующей пользовательской кампанией и предоставляет эти данные вам в [Центре партнеров](https://partner.microsoft.com/dashboard).

> [!IMPORTANT]
> Эти данные отслеживаются только для пользователей в Windows10. Пользователи, использующие другие операционные системы, могут по-прежнему переходить по ссылке на страницу с описанием вашего приложения, но данные об этих действиях пользователя не будут включены.
...
* Вы можете просматривать данные о просмотров и преобразований страницы для вашего приложения или надстройки на диаграммах **страницы представления и преобразования приложения по Идентификатору кампании** и **преобразования общее кампании** в [отчете о приобретениях](acquisitions-report.md).
...
*Конверсия* пользовательской компании — это приобретение приложения, к которому привел щелчок по URL-ссылке, продвигаемой через пользовательскую кампанию. Существуют разные сценарии определения конверсии для **представления страницы приложения и конверсий по Идентификатору кампании** и **конверсий в ходе кампании общее** диаграммах в [отчете о приобретениях](acquisitions-report.md) и для определения конверсии для [ программном получении идентификатора кампании](#programmatically).
