...
Вы можете предоставить [снимки экрана](#screenshots), [логотипы](#store-logos), [трейлеры](#trailers)и другие связанные с оформлением ресурсы, необходимо включить в описание вашего приложения в магазине. Некоторые из них являются обязательными, а некоторые— нет (тем не менее, чтобы обеспечить наилучшее представление приложения в Store, важно добавлять некоторые необязательные изображения).
...
Магазин может также использовать значок вашего приложения и другие изображения, которые включены в пакет вашего приложения. Запустите [Комплект сертификации приложений для Windows](../debug-test-perf/windows-app-certification-kit.md), чтобы определить, не отсутствуют ли какие-либо обязательные изображения. Инструкции и рекомендации о том, эти изображения см. в разделе [значки и логотипы приложений](../design/style/app-icons-and-logos.md).
...
Это изображение используется для правильного отображения приложения на Windows Phone 8.1 или более ранних версий. Если ранее опубликованное приложение поддерживает Windows Phone 8.1 или более ранних версий, и вы не предоставите это изображение, эти пользователи будут видеть пустой значок в описании вашего приложения. (Это также относится к клиентам, использующим Windows 10 Если ваше приложение содержит только пакеты, предназначенные для Windows Phone 8.1 или более ранних версий.)

Если вашей отправки *только* содержит пакеты UWP, вам не нужно предоставить это изображение (Если вы установили флажок для **для клиентов с Windows 10 и Xbox, отображать выложенные изображения логотипа вместо изображений из моих пакетов**, как описано в следующем раздел).

### <a name="display-only-uploaded-logo-images-in-the-store"></a>Отображение только выложенные изображения логотипа в магазине

Вы можете запретить Store использовать изображения логотипа в ваших пакетах приложения при показе страницы приложения клиентам, использующим Windows 10 (включая Xbox) и заставить магазин использовал только загружаемые изображения. Так вы сможете эффективнее настроить внешний вид вашего приложения в Store для клиентов, использующих Windows 10 (включая Xbox). (Если ранее опубликованное приложение поддерживает более ранние версии ОС, эти пользователи могут оставаться изображения из пакетов.)

У хранилище использовать только выложенные (для пользователей Windows 10, включая Xbox), изображения и не используйте изображения, любой из пакетов, установите флажок заблокированном **для клиентов, использующих ОС Windows 10 и Xbox, отображать выложенные изображения логотипа вместо изображений из моих пакетов **.
...
## <a name="trailers-and-additional-assets"></a>Трейлеры и дополнительные ресурсы
...
> [!TIP]
> Изображение [обложки главное изображение формата 16:9](#windows-10-and-xbox-image-169-super-hero-art) особенно рекомендуется, если вы планируете добавить [видеоанонсы](#trailers) в Вашем магазине описания; Если его не добавить, трейлеры не будут отображаться в верхней части описания.


### <a name="trailers"></a>Трейлеры

Трейлеры— это короткие видеоролики, которые помогают клиентам увидеть ваш продукт в действии, чтобы они могли лучше понять, что он из себя представляет. Они отображаются в верхней части приложения описание в Store (при условии, что включать изображение [обложки главное изображение формата 16:9](#windows-10-and-xbox-image-169-super-hero-art) ). 
...
> [!IMPORTANT]
> При использовании трейлеров необходимо также предоставить раздел [16:9 главное](#windows-10-and-xbox-image-169-super-hero-art) изображение в порядке для ваши трейлеры отображались в верхней части описания в магазине. Это изображение будет отображаться после завершения воспроизведения трейлеров.
...
Чтобы удалить трейлер из описания, щелкните значок **X** рядом с именем соответствующего файла. Вы можете выбрать, следует ли удалить его из текущего описания в магазине вы работаете, или удалить его из всех описаний вашего продукта в магазине (на всех языках).
...
В разделе **Windows 10 и Xbox изображение** изображение **альбома главное изображение формата 16:9 (1920 x 1080 или 3840 x 2160 пикселей)** используется различных макетов в Microsoft Store на всех типах устройств Windows 10 (включая Xbox). Рекомендуем предоставить это изображение независимо от того, для какой операционной системы или типа устройств предназначено приложение.
...
Если ваше приложение уже отправили поддерживает более ранние версии ОС (Windows 8.x и/или Windows Phone 8.x), необходимо предоставить эти изображения в порядке для нас в подборки вашего приложения в рекламных макетах (хотя они не гарантирует, что ваше приложение будет включено в подборку). Если ваше приложение не поддерживает эти более ранние версии ОС, пропустите этот раздел. (Этот раздел ранее назывался **Необязательные рекламные изображения**.)

**Для ОС WindowsPhone8.1 и более ранних версий** в рекламных целях можно предоставить изображения двухразмеров: **1000 x 800пикселей (5:4)** и **358 x 358пикселей (1:1)**. Если ваше приложение работает в Windows Phone 8.1 или более ранних версий, мы рекомендуем предоставить изображения двух размеров.  
...
**Для ОС Windows8.1 и более ранних версий** в некоторых рекламных макетах могут использоваться изображения размером **414 x 180**пикселей. Если ваше приложение работает в Windows 8.1 или более ранних версий, мы рекомендуем предоставить изображение этого sizen.
