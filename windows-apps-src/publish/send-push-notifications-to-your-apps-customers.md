...
3. В разделе **Выберите шаблон** выберите нужный тип уведомления. Подробные сведения см. в разделе [Типы шаблонов уведомлений](#notification-template-types).
4. На следующей странице выберите приложение (которое необходимо настроить, чтобы получать уведомления с использованием пакета Microsoft Store Services SDK).
...
8. Если вы хотите отфильтровать получателей, чтобы ваше уведомление получали только люди, использующие определенные языки или находящиеся в определенных часовых поясах, установите флажок **Использовать фильтры**. Затем можно указать параметры языка и (или) часовой пояс, которые вы хотите использовать.
