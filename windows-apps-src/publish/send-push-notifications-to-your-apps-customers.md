...
3. В разделе **Выбор шаблона** выберите [тип уведомления](#notification-template-types) для отправки и нажмите кнопку **ОК**.
4. На следующей странице с помощью раскрывающемся меню выберите **Одно приложение** или **Несколько приложений**, для которых требуется создать уведомление. Можно выбрать только приложения, которые были [настроены на получение уведомлений с помощью пакета SDK служб Microsoft хранилища](../monetize/configure-your-app-to-receive-dev-center-notifications.md).
...
2.  В таблице существующего уведомлений выберите **в процессе выполнения** или **завершено**и посмотрите на столбцы **доставки** и **Частота запуска приложения** для просмотра высокого уровня производительности каждого уведомления.
...
1.  На информационной панели Центра разработки для Windows разверните раздел **Привлечение**, а затем выберите **Уведомления**.
2.  На вкладке **уведомления** перейдите на вкладку **Анализ** . На этой вкладке отображаются следующие данные:
    * Графическое представление представления различных состояний действие пользователя для всплывающие уведомления и уведомления центр действие.
    * Представления карты World click через-курсов для всплывающие уведомления и действие центра обработки уведомлений.
3. В верхней части страницы можно выбрать период времени, для которого требуется отобразить данные. Значением по умолчанию является 30D (30дней), однако вы можете отобразить данные за 3, 6 или 12 месяцев, либо для заданного вручную диапазона дат. Также можно развернуть **фильтры** для всех данных, приложения и рынка.
