...
-   **Имя семейства пакетов (PFN)** : Это значение используется с некоторыми API-интерфейсами Windows.
-   **ИД безопасности пакета**: Это значение потребуется для отправки уведомлений WNS в приложение. Дополнительные сведения см. в [обзоре служб push-уведомлений Windows (WNS)](../design/shell/tiles-and-notifications/windows-push-notification-services--wns--overview.md).
