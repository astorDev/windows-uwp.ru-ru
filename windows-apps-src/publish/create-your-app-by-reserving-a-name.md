...
ms.locfileid: "2888173"
---
# <a name="create-your-app-by-reserving-a-name"></a>Создание приложения посредством резервирования имени

Первым шагом для создания нового приложения в панели [мониторинга центра разработчиков Windows](https://partner.microsoft.com/dashboard) — это резервирование имени приложения. Каждое зарезервированное имя (иногда называемое *названием* приложения) должно быть уникальным в Microsoft Store.
...
> [!IMPORTANT]
> Можно зарезервировать дополнительные имена для приложения и следующим шагом нужно использовать один из них в опубликованной версии вашего приложения вместо то, что зарезервировать при создании приложения в панели мониторинга. Тем не менее Обратите внимание, что здесь имя будет использоваться в некоторых ваше приложение [identity сведений](view-app-identity-details.md), таких как **Имя семейства пакет (PFN)**. Эти значения могут быть видны для некоторых пользователей и не может быть изменяется, поэтому убедитесь, что имя, зарезервированные подходит для использования в этом.
