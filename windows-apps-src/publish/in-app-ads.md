...
4.  В раскрывающемся меню **Тип группы объявлений** выберите тип рекламы.
    ...
    * Если в вашем приложении отображаются объявления, установите флажок **заголовка**.
    * Если в вашем приложении отображаются interstitial видео ad или interstitial объявления, выберите **видео interstitial** или **interstitial баннер** (обязательно выберите соответствующий параметр для типа interstitial ad, которые необходимо показать).
    * Если в вашем приложении отображаются собственные ad, выберите **собственная**.
...
* Если ваше приложение отображает [собственный ads](../monetize/native-ads.md), передайте эти значения конструктору **NativeAdsManagerV2** .
  > [!IMPORTANT]
...
2. На странице **ads из приложения** перейдите в раздел **Параметры посредника** и настройки параметров.

    * По умолчанию установлен флажок **Позволить Майкрософт выбрать оптимальные настройки посредника для вашего приложения**. Рекомендуется использовать этот параметр. Этот параметр использует алгоритмы машинного обучения, чтобы можно было автоматически выбрать настройки рекламного посредника для вашего приложения и тем самым помочь вам максимально увеличить доход от рекламы во всех странах, в которых доступно ваше приложение. Если используется этот параметр, вы также можете сетях ad, которые вы хотите использовать в конфигурации. Снимите флажок сетей ad, которые не нужно входить в состав конфигурации и нашего алгоритм гарантирует, что ваше приложение получает ads из выбранного ad сетей.
    * Если вы хотите выбрать собственный ad посредника параметры, выберите **изменить настройки по умолчанию**.

    > [!NOTE]
    > Оставшиеся действия, описанные в этом разделе применяются только в том случае, если выбрать **Изменение параметров по умолчанию**.
...
|--------------|---------------|---------------------|
| Отношения и AppNexus |  Это сеть ad управляемой корпорацией Майкрософт, который выступает ads через партнера в сети, Oath и AppNexus.<p/>**Примечание**: Oath и AppNexus — это всегда ранг первые в списке **сетей ad платными** единицы заголовок ad и его нельзя изменить в нижнем ранжирования для этих типов ads. | Баннеры, межстраничная видеореклама |
| AppNexus (прямая) | Выберите этот параметр для обслуживания рекламу от [AppNexus](https://www.appnexus.com). | Межстраничная видеореклама, собственные объявления  |
...
| Рекомендации по контента MSN |  Выберите этот параметр для обслуживания рекламу от контента рекомендаций MSN. |  Баннеры, межстраничные баннеры  |
...
| Undertone | Выберите этот параметр для обслуживания рекламу от [Undertone](https://www.undertone.com/). | Баннер interstitial |
