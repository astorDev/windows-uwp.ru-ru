...
> [!NOTE]
> Если для группы объявлений отсутствуют действия за последние полгода, мы пометить как **неактивное**и в конечном итоге удалим с информационной панели. С помощью фильтров можно показать только **Активные** или **Неактивные** группы объявлений. Если вы увидите, что какая-либо группа объявлений помечена как **Неактивная** по ошибке, [обратитесь в службу поддержки](http://aka.ms/storesupport).
...
2. На странице " **Реклама в приложении** " перейдите в разделе **Параметры посредника** и настройку параметров.
    ...
    * Если вы хотите выбрать собственные настройки посредника, следует **Изменить параметры по умолчанию**.
    ...
    > [!NOTE]
    > Оставшиеся инструкции в этом разделе применимы только в том случае, если вы решите **изменить настройки по умолчанию**.
