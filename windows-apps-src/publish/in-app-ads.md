...
ms.locfileid: "84945932"
---
# <a name="in-app-ads"></a>Реклама в приложении

>[!WARNING]
> Начиная с 1 июня 2020 г. платформа Microsoft AD монетизацию для приложений Windows UWP будет выключена. [Дополнительные сведения](https://social.msdn.microsoft.com/Forums/windowsapps/en-US/db8d44cb-1381-47f7-94d3-c6ded3fea36f/microsoft-ad-monetization-platform-shutting-down-june-1st?forum=aiamgr)

Используйте страницу **Monetize** &gt; **рекламных объявлений монетизации в приложении** в [центре партнеров](https://partner.microsoft.com/dashboard) для создания и управления единицами AD для:
...
1.  Перейдите на страницу **Monetize** &gt; **рекламных объявлений монетизации in-app** в центре партнеров и щелкните **создать единицу AD**.
...
* Если приложение отображает [рекламу внутреннего](../monetize/interstitial-ads.md), передайте эти значения в метод [рекуестад](https://docs.microsoft.com/uwp/api/microsoft.advertising.winrt.ui.interstitialad.requestad) объекта [интерститиалад](https://docs.microsoft.com/uwp/api/microsoft.advertising.winrt.ui.interstitialad) .
...
5. В разделе **платные сети ad** установите флажок в столбце **Активный** для каждой [платной сети](#paid-networks) , которую необходимо использовать, а затем используйте стрелки в столбце **ранг** , чтобы упорядочить сети по рангу (это указывает, как часто каждая сеть должна использоваться элементом управления).
6. Если вы выбрали группу объявлений **Баннер** или **Межстраничное объявление**, вы также увидите раздел с именем **Другие рекламные сети**. Сети в этом разделе не приносят вам прибыль за показы. Вместо этого они демонстрируют объявления из других источников (например, в рамках кампаний по продвижению приложений).
    ...
    В разделе **другие сети Active Directory** установите флажок в столбце **Активный** для каждой [другой сети](#other-networks) , которую необходимо использовать, а затем используйте стрелки в столбце **ранг** , чтобы упорядочить сети по рангу (это указывает, как часто каждая сеть должна использоваться элементом управления). В настоящее время поддерживаются другие сети, перечисленные ниже.
...
| Smaato |  Выберите этот вариант, чтобы показывать объявления от сети [Smaato](https://www.smaato.com/). |  Баннер  |
...
| Taboola |  Выберите этот вариант, чтобы показывать объявления от сети [Taboola](https://www.taboola.com/). |  Баннер  |
