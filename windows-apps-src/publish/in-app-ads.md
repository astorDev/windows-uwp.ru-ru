...
ms.locfileid: "9048481"
---
# <a name="in-app-ads"></a>Реклама в приложении

Используйте **Монетизация** &gt; страницы **в приложении рекламы** в [Центре партнеров](https://partner.microsoft.com/dashboard) на создание и управление группами объявлений для:
...
* Публиковалась Windows 8.x и Windows Phone 8.x приложения, использующие [Microsoft Advertising SDK для Windows и Windows Phone 8.x](https://aka.ms/store-8-sdk).
...
> [!NOTE]
> Если группу объявлений отсутствуют действия за последние полгода, мы будет помечать как **неактивный**и в конечном итоге удалим с центра партнеров. С помощью фильтров можно показать только **Активные** или **Неактивные** группы объявлений. Если вы увидите, что какая-либо группа объявлений помечена как **Неактивная** по ошибке, [обратитесь в службу поддержки](https://aka.ms/storesupport).
...
| Рекомендации по MSN содержимого |  Выберите этот вариант, чтобы показывать объявления от рекомендаций MSN содержимого. |  Баннеры, межстраничные баннеры  |
...
Шаг при [Создайте группу объявлений](#create-ad-unit) или [выберите существующую группу объявлений](#available-ad-units), если выбранное для рекламных блоков приложение имеет по крайней мере одну отправку и достигло разделе **соответствие требованиям COPPA** отображается в нижней части страницы [в магазине](../publish/the-app-certification-process.md#in-the-store) в приложении процесс сертификации.
