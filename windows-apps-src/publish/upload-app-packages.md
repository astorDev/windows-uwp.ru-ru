...
ms.prod: windows
ms.technology: uwp
keywords: Windows 10, uwp, пакеты, отправка, загружаемого пакета
...
Подробные сведения о составе и структуре пакета см. в разделе [Требования к пакету приложения](app-package-requirements.md). Также необходимо узнать о [как номера версий может повысить пакетов доставляются для конкретных клиентов](package-version-numbering.md) и [распределение пакетов для разных операционных систем](guidance-for-app-package-management.md).
...
Если мы обнаружения ошибок с помощью пакета при проверке его, мы будем отображение сообщений, чтобы знать, что происходит. Вам потребуется удалить пакет, устранить проблему и повторите попытку загрузки. Также могут появиться предупреждения о возможных проблемах, однако отправка пакета в этом случае не запрещается.
...
Пакетов загруженному перечисленные здесь, сгруппированные по целевой операционной системы. Отображается имя, версия и архитектура пакета. Для просмотра дополнительных сведений, таких как поддерживаемые языки, возможности приложения и размер файла для каждого пакета, щелкните **Показать сведения**.
