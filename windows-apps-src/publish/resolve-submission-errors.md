...
Если вы отправляете приложение UWP, вы можете заметить ошибку в процессе предварительной обработки, если файл пакета не является .appxupload, созданным Visual Studio для Магазина. Убедитесь, что при создании файла пакета приложения, выполните действия, описанные в [пакет приложения UWP с помощью Visual Studio](../packaging/packaging-uwp-apps.md) и загрузить только файл appxupload на странице " [пакеты](upload-app-packages.md) " для отправки, а не appx или appxbundle.
...
> [!IMPORTANT]
>  Если имя, которое вы хотите использовать недоступен, другое приложение может оно уже зарезервировано для этого имени. Если ваше приложение уже опубликовано под этим именем, или если вы считаете, что вы имеете право использовать его, [обратитесь в службу поддержки](https://go.microsoft.com/fwlink/p/?LinkId=331509).  
