...
Если вы отправляете приложение UWP, вы можете заметить ошибку в процессе предварительной обработки, если файл пакета не является .appxupload, созданным Visual Studio для Магазина. Убедитесь, что при создании файла пакета вашего приложения, выполните действия, описанные в [пакет приложения UWP с использованием Visual Studio](../packaging/packaging-uwp-apps.md) и только отправка файла .appxupload на странице [пакетов](upload-app-packages.md) подачи, не appx или .appxbundle.
...
## <a name="desktop-application"></a>Приложения для настольных систем

Если вы планируете отправить пакет, содержащий Win32 и UWP двоичные файлы, убедитесь, что создание этого пакета при помощи проекте упаковки Windows, доступные в Visual Studio 2017 обновления 4. При создании пакета с помощью шаблона проекта UWP может не иметь возможность отправки, упакуйте хранилище или sideload его на других компьютерах. Даже в том случае, если пакет публикацию успешно, могут работать непредвиденным образом на Компьютере пользователя. Дополнительные сведения см в [пакет приложения с помощью Visual Studio (рабочий стол мост)]( https://docs.microsoft.com/windows/uwp/porting/desktop-to-uwp-packaging-dot-net).
...
> [!IMPORTANT]
>  Если имя, которое вы хотите использовать недоступен, другого приложения может уже зарезервировано таким именем. Если ваше приложение уже публикуется в поле имя, или если у вас есть права на использование его [обратиться в службу поддержки](https://go.microsoft.com/fwlink/p/?LinkId=331509).  
