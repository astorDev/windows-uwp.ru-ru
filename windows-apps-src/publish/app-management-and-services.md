...
Если вы публикуете игру, можно включить [Xbox Live создателей программы](http://xbox.com/developers/creators-program) на этой странице. Это позволяет начать функции настройки и тестирования Xbox Live и в конечном счете публикация игра Xbox Live создателей программы.

Дополнительные сведения можно найти в [Начало работы с программой создателей Xbox Live](../xbox-live/get-started-with-creators/get-started-with-xbox-live-creators.md) и [Создайте новый заголовок Xbox Live создателей программы и их публикация в тестовой среде](../xbox-live/get-started-with-creators/create-and-test-a-new-creators-title.md).
...
Хранилище Microsoft коллекции интерфейсов API и покупки хранилища Майкрософт для получения доступа к сведения о владельце для приложений и надстроек, необходимо ввести связанного Azure AD идентификаторы клиента здесь. Заметьте, что для вступления изменений в силу может потребоваться до 16 часов.
...
f продукта интегрируется с Azure AD и вызывает API, запрашивающие [разрешения приложения или делегированы разрешения](https://developer.microsoft.com/graph/docs/concepts/permissions_reference) , которые требуются разрешения администратора, введите код клиента Azure AD. Это позволяет администраторам получить приложение для их разрешения grant организации для продукта действовать от имени всех пользователей в клиента.

Дополнительные сведения в разделе [запрос разрешения для всей клиента](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-v2-scopes#requesting-consent-for-an-entire-tenant).
...
В разделе **WNS/MPNS** предоставляет параметры для создания и отправки уведомлений пользователям вашего приложения. 

> [!TIP]
> Для приложений, UWP мы рекомендуем вам использовать параметр **уведомлений** на панели мониторинга. Эта функция позволяет отправлять уведомления для всех клиентов ваше приложение или для целевого подмножество Windows 10 клиентам, которые соответствуют требованиям определен в [Сегмент клиента](create-customer-segments.md). Дополнительные сведения см. в разделе [Отправка push-уведомлений пользователям вашего приложения](send-push-notifications-to-your-apps-customers.md).

В зависимости от типа пакет вашего приложения и его определенные требования можно также использовать один из следующих вариантов: 
...
-   **Службу push-уведомлений Microsoft (MPNS)** можно использовать на Windows Phone вместе с XAP-пакетами. Можно отправлять ограниченное число уведомлений без проверки подлинности, ничего здесь не настраивая, однако рекомендуется использовать уведомления с проверкой подлинности во избежание ограничения пропускной способности. Если вы используете MPNS, то необходимо Отправка сертификат в соответствующем поле на странице **WNS/MPNS** . Дополнительные сведения можно найти в разделе [Настройка веб-службы с проверкой подлинности для отправки push-уведомлений в Windows Phone 8](http://go.microsoft.com/fwlink/p/?LinkId=528736).
