...
ms.prod: windows
ms.technology: uwp
keywords: Windows 10, uwp, azure ad приложения, aad, пользователей, группы, несколько пользователей, нескольких пользователей
...
**Пользователей** в разделе Центр разработчиков Windows (в разделе **Параметры учетной записи**) позволяет использовать Azure Active Directory для добавления пользователей в свою учетную запись центра разработки. Каждому пользователю назначается роль (или набор пользовательских разрешений), которая определяет доступ пользователя к учетной записи. Вы также можете добавлять [группы пользователей](#groups) и [приложения Azure AD](#azure-ad-applications) для предоставления им доступа к вашей учетной записи Центра разработки.
...
1.  Выберите значок шестеренки (рядом с правом верхнем углу панели мониторинга) и выберите пункт **Параметры учетной записи**. В меню **Параметры** выберите **пользователей**.
...
-   **Добавление существующих пользователей**: выберите пользователей, которые уже существуют в телефонном справочнике организации и предоставить им доступ к учетной записи центра разработки, с помощью способов, описанных выше.
-   **Создание новых пользователей**: Создание новой учетных записей пользователей для добавления как каталог вашей организации и учетной записи центра разработки
...
1.  На странице **Users** (в разделе **Параметры учетной записи**) выберите **Добавить пользователей**и нажмите кнопку **Создать новых пользователей**.
...
1.  На странице **Users** (в разделе **Параметры учетной записи**) выберите **Добавить пользователей**и нажмите кнопку **Пригласить пользователей по электронной почте**.
...
1.  На странице **Users** (в разделе **Параметры учетной записи**) выберите имя учетной записи пользователя, которую требуется изменить.
2.  В нижней части страницы нажмите кнопку **сбросить пароль** .
...
1.  Выберите значок шестеренки (рядом с правом верхнем углу панели мониторинга) и выберите пункт **Параметры учетной записи**. В меню **Параметры** выберите **пользователей**.
2. Со страницы **Пользователи** выберите **Добавить группы**.
...
1.  На странице **Пользователи** (в разделе **Параметры учетной записи**) щелкните **Добавить группы**.
...
1.  1.  Выберите значок шестеренки (рядом с правом верхнем углу панели мониторинга) и выберите пункт **Параметры учетной записи**. В меню **Параметры** выберите **пользователей**.
...
1.  На странице **Users** (в разделе **Параметры учетной записи**) выберите **Добавить приложения Azure AD**.
2.  На следующей странице выберите **приложение новые Azure AD**.
...
1.  На странице **Users** (в разделе **Параметры учетной записи**) выберите имя приложения Azure AD.
    > [!TIP]
...
4.  Если вы хотите создать несколько разделов, выберите команду **Добавить другой ключ**.
...
1.  На странице **Users** (в разделе **Параметры учетной записи**) выберите имя пользователя, группы или учетная запись Azure AD приложений, которую требуется изменить.
...
На странице " **Пользователи** " (в разделе **Параметры учетной записи**) выберите ссылку, показанный в разделе **действия** для пользователя, которого вы хотите просмотреть журнал. Вы можете просматривать URL-адреса всех страниц, которые пользователь посетил за последние 30 дней.

<span id="remove" />

## <a name="remove-users-groups-and-azure-ad-applications"></a>Удаление пользователей, групп и приложений Azure Active Directory

Чтобы удалить пользователя, группы или Azure AD приложения из учетной записи центра разработки, выберите **Удалить** ссылку, которая отображается по имени на странице " **Пользователи** ". После подтверждения удаления учетной записи этот пользователь, группа или приложение Azure AD больше не смогут получать доступ к вашей учетной записи Центра разработки (если вы не добавите такой доступ позднее).
