...
При выборе пакета UWP из опубликованной отправки, Microsoft Store всегда использует версию наибольший номер пакета, которая пригодна для устройства Windows 10 пользователя. Это дает больше гибкости и контроля над тем, какие пакеты будут предоставлены пользователям для различных устройств. Важно, что вы можете отправлять эти пакеты в любом порядке. При этом необязательно предоставлять пакеты с более поздней версией с каждой последующей отправкой.
...
Если вы храните копии пакетов, вы получите возможность откатить пакет вашего приложения в магазине до предыдущего пакета Windows10 Если необходимо обнаружить проблемы с выпуском. Это временный способ ограничить негативные последствия для пользователей во время написания время, чтобы устранить проблему.
