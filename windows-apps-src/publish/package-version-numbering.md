...
> [!IMPORTANT]
> Для пакетов Windows 10 последняя (четвертая) часть номера версии зарезервирована для использования магазином и должна быть оставлена равной 0 при создании пакета (хотя магазин может изменить значение в этом разделе).
...
> [!TIP]
> Если ваше приложение также содержит пакеты для Windows 8, Windows 8.1 и Windows Phone 8.1, номер версии любого пакета Windows 10 всегда должен быть больше, чем любой из номера версии в эти пакеты. (Подробнее см. в разделе [Добавление пакетов для Windows 10 в ранее опубликованное приложение](https://docs.microsoft.com/en-us/windows/uwp/publish/guidance-for-app-package-management#adding-packages-for-windows-10-to-a-previously-published-app).)

Вы можете предоставить несколько пакетов Windows 10 с одинаковым номером версии. Однако пакеты с одинаковым номером версии не могут использовать одинаковую архитектуру, так как полное удостоверение, которое Магазин применяет для каждого пакета, должно быть уникальным. Подробнее: [**Удостоверение**](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-identity).

При предоставлении несколько пакетов Windows 10, которые используют один и тот же номер версии, архитектура (в порядке x64, x 86, ARM, нейтральная) будет использоваться для решить, какие из них имеет больший приоритет (при хранилище определяет, какой пакет предоставить на устройстве пользователя). При ранжировании пакетов приложений, использующих одну версию, учитывается самый высокий ранг архитектуры в пакете: пакет для архитектуры x64 получает более высокий ранг, чем пакет, который содержит только архитектуру x86.
...
Если вы храните копии, вы сможете откатить пакет вашего приложения в магазине до предыдущего пакета Windows 10 если обнаружите проблемы с выпуском. Это временный способ ограничить негативные последствия для пользователей, пока вы устраняете проблему.
