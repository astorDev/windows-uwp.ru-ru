...
> [!IMPORTANT]
> Для пакетов Windows 10 (UWP) последняя (четвертая) часть номера версии зарезервирована для использования магазином и должна быть оставлена равной 0 при создании пакета (хотя магазин может изменить значение в этом разделе). Другие разделы должны устанавливаться в виде целого числа от 0 до 65535 (за исключением первого раздела, который не может быть 0).
...
Это дает вам достаточно гибкости для развития вашего приложения с течением времени. Можно передать и отправлять новые пакеты, использующие меньшие номера версий, чтобы добавить поддержку для устройств с Windows 10, которые вы ранее не поддерживали, можно добавлять пакеты с большими более строгими зависимостями, чтобы воспользоваться преимуществами оборудования или Операционной системы или можно добавлять пакеты с большими, которые служат обновлениями для части или всей существующей клиентской базы.
...
Чтобы пользователи постепенно перешли на единый пакет для семейства универсальных устройств, что используют предоставляя промежуточные обновления для конкретных семейств устройств (включая те, которые воспользоваться API Windows10) можно использовать правила нумерации пакета. В примере ниже показано, как те же правила применяются согласованно через ряд отправки для одного приложения.
...
Если вы храните копии пакетов, вы получите возможность откатить пакет вашего приложения в магазине до предыдущего пакета Windows10 если следует обнаружить проблемы с выпуском. Это временный способ ограничить негативные последствия для пользователей во время создания времени, чтобы устранить проблему.
...
Чтобы исправить проблемы для пользователей, которые уже получили проблемный пакет, вы можете отправить пакет Windows10 с более высоким номером версии, чем у некорректного пакета как как можно скорее. После сертификации этой отправки все пользователи получат обновленный пакет, так как у него будет самый большой номер версии.
...
Кроме того номер версии пакетов Windows8.1 всегда должен быть больше номера версий всех пакетов Windows8 для того же приложения. Другими словами номер версии любого отправляемого пакета Windows8 должен быть меньше, чем номер версии любого пакета Windows8.1, отправляемого для того же приложения.
