...
ms.locfileid: "2795082"
---
# <a name="package-version-numbering"></a>Нумерация версий пакета

У каждого пакета, который вы предоставляете, должен быть номер версии (значение атрибута **Version** элемента **Package/Identity** в манифесте приложения). В Microsoft Store применяются некоторые правила, связанные с номерами версий, которые используются по-разному в различных версиях ОС.
...
> [!IMPORTANT]
> Для пакетов Windows 10 (четвертый) последней части номера версии зарезервирован для использования хранилища и должно остаться как 0 при создании пакета (несмотря на то, что хранилище могут изменить значение в данном разделе).

При выборе пакетаWindows 10 из опубликованной отправки Microsoft Store всегда использует версию пакета с наибольшим номером, которая пригодна для устройства пользователя. Это дает больше гибкости и контроля над тем, какие пакеты будут предоставлены пользователям для различных устройств. Важно, что вы можете отправлять эти пакеты в любом порядке. При этом необязательно предоставлять пакеты с более поздней версией с каждой последующей отправкой.

> [!TIP]
> Если ваше приложение также пакетов для Windows 8, Windows 8.1 и/или 8.1 Windows Phone, номер версии любой пакет Windows 10 всегда должно быть больше любой из номеров версий в эти пакеты. (Подробнее см. в разделе [Добавление пакетов для Windows 10 в ранее опубликованное приложение](https://docs.microsoft.com/en-us/windows/uwp/publish/guidance-for-app-package-management#adding-packages-for-windows-10-to-a-previously-published-app).)

Можно задать несколько пакетов Windows 10 с тем же номером версии. Однако пакеты с одинаковым номером версии не могут использовать одинаковую архитектуру, так как полное удостоверение, которое Магазин применяет для каждого пакета, должно быть уникальным. Подробнее: [**Удостоверение**](https://docs.microsoft.com/uwp/schemas/appxpackage/uapmanifestschema/element-identity).

Если имеется несколько пакетов Windows 10, использующие один и тот же номер версии, архитектура (в порядке x64, x 86, ARM, neutral) будет использоваться для решить, какой из них выше ранга (если хранилище определяет, какой пакет для предоставления устройства клиента). При ранжировании пакетов приложений, использующих одну версию, учитывается самый высокий ранг архитектуры в пакете: пакет для архитектуры x64 получает более высокий ранг, чем пакет, который содержит только архитектуру x86.
...
Если вы сохраните копии, можно будет откат пакет вашего приложения в магазине более ранних пакет Windows 10 при обнаружении проблем, связанных с выпуска. Это временный способ ограничить негативные последствия для пользователей, пока вы устраняете проблему.

Для этого создайте новый [отправки](app-submissions.md). Удалите проблемный пакет и отправьте старый пакет в Магазин. У пользователей, которые уже получили откатываемый пакет, по-прежнему будет проблемный пакет (поскольку номер версии старого пакета меньше). Но это не позволит другим пользователям скачать проблемный пакет, при этом приложение по-прежнему будет доступно в Магазине.
...
Для пакетов .appx, предназначенных для Windows8 и Windows8.1, то же правило применяется к архитектуре: номер версии пакета в новой отправке всегда должен быть больше номера последнего пакета для той же архитектуры, опубликованного в Microsoft Store.
...
> [!NOTE]
> Если ваше приложение также имеет пакеты Windows 10, номер версии Windows 10 пакетов должен быть выше, чем для каких-либо пакетов Windows 8, Windows 8.1 и/или 8.1 Windows Phone. (Подробнее см. в разделе [Добавление пакетов для Windows 10 в ранее опубликованное приложение](guidance-for-app-package-management.md#adding-packages-for-windows-10-to-a-previously-published-app).)
