...
При выборе пакета UWP из опубликованной отправки, в Microsoft Store всегда использует версию наибольший номер пакета, которая пригодна для устройства Windows 10 пользователя. Это дает больше гибкости и контроля над тем, какие пакеты будут предоставлены пользователям для различных устройств. Важно, что вы можете отправлять эти пакеты в любом порядке. При этом необязательно предоставлять пакеты с более поздней версией с каждой последующей отправкой.
...
Если вы храните копии пакетов, вы получите возможность отката пакета приложения в магазине до предыдущего пакета Windows 10, если необходимо обнаружить проблемы с выпуском. Это временный способ ограничить негативные последствия для пользователей, во время создания можно время, чтобы устранить проблему.
...
Чтобы исправить проблемы для пользователей, которые уже получили проблемный пакет, вы можете отправить новый пакет Windows 10 с более высоким номером версии, чем у некорректного пакета, как можно скорее. После сертификации этой отправки все пользователи получат обновленный пакет, так как у него будет самый большой номер версии.
