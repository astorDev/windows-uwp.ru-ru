...
- В столбце **Поле** содержится имя, связанное со всеми частями описания в Магазине. Эти имена соответствуют тем же элементам, которые вы предоставляете при создании описания в магазине в центре партнеров, однако некоторые имена немного отличаются. Для элементов, в которых можно ввести несколько элементов одинакового типа, будут отображаться несколько строк; до максимально доступного для вас числа. Например, для элемента **Возможности приложения** будут доступны строки **Возможность1**, **Возможность2**и так далее вплоть до строки **Возможность20** (так как вы можете предоставить до 20 возможностей приложения).
...
> [!TIP]
> Чтоб определить, какой актив соответствует URL-адресу, вы можете ввести URL-адрес в браузере, чтобы просмотреть изображение (или скачать видеотрейлер).  Вы должны выполнить вход в учетную запись центра партнеров в этот URL-адрес для работы.

Если вы хотите использовать новый ресурс, который не был добавлен ранее в центр партнеров, можно сделать путем импорта ваших описаний в качестве папки, а не как один CSV-файл. Необходимо создать папку, содержащую CSV-файл. Затем добавьте изображения из этой же папки в корневую папку или во вложенную папку. В поле вам потребуется ввести полный путь, включая имя корневой папки.
...
> [!IMPORTANT]
> Если ваше экспортированное описание содержит трейлеры, иметь в виду, что URL-адрес к трейлеру или его эскизу из CSV-файла полностью удалит удаленный файл из центра партнеров, и вы больше не будет доступа к нему (если он также используется в ano может описания, где оно не было удалено). 
...
Никакие данные из CSV-файла будут сохранены в центре партнеров, пока все ошибки в файле будут устранены, даже для полей без ошибок. После импорта CSV-файл, который не содержит ошибки, предоставленные вами сведения описания будут сохранены в центре партнеров и будет использоваться для этой отправки.
...
Для надстроек, импорта и экспорта описаний в магазине используется же процесса, описанного выше, за исключением того, что вы будете видеть только три поля, соответствующие [описания в магазине для надстройки](create-add-on-store-listings.md): **Описание**, **заголовок**и **StoreLogo300x300** () называется **значок** на странице описания в магазине в центре партнеров). Поле **Заголовок** является обязательным, а два остальных поля являются необязательными.
