...
ms.locfileid: "7419490"
---
# <a name="import-and-export-store-listings"></a>Импорт и экспорт описаний в Store

Вместо [ввода данных для описаний в магазине непосредственно в центре партнеров](create-app-store-listings.md)вы можете добавлять или обновлять сведения путем экспорта ваших описаний в виде CSV-файла, ввода ваших сведений и ресурсов и последующего импорта обновленного файла. Этот способ можно использовать для создания описаний с нуля или для обновления уже созданных описаний.

Эта возможность особенно полезна, если вы хотите создать описания в Store для вашего продукта на нескольких языках, так как можно скопировать и вставить одни и те же сведения в нескольких полей и легко внести любые изменения, которые должны применяться для определенных языков. Тем не менее этот способ нельзя использовать для создания и обновления [описания в магазине для определенной платформы](create-platform-specific-store-listings.md) для ранее опубликованных приложений, которые поддерживают более ранние версии ОС. 
...
Имейте в виду, что всегда можно создавать и обновлять описания непосредственно в центре партнеров (даже если ранее вы использовали метод импорта и экспорта). Обновление непосредственно в центре партнеров может быть проще, когда вы вносите небольшие изменения, но вы можете использовать любой из методов в любое время.
...
- В столбце **Поле** содержится имя, связанное со всеми частями описания в Магазине. Эти имена соответствуют тем же элементам, которые вы предоставляете при создании описания в магазине в центре партнеров, несмотря на то, что некоторые имена немного отличаются. Для элементов, в которых можно ввести несколько элементов одинакового типа, будут отображаться несколько строк; до максимально доступного для вас числа. Например, для элемента **Возможности приложения** будут доступны строки **Возможность1**, **Возможность2**и так далее вплоть до строки **Возможность20** (так как вы можете предоставить до 20 возможностей приложения).
- **Идентификатор** столбец содержит число, которое центр партнеров связывает с каждым полем. 
- В столбце **Тип** содержит общие рекомендации о том, какой тип данных следует предоставлять для этого поля, например **текст** или **относительный путь (или URL-адрес файла в центре партнеров)**. 
...
Удаление сведений в поле для одного языка без удаления всей строки происходит по-другому в зависимости от поля. При работе с полями с **Типом** **Текст** удаление сведений в поле приводит к удалению этой записи из описания на этом языке.  Однако удаление сведений в поле для изображения, такого как снимок экрана или логотип, не будет никакого эффекта; Предыдущее изображение по-прежнему будет использоваться, если вы удалите, изменив его непосредственно в центре партнеров. Удаление сведений для поля трейлера фактическому удалению этого трейлера из центра партнеров, поэтому убедитесь, что у вас есть копия всех необходимых файлов прежде чем сделать это.
...
Предоставление сведений для полей, которые соответствуют ресурсам, таким как изображения и трейлеры, представляет собой немного более сложный процесс. **Текст**, **Тип** для этих ресурсов не **относительный путь (или URL-адрес файла в центре партнеров)**. 
...
> [!TIP]
> Чтоб определить, какой актив соответствует URL-адресу, вы можете ввести URL-адрес в браузере, чтобы просмотреть изображение (или скачать видеотрейлер).  Необходимо войти в учетную запись центра партнеров в этот URL-адрес для работы.

Если вы хотите использовать новый ресурс, который не был добавлен ранее в центр партнеров, это можно сделать путем импорта ваших описаний в качестве папки, а не в качестве одного CSV-файла. Необходимо создать папку, содержащую CSV-файл. Затем добавьте изображения из этой же папки в корневую папку или во вложенную папку. В поле вам потребуется ввести полный путь, включая имя корневой папки.
...
Например, если именем вашей корневой папки является **my_folder** и вы хотите использовать изображение с именем **screenshot1.png** для **DesktopScreenshot1**, можно добавить изображение screenshot1.png в корень этой папки, а затем ввести **my_folder/screenshot1.png** в поле **DesktopScreenshot1**. Если вы создали папку с изображениями в корневой папке, а затем поместили в нее изображение screenshot1.jpg, введите в поле **my_folder/images/screenshot1.png**. Обратите внимание, что после импорта описаний с использованием папки пути к вашим изображениям будут преобразованы в URL-адреса к файлам в центре партнеров следующем экспорте ваших описаний. Можно скопировать и вставить эти URL-адреса, чтобы воспользоваться ими повторно (например, использовать одни и те же ресурсы в нескольких языках описания). 

> [!IMPORTANT]
> Если ваше экспортированное описание содержит трейлеры, иметь в виду, что URL-адрес к трейлеру или его эскизу из CSV-файла полностью удалит удаленный файл из центра партнеров, и вы больше не сможете доступа к нему (если он также используется в ano может описания, где оно не было удалено). 
...
Никакие данные из CSV-файла будут сохранены в центре партнеров, пока все ошибки в файле устраните, даже для полей без ошибок. После импорта CSV-файл, который не содержит ошибки, предоставленные вами сведения описания будут сохранены в центре партнеров и будет использоваться для этой отправки.

Вы можете продолжать вносить обновления в свои описания путем импорта другого обновленного CSV-файла или внесения изменений непосредственно в центре партнеров.

## <a name="add-ons"></a>Надстройки

Для надстроек, импорта и экспорта описаний в магазине используется том же процессе, описанные выше, за исключением того, что вы будете видеть только три поля, соответствующие [описания в магазине для надстройки](create-add-on-store-listings.md): **Описание**, **заголовок**и **StoreLogo300x300** () называется **значок** странице описания в магазине в центре партнеров). Поле **Заголовок** является обязательным, а два остальных поля являются необязательными.
