...
В этом примере показано определение настраиваемых XamlLight, которое применяется к целевой UIElements и кисти многоцветный центре внимания.
...
Затем можно применить этот индикатор на любом XAML элементов интерфейса пользователя или кисти светло-их. В этом примере показаны различные возможных вариантов использования.

> [!Important]
> Для [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), удалите два вхождения `local:OrangeSpotLight.IsTarget="True"` из ниже разметку. Вложенные свойства уже задано в коде.
...
Результаты этой XAML выглядеть следующим образом.

![Примеры элементов освещена с xaml light](images/orange-spot-light.png)
