...
Поскольку ListView (`ThumbnailList`) — это элемент управления XAML, который уже прокручивается, прокрутка должна быть связана с родительским элементом (`ContentPanel`), когда достигается элемент самого верхнего уровня и дальнейшая прокрутка невозможна. (ContentPanel является, которому нужно применить модификаторы источника). Чтобы это сделать, необходимо присвоить ScrollViewer.IsVerticalScrollChainingEnabled **true** в разметке ListView. Кроме того, потребуется задать значение **Всегда** для режима сцепления в свойстве VisualInteractionSource.
