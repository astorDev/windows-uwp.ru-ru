...
Для каждого из различных модальностей входной указатель существует несколько входных состояний может быть входных данных, в котором перемещается: Наведите указатель мыши, нажата, нажата и переместить. PointerPositionPropertySet сохраняет положение указателя мыши и пера только в состояниях наведения, нажатия, нажатия и перемещения.
