...
- Возможность связать объект с подсветкой перемещать их вместе независимо от остальной части сцены для включения сценариев, таких как [Отображение](/design/style/reveal.md) Fluent выделения.
...
| [DistantLight](/uwp/api/windows.ui.composition.distantlight) | Бесконечно большого дорогостоящим источника света, излучающий свет в одном направлении. Как солнце. |
...
**ExclusionsFromTargets** дает возможность удаления освещения визуального объекта или поддерева визуальных элементов так же, как добавлять конечные объекты. Дочерние элементы в дереве корнем, визуальный элемент, который будет исключен есть, не освещаются в результате.
...
**Количество света** | Вы можете использовать любые два источники света не внешнее композиции в любом сочетании интересующих пользовательского интерфейса. Внешнее источники света не ограничены; затруднительно, точки и удаленных источников света — это.
**Время существования** | CompositionLight могут возникнуть жизненным циклом условий (пример: сборщик мусора может перезапуститься объект light, прежде чем он используется).  Мы рекомендуем отслеживание ссылку на ваш источники света, добавив источники света в качестве элемента для управления жизненным циклом приложения.
**Преобразования** | Источники света должен быть размещен в узле выше пользовательского интерфейса, который использует эффектов, как [преобразования перспективы](/design/layout/3-d-perspective-effects.md) в вашей визуальной структуры необходимо нарисовать надлежащим образом.
...
> [!NOTE]
> Если несколько источников света нажатия одной Visual или всякий раз, когда значение цвета света получает достаточно большой, чтобы превышает 1,0, цвет освещения может измениться из-за закрепления канала цвета источников света.
...
- Определение объектов для освещения: предназначенные для свет в соответствующих визуальных элементов.
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect) часто используется для создания материалов. SceneLightingEffect является результатом используется, если вы хотите добиться более сложной, например включение отражающие свойства на изображение и/или предоставление иллюзию глубины с обычной карты. SceneLightingEffect предоставляет возможность для настройки пользовательского интерфейса с помощью свойства освещения, например зеркальное и суммы. Можно настроить эффекты освещения с остальным конвейера эффектов, позволяя по отдельности наложения и составления реакции различных освещения с содержимым.

> [!NOTE]
> Освещение сцены не создает тени; оно является результатом акцент на двумерной отрисовки.  Он не учитывают рассмотрения 3D освещения сценариях, которые включают моделями реального освещения, включая тени.
