...
| Тип | Описание |
| --- | --- |
| [AmbientLight](/uwp/api/windows.ui.composition.ambientlight) | Источник света, излучающий без направления света, который отображается отражаться на все в сцене. |
...
**ExclusionsFromTargets** дает возможность удаления освещения визуального объекта или поддерева визуальных элементов так же, как добавление конечных объектов. Дочерние элементы в дереве доверию, визуальный элемент, который будет исключен есть, не освещаются в результате.
...
Концепция | Сведения
--- | ---
**Фоновое освещение** | Добавление без фоновое освещение сцены будут отключены все существующие света.  Элементы, отличных от внешнего освещения не предназначено будут отображаться черным.  Для освещения окружающих визуальные элементы, не целевых по света в естественный способ, используйте внешнего освещения в сочетании с других источников света.
**Количество света** | Вы можете использовать любые два источники света не внешний композиции в любом сочетании для ориентировать ваш пользовательский Интерфейс. Внешнее источники света не запрещены; затруднительно, точки и дорогостоящим источники света являются.
**Время существования** | CompositionLight могут возникнуть жизненным циклом условий (пример: сборщик мусора может сбросить объект light, прежде чем он используется).  Рекомендуется отслеживание ссылку на ваш источники света, добавив источники света в качестве элемента для обеспечения управления временем существования приложения.
...
**Целевые объекты и пространство координат** | CoordinateSpace — это visual пространство, в котором все необходимо настроить свойства источников света. CompositionLight.Targets должен находиться в дереве CoordinateSpace.
...
**Direction** | Направления света. Направление, в котором указания источника света задается относительно его Visual [CoordinateSpace](/uwp/api/windows.ui.composition.distantlight.coordinatespace) .
**Пространство координат** | Каждый визуальный элемент имеет неявный трехмерном пространстве координат. Направление X — слева направо. Вдоль оси Y — сверху вниз. Направлении Z является точкой за пределы плоскости. Исходное точки координаты — верхний левый угол визуального объекта, а единицы — независимых пикселей устройства (DIP). Смещение света, определенные в этом координат.
...
> [!NOTE]
> Если несколько источников света нажатия же Visual или всякий раз, когда значение цвета света получает достаточно большой, чтобы превышать 1.0, цвет освещения может измениться из-за закрепления канала цвета источников света.
...
[SceneLightingEffect](/uwp/api/Windows.UI.Composition.Effects.SceneLightingEffect) часто используется для создания материала. SceneLightingEffect происходит, используются, когда вы хотите добиться более сложной, например включение отражающие свойства на изображение и/или предоставление иллюзию глубины с обычной карты. SceneLightingEffect предоставляет возможность для настройки пользовательского интерфейса с помощью свойства освещения, например зеркальное и сумм. Вы можете дополнительно настроить эффекты освещения с остальным конвейера эффекты, позволяя по отдельности наложения и составления реакций различных освещения с содержимым.

> [!NOTE]
> Освещение сцены не создают тени; Это акцент на двумерной отрисовки эффекта.  Он не учитывают рассмотрения 3D освещения сценариях, которые включают моделями реального освещения, включая тени.
