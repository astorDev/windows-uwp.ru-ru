...
В следующем примере кода показано скрипта взаимодействия. В примере объединяются типы из контактную зону на основе среды выполнения Windows композиции Windows вместе с типами взаимодействия заголовки и код, который визуализирует текст, с помощью COM-based DirectWrite и Direct2D API-интерфейсы. В примере используется [ **BeginDraw** ](https://docs.microsoft.com/windows/desktop/api/windows.ui.composition.interop/nf-windows-ui-composition-interop-icompositiondrawingsurfaceinterop-begindraw) и [ **EndDraw** ](https://docs.microsoft.com/windows/desktop/api/windows.ui.composition.interop/nf-windows-ui-composition-interop-icompositiondrawingsurfaceinterop-enddraw) чтобы сделать его простым для взаимодействия между этими технологиями. В примере используется DirectWrite для размещения текста, а затем она использует Direct2D их вывода. Графическое устройство композиции принимает устройство Direct2D непосредственно во время инициализации. Это позволяет **BeginDraw** для возврата **ID2D1DeviceContext** указатель на интерфейс, который значительно эффективнее, чем создать контекст Direct2D программы-оболочки для возвращаемого приложения Интерфейс ID3D11Texture2D в каждой операцией рисования.

Существует два примера кода ниже. Во-первых, [ C++/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) пример (которая завершена), а затем C++/CX примере (который опускает DirectWrite и Direct2D части примера).

Для использования C++/WinRT приведенном ниже примере кода, сначала создайте **приложения Core (C++/WinRT)** проект в Visual Studio (требования, см. в разделе [поддержка Visual Studio C++/WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)). При создании проекта, выберите в качестве требуемой версии **Windows 10 версии 1803 (10.0; Сборка 17134)** . Это версия, для которого этот код был разработан и протестирован. Замените содержимое файла вашего `App.cpp` исходный файл кода с помощью кода ниже, а затем построить и запустить. Приложение отображает строку «Hello, World!» в черный текст на прозрачном фоне.
