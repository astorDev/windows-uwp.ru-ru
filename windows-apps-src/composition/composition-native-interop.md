...
В следующем примере кода показан сценария взаимодействия. Пример объединяет типа из среды выполнения Windows под управлением контактную зону Windows Composition вместе с типами из взаимодействия заголовки и код, который отображает текст, с помощью интерфейсов API Direct2D и DirectWrite на основе модели COM. В примере используется [**BeginDraw**](https://msdn.microsoft.com/library/windows/apps/mt620059.aspx) и [**EndDraw**](https://msdn.microsoft.com/library/windows/apps/mt620060) для облегчения управления ими оптимальную для взаимодействия между этими технологиями. В примере DirectWrite для размещения текста, и он использует Direct2D для его отрисовки. Графическое устройство композиции принимает устройство Direct2D непосредственно во время инициализации. Это позволяет **BeginDraw** возвратить указатель интерфейса **ID2D1DeviceContext** , являющийся значительно эффективнее создания контекста Direct2D и оболочки для возвращаемого интерфейса ID3D11Texture2D при каждой операции рисования.

Существует два примера кода ниже. Во-первых, [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) пример (это полный), а затем C + +/ CX пример кода (который пропускает DirectWrite и Direct2D части примера).

Для использования C + +/ WinRT кода ниже пример, сначала создайте новый **приложения основных компонентов (C + +/ WinRT)** проекта в Visual Studio (требования, см. в разделе [Поддержка Visual Studio для C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)). При создании проекта, выберите в качестве своей целевой версии **Windows 10 версии 1803 (10.0; Сборка 17134)**. Это версия, для которой этот код был создан и протестированными. Замените содержимое вашего `App.cpp` исходный файл кода с помощью приведенного ниже кода, а затем выполнить сборку и запустить. Приложение отображает строку «Hello, World!» в черный текст на прозрачный фон.
