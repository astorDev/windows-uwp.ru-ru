...
Для использования C + +/ WinRT кода ниже пример, сначала создайте новый **приложения основных компонентов (C + +/ WinRT)** проекта в Visual Studio (требования, см. в разделе [Поддержка Visual Studio для C + +/ WinRT и VSIX](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt#visual-studio-support-for-cwinrt-xaml-and-the-vsix)). При создании проекта, выберите в качестве своей целевой версии **Windows 10 версии 1803 (10.0; Сборка 17134)**. Это версия, для которой этот код был создан и протестированными. Замените содержимое вашего `App.cpp` исходный файл кода с помощью приведенного ниже кода, а затем выполнить сборку и запустить. Приложение отображает строку «Hello, World!» в черный текст на прозрачный фон.
