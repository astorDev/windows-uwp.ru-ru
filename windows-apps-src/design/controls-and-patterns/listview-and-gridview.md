...
ListView отображает данные в одном столбце и упорядочивает их по вертикали. ListView лучше подходит для элементов, использующим текст в качестве точки фокуса, и коллекций, которые предназначены для чтения сверху вниз (т. е. упорядочены в алфавитном порядке). Несколько распространенных вариантов использования ListView включают в себя списки сообщений и результатов поиска.
...
Подробнее о концепциях привязки данных см. в разделе [Общие сведения о привязке данных](https://docs.microsoft.com/windows/uwp/data-binding/data-binding-quickstart).
