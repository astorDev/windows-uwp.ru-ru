...
Если подсказку закрывает содержимое, которое он ссылается, вы можете настроить ее размещение, с помощью нового свойства **PlacementRect** . PlacementRect привязывает положение подсказки и выступать в качестве области, не будут загородить подсказку, предоставленные достаточно места на экране для рисования подсказку за пределами этой области. Вы можете указать начало координат прямоугольника относительно владельцем подсказку и высоту и ширину области исключений. Свойство [размещения](/uwp/api/windows.ui.xaml.controls.tooltip.placement) определит, если подсказки следует нарисовать выше, ниже, слева или справа от PlacementRect. 
