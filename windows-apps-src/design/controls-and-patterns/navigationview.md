...
_Представление переходов поддерживает сверху и левой панели навигации или меню_
...
>
> Для некоторых функций NavigationView, например для навигации _верхнего_ , требуется Windows 10, версия 1809 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) или более поздней версии или [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

## <a name="is-this-the-right-control"></a>Выбор подходящего элемента управления

NavigationView — это элемент управления адаптивных навигации, который лучше всего подойдет для:
...
> Свойство PaneDisplayMode требуется Windows 10, версия 1809 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) или более поздней версии или [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/).
...
- У вас есть 5 или меньше категории навигации верхнего уровня, которые одинаково важны и любые дополнительные навигации верхнего уровня, категории, которые в конечном счете в раскрывающемся меню переполнения считаются столь важны.
...
- У вас есть 5-10 менее важно навигации верхнего уровня категории.
...
:::row:::
    ...
    Она отображается только значки, пока открыт и располагается слева от содержимого.</br>
...
:::row:::
    ...
    Только кнопку "меню" отображается, пока не будет открыта области. При открытии, он располагается слева от содержимого.</br>
...
По умолчанию PaneDisplayMode задано значение Auto. В автоматический режим представление навигации адаптирует между LeftMinimal при узком, чтобы LeftCompact окна и затем слева по мере шире окна. Дополнительные сведения см. в разделе [адаптивное поведение](#adaptive-behavior) .
...
![Макет навигации сверху](images/topnav-anatomy.png)<br/>
_Верхней части макета_

![Макет навигации влево](images/leftnav-anatomy.png)<br/>
_Макет навигации влево_

### <a name="pane"></a>Панель

Свойство PaneDisplayMode можно использовать для размещения панели над содержимым или слева от содержимого.
...
- Объекты [NavigationViewItemSeparator](/uwp/api/windows.ui.xaml.controls.navigationviewitemseparator) . Разделители для группировки элементов навигации. Значение свойства [Opacity](/uwp/api/windows.ui.xaml.controls.navigationviewitemseparator.opacity) 0 для отрисовки в качестве разделителя места.
...
- Кнопка меню для переключения на панели открывать и закрывать. Когда панель открыта в более крупных окнах приложения, вы можете скрыть эту кнопку с помощью свойства [IsPaneToggleButtonVisible](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.IsPaneToggleButtonVisible).

Представление переходов имеет кнопки "Назад", который размещается в верхнем левом углу панели. Тем не менее он не автоматически обрабатывают обратной навигации и добавлять содержимое в стек переходов назад. Чтобы реализовать навигацию в обратном направлении, см. в разделе [обратной навигации](#backwards-navigation) раздела.

Вот Анатомия подробные панели для позиции верхней и левой панели.
...
#### <a name="pane-title-and-header"></a>Название области и заголовка
...
:::row:::
    ...
    _Расположенной слева области заголовка_<br>
    :::column-end:::
:::row-end:::
...
:::row:::
    :::column:::
    ![Пользовательские панели содержимого основные навигации](images/navview-freeform-pane-top.png)<br>
     _Пользовательское содержимое для верхней панели_<br>
    :::column-end:::
    :::column:::
...
Области заголовка вертикально выравнивается с кнопкой навигацией в левой области положении и располагается под области в положение верхней панели. Он имеет фиксированную высоту в 52 пикс. Она содержит заголовок страницы категории выбранного навигации. Заголовок закреплен в верхней части страницы и выступает в качестве точки отсечения прокрутки для области содержимого.
...
![Пример область содержимого представления навигации](images/nav-content.png)<br/>
...
Мы рекомендуем поля 12 пикселям для области содержимого, когда NavigationView находится в **минимальном** режиме поля значение 24 пикселя в противном случае.

## <a name="adaptive-behavior"></a>Адаптивное поведение

По умолчанию представление переходов автоматически изменяет режим отображения в зависимости от доступного экранного пространства экрана. Свойства [CompactModeThresholdWidth](/uwp/api/windows.ui.xaml.controls.navigationview.compactmodethresholdwidth) и [ExpandedModeThresholdWidth](/uwp/api/windows.ui.xaml.controls.navigationview.expandedmodethresholdwidth) указать точки останова, в которых изменении режима отображения. Вы можете изменить эти значения, чтобы настроить поведение режима отображения адаптивных.
...
- Развернутого левой для ширины окна большой (1008 пикселей или более поздней версии).
...
Дополнительные сведения о размеров окон для адаптивное поведение см. в разделе [размеров экрана и точки останова](../layout/screen-sizes-and-breakpoints-for-responsive-design.md).
...
Второй распространенный шаблон адаптивного — использовать расширенные левой панели на больших окон ширины и только кнопки меню на обоих ширины окна средних и мелких.
...
_Навигация представление «минимальный» адаптивное поведение_

Чтобы настроить это поведение, установите CompactModeThresholdWidth по ширине, в котором вы хотите, чтобы панели, чтобы свернуть. Здесь он изменяется по умолчанию 640 для 1007. Вы также должны задать ExpandedModeThresholdWidth, чтобы убедиться, что не конфликтующие значения.
...
Третий распространенный шаблон адаптивных является использование развернутого левой панели на больших окон ширины и LeftCompact, только для значков, панель навигации для обоих средних и мелких окон ширины.
...
### <a name="no-adaptive-behavior"></a>Не адаптивное поведение

Чтобы отключить автоматическое адаптивное поведение, установите PaneDisplayMode значение, отличных от автоматически. Здесь имеет значение для LeftMinimal, чтобы только кнопку "меню" отображается независимо от того, ширина окна.

![Левый навигации не адаптивное поведение](images/adaptive-behavior-none.png)<br/>
_Представление навигации с помощью PaneDisplayMode значение LeftMinimal_

```xaml
<NavigationView PaneDisplayMode="LeftMinimal" />
```

Как описано ранее в разделе _режимов отображения_ , вы можете задать панели, чтобы всегда быть в верхнем, всегда развернутого, всегда compact или всегда минимальный. Вы также можете управлять режимов отображения самостоятельно в коде приложения. В следующем разделе показан пример этого.

### <a name="top-to-left-navigation"></a>Сверху слева навигации

При использовании верхнего навигации в приложении, элементов навигации свернуть как уменьшение ширины окна в меню переполнения. Если узкой окна приложения, он может обеспечить удобство работы для переключения PaneDisplayMode сверху LeftMinimal навигации, а не позволяя все, что свернуть элементы в меню переполнения.

Мы рекомендуем использовать верхней части большие размеры экрана и левой навигационной на небольшие размеры окна, когда:

- У вас есть набор одинаково важные навигации верхнего уровня категорий, чтобы будет отображаться вместе, таким образом, если одну категорию в этот набор не помещаются на экране, можно свернуть, чтобы левой навигационной, чтобы предоставить им равно важности.
- Вы хотите сохранить гораздо содержимого места максимально небольшое окно размеров.
...
> [!TIP]
> При использовании AdaptiveTrigger.MinWindowWidth визуальное состояние активируется при шире, чем указанный Минимальная ширина окна. Это означает, что по умолчанию XAML определяет узкой окна, а VisualState изменений, которые применяются, когда окно получает шире. По умолчанию PaneDisplayMode для представление переходов автоматически, поэтому при ширина окна составляет меньше или равно CompactModeThresholdWidth, LeftMinimal навигации используется. Когда окно получит шире, VisualState переопределяет значение по умолчанию и используется верхней части.

## <a name="navigation"></a>Навигация

Представление переходов не выполняет каких-либо задач переходов автоматически. Когда пользователь касается элемента навигации, представление навигации показан этот элемент как выбранный и вызывает событие [ItemInvoked](/uwp/api/windows.ui.xaml.controls.navigationview.ItemInvoked) . Если касание приводит нового элемента, также вызывается событие [SelectionChanged](/uwp/api/windows.ui.xaml.controls.navigationview.SelectionChanged) .

Вы можете обработать событие, либо для выполнения задач, связанных с запрошенный навигации. Выбор, вы должны обрабатывать зависит от поведение, которое требуется для вашего приложения. Как правило перейдите на страницу запрошенный и обновить заголовок представления навигации в ответ на эти события.

**ItemInvoked** возникает в любое время, пользователь касается элемента навигации, даже если он уже установлен. (Элемент также могут вызываться с эквивалентным действием, с помощью мыши, клавиатуры или других методов ввода. Дополнительные сведения см. в разделе [ввода и взаимодействия](../input/index.md).) При переходе в обработчике ItemInvoked по умолчанию будет перезагружен страницы и повторяющиеся запись добавляется в стеке навигации. Если при вызове элемента, следует запретить перезагрузки страницы или убедитесь, что запись повторяющихся не создается в стек переходов при перезагрузке страницы. (См. Примеры кода).

**SelectionChanged** может инициироваться вызов элемента, которое не текущий выбранный пользователем или изменяя программно выбранного элемента. Если изменения выделения происходит потому, что пользователь вызывает элемент, сначала возникновении события ItemInvoked. Если программный изменения выделения, ItemInvoked не вызывается.

### <a name="backwards-navigation"></a>Навигация в обратном направлении

NavigationView имеет встроенную кнопку «Назад»; но, как и в случае с переходов вперед, он не выполняет обратной навигации автоматически. Когда пользователь касается кнопки "Назад", возникает событие [BackRequested](/uwp/api/windows.ui.xaml.controls.navigationview.BackRequested) . Вы обрабатываете это событие для выполнения обратной навигации. Дополнительные сведения и примеры кода, см. в разделе [Журнал навигации и навигация в обратном навигации](../basics/navigation-history-and-backwards-navigation.md).
...
- [IsBackButtonVisible](/uwp/api/windows.ui.xaml.controls.navigationview.IsBackButtonVisible): используйте, чтобы отобразить или скрыть кнопки "Назад". Это свойство принимает значение перечисления [NavigationViewBackButtonVisible](/uwp/api/windows.ui.xaml.controls.navigationviewbackbuttonvisible) и установлено значение **Auto** по умолчанию. При сворачивании кнопки не зарезервированы для него в макете.
- [Значением IsBackEnabled](/uwp/api/windows.ui.xaml.controls.navigationview.IsBackEnabled): используйте, чтобы включить или отключить кнопки "Назад". Вы можете выполнить привязку данных, это свойство, чтобы свойство [CanGoBack](/uwp/api/windows.ui.xaml.controls.frame.cangoback) кадра навигации. **BackRequested** не вызывается, если **значением IsBackEnabled** является **"false"**.
...
В примере показано, рекомендуется настроить данные переходов, который будет работать для многих распространенных сценариев. В нем также показано, как реализовать обратную навигацию с помощью обратной кнопки и клавиатуры навигации NavigationView.

В этом коде предполагается, что ваше приложение содержит страницы с следующие имена для перехода к: _домашней страницы_, _AppsPage_, _GamesPage_, _MusicPage_, _MyContentPage_и _SettingsPage_. Код для этих страниц не отображается.

> [!IMPORTANT]
> Сведения о страницах приложения хранится в [ValueTuple](https://docs.microsoft.com/dotnet/api/system.valuetuple). Эта структура требует, что минимальная версия проекта приложения должно быть SDK 17763 или более поздней версии. Если вы используете версию WinUI NavigationView для более ранних версий Windows 10, вместо этого можно использовать [пакет System.ValueTuple NuGet](https://www.nuget.org/packages/System.ValueTuple/) .

> [!IMPORTANT]
> Этот код показано, как использовать NavigationView версию [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/) . Если вместо этого использовать версию платформы NavigationView минимальную версию для проекта приложения должны быть SDK 17763 или более поздней версии. Чтобы использовать версию платформы, удалите все ссылки `muxc:`.
...
Чтобы изменить фон панели, можно переопределить ресурсы темы XAML, используются для отрисовки фона в каждого режима. (Этот способ используется вместо одного свойства PaneBackground поддержки другой фон для различных режимов отображения.)

В этой таблице показано, какие ресурсы темы используется в каждом режиме отображения.
...
В этом примере показано, как переопределить ресурсы темы в файле App.xaml. При переопределении ресурсы темы, необходимо всегда предоставить «По умолчанию» и «Высокая контрастность» словари ресурсов, как минимум и словари для «Света» или «Dark» ресурсы при необходимости. Дополнительные сведения см. в разделе [ResourceDictionary.ThemeDictionaries](/uwp/api/windows.ui.xaml.resourcedictionary.themedictionaries).

> [!IMPORTANT]
> Этот код показано, как использовать версию [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/) AcrylicBrush. Если вместо этого использовать версию платформы AcrylicBrush минимальную версию для проекта приложения должны быть пакет SDK 16299 или более поздней версии. Чтобы использовать версию платформы, удалите все ссылки `muxm:`.
