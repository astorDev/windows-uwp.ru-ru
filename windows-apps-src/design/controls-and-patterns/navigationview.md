...
- Значки не описывают четко категорий навигации приложения.

:::row:::
    :::column:::
    ### <a name="left"></a>Влево
    Панель развернута и расположенный слева от содержимого.</br>
...
По умолчанию PaneDisplayMode установлено значение Auto. В автоматический режим представление переходов адаптируется между LeftMinimal при узком, чтобы LeftCompact окна и затем слева по мере ширина окна. Дополнительные сведения см. в разделе [адаптивное поведение](#adaptive-behavior) .
...
![Макет левое меню навигации](images/leftnav-anatomy.png)<br/>
_Левой навигационной макета_

### <a name="pane"></a>Панель

Чтобы расположить панель над содержимым или слева от содержимого можно использовать свойство PaneDisplayMode.
...
Расположенной слева области также содержит следующие действия:
...
1. Кнопкой "Параметры" (необязательно)
...
Содержимое можно поместить в нижнем колонтитуле панели путем добавления его в свойство [PaneFooter](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneFooter) .

:::row:::
    :::column:::
    ![Верхняя нижнем колонтитуле панели](images/navview-freeform-footer-top.png)<br>
...
Текстовое содержимое можно поместить в области заголовка панели, установив свойство [PaneTitle](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneTitle) . Он принимает строку и отображает текст рядом с кнопку "меню".

Добавление нетекстовой содержимого, например изображения или логотип, можно разместить любого элемента в заголовке панели путем добавления его в свойство [PaneHeader](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneHeader) .
...
Содержимое можно поместить в области путем добавления его в свойство [PaneCustomContent](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.PaneCustomContent) .
...
Второй распространенный шаблон адаптивного — использовать расширенные левой панели на больших окон ширины, а только кнопки меню на обоих ширины окна малых и средних.
...
Третий распространенный шаблон адаптивных — использовать расширенные левой панели на больших окон ширины и LeftCompact, только для значков, панель навигации для обоих ширины окна малых и средних.

Мы рекомендуем использовать этот формат, если:

- Очень важно всегда отображаться все параметры навигации на экране.
- Категории навигации могут появляться четко со значками.

![Компактный адаптивное поведение навигации влево](images/adaptive-behavior-compact.png)<br/>
_Навигация представление «compact» адаптивное поведение_
...
Чтобы отключить автоматическое адаптивное поведение, установите PaneDisplayMode значение, отличное от автоматически. Здесь он имеет значение для LeftMinimal, чтобы только кнопку "меню" отображается независимо от того, ширина окна.
...
Как описано ранее в разделе _режимов отображения_ , вы можете задать панель всегда быть в верхнем, всегда развернутого, всегда compact или всегда минимальный. Вы также можете управлять режимов отображения самостоятельно в коде приложения. В следующем разделе показан пример этого.
...
- У вас есть набор одинаково важные навигации верхнего уровня категорий для отображения друг с другом, таким образом, если одну категорию в этот набор не помещаются на экране, можно свернуть, чтобы левой навигационной присвоит им одинаково важны.
...
В этом примере показано, как можно использовать NavigationView верхняя панель навигации на большие размеры экрана и левой навигационной панели на небольших размеры экрана. Его можно было адаптировать для навигации слева, удаляя параметры навигации _верхнего_ в VisualStateManager.

В примере рекомендуемый способ настройки данных навигации, которые будут работать для многих распространенных сценариев. В нем также показано, как реализовать обратную навигацию с помощью обратной кнопки и клавиатуры навигации NavigationView.
...
- область — сплошным цветом серую развернуто слева, рядом с содержимым (в левом режиме).
...
В этом примере показано, как переопределить ресурсы темы в файле App.xaml. При переопределении ресурсы темы, необходимо также обеспечивать «По умолчанию» и «Высокая контрастность» словари ресурсов, как минимум и словари для «Света» или «Dark» ресурсы при необходимости. Дополнительные сведения см. в разделе [ResourceDictionary.ThemeDictionaries](/uwp/api/windows.ui.xaml.resourcedictionary.themedictionaries).
