...
![Панель навигации расширен](images/displaymode-left.png)
...
NavigationView — это элемент управления адаптивной переходов, который лучше всего подойдет для:
...
:::row:::
    :::column:::
    ### <a name="left"></a>Влево
    Отображение развернутого левой панели положение.
    :::column-end:::
    :::column span="2":::
...
- Желание категорий очень заметными навигации с помощью меньше места для другого содержимого приложения.
...
- Значки не четко описаны категории навигации вашего приложения.

:::row:::
    :::column:::
    ### <a name="leftcompact"></a>LeftCompact
    Отображает тонких полосой со значками слева.
    :::column-end:::
    :::column span="2":::
    ![compact панель навигации](images/displaymode-leftcompact.png)
    :::column-end:::
:::row-end:::
...
:::row:::
    :::column:::
    <b>Левая панель навигации</b><br>
    ![Навигации левый нижний области](images/navview-freeform-footer-left.png)<br>
    :::column-end:::
    :::column:::
     <b>Верхней панели навигации</b><br>
...
При соблюдении требований к оборудованию и программному обеспечению NavigationView автоматически использует на панели [Акриловый материал](../style/acrylic.md) и [эффект](../style/reveal.md) только на панели слева.

## <a name="header"></a>Заголовок

![navview универсального изображение области заголовка](images/nav-header.png)

Области заголовка вертикально выравнивается с кнопкой навигацией в левой области положении и располагается под области в положение верхней панели. Он имеет фиксированную высоту в 52 пикселей. Она содержит заголовок страницы категории выбранного навигации. Заголовок закреплен в верхней части страницы и выступает в качестве точки отсечения прокрутки для области содержимого.
...
Второй распространенный шаблон адаптивного — использовать расширенные левой панели на больших окон ширины и гамбургера для обоих ширины окна средних и небольших.
...
Третий распространенный шаблон адаптивного — использовать расширенные левой панели на больших окон ширины и левую только для значка панель навигации для обоих ширины окна средних и небольших. Хорошим примером этого является приложение "Почта".
...
### <a name="top-to-left-navigation"></a>Верхнего левого навигации

Мы рекомендуем использовать верхней части большие размеры экрана и навигации слева от небольшой размер окна, когда:

- У вас есть набор одинаково важных навигации верхнего уровня категории для отображения друг с другом, таким образом, если одну категорию в этот набор не помещаются на экране, можно свернуть, чтобы левой навигационной присвоит им равно важности.
...
В модели вкладки привязаны выбора и фокусировки. Действие, обычно смены фокус также сместится выбора. В следующем примере правой arrowing перемещаются индикатор выделения отображается для экранной лупы. Этого можно достичь, задав для свойства [SelectionFollowsFocus](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.selectionfollowsfocus) включена.
...
Для замены содержимого при изменении выбора вкладки, вы можете использовать метод [NavigateWithOptions](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.frame.NavigateToType) кадра с FrameNavigationOptions.IsNavigationStackEnabled задано значение False и NavigateOptions.TransitionInfoOverride значение соответствующие для параллельных Анимация перетаскивания. Например см. в [примере кода](#code-example) ниже.
...
Ниже приведен пример начала до конца как NavigationView можно включить с помощью верхняя панель навигации на большие размеры экрана и левой навигационной панели на небольших размеры.
...
![Пример NavigationView-комплексного](images/nav-code-example.gif)
...
Скользил + вид Если ваше приложение содержит страницы, использующие ScrollViewer и панели навигации верхнего в заданное положение, мы рекомендуем наличие содержимого прокрутки под верхней навигационной панели. Это дает типом залипание заголовок поведение в приложение.
...
Если ваше приложение имеет очень длинным прокрутке содержимого, может потребоваться следует рассмотреть возможность включения закрепленные заголовки, которые присоединяются к верхней навигационной панели и формы плавного поверхности. 

![закрепленные заголовок navview прокрутки](images/nav-scroll-stickyheader.png)
...
Иногда Если пользователь прокрутку, может потребоваться скрыть панель навигации, достичь, задав свойство [IsPaneVisible](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.navigationview.ContentOverlay) на NavigationView значение "false".
