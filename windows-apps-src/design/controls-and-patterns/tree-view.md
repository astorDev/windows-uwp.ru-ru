...
- (Ознакомительная версия) TreeViewItem как корень шаблон элемента TreeView
...
Чтобы создать иерархическое представление, используя привязку данных, задайте иерархическую структуру свойству TreeView.ItemsSource. Затем в ItemTemplate, установите дочерние коллекции элементов в свойство TreeViewItem.ItemsSource.
...
См. в разделе _Иерархическое представление, используя привязку данных_ "Примеры" Полный код.
...
Нужно ли использовать ItemsSource или RootNodes, фактические элементы, которые используются для отображения каждого узла — под названием «контейнер» – — это объект [TreeViewItem](/uwp/api/windows.ui.xaml.controls.treeviewitem) . Задать стиль контейнера, с помощью TreeView свойства ItemContainerStyle или ItemContainerStyleSelector.
...
Выбора или отмены выбора родительский узел будет выбрать или отменить выбор всех дочерних элементов этого узла. Если некоторые, но не все из выбраны дочерние элементы, принадлежащие родительский узел, флажок для родительского узла отображается как неопределенное состояние (заполненный черный ящик).

![Выделение нескольких элементов в иерархическом представлении](images/treeview-selection.png)
...
В этом примере показано, как создать такое же представление дерева аналогичен предыдущему примеру. Тем не менее вместо создания иерархии данных в XAML, данные создается в коде и привязывается к свойству ItemsSource иерархическом представлении. (Обработчики событий кнопки, показанный в предыдущем примере применяются в этом примере также.)
