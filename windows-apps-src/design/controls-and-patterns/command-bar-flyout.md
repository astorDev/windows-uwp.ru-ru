...
TextCommandBarFlyout отображает текст команды в элементах управления TextBox, TextBlock, RichTextBlock, RichEditBox и PasswordBox. Команды автоматически настраиваются соответствующим образом для выделенного текста. Используйте CommandBarFlyout для замены команд текста по умолчанию на элементы управления текстом.
...
Упреждающее вызову команды отображаются автоматически, когда пользователь взаимодействует с элементом, связанные с команды. Например команды форматирования текста может всплывающие при выборе текста в текстовом поле. В этом случае всплывающий элемент панели команд не имеет фокуса. Вместо этого он передает соответствующие команды вблизи элемент, который пользователь взаимодействует с. Если пользователь не взаимодействовать с командами, они являются закрыта.
...
Вы можете использовать CommandBarFlyout в способ или даже оба типа привязки.
...
![Пример всплывающий элемент панели свернутой команд](images/command-bar-flyout-img-collapsed.png)

> _Всплывающий элемент панели свернутой команд_

![Пример всплывающий элемент панели команд развернутого](images/command-bar-flyout-img-expanded.png)

> _Всплывающий элемент панели команд развернутого_
...
Для отображения команд в контекстном меню, вы обычно назначить всплывающий элемент свойство [ContextFlyout](/uwp/api/windows.ui.xaml.uielement.contextflyout) элемента пользовательского интерфейса. Таким образом, открыв всплывающий элемент обрабатывается с помощью элемента, и вам не нужно выполнять никаких действий более.
...
Здесь AppBarElementContainer используется для добавления дополнительных элементов всплывающий элемент панели команд. SplitButton добавляется к основным командам, чтобы разрешить выбор цвета. StackPanel добавляется вспомогательные команды, чтобы разрешить макет более сложные элементы управления масштабированием.
...
> [!NOTE]
> В этом примере показан только всплывающий элемент панели команд пользовательского интерфейса, он не реализует все команды, которые отображаются. Дополнительные сведения о реализации команд см. в разделе [кнопки](buttons.md) и [основы проектирования команд](../basics/commanding-basics.md).

![Всплывающий элемент панели команд с кнопкой "комбинированный режим"](images/command-bar-flyout-split-button.png)

> _Всплывающий элемент панели свернутой команд с открытым SplitButton_

![Всплывающий элемент панели команд с сложный пользовательский Интерфейс](images/command-bar-flyout-custom-ui.png)
...
## <a name="create-a-context-menu-with-secondary-commands-only"></a>Создание контекстного меню с только вспомогательные команды

Можно использовать CommandBarFlyout с только вспомогательные команды как [контекстное меню](menus.md)вместо MenuFlyout.
...
![Всплывающий элемент панели команд с в раскрывающемся меню кнопки](images/command-bar-flyout-dropdown.png)
...
> _Всплывающий элемент панели команд текста при выделении текста_
...
TextCommandBarFlyout не может быть изменен и автоматически управляется каждый текстовый элемент управления. Тем не менее можно заменить значение по умолчанию TextCommandBarFlyout пользовательские команды.

- Чтобы заменить значение по умолчанию TextCommandBarFlyout, которая отображается при выделении текста, можно создать пользовательские CommandBarFlyout (или другой тип всплывающий элемент) и назначить его свойству **SelectionFlyout** . Если SelectionFlyout значение **null**, команды не будут отображаться на выбор.
- Для замены по умолчанию TextCommandBarFlyout, которые отображаются в контекстном меню, назначьте пользовательские CommandBarFlyout (или другой тип всплывающий элемент) свойство **ContextFlyout** элемента управления текстом. Если ContextFlyout значение **null**, вместо TextCommandBarFlyout отображается всплывающее меню, показанный в предыдущих версиях управления текстом.
...
- [Пример команд XAML](https://go.microsoft.com/fwlink/p/?LinkId=620019)
