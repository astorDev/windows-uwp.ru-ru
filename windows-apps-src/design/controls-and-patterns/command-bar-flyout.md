...
- В развернутом режиме отображаются основные и дополнительные команды. (Если элемент управления имеет только вспомогательные элементы, они отображаются так, как элемент MenuFlyout.)
...
Используйте элемент управления CommandBarFlyout для отображения набор команд для пользователя, таких как кнопки и элементы меню, в контексте текущего элемента на холсте приложения.
...
Для вызова всплывающий элемент или меню, связанный с элементом на холсте пользовательского интерфейса обычно существует два способа: _упреждающего вызов_ и _реактивный вызова_.

Упреждающее вызова команды отображаются автоматически, когда пользователь взаимодействует с элементом, который команды связаны с. Например команды форматирования текста может мультимедийном пользователь выбирает текст в текстовом поле. В этом случае всплывающий элемент панели команд не имеет фокуса. Вместо этого он представляет соответствующих команд вблизи элемент, который пользователь взаимодействует с. Если пользователь не взаимодействовать с помощью команд, они являются закрыта.

Реактивные вызова команды отображаются в ответ на явное действие пользователя для запроса команды; Например щелкните правой кнопкой мыши. Это соответствует традиционных концепцию [контекстное меню](menus.md).
...
В этом примере показано, как создать всплывающий элемент панели команд и использовать его как заблаговременно, так и проектировать. При касании изображения, всплывающий элемент отображается в свернутом режиме. При отображении как контекстного меню, всплывающий элемент отображается в развернутом режиме. В любом случае пользователь может развернуть или свернуть всплывающий элемент после открытия.
...
При отображении контекстно-зависимые команды заблаговременно, только основные команды отображения по умолчанию (свернуто всплывающий элемент панели команд следует). Поместите наиболее важные команды в коллекцию основных команд и дополнительные команды, которые обычно поступает в контекстном меню в коллекцию вспомогательные команды.

Заблаговременно отобразить команды, обычно обработки событий [щелкните](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click) или [Tapped](/uwp/api/windows.ui.xaml.uielement.tapped) для отображения всплывающего элемента панель команд. Задайте всплывающий элемент [ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode) **промежуточный** или **TransientWithDismissOnPointerMoveAway** , чтобы открыть всплывающее окно в свернутом режиме без прохождения фокусировки.

Начиная с Windows 10 Insider Preview, текстовые элементы управления имеют свойство **SelectionFlyout** . При назначении всплывающий элемент этого свойства, автоматически отображается при выборе текста.

### <a name="show-commands-reactively"></a>Отображение команд проектировать

Контекстно-зависимые команды Показывать факту контекстного меню, дополнительные команды отображаются по умолчанию (должен быть развернут всплывающий элемент панели команд). В этом случае всплывающий элемент панели команд могут иметь основные и дополнительные команды, или только вспомогательные команды.

Для отображения команд в контекстном меню, обычно назначить всплывающий элемент в свойство [ContextFlyout](/uwp/api/windows.ui.xaml.uielement.contextflyout) элемента пользовательского интерфейса. Таким образом, открыв всплывающий элемент обрабатывается элемента, и вам не нужно ничего более.

Если нужно обработать, отображается всплывающий элемент самостоятельно (например, в событии [RightTapped](/uwp/api/windows.ui.xaml.uielement.righttapped) ), задать всплывающий элемент [ShowMode](/uwp/api/windows.ui.xaml.controls.primitives.flyoutbase.showmode) в **стандартных** открывать всплывающий элемент в развернутом режиме и установить на него фокус.
...
По умолчанию элементы панели команд добавляются в коллекцию **PrimaryCommands**. Эти команды отображаются на панели команд и отображаются в свернутой и расширенного режима. В отличие от CommandBar основные команды не автоматически переполнения ко второстепенным командам и может быть усечен.
...
Можно добавить другие элементы управления всплывающий элемент панели команд, обтекания в AppBarElementContainer. Это позволяет добавлять элементы управления, такие как [DropDownButton]() или [SplitButton]()или добавить контейнеры как [StackPanel]() для создания более сложного пользовательского интерфейса.

> [!NOTE]
> Чтобы добавить в коллекции основной или вспомогательной команды всплывающий элемент панели команд, элемент должен реализовать интерфейс [ICommandBarElement](/uwp/api/windows.ui.xaml.controls.icommandbarelement) . AppBarElementContainer является оболочкой, реализующий этот интерфейс, поэтому можно добавить элемент на панели команд, даже если он не реализовано самого интерфейса.
...
> [!NOTE]
> В этом примере показан только командной строке всплывающий элемент пользовательского интерфейса, не реализует все команды, которые отображаются. Дополнительные сведения о реализации команд см. в разделе [кнопок](buttons.md) и [основы проектирования команд](../basics/commanding-basics.md).
...
![Командной строке всплывающий элемент с в раскрывающемся меню кнопки](images/command-bar-flyout-button-menu.png)
...
[TextCommandBarFlyout](/uwp/api/microsoft.ui.xaml.controls.textcommandbarflyout) является всплывающего окна специальные команды, содержащий команды для редактирования текста. Каждый текстовый элемент управления автоматически показывает TextCommandBarFlyout как контекстного меню (щелкните правой кнопкой мыши), или при выборе текста. Всплывающий элемент панели команд текст адаптируется для выделения текста, чтобы отображать только соответствующих команд.
...
| Проверки правописания | Когда IsSpellCheckEnabled имеет **значение true** и с ошибками текст выделен. |
...
TextCommandBarFlyout невозможно изменить и управляется автоматически с каждым элементом управления текстом. Тем не менее можно заменить значение по умолчанию TextCommandBarFlyout пользовательские команды.

- Для замены TextCommandBarFlyout, которые отображаются на выделение текста по умолчанию, можно создать пользовательские CommandBarFlyout (или другой тип всплывающий элемент) и назначьте его свойству **SelectionFlyout** . Если SelectionFlyout значение **null**, команды не будут отображаться на выбор.
- Для замены по умолчанию TextCommandBarFlyout, которые отображаются в контекстном меню, назначьте пользовательские CommandBarFlyout (или другой тип всплывающий элемент) свойство **ContextFlyout** для элемента управления текстом. Если ContextFlyout значение **null**, вместо TextCommandBarFlyout отображается всплывающее меню, показанный в предыдущих версиях текстовый элемент управления.
