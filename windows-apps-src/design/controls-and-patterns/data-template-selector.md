...
Некоторые элементы визуального отображения элемента списка можно контролировать с помощью всего одного шаблона данных путем привязки определенных свойств. Например, элементы могут иметь разные значки путем привязки к свойству источника значка в шаблоне данных и присвоения каждому элементу другого значения для этого свойства источника значка. Это позволит повысить производительность по сравнению с использованием DataTemplateSelector.
