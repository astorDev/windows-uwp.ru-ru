...
ms.locfileid: "4416604"
---
# <a name="buttons"></a>Кнопки

Кнопка предоставляет пользователю возможность вызвать немедленное действие. Некоторые кнопки предназначенного для конкретных задач, например навигации, повторяющиеся действия или отображения меню.
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | (Ознакомительная версия) Кнопки переключения с двух сторон. Одной стороны переключает или отключить, и другой стороне откроется меню.
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) — это кнопка, которая отображается шеврона визуальный индикатор было присоединенных всплывающего, содержащий дополнительные параметры. Он имеет такое же поведение, как стандартные кнопки всплывающего элемента; внешний вид отличается.
...
> [!TIP]
> См. Дополнительные сведения о всплывающих элементов [меню и контекстные меню](menus.md).

### <a name="example---drop-down-button"></a>Пример: раскрывающийся список кнопки

В этом примере показано, как создать раскрывающийся список кнопки с всплывающий элемент, содержащий команды для выравнивания абзацев в элементе RichEditBox. (Дополнительные сведения и код см. [поле с форматом](rich-edit-box.md)).
...
- При открытии раскрывающегося списка вызова обработки элементов в раскрывающемся списке вниз, чтобы изменить оба, что параметр выбран, а затем вызвать его. Очень важно для вызова элемента всплывающего элемента, так как событие не возникает при использовании сенсорного управления нажатия кнопки.
...
Разделение кнопки-переключателя обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для отключение списков, пока раскрывающийся список используется для выбора стиля в списке.
