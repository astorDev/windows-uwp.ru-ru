...
Кнопка предоставляет пользователю возможность вызвать немедленное действие. Некоторые кнопки являются специализированными для определенных задачах, например, навигация, повторяющиеся действия или отображения меню.
...
[RepeatButton](/uwp/api/windows.ui.xaml.controls.primitives.repeatbutton) | Кнопка, которая порождает событие Click постоянно нажатой кнопке.
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | (Ознакомительная версия) Кнопки переключения с двух сторон. Одной стороны переключает или отключить, а другая сторона открывает меню.
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) — это кнопка, которая отображается шеврона как визуальный индикатор, есть ли у присоединенных всплывающего, содержащий дополнительные параметры. Он имеет такое же поведение, как стандартные кнопки всплывающего элемента; внешний вид отличается.
...
В этом примере показано, как создать раскрывающийся список кнопки с всплывающий элемент, содержащий команды для выравниванием абзацев в элементе RichEditBox. (Дополнительные сведения и код см. [поле с форматом](rich-edit-box.md)).
...
- При открытии раскрывающегося дескриптор вызова элементов в раскрывающемся списке вниз, чтобы изменить оба, что параметр выбран, а затем вызвать его. Очень важно для вызова элемента всплывающего элемента, так как событие не возникает при использовании сенсорного управления нажатия кнопки.
...
Разделение кнопки-переключателя обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для отключение списков, пока раскрывающийся список позволяет выбрать стиль списка.
...
- ToggleSplitButton не имеет свойства **IsThreeState** или **неопределенный** события элемента.
