...
[HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) | Кнопка A, который стилизованная как гиперссылка, используемыми для навигации. Подробнее см. на странице [Гиперссылки](hyperlinks.md).
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | (Ознакомительная версия) Кнопки переключения с двух сторон. Одной стороны переключает Вкл. / выкл., и другой стороне откроется меню.
...
> Исключение: для навигации по мастеру используйте кнопки "Вперед" и "Назад". Для других типов обратной навигации верхнего уровня, или используйте [Кнопка "Назад"](../basics/navigation-history-and-backwards-navigation.md).
...
Раскрывающийся список кнопки наследует события Click, но обычно не используется. Вместо этого использовать свойство всплывающий элемент для присоединения всплывающий элемент и вызывать действия с помощью параметров меню в всплывающий элемент. Всплывающий элемент автоматически открывается при нажатии кнопки.
...
![Раскрывающийся список кнопки с помощью команд выравнивания](images/drop-down-button-align.png)
...
— Это обычное поведение для разворачивающуюся кнопку:
...
- При открытом раскрывающегося дескриптор вызова элементов в раскрывающемся списке вниз, чтобы изменить оба, что параметр выбран, а затем вызвать его. Очень важно для вызова элемента всплывающего элемента, так как событие не возникает при использовании сенсорного управления нажатия кнопки.
...
Разделение кнопки-переключателя обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для включить списки или отключить, пока раскрывающийся список позволяет выбрать стиль списка.
...
- ToggleSplitButton не имеет свойства **IsThreeState** или события **неопределенный** элемента.
...
![Разделение кнопки-переключателя для выбора список стилей](images/toggle-split-button-open.png)
