...
Платформа XAML предоставляет стандартный элемент управления кнопки, а также несколько элементов управления специальные кнопки.
...
[RepeatButton](/uwp/api/windows.ui.xaml.controls.primitives.repeatbutton) | Кнопка, которая вызывает событие Click постоянно при нажатии.
[HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) | Кнопка A, стилем как гиперссылка, используемыми для навигации. Подробнее см. на странице [Гиперссылки](hyperlinks.md).
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | (Ознакомительная версия) Кнопки-переключателя с двух сторон. Одной стороны переключает включено выключено, и другой стороне откроется меню.

| **Получить библиотеку пользовательского интерфейса Windows** |
| - |
| DropDownButton, кнопкой-разделителем и ToggleSplitButton включен в состав библиотеки пользовательского интерфейса Windows, пакет NuGet, содержащий новые элементы управления и функции пользовательского интерфейса для приложений UWP. Дополнительные сведения, включая инструкции по установке см. [Общие сведения о библиотеке пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/). |

| **API-интерфейсы платформы** | **API-интерфейсы библиотеки пользовательского интерфейса Windows** |
| - | - |
| [Событие click](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click), [свойство команды](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.command) | [Класс DropDownButton](/uwp/api/microsoft.ui.xaml.controls.dropdownbutton), [SplitButton класса](/uwp/api/microsoft.ui.xaml.controls.splitbutton), [класс ToggleSplitButton](/uwp/api/microsoft.ui.xaml.controls.togglesplitbutton) |
...
Не используйте кнопки, когда действие — переход на другую страницу; Вместо этого используйте [HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) . Подробнее см. на странице [Гиперссылки](hyperlinks.md).
...
Используйте **SplitButton** , если вы хотите, чтобы пользователь мог инициировать немедленное действие, или выбрать дополнительные параметры независимо друг от друга.
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) — это кнопка, которая отображается шеврона как визуальный индикатор, что на нем установлены присоединенных всплывающего, содержащий дополнительные параметры. Он имеет такое же поведение, как обычная кнопка с всплывающий элемент; только внешний отличается.

Раскрывающийся список кнопки наследует события Click, но обычно она не используется. Вместо этого можно использовать свойство всплывающий элемент для присоединения всплывающий элемент и вызова действия с помощью параметров меню в всплывающий элемент. Всплывающий элемент автоматически открывается при нажатии кнопки.

> [!TIP]
> См. Дополнительные сведения о всплывающие элементы [меню и контекстные меню](menus.md).

### <a name="example---drop-down-button"></a>Пример: раскрывающийся список кнопки

В этом примере показано, как создать раскрывающийся список кнопки с всплывающий элемент, содержащий команды для выравнивания абзацев в элементе RichEditBox. (Дополнительные сведения и кода, см. [поле с форматом](rich-edit-box.md)).
...
> [!NOTE]
> При вызове сенсорный ввод, кнопку с разделением ведет себя как раскрывающийся список, кнопка; Оба частями кнопки вызова всплывающий элемент. С помощью других методов ввода пользователь может вызывать либо половине кнопки отдельно.
...
- При открытии раскрывающегося дескриптор вызова элементов в раскрывающемся списке до оба изменения, что параметр выбран, а затем вызвать его. Очень важно для вызова элемента всплывающего элемента, так как событие не возникает при использовании сенсорного управления нажатия кнопки.
...
Разделение кнопки-переключателя обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для включить или отключить, списки во время раскрывающегося списка используется для выбора стиля в списке.
...
- ToggleSplitButton не имеет свойства **IsThreeState** или **неопределенный** события.
- Свойство [ToggleSplitButton.IsChecked](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischecked) является просто **bool**, не **допускающие значение NULL bool**.
...
### <a name="example---toggle-split-button"></a>Пример: кнопка разделения переключатель
