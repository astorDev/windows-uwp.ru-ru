...
ms.locfileid: "8922435"
---
# <a name="buttons"></a>Кнопки

Кнопка предоставляет пользователю возможность вызвать немедленное действие. Некоторые кнопки предназначенного для определенных задачах, например навигации, повторяющиеся действия или отображения меню.
...
------- | -----------
[Кнопка](/uwp/api/windows.ui.xaml.controls.button) | Запускает немедленное действие. Можно использовать событие Click или привязка команд.
...
[HyperlinkButton](/uwp/api/windows.ui.xaml.controls.hyperlinkbutton) | A, кнопку стилем как гиперссылка, используемыми для навигации. Подробнее см. на странице [Гиперссылки](hyperlinks.md).
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) | Кнопка с шеврона для открытия присоединенных всплывающего.
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | Кнопки-переключателя с двух сторон. Одной стороны переключает Вкл. / выкл., и другой стороне откроется меню.

| **Получить библиотеку пользовательского интерфейса Windows** |
| - |
| DropDownButton, кнопкой-разделителем и ToggleSplitButton включены в состав библиотеки пользовательского интерфейса Windows, пакет NuGet, содержащий новые элементы управления и функции пользовательского интерфейса для приложений UWP. Дополнительные сведения, включая инструкции по установке см. в разделе [Общие сведения о библиотеке пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/). |

| **API-интерфейсы платформы** | **API библиотеки пользовательского интерфейса Windows** |
| - | - |
| [Событие click](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click), [свойство команды](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.command) | [Класс DropDownButton](/uwp/api/microsoft.ui.xaml.controls.dropdownbutton), [SplitButton классов](/uwp/api/microsoft.ui.xaml.controls.splitbutton), [класс ToggleSplitButton](/uwp/api/microsoft.ui.xaml.controls.togglesplitbutton) |
...
Используйте **DropDownButton** , когда кнопка имеет всплывающий элемент, содержащий дополнительные параметры. По умолчанию шеврона предоставляет визуальную индикацию, что кнопка включает в себя всплывающий элемент.
...
> DropDownButton требуется Windows 10, версия 1809 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) или более поздней версии или [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) — это кнопка, которая отображается шеврона как визуальный индикатор, есть ли у присоединенных всплывающего, содержащий дополнительные параметры. Он имеет то же поведение, как стандартные кнопки всплывающего элемента; только внешний отличается.

Раскрывающийся список кнопки наследует события Click, но обычно не используется. Вместо этого использовать свойство всплывающий элемент для присоединения всплывающий элемент и вызова действия с помощью параметров меню в всплывающий элемент. Всплывающий элемент автоматически открывается при нажатии кнопки.
...
В этом примере показано, как создать кнопку раскрывающегося списка с всплывающий элемент, содержащий команды для выравнивания абзацев в элементе RichEditBox. (Дополнительные сведения и кода, см. [поле с форматом](rich-edit-box.md)).
...
## <a name="create-a-split-button"></a>Создать кнопку "комбинированный режим"

> SplitButton требуется Windows 10, версия 1809 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) или более поздней версии или [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

[SplitButton](/uwp/api/windows.ui.xaml.controls.splitbutton) состоит из двух частей, которые могут быть вызваны отдельно. Одна из частей ведет себя как обычная кнопка и вызывает немедленное действие. Другие части вызывает всплывающий элемент, содержащий дополнительные параметры, которые пользователь может выбрать из.

> [!NOTE]
> При вызове сенсорный ввод, кнопку с разделением ведет себя как кнопку раскрывающегося списка; Оба частями кнопки вызова всплывающий элемент. С помощью других методов ввода пользователь может вызывать либо половине кнопки отдельно.
...
- При открытии раскрывающегося дескриптор вызова элементов в раскрывающемся списке вниз, чтобы оба изменить, что параметр выбран, а затем вызвать его. Очень важно для вызова элемента всплывающего элемента, так как событие не возникает при использовании сенсорного управления нажатия кнопки.

> [!TIP]
> Существует множество способов разместить элементы в раскрывающемся списке и обрабатывать их вызова. Если вы используете ListView или GridView, один из способов — обрабатывать событие SelectionChanged. Если это сделать, задайте [SingleSelectionFollowsFocus](/uwp/api/windows.ui.xaml.controls.listviewbase.singleselectionfollowsfocus) значение **false**. Это позволяет пользователям перемещаться с помощью клавиатуры без вызова элемента на каждое изменение параметров.

### <a name="example---split-button"></a>Пример: разворачивающуюся кнопку

В этом примере показано, как создать разворачивающуюся кнопку, которая используется для изменения цвета переднего плана выбранного текста в элементе RichEditBox. (Дополнительные сведения и кода, см. [поле с форматом](rich-edit-box.md)).
...
> ToggleSplitButton требуется Windows 10, версия 1809 ([SDK 17763](https://developer.microsoft.com/windows/downloads/windows-10-sdk)) или более поздней версии или [Библиотеки пользовательского интерфейса Windows](https://docs.microsoft.com/uwp/toolkits/winui/).

[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) состоит из двух частей, которые могут быть вызваны отдельно. Одна из частей ведет себя как кнопки-переключателя, можно включить или отключить. Другие части вызывает всплывающий элемент, содержащий дополнительные параметры, которые пользователь может выбрать из.
...
В следующем примере показано, как кнопка разделения переключателя могут быть использованы для Включение или выключение форматирования списка и изменить стиль списка в элементе RichEditBox. (Дополнительные сведения и кода, см. [поле с форматом](rich-edit-box.md)).

![Разделение кнопки-переключателя для выбора списка стилей](images/toggle-split-button-open.png)
