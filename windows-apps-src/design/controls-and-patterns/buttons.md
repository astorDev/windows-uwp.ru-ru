...
Разделение выключатель обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для отключение списков, пока раскрывающийся список позволяет выбрать стиль списка.
