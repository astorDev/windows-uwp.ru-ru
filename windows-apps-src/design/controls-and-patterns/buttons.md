...
ms.locfileid: "4502100"
---
# <a name="buttons"></a>Кнопки

Кнопка предоставляет пользователю возможность вызвать немедленное действие. Некоторые кнопки являются специализированными для конкретной задачи, например навигации, повторяющиеся действия или отображения меню.
...
[SplitButton](/uwp/api/windows.ui.xaml.controls.splitbutton) состоит из двух частей, которые могут быть вызваны отдельно. Одна из частей ведет себя как обычная кнопка и вызывает немедленное действие. Другие части вызывает всплывающий элемент, который содержит дополнительные параметры, которые пользователь может выбрать из.

> [!NOTE]
> При вызове сенсорный ввод, кнопку с разделением ведет себя как раскрывающийся список кнопки. Оба частями кнопки вызова всплывающий элемент. С помощью других методов ввода пользователь может вызывать либо половине кнопки отдельно.
...
- При открытии раскрывающегося вызов обработки элементов в раскрывающемся списке до оба изменения, что параметр выбран, а затем вызвать его. Очень важно для вызова элемента всплывающего элемента, так как событие не возникает при использовании сенсорного управления нажатия кнопки.
...
Разделение кнопки-переключателя обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для отключение списки, пока раскрывающийся список позволяет выбрать стиль списка.
...
- ToggleSplitButton нет свойству **IsThreeState флажка** или **неопределенный** событий.
...
В следующем примере показано, как кнопка разделения переключателя могут быть использованы для включения форматирования или отключение списка и изменить стиль списка в элементе RichEditBox. (Дополнительные сведения и код см. [поле с форматом](rich-edit-box.md)).
