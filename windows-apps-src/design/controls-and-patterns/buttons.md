...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) | Кнопка с шеврона, чтобы открыть присоединенных всплывающего.
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) | Кнопки переключения с двух сторон. Одной стороны переключает включено выключено, и другой стороне откроется меню.
...
| - | - |
| [Событие click](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.click), [свойство команды](/uwp/api/windows.ui.xaml.controls.primitives.buttonbase.command) | [Класс DropDownButton](/uwp/api/microsoft.ui.xaml.controls.dropdownbutton), [класс SplitButton](/uwp/api/microsoft.ui.xaml.controls.splitbutton) [ToggleSplitButton класса](/uwp/api/microsoft.ui.xaml.controls.togglesplitbutton) |
...
[DropDownButton](/uwp/api/windows.ui.xaml.controls.dropdownbutton) — это кнопка, которая отображается шеврона визуальный индикатор было присоединенных всплывающего, содержащий дополнительные параметры. Он имеет такое же поведение, как стандартные кнопки всплывающего элемента; только внешний отличается.

Раскрывающийся список кнопки наследует события Click, но обычно не используется. Вместо этого можно использовать свойство всплывающий элемент для присоединения всплывающий элемент и вызывать действия с помощью параметров меню в всплывающий элемент. Всплывающий элемент автоматически открывается при нажатии кнопки.

> [!TIP]
> Дополнительные сведения о всплывающие элементы см. в разделе [меню и контекстные меню](menus.md).

### <a name="example---drop-down-button"></a>Пример: раскрывающийся список кнопки

В этом примере показано, как создать раскрывающийся список кнопки с всплывающий элемент, содержащий команды для выравнивание абзацев в элементе RichEditBox. (Дополнительные сведения и код см. [поле с форматом](rich-edit-box.md)).
...
- Когда пользователь нажимает кнопку часть, обработайте событие Click для вызова вариантом, который в данный момент выбран в раскрывающемся списке.
...
[ToggleSplitButton](/uwp/api/windows.ui.xaml.controls.togglesplitbutton) состоит из двух частей, которые могут быть вызваны отдельно. Одна из частей ведет себя как переключатель, который можно включить или отключить. Другие части вызывает всплывающий элемент, который содержит дополнительные параметры, которые пользователь может выбрать из.

Разделение кнопки-переключателя обычно используется для включения или отключения функции, когда возможность имеет несколько параметров, которые пользователь может выбрать из. Например в редакторе документ, он могут быть использованы для включить списки или отключить, а раскрывающийся список позволяет выбрать стиль списка.
...
В отличие от [ToggleButton](/uwp/api/windows.ui.xaml.controls.primitives.togglebutton)ToggleSplitButton нет неопределенное состояние. В результате вы должны учитывать эти различия:

- ToggleSplitButton нет, либо свойству **IsThreeState флажка** или **неопределенный** событие.
...
- ToggleSplitButton имеет только [IsCheckedChanged](/uwp/api/windows.ui.xaml.controls.togglesplitbutton.ischeckedchanged) события; он не имеет отдельных событий **Checked** и **Unchecked** .
...
![Разделение кнопки-переключателя для выбора стили списков](images/toggle-split-button-open.png)
