...
По умолчанию на панели команд отображаются строки кнопки со значками и необязательный «Показать больше» кнопку, которая представлена многоточие \[••• выбора\]. Так выглядит панель команд, созданная с помощью кода из примера, показанного ниже. Она отображается в закрытом компактном состоянии.
...
- «См. Дополнительные» \[••• выбора\] кнопка отображается в правой части панели. Нажав клавишу «Си дополнительные» \[••• выбора\] кнопка показывает основную команду метки и открывает меню переполнения, если существуют дополнительные команды. Кнопка не будет видна при отсутствии значков основных команд или второстепенных значков. Чтобы изменить поведение, заданное по умолчанию, используйте свойство [OverflowButtonVisibility](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.commandbar.overflowbuttonvisibility).
- Меню переполнения отображается, только когда панель команд открыта и заполнено значение свойства [SecondaryCommands](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.commandbar.secondarycommands). Основные команды будут перемещаться в область SecondaryCommands, если пространство ограничено. Чтобы изменить поведение, заданное по умолчанию, используйте свойство [IsDynamicOverflowEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.commandbar.isdynamicoverflowenabled).
...
- *Если команда, которая будет постоянно отображаться на страницах, лучше оставить эту команду в нужном расположении.*
- *Рекомендуется поместить Accept, Да и отклонить, нет, ОК команды слева от будет отменить. Согласованность позволяет перевели в системе и помогает им перенести свои знания, навигации приложения из приложения в приложение.*
...
Если для свойства [ClosedDisplayMode](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.closeddisplaymode) установлено значение **Compact**, содержимое может обрезаться, если оно превышает компактный размер панели команд. Вы должны обрабатывать события [Opening](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.opening) и [Closed](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.closed), чтобы отображать и скрывать части пользовательского интерфейса в области содержимого так, чтобы они не обрезались. Дополнительную информацию можно найти в разделе [Открытое и закрытое состояние](#open-and-closed-states).


## <a name="open-and-closed-states"></a>Открытое и закрытое состояние

Панель команд может быть открыта или закрыта. Когда он открыт, он показан основной кнопки с текстовые метки и откроется меню переполнения (если имеются дополнительные команды).
В командной строке откроется меню переполнения вверх (выше основные команды) или вниз (см. ниже основные команды). Направление по умолчанию работает, но если не хватает места, чтобы открыть меню переполнения вверх, на панели команд открывает его вниз. 

Пользователя можно переключаться между этими состояниями, нажав клавишу «Си дополнительные» \[••• выбора\] кнопку. Вы можете переключаться между ними программным способом, установив свойство [IsOpen](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.isopen). 
...
Если пользователь взаимодействует с другими частями приложения, когда панель команд открыта, панель команд автоматически закрывается. Это называется *исчезновением*. Вы можете управлять исчезновением, установив свойство [IsSticky](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.appbar.issticky). Когда `IsSticky="true"`, панель остается открытым, пока пользователь нажмет «Си дополнительные» \[••• выбора\] кнопку или выбирает элемент в меню переполнения. 

Рекомендуется избегать панелей команд прикрепленные, так как они не соответствует ожиданиям пользователей для [свет, закрыть и поведение фокуса клавиатуры](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/menus#light-dismiss).
...
- **Compact**: Режим по умолчанию. Отображает содержимое, значки основную команду без метки и «см. раздел» Дополнительные \[••• выбора\] кнопку.
- **Минимальный**: Показывает только тонким полосы, выступает в роли «увеличить» \[••• выбора\] кнопку. Пользователь может открыть панель нажатием в любом месте панели.
- **Скрытый**. На панели команд не отображается, если он закрыт. Это может быть полезно для отображения контекстно-зависимых команд с помощью встроенной панели команд. В этом случае следует открыть панель команд программным способом, установив свойство **IsOpen** или изменив значение свойства ClosedDisplayMode на **Minimal** или **Compact**.
...
>**Устройства Touch**: Если панель команд должен оставаться видимым для пользователя в открывшемся сенсорную клавиатуру или мягкие Input Panel (SIP), то можно назначить панели команд, чтобы [BottomAppBar](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.page.bottomappbar) свойства страницы, и он будет перемещен в остаются видимыми, при наличии SIP . В противном случае следует встроить панель команд и позиционировать ее относительно содержимого приложения.
