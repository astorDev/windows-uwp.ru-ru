...
ms.locfileid: "8945531"
---
# <a name="create-adaptive-tiles"></a>Создание адаптивных плиток

Шаблоны адаптивных плиток — это новая функция в Windows10, что позволяет создавать собственные содержимое уведомления плитки, с помощью простого и гибкого языка разметки, адаптируется к различной плотности экрана. В этой статье рассказывается, как создать адаптивные живые плитки для вашего приложения универсальной платформы Windows (UWP). Полный список адаптивных элементов и атрибутов см. в разделе [Схема адаптивных плиток](../tiles-and-notifications/tile-schema.md).
...
**Примечание**на мобильных устройствах Windows Mobile не поддерживает угловой логотип, поэтому «logo» и «nameAndLogo» по умолчанию «имя».
...
**Примечание**  это отличается от Windows8.x, в котором по умолчанию использовалось значение «logo».
...
**Примечание**по умолчанию используется стиль caption Если hint-style не указан.
...
**Примечание**является единственным допустимым дочерним элементом элемента группы подгруппы.
