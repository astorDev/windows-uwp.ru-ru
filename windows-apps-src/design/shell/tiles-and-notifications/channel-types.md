...
ms.locfileid: "82970859"
---
# <a name="choosing-the-right-push-notification-channel-type"></a>Выбор правильного типа канала push-уведомлений

В этой статье рассматриваются три типа каналов push-уведомлений Windows (основной, дополнительный и альтернативный), которые помогают доставлять содержимое в приложение. 

(Дополнительные сведения о создании push-уведомлений см. в разделе [Обзор Windows push Notification Services (WNS)](../tiles-and-notifications/windows-push-notification-services--wns--overview.md).) 

## <a name="types-of-push-channels"></a>Типы каналов push-уведомлений 

Существует три типа каналов push-уведомлений, которые можно использовать для отправки уведомления в приложение Windows. К ним относятся: 

[Основной канал](https://docs.microsoft.com/uwp/api/windows.networking.pushnotifications.pushnotificationchannelmanagerforuser#Methods_) — «традиционный» канал для push-уведомлений. Может использоваться любым приложением в магазине для отправки уведомлений о всплывающих уведомлениях, плиток, необработанных сообщений или значков. [Дополнительные сведения](windows-push-notification-services--wns--overview.md)см. здесь.

[Вторичный канал плитки](https://docs.microsoft.com/uwp/api/windows.networking.pushnotifications.pushnotificationchannelmanagerforuser#Methods_) — используется для отправки обновлений плитки на вторичную плитку. Может использоваться только для отправки уведомлений плитки или индикатора событий вспомогательной плитке, закрепленной на начальном экране пользователя

[Альтернативный канал](https://docs.microsoft.com/uwp/api/windows.networking.pushnotifications.pushnotificationchannelmanagerforuser#Methods_) — новый тип канала, добавленный в обновлении Creators Update. Это позволяет отправлять необработанные уведомления в любое приложение Windows, включая тех, которые не зарегистрированы в хранилище. 
...
-   Отправка необработанных push-уведомлений в Windows, работающий на любом устройстве Windows. Альтернативные каналы допускают только необработанные уведомления (Однако вы по-прежнему можете переключить фоновую задачу на локальное отображение всплывающих уведомлений или плиток).
...
-   Сервер приложения не может отправлять уведомления push-уведомлений, плиток или эмблем. Вы можете отправлять только необработанные уведомления push-уведомлений. Ваше приложение по-прежнему сможет отправлять локальные уведомления из фоновой задачи. 
...
* [Перехват уведомлений для выполняемых приложений ](https://docs.microsoft.com/previous-versions/windows/apps/hh465450(v=win.10))
