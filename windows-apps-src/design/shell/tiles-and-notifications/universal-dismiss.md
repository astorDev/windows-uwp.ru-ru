---
Description: Сведения об использовании универсальных закрыть на всплывающие уведомления.
...
> **RemoteId**: Идентификатор, который однозначно идентифицирует уведомление *на устройствах*.
...
**Известная проблема**: Получение **RemoteId** через `ToastNotificationHistory.GetHistory()` API всегда возвращает пустую строку, а не **RemoteId** вы указали. Не волнуйтесь, все работает — это означает, что значение недействительно.
