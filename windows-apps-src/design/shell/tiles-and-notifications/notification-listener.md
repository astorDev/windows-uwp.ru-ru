...
Вместо событие переднего плана, используйте код, показанный ранее для [модели с одним процессом](../../../launch-resume/create-and-register-an-inproc-background-task.md) фоновой задачи. Фоновая задача также позволяют получать события уведомления об изменении оба во время работы приложения скрытые или не выполняется.
