...
При тестировании приложения можно заметить, что фоновая задача иногда откладывается и не запускается несколько минут. Чтобы устранить эту проблему, необходимо будет напоминать того в параметры системы -> системные -> батарея -> Использование заряда батареи конкретными приложениями, найти приложение в списке, выберите его и замените его на «всегда разрешена работа в фоновом режиме».После этого фоновая задача будет всегда запускаться в течение секунды получения уведомления.