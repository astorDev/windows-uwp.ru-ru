...
| Свойство | Тип | Обязательный | Описание |
|---|---|---|---|
| **Id** | string | true | Id является обязательным и используется для сопоставления введенного пользователем текста с парой ключ-значение идентификатора и значения, которое ваше приложение использует позже. |
| **Title** | string | false | Текст заголовка для отображения над текстовым полем. |
| **PlaceholderContent** | string | false | Замещающий текст, который будет отображаться в текстовом поле, когда пользователь еще не ввел какой-либо текст. |
| **DefaultInput** | string | false | Исходный текст для размещения в текстовом поле. Оставьте это значение null для пустого текстового поля. |
...
| Свойство | Тип | Обязательный | Описание |
|---|---|---|---|
| **Id** | string | true | Параметр Id является обязательным. Если пользователь выбрал этот элемент, этот идентификатор будет передаваться обратно в код вашего приложения, представляя выбор пользователя. |
| **Content** | string | true | Параметр Content является обязательным и является строкой, которая отображается на элементе выбора. |
...
|---|---|---|---|
| **Id** | string | true | Id является обязательным и используется для сопоставления введенного пользователем текста с парой ключ-значение идентификатора и значения, которое ваше приложение использует позже. |
| **Title** | string | false | Текст заголовка для отображения над полем выбора. |
...
| Свойство | Тип | Обязательный | Описание |
|---|---|---|---|
| **Содержимое** | string | true | Обязательный. Текст для отображения на кнопке. |
| **Аргументы** | string | true | Обязательный. Определяемая приложением строка аргументов, которую приложение будет получать позже, если пользователь нажмет эту кнопку. |
...
| **ProtocolActivationTargetApplicationPfn** | string | false | Если вы используете *ToastActivationType.Protocol*, можно дополнительно указать целевой объект PFN, чтобы независимо от регистрации нескольких приложений для обработки одного и того же URI-протокола всегда запускались нужные приложения. |
...
| Свойство | Тип | Обязательный | Описание |
|---|---|---|---|
| **Содержимое** | string | true | Обязательный. Отображаемый текст. |
...
| **Loop** | логическое значение | false | Задайте значение true, если звук должен повторяться, пока отображается всплывающее уведомление; для разового воспроизведения используйте значение false (вариант по умолчанию). |
...
| Свойство | Тип | Обязательный | Описание |
|---|---|---|---|
| **Id** | string | true | Идентификатор, созданный разработчиком, который уникально определяет этот заголовок. Если два уведомления имеют один и тот же идентификатор заголовка, то они будет отображаться под одним и тем же заголовком в центре уведомлений. |
...
* [Краткое руководство: отправка локального всплывающего уведомления и обработка активации](https://blogs.msdn.com/b/tiles_and_toasts/archive/2015/07/08/quickstart-sending-a-local-toast-notification-and-handling-activations-from-it-windows-10.aspx)
