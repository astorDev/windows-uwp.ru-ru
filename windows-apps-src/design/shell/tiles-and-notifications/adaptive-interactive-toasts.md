...
* **Запустите**: Определяет, какие аргументы передаются обратно в ваше приложение при нажатии вашей тоста, позволяя прямая ссылка в правильный содержимое, отображаемые тост. Дополнительные сведения см. в разделе [отправить локального всплывающих](send-local-toast.md).
* **Visual**: Visual часть тост, включая универсальные привязка, которая содержит текст и изображения.
* **действия**: Интерактивная часть тост, включая входные данные и действия.
* **аудио**: Элементы управления аудио воспроизводиться при тост отображается для пользователя.
...
**Возможности обновления Годовщина**: Всплывающие уведомления можно отобразить изображение с параметрами, при которой является основной [ **ToastGenericHeroImage** ](toast-schema.md#toastgenericheroimage) отображена в пределах этого всплывающего баннера и внутри центра поддержки. Размер изображения — 364x180 пикселей при 100% масштабе.
...
**Возможности обновления Годовщина**: Если вам нужно ссылаться на источник содержимого, можно использовать текст однозначного соответствия примитивов. Этот текст всегда отображается в нижней части уведомления, удостоверение приложения или уведомления метку времени.
...
**Возможности Creators Update**: Теперь вы можете переопределить системные отметка собственные отметки времени, который точно представляет, когда был создан/сведения и содержимое сообщения. Эта метка времени отображается в центре уведомлений.
...
**Возможности Creators Update**: Вы можете предоставить индикатор хода выполнения в вашей всплывающее уведомление для пользователей о ход выполнения операции, такие как файлы для загрузки и многое другое.
...
**Возможности Creators Update**: Вы можете сгруппировать уведомления в разделе заголовков в центре поддержки. Например, можно озаглавить сообщения из группового чата, сгруппировать уведомления на общую тему и так далее.
...
**Возможности обновления Годовщина**: Помимо содержимого, указанных выше можно также отобразить дополнительные адаптивного содержимого, который является видимым, когда она открыта тост.
...
**Новое в Fall Creators Update**: На кнопках активации фон, можно использовать после активации поведения **PendingUpdate** для создания нескольких шагов взаимодействий в вашей всплывающие уведомления. Когда пользователь нажимает кнопку, активируется фоновая задача и всплывающее уведомление переводится в состояние «ожидается обновление» и остается на экране, пока ваша фоновая задача не заменит его на новое всплывающее уведомление.
...
**Возможности обновления Годовщина**: Дополнительные контекстные меню действия можно добавить в существующие контекстное меню, которое отображается при щелчке правой кнопкой мыши всплывающее уведомление из в центре поддержки. Обратите внимание, что это меню появляется только после щелчка правой кнопкой мыши в центре уведомлений. Оно не появляется при щелчке правой кнопкой мыши в окне всплывающего уведомления.
...
-   При необходимости задайте строку пользовательского содержимого:
    -   Если не указать строку, мы используем автоматически локализованных строк для «Отложить» и «Отклонить».
...
* **Напоминание**: Уведомление будет оставаться на экране, пока пользователь не закрывает его или выполняет действие. В Windows Mobile всплывающие уведомления также отображаются сразу в развернутом виде. Звук будет воспроизводиться.
* **Сигнал**: Помимо поведения напоминание сигналы Кроме того будет цикл аудио с помощью звукового сигнала по умолчанию.
