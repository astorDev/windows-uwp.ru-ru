...
При проверке подлинности с помощью WNS облачная служба отправляет HTTP-запрос по протоколу SSL. Параметры предоставляются в формате "application/x-www-for-urlencoded". Укажите идентификатор безопасности пакета в поле "Client\_ID" (идентификатор клиента) и секретный ключ в поле "Client\_Secret", как показано в следующем примере. Дополнительные сведения о синтаксисе см. в справочнике по [запросу маркера доступа](https://docs.microsoft.com/previous-versions/windows/apps/hh465435(v=win.10)).
