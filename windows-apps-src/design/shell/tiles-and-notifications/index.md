...
| [Содержимое плиток](create-adaptive-tiles.md) | Содержимое уведомления плитки указывается с помощью адаптивных, новая функция в Windows10, что позволяет создавать собственные плитке содержимое уведомления с помощью простого и гибкого языка разметки, адаптируется к различной плотности экрана. В этой статье рассказывается, как создать адаптивные живые плитки для вашего приложения универсальной платформы Windows (UWP). |
