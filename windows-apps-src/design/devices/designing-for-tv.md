...
См. в разделе [взаимодействие с помощью Геймпада и пульта дистанционного управления](../input/gamepad-and-remote-interactions.md) рекомендации по взаимодействия в приложениях UWP на просмотр на *большом* расстоянии.
...
В дополнение к предыдущей проектирования и аспекты макета существует несколько оптимизации [геймпада и пульта дистанционного управления](../input/gamepad-and-remote-interactions.md) , которые следует учитывать при сборке приложения.

| Возможность        | Описание           |
| -------------------------------------------------------------- |--------------------------------|
| [Перемещение фокуса по осям X и Y и взаимодействие](../input/gamepad-and-remote-interactions.md#xy-focus-navigation-and-interaction) | **Фокусом XY** позволяет пользователю осуществлять навигацию по пользовательскому Интерфейсу приложения. Однако это позволяет пользователю перемещаться только вверх, вниз, влево или вправо. Рекомендации по выполнению этих и других требований приведены в этом разделе. |
| [Режим мыши](../input/gamepad-and-remote-interactions.md#mouse-mode)|Перемещение фокуса не практический или даже возможно, для некоторых типов приложений, таких как "карты" или рисования и раскраски приложений. В этих случаях **режим мыши** позволяет свободно перемещаться с помощью геймпада или пульта ДУ, так же, как мышь на Компьютере.|
| [Визуальный элемент фокуса](../input/gamepad-and-remote-interactions.md#focus-visual)  | Визуальный элемент фокуса — это границу, которая выделяет текущего элемента пользовательского интерфейса. Это позволяет пользователю быстро определить пользовательский Интерфейс их перемещения по или взаимодействует с.  |
| [Включение фокуса](../input/gamepad-and-remote-interactions.md#focus-engagement) | Включение фокуса требуется пользователь должен нажать кнопку **A/Выбор** игрового контроллера или пульта дистанционного управления, когда фокус на элемент пользовательского интерфейса для взаимодействия с ним. |
| [Аппаратные кнопки](../input/gamepad-and-remote-interactions.md#hardware-buttons) | Геймпад и пульт дистанционного управления предоставляют разные кнопки и конфигурации. |
...
Для более точное управление как элементы добавляются в представление, если ваше приложение предназначено для версии 1803 или более поздней версии, можно использовать [событие UIElement.BringIntoViewRequested](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.bringintoviewrequested). Для **ListView**его можно поместить на [ItemsPanel](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.itemscontrol.itemspanel) /**GridView** перехватить раньше внутреннего **ScrollViewer** , как показано в следующем фрагменте кода:
