...
_**Все фильмы, показанные на снимке экрана, доступны в видеороликах Майкрософт & ТЕЛЕВИЗОРах.**_  
...
| [Рекомендации по элементам управления ИП](#guidelines-for-ui-controls)  |  Имеется несколько элементов управления пользовательского интерфейса, которые хорошо работают на разных устройствах, но обладают некоторыми особенностями при использовании вместе с телевизором. Ознакомьтесь с рекомендациями по использованию этих элементов управления при разработке приложений, взаимодействие с которыми будет выполняться на большом расстоянии. |
| [Пользовательский триггер состояния визуального элемента для Xbox](#custom-visual-state-trigger-for-xbox) | Чтобы адаптировать приложение UWP для просмотра на большом расстоянии, рекомендуется использовать пользовательский *триггер визуального состояния* для изменения макета, если приложение определяет, что оно было запущено на консоли Xbox. |

Помимо приведенных выше рекомендаций по проектированию и разметке, при создании приложения следует учитывать ряд вариантов оптимизации [взаимодействия планшета и пульта дистанционного управления](../input/gamepad-and-remote-interactions.md) .

| Функция        | Описание           |
| -------------------------------------------------------------- |--------------------------------|
| [Навигация по фокусу и взаимодействие с КООРДИНАТами](../input/gamepad-and-remote-interactions.md#xy-focus-navigation-and-interaction) | **Точечная Навигация по фокусу** позволяет пользователю перемещаться по пользовательскому интерфейсу приложения. Однако это позволяет пользователю перемещаться только вверх, вниз, влево или вправо. Рекомендации по выполнению этих и других требований приведены в этом разделе. |
| [Режим мыши](../input/gamepad-and-remote-interactions.md#mouse-mode)|Навигация по фокусу на XY не является практичной или даже возможной для некоторых типов приложений, таких как карты или приложения рисования и рисования. В таких случаях **режим мыши** позволяет пользователям свободно перемещаться по планшету или пульту дистанционного управления, как и мышь на ПК.|
| [Визуальный элемент фокусировки](../input/gamepad-and-remote-interactions.md#focus-visual)  | Визуальный элемент фокуса — это граница, которая выделяет текущий фокус в элементе пользовательского интерфейса. Это помогает пользователю быстро выявить пользовательский интерфейс, с которым осуществляется переход или взаимодействие.  |
| [Участие в фокусе](../input/gamepad-and-remote-interactions.md#focus-engagement) | Для взаимодействия с фокусом пользователь должен нажать кнопку **A/выбрать** на планшете или пульте дистанционного управления, если элемент пользовательского интерфейса имеет фокус, чтобы взаимодействовать с ним. |
| [Аппаратные кнопки](../input/gamepad-and-remote-interactions.md#hardware-buttons) | Игровой планшет и пульт дистанционного управления предоставляют очень разные кнопки и конфигурации. |
...
- [Взаимодействие с помощью геймпада и пульта дистанционного управления](../input/gamepad-and-remote-interactions.md)
