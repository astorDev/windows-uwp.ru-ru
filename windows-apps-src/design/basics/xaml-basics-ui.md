...
2. Далее необходимо клонировать или скачать пример. Нажмите кнопку **Clone or download** (Клонировать или скачать). Откроется подменю.
    ...
    **Если вы знакомы с GitHub:**
    ...
    б. Перейдите к каталогу `Windows-appsample-photo-lab\xaml-basics-starting-points\user-interface`.
...
7. На панели "Свойства" введите **margin** (поле) в поле поиска, чтобы быстро найти свойство **Margin** (поле). Задайте значения левого и нижнего полей равными 24.
...
3. На панели "Структура документа" щелкните правой кнопкой мыши **ImageGridView**. В контекстном меню выберите **Изменить дополнительные шаблоны > Изменить созданные элементы (ItemTemplate) > Создать пустой...** . Откроется диалоговое окно **Создание ресурса**.
...
Запустите приложение, чтобы просмотреть элемент **GridView** с только что созданным шаблоном элемента. Возможно, вы не увидите элемент управления оценки, так как он состоит из белых звезд на белом фоне. Мы изменим цвет фона позже.
...
5. В этом диалоговом окне измените значение ключа Name на **ImageGridView_ItemsPanelTemplate**, а затем нажмите кнопку **ОК**.
    ...
    Мы уже использовали различные панели для расположения элементов управления в приложении, а в элементе **GridView** есть внутренняя панель для управления расположением его элементов. Получив доступ к этой панели (**ItemsWrapGrid**), можно изменить ее свойства, чтобы поменять расположение элементов внутри **GridView**.
