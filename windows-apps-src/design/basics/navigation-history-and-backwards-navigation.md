...
Чтобы создать кнопки "Назад", используйте элемент управления " [Button](../controls-and-patterns/buttons.md) " с `NavigationBackButtonNormalStyle` стиле и разместите кнопку в верхнем левом углу пользовательского интерфейса приложения (Дополнительные сведения см. в разделе ниже примеров кода XAML).
...
- Если ваше приложение **не с вкладками**, кнопка «Назад» отображается в строке заголовка. Visual взаимодействие и взаимодействие пользователя с кнопкой «Назад» не изменяются от предыдущих сборок.
