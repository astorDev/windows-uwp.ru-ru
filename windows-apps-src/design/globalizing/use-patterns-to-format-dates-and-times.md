...
Компонент даты соответствует стандартному шаблону формата "month day" ("месяц день"), а компонент времени— стандартному шаблону формата "hour minute" ("час минута"). Таким образом можно создавать модули формирования для нужных шаблонов даты и времени формат и затем объединять их выходные данные, используя строку формата.
