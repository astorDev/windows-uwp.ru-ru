---
Description: Используйте API Windows. Globalization. Датетимеформаттинг с пользовательскими шаблонами и шаблонами для вывода даты и времени в точности в нужном формате.
...
**Обратите внимание** ,  это всего лишь Обзор шаблонов формата. Более подробные сведения о стандартных и нестандартных шаблонах формата см. в разделе "Замечания" класса [**DateTimeFormatter**](/uwp/api/windows.globalization.datetimeformatting?branch=live).
...
С другой стороны нестандартный шаблон формата зависит от языка и региональных параметров. Давайте вернемся к шаблону формата для нашего шаблона формата.
...
**Шаблоны формата, например "месяц день"**
...
**Шаблоны формата, например "{month. Full} {день. integer}"**
...
Компонент даты соответствует стандартному шаблону формата "month day" ("месяц день"), а компонент времени — стандартному шаблону формата "hour minute" ("час минута"). Таким образом, можно создать модули форматирования для соответствующих шаблонов формата даты и времени, а затем объединить их выходные данные с помощью строки локализуемого формата.
...
`CustomDateTimeFormatString` — это идентификатор ресурса, ссылающийся на локализуемый ресурс в файле ресурсов (. resw). Для языка по умолчанию на английском языке (США) для этого параметра задается значение "{0} | {1}"вместе с комментарием, указывающим, что"{0}"— это дата, а"{1}"— время. Таким образом, переводчики могут настраивать элементы формата нужным образом. Например, они могут изменить порядок элементов, если для того или иного языка или региона естественнее указывать время перед датой. Они также могут заменить "|" другим знаком разделения.
...
* [Windows. Globalization. Датетимеформаттинг](/uwp/api/windows.globalization.datetimeformatting?branch=live)
