...
Файлам ресурсов или их папкам следует присваивать квалификаторы ресурсов языка. Дополнительные сведения о квалификаторах ресурсов см. в разделе [Адаптация ресурсов с учетом языка, масштаба, высокой контрастности и других квалификаторов](../../app-resources/tailor-resources-lang-scale-contrast.md). Файл ресурсов может быть изображения (и другие ресурсы), или он может быть файл ресурсов контейнера, например *.resw* , которое содержит текстовые строки.

**Примечание** Даже ресурсам языка по умолчанию вашего приложения по необходимо задать квалификатор языка. Например, если язык приложения по умолчанию — английский язык (США), затем квалифицировать ваши ресурсы как `\Assets\Images\en-US\logo.png`.

- Windows выполняет сложный анализ, включающий региональные варианты, например en US и en-GB. Таким образом включить вложенную тег региона соответствующим образом. См. раздел [Как система управления ресурсами сопоставляет языковые теги](../../app-resources/how-rms-matches-lang-tags.md).
- Указано вложенный тег языка сценария в квалификатора не значение Suppress-Script, определенных для языка. Например, вместо zh-CN или zh-TW, используйте zh-Hant, zh-Hant-TW или zh-Hans (Дополнительные сведения см. в разделе [подтегов языков IANA](http://go.microsoft.com/fwlink/p/?linkid=227303)).
- Для языков, имеющих единый стандарт нет необходимости включать квалификатор региона. Например используйте ja вместо ja-JP.
...
### <a name="not-all-resources-need-to-be-localized"></a>Необходимо локализовать не все ресурсы

Локализация не является обязательным для всех ресурсов.

- Как минимум убедитесь, что существуют все ресурсы языка по умолчанию.
- Подмножество некоторые ресурсы могут быть достаточно тесно связанные языка (частичная локализация). Например, не все пользовательские интерфейсы приложения могут быть переведены на каталанский, если приложение содержит полный набор ресурсов на испанском языке. Для пользователей, которые говорят на каталанском и на испанском языке ресурсы, которые не доступны в каталанский отображаются на испанском языке.
- Некоторые ресурсы, может потребоваться исключения для определенных языков, а большинство других карту ресурсов с общим ресурсом. В этом случае пометьте ресурс, который будет использоваться для всех языков тегом неопределенного языка «und». Windows интерпретирует тег языка "und" подобно подстановочному знаку ("\*"), отдавая предпочтение основному языку приложения при отсутствии точного совпадения. Например, если несколько ресурсов для финского языка отличаются, а остальные ресурсы одинаковы для всех языков, финский ресурс необходимо отметить языковым тегом финского языка, а остальные — тегом «und».
- Для ресурсов, которые основываются на сценарии языка, таких как шрифт или высота текста, используйте неопределенный языковой тег с указанным сценарием: "und -&lt;сценарий&gt;". Например, для латинских шрифтов используйте `und-Latn\\fonts.css`, а для кириллических — `und-Cryl\\fonts.css`.
