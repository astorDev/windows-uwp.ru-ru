---
Description: Многоязыковых приложений Toolkit (MAT) 4.0 интегрируется с Microsoft Visual Studio 2019 г. для предоставления приложений универсальной платформы Windows благодаря поддержке перевода, управлению файлами перевода и инструментам для редактирования.
...
ms.locfileid: "67820585"
---
# <a name="use-the-multilingual-app-toolkit-40"></a>Использование набора средств для многоязычных приложений версии 4.0

Многоязыковых приложений Toolkit (MAT) 4.0 интегрируется с Microsoft Visual Studio 2019 г. для предоставления приложений универсальной платформы Windows благодаря поддержке перевода, управлению файлами перевода и инструментам для редактирования. Вот некоторые преимущества этого набора средств.
...
- [Многоязыковой расширение приложения для Toolkit 4.0 для Visual Studio 2017 и более поздних версий](https://marketplace.visualstudio.com/items?itemName=MultilingualAppToolkit.MultilingualAppToolkit-18308). Он содержит расширения КОВРИК 4.0 для Visual Studio 2019, в виде установщика .vsix.
...
Если вы используете Visual Studio 2017 или Visual Studio 2019, затем загрузите и запустите оба установщика, один за другим. Если вы используете Visual Studio 2015 или Visual Studio 2013, скачайте и запустите установщик .msi.
...
- Для каждого добавленного языка создается новая папка по имени [языкового тега BCP-47](https://go.microsoft.com/fwlink/p/?linkid=227302) соответствующего языка. В этой папке создаются новые файлы ресурсов (.resw). Они будут совпадать с файлами ресурсов, содержащими строки языка по умолчанию.
...
Для перевода ресурсов приложения можно использовать установленные поставщики перевода, такие как [Языковой портал Майкрософт](https://go.microsoft.com/fwlink/p/?LinkId=330295) и [Microsoft Translator](https://go.microsoft.com/fwlink/p/?LinkId=258220). Если поставщик поддерживает определенный язык, рядом с названием языка отображается значок поставщика в диалоговом окне "Языки перевода".
...
Выберите **выходные данные: Сообщение** в диалоговое окно экспорта строки ресурсов, а затем нажмите кнопку ОК и файлы будут ZIP-архив и подключенный к нового сообщения электронной почты. Выберите **выходные данные: Расположение папки файлов**браузера папку и нажмите кнопку ОК, при необходимости выбрать для файлов быть ZIP-файлы, нажмите кнопку OK еще раз и файлы могут быть (ZIP- и) сохранен в расположении, вы выбрали, в новой папке с именем проекта.

После того как локализаторы завершат перевод и вы получите переведенные XLF-файлы, вы можете импортировать эти файлы в свой проект. Выберите нужный .xlf файлы в обозревателе решений, щелкните их правой кнопкой мыши и выберите **набор многоязычных инструментов** > **импорта и очистки переводы...** . Нажмите кнопку **добавить**, перейдите к файлам, .xlf или ZIP-файл и нажмите кнопку **импорта**.
...
- [Языковой портал Майкрософт](https://go.microsoft.com/fwlink/p/?LinkId=330295) поддерживает многократное использование переводов и сопоставление с терминологией на основе переводов пользовательского интерфейса для продуктов и служб Майкрософт.
- [Microsoft Translator](https://go.microsoft.com/fwlink/p/?LinkId=258220) предоставляет службы машинного перевода по требованию.

Вы и ваши переводчики можете управлять статусом переводов в многоязыковом редакторе, чтобы позднее просмотреть переводы, вызывающие сомнения. Можно задать состояние каждой строки в **свойства** вкладки. Значения состояния являются: **Новый**, **проверку**, **переведенный**, **окончательный**, и **утверждает**. Индикатор слева от строки показывает состояние. Когда все строки в многоязычном редакторе отмечены зеленым, перевод завершен.
...
* [Установщик .vsix многоязыковых приложений Toolkit 4.0](https://marketplace.visualstudio.com/items?itemName=MultilingualAppToolkit.MultilingualAppToolkit-18308)
* [Установщик MSI многоязыковых приложений Toolkit 4.0](https://developer.microsoft.com/en-us/windows/develop/multilingual-app-toolkit)

## <a name="translation-services"></a>Служба перевода

* [Портал Microsoft Language](https://go.microsoft.com/fwlink/p/?LinkId=330295)
