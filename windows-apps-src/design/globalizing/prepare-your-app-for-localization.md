...
При установке приложения, содержащий данные локализованных вы можете обнаружить, что только язык по умолчанию доступно для приложения, несмотря на то, что вы включили изначально ресурсы на нескольких языках. Это происходит из-за того, который оптимизированный процесс установки только установить языковые ресурсы, которые соответствуют текущего языка и региональных параметров устройства. Это означает, что если ваше устройство настроено для en-us при установке приложения только en-us языковые ресурсы будут установлены. Если изменить язык по умолчанию операционной системы, приложение будет по-прежнему будет отображаться только en-us ресурсов так как это единственный язык, установленный для приложения. В настоящее время не существует способа для установки поддержки дополнительных языков для вашего приложения после первоначальной установки. 

Если вы хотите убедиться, что все языковые ресурсы доступны после установки вы можете cerate файл конфигурации для пакета приложения, указывающая, что некоторые ресурсы требуются во время установки. В этом файле конфигурации может потребоваться установить любому ресурсу, включая языковые ресурсы. Для установки Дополнительные сведения о ресурсах, гарантируя, см. в этом документе: [Убедитесь, что ресурсы установлены на устройстве, независимо от того, требуется устройство](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140))
 
Эта функция оптимизации установки автоматически включается, когда во время упаковки создает пакет appxbundle для вашего приложения. При необходимости, чтобы убедиться, что установлены все ресурсы можно отключить создание appxbundle, когда пакет вашего приложения. Однако это не рекомендуется, так как он может увеличить время установки приложения. Вместо этого необходимо создать файл конфигурации упаковки согласно выше и требуется только необходимые ресурсы, позволяя для продолжения для оптимизации размещения ненужные ресурсы. 
 
Вы можете отключить appxbundle поколения и включить все упакованных ресурсы, задав для атрибута «Создать пакет приложений» значение «никогда». 
