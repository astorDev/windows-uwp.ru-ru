...
В качестве другого примера рассмотрим предложения «Напомнить через {0} мин.» Использование "minute(s)" подходит для английского, но в других языках могут использоваться другие слова. Например, в польском в зависимости от контекста используются слова minuta, minuty или minut.
...
При установке приложения, содержащий локализованные данные, может оказаться, что только язык по умолчанию доступен для приложения, несмотря на то, что изначально включенные ресурсы для нескольких языков. Это обусловлено оптимизированный процесс установки только Установка языковых ресурсов, которые соответствуют текущего языка и региональных параметров устройства. Таким образом Если устройство настроено для en US, языковые ресурсы en US устанавливаются вместе с вашим приложением.

> [!NOTE]
> Не поддерживается для установки поддержки дополнительных языков для вашего приложения после первоначальной установки. Если изменить язык по умолчанию после установки приложения, приложение продолжает использовать только исходные языковые ресурсы.

Если вы хотите убедиться, что все языковые ресурсы будут доступны после установки, создайте файл конфигурации для пакета приложения, которое указывает, что некоторые ресурсы требуются во время установки (в том числе языковые ресурсы). Эта функция оптимизированного установки включается автоматически при создании приложения .appxbundle во время упаковки. Дополнительные сведения см. в разделе [убедитесь, что установка ресурсов в устройстве независимо от потребности устройства](https://docs.microsoft.com/en-us/previous-versions/dn482043(v=vs.140)).

При необходимости чтобы убедиться, что все ресурсы являются установлено (не только ее часть), можно отключить создание .appxbundle, при упаковке приложения. Однако это не рекомендуется, так как это может увеличить время установки приложения.

Отключите автоматическое создание .appxbundle, задав атрибут «Создать набор приложений» на «никогда»:

1. В Visual Studio щелкните правой кнопкой мыши имя проекта
2. Выберите **Store** -> **создать пакеты приложения...**
3. В **создайте свои пакеты** диалоговом окне выберите **я хочу создать пакеты для отправки в Microsoft Store, используя новое имя приложения** и нажмите кнопку **Далее**.
4. В **выберите имя приложения** диалоговое окно, выберите или создайте имя для пакета приложения.
5. В **Выбор и Настройка пакетов** диалоговом окне set **создать комплект приложения** для **никогда**.
...
Строка формата в этом примере ориентирована на английский язык (США). Однако она, к примеру, не подходит для немецкого языка, в котором день и месяц отображаются в обратном порядке. Убедитесь, что переводчик знает цель каждого параметра, чтобы их можно изменить порядок элементов формата в строке формата (например, "{1} {0}«) в соответствии с целевым языком.
...
3. В Resources.resw для общие ja-JP: Добавьте строковый ресурс для Appname «希蒼»
4. В Resources.altform-msft-phonetic.resw для японского фуригана ресурсов: Добавьте значение фуригана для AppName «のあ»
...
- [Ресурсы приложения и примеры локализации](https://go.microsoft.com/fwlink/p/?linkid=254478)
