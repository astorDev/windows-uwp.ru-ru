...
Строка формата в этом примере ориентирована на английский язык (США). Однако она, к примеру, не подходит для немецкого языка, в котором день и месяц отображаются в обратном порядке. Убедитесь, что переводчик знает назначение каждого из параметров, таким образом, чтобы он мог изменить порядок элементов формата в строке формата (например, «{1} {0}») как соответствующего языка.
