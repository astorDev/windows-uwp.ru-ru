...
-   [**AutoReverse**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.animation.timeline.autoreverse): после достижения последнего ключевого кадра, кадры повторяются в обратном порядке с конца. Это удваивает кажущуюся продолжительность анимации.
-   [**BeginTime**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.animation.timeline.begintime): задерживает начала анимации. Отсчет временной шкалы для значений **KeyTime** в кадрах не начинается, пока не будет достигнуто значение **BeginTime**, что исключает риск обрезания кадров.
-   [**FillBehavior**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.animation.timeline.fillbehavior): Определяет, что происходит при достижении последнего ключевого кадра. **FillBehavior** не влияет на какие-либо промежуточные ключевые кадры.
...
-   [**SpeedRatio** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.animation.timeline.speedratioproperty) (еще не используются)
...
-   [**BackEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.BackEase): Возвращающую движение анимации немного, прежде чем она начнет выполняться по заданному пути.
-   [**BounceEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.BounceEase): Создает эффект отскока.
-   [**CircleEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.CircleEase): Создает анимацию, которая ускоряется и замедляется с помощью тригонометрической функции.
-   [**CubicEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.CubicEase): Создает анимацию, которая ускоряется и замедляется с помощью формулы f(t) = t3.
-   [**ElasticEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.ElasticEase): Создает анимацию, которая напоминает пружину, и обратно до до полного успокоения.
-   [**ExponentialEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.ExponentialEase): Создает анимацию, которая ускоряется и замедляется с помощью экспоненциальной формулы.
-   [**PowerEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.PowerEase): Создает анимацию, которая ускоряется и замедляется с помощью формулы f(t) = tp, где p равно значению [ **Power** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.media.animation.powerease.power) свойство.
-   [**QuadraticEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.QuadraticEase): Создает анимацию, которая ускоряется и замедляется с помощью формулы f(t) = t2.
-   [**QuarticEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.QuarticEase): Создает анимацию, которая ускоряется и замедляется с помощью формулы f(t) = t4.
-   [**QuinticEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.QuinticEase): Создать анимацию, которая ускоряется и замедляется с помощью формулы f(t) = t5.
-   [**Схемы SineEase**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.SineEase): Создает анимацию, которая ускоряется или замедляется по формуле синуса.
