...
В этом коротком видео приложение использует подключенную анимацию, чтобы анимировать изображение элемента, так как «продолжает» становится частью заголовка следующей страницы. Этот эффект помогает сохранить контекст пользователя во время перехода.
...
Когда пользователь переходит вперед в приложении (A-B), подключенных элемента отображается физически «запуска на странице». Соответственно, элемент отображается для перемещения вперед в z пространстве и удаляет немного как эффект выполнения удержание сила притяжения. Разработчики эффекты силы тяжести, элемент получает скорости и ускоряет в его конечное расположение. В результате получается анимацию «scale и dip». |
...
Для обратной навигации (Page_B для Page_A), выполните те же действия, но отменяются страниц источника и назначения.

Когда пользователи переходят назад, они ожидают его возвращается к предыдущему состоянию, как можно скорее. Таким образом рекомендуется использовать, [DirectConnectedAnimationConfiguration](). Эта анимация быстрее, более прямолинейно и с замедлением в анимации.
...
Чтобы подготовить подключенную анимацию с данным элементом списка, вызовите метод [PrepareConnectedAnimation](/uwp/api/windows.ui.xaml.controls.listviewbase.prepareconnectedanimation) уникальный ключ, элемент и имя «PortraitEllipse».
...
Если согласованная анимация использует сила притяжения конфигурации, сила притяжения применяется к элементом подключенной анимации и согласованные элементы. Согласованные элементы будут «swoop» вместе с подключенным элемент Чтобы действительно согласованные элементы.
