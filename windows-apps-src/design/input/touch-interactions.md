...
<tr><td rowspan="3">Precision</td><td>Контактная поверхность кончиков пальцев больше, чем отдельная пара координат X-Y, поэтому увеличивается вероятность случайного выполнения команд.</td><td>Мышь и перо передают точные координаты X-Y.</td><td>Аналогично мыши.</td></tr>
...
## <a name="timing"></a>Временные параметры
...
| [**поинтервхилчанжед**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.pointerwheelchanged)   | Происходит при изменении Дельта-значения колесика мыши и при сжатии сенсорной панели.         |
