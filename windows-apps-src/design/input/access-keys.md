...
_Значки клавишную подсказку для ключей доступа в Microsoft Word_
...
![Ключи доступа в Microsoft Word](images/accesskeys/primary-access-keys-word.png)
_ключи доступа в Microsoft Word_

![Ключи вторичного доступа в Microsoft Word](images/accesskeys/secondary-access-keys-word.png)
_ключей вторичного доступа в Microsoft Word_
...
_Основной области панели команд и поддерживаемые комбинации клавиш_

![Дополнительные клавиши доступа для CommandBar](images/accesskeys/secondary-access-keys-commandbar.png)

_Вторичный область панели команд и поддерживаемые комбинации клавиш_
...
_Английский и испанский папки строки ресурсов_
...
_Укажите свойство AccessKey, указанный в файле resources.resw_
...
_Пример автоматического размещения ключевой подсказки_

Но если нужен дополнительный контроль над размещением подсказки клавиши, рекомендуется следующее:

1.  **Принципу очевидный ассоциации**: Пользователя можно легко связать элемент управления с ключевой подсказки.

    1\.  Подсказка KeyTip должна находиться **рядом** с элементом, для которого есть клавиша доступа (владельцем).  
    2\.  Подсказка KeyTip не должна **закрывать включенные элементы**, имеющие клавиши доступа.   
    В.  Если подсказку KeyTip невозможно разместить рядом с ее владельцем, она должна перекрывать владельца. 

2.  **Возможность обнаружения**: Пользователь может быстро обнаруживать ключевая подсказка элемента управления.

    1\.  Подсказка KeyTip никогда не должна **перекрывать** другие подсказки клавиш.  

3.  **Просто сканирования:** Пользователь может легко вытянуть ключевых подсказок.

    1\.  Подсказки клавиш должны быть **совмещены** друг с другом и элементом пользовательского интерфейса.
    2\.  Подсказки клавиш должны быть максимально **сгруппированы**. 
...
Эти режимы размещения являются: Верхней, нижней, справа, слева, скрытые, Center и автоматически.

![Режимы размещения подсказки клавиши](images/accesskeys/keytip-postion-modes.png)

_Режимы размещения ключевых подсказок_
...
_Значение вертикального и горизонтального смещения для ключа, совет_
...
_Приводит к краю экрана ключевая подсказка для положение автоматически_
...
_Параметры настройки ключевых подсказок_
...
Если задать свойство AccessKey на элементе управления UIElement или TextElement, можно использовать свойство [AutomationProperties.AccessKey](https://docs.microsoft.com/dotnet/api/system.windows.automation.automationproperties.accesskey?view=netframework-4.8) для получения этого значения. Помощники, такие как экранный диктор, считывают значение этого свойства каждый раз, когда элемент получает фокус.
