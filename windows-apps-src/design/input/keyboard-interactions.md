...
***Распространенные шаблоны взаимодействия являются общими для клавиатуры и gamepad***

В этом разделе мы уделим особое внимание проектированию приложений UWP для ввода с клавиатуры на ПК. Тем не менее, взаимодействие с помощью хорошо спроектированной клавиатуры важно для поддержки специальных возможностей средства, такие как экранный диктор Windows, с помощью [клавиатуры программного обеспечения](#software-keyboard) сенсорной клавиатуры и экранная клавиатура (OSK) и для обработки других типы устройство ввода, например игровой приставки Xbox и удаленное управление.
...
***Универсальная платформа Windows поддерживает клавиатуры с любого устройства***
...
При необходимости, мы будем определить основные различия необходимо учитывать и описывают все варианты решений, которые следует учитывать.

Ниже приводится список устройств и средств, описанных в этом разделе:

| Устройство/средство                       | Описание     |
|-----------------------------------|-----------------|
|Клавиатура (аппаратная и программная)   |В дополнение к стандартной аппаратную клавиатуру, приложения универсальной платформы Windows поддерживают две клавиатуры программного обеспечения: [сенсорного ввода (или программного обеспечения) клавиатуры](#software-keyboard) и [Экранная клавиатура](#on-screen-keyboard).|
...
- Поддержка сочетаний клавиш
    ...
    - Укажите ключи доступа для перехода пользовательского интерфейса приложения (см. в разделе [ключи доступа](access-keys.md))
...
![Фокус visual](images/keyboard/focus-visual.png)
...
Чтобы использовать элемент управления (в том числе элементы навигации) с помощью клавиатуры, фокус должен быть размещен на элементе. Один из способов для элемента управления получать фокус клавиатуры — сделайте его доступным через навигацию по вкладкам, указав его как вкладка остановки в последовательности табуляции вашего приложения.

Для элемента управления, которые будут включены в последовательности табуляции [IsEnabled](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.control#Windows_UI_Xaml_Controls_Control_IsEnabled) свойству должно быть присвоено **true** и [IsTabStop](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_IsTabStop) свойству должно быть присвоено **true**.

Чтобы специально исключить элемент из последовательности табуляции, установите [IsTabStop](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_IsTabStop) свойства **false**.
...
Последовательность табуляции по умолчанию можно переопределить, задав [TabIndex](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex) свойство.
...
Все интерактивные элементы управления должны иметь табуляции (если они используются в [группы](#control-group)), при запуске неинтерактивных элементов управления, например метки, следует не.
...
См. в разделе [специальные возможности клавиатуры](../accessibility/keyboard-accessibility.md) Дополнительные сведения о настройке позициями табуляции.

#### <a name="try-to-coordinate-tab-order-and-visual-order"></a>Старайтесь согласовать остановки перехода и визуальный порядок

Координация последовательности табуляции и порядка при визуальном (также называется чтения порядок либо порядок отображения) помогает уменьшить сложности для пользователей при переходе через пользовательский Интерфейс приложения.
...
Исходный фокус указывает элемент пользовательского интерфейса, получающий фокус при первом запуске или активации приложения или страницы. При использовании клавиатуры, он получен из этого элемента, когда пользователь начинает взаимодействие с пользовательским Интерфейсом приложения.

Для приложений UWP, начальной фокус перемещается к элементу с самым высоким [TabIndex](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex) , может получать фокус. Дочерние элементы элементов управления контейнера игнорируются. При одинаковых показателях фокус получает первый элемент визуального дерева.
...
#### <a name="dont-set-initial-focus-on-an-element-that-exposes-a-potentially-negative-or-even-disastrous-outcome"></a>Не устанавливать исходный фокус на элементе, который предоставляет негативно или даже иногда катастрофой результат

Этот уровень функциональных возможностей должен быть выбора пользователя. Установка начального фокуса на элементе, активация которого приводит к значительным результатам, может стать причиной потери данных или доступа к системе. Например не установить фокус на кнопку delete при переходе по адресу электронной почты.

См. в разделе [сосредоточиться навигации](focus-navigation.md) Дополнительные сведения о переопределении последовательности табуляции.
...
-   **Клавиши со стрелками** предоставляют возможность «внутренней навигации» для элемента управления. Когда пользователь входит во «внутреннюю навигацию», клавиши со стрелками не выполняют переход за пределы элемента управления. Вот несколько примеров.
    -   Стрелка вверх или вниз клавиши можно перемещать фокус внутри `ListView` и `MenuFlyout`
    -   Изменение выбранных значений `Slider` и `RatingsControl`
    -   Переместить курсор внутри `TextBox`
    -   Развернуть или свернуть элементы внутри `TreeView`

Используйте такое поведение по умолчанию для оптимизации приложения с помощью клавиатуры.
...
Предоставляя стрелку переходы по ключам в набор связанных элементов управления повышает их связь внутри организации из пользовательского интерфейса приложения.
...
***Взаимодействие с набором связанных кнопок упрощается с помощью ключа навигации стрелку***
...
Путем определения одной позиции табуляции для коллекции связанных или дополняющих друг друга, элементов управления, можно свести к минимуму количество общую позициями табуляции в вашем приложении.
...
***Взаимодействие с накоплением два элемента управления ListView можно сделать более простым и эффективным, исключая табуляции и переходы с только что клавиш со стрелками.***
...
Сочетания клавиш приложение можно сделать проще в использовании, предоставляя и улучшенная поддержка специальных возможностей и повышенная эффективность пользователей клавиатуры.

Помимо поддержки в вашем приложении с помощью клавиатуры и активации, рекомендуется также обеспечить сочетания клавиш для функциональности приложения. Вкладки навигации обеспечивает хорошее "и" базовый уровень поддержка клавиатуры, но с более сложного интерфейса пользователя, может потребоваться добавить поддержку для сочетания клавиш также. 
...
-   [Ускорители](#accelerators) являются ярлыками, которые вызывают команду приложения. Приложение может или не может предоставлять пользовательский Интерфейс, который соответствует команде. Ускорители обычно состоят из клавишу Ctrl, а также письма ключ.
-   [Ключи доступа](#access-keys) являются ярлыками, которые установить фокус на конкретный пользовательский Интерфейс в приложении. Typicaly ключи доступа состоят из клавишу Alt, а также письма ключ.

Предоставление согласованного сочетания клавиш, которые поддерживают схожих задач в приложениях становятся гораздо более полезные и мощные и помогает пользователям запоминать.

#### <a name="accelerators"></a>Ускорители

Ускорители помогают пользователям выполнять общие действия в приложении, гораздо быстрее и эффективнее. 

Примеры ускорителей:
-   Нажать сочетание клавиш Ctrl + N ключевых в любом месте в **Mail** приложение запускает элементу электронной почты.
-   Нажав клавиши Ctrl + E ключа в любом месте в Microsoft Edge (и во многих приложениях Microsoft Store) запускает поиск.

Ускорители обладают следующими характеристиками:
-   Они в основном используются клавиши Ctrl и функция ключа последовательности (сочетания клавиш Windows system также использовать сочетание клавиш Alt + не буквенно-цифровых клавиш и клавишу с логотипом Windows).
...
-   Они действуют во всем приложении, когда это возможно.
-   Они должны назначаться согласованно запомненные и напрямую не задокументированы.
...
-   Они описаны непосредственно в пользовательском Интерфейсе рядом с элементом управления, через [ключевых подсказок](access-keys.md).
-   Они действуют только в текущем окне и перемещают фокус на соответствующий пункт меню или элемент управления.
-   Ключи доступа должен быть назначен согласованно часто используемых команд (особенно фиксации кнопки), когда это возможно.
-   Они локализуются.

#### <a name="common-keyboard-shortcuts"></a>Основные сочетания клавиш

В следующей таблице приведен небольшая часть часто используемых сочетаний клавиш. 

| Action                               | Командная клавиша                                      |
|--------------------------------------|--------------------------------------------------|
| Выделить все                           | CTRL+A                                           |
...
Полный список сочетаний клавиш системы Windows, см. в разделе [сочетания клавиш для Windows](https://support.microsoft.com/help/12445/windows-keyboard-shortcuts). Распространенные ярлыков приложений, см. в разделе [сочетания клавиш для приложений Microsoft](https://support.microsoft.com/help/13805/windows-keyboard-shortcuts-in-apps).
...
### <a name="control-group"></a>Группа элементов управления
...
Рекомендуется добавить поддержку ключей навигации стрелки, при помощи следующих средств:

<table>
  <tr>
    <td>
      <p><img src="images/keyboard/dialog.png" alt="Dialog buttons"/></p>
      <p><sup>Диалоговое окно кнопок</sup></p>
      <p><img src="images/keyboard/radiobutton.png" alt="Radio buttons"/></p>
      <p><sup>Переключатели</sup></p>     
    </td>
    <td>
      ...
      <p><sup>ListItems и свойства</sup></p>
    </td>    
  </tr>
</table>
...
В зависимости от функциональности и макет приложения лучшим вариантом навигации для группы управления может быть одной позиции табуляции с стрелку навигации для дочерних элементов, несколько табуляции или сочетания.
...
-   `RadioButtons`
-   Несколько `ListViews` , выглядеть и вести себя как один `ListView`
...
*С помощью клавиатуры, нажмите клавишу пользователей* ***ввод*** *ключ для отправки поискового запроса*

<table>
  <tr>
    <td>
      <p><img src="images/keyboard/auto-suggest-narrator-1.png" alt="autosuggest narrator focus"/></p>
      <p><em>Экранный диктор, нажмите пользователей <strong>ввод</strong> ключ для отправки поискового запроса</em></p>
    </td>
    <td>
      <p><img src="images/keyboard/auto-suggest-narrator-2.png" alt="autosuggest narrator focus on search"/></p>
      <p><em>Экранного диктора, пользователи могут также получить доступ к кнопку поиска с помощью <strong>Caps Lock + клавиша со стрелкой вправо</strong>, нажав клавишу <strong>пространства</strong> ключ</em></p>
    </td>
  </tr>
</table>
...
См. в разделе [взаимодействия игровой и удаленное управление](gamepad-and-remote-interactions.md) Дополнительные сведения о поддержке универсальной платформы Windows для игровой и входные данные для удаленного управления.
...
| HOME или END      | Н/Д                                 | Н/Д                 |
| PAGE UP и PAGE DOWN  | Кнопка курка для вертикальной прокрутки, кнопка бампера для горизонтальной прокрутки   | Н/Д                 |
...
#### <a name="directional-navigation"></a>Направленное перемещение между элементами
...
В отличие от клавиатуры, когда приложение opts из [режим мыши](gamepad-and-remote-interactions.md#mouse-mode), направленное перемещение между элементами применяется для всего приложения для игровой и удаленное управление. См. в разделе [взаимодействия игровой и удаленное управление](gamepad-and-remote-interactions.md) Дополнительные сведения об оптимизации направленное перемещение между элементами.
...
<table>
  <tr>
    <td>
      ...
      <p><em><strong>Направленное перемещение между элементами поддерживается</strong></br>С помощью стрелок (клавиши со стрелками, игровой и удаленное управление панели D), пользователь может перемещаться между различными элементами управления.</em></p>
    </td>
    <td>
      <p><img src="images/keyboard/no-directional-navigation.png" alt="no directional navigation"/></p>
      <p><em><strong>Направленное перемещение между элементами не поддерживается</strong> </br>Пользователь не может перемещаться между различными элементами управления с помощью клавиш направления. Не влияет на другие методы перемещения между элементами управления (клавиши tab).</em></p>
    </td>
  </tr>
</table>

### <a name="built-in-keyboard-optimization"></a>Встроенные оптимизации клавиатуры
...
***Несколько столбцов и строк стрелку переходы по ключам***
...
Когда при развертке по строкам (элементы заполняются слева направо, сверху вниз) фокус находится на последнем элементе в строке и нажата клавиша со стрелкой вправо, фокус перемещается на первый элемент в следующей строке. Это же происходит в обратном порядке: Когда фокус перемещается к первому элементу в строке и нажата клавиша со стрелкой влево, фокус перемещается к последнему элемента в предыдущей строке.

Когда при развертке по столбцам (элементы заполняются сверху вниз и слева направо) фокус находится на последнем элементе в столбце и пользователь нажимает клавишу со стрелкой вниз, фокус перемещается на первый элемент следующего столбца. Это же происходит в обратном порядке: Когда фокус перемещается к первому элементу в столбце и нажимается клавиша со стрелкой вверх, фокус перемещается к последнему элемента в предыдущей статье.

<table>
  <tr>
    <td>
      <p><img src="images/keyboard/row-major-keyboard.png" alt="row major keyboard navigation"/></p>
      <p><em>Строки основной с помощью клавиатуры</em></p>
    </td>
    <td>
      <p><img src="images/keyboard/column-major-keyboard.png" alt="column major keyboard navigation"/></p>
      <p><em>Столбец основной с помощью клавиатуры</em></p>
    </td>
  </tr>
</table>

#### <a name="popup-ui"></a>Всплывающее окно пользовательского интерфейса

Как уже упоминалось, следует убедиться, что соответствует направленное перемещение между элементами порядок элементов управления в пользовательском Интерфейсе приложения.

Некоторые элементы управления (например, контекстного меню, меню переполнения панели команд и меню автозаполнения) для отображения всплывающего меню, расположение и направление (вниз по умолчанию) относительно основного элемента управления и доступного пространства на экране. Обратите внимание на то, что направление Открытие может зависеть от различных факторов, во время выполнения.
...
Для этих элементов управления при первом открытии меню (и элемент не был выбран пользователем), клавишу со стрелкой вниз всегда устанавливает фокус к первому элементу во время клавиша со стрелкой вверх всегда устанавливает фокус на последнюю элемента меню. 

Если нажимается клавиша со стрелкой вниз, последний элемент имеет фокус, фокус перемещается к первому элементу в меню. Аналогичным образом Если первый элемент имеет фокус и нажимается клавиша со стрелкой вверх, фокус перемещается к последнему элемента меню. Это поведение называется *Велосипедная* его можно использовать для перемещения всплывающих меню, которые можно открыть в непредсказуемых направлениях.

> [!NOTE]
> Велосипедная следует избегать в UI не всплывающее окно, которых пользователи могут поступать чувствовать треппинга в бесконечном цикле. 

Мы рекомендуем, что такое же поведение эмуляции в пользовательских элементах управления. Образец кода для реализации такого поведения можно найти в [навигации с фокусом программный](focus-navigation-programmatic.md#find-the-first-and-last-focusable-element) документации.
...
### <a name="software-keyboard"></a>Сочетания программного обеспечения
...
***Windows 10 сенсорной клавиатуры***
...
***Сенсорное устройство клавиатуры по умолчанию режим макета***

![сенсорная клавиатура в режиме развернутой раскладки](images/keyboard/extendedview.png)

***Сенсорной клавиатуры в развернутом режиме***
...
**Примечание**  сенсорной клавиатуры не поддерживает переключение и большинство команд системы.
