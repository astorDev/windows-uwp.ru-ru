...
Чтобы специально исключить элемент управления из последовательности табуляции, присвойте [IsTabStop](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_IsTabStop) свойству IsTabStop **значение false**.
...
Можно переопределить последовательность табуляции по умолчанию, установив свойство [TabIndex](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex).
...
-   **Клавиши со стрелками** предоставляют возможность «внутренней навигации» для элемента управления. Когда пользователь входит во «внутреннюю навигацию», клавиши со стрелками не выполняют переход за пределы элемента управления. Некоторые примеры:
    ...
    -   Изменить выбранные в данный момент `Slider` значения для и`RatingsControl`
