---
Description: Узнайте, как разрабатывать и оптимизировать приложения Windows, чтобы обеспечить лучшую производительность для пользователей с клавиатурой и с ограниченными возможностями и другими требованиями к специальным возможностям.
...
Узнайте, как разрабатывать и оптимизировать приложения Windows, чтобы обеспечить лучшую производительность для пользователей с клавиатурой и с ограниченными возможностями и другими требованиями к специальным возможностям.

На разных устройствах ввод с клавиатуры является важной частью всего процесса взаимодействия приложений Windows. Хорошо разработанное взаимодействие с помощью клавиатуры позволяет эффективно переходить по пользовательскому интерфейсу приложения и получать доступ ко всем функциональным возможностям, не отрывая рук от клавиатуры.
...
|-----------------------------------|-----------------|
|Клавиатура (аппаратная и программная)   |Помимо стандартной аппаратной клавиатуры, приложения Windows поддерживают две клавиатуры: [сенсорную (или программную) клавиатуру](#software-keyboard) и [экранную клавиатуру](#on-screen-keyboard).|
|Геймпад и пульт дистанционного управления         |Геймпад Xbox и пульт дистанционного управления — основные устройства ввода при [взаимодействии на расстоянии 3 м](../devices/designing-for-tv.md). Конкретные сведения о поддержке Windows для игрового планшета и удаленного управления см. в разделе [взаимодействие планшета и удаленного](gamepad-and-remote-interactions.md)управления.|
...
Чтобы элемент управления включался в последовательность табуляции, свойство " [включилось](/uwp/api/windows.ui.xaml.controls.control#Windows_UI_Xaml_Controls_Control_IsEnabled) " должно иметь значение **true** , а свойство [IsTabStop](/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_IsTabStop) должно иметь значение **true**.

Чтобы специально исключить элемент управления из последовательности табуляции, присвойте [IsTabStop](/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_IsTabStop) свойству IsTabStop **значение false**.
...
Можно переопределить последовательность табуляции по умолчанию, установив свойство [TabIndex](/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex).
...
Для приложений UWP устанавливается исходный фокус на элементе с наиболее высоким значением [TabIndex](/uwp/api/Windows.UI.Xaml.Controls.Control#Windows_UI_Xaml_Controls_Control_TabIndex) из всех, которые могут получать фокус. Дочерние элементы элементов управления контейнера игнорируются. При одинаковых показателях фокус получает первый элемент визуального дерева.

#### <a name="set-initial-focus-on-the-most-logical-element"></a>Установите начальный фокус на самом логичном элементе

Установите начальный фокус на элементе пользовательского интерфейса для первого, или основного, действия, которое пользователи, скорее всего, выполнят при запуске приложения или переходе на страницу. Некоторые примеры:
...
-   **Клавиши со стрелками** предоставляют возможность «внутренней навигации» для элемента управления. Когда пользователь входит во «внутреннюю навигацию», клавиши со стрелками не выполняют переход за пределы элемента управления. Некоторые примеры:
    -   Клавиша со стрелкой вверх или вниз перемещает фокус внутрь `ListView` и`MenuFlyout`
...
#### <a name="enter-key"></a>ВВОД - клавиша
...
-   `AppBarButtons`в элементе`CommandBar`
-   `ListItems`или `GridItems` внутри `ListView` или`GridView`
-   `Buttons`содержащие`ContentDialog`
...
<table>
  <tr>
    ...
    <td>
      ...
      <p><sup>ListItems и GridItems</sup></p>
    </td>    
  </tr>
</table>
...
-   `RadioButtons`
-   Несколько `ListViews` выглядит так же, как и одно`ListView`
...
| Введите:         | Кнопка A                            | Кнопка выбора       |
| ESC        | Кнопка B                            | Кнопка "Назад"         |
...
Направленная навигация управляется с помощью вспомогательного класса UWP [Диспетчер фокуса](/uwp/api/Windows.UI.Xaml.Input.FocusManager), который принимает нажатую клавишу направления (клавишу со стрелкой, крестовину) и пытается переместить фокус в соответствующем визуальном направлении.
...
<table>
  <tr>
    <td>
      ...
      <p><em><strong>Поддержка направленной навигации</strong></br>С помощью клавиш направления (стрелки клавиатуры, геймпад и крестовины пульта дистанционного управления) пользователь может перемещаться между различными элементами управления.</em></p>
    </td>
    <td>
      <p><img src="images/keyboard/no-directional-navigation.png" alt="no directional navigation"/></p>
      <p><em><strong>Навигация с направлением не поддерживается</strong> </br>Пользователь не может перемещаться между различными элементами управления с помощью клавиш направления. Другие методы навигации между элементами управления (клавиша табуляции) не поддерживаются.</em></p>
    </td>
  </tr>
</table>
...
<table>
  <tr>
    <td>
      ...
      <p><em>Навигация с помощью клавиатуры при развертке по строкам</em></p>
    </td>
    <td>
      <p><img src="images/keyboard/column-major-keyboard.png" alt="column major keyboard navigation"/></p>
      <p><em>Навигация с помощью клавиатуры при развертке по столбцам</em></p>
    </td>
  </tr>
</table>
...
* [Представления navigationview управления клавиатурой](/windows/uwp/design/controls-and-patterns/navigationview#hierarchical-navigation) 
