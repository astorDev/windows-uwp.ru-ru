...
***Многие взаимодействия, которые являются общими для геймпада, пульта дистанционного управления и клавиатуры***

Сборка взаимодействия в приложениях универсальной платформы Windows (UWP), убедитесь, что ваше приложение удобным и доступен через оба традиционные типы ввода компьютеров, ноутбуков, и планшетов (мышь, клавиатуру, сенсорный ввод и т. д.), а также типов ввода типично для Xbox и ТВ на *большом* расстоянии, такие как геймпад и пульт дистанционного управления.

См. в разделе [Проектирование для Xbox и Телевизора](../devices/designing-for-tv.md) инструкции Общая структура приложений UWP на *большом* расстоянии.

## <a name="overview"></a>Обзор

В этом разделе мы рассмотрим, что следует учесть при проекта взаимодействия (или что этого не сделать, если платформа ищет после него вы) и предоставить руководство, рекомендации и предложения по созданию приложения UWP, которые приятным для использования вне зависимости от устройство, типа ввода или прав пользователей и предпочтений.

В итоге ваше приложение должно быть интуитивно понятными и его использование в среде *взаимодействии на расстоянии* как на *большом* расстоянии (и наоборот). Поддерживает основные устройства пользователя, создание пользовательского интерфейса, фокус четко обозначать, упорядочить содержимое, поэтому навигации согласованной и предсказуемой и предоставьте пользователям Самый короткий путь возможных в том, что они хотят сделать.
...
## <a name="optimize-for-both-2-foot-and-10-foot-experiences"></a>Оптимизация для взаимодействии на расстоянии и большом расстоянии

Как минимум рекомендуется протестировать приложение, чтобы убедиться, что они хорошо работают в сценарии 2 на расстоянии и большом расстоянии, и что все функциональные возможности обнаружения и доступность для Xbox [геймпада и пульта дистанционного управления](#gamepad-and-remote-control).

Ниже приведены некоторые другие способы, вы можете оптимизировать приложение для использования при взаимодействии на расстоянии и большом расстоянии и с устройствами ввода (каждый ссылки на соответствующий раздел в этой статье).

> [!NOTE]
> Так как геймпады Xbox и пульты управления поддерживают многие виды поведения клавиатуры UWP и взаимодействия, эти рекомендации, не подходит для обоих типов ввода. См. в разделе [взаимодействие с помощью клавиатуры](keyboard-interactions.md) для более подробные сведения о клавиатуры.

| Возможность        | Описание           |
| -------------------------------------------------------------- |--------------------------------|
| [Перемещение фокуса по осям X и Y и взаимодействие](#xy-focus-navigation-and-interaction) | **Фокусом XY** позволяет пользователю осуществлять навигацию по пользовательскому Интерфейсу приложения. Однако это позволяет пользователю перемещаться только вверх, вниз, влево или вправо. Рекомендации по выполнению этих и других требований приведены в этом разделе. |
| [Режим мыши](#mouse-mode)|Перемещение фокуса не практический или даже возможно, для некоторых типов приложений, таких как "карты" или рисования и раскраски приложений. В этих случаях **режим мыши** позволяет свободно перемещаться с помощью геймпада или пульта ДУ, так же, как мышь на Компьютере.|
| [Визуальный элемент фокуса](#focus-visual)  | Визуальный элемент фокуса — это границу, которая выделяет текущего элемента пользовательского интерфейса. Это позволяет пользователю быстро определить пользовательский Интерфейс их перемещения по или взаимодействует с.  |
| [Включение фокуса](#focus-engagement) | Включение фокуса требуется пользователь должен нажать кнопку **A/Выбор** игрового контроллера или пульта дистанционного управления, когда фокус на элемент пользовательского интерфейса для взаимодействия с ним. |
| [Аппаратные кнопки](#hardware-buttons) | Геймпад и пульт дистанционного управления предоставляют разные кнопки и конфигурации. |
...
\* Если ни событие [KeyDown](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.keydown), ни событие [KeyUp](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.keyup) кнопки "B" не обрабатываются приложением, запускается событие [SystemNavigationManager.BackRequested](https://docs.microsoft.com/uwp/api/windows.ui.core.systemnavigationmanager.backrequested), которое ведет к обратной навигации в приложении. Однако это необходимо реализовывать самостоятельно, как показано в следующем фрагменте кода.
...
Можно создавать приложения UWP, оптимизированных для определенного устройства или взаимодействие, но универсальная платформа Windows также позволяет создавать приложения, которые можно успешно использовать на всех устройствах, при взаимодействии на расстоянии и большом расстоянии и независимо от того, ввод возможности устройств или пользователей. С помощью рекомендаций в этой статье гарантирует, что приложение работает так же хорошо, как это может быть на Телевизоре и ПК.

## <a name="related-articles"></a>Еще по теме

- [Проектирование для Xbox и телевизора](../devices/designing-for-tv.md)
- [Азбука устройств для приложений универсальной платформы Windows (UWP)](index.md)
