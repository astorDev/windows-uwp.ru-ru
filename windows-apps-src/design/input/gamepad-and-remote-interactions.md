---
Description: Оптимизируйте приложения для входных данных из игровой приставки Xbox и удаленное управление.
...
***Многие возможности взаимодействия являются общими для игровой, удаленного управления и клавиатуры***

Разрабатывать интерфейсы взаимодействия в приложениях универсальной платформы Windows (UWP), убедитесь, что приложение можно использовать и доступно через как традиционные входные типы ПК, переносные компьютеры и планшеты (мыши, клавиатуры, сенсорный и т. д.), а также типов входных данных Типичный ТВ и Xbox *10 foot* возникают, например игровой и удаленного управления.

См. в разделе [проектирование для Xbox и Телевизионных](../devices/designing-for-tv.md) для Общие рекомендации для приложений UWP в *10 foot* столкнуться.

## <a name="overview"></a>Обзор

В этом разделе мы обсудим, что вам следует рассмотреть возможность в вашей Разработка взаимодействия (или что этого не сделать, если платформа выглядит после его для вас) и содержит инструкции, рекомендации и варианты для создания приложений универсальной платформы Windows, которые помогут использовать вне зависимости от устройства, тип входных данных, или прав пользователей и настроек.

Самой нижней строке ваше приложение должно быть как интуитивно понятный и простой в использовании в *2 foot* среда, так как он находится в *10 foot* среды (и наоборот). Поддерживает основные устройства пользователя, обеспечить пользовательский Интерфейс сосредоточиться ясное и unmistakable, упорядочивания содержимого, поэтому навигации согласованный и предсказуемый и предоставить пользователям кратчайшего пути возможность действия требуется выполнить.
...
## <a name="optimize-for-both-2-foot-and-10-foot-experiences"></a>Оптимизация для 2 foot и 10-фут

Как минимум, рекомендуется протестировать приложение, чтобы убедиться, они хорошо работать в сценарии 2-foot и 10 foot, и что все функциональные возможности доступными и для Xbox [gamepad и удаленное управление](#gamepad-and-remote-control).

Вот некоторые другие возможности, вы можете оптимизировать приложение для использования в самом общем 2 и 10-foot и всех входных устройствами (каждой ссылки в соответствующем разделе этой статьи).

> [!NOTE]
> Так как игровые панели Xbox и дистанционного управления поддерживают многие UWP клавиатуры работой с, эти рекомендации подходят для обоих типов входных данных. См. в разделе [сочетания взаимодействия](keyboard-interactions.md) более подробные сведения о клавиатуры.

| Функция        | Описание           |
| -------------------------------------------------------------- |--------------------------------|
| [XY навигации с фокусом и взаимодействие](#xy-focus-navigation-and-interaction) | **Навигации с фокусом XY** позволяет пользователю перемещаться в пользовательском Интерфейсе приложения. Однако это позволяет пользователю перемещаться только вверх, вниз, влево или вправо. Рекомендации по выполнению этих и других требований приведены в этом разделе. |
| [Режим мыши](#mouse-mode)|Навигации с фокусом XY не практические или даже возможно для некоторых типов приложений, таких как карты или и рисования приложений. В этих случаях **режим мыши** позволяет пользователям свободно перейдите с игровой или удаленного управления, так же, как мышь на ПК.|
| [Фокус visual](#focus-visual)  | Задача visual-границу, которая выделяет фокус элемента пользовательского интерфейса. Это помогает пользователю быстро определить пользовательский Интерфейс, их просмотра или взаимодействия с.  |
| [Службы взаимодействия фокус](#focus-engagement) | Фокус engagement требует нажатия пользователем клавиши **A/Выбор** кнопку на игровой или удаленного управления, когда элемент пользовательского интерфейса получает фокус, чтобы взаимодействовать с ним. |
| [Аппаратные кнопки](#hardware-buttons) | Игровой и удаленное управление предоставляют очень разные кнопки и конфигураций. |
...
\*Когда ни одно [KeyDown](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.keydown) , ни [KeyUp](https://docs.microsoft.com/uwp/api/windows.ui.xaml.uielement.keyup) события для кнопки B обрабатываются приложением, [SystemNavigationManager.BackRequested](https://docs.microsoft.com/uwp/api/windows.ui.core.systemnavigationmanager.backrequested) будет создаваться событие, которые должны результат обратно в навигации в приложении. Однако это необходимо реализовывать самостоятельно, как показано в следующем фрагменте кода.
...
| Увеличение/уменьшение масштаба        | Ctrl +/- | Левый и правый триггеры | Нет | `ScrollViewer`, представления, которые поддерживают увеличение и уменьшение |
...
| [Открытие контекстного меню](#commandbar-and-contextflyout) | Щелкните правой кнопкой мыши | Кнопка меню | [ContextFlyout](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.UIElement.ContextFlyout) | Контекстные меню |
...
- [Реорганизация пользовательского интерфейса](#ui-rearrange)
- [Службы взаимодействия фокус](#engagement)
...
#### Проблема. Элементы пользовательского интерфейса, после прокручиваемого списка или сетки <a name="problem-ui-elements-located-after-long-scrolling-list-grid"></a>
...
**Реорганизация пользовательского интерфейса <a name="ui-rearrange"></a>**
...
**Службы взаимодействия фокус <a name="engagement"></a>**
...
#### <a name="problem-scrollviewer-without-any-focusable-elements"></a>Проблема. ScrollViewer без какие-либо элементы, способному получать фокус
...
![Приложение недвижимости: Средство прокрутки со только текст](images/designing-for-tv/2d-focus-navigation-and-interaction-scrollviewer.png)

#### <a name="problem-free-scrolling-ui"></a>Проблема. Бесплатные прокрутки пользовательского интерфейса
...
Вы можете создавать приложения универсальной платформы Windows, которые оптимизированы для конкретного устройства или средства, но на универсальную платформу Windows также позволяет создавать приложения, которые могут использоваться на устройствах, в самом общем 2 и 10-foot и без учета ввода успешно возможности устройства или пользователя. С помощью рекомендаций в этой статье гарантирует, что приложения не хуже, что может произойти на Телевизор и ПК.
