...
label: Text scaling
template: detail.hbs
keywords: Отображение UWP, текст, масштабирование, специальные возможности, «специальные возможности», «Text сделать больше», взаимодействие с пользователем, ввод
ms.author: kbridge
ms.date: 08/02/2018
ms.topic: article
...
Если ваше приложение UWP содержит пользовательские элементы управления, поверхности пользовательский текст, высоты жестко управления, более ранние структуры или сторонние платформы, скорее всего необходимо внести некоторые обновления, чтобы обеспечить единообразие и полезные для пользователей.  
...
При изменении размера текста, элементы управления и контейнеров необходимо также изменить размер и адаптировать его новый макет и текст. Как упоминалось ранее, в зависимости от приложения, инфраструктуры и платформ, большую часть этих действий выполняется за вас. В следующем руководстве взаимодействия с Пользователем рассматриваются случаи, когда не.
...
Например все элементы управления текстом UWP поддерживает полный текст, масштабирование взаимодействие без настройки или создания шаблонов.
...
### <a name="use-auto-sizing"></a>Автоматическом выборе размера
...
Обработайте событие системы UISettings [TextScaleFactorChanged](https://docs.microsoft.com/uwp/api/windows.ui.viewmanagement.uisettings.textscalefactorchanged) в настраиваемых инфраструктуры и элементах управления. Это событие вызывается каждый раз, когда пользователь устанавливает коэффициент масштабирования текста в своей системе.

## <a name="summary"></a>Краткий обзор

В этом разделе представлен обзор текста, масштабирование поддержка в Windows и включает руководство по взаимодействию с Пользователем и разработчиков о том, как настраивать взаимодействие с пользователем.
