---
Description: API-интерфейсы основного текста в пространстве имен Windows.UI.Text.Core активируют приложение универсальной платформы Windows (UWP) для получения ввода текста из любой текстовой службы, поддерживаемой устройствами с Windows.
...
ms.locfileid: "66363830"
---
# <a name="custom-text-input"></a>Ввод пользовательского текста



API-интерфейсы основного текста в пространстве имен [**Windows.UI.Text.Core**](https://docs.microsoft.com/uwp/api/Windows.UI.Text.Core) активируют приложение универсальной платформы Windows (UWP) для получения ввода текста из любой текстовой службы, поддерживаемой устройствами с Windows. API очень похожи на API [инфраструктуры текстовых служб](https://docs.microsoft.com/windows/desktop/TSF/text-services-framework) в этом приложении в том, что приложению не требуются подробные данные текстовых служб. Это позволяет приложению получать текст на любом языке и с помощью любого типа ввода, например клавиатуры, речи или пера.

> **Важные API**: [**Windows.UI.Text.Core**](https://docs.microsoft.com/uwp/api/Windows.UI.Text.Core), [**CoreTextEditContext**](https://docs.microsoft.com/uwp/api/Windows.UI.Text.Core.CoreTextEditContext)
...
## <a name="architecture"></a>Architecture (Архитектура)
...
Например, в текстовый диапазон, представленной выше, диапазон \[0, 5\] указывает слова «Hello». Значение **StartCaretPosition** должно быть меньше или равно **EndCaretPosition**. Диапазон \[5, 0\] является недопустимым.
...
![Пример текстового потока диаграммы](images/coretext/stream-2.png) существуют следующие два выбора: \[0, 1\] и \[6, 11\]. Поле ввода должны сообщить только одну из них. либо \[0, 1\] или \[6, 11\].
...
Например, это состояние элемента управления редактированием до того, как пользователь ввел "d". Курсор находится в \[10, 10\].

![Пример текстового потока диаграммы](images/coretext/stream-3.png) при вводе «d», [ **TextUpdating** ](https://docs.microsoft.com/uwp/api/windows.ui.text.core.coretexteditcontext.textupdating) события со следующими [ **CoreTextTextUpdatingEventArgs** ](https://docs.microsoft.com/uwp/api/Windows.UI.Text.Core.CoreTextTextUpdatingEventArgs) данных:

-   [**Диапазон**](https://docs.microsoft.com/uwp/api/windows.ui.text.core.coretexttextupdatingeventargs.range) = \[10, 10\]
...
Например, это состояние элемента управления редактированием до вставки пользователем слова World. Курсор находится в \[6, 6\].

![Пример текстового потока диаграммы](images/coretext/stream-5.png) пользователь выполняет действие вставки и заканчивается в элемент управления редактирования со следующим текстом:

![Пример текстового потока диаграммы](images/coretext/stream-4.png) в этом случае следует вызывать [ **NotifyTextChanged** ](https://docs.microsoft.com/uwp/api/windows.ui.text.core.coretexteditcontext.notifytextchanged) с этими аргументами:
...
Например, рассмотрим элемент управления редактированием, который предоставляет функцию исправления, оформляющую сужения. Это состояние элемента управления редактированием до того, как пользователь нажал клавишу ПРОБЕЛ для активации исправления. Курсор находится в \[3, 3\].

![Пример текстового потока диаграммы](images/coretext/stream-6.png) пользователь нажимает клавишу ПРОБЕЛ и соответствующий [ **TextUpdating** ](https://docs.microsoft.com/uwp/api/windows.ui.text.core.coretexteditcontext.textupdating) события. Элемент управления редактированием принимает обновление текста. Это состояние элемента управления редактированием в течение короткого момента перед завершением исправления. Курсор находится в \[4, 4\].

![Пример текстового потока диаграммы](images/coretext/stream-7.png) за пределами [ **TextUpdating** ](https://docs.microsoft.com/uwp/api/windows.ui.text.core.coretexteditcontext.textupdating) обработчик событий элемента управления редактирования делает следующие исправления. Это состояние элемента управления редактированием после завершения исправления. Курсор находится в \[5, 5\].

![Пример текстового потока диаграммы](images/coretext/stream-8.png) в этом случае следует вызывать [ **NotifyTextChanged** ](https://docs.microsoft.com/uwp/api/windows.ui.text.core.coretexteditcontext.notifytextchanged) с этими аргументами:
...
**Примеры**
* [Пользовательский элемент управления Edit образец](https://go.microsoft.com/fwlink/?linkid=831024)  
 **архивировать примеры**
