...
> **Важные API**: [**Windows.UI.Input**](https://docs.microsoft.com/uwp/api/Windows.UI.Input), [ **Windows.UI.Xaml.Input**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Input)
...
-   Отображайте индикаторы сдвига и полосы прокрутки, чтобы давать подсказки о расположении и размере. Скрывайте их, если в приложении реализован специальный метод навигации.
    ...
    **Примечание**  в отличие от стандартных прокрутки полосы, индикаторы панорамирования являются исключительно информативными. Они не предоставляются для устройств ввода, и с ними нельзя производить никаких манипуляций.

     

**Панорамирование (одномерный переполнение) с одной осью**
...
**Полилинии панорамирования (двухмерный переполнение)**
...
**Представление разбитого на страницы**
...
**Цепочки внедренный или вложенного содержимого**
...
При сенсорном взаимодействии сдвиг с помощью жеста прокрутки или скольжения одним или несколькими пальцами похож на прокрутку с помощью мыши. Сдвиг больше похож на поворот колесика мыши или перетаскивание ползунка полосы прокрутки, чем на щелчок в полосе прокрутки. Если различие в API, или требуется пользовательский Интерфейс Windows конкретного устройства, мы просто см. оба взаимодействия как сдвига.

> <div id="main">
> <strong>Windows 10 Fall Creators Update — изменение поведения</strong> по умолчанию, вместо выделения текста active пера теперь выполняет прокрутку/планы в приложениях универсальной платформы Windows (например, касания, сенсорной панели и пассивный пера).  
> Если ваше приложение зависит от предыдущего поведения, вы можете переопределить прокрутку пера и вернуться к предыдущему алгоритму. Дополнительные сведения см. в разделе Справочник по API для <a href="https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.scrollviewer">класс ScrollViewer</a>.
> </div>
...
-   Замедление (инерции): Поднятие панорамирования запустить замедление причины пальцами. напоминает торможение на скользкой поверхности);
-   Распределение: Панорамирования momentum во время замедление вызывает небольшое влияние bounce обратной, при достижении или точку привязки, либо границе области содержимого.
...
**Панорамирование пользовательского интерфейса**
...
**Примечание**  панорамирование индикаторы отображаются, только если контакта сенсорного ввода находится внутри pannable области. Аналогичным образом полоса прокрутки отображается, только когда указатель мыши, курсор пера или фокус клавиатуры находится в области, поддерживающей прокрутку.
