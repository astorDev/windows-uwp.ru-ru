...
> **Важные API**: [**Windows.Media.SpeechRecognition**](https://docs.microsoft.com/uwp/api/Windows.Media.SpeechRecognition)
...
## <a name="configure-speech-recognition"></a>Настройка средства распознавания речи

Для поддержки распознавания речи с вашим приложением, пользователю необходимо подключиться и включение микрофона на своем устройстве и принять корпорации Майкрософт о конфиденциальности предоставление разрешения для приложения для его использования.

Чтобы автоматически пользователю с помощью диалогового окна системы, запрашивающего разрешение на доступ и использование звуковому каналу микрофона (пример из [распознавания речи и синтеза речи-пример](https://go.microsoft.com/fwlink/p/?LinkID=619897) показано ниже), просто задайте  **"Микрофон"** [возможность устройства](https://docs.microsoft.com/uwp/schemas/appxpackage/appxmanifestschema/element-devicecapability) в [манифест пакета приложения](https://docs.microsoft.com/uwp/schemas/appxpackage/appx-package-manifest). Дополнительные сведения см. в разделе [объявления возможностей приложения](https://docs.microsoft.com/windows/uwp/packaging/app-capability-declarations).

![Политика конфиденциальности для доступ к микрофону](images/speech/privacy.png)

Если пользователь нажимает кнопку Да, чтобы предоставить доступ к микрофону, приложение будет добавлено в список утвержденных приложений на параметры -> конфиденциальность -> Страница "микрофон". Тем не менее как пользователь может выбрать отключить этот параметр в любое время, следует убедиться, что приложение имеет доступ к микрофону перед попыткой его использования.

Если также требуется для поддержки режима диктовки, Cortana или другими службами распознавания речи (такие как [предопределенные грамматики](#predefined-grammars) определенные в разделе ограничение), необходимо также подтвердить, что **распознавания речи Online** (Параметры -> конфиденциальность -> речи) включен.

В этом фрагменте показано, как можно проверить при наличии микрофон, и если он имеет разрешение на работу приложения.
...
**Примечание**  поскольку предопределенные грамматики диктовки и поиска в Интернете могут быть большими, и они находятся в сети (не на устройстве), производительность может быть так же быстро, как и в пользовательских грамматики, установленных на устройстве.     
...
Сначала мы инициализируем глобальную переменную (HResultPrivacyStatementDeclined) до значения HResult 0x80045509. См. в разделе [обработки исключений для в C\# или Visual Basic](https://docs.microsoft.com/previous-versions/windows/apps/dn532194(v=win.10)).
...
См. в разделе [ **SpeechRecognitionTopicConstraint**](https://docs.microsoft.com/uwp/api/Windows.Media.SpeechRecognition.SpeechRecognitionTopicConstraint).

### <a name="programmatic-list-constraints"></a>Программный список ограничений 
...
См. в разделе [ **SpeechRecognitionListConstraint**](https://docs.microsoft.com/uwp/api/Windows.Media.SpeechRecognition.SpeechRecognitionListConstraint).

### <a name="srgs-grammars"></a>Грамматик SRGS

Грамматика SRGS – это статический документ, который, в отличие от программного ограничения-списка, использует формат XML, определенный в спецификации [SRGS Version 1.0](https://go.microsoft.com/fwlink/p/?LinkID=262302). Грамматика SRGS предоставляет больший контроль над распознаванием речи и позволяет создавать несколько семантических значений в одном распознавании.

 См. в разделе [ **SpeechRecognitionGrammarFileConstraint**](https://docs.microsoft.com/uwp/api/Windows.Media.SpeechRecognition.SpeechRecognitionGrammarFileConstraint).

### <a name="voice-command-constraints"></a>Голосовые команды ограничения

С помощью XML-файлов определения голосовых команд можно задать команды, которые пользователь может произносить, чтобы выполнять определенные действия при активации вашего приложения. Дополнительные сведения см. в разделе [активировать приложение переднего плана с помощью голосовых команд через Cortana](https://docs.microsoft.com/cortana/voice-commands/launch-a-foreground-app-with-voice-commands-in-cortana).

См. в разделе [ **SpeechRecognitionVoiceCommandDefinitionConstraint**](https://docs.microsoft.com/uwp/api/Windows.Media.SpeechRecognition.SpeechRecognitionVoiceCommandDefinitionConstraint)/

**Примечание**  используется тип ограничения зависит от сложности распознавания опыта, вы хотите создать. Каждый может оказаться наилучшим для конкретной задачи распознавания, и в приложении может найтись место всем типам ограничений.
