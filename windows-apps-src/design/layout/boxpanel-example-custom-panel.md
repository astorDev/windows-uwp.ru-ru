...
> [!NOTE]
> Внутренняя логика [ **StackPanel** ](https://msdn.microsoft.com/library/windows/apps/br209635) также имеет такое поведение: **StackPanel** передает значение бесконечный измерения [ **мер** ](https://msdn.microsoft.com/library/windows/apps/br208952) на дочерние элементы, указывающее, что не имеет ограничений на дочерние элементы в измерении «ориентация». Класс **StackPanel** обычно принимает размер динамически, чтобы разместить все дочерние элементы в стеке, который растет в этом измерении.
