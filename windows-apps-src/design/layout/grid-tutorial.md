---
Description: В этом руководстве описывается создание базового пользовательского интерфейса приложения. В ней поясняется и демонстрируется использование двух наиболее распространенных элементов XAML — Grid и StackPanel.
title: Используйте Grid и StackPanel для создания простого приложения.
template: detail.hbs
ms.date: 05/19/2017
ms.topic: article
...
- Windows 10 и Microsoft Visual Studio 2015 или более поздней версии. (Для текущих разработки и обновлений безопасности рекомендуется использовать новейшие версии Visual Studio) [Щелкните здесь, чтобы узнать, как настроить Visual Studio](../../get-started/get-set-up.md).
...
Новый элемент **Grid** создает набор строк и столбцов, определяя таким образом макет интерфейса приложения. Первый столбец имеет **ширину** "3\*", а второй — "5\*", разделив горизонтальное пространство между двумя столбцами с соотношением 3:5. Аналогичным образом две строки имеют **высоту** "2\*" и "\*" соответственно, поэтому **Сетка** выделяется в два раза больше пространства для первой строки, как для второй ("\*" совпадает с "1\*"). Эти соотношения сохраняются даже при изменении размера окна или смене устройства.
