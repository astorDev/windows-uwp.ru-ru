...
На практике используется сочетание статических и гибких элементов для создания пользовательского интерфейса. Вы по-прежнему используете статические элементы и значения в некоторых местах, но убедитесь, что общий пользовательский интерфейс отвечает различным разрешениям, размерам экранов и представлениям.
...
Свойства макета управляют размером и положением элемента. Чтобы создать Жидкий макет, используйте автоматическое или пропорциональное изменение размера элементов и разрешите панели макета располагать их дочерними элементами по мере необходимости. 

Ниже приведены некоторые распространенные свойства макета и способы их использования для создания гибких макетов.

**Высота и ширина**

Свойства [**Height**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.height) и [**Width**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.width) определяют размер элемента. Можно использовать статические значения, измеренные в эффективных пикселях, функцию автоматического выбора размера либо пропорциональный размер. 
...
Пропорциональное изменение размеров, также именуемое *задание размера с помощью звезды*, распределяет доступное пространство среди строк и столбцов сетки с помощью взвешенных пропорций. В XAML значения типа "звезда" выражаются как \* (или *n*\* для взвешенного размера Star). Например, чтобы указать, что один столбец двухстолбцового макета в пять раз шире, чем другой, используйте значения «5\*» и «\*» для свойств [**Width**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.columndefinition.width) в элементах [**ColumnDefinition**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ColumnDefinition).
...
Column_4 | **2**\* | После вычисления значений ширины для автоматических столбцов этот столбец получает часть оставшейся ширины. Ширина столбца Column_4 будет в два раза больше ширины столбца Column_2.
...
Чтобы получить размер элемента во время выполнения, используйте свойства только для чтения [**ActualHeight**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.actualheight) и [**ActualWidth**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.frameworkelement.actualwidth) вместо Height и Width.
...
> [!Important]
> В предыдущем примере присоединенное свойство VisualStateManager. VisualStateGroups задано для элемента **Grid** . При использовании StateTriggers всегда следует убедиться, что свойство VisualStateGroups является присоединенным к первому дочернему элементу корня, чтобы триггеры запускались автоматически. (Здесь **Grid** — это первый дочерний элемент корня **Page**).
...
Предыдущие действия создают только файл XAML, но не связанный с ним файл кода программной части. Вместо этого представление XAML связывается с существующим файлом кода программной части с помощью квалификатора DeviceName, который является частью имени файла или папки. Это имя квалификатора можно сопоставить со строковым значением, которое представляет семейство устройств для устройства, на котором в настоящее время запущено приложение, например «Настольное», «Планшет» или другое (см. раздел [**ResourceContext.QualifierValues**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.resources.core.resourcecontext.qualifiervalues)).
...
**Добавление XAML-страницы в приложение**
1. Выберите элемент "Проект" > "Добавить новый элемент". Откроется диалоговое окно "Добавление нового элемента".
...
- [Пример методов реагирования (GitHub)](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/XamlResponsiveTechniques)
...
- [Пример специализированного множественного представления (GitHub)](https://github.com/Microsoft/Windows-universal-samples/tree/master/Samples/XamlTailoredMultipleViews)
