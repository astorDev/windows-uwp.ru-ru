...
**Примечание**. Если контейнеры не являются четко определенные (например не отличительный цвет фона), или когда имеется ссылка, чтобы увидеть больше текста, затем использовать кнопку с многоточием.
