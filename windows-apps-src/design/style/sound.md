...
```
**ElementSoundPlayer** имеет три различных состояния: **На** **Off** и **автоматически**.
...
- **Автоматически**. Пространственные звук будет включен после звуковой включен. 
- **Off**: Пространственные аудио всегда равно off, даже если звука на.
- **Вкл.**: Всегда будет воспроизводиться пространственных аудио.
...
**ElementSoundMode** имеет два состояния: **Отключение** и **по умолчанию**. По умолчанию установлено значение **Default**. Если задано значение **Off**, все звуки, воспроизводимые элементом управления, *за исключением фокуса*, будут отключены.
...
При переходе между панелей или представления в пределах страницы приложения (см. в разделе [вкладки и преобразующая](../controls-and-patterns/pivot.md)), обычно наблюдается двунаправленный перемещения. Это означает, что можно перейти к следующему или предыдущему представлению/панели, не покидая текущую страницу приложения.
...
* [Документации по классу ElementSoundPlayer](https://docs.microsoft.com/en-us/uwp/api/windows.ui.xaml.elementsoundplayer)
