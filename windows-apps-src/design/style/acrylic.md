---
description: Тип кисть, которая создает прозрачные текстуру.
...
Наиболее заметная характеристика акрила — его прозрачность. Для акриловых поверхностей возможны два типа, определяющих, какое изображение проступает через этот материал.
 ...
 Слой несколько акриловая поверхности с осторожностью: несколько уровней acrylic фона можно создать отвлекает оптическое иллюзиях.
...
Рассмотрите следующие варианты использования, чтобы решить, как лучше встраивать acrylic в приложении:

### <a name="horizontal-navigation-or-commanding"></a>Навигация по горизонтали или системе команд

Если приложение не сможет использовать NavigationView, и вы планируете добавлять acrylic самостоятельно, рекомендуется использовать относительно прозрачные acrylic с прозрачностью оттенок 60%.
...
![Приложение карт, с помощью горизонтальной системе команд в приложении](images/Maps_In_App_Acrylic_1.png)

Кроме того наличие вашего содержимого расширение или прокрутки под acrylic вверху обеспечит приложения более мощных и простой.

### <a name="vertical-panes"></a>Области по вертикали

Для вертикального областей или областей, которые помогают разделе Выкл содержимого приложения мы рекомендуем использовать вместо acrylic непрозрачный фон. Если на вертикальной панели открывается на основе содержимого, как показано в NavigationView **Compact** или **минимальным** режимов, мы рекомендуем использовать в приложении acrylic помогает поддерживать контекст страницы, когда у пользователя есть в этой области откройте.

### <a name="transient-surfaces"></a>Временной поверхности

Для приложений с помощью меню всплывающее окно, не модальные всплывающие окна, или закрыть свет областей, рекомендуется использовать acrylic фона.
...
В режиме высокой контрастности вместо акриловой поверхности пользователи видят знакомый цвет фона по своему выбору. Кроме того как acrylic фона, так и в приложении acrylic отображаются как сплошным цветом:
 - Когда пользователь отключает прозрачности в параметрах > персонализации > цвета
 - При активации режима Экономия заряда
 - приложение запускается на оборудовании с низким быстродействием.

Кроме того только на фон acrylic заменит его полупрозрачность и текстуры сплошным цветом:
...
Не рекомендуется размещать на акриловой поверхности текст контрастного цвета, так как такое сочетание, скорее всего, не будет соответствовать минимальным требованиям к коэффициенту контрастности при размере шрифта 15 пикселей. Постарайтесь не размещать [гиперссылки](../controls-and-patterns/hyperlinks.md) поверх элементов акрила. Кроме того, если вы решите использовать настройки цвета или уровня прозрачности, отличные от предусмотренных по умолчанию в ресурсах темы, проверьте, как это скажется на разборчивости текста.
...
<table>
    ...
    <tr>
        ...
        <td> <b>Рекомендуемый способ использования:</b> Это универсальные акриловая ресурсы, которые хорошо работают в самых разнообразных данные об использовании. Если ваше приложение содержит второстепенный текст цвета AltMedium с размером текста менее 18 пикселей, разместите за этим текстом 80 % акриловый ресурс, чтобы <a href="../accessibility/accessible-text-requirements.md">соответствовать требованиям к коэффициенту контрастности</a>. </td>
    </tr>
    <tr>
    ...
    <tr>
        <td> <b>Рекомендуемый способ использования:</b> Если приложение использует дополнительный текст AltMedium цвета с размером текста 18px или больше, можно поместить эти более прозрачным ресурсы акриловая 70% за текстом. Рекомендуем использовать эти ресурсы в верхней горизонтальной навигационной и командной областях вашего приложения.  </td>
    </tr>
    <tr>
    ...
    <tr>
        <td> <b>Рекомендуемый способ использования:</b> При помещении только основной текст цвет AltHigh через acrylic, приложения могут использовать эти ресурсы на 60%. Рекомендуем рисовать <a href="../controls-and-patterns/navigationview.md">вертикальную панель навигации</a> ("кнопку-гамбургер"), используя 60 % акрил. </td>
    </tr>
</table>
...
<table>
    <tr>
        <th align="center">Ключ ресурса</th>
        <th align="center">Непрозрачность тона</th>
        <th align="center"><a href="color.md">И резервных точек оттенок цвета</a> </th>
    </tr>
    <tr>
...
Акриловую поверхность в вашем приложении можно подкрасить в какой-либо цвет, чтобы придать ей фирменный стиль или добиться гармонии с другими элементами на странице. Чтобы перейти от черно-белой шкалы к цветной, необходимо определить собственные акриловые кисти с помощью указанных ниже свойств.
 ...
 - **TintOpacity** — непрозрачность цветного слоя. Мы рекомендуем 80% непрозрачности отправной точки, несмотря на то, что разные цвета может выглядеть более привлекательно в других translucencies.
 - **TintLuminosityOpacity**: управляет насыщенности, который сможет пройти через акриловая surface в фоновом режиме.
 - **BackgroundSource** — флаг, указывающий тип акриловой поверхности (акрил фона или приложения).
 - **FallbackColor**: сплошной цвет, заменяющий acrylic в Экономия заряда. Акрил фона также сменяется резервным цветом, если окно приложения на рабочем столе неактивно или приложение запущено на телефоне или Xbox.
...
![По сравнению с непрозрачности оттенка opactity яркость](images/LuminosityVersusTint.png)
...
> [!Note] 
> Если не указать значение TintLuminosityOpacity, система автоматически настроит его значение на основе TintColor и TintOpacity.
...
[**Отобразить выделение**](reveal.md)
