...
Рассмотрим следующие шаблоны использования, чтобы выбрать оптимальный способ включения акрил в приложение:

### <a name="horizontal-navigation-or-commanding"></a>Навигация по горизонтали или команды

Если ваше приложение не поддерживает NavigationView и вы планируете добавить акриловую поверхность, рекомендуется использовать акрил относительно Полупрозрачный с прозрачностью 60% оттенка.
...
![Приложение "карты", с помощью горизонтальной команд в приложении](images/Maps_In_App_Acrylic_1.png)

Кроме того наличие содержимого расширить или прокрутки в разделе акриловой вверху будет внешний вид приложения более современные методы и эффективной работы.

### <a name="vertical-panes"></a>Вертикальными панелями

Вертикальными панелями или поверхности, помогающие раздел отключать содержимого вашего приложения мы рекомендуем использовать непрозрачный фон вместо акрила. Если ваш вертикальными панелями открыть поверх содержимого, как в **Collapsed** или **Минимальный** режимы NavigationView мы рекомендуем использовать акрил приложения для сохранения контекста страницы, когда пользователь этой области откройте.

### <a name="transient-surfaces"></a>Промежуточный поверхности

Для приложений с помощью всплывающих элементов меню, всплывающих окон немодальным, или исчезающий панелями, мы рекомендуем использовать акрил фона.

![Шаблон почтового приложения, с помощью информационных всплывающий элемент](images/Mail_TransientContextMenu.png)

Многие из наших элементов управления будет использовать акрил по умолчанию. [MenuFlyouts](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/menus), [AutoSuggestBox](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/auto-suggest-box), [поле со списком](https://docs.microsoft.com/uwp/api/windows.ui.xaml.controls.combobox) и аналогичные элементов управления с помощью всплывающих окон dimiss свет будет использовать промежуточный акрил при их вызове.

> [!Note]
> Отрисовка акриловых поверхностей интенсивно GPU, что энергопотребление устройства и сокращает время работы от батареи. Эффекты акриловых поверхностей автоматически отключаются, когда устройство переходит в режим экономии заряда, и пользователи могут отключить эти эффекты для всех приложений, при желании.
...
Акриловую поверхность в вашем приложении можно подкрасить в какой-либо цвет, чтобы придать ей фирменный стиль или добиться гармонии с другими элементами на странице. Чтобы перейти от черно-белой шкалы к цветной, необходимо определить собственные акриловые кисти с помощью указанных ниже свойств.
 ...
 - **TintOpacity**— непрозрачность цветного слоя. Мы рекомендуем 80% непрозрачности как отправную точку, несмотря на то, что разные цвета могут выглядеть привлекательнее при других translucencies.
...
* Не создавайте рабочего стола arylic поверхности большого фонового приложения — это нарушает модели акрил используется главным образом для промежуточный поверхности.
