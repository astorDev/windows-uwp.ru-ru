...
* Используйте акрил приложения для поддержки пользовательского интерфейса, такие как на поверхности, которые могут перекрываться содержимого при прокрутке или взаимодействовать.
...
Приведенными ниже шаблонами использования, чтобы выбрать оптимальный способ включения акрил в приложение:
...
Кроме того наличие вашего содержимого расширения или прокрутки в разделе акрил в верхней будет внешний вид приложения более современные методы и эффективной работы.

### <a name="vertical-panes"></a>Вертикальными панелями

Вертикальными панелями или поверхности, помогающие разделе Отключение содержимого вашего приложения мы рекомендуем использовать непрозрачный фон вместо акрила. Если ваш вертикальными панелями открыть поверх содержимого, как в NavigationView **Compact** или **Минимальный** режим, мы рекомендуем использовать акрил приложения в целях обеспечения контекста страницы, когда пользователь этой панели открывать.
...
В режиме высокой контрастности вместо акриловой поверхности пользователи видят знакомый цвет фона по своему выбору. Кроме того как акрил фона и акрил приложения отображаются сплошным цветом:
 - Когда пользователь отключает прозрачность в > параметры персонализации > цвета
