---
author: mijacobs
description: Тип кисти, который создает прозрачные текстуры.
...
:::row:::
    :::column:::
        Acrylic в быстрое темы ![Acrylic в быстрое темы](images/Acrylic_LightTheme_Base.png)
    :::column-end:::
    :::column:::
        Acrylic в теме темный ![Acrylic в темный темы](images/Acrylic_DarkTheme_Base.png)
    :::column-end:::
:::row-end:::
...
:::row:::
    ::: диапазон столбцов::: ![некоторые изображения](images/XAML-controls-gallery-app-icon.png)
    :::column-end:::
    ::: диапазон столбцов = «2»::: **Коллекция элементов управления XAML**<br>
        При наличии установленного приложения коллекция элементов управления XAML, щелкните <a href="xamlcontrolsgallery:/item/Acrylic">здесь</a> для открытия приложения и просмотреть acrylic в действии.
...
Если ваше приложение не может использовать NavigationView и планирование на добавления acrylic, рекомендуется использовать относительно прозрачные acrylic с непрозрачность tint 60%.
...
> [!Note]
> Отображение acrylic поверхности интенсивно графического Процессора, которой можно увеличить потребление электроэнергии устройства и сократить время автономной работы. Acrylic эффекты автоматически отключаются при устройств в режим заставки батарей, и пользователи могут отключить acrylic эффекты для всех приложений, при этом они.


## <a name="acrylic-blend-types"></a>Типы акрилового материала
Наиболее заметно характеристика acrylic's является его translucency. Для акриловых поверхностей возможны два типа, определяющих, какое изображение проступает через этот материал.
...
В режиме высокой контрастности вместо акриловой поверхности пользователи видят знакомый цвет фона по своему выбору. Кроме того acrylic фон и acrylic из приложения отображаются в виде сплошного цвета:
 - Когда пользователь выключает прозрачность в параметрах > личной настройки > цвет
 - При активации режима батарей заставки
 - приложение запускается на оборудовании с низким быстродействием.

Кроме того только acrylic фона заменит его translucency и текстуры сплошной цвет:
...
<table>
    ...
    <tr>
        <td> <b>Рекомендуется использования:</b> Если ваше приложение использует дополнительного текста цвет AltMedium 18px или большего размера текста, можно поместить эти дополнительные прозрачные ресурсы acrylic 70% за текстом. Рекомендуем использовать эти ресурсы в верхней горизонтальной навигационной и командной областях вашего приложения.  </td>
    </tr>
    <tr>
...
Акриловую поверхность в вашем приложении можно подкрасить в какой-либо цвет, чтобы придать ей фирменный стиль или добиться гармонии с другими элементами на странице. Чтобы перейти от черно-белой шкалы к цветной, необходимо определить собственные акриловые кисти с помощью указанных ниже свойств.
 ...
 - **TintOpacity**— непрозрачность цветного слоя. Мы рекомендуем непрозрачность 80% в качестве отправной точки, несмотря на то, что разные цвета может выглядеть более удобных в других translucencies.
 - **BackgroundSource**— флаг, указывающий тип акриловой поверхности (акрил фона или приложения).
 - **FallbackColor**: сплошной цвет, которая заменяет acrylic в заставки батарей. Акрил фона также сменяется резервным цветом, если окно приложения на рабочем столе неактивно или приложение запущено на телефоне или Xbox.
