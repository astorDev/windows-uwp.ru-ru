...
- Можно легко установить цвета светлой, темной или распространено для обеих тем, установив одно свойство по API
- Цвета на **ColorSchemeResources** каскадом до все похожие элементы управления, которые также используют этот цвет системы
  * Это гарантирует, что у вас будет материала единообразной цветовой вашего приложения при сохранении его внешний вид вашего фирменного стиля
- Влияет на все визуальные состояния, анимации и варианты opacity без необходимости изменить шаблон

### <a name="how-to-use-colorschemeresources"></a>Использование ColorSchemeResources

ColorSchemeResources — это API, который сообщает системе, какие ресурсы предусматривать место. ColorSchemeResources должен выполнить с [x: Key](https://docs.microsoft.com/windows/uwp/xaml-platform/x-key-attribute), который может использоваться один из трех вариантов:
- По умолчанию
...
Параметр x: Key гарантирует, что цвета соответствующим образом изменить тему системы или приложения, следует требуется различных пользовательского внешнего вида при в двух цветовых.
...
Например, если определенные два системных цветов - **SystemBaseLowColor** и **SystemBaseMediumLowColor** в виде сетки, а затем поместили две кнопки на странице: одна внутри этой сетки и один за пределами:
...
![с областью системных цветов "Button"](images/color/scopedcolors_cyan_button.png)

Тем не менее так как все наши системных цветов располагаться сверху вниз, чтобы другие элементы управления слишком, задав **SystemBaseLowColor** и **SystemBaseMediumLowColor** повлияет на несколько кнопок. В этом случае элементы управления, как **ToggleButton**, **RadioButton** и **ползунок** будет также быть влияет эти изменения цвета системы, следует этих элементов управления можно поместить в выше области exampl сетки.
Если вы хотите определить область системы цвет изменение *только один элементам управления* можно сделать путем определения **ColorSchemeResources** внутри этого элемента управления ресурсами:
...
Вложенное системных цветов можно также она выполняется таким образом, переключив **ColorSchemeResources** в ресурсах вложенных элементов в разметке макет вашего приложения:
...
В таком случае наследования **Button_A** цвета определить в **Grid_A**на ресурсах и **Вложенные кнопки** — это наследовании **Grid_B**элемента ресурсов цвета. С помощью расширения, это означает, что любые другие элементы управления размещаются внутри **Grid_B** будет проверить или применить **Grid_B**элемента ресурсов во-первых, перед проверкой или применения ресурсов **Grid_A**пользователя, и наконец применения наших цвета по умолчанию, если ничего не определены на уровне страницы или приложения.
...
Не ограничиваются контейнера или ресурсы страницы, а также можно определить эти системные цвета в ResourceDictionary, могут быть объединены в любой области так, как вы обычно необходимо объединить словарь.
...
Теперь все ресурсы темы и пользовательские цвета можно помещаются в один словарь ресурсов **MyCustomTheme** и область, где без необходимости беспокоиться о дополнительных помех в разметке макета.
