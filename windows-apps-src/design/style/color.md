...
- Можно легко значение цвета светлой, темной и распространено на обоих темы, задав для одного свойства по API
...
ColorSchemeResources — это API-Интерфейс, который сообщает системе, какие ресурсы что область where. ColorSchemeResources должен выполнить с [x: Key](https://docs.microsoft.com/windows/uwp/xaml-platform/x-key-attribute), который может использоваться один из трех вариантов:
- По умолчанию
  * Будет отображаться изменения цвета [света](https://docs.microsoft.com/windows/uwp/design/style/color#light-theme) и [темной](https://docs.microsoft.com/windows/uwp/design/style/color#dark-theme) теме
- Light
...
- Dark
  * Будет отображать изменения цвета только в [темной теме](https://docs.microsoft.com/windows/uwp/design/style/color#dark-theme)

Параметр x: Key гарантирует, что цветов соответствующим образом изменить тему системы или приложения, вы хотите другой пользовательский внешний вид при в двух цветовых.

### <a name="how-to-apply-scoped-colors"></a>Применение цвета с областью действия

Определение ресурсы с помощью **ColorSchemeResources** API-интерфейса в XAML позволяет воспользоваться любой цвет системы или кисти, которая находится в библиотеку [ресурсов темы](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/xaml-theme-resources) и переопределить их в пределах области страницы или контейнера.
...
Вы получите **Button_A** , применяемые цвета и **Button_B** останется привлекательный как кнопка по умолчанию нашей системы:
...
Если вы хотите определить область системы цвет изменить *только один элементам управления* можно сделать путем определения **ColorSchemeResources** внутри этого элемента управления ресурсами:
...
```
По сути имеют то же самое как перед, но теперь любых других элементов управления, добавлен к сетке не получают изменения цвета. Это связано с эти системные цвета относятся к **Button_A** только.

### <a name="nesting-scoped-resources"></a>Ресурсы уровня вложения

Вложенное системных цветов также возможно и выполняется таким образом, переключив **ColorSchemeResources** в ресурсы вложенных элементов в разметке макет вашего приложения:
...
В этом примере наследования **Button_A** цвета определить **Grid_A**ресурсы и **Вложенные кнопки** — это наследовании **Grid_B**ресурсы цвета. С помощью расширения, это означает, что любые другие элементы управления размещаются внутри **Grid_B** будет проверять или применить **Grid_B**ресурсы во-первых, перед проверкой или применения **Grid_A**ресурсы, и наконец применения наших цвета по умолчанию, если ничего не определяется уровне страницы или приложения.
...
Не ограничены только для контейнера или ресурсы страницы, а также можно определить эти системные цвета в ResourceDictionary, могут быть объединены в любой области так, как вы обычно следует объединить словарь.
...
На странице, содержащий макет просто объедините этот словарь в область, которую требуется:
...
Теперь все ресурсы темы и пользовательских цветов можно в один словарь ресурсов **MyCustomTheme** и областью, где без необходимости беспокоиться о дополнительных помехи в разметке макета.
