...
Параметр x: Key гарантирует, что цвета соответствующим образом изменить тему системы или приложения, вы хотите различных пользовательского внешнего вида в любом темы.
...
Например, если определены два системных цветов - **SystemBaseLowColor** и **SystemBaseMediumLowColor** в виде сетки, а затем поместили две кнопки на странице: один внутри этой сетки и один за пределами:
...
```
По сути имеют то же самое как перед, но теперь любых других элементов управления, добавлен к сетке не ответит изменения цвета. Это связано с эти системные цвета относятся к **Button_A** только.

### <a name="nesting-scoped-resources"></a>Ресурсы уровня вложения

Вложенное системных цветов также возможно и выполняется таким образом, переключив **ColorSchemeResources** в ресурсах вложенных элементов внутри разметки макет вашего приложения:
...
В этом примере наследования **Button_A** цвета определить в ресурсах **Grid_A**элемента и **Вложенные кнопки** — это наследовании **Grid_B**элемента ресурсов цвета. С помощью расширения, это означает, что любые другие элементы управления размещаются внутри **Grid_B** будет проверить или применить **Grid_B**элемента ресурсов во-первых, перед проверкой или применения ресурсов **Grid_A**пользователя, и наконец применения наших цвета по умолчанию, если ничего не определены на уровне страницы или приложения.
...
Теперь все ресурсы темы и пользовательские цвета можно помещаются в один словарь ресурсов **MyCustomTheme** и предусматривать там, где без необходимости беспокоиться о дополнительных помех в разметке макета.
