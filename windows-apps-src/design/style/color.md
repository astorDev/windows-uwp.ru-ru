...
Помимо определения собственные цвета в вашем приложении, можно также определить область наших systematized цвета для нужного регионов во всем приложении с помощью тега **ColorSchemeResources** . Этот API позволяет вам не только колоризации и темы больших групп элементов управления одновременно, задав несколько свойств, но также предоставляет вам множество других систем преимущества, не получите обычно с определении собственные цвета вручную:

- Не повлияет на любой цвет, настраиваются с помощью **ColorSchemeResources** высокой контрастности
  * Это означает ваше приложение, будут доступны другим пользователям без дополнительных проектирования и разработки затраты
- Можно легко установить цвета светлой, темной или распространено на использовании обеих тем, задав для одного свойства по API
- Цвета задания **ColorSchemeResources** каскадом до все похожие элементы управления, которые также используют этот цвет системы
...
ColorSchemeResources — это API, который сообщает, что системы, какие ресурсы что область, где. ColorSchemeResources должны выполнить с [x: Key](https://docs.microsoft.com/windows/uwp/xaml-platform/x-key-attribute), может быть один из трех вариантов:
- По умолчанию
...
Параметр x: Key гарантирует, что цветов соответствующим образом изменить тему системы или приложения, вы хотите различных пользовательского внешнего вида в любом темы.

### <a name="how-to-apply-scoped-colors"></a>Применение цвета с областью действия

Определение ресурсы с помощью **ColorSchemeResources** API-интерфейса в XAML позволяет воспользоваться любой системный цвет или кисти, которая находится в нашей библиотеке [ресурсы темы](https://docs.microsoft.com/windows/uwp/design/controls-and-patterns/xaml-theme-resources) и переопределить их в пределах области страницы или контейнера.

Например, если вы определенные два системных цветов - **SystemBaseLowColor** и **SystemBaseMediumLowColor** внутри grid, а затем поместили две кнопки на странице: один внутри этой сетки и один за пределами:
...
Получите **Button_A** , примененные новые цвета и **Button_B** будет оставаться привлекательный как кнопка по умолчанию нашей системы:

![с областью системных цветов на кнопке](images/color/scopedcolors_cyan_button.png)

Тем не менее так как все наши системных цветов каскадом слишком до других элементов управления, задав **SystemBaseLowColor** и **SystemBaseMediumLowColor** повлияет на больше, чем просто кнопки. В этом случае элементы управления, как **ToggleButton**, **RadioButton** и **ползунок** будет также быть влияет эти изменения цвета системы, следует этих элементов управления можно поместить в выше области exampl сетки.
...
```
По сути имеют то же самое как перед, но теперь не выбирать любых других элементов управления, добавлен к сетке изменения цвета. Это связано с эти системные цвета относятся к **Button_A** только.

### <a name="nesting-scoped-resources"></a>Ресурсы предусматривать вложения

Системные цвета, вложенное также возможны и выполняется таким образом, переключив **ColorSchemeResources** в ресурсы вложенных элементов внутри разметки макет вашего приложения:
...
В этом примере наследования **Button_A** цвета определить в ресурсах **Grid_A**пользователя и **Вложенные кнопки** — это наследовании **Grid_B**ресурсы цветов. С помощью расширения, это означает, что любые другие элементы управления размещаются внутри **Grid_B** будет проверить или применить ресурсы **Grid_B**сначала перед проверкой или применения **Grid_A**ресурсы, и наконец применения наших цвета по умолчанию, если ничего не определяется уровне страницы или приложения.
...
Во-первых следует создать ResourceDictionary. Затем поместите **ColorSchemeResources** в ThemeDictionaries и переопределить нужные системные цвета:
...
На странице, содержащий макет просто объедините этот словарь в в области, которые требуется:
...
Теперь все ресурсы темы и пользовательские цвета можно разместить в один словарь ресурсов **MyCustomTheme** и область, где без необходимости беспокоиться о дополнительных помехи в разметке макета.

### <a name="other-ways-to-define-color-resources"></a>Другие способы для определения ресурсы цвета
