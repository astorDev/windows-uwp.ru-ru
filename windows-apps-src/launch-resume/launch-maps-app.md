...
<p>Пример:</p>
<p>ss=1</p></td>
<td align="left"><p>Обозначает, что отображается представление на уровне улиц при <code>ss=1</code>. Если опустить параметр **ss**, будут получены те же результаты, что и при указании <code>ss=0</code>. Используйте совместно с параметром **cp**, чтобы указать местоположение представления на уровне улиц.</p>
<div class="alert">
...
<p>Кроме того, можно определить неполный маршрут. Например, можно задать только начало маршрута с помощью <code>rtp="A"~</code>. В этом случае отображается панель ввода маршрутов, в которой указанный пункт находится в поле **Из:**, а фокус находится в поле **Куда:**.</p>
<p>Если указан только конец маршрута, как в случае <code>rtp=~"B"</code>, отображается панель маршрутов, где указанный пункт находится в поле **Куда:**. Если доступно точное текущее местоположение, оно заполняется из поля **Из** с фокусом.</p>
...
Синтаксис параметров приложения «Карты» для схемы URI **ms-settings:** описан ниже. **maps-downloadmaps** указывается вместе с URI **ms-settings:** в формате **ms-settings:maps-downloadmaps?**, чтобы указать страницу автономных карт приложения «Параметры».
