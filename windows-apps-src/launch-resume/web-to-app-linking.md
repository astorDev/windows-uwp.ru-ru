...
Например, если в приведенном выше примере указано `"excludePaths" : [ "/news/*", "/blog/*" ]`, ваше приложение будет поддерживать все пути, начинающиеся с адреса вашего веб-сайта (например, msn.com), **за исключением** тех, которые указаны в разделе `/news/` и `/blog/`. **MSN.com/weather.HTML** будет поддерживаться, но не **MSN.com/News/Topnews.HTML**.
...
Пути исключений проверяются в первую очередь, и, если обнаруживается соответствие, эта страница будет открыта в браузере, а не в заданном приложении. В приведенном выше примере "/Невс/\*" включает все страницы по этому пути, а "/Невс @ no__t-1" (без прямой косой черты "Новости") содержит любые пути в разделе "newss @ no__t-2", например "невслокал/", "невсинтернатионал/" и т. д.
...
## <a name="test-it-out-local-validation-tool"></a>Протестируйте ее: Средство локальной проверки
...
% WINDIR% \\system32 @ no__t-1**апфострегистратионверифиер. exe**
...
-   Имя семейства пакетов (PFN): PFN приложения
-   Путь к файлу: Файл JSON для локальной проверки (например, C: \\SomeFolder @ no__t-1Windows-App-Web-Link).
...
KeyName Значение `ForceValidation`: `1`

## <a name="test-it-web-validation"></a>Протестируйте ее: Веб-проверка
