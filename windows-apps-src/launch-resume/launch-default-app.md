...
| Схема URI | Запускает |
| ----------:|----------|
|[bingmaps:, ms диск to: и ms стека to: ](#maps-app-uri-schemes) | Приложение «Карты» |
|[http:](#http-uri-scheme) | Веб-браузер по умолчанию |
|[mailto:](#email-uri-scheme) | Приложение электронной почты по умолчанию |
|[MS вызова:](#call-app-uri-scheme) |  Приложение «Звонок» |
|[MS чата:](#messaging-app-uri-scheme) | Приложение «Сообщения» |
...
Используйте **ms вызова:** Схема URI, чтобы запустить приложение вызова.
...
Используйте **mailto:** Схема URI, чтобы запустить приложение "Почта" по умолчанию.
...
Используйте **http:** Схема URI, чтобы запустить веб-браузер по умолчанию.
...
Используйте **ms чата:** Схема URI для запуска приложения обмена сообщениями Windows.
...
Используйте **ms tonepicker:** Схема URI, чтобы выбрать мелодии звонка, оповещений и системы тона. Можно также сохранить новые мелодии звонка и получить отображаемое имя звукового сигнала.
...
Используйте **ms yellowpage:** Схема URI, чтобы запустить приложение рядом номеров.

| Схема URI | Результаты |
|------------|---------|
| MS-yellowpage:? входной =\[ключевое слово\]& метод =\[строку или T9\] | Запуск приложения «Телефоны мест по соседству».<br>`input` — Это ключевое слово, которое требуется найти.<br>`method` ссылается на тип выполняемого поиска (строку или поиска T9).<br>Если параметр `method` имеет значение `T9` (тип клавиатуры), параметр `keyword` должен быть цифровой строкой, соответствующей буквам на клавиатуре T9, по которым требуется произвести поиск.<br>Если параметр `method` имеет значение `String`, параметр `keyword` представляет собой ключевое слово, которое требуется найти. |

### <a name="people-app-uri-scheme"></a>Схема URI приложения «Люди»

Используйте **людей ms:** Схема URI для запуска приложения "люди".
...
| ms-photos:videoedit?Action={action} | Необязательный параметр, который открывает его фотографии в заданном видео режиме редактирования, где {action} — один из: **SlowMotion**, **FrameExtraction**, **Trim**, **представление**, **рукописного ввода**. Если действие не задано, по умолчанию используется **View** |
| ms-photos:videoedit?StartTime={timespan} | Необязательный параметр, указывающий, откуда начать воспроизведение видео. `{timespan}` должен быть в формате `"hh:mm:ss.ffff"`. Если не указан, по умолчанию `00:00:00.0000` |

### <a name="settings-app-uri-scheme"></a>Схема URI приложения «Параметры»

Используйте **ms параметры:** Схема URI для [открыть параметры Windows и](launch-settings-app.md). Запуск приложения «Параметры» — важная составляющая создания приложения, учитывающего требования конфиденциальности. Если ваше приложение не может получить доступ к конфиденциальному ресурсу, рекомендуется предоставить пользователю удобную ссылку на параметры конфиденциальности для этого ресурса. Например, следующий URI открывает приложение «Параметры» и отображает параметры конфиденциальности камеры.
...
Используйте **ms-windows-store:** Схема URI для [запустите приложение универсальной платформы Windows](launch-store-app.md). Откройте страницы сведений о продукте, просмотра страниц и страницы поиска, и т.д. Например следующий URI открывает приложение универсальной платформы Windows и запускает домашней страницы Store.

`ms-windows-store://home/`

Дополнительные сведения см. в разделе [Запуск приложения UWP](launch-store-app.md).

### <a name="weather-app-uri-scheme"></a>Схема URI для приложения погоды

Используйте **msnweather:** Схема URI, чтобы запустить приложение погоды.

| Схема URI | Результаты |
|------------|---------|
| msnweather://forecast?LA=\[широты\]& ло =\[долготы\] | Запускает приложение погоды на странице прогноз на основе координат географического расположения.<br>`latitude` — Это Широта расположения.<br> `longitude` ссылается на Долгота расположения.<br> |
