...
1.  Создайте новый проект для фоновых задач и добавьте его в решение. Для этого щелкните правой кнопкой мыши узел решений в **Обозревателе решений** и выберите **Add** \> **Новый проект**. Затем выберите тип проекта **Компонента среды выполнения Windows** , имя проекта и нажмите кнопку ОК.
...
Запросите задержку в метод **Run** перед вызовом асинхронного метода. Сохраните отсрочки член класса данных таким образом, чтобы его можно получить доступ из асинхронного метода. Объявите задержку завершенной после выполнения асинхронного кода.
