...
2.  Создайте ссылку на проект фоновых задач в проекте приложения UWP. Для C# или с приложением на C++ в проекте приложения щелкните **ссылки на** правой кнопкой мыши и выберите **Добавить новую ссылку**. В разделе **Решение** выберите **Проекты**, затем выберите имя своего проекта фоновых задач и нажмите **ОК**.
...
4.  Если в вашей фоновой задаче выполняется асинхронный код, для нее необходимо использовать задержку. Если вы не используете задержку, то процесс фоновой задачи может неожиданно Если метод **Run** возвращается до все асинхронные работа выполняется до завершения.
...
> [!NOTE]
> Вы также можете создать функцию для регистрации фоновых задач&mdash;см. [Регистрация фоновой задачи](register-a-background-task.md). В этом случае вместо выполнения следующих трех шагов, можно просто создать триггер и предоставить их функции регистрации, указав имя задачи, точку входа задачи и (необязательно) условие.
...
Например этот код создает новую фоновую задачу и задает его для выполнения при срабатывании триггера **TimeZoneChanged** :
...
Следует зарегистрировать метод с помощью [**BackgroundTaskCompletedEventHandler**](https://msdn.microsoft.com/library/windows/apps/br224781), чтобы ваше приложение могло получить результаты от фоновой задачи. При запуске или возобновлении приложения будет вызван помеченный метод, если фоновая задача завершилась с момента последней работы приложения на переднем плане. (Метод OnCompleted будет вызван немедленно, если фоновая задача завершается во время работы приложения на переднем плане в настоящее время.)
...
5.  В **точка входа:** textbox, введите пространство имен и имя вашего фонового класса для Tasks.ExampleBackgroundTask — в этом примере.
