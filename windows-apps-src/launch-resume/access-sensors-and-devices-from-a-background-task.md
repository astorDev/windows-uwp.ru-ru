...
[**DeviceUseTrigger** ](https://msdn.microsoft.com/library/windows/apps/dn297337) позволяет вашей датчиков доступа приложение универсальной Windows и периферийные устройства в фоновом режиме, даже в том случае, если приостановлено приложения переднего плана. Например, в зависимости от того, где выполняется приложение, оно может использовать фоновую задачу, чтобы синхронизировать данные с устройством или отслеживать показания датчиков. Чтобы сохранить уровень заряда и обеспечить соответствующее согласие пользователя, использование [**DeviceUseTrigger**](https://msdn.microsoft.com/library/windows/apps/dn297337) регулируется политиками, описанными в этой теме.

Для доступа к датчикам или периферийным устройствам в фоновом режиме создайте фоновую задачу, которая использует [**DeviceUseTrigger**](https://msdn.microsoft.com/library/windows/apps/dn297337). Пример выполнения на ПК см. в разделе [Пример настраиваемого устройства USB](https://go.microsoft.com/fwlink/p/?LinkId=301975 ). Пример для телефона см. в разделе [Пример фоновых датчиков](https://go.microsoft.com/fwlink/p/?LinkId=393307).
...
| API датчиков      | ![deviceservicingtrigger поддерживает универсальные API датчиков](images/ap-tools.png) (ограничено датчиками в [семействе универсальных устройств](https://msdn.microsoft.com/library/windows/apps/dn894631)) |
...
**Важные**    учитывать следующие важные моменты при использовании [ **DeviceUseTrigger**](https://msdn.microsoft.com/library/windows/apps/dn297337):

-   Возможность программно активации фоновой задачи, использующие [ **DeviceUseTrigger** ](https://msdn.microsoft.com/library/windows/apps/dn297337) впервые появилась в Windows 8.1 и Windows Phone 8.1.
...
**Совет**  чтобы увидеть, как работают эти фоновые задачи, загрузите образец. Пример выполнения на ПК см. в разделе [Пример настраиваемого устройства USB](https://go.microsoft.com/fwlink/p/?LinkId=301975 ). Пример для телефона см. в разделе [Пример фоновых датчиков](https://go.microsoft.com/fwlink/p/?LinkId=393307).
 
...
| Еще не достигнуто максимальное число фоновых задач [DeviceUseTrigger](https://msdn.microsoft.com/library/windows/apps/dn297337) на устройстве (на котором выполняется приложение). | **Семейство настольных устройств**: Неограниченного количества задач можно зарегистрировать и выполняться параллельно. **Семейства мобильных устройств**: Задача 1 на устройстве 512 МБ; в противном случае — 2 задачи можно зарегистрировать и параллельного выполнения. |
| Максимальное число периферийных устройств или датчиков, доступных приложению из одной фоновой задачи [DeviceUseTrigger](https://msdn.microsoft.com/library/windows/apps/dn297337) при использовании поддерживаемых API или протоколов. | Без ограничений |
...
| Ограничение по времени по часам — общее количество времени, в течение которого задача приложения может выполняться в фоновом режиме. | **Семейство настольных устройств**: 10 минут. **Семейства мобильных устройств**: Без временных ограничений. Для сохранения ресурсов не более 1 или 2 задач могут выполняться одновременно. |
