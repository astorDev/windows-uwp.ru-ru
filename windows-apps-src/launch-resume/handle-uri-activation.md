...
> [!NOTE]
> При запуске через контракт протокола, убедитесь, что, кнопку "Назад" должна возвращать пользователя обратно на экран, который запустил это приложение, а не к предыдущему содержимому приложения.
