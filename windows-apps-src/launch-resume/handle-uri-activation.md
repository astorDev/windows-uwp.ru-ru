...
| **Исполняемый файл** | Указывает исполняемый файл по умолчанию для протокола. Если он не указан, используется исполняемый файл приложения. Если указан, строка должна содержать от 1 до 256 символов, заканчиваться на ".exe" и не содержать символов <, >, :, ", |, ? и *. Если указан, **точка входа** также используется. Если **точка входа** не указана, используется точка входа, заданная для приложения. |
...
> **Примечание.**  Если вы создаете новое имя схемы URI для своего приложения, следуйте рекомендациям [RFC 4395](http://go.microsoft.com/fwlink/p/?LinkID=266550). Это гарантирует, что созданное вами имя будет соответствовать стандартам для схем URI.
