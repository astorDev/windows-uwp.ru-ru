...
> Например, параметры URI могут использоваться для запуска приложения на странице учетной записи пользователя, однако их ни в коем случае не следует использовать для прямого внесения изменений в учетную запись пользователя. **Примечание.** Если вы создаете новое имя схемы URI для своего приложения, следуйте рекомендациям [RFC 4395](http://go.microsoft.com/fwlink/p/?LinkID=266550).
...
> При запуске через контракт протокола в приложениях должен быть предусмотрен пользовательский интерфейс, при помощи которого можно вернуться на начальную страницу приложения. **Примечание.** Эта статья адресована разработчикам приложений для Windows 10 на базе универсальной платформы Windows (UWP).
