...
* В процессе: к приложению и его фонового процесса выполнения в одном процессе
* Исходящий процесс: приложение и фонового процесса выполнялись в отдельных процессах.
...
Фон ожидания процесса задачи, устойчивости, так как фоновый процесс не может снизить процесс вашего приложения, если возникновения непредвиденных обстоятельств. Однако устойчивость, поступающие по цене больше сложности взаимодействия между процессами между задачами фон и приложения управления.

Фон ожидания процесса задач реализованы в качестве lightweight классы, реализующие интерфейс [**IBackgroundTask**](https://msdn.microsoft.com/library/windows/apps/br224794) под управлением операционной системы в отдельном процессе (backgroundtaskhost.exe). Регистрация в фоновом режиме с помощью класса [**BackgroundTaskBuilder**](https://msdn.microsoft.com/library/windows/apps/br224768) . Имя класса используется для указания точки входа при регистрации фоновой задачи.

В Windows 10 версии 1607 можно включить фоновую активность, не создавая фоновой задачи. Вместо этого можно запустить фонового кода непосредственно в процесс приложений переднего плана.
...
Добавьте условие **InternetAvailable** в фоновую задачу [BackgroundTaskBuilder.AddCondition](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.BackgroundTaskBuilder) для задержки активации фоновой задачи, пока не заработает сетевой стек. Это условие сохраняет питания, так как в фоновом не выполняется, пока есть доступ к сети. Это условие не поддерживает активацию в режиме реального времени.

Если фоновой задачи требуется подключение к сети, необходимо задайте [IsNetworkRequested](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.BackgroundTaskBuilder) , чтобы избежать сети копирование во время выполнения задачи фона. Это указывает инфраструктуре фоновых задач на необходимость поддержания соединения во время выполнения задачи, даже если устройство переходит в режим ожидания с подключением. Если фоновая задача не **IsNetworkRequested**, затем фоновой задачи не сможет получить доступ к сети при работе в режиме ожидания подключения (например, при отключении экрана телефона.)
 
Дополнительные сведения о состоянии задач фона [условия для выполнения в фоновом режиме](set-conditions-for-running-a-background-task.md)см.
...
Тем не менее для корпоративных приложений и приложений, которые не будут опубликованы в магазине Майкрософт, см [в фоновом режиме неограниченное время](run-in-the-background-indefinetly.md) , чтобы узнать, как использовать возможности для выполнения задач фона или расширенного выполнение сеанса в фоновом режиме неопределенное время.
...
Проверьте регистрацию фона задач во время запуска приложения. Убедитесь, что ваше приложение разгруппировании фоновых задач присутствуют в BackgroundTaskBuilder.AllTasks. Заново зарегистрируйте из них, которые не отображаются. Отмена регистрации все задачи, которые больше не требуется. Это гарантирует, что все регистрации задачи фона актуальны каждый раз при запуске приложения.
