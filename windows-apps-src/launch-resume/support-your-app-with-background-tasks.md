...
## Фоновые задачи, запускаемые в одном или нескольких процессах

Существует два подхода к реализации фоновых задач: в процессе и вне его. Поддержка выполняемых в процессе фоновых задач появилась в Windows 10 версии 1607, чтобы упростить написание фоновых задач. Однако вы по-прежнему можете создавать фоновые задачи вне процесса. См. рекомендации о том, когда записывать фоновую задачу в процессе и вне его, в разделе [Рекомендации по фоновым задачам](guidelines-for-background-tasks.md).
...
Чтобы быстро приступить к работе с фоновыми задачами, выполняемыми в том же процессе, выполните инструкции из раздела [Создание и регистрация фоновой задачи, выполняемой в том же процессе](create-and-register-a-singleprocess-background-task.md).

Чтобы быстро приступить к работе с фоновыми задачами, выполняемыми в разных процессах, см. раздел [Создание и регистрация фоновой задачи, которая запускается в отдельном процессе](create-and-register-a-background-task.md).
...
> [!IMPORTANT]
> Триггеры **DeviceUseTrigger** и **DeviceServicingTrigger** невозможно использовать с фоновыми задачами, выполняемыми в одном процессе.
