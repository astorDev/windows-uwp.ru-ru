...
Вне процесса фоновые задачи являются более устойчивыми, поскольку фоновый процесс не может прекратить работу приложения, если что-то пойдет не так. Но устойчивость порождает цену управление взаимоотношениями между процессами между приложением и фоновой задачи.

Выполняемые вне процесса фоновые задачи реализуются как облегченные классы, реализующие интерфейс [**IBackgroundTask**](https://msdn.microsoft.com/library/windows/apps/br224794) , ОС выполняет в виде отдельного процесса (backgroundtaskhost.exe). Регистрация фоновой задачи с помощью класса [**BackgroundTaskBuilder**](https://msdn.microsoft.com/library/windows/apps/br224768) . Имя класса используется для указания точки входа при регистрации фоновой задачи.
