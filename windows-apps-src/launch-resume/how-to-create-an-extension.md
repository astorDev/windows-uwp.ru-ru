...
- Разверните решение. Выполните **сборку** > , чтобы**Развернуть решение**.
...
При создании основного приложения для расширения вы создаете возможность разработать экосистему на основе вашего приложения, в рамках которой другие разработчики могут улучшить ваше приложение в таких направлениях, на которые вы, возможно, не рассчитывали или не имели ресурсов. Рассмотрите возможность расширения Microsoft Office, расширений Visual Studio, расширений браузера и т. д. Это позволяет создавать более широкие возможности для приложений, которые выходят за пределы функций, поставляемых с. Расширения могут увеличить полезность и долговечность приложения.
...
`<uap3:Extension Category="windows.appExtensionHost">` обозначает это приложение как узел расширения.
...
`<uap3:Extension Category="windows.appExtension">` обозначает это приложение как расширение.
...
`<uap3:Properties>` — это необязательный элемент, содержащий пользовательские метаданные, которые могут читаться в среде выполнения. В примере кода расширение реализовано как служба приложения, поэтому основному приложению нужен способ получения имени службы приложения для ее вызова. Имя службы приложения определяется в элементе <Service>, который уже определен (ему можно дать любое имя). Основное приложение в примере кода находит это свойство во время выполнения, чтобы получить имя службы приложения.
...
В этом примере протоколом для аргументов является набор **значений** , содержащий пары "ключ-значение" с именем "arg" + номер аргумента, например @no__t – 1 и `Arg2`. Основное приложение передает все аргументы классу **ValueSet**, после чего расширение использует нужные ему аргументы. Если расширение может вычислить результат, основное приложение ожидает от него возврата **ValueSet**, чтобы получить ключ с именем `Result`, который содержит значение расчета. Если этот ключ отсутствует, основное приложение делает вывод, что расширению не удалось выполнить расчет.
