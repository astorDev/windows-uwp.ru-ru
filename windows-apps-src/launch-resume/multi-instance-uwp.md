...
Устанавливаются два шаблона: **Приложение UWP с несколькими экземплярами**, содержащее шаблон для создания приложения c несколькими экземплярами, и **Приложение UWP с несколькими экземплярами и перенаправлением**, обеспечивающее дополнительную логику, на основе которой можно либо запускать новый экземпляр, либо выборочно активировать экземпляр, который уже был запущен. Например возможно может потребоваться только один экземпляр редактирования тот же документ, откройте, размещая экземпляра, этот файл на переднем плане, вместо запуска нового экземпляра.

Оба шаблона добавляют `SupportsMultipleInstances` для `package.appxmanifest` файла. Обратите внимание на префикс пространства имен `desktop4` и `iot2`: только проектов, ориентированных на настольные ПК и Интернета вещей (IoT), поддерживающих запуск нескольких экземпляров.
