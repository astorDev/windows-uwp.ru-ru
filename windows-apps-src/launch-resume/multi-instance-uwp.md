...
Из Windows 10 версии 1803 (10.0; Сборка 17134), приложение UWP, можно выбрать поддерживают несколько экземпляров. Если запущен экземпляр приложения UWP с несколькими экземплярами и поступают последующие запросы на активацию, платформа не будет активировать существующий экземпляр. Вместо этого будет создан новый экземпляр, выполняемый в отдельном процессе.

> [!IMPORTANT]
> Поддержка нескольких экземпляров поддерживается для приложений JavaScript, но перенаправление нескольких экземпляров. Так как поддержка нескольких экземпляров перенаправление не поддерживается для приложений JavaScript, [ **AppInstance** ](/uwp/api/windows.applicationmodel.appinstance) класс не подходит для таких приложений.

## <a name="opt-in-to-multi-instance-behavior"></a>Включить поведение несколькими экземплярами
...
Устанавливаются два шаблона: **Приложения универсальной платформы Windows несколькими экземплярами**, который предоставляет шаблон для создания нескольких экземпляров приложения, и **приложения универсальной платформы Windows перенаправления для несколькими экземплярами**, который предоставляет дополнительную логику, вы можете создать на либо запустить новый экземпляр или выборочно активируйте экземпляр, который уже был запущен. Например возможно требуется один экземпляр одновременно редактирования тот же документ, откройте, вставленные экземпляра, содержащего этот файл переднего плана, а не запуска нового экземпляра.

Добавьте оба шаблона `SupportsMultipleInstances` для `package.appxmanifest` файла. Обратите внимание, префикс пространства имен `desktop4` и `iot2`: только проектов, нацеленных на настольные системы, или проекты для Интернета вещей (IoT), поддержка нескольких экземпляров.
...
Чтобы увидеть это в действии, просмотрите этот видеоролик о создании нескольких экземпляров приложений универсальной платформы Windows.

> [!VIDEO https://www.youtube.com/embed/clnnf4cigd0]

Шаблон **Приложения UWP с несколькими экземплярами и перенаправлением** добавляет `SupportsMultipleInstances` в файл package.appxmanifest, как показано выше, а также добавляет **Program.cs** (или **Program.cpp**, если вы используете версию шаблона для C++) в ваш проект, содержащий функцию `Main()`. Логика перенаправления с активацией выполняется в функции `Main`. Шаблон для **Program.cs** показан ниже.

[ **AppInstance.RecommendedInstance** ](/uwp/api/windows.applicationmodel.appinstance.recommendedinstance) свойство представляет предоставленный оболочкой экземпляр предпочтительный для данного запроса активации, если таковой имеется (или `null` если ее нет). Если оболочка предоставляет предпочтение, можно перенаправить активации этого экземпляра или его можно пропустить, если выбран.
...
- Приложения могут использовать [ **ResourceGroup** ](https://docs.microsoft.com/windows/uwp/launch-resume/declare-background-tasks-in-the-application-manifest) объявления в манифесте для группирования нескольких фоновых задач в том же узле. Это конфликтует с моделью с несколькими экземплярами, где каждая активация переходит в отдельный хост-процесс. Поэтому приложения не могут объявлять и `SupportsMultipleInstances`, и `ResourceGroup` в манифесте.

## <a name="sample"></a>Пример

См. в разделе [пример многоэкземплярной](https://aka.ms/Kcrqst) пример перенаправления активации несколькими экземплярами.
