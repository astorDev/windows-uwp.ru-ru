...
> [!NOTE]
> Если устройству не хватает памяти, фоновые задачи могут быть завершены для всех семейств устройств (за исключением настольных компьютеров). Если не отображается в нехватки памяти исключение или приложение не обрабатывает его, а затем в фоновом завершается без предупреждения и события OnCanceled. Это позволяет не прерывать работу пользователя с активным приложением. Ваша фоновая задача должна поддерживать этот сценарий.
...
В методе **OnCanceled** , созданной на шаге 1 значение переменной флаг **\_CancelRequested** **значение true**.

Полный [Пример задач фона]( http://go.microsoft.com/fwlink/p/?linkid=227509) метод **OnCanceled** задает **\_CancelRequested** значение **true** и записывает потенциально полезные отладки.
...
В метод **Run** в фоновом зарегистрируйте метод обработчика событий **OnCanceled** перед началом работы. В фоновой задаче внутри процесса можно выполнить эту регистрацию как часть инициализации приложения. Например используйте следующую строку кода.
...
При получении запроса на отмену необходимо остановить работу метода, выполняющего фоновую работу, и выйти из него, если для переменной **\_cancelRequested** установлено значение **true**. Для выполнения задач в процессе фона это означает, возврат из метода **OnBackgroundActivated** . Для получения процесс фоновых задач это означает, возвращение из метод **Run** .

Измените код класса вашей фоновой задачи, чтобы проверить переменную флага во время выполнения задачи. Если **\_cancelRequested** становится задано значение true, остановка рабочего продолжить.

[Пример задач фона](http://go.microsoft.com/fwlink/p/?LinkId=618666) включает в себя проверку, которая останавливает периодических таймера обратного вызова при отмене в фоновом.
...
Для иллюстрации: пример кода показывает части метод **Run** (и таймера обратного вызова) из [примера задачи фона](http://go.microsoft.com/fwlink/p/?LinkId=618666).

## <a name="run-method-example"></a>Пример метода Run

Выполнить метод **Run** и код обратного вызова таймера, с [фона задач пример](http://go.microsoft.com/fwlink/p/?LinkId=618666) ниже показаны для контекста.
