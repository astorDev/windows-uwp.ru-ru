...
При получении запроса на отмену необходимо остановить работу метода, выполняющего фоновую работу, и выйти из него, если для переменной **\_cancelRequested** установлено значение **true**. Для фоновых задач, которые выполняются в этом же процессе, это означает возврат из метода **OnBackgroundActivated()**. Для фоновых задач вне процесса это означает возврат из метода **Run()**.
