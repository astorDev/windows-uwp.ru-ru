...
> [!NOTE]
> При запуске через контракт файлов, убедитесь, что этот кнопку "Назад" должна возвращать пользователя обратно на экран, который запустил это приложение, а не к предыдущему содержимому приложения.

Мы рекомендуем создать новый элемент XAML **Frame** для каждого события активации, открывающего новую страницу. Таким образом, стек переходов для нового фрейма XAML не включать предыдущее содержимое, которое могло в текущем окне во время приостановки приложения. Если вы решили использовать один элемент XAML **Frame** для запуска и для контрактов файла, должны очищать страницы в журнале навигации **кадра**перед переходом на новую страницу.
