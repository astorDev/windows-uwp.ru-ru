...
1. В Visual Studio 2015 или более поздней версии, создайте новый проект приложения универсальной платформы Windows и назовите его **AppServiceProvider**.
    ...
    2. В **создайте новый проект** выберите **пустое приложение (универсальные Windows) C#** . Это приложение будет предоставлять службу приложения другим приложениям UWP.
    3. Нажмите кнопку **Далее**, а затем назовите проект **AppServiceProvider**, выберите расположение для нее и нажмите кнопку **создать**.

2. Ответ на приглашение выберите **целевой** и **Минимальная версия** для проекта, выберите по крайней мере **10.0.14393**. Если вы хотите использовать новый **SupportsMultipleInstances** атрибут, необходимо использовать Visual Studio 2017 или Visual Studio 2019 и целевой **10.0.15063** (**Windows 10 Creators Update**) или более поздней версии.
...
4.  Переименуйте **Class1.cs** для **Inventory.cs**и замените код заглушки для **Class1** с новый класс фоновой задачи с именем **инвентаризации**:
    ...
    **OnTaskCanceled** вызывается, когда задача была отменена. Задача отменяется, когда клиентское приложение удаляет [AppServiceConnection](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.AppService.AppServiceConnection), клиентского приложения приостанавливается, ОС завершается или переходит в спящий режим или операционной системе не хватает ресурсов для выполнения задачи.
...
Обратите внимание, что **OnRequestReceived** — **async** так, как awaitable метод вызов сделан [SendResponseAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.appservice.appservicerequest.sendresponseasync) в этом примере.

Отсрочки берется таким образом, служба может использовать **async** методы в **OnRequestReceived** обработчика. Это гарантирует, что вызов **OnRequestReceived** не завершится до тех пор, пока не будет обработано сообщение.  [SendResponseAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.appservice.appservicerequest.sendresponseasync) отправляет результат вызывающей стороне. **SendResponseAsync** не сигнализирует о завершении вызова. Это задержка, сообщающий о завершении [SendMessageAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.appservice.appserviceconnection.sendmessageasync) , **OnRequestReceived** завершения. Вызов **SendResponseAsync** заключается в блок try/finally, так как необходимо выполнить, даже если отсрочки **SendResponseAsync** возникло исключение.

Приложения служб, использующих [ValueSet](https://docs.microsoft.com/uwp/api/Windows.Foundation.Collections.ValueSet) объектов для обмена данными. Размер данных, которые можно передать, ограничен только ресурсами системы. Не существует предопределенных параметров, которые можно использовать в **ValueSet**. Вам необходимо решить, какие значения параметров вы будете использовать для определения протокола для службы приложений. Вызывающий код необходимо писать с учетом этого протокола. В этом примере мы выбрали параметр с именем `Command`, значение которого указывает, что должна предоставить служба приложений: сведения об имени складской позиции или ее цене. Индекс имени складской позиции хранится в параметре `ID`. Возвращаемое значение хранится в параметре `Result`.

[AppServiceClosedStatus](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.AppService.AppServiceClosedStatus) enum возвращается вызывающему объекту указать, успешно ли выполнено вызов службы приложений. Сбой вызова службы приложений может возникнуть, если ОС прерывает конечную точку службы, так как ее ресурсы исчерпаны. Может возвращать Дополнительные сведения об ошибке с помощью [ValueSet](https://docs.microsoft.com/uwp/api/Windows.Foundation.Collections.ValueSet). В этом примере мы используем параметр `Status`, чтобы возвращать в вызывающий код более подробные сведения об ошибке.

Вызов [SendResponseAsync](https://docs.microsoft.com/uwp/api/windows.applicationmodel.appservice.appservicerequest.sendresponseasync) возвращает [ValueSet](https://docs.microsoft.com/uwp/api/Windows.Foundation.Collections.ValueSet) вызывающему объекту.
...
5. Замените заглушку обработчика нажатия кнопки указанным ниже кодом. Не забудьте включить объявление поля `inventoryService`.
   ...
   ```
    ...
    Объект [ValueSet](https://docs.microsoft.com/uwp/api/Windows.Foundation.Collections.ValueSet) с именем `message` создается для задания текста команды, нужно отправить в службу приложений. Пример службы приложения будет ждать команду, чтобы указать, какое из 2 действий необходимо выполнить. Мы получить индекс из текстового поля в клиентском приложении, а затем вызвать службу `Item` команду, чтобы получить описание элемента. Затем с помощью команды `Price` мы получаем цену элемента. Результат используется в качестве текста кнопки.

    Так как [AppServiceResponseStatus](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.AppService.AppServiceResponseStatus) определяет только то, удалось ли операционная система для совершения вызова в службе приложений, мы проверяем `Status` в [ValueSet](https://docs.microsoft.com/uwp/api/Windows.Foundation.Collections.ValueSet) мы получаем от приложения службы, чтобы убедиться, что он смог выполнить запрос.
...
* Используйте [Windows.ApplicationModel.AppService.AppServiceConnection](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.AppService.AppServiceConnection) для вызова службы.
