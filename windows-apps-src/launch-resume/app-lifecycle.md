...
Windows 10 версии 1607, появились два больше состояний модели приложения. **Запуск в фоновом режиме** и **выполняться в фоновом режиме**. Мы также рассмотрим эти новые состояния в следующих разделах.
...
**Примечание**  поскольку [ **возобновление** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.application.resuming) событие не вызывается из потока пользовательского интерфейса, необходимо использовать диспетчер, если код в обработчике resume обменивается данными с помощью пользовательского интерфейса. Пример кода см. в статье [Обновление потока пользовательского интерфейса из фонового потока](https://github.com/Microsoft/Windows-task-snippets/blob/master/tasks/UI-thread-access-from-background-thread.md).
...
Не существует какого-либо особого события, указывающего на то, что пользователь закрыл приложение. При закрытии приложения пользователем оно сначала приостанавливается, чтобы вы могли сохранить его состояние. В Windows 8.1 и более поздних версиях после закрытия пользователем приложения, приложение удаляется из экрана и переключение списка, но явно не завершен.

**Поведение закрыт на уровне пользователей:**   Если приложению требуется выполнение определенных действий при его закрытии пользователем, чем при его закрытии, Windows, можно использовать обработчик событий активации для определения ли приложение был завершен, пользователь, либо параметром Windows. Описания состояний **ClosedByUser** и **Terminated** приведены в справочных документах по перечислению [**ApplicationExecutionState**](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation.ApplicationExecutionState).
...
-   [**Windows.ApplicationModel** ](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel) пространства имен
-   [**Windows.ApplicationModel.Activation** ](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Activation) пространства имен
-   [**Windows.ApplicationModel.Core** ](https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Core) пространства имен
-   [**Windows.UI.Xaml.Application** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Application) класс (XAML)
-   [**Windows.UI.Xaml.Window** ](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Window) класс (XAML)
