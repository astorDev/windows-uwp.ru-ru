...
Для всестороннее **OnBackgroundActivated** пример, см. в разделе [преобразовать службу приложений для запуска в том же процессе, как его приложение host](convert-app-service-in-process.md).
