...
**Фоновые задачи. выполняемые в том же или в отдельном процессе:** в Windows 10 версии 1607 появились [фоновые задачи с одним процессом](create-and-register-a-singleprocess-background-task.md), которые позволяют выполнять фоновый код в том же процессе, что и основное приложение. Чтобы решить, какие фоновые задачи использовать (выполняющиеся в том же или в отдельном процессе), учитывайте следующее:
...
|Простота    | Фоновые задачи, выполняющиеся в одном процессе, не требуют взаимодействия между процессами и проще в разработке.  |
...
|VoIP | Фоновые задачи с одним процессом не поддерживают активации фоновой задачи VoIP в приложении. |  
...
**Обновление манифеста приложения:** для фоновых задач, которые выполняются в отдельном процессе, объявите каждую фоновую задачу в манифесте приложения вместе с типом триггеров, который используется с этой задачей. В противном случае ваше приложение не сможет зарегистрировать фоновую задачу во время выполнения.

Фоновые задачи, которые выполняются в том же процессе, что и приложение переднего плана, не требуется объявлять в манифесте приложения. Дополнительные сведения об объявлении в манифесте фоновых задач, которые выполняются в отдельном процессе, см. в разделе [Объявление фоновых задач в манифесте приложения](declare-background-tasks-in-the-application-manifest.md).
...
*Относится к фоновым задачам как в одном процессе, так и в отдельном процессе*
...
*Относится только к фоновым задачам в отдельном процессе*
...
*Относится только к фоновым задачам в одном процессе*
