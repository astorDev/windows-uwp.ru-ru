...
Приложения универсальной платформы Windows (UWP) могут выполнять все поддерживаемые типы задач без закрепления на экране блокировки. Однако приложения должны вызвать [**жетакцессстате**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundexecutionmanager.getaccessstatus) и убедиться, что приложение не запрещает выполнение в фоновом режиме. Убедитесь, что [**жетакцессстатус**] не возвращает одно из запрещенных перечислений [**баккграундакцессстатус**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundaccessstatus) . Например, этот метод возвращает https://docs.microsoft.com/uwp/api/Windows.ApplicationModel.Background.BackgroundAccessStatus) значение (если пользователь явно запретил разрешения фоновой задачи для приложения в параметрах устройства).

Если приложению запрещено работать в фоновом режиме, приложение должно вызвать [**рекуестакцессасинк**](https://docs.microsoft.com/uwp/api/windows.applicationmodel.background.backgroundexecutionmanager.getaccessstatus) и убедиться, что ответ не отклоняется перед регистрацией фоновых задач.

Дополнительные сведения о доступных пользователю настройках фоновых задач и экономии заряда см. в разделе [Оптимизация фоновой активности](https://docs.microsoft.com/windows/uwp/debug-test-perf/optimize-background-activity). 
