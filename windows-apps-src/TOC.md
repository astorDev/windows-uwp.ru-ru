# [Начало работы с универсальной платформой Windows](https://developer.microsoft.com/windows/getstarted)
## [Что такое приложение UWP?](get-started/whats-a-uwp.md)
## [Руководство по работе с приложениями универсальной платформы Windows (UWP)](get-started/universal-application-platform-guide.md)
## [Подготовка](get-started/get-set-up.md)
### [Включение устройства для разработки](get-started/enable-your-device-for-development.md)
## [Регистрация](get-started/sign-up.md)
## [Ваше первое приложение](get-started/your-first-app.md)
### [Создание приложения «Hello, world» (C# и VB)](get-started/create-a-hello-world-app-xaml-universal.md)
### [Создание приложения «Hello, world» (JS)](get-started/create-a-hello-world-app-js-universal.md)
### [Создание приложения «Hello, world» (C++)](get-started/create-a-basic-windows-10-app-in-cpp.md)
### [Планирование приложения](get-started/plan-your-app.md)
### [Дальнейшие действия](get-started/learn-more.md)
## [Что нового в Windows для разработчиков](whats-new/windows-10-sdk-preview.md)
### [Что нового в Windows 10 (версия 1511)](whats-new/windows-10-version-1511.md)
### [Что нового в Windows 10 (версия RTM)](whats-new/windows-10-rtm.md)
# [Оформление и пользовательский интерфейс](https://developer.microsoft.com/windows/design)
## [Макет](layout/index.md)
### [Введение в проектирование пользовательского интерфейса приложения](layout/design-and-ui-intro.md)
### [Основы навигации](layout/navigation-basics.md)
#### [Журнал и навигация в обратном направлении](layout/navigation-history-and-backwards-navigation.md)
#### [Реализация навигации между одноранговыми элементами](layout/peer-to-peer-navigation-between-two-pages.md)
#### [Отображение нескольких представлений](layout/show-multiple-views.md)
### [Основные сведения о командных элементах](layout/commanding-basics.md)
#### [Активный холст](layout/active-canvas.md)
### [Основные сведения о содержимом](layout/content-basics.md)
### [Размеры экрана и точки останова](layout/screen-sizes-and-breakpoints-for-responsive-design.md)
### [Определение макетов страниц с помощью XAML](layout/layouts-with-xaml.md)
### [Панели макета](layout/layout-panels.md)
#### [Пользовательские панели](layout/custom-panels-overview.md)
#### [Пример настраиваемой панели: BoxPanel](layout/boxpanel-example-custom-panel.md)
### [Выравнивание, поля и заполнение](layout/alignment-margin-padding.md)
## [Стиль](style/index.md)
### [Цвет](style/color.md)
### [Значки](style/icons.md)
### [Движение](style/motion.md)
### [Звук](style/sound.md)
### [Шрифтовое оформление](style/typography.md)
#### [Шрифты](style/fonts.md)
#### [Значки Segoe MDL2](style/segoe-ui-symbol-font.md)
### [Настройка стиля элементов управления](controls-and-patterns/styling-controls.md)
#### [Шаблоны элементов управления XAML](controls-and-patterns/control-templates.md)
#### [Ссылки на ресурсы ResourceDictionary и XAML](controls-and-patterns/resourcedictionary-and-xaml-resource-references.md)
#### [Ресурсы темы XAML](controls-and-patterns/xaml-theme-resources.md)
## [Элементы управления и шаблоны](controls-and-patterns/index.md)
### [Введение](controls-and-patterns/controls-and-events-intro.md)
### [Указатель элементов управления по функциям](controls-and-patterns/controls-by-function.md)
### [Панель приложения и панель команд](controls-and-patterns/app-bars.md)
### [Поле автозаполнения](controls-and-patterns/auto-suggest-box.md)
### [Кнопки](controls-and-patterns/buttons.md)
### [Дата и время](controls-and-patterns/date-and-time.md)
#### [Элемент выбора даты в календаре](controls-and-patterns/calendar-date-picker.md)
#### [Представление календаря](controls-and-patterns/calendar-view.md)
#### [Элемент выбора даты](controls-and-patterns/date-picker.md)
#### [Элемент выбора времени](controls-and-patterns/time-picker.md)
### [Флажок](controls-and-patterns/checkbox.md)
### [Представление пролистывания](controls-and-patterns/flipview.md)
### [Концентратор](controls-and-patterns/hub.md)
### [Гиперссылки](controls-and-patterns/hyperlinks.md)
### [Изображения и кисти изображений](controls-and-patterns/images-imagebrushes.md)
### [Списки](controls-and-patterns/lists.md)
### [Основные и подробные данные](controls-and-patterns/master-details.md)
### [Воспроизведение мультимедиа](controls-and-patterns/media-playback.md)
#### [Пользовательские элементы управления транспортировкой](controls-and-patterns/custom-transport-controls.md)
### [Меню, диалоговые окна и всплывающие окна](controls-and-patterns/dialogs-popups-menus.md)
### [Панель навигации](controls-and-patterns/nav-pane.md)
### [Ход выполнения](controls-and-patterns/progress-controls.md)
### [Переключатель](controls-and-patterns/radio-button.md)
### [Элементы управления прокруткой и панорамированием](controls-and-patterns/scroll-controls.md)
### [Поиск](controls-and-patterns/search.md)
### [Контекстное масштабирование](controls-and-patterns/semantic-zoom.md)
### [Ползунок](controls-and-patterns/slider.md)
### [Комбинированный режим](controls-and-patterns/split-view.md)
### [Вкладки и элемент управления «Сводка»](controls-and-patterns/tabs-pivot.md)
### [Текст](controls-and-patterns/text-controls.md)
#### [Метки](controls-and-patterns/labels.md)
#### [Поле ввода пароля](controls-and-patterns/password-box.md)
#### [Блок форматируемого текста](controls-and-patterns/rich-edit-box.md)
#### [Блок форматированного текста](controls-and-patterns/rich-text-block.md)
#### [Проверка правописания и прогнозирование](controls-and-patterns/spell-checking-and-prediction.md)
#### [Блок текста](controls-and-patterns/text-block.md)
#### [Текстовое поле](controls-and-patterns/text-box.md)
### [Плитки, индикаторы событий и уведомления](controls-and-patterns/tiles-badges-notifications.md)
#### [Плитки](controls-and-patterns/tiles-and-notifications-creating-tiles.md)
#### [Адаптивные плитки](controls-and-patterns/tiles-and-notifications-create-adaptive-tiles.md)
##### [Схема адаптивных плиток](controls-and-patterns/tiles-and-notifications-adaptive-tiles-schema.md)
#### [Руководство по работе с ресурсами плиток и значков](controls-and-patterns/tiles-and-notifications-app-assets.md)
#### [Специальные шаблоны плиток](controls-and-patterns/tiles-and-notifications-special-tile-templates-catalog.md)
#### [Адаптивные и интерактивные всплывающие уведомления](controls-and-patterns/tiles-and-notifications-adaptive-interactive-toasts.md)
#### [Визуализатор уведомлений](controls-and-patterns/tiles-and-notifications-notifications-visualizer.md)
#### [Методы доставки уведомлений](controls-and-patterns/tiles-and-notifications-choosing-a-notification-delivery-method.md)
#### [Локальные уведомления плиток](controls-and-patterns/tiles-and-notifications-sending-a-local-tile-notification.md)
#### [Периодические уведомления](controls-and-patterns/tiles-and-notifications-periodic-notification-overview.md)
#### [Службы push-уведомлений Windows (WNS)](controls-and-patterns/tiles-and-notifications-windows-push-notification-services--wns--overview.md)
##### [Код, генерируемый мастером push-уведомлений](controls-and-patterns/tiles-and-notifications-the-code-generated-by-the-push-notification-wizard.md)
#### [Необработанные уведомления](controls-and-patterns/tiles-and-notifications-raw-notification-overview.md)
### [Тумблер](controls-and-patterns/toggles.md)
### [Всплывающая подсказка](controls-and-patterns/tooltips.md)
### [Представление веб-страницы](controls-and-patterns/web-view.md)
## [Ввод и устройства](input-and-devices/index.md)
### [Азбука методов ввода](input-and-devices/input-primer.md)
#### [Кортана](input-and-devices/cortana-interactions.md)
##### [Рекомендации по проектированию](input-and-devices/cortana-design-guidelines.md)
##### [Запуск приложения переднего плана](input-and-devices/launch-a-foreground-app-with-voice-commands-in-cortana.md)
##### [Изменение списков фраз в формате VCD](input-and-devices/dynamically-modify-voice-command-definition--vcd--phrase-lists.md)
##### [Запуск фонового приложения](input-and-devices/launch-a-background-app-with-voice-commands-in-cortana.md)
##### [Взаимодействие с фоновым приложением](input-and-devices/interact-with-a-background-app-in-cortana.md)
##### [Прямая ссылка на фоновое приложение](input-and-devices/deep-link-into-your-app-from-cortana.md)
##### [Поддержка голосовых команд на естественном языке](input-and-devices/support-natural-language-voice-commands-in-cortana.md)
#### [Клавиатура](input-and-devices/keyboard-interactions.md)
##### [Реакция на наличие сенсорной клавиатуры](input-and-devices/respond-to-the-presence-of-the-touch-keyboard.md)
##### [Использование типа вводимых данных для изменения сенсорной клавиатуры](input-and-devices/use-input-scope-to-change-the-touch-keyboard.md)
#### [Мышь](input-and-devices/mouse-interactions.md)
#### [Геймпад и пульт дистанционного управления](input-and-devices/gamepad-and-remote-interactions.md)
#### [Перо](input-and-devices/pen-and-stylus-interactions.md)
##### [Распознавание росчерков пера](input-and-devices/convert-ink-to-text.md)
##### [Хранение и извлечение росчерков пера](input-and-devices/save-and-load-ink.md)
#### [Взаимодействие с помощью голосовых функций](input-and-devices/speech-interactions.md)
##### [Распознавание речи](input-and-devices/speech-recognition.md)
##### [Определение языка для распознавателя речи](input-and-devices/specify-the-speech-recognizer-language.md)
##### [Определение настраиваемых ограничений распознавания](input-and-devices/define-custom-recognition-constraints.md)
##### [Непрерывная диктовка](input-and-devices/enable-continuous-dictation.md)
##### [Проблемы голосового ввода](input-and-devices/manage-issues-with-audio-input.md)
##### [Время ожидания распознавания речи](input-and-devices/set-speech-recognition-timeouts.md)
#### [Взаимодействие с помощью сенсорного экрана](input-and-devices/touch-interactions.md)
#### [Взаимодействия с сенсорной панелью](input-and-devices/touchpad-interactions.md)
#### [Несколько типов ввода](input-and-devices/multiple-input-design-guidelines.md)
#### [Визуальное масштабирование и изменение размера](input-and-devices/guidelines-for-optical-zoom.md)
#### [Сдвиг](input-and-devices/guidelines-for-panning.md)
#### [Поворот](input-and-devices/guidelines-for-rotation.md)
#### [Выделение текста и изображений](input-and-devices/guidelines-for-textselection.md)
#### [Таргетинг](input-and-devices/guidelines-for-targeting.md)
#### [Визуальная обратная связь](input-and-devices/guidelines-for-visualfeedback.md)
#### [Распознавание устройств ввода](input-and-devices/identify-input-devices.md)
#### [Пользовательский ввод текста](input-and-devices/custom-text-input.md)
#### [Работа с данными указателя](input-and-devices/handle-pointer-input.md)
### [Азбука устройств](input-and-devices/device-primer.md)
#### [Проектирование для Xbox и телевизора](input-and-devices/designing-for-tv.md)
## [Удобство использования](usability/index.md)
### [Специальные возможности](accessibility/accessibility.md)
#### [Обзор](accessibility/accessibility-overview.md)
#### [Проектирование инклюзивного программного обеспечения](accessibility/designing-inclusive-software.md)
#### [Разработка инклюзивных приложений для Windows](accessibility/developing-inclusive-windows-apps.md)
#### [Проверка специальных возможностей](accessibility/accessibility-testing.md)
#### [Специальные возможности в Магазине](accessibility/accessibility-in-the-store.md)
#### [Контрольный список специальных возможностей](accessibility/accessibility-checklist.md)
#### [Предоставление основных сведений о специальных возможностях](accessibility/basic-accessibility-information.md)
#### [Специальные возможности клавиатуры](accessibility/keyboard-accessibility.md)
#### [Темы с высокой контрастностью](accessibility/high-contrast-themes.md)
#### [Требования к специальным возможностям отображения текста](accessibility/accessible-text-requirements.md)
#### [Рекомендации по специальным возможностям, которых следует избегать](accessibility/practices-to-avoid.md)
#### [Настраиваемые одноранговые элементы автоматизации](accessibility/custom-automation-peers.md)
##### [Шаблоны и интерфейсы элементов управления](accessibility/control-patterns-and-interfaces.md)
### [Параметры приложения](app-settings/app-settings-and-data.md)
#### [Руководства](app-settings/guidelines-for-app-settings.md)
#### [Хранение и извлечение параметров и данных приложения](app-settings/store-and-retrieve-app-data.md)
### [Глобализация и локализация](globalizing/globalizing-portal.md)
#### [Возможности и ограничения](globalizing/guidelines-and-checklist-for-globalizing-your-app.md)
#### [Настройка макета и шрифтов, добавление поддержки написания справа налево](globalizing/adjust-layout-and-fonts--and-support-rtl.md)
#### [Форматирование дат и времени](globalizing/use-patterns-to-format-dates-and-times.md)
#### [Управление языковыми и региональными параметрами](globalizing/manage-language-and-region.md)
#### [Подготовка приложения к локализации](globalizing/prepare-your-app-for-localization.md)
#### [Размещение строк пользовательского интерфейса в ресурсах](globalizing/put-ui-strings-into-resources.md)
#### [Использование форматов международного стандарта](globalizing/use-global-ready-formats.md)
### [Руководство по справке для приложений](in-app-help/guidelines-for-app-help.md)
#### [Инструктивный пользовательский интерфейс](in-app-help/instructional-ui.md)
#### [Справка в приложении](in-app-help/in-app-help.md)
#### [Внешняя справка](in-app-help/external-help.md)
## [Язык дизайна Майкрософт](https://developer.microsoft.com/windows/design/microsoft-design-language)
## [Загрузки](design-downloads/index.md)
# [Разработка приложений для Windows](https://developer.microsoft.com/windows/develop)
## [Связь между приложениями](app-to-app/index.md)
### [Предоставление общего доступа к данным](app-to-app/share-data.md)
### [Получение данных](app-to-app/receive-data.md)
### [Копирование и вставка](app-to-app/copy-and-paste.md)
### [Перетаскивание](app-to-app/drag-and-drop.md)
### [Использование EDP для защиты корпоративных данных, перемещаемых между приложениями](app-to-app/use-edp-to-protect-enterprise-data-transferred-between-apps.md)
## [Звук, видео и камера](audio-video-camera/index.md)
### [Фото- и видеосъемка с помощью CameraCaptureUI](audio-video-camera/capture-photos-and-video-with-cameracaptureui.md)
### [Фото- и видеозахват с помощью MediaCapture](audio-video-camera/capture-photos-and-video-with-mediacapture.md)
#### [Профили камеры](audio-video-camera/camera-profiles.md)
#### [Задание свойств кодирования мультимедиа](audio-video-camera/set-media-encoding-properties.md)
#### [Фотозахват HDR (High Dynamic Range)](audio-video-camera/high-dynamic-range-hdr-photo-capture.md)
#### [Доступ к элементам управления фото- и видеозахватом на устройстве](audio-video-camera/capture-device-controls-for-photo-and-video-capture.md)
#### [Доступ к элементам управления видеозахватом на устройстве](audio-video-camera/capture-device-controls-for-video-capture.md)
#### [Эффекты для видеозахвата](audio-video-camera/effects-for-video-capture.md)
#### [Анализ сцен для захвата мультимедиа](audio-video-camera/scene-analysis-for-media-capture.md)
#### [Переменная последовательность фотографий](audio-video-camera/variable-photo-sequence.md)
#### [Получение кадра предварительного просмотра](audio-video-camera/get-a-preview-frame.md)
#### [Простой доступ к просмотру камеры](audio-video-camera/simple-camera-preview-access.md)
### [Обнаружение лиц на изображениях или в видео](audio-video-camera/detect-and-track-faces-in-an-image.md)
### [Пользовательские видеоэффекты](audio-video-camera/custom-video-effects.md)
### [Создание и редактирование мультимедиа](audio-video-camera/media-compositions-and-editing.md)
### [Свойства сведений о звуковых устройствах](audio-video-camera/audio-device-information-properties.md)
### [Обработка изображений](audio-video-camera/imaging.md)
#### [Справочник по параметрам BitmapEncoder](audio-video-camera/bitmapencoder-options-reference.md)
#### [Метаданные изображения](audio-video-camera/image-metadata.md)
### [Перекодирование файлов мультимедиа](audio-video-camera/transcode-media-files.md)
### [Обработка файлов мультимедиа в фоновом режиме](audio-video-camera/process-media-files-in-the-background.md)
### [Воспроизведение мультимедиа с помощью класса MediaSource](audio-video-camera/media-playback-with-mediasource.md)
### [Адаптивная потоковая передача](audio-video-camera/adaptive-streaming.md)
### [Воспроизведение звука в фоновом режиме](audio-video-camera/background-audio.md)
### [Системные элементы управления транспортом мультимедиа](audio-video-camera/system-media-transport-controls.md)
### [Трансляция мультимедиа](audio-video-camera/media-casting.md)
### [Звуковые графики](audio-video-camera/audio-graphs.md)
### [MIDI](audio-video-camera/midi.md)
### [Независимый от камеры фонарик](audio-video-camera/camera-independent-flashlight.md)
### [Поддерживаемые кодеки](audio-video-camera/supported-codecs.md)
### [DRM PlayReady](audio-video-camera/playready-client-sdk.md)
#### [Аппаратное управление цифровыми правами RM](audio-video-camera/hardware-drm.md)
#### [Адаптивная потоковая передача с защитой PlayReady](audio-video-camera/adaptive-streaming-with-playready.md)
### [Расширение зашифрованного мультимедиа PlayReady](audio-video-camera/playready-encrypted-media-extension.md)
## [Контакты и календарь](contacts-and-calendar/index.md)
### [Выбор контактов](contacts-and-calendar/selecting-contacts.md)
### [Отправка сообщения электронной почты](contacts-and-calendar/sending-email.md)
### [Отправка SMS-сообщения](contacts-and-calendar/sending-an-sms-message.md)
### [Управление встречами](contacts-and-calendar/managing-appointments.md)
## [Доступ к данным](data-access/index.md)
### [EntityFramework7 с SQLite для приложений C#](data-access/entity-framework-7-with-sqlite-for-csharp-apps.md)
### [Базы данных SQLite](data-access/sqlite-databases.md)
## [Привязка данных](data-binding/index.md)
### [Общие сведения о привязке данных](data-binding/data-binding-quickstart.md)
### [Подробно о привязке данных](data-binding/data-binding-in-depth.md)
### [Демонстрационные данные в рабочей области конструирования и для создания прототипов](data-binding/displaying-data-in-the-designer.md)
### [Привязка к иерархическим данным и создание представления основных и подробных данных](data-binding/how-to-bind-to-hierarchical-data-and-create-a-master-details-view.md)
## [Отладка, тестирование и производительность](debug-test-perf/index.md)
### [Развертывание и отладка приложений UWP](debug-test-perf/deploying-and-debugging-uwp-apps.md)
### [Средства тестирования и отладки для PLM](debug-test-perf/testing-debugging-plm.md)
### [Тестирование при помощи эмулятора устройства с Windows10(Майкрософт)](debug-test-perf/test-with-the-emulator.md)
### [Проверка приложений Surface Hub с использованием Visual Studio](debug-test-perf/test-surface-hub-apps-using-visual-studio.md)
### [Бета-тестирование](debug-test-perf/beta-testing.md)
### [Портал устройств Windows](debug-test-perf/device-portal.md)
#### [Портал устройств для ПК](debug-test-perf/device-portal-desktop.md)
#### [Портал устройств для мобильных устройств](debug-test-perf/device-portal-mobile.md)
#### [Портал устройств для Xbox](debug-test-perf/device-portal-xbox.md)
#### [Основные API портала устройств](debug-test-perf/device-portal-api-core.md)
### [Комплект сертификации приложений для Windows](debug-test-perf/windows-app-certification-kit.md)
#### [Тесты комплекта сертификации приложений для Windows](debug-test-perf/windows-app-certification-kit-tests.md)
### [Производительность](debug-test-perf/performance-and-xaml-ui.md)
#### [Планирование производительности](debug-test-perf/planning-and-measuring-performance.md)
#### [Оптимизация пользовательского интерфейса ListView и GridView](debug-test-perf/optimize-gridview-and-listview.md)
#### [Виртуализация данных ListView и GridView](debug-test-perf/listview-and-gridview-data-optimization.md)
#### [Улучшение производительности сборки мусора](debug-test-perf/improve-garbage-collection-performance.md)
#### [Обеспечение быстрого отклика потока пользовательского интерфейса](debug-test-perf/keep-the-ui-thread-responsive.md)
#### [Оптимизация разметки XAML](debug-test-perf/optimize-xaml-loading.md)
#### [Оптимизация макета XAML](debug-test-perf/optimize-your-xaml-layout.md)
#### [Советы по MVVM и обеспечению производительности языка](debug-test-perf/mvvm-performance-tips.md)
#### [Рекомендации по повышению производительности запуска приложения](debug-test-perf/best-practices-for-your-app-s-startup-performance.md)
#### [Оптимизация анимаций, мультимедиа и изображений](debug-test-perf/optimize-animations-and-media.md)
#### [Оптимизация времени приостановки и возобновления работы](debug-test-perf/optimize-suspend-resume.md)
#### [Оптимизация доступа к файлам](debug-test-perf/optimize-file-access.md)
#### [Компоненты среды выполнения Windows и оптимизация межпрограммного взаимодействия](debug-test-perf/windows-runtime-components-and-optimizing-interop.md)
#### [Инструменты для профилирования и производительности](debug-test-perf/tools-for-profiling-and-performance.md)
### [Адаптивный код версии](debug-test-perf/version-adaptive-code.md)
## [Устройства, датчики и питание](devices-sensors/index.md)
### [Включение возможностей устройств](devices-sensors/enable-device-capabilities.md)
### [Предоставление доступа в пользовательском режиме](devices-sensors/enable-usermode-access.md)
### [Перечисление устройств](devices-sensors/enumerate-devices.md)
#### [Создание средства выбора устройств](devices-sensors/build-a-device-selector.md)
#### [Перечисление устройств по сети](devices-sensors/enumerate-devices-over-a-network.md)
#### [Свойства сведений об устройстве](devices-sensors/device-information-properties.md)
#### [Идентификаторы класса службы AEP](devices-sensors/aep-service-class-ids.md)
### [Связывание устройств](devices-sensors/pair-devices.md)
### [Связывание вне диапазона](devices-sensors/out-of-band-pairing.md)
### [Датчики](devices-sensors/sensors.md)
#### [Калибровка датчиков](devices-sensors/calibrate-sensors.md)
#### [Положение датчиков в пространстве](devices-sensors/sensor-orientation.md)
#### [Использование акселерометра](devices-sensors/use-the-accelerometer.md)
#### [Использование компаса](devices-sensors/use-the-compass.md)
#### [Использование гирометра](devices-sensors/use-the-gyrometer.md)
#### [Использование инклинометра](devices-sensors/use-the-inclinometer.md)
#### [Использование датчика освещенности](devices-sensors/use-the-light-sensor.md)
#### [Использование датчика положения в пространстве](devices-sensors/use-the-orientation-sensor.md)
### [Bluetooth](devices-sensors/bluetooth.md)
#### [RFCOMM](devices-sensors/send-or-receive-files-with-rfcomm.md)
#### [GATT](devices-sensors/gatt-scenarios.md)
### [Печать и сканирование](devices-sensors/printing-and-scanning.md)
#### [Epson ESC/POS с форматированием](devices-sensors/epson-esc-pos-with-formatting.md)
#### [Печать из приложения](devices-sensors/print-from-your-app.md)
#### [Настройка пользовательского интерфейса предварительного просмотра](devices-sensors/customize-the-print-preview-ui.md)
#### [Сканирование из приложения](devices-sensors/scan-from-your-app.md)
### [Трехмерная печать](devices-sensors/3d-printing.md)
#### [Трехмерная печать из приложения](devices-sensors/3d-print-from-app.md)
#### [Создание пакета 3MF](devices-sensors/generate-3mf.md)
### [NFC](devices-sensors/nfc.md)
#### [Создание приложения для смарт-карты NFC](devices-sensors/host-card-emulation.md)
### [Получение сведений об аккумуляторе](devices-sensors/get-battery-info.md)
## [Функции корпоративного уровня](enterprise/index.md)
### [Защита корпоративных данных (EDP)](enterprise/edp-hub.md)
### [Общее корпоративное хранилище](enterprise/enterprise-shared-storage.md)
## [Файлы, папки и библиотеки](files/index.md)
### [Перечисление и запрос файлов и папок](files/quickstart-listing-files-and-folders.md)
### [Создание, запись и чтение файла](files/quickstart-reading-and-writing-files.md)
### [Получение свойств файла](files/quickstart-getting-file-properties.md)
### [Открытие файлов и папок с помощью средства выбора](files/quickstart-using-file-and-folder-pickers.md)
### [Сохранение файла с помощью средства выбора](files/quickstart-save-a-file-with-a-picker.md)
### [Использование защиты корпоративных данных (EDP) для защиты файлов](files/protect-your-enterprise-data-with-edp.md)
### [Использование защиты корпоративных данных (EDP) для защиты потоков и буферов](files/use-edp-to-protect-streams-and-buffers.md)
### [Доступ к содержимому домашней группы](files/quickstart-accessing-homegroup-content.md)
### [Определение доступности файлов Microsoft OneDrive](files/quickstart-determining-availability-of-microsoft-onedrive-files.md)
### [Файлы и папки в библиотеках музыки, изображений и видео](files/quickstart-managing-folders-in-the-music-pictures-and-videos-libraries.md)
### [Отслеживание недавно использовавшихся файлов и папок](files/how-to-track-recently-used-files-and-folders.md)
### [Доступ к SD-карте](files/access-the-sd-card.md)
### [Разрешения на доступ к файлам](files/file-access-permissions.md)
## [Игры и DirectX](gaming/index.md)
### [Руководство по разработке игр для Windows10](gaming/e2e.md)
### [Игровые технологии для приложений универсальной платформы Windows](gaming/game-development-platform-guide.md)
### [Шаблоны проектов и средства для игр](gaming/prepare-your-dev-environment-for-windows-store-directx-game-development.md)
#### [Шаблоны проектов игр DirectX](gaming/user-interface.md)
#### [Средства Visual Studio для программирования игр](gaming/set-up-visual-studio-for-game-development.md)
#### [Средства диагностики графики](gaming/use-the-directx-runtime-and-visual-studio-graphics-diagnostic-features.md)
### [Объект приложения и DirectX](gaming/about-the-metro-style-user-interface-and-directx.md)
### [Запуск и возобновление работы приложений](gaming/launching-and-resuming-apps-directx-and-cpp.md)
#### [Активация приложения](gaming/how-to-activate-an-app-directx-and-cpp.md)
#### [Приостановка работы приложения](gaming/how-to-suspend-an-app-directx-and-cpp.md)
#### [Возобновление работы приложения](gaming/how-to-resume-an-app-directx-and-cpp.md)
### [Двумерная графика для игр DirectX](gaming/working-with-2d-graphics-in-your-directx-game.md)
### [Простая трехмерная графика для игр DirectX](gaming/an-introduction-to-3d-graphics-with-directx.md)
#### [Настройка ресурсов DirectX и отображение изображения](gaming/setting-up-directx-resources.md)
#### [Создание шейдеров и рисование примитивов](gaming/creating-shaders-and-drawing-primitives.md)
#### [Применение глубины и эффектов к примитивам](gaming/using-depth-and-effects-on-primitives.md)
#### [Применение текстур к примитивам](gaming/applying-textures-to-primitives.md)
#### [Создание и отображение простой сетки](gaming/create-a-basic-mesh.md)
### [Загрузка ресурсов в игре DirectX](gaming/load-a-game-asset.md)
#### [Полный код для BasicLoader](gaming/complete-code-for-basicloader.md)
#### [Полный код класса BasicReaderWriter](gaming/complete-code-for-basicreaderwriter.md)
#### [Полный код для DDSTextureLoader](gaming/complete-code-for-ddstextureloader.md)
### [Создание простой игры UWP c использованием DirectX](gaming/tutorial--create-your-first-metro-style-directx-game.md)
#### [Настройка проекта игры](gaming/tutorial--setting-up-the-games-infrastructure.md)
#### [Определение структуры приложения UWP](gaming/tutorial--building-the-games-metro-style-app-framework.md)
#### [Определение основного игрового объекта](gaming/tutorial--defining-the-main-game-loop.md)
#### [Сборка структуры визуализации](gaming/tutorial--assembling-the-rendering-pipeline.md)
#### [Добавление пользовательского интерфейса](gaming/tutorial--adding-a-user-interface.md)
#### [Добавление элементов управления](gaming/tutorial--adding-controls.md)
#### [Добавление звука](gaming/tutorial--adding-sound.md)
#### [Расширение образца игры](gaming/tutorial-resources.md)
### [Разработка Marble Maze — игры универсальной платформы Windows — на C++ и DirectX](gaming/developing-marble-maze-a-windows-store-game-in-cpp-and-directx.md)
#### [Основные положения примера Marble Maze](gaming/marble-maze-sample-fundamentals.md)
#### [Структура приложения Marble Maze](gaming/marble-maze-application-structure.md)
#### [Добавление визуального содержимого в образец Marble Maze](gaming/adding-visual-content-to-the-marble-maze-sample.md)
#### [Добавление средств ввода и взаимодействия в пример Marble Maze](gaming/adding-input-and-interactivity-to-the-marble-maze-sample.md)
#### [Добавление звука в пример Marble Maze](gaming/adding-audio-to-the-marble-maze-sample.md)
### [Поддержка ориентации экрана](gaming/supporting-screen-rotation-directx-and-cpp.md)
### [Звук для игр](gaming/working-with-audio-in-your-directx-game.md)
### [Сенсорные элементы управления для игр](gaming/tutorial--adding-touch-controls-to-your-directx-game.md)
### [Элементы управления перемещением и обзором для игр](gaming/tutorial--adding-move-look-controls-to-your-directx-game.md)
### [Оптимизация ввода и цикла отрисовки](gaming/optimize-performance-for-windows-store-direct3d-11-apps-with-coredispatcher.md)
### [Наложения и масштабирование цепочки буферов](gaming/multisampling--scaling--and-overlay-swap-chains.md)
### [Сокращение задержки с помощью цепочек буферов DXGI 1.3](gaming/reduce-latency-with-dxgi-1-3-swap-chains.md)
### [Множественная дискретизация в приложениях UWP](gaming/multisampling--multi-sample-anti-aliasing--in-windows-store-apps.md)
### [Обработка сценариев удаления устройства в Direct3D11](gaming/handling-device-lost-scenarios.md)
### [Асинхронное программирование в играх](gaming/asynchronous-programming-directx-and-cpp.md)
### [Поддержка сети в играх](gaming/work-with-networking-in-your-directx-game.md)
### [Специальные возможности для игр](gaming/accessibility-for-games.md)
### [Облако для игр](gaming/cloud-for-games.md)
### [Взаимодействие DirectX и XAML](gaming/directx-and-xaml-interop.md)
### [Упаковка игры](gaming/package-your-windows-store-directx-game.md)
### [Руководства по портированию игр](gaming/porting-guides.md)
#### [Перенос из DirectX9 на универсальную платформу Windows](gaming/porting-your-directx-9-game-to-windows-store.md)
##### [Переход от DirectX 9 на DirectX 11.1 и универсальная платформа Windows](gaming/porting-considerations.md)
###### [Планирование портирования приложений на DirectX](gaming/plan-your-directx-port.md)
###### [Важные изменения в Direct3D11.1 по сравнению с Direct3D9](gaming/understand-direct3d-11-1-concepts.md)
###### [Сопоставление функций DirectX 9 с API DirectX 11.1](gaming/feature-mapping.md)
##### [Пошаговое руководство: портирование простого приложения с Direct3D 9 на DirectX 11.1 и на универсальную платформу Windows](gaming/walkthrough--simple-port-from-direct3d-9-to-11-1.md)
###### [Инициализация Direct3D 11](gaming/simple-port-from-direct3d-9-to-11-1-part-1--initializing-direct3d.md)
###### [Преобразование инфраструктуры отрисовки](gaming/simple-port-from-direct3d-9-to-11-1-part-2--rendering.md)
###### [Портирование игрового цикла](gaming/simple-port-from-direct3d-9-to-11-1-part-3--viewport-and-game-loop.md)
##### [Пошаговое руководство: реализация объемных теней с помощью буферов глубины в Direct3D 11](gaming/implementing-depth-buffers-for-shadow-mapping.md)
###### [Создание ресурсов устройства для буфера глубины](gaming/create-depth-buffer-resource--view--and-sampler-state.md)
###### [Прорисовка карты теней в буфере глубины](gaming/render-the-shadow-map-to-the-depth-buffer.md)
###### [Прорисовка сцены с тестированием глубины](gaming/render-the-scene-with-depth-testing.md)
###### [Поддержка карт теней на различном оборудовании](gaming/target-a-range-of-hardware.md)
##### [Вопросы и ответы по переносу на DirectX11.1](gaming/directx-porting-faq.md)
#### [Перенос из OpenGL ES2.0 в Direct3D11.1](gaming/port-from-opengl-es-2-0-to-directx-11-1.md)
##### [Сопоставление OpenGL ES 2.0 и Direct3D 11.1](gaming/map-concepts-and-infrastructure.md)
###### [Планирование переноса с OpenGL ES2.0 на Direct3D](gaming/compare-opengl-es-2-0-api-design-to-directx.md)
###### [Сравнение кода EGL с DXGI и Direct3D](gaming/moving-from-egl-to-dxgi.md)
###### [Сравнение атрибутов буферов, однородностей и вершин OpenGL ES 2.0 с Direct3D](gaming/porting-uniforms-and-attributes.md)
###### [Сравнение конвейера шейдеров OpenGL ES2.0 с Direct3D](gaming/change-your-shader-loading-code.md)
##### [Инструкции: перенос простого обработчика OpenGL ES 2.0 в Direct3D 11.1](gaming/port-a-simple-opengl-es-2-0-renderer-to-directx-11-1.md)
###### [Перенос объектов шейдеров](gaming/port-the-shader-config.md)
###### [Перенос данных и буферов вершин](gaming/port-the-vertex-buffers-and-data-config.md)
###### [Перенос GLSL](gaming/port-the-glsl.md)
###### [Рисование объектов на экране](gaming/draw-to-the-screen.md)
##### [Справочник по переносу с OpenGL ES 2.0 на Direct3D 11.1](gaming/opengl-es-2-0-to-directx-11-1-reference.md)
###### [Справочные материалы по переносу с GLSL на HLSL](gaming/glsl-to-hlsl-reference.md)
### [Ресурсы по программированию игр](gaming/additional-directx-game-programming-resources.md)
## [Графика и анимация](graphics/index.md)
### [Рисование фигур](graphics/drawing-shapes.md)
### [Использование кистей](graphics/using-brushes.md)
### [Обзор анимаций](graphics/animations-overview.md)
#### [Раскадрованные анимации](graphics/storyboarded-animations.md)
#### [Анимации по ключевым кадрам и на основе функций для реалистичной анимации](graphics/key-frame-and-easing-function-animations.md)
### [Обзор преобразований](graphics/transforms-overview.md)
#### [Трехмерные эффекты для пользовательского интерфейса XAML](graphics/3-d-perspective-effects.md)
### [Визуальный уровень](graphics/visual-layer.md)
#### [Визуальное дерево композиции](graphics/composition-visual-tree.md)
#### [Анимации композиции](graphics/composition-animation.md)
#### [Эффекты композиции](graphics/composition-effects.md)
#### [Кисти композиции](graphics/composition-brushes.md)
#### [Собственный DX композиции и взаимодействие D2D с BeginDraw и EndDraw](graphics/composition-native-interop.md)
## [Запуск, возобновление и фоновые задачи](launch-resume/index.md)
### [Жизненный цикл приложения](launch-resume/app-lifecycle.md)
#### [Обработка предварительного запуска приложения](launch-resume/handle-app-prelaunch.md)
#### [Обработка активации приложения](launch-resume/activate-an-app.md)
#### [Обработка приостановки работы приложения](launch-resume/suspend-an-app.md)
#### [Обработка возобновления работы приложения](launch-resume/resume-an-app.md)
### [Запуск приложения для результатов](launch-resume/how-to-launch-an-app-for-results.md)
### [Создание и использование службы приложения](launch-resume/how-to-create-and-consume-an-app-service.md)
### [Запуск приложения по умолчанию для URI](launch-resume/launch-default-app.md)
#### [Запуск приложения "Параметры" для Windows](launch-resume/launch-settings-app.md)
#### [Запуск приложения Магазина Windows](launch-resume/launch-store-app.md)
### [Запуск приложения "Карты Windows"](launch-resume/launch-maps-app.md)
#### [Запуск приложения "Люди"](launch-resume/launch-people-apps.md)
### [Обработка активации URI](launch-resume/handle-uri-activation.md)
#### [Зарезервированные имена файлов и схем URI](launch-resume/reserved-uri-scheme-names.md)
### [Запуск приложения по умолчанию для файла](launch-resume/launch-the-default-app-for-a-file.md)
### [Обработка активации файла](launch-resume/handle-file-activation.md)
### [Запуск приложения с помощью автозапуска](launch-resume/auto-launching-with-autoplay.md)
### [Поддержка приложения с помощью фоновых задач](launch-resume/support-your-app-with-background-tasks.md)
#### [Доступ к датчикам и устройствам из фоновой задачи](launch-resume/access-sensors-and-devices-from-a-background-task.md)
#### [Руководство по работе с фоновыми задачами](launch-resume/guidelines-for-background-tasks.md)
#### [Создание и регистрация фоновой задачи](launch-resume/create-and-register-a-background-task.md)
#### [Отладка фоновой задачи](launch-resume/debug-a-background-task.md)
#### [Объявление фоновых задач в манифесте приложения](launch-resume/declare-background-tasks-in-the-application-manifest.md)
#### [Обработка отмененной фоновой задачи](launch-resume/handle-a-cancelled-background-task.md)
#### [Отслеживание хода выполнения и завершения фоновых задач](launch-resume/monitor-background-task-progress-and-completion.md)
#### [Регистрация фоновой задачи](launch-resume/register-a-background-task.md)
#### [Реагирование на системные события с помощью фоновых задач](launch-resume/respond-to-system-events-with-background-tasks.md)
#### [Запуск фоновой задачи по таймеру](launch-resume/run-a-background-task-on-a-timer-.md)
#### [Задание условий выполнения фоновой задачи](launch-resume/set-conditions-for-running-a-background-task.md)
#### [Обновление живой плитки из фоновой задачи](launch-resume/update-a-live-tile-from-a-background-task.md)
#### [Использование триггера обслуживания](launch-resume/use-a-maintenance-trigger.md)
### [Добавление экрана-заставки](launch-resume/add-a-splash-screen.md)
### [Более продолжительное отображение экрана-заставки](launch-resume/create-a-customized-splash-screen.md)
## [Карты и расположение](maps-and-location/index.md)
### [Запрос ключа проверки подлинности](maps-and-location/authentication-key.md)
### [Отображение карт](maps-and-location/display-maps.md)
### [Элемент управления картой](maps-and-location/controls-map.md)
### [Отображение POI](maps-and-location/display-poi.md)
### [Отображение дорог и маршрутов](maps-and-location/routes-and-directions.md)
### [Наложение мозаичных изображений](maps-and-location/overlay-tiled-images.md)
### [Запуск геокодирования](maps-and-location/geocoding.md)
### [Получение сведений о текущем расположении](maps-and-location/get-location.md)
### [Руководство по разработке приложений, учитывающих данные о расположении](maps-and-location/guidelines-and-checklist-for-detecting-location.md)
### [Настройка геозоны](maps-and-location/set-up-a-geofence.md)
### [Руководство для разработчиков относительно создания геозон](maps-and-location/guidelines-for-geofencing.md)
## [Монетизация приложения и привлечение клиентов](monetize/index.md)
### [Исключение или ограничение функций в пробной версии](monetize/exclude-or-limit-features-in-a-trial-version-of-your-app.md)
### [Поддержка покупки внутренних продуктов приложения](monetize/enable-in-app-product-purchases.md)
#### [Поддержка покупок потребляемых внутренних продуктов приложения](monetize/enable-consumable-in-app-product-purchases.md)
#### [Управление большим каталогом внутренних продуктов приложения](monetize/manage-a-large-catalog-of-in-app-products.md)
#### [Проверка покупок продуктов с помощью квитанций](monetize/use-receipts-to-verify-product-purchases.md)
### [Пакет SDK Microsoft Store Engagement and Monetization](monetize/monetize-your-app-with-the-microsoft-store-engagement-and-monetization-sdk.md)
### [Показ рекламы в приложении](monetize/display-ads-in-your-app.md)
#### [Показ рекламы с помощью библиотек Microsoft advertising](monetize/display-ads-using-the-microsoft-advertising-libraries.md)
##### [Начало работы с библиотеками Microsoft advertising](monetize/get-started-with-microsoft-advertising-libraries.md)
###### [Установка библиотек Microsoft Advertising](monetize/install-the-microsoft-advertising-libraries.md)
###### [В чем различие: AdMediatorControl или AdControl](monetize/what-is-the-difference-admediatorcontrol-or-adcontrol.md)
###### [Процессы создания приложений с рекламой](monetize/workflows-for-creating-apps-with-ads.md)
###### [Заметки о выпуске](monetize/release-notes-for-the-advertising-libraries.md)
###### [Известные проблемы](monetize/known-issues-for-the-advertising-libraries.md)
###### [Дополнительная справка](monetize/additional-help.md)
###### [Важное примечание— EULA](monetize/important-notice-eula.md)
##### [Информация для разработчиков](monetize/developer-information.md)
###### [Рекомендации по взаимодействию с пользователем и работе с пользовательским интерфейсом](monetize/ui-and-user-experience-guidelines.md)
###### [Значения тестового режима](monetize/test-mode-values.md)
###### [Настройка рекламных блоков в приложении](monetize/set-up-ad-units-in-your-app.md)
###### [Обработка ошибок](monetize/error-handling-with-advertising-libraries.md)
###### [Отправка приложения с рекламой в Магазин Windows](monetize/submit-an-app-with-ads-to-the-windows-store.md)
###### [Поддерживаемые размеры рекламных баннеров](monetize/supported-ad-sizes-for-banner-ads.md)
###### [Поддерживаемые рынки для Microsoft Advertising](monetize/supported-markets-for-microsoft-advertising.md)
##### [Пошаговые руководства для разработчиков](monetize/developer-walkthroughs.md)
###### [Промежуточная видеореклама](monetize/interstitial-ads.md)
###### [AdControl в XAML и .NET](monetize/adcontrol-in-xaml-and--net.md)
###### [AdControl в HTML 5 и JavaScript](monetize/adcontrol-in-html-5-and-javascript.md)
###### [AdControl в Windows Phone Silverlight](monetize/adcontrol-in-windows-phone-silverlight.md)
##### [Примеры кода](monetize/code-samples.md)
###### [Пример свойств HTML](monetize/html-properties-example.md)
###### [Пример свойств XAML](monetize/xaml-properties-example.md)
###### [Создание AdControl в JavaScript](monetize/create-an-adcontrol-in-javascript.md)
###### [События AdControl в JavaScript](monetize/adcontrol-events-in-javascript.md)
###### [События AdControl в C#](monetize/adcontrol-events-in-c.md)
###### [Обработка ошибок AdControl](monetize/adcontrol-error-handling.md)
####### [Пошаговое руководство по обработке ошибок в XAML/C#](monetize/error-handling-in-xamlc-walkthrough.md)
####### [Пошаговое руководство по обработке ошибок в JavaScript](monetize/error-handling-in-javascript-walkthrough.md)
###### [Пример кода промежуточной рекламы в C#](monetize/interstitial-ad-sample-code-in-c.md)
###### [Пример кода промежуточной рекламы в JavaScript](monetize/interstitial-ad-sample-code-in-javascript.md)
##### [Руководства по устранению неполадок](monetize/troubleshooting-guides.md)
###### [Руководство по устранению неполадок для HTML и JavaScript](monetize/html-and-javascript-troubleshooting-guide.md)
###### [Руководство по устранению неполадок в XAML и C#](monetize/xaml-and-c-troubleshooting-guide.md)
#### [Использование рекламного посредника для увеличения дохода](monetize/use-ad-mediation-to-maximize-revenue.md)
##### [Выбор рекламных сетей и управление ими](monetize/select-and-manage-your-ad-networks.md)
##### [Добавление и использование элемента управления рекламным посредником](monetize/add-and-use-the-ad-mediator-control.md)
##### [Проверка реализации рекламного посредника](monetize/test-your-ad-mediation-implementation.md)
##### [Отправка приложения и настройка рекламного посредника](monetize/submit-your-app-and-configure-ad-mediation.md)
##### [Устранение неполадок рекламного посредника](monetize/troubleshoot-ad-mediation.md)
### [Выполнение экспериментов в приложении с использованием A/B-тестирования](monetize/run-app-experiments-with-a-b-testing.md)
#### [Определение эксперимента на панели мониторинга в Центре разработки](monetize/define-your-experiment-in-the-dev-center-dashboard.md)
#### [Кодирование приложения для экспериментов](monetize/code-your-experiment-in-your-app.md)
#### [Управление экспериментом на информационной панели Центра разработки](monetize/manage-your-experiment.md)
#### [Создание и запуск первого эксперимента с использованием A/B-тестирования](monetize/create-and-run-your-first-experiment-with-a-b-testing.md)
### [Запуск Центра отзывов из приложения](monetize/launch-feedback-hub-from-your-app.md)
### [Использование служб МагазинаWindows](monetize/using-windows-store-services.md)
#### [Доступ к аналитическим данным](monetize/access-analytics-data-using-windows-store-services.md)
##### [Сведения о покупках приложения](monetize/get-app-acquisitions.md)
##### [Сведения о покупках IAP](monetize/get-in-app-acquisitions.md)
##### [Получение данных отчетов об ошибках](monetize/get-error-reporting-data.md)
##### [Сведения об оценках приложения](monetize/get-app-ratings.md)
##### [Получение отзывов о приложении](monetize/get-app-reviews.md)
#### [Просмотр и предоставление продуктов из службы](monetize/view-and-grant-products-from-a-service.md)
##### [Запрос продуктов](monetize/query-for-products.md)
##### [Объявление потребляемого продукта в качестве выполненного](monetize/report-consumable-products-as-fulfilled.md)
##### [Предоставление бесплатных продуктов](monetize/grant-free-products.md)
##### [Обновление ключа идентификатора Магазина Windows](monetize/renew-a-windows-store-id-key.md)
## [Сетевые и веб-службы](networking/index.md)
### [Основы работы с сетями](networking/networking-basics.md)
### [Какую сетевую технологию выбрать?](networking/which-networking-technology.md)
### [Передача данных по сети в фоновом режиме](networking/network-communications-in-the-background.md)
### [Сокеты](networking/sockets.md)
### [WebSockets](networking/websockets.md)
### [HttpClient](networking/httpclient.md)
### [Каналы RSS и Atom](networking/web-feeds.md)
### [Фоновая передача данных](networking/background-transfers.md)
### [Пометка сетевых подключений идентификатором EDP](networking/tagging_network_connections_with_edp_identity.md)
## [Создание пакетов приложений](packaging/index.md)
### [Создание пакетов приложений UWP](packaging/packaging-uwp-apps.md)
### [Установка приложений с помощью средства WinAppDeployCmd.exe](packaging/install-universal-windows-apps-with-the-winappdeploycmd-tool.md)
### [Объявления возможностей приложения](packaging/app-capability-declarations.md)
## [Перенос приложений в Windows10](porting/index.md)
### [Переход с WindowsPhoneSilverlight на UWP](porting/wpsl-to-uwp-root.md)
#### [Сопоставление пространства имен и класса](porting/wpsl-to-uwp-namespace-and-class-mappings.md)
#### [Перенос проекта](porting/wpsl-to-uwp-porting-to-a-uwp-project.md)
#### [Устранение неполадок](porting/wpsl-to-uwp-troubleshooting.md)
#### [Перенос XAML и пользовательского интерфейса](porting/wpsl-to-uwp-porting-xaml-and-ui.md)
#### [Перенос для ввода-вывода, устройств и моделей приложений](porting/wpsl-to-uwp-input-and-sensors.md)
#### [Перенос уровней бизнеса и данных](porting/wpsl-to-uwp-business-and-data.md)
#### [Перенос для форм-фактора и взаимодействия с пользователем](porting/wpsl-to-uwp-form-factors-and-ux.md)
#### [Практический пример: Bookstore1](porting/wpsl-to-uwp-case-study-bookstore1.md)
#### [Практический пример: Bookstore2](porting/wpsl-to-uwp-case-study-bookstore2.md)
### [Переход от среды выполнения Windows 8.x к UWP](porting/w8x-to-uwp-root.md)
#### [Перенос проекта](porting/w8x-to-uwp-porting-to-a-uwp-project.md)
#### [Устранение неполадок](porting/w8x-to-uwp-troubleshooting.md)
#### [Перенос XAML и пользовательского интерфейса](porting/w8x-to-uwp-porting-xaml-and-ui.md)
#### [Перенос для ввода-вывода, устройств и моделей приложений](porting/w8x-to-uwp-input-and-sensors.md)
#### [Практический пример: Bookstore1](porting/w8x-to-uwp-case-study-bookstore1.md)
#### [Практический пример: Bookstore2](porting/w8x-to-uwp-case-study-bookstore2.md)
#### [Пример: QuizGame](porting/w8x-to-uwp-case-study-quizgame.md)
### [Переход от классических приложений к UWP](porting/desktop-to-uwp-root.md)
#### [Обзор преобразователя классических приложений (Project Centennial)](porting/desktop-to-uwp-run-desktop-app-converter.md)
#### [Ручное преобразование классического приложения для Windows в приложение UWP](porting/desktop-to-uwp-manual-conversion.md)
#### [Расширения преобразованных классических приложений](porting/desktop-to-uwp-extensions.md)
#### [Подписывание преобразованного классического приложения](porting/desktop-to-uwp-signing.md)
#### [Развертывание и отладка преобразованного приложения UWP](porting/desktop-to-uwp-deploy-and-debug.md)
### [Обновление проекта MicrosoftVisualStudio2015RC UWP до RTM-версии](porting/update-your-visual-studio-2015-rc-project-to-rtm.md)
### [Сопоставление концепции приложений для Windows для разработчиков iOS и Android](porting/android-ios-uwp-map.md)
### [Переход с iOS на UWP](porting/ios-to-uwp-root.md)
#### [Выбор подхода к разработке приложений iOS и UWP](porting/selecting-an-approach-to-ios-and-uwp-app-development.md)
#### [Знакомство с UWP для разработчиков iOS](porting/getting-started-with-uwp-for-ios-developers.md)
##### [Приступая к работе: создание проекта](porting/getting-started-creating-a-project.md)
##### [Приступая к работе: выбор языка программирования](porting/getting-started-choosing-a-programming-language.md)
##### [Приступая к работе: введение в Visual Studio](porting/getting-started-getting-around-in-visual-studio.md)
##### [Приступая к работе: общие элементы управления](porting/getting-started-common-controls.md)
##### [Приступая к работе: навигация](porting/getting-started-navigation.md)
##### [Приступая к работе: анимация](porting/getting-started-animation.md)
##### [Начало работы: что дальше?](porting/getting-started-what-next.md)
#### [Установка Windows10 на Mac](porting/setting-up-your-mac-with-windows-10.md)
### [Размещенные веб-приложения](https://developer.microsoft.com/windows/bridges/hosted-web-apps)
#### [Преобразование в Windows](porting/hwa-create-windows.md)
#### [Преобразование на Mac](porting/hwa-create-mac.md)
#### [Функции доступа](porting/hwa-access-features.md)
#### [Преобразование приложения Chrome](porting/hwa-chrome-conversion.md)
## [Безопасность](security/index.md)
### [Введение в разработку безопасных приложений для Windows](security/intro-to-secure-windows-app-development.md)
### [Проверка подлинности и удостоверение пользователя](security/authentication-and-user-identity.md)
#### [Хранилище учетных данных](security/credential-locker.md)
#### [Биометрия отпечатков пальцев](security/fingerprint-biometrics.md)
#### [Microsoft Passport и Windows Hello](security/microsoft-passport.md)
##### [Создание приложение для входа с использованием Microsoft Passport](security/microsoft-passport-login.md)
##### [Создание службы входа Microsoft Passport](security/microsoft-passport-login-auth-service.md)
#### [Смарт-карты](security/smart-cards.md)
#### [Общие сертификаты](security/share-certificates.md)
#### [Диспетчер учетных веб-записей](security/web-account-manager.md)
#### [Разблокировка с устройствами-компаньонами](security/companion-device-unlock.md)
#### [Брокер веб-проверки подлинности](security/web-authentication-broker.md)
### [Шифрование](security/cryptography.md)
#### [Сертификаты](security/certificates.md)
#### [Криптографические ключи](security/cryptographic-keys.md)
#### [Защита данных](security/data-protection.md)
#### [Коды проверки подлинности сообщений, хэши и подписи](security/macs-hashes-and-signatures.md)
#### [Ограничения на экспорт шифрования](security/export-restrictions-on-cryptography.md)
#### [Общие задачи шифрования](security/common-cryptography-tasks.md)
##### [Создание случайных значений](security/create-random-numbers.md)
##### [Сравнение буферов](security/compare-buffers.md)
##### [Преобразования между строками и двоичными данными](security/convert-between-strings-and-binary-data.md)
##### [Копирование в массивы байтов и из них](security/copy-to-and-from-byte-arrays.md)
##### [Кодирование и декодирование данных](security/encode-and-decode-data.md)
## [Потоки и асинхронное программирование](threading-async/index.md)
### [Асинхронное программирование (приложения UWP)](threading-async/asynchronous-programming-universal-windows-platform-apps.md)
### [Асинхронное программирование на языке C++ (приложения UWP)](threading-async/asynchronous-programming-in-cpp-universal-windows-platform-apps.md)
### [Рекомендации по использованию пула потоков](threading-async/best-practices-for-using-the-thread-pool.md)
### [Вызов асинхронных API в C# и VisualBasic](threading-async/call-asynchronous-apis-in-csharp-or-visual-basic.md)
### [Создание периодического рабочего элемента](threading-async/create-a-periodic-work-item.md)
### [Отправка рабочего элемента в пул потоков](threading-async/submit-a-work-item-to-the-thread-pool.md)
### [Отправка рабочего элемента по таймеру](threading-async/use-a-timer-to-submit-a-work-item.md)
## [Приложения UWP для Xbox One](xbox-apps/index.md)
### [Начало работы](xbox-apps/getting-started.md)
### [Что нового](xbox-apps/whats-new.md)
### [Известные проблемы](xbox-apps/known-issues.md)
### [Вопросы и ответы](xbox-apps/frequently-asked-questions.md)
### [Активация режима разработчика в Xbox One](xbox-apps/devkit-activation.md)
### [Средства](xbox-apps/introduction-to-xbox-tools.md)
#### [Fiddler](xbox-apps/uwp-fiddler.md)
### [Настройка среды разработки](xbox-apps/development-environment-setup.md)
### [Рекомендации](xbox-apps/tailoring-for-xbox.md)
#### [Отключение режима мыши](xbox-apps/how-to-disable-mouse-mode.md)
#### [Отключение масштабирования](xbox-apps/disable-scaling.md)
#### [Отключение нерабочей области](xbox-apps/turn-off-overscan.md)
### [Распределение системных ресурсов](xbox-apps/system-resource-allocation.md)
### [Введение в многопользовательские приложения](xbox-apps/multi-user-applications.md)
### [Примеры](xbox-apps/samples.md)
### [Перенос существующих игр на Xbox](xbox-apps/development-lanes-landing.md)
#### [Особая версия C++](xbox-apps/development-lanes-custom-cpp.md)
#### [Unity](xbox-apps/development-lanes-unity.md)
#### [HTML/JS](xbox-apps/development-lanes-html.md)
### [Отключение режима разработчика в Xbox One](xbox-apps/devkit-deactivation.md)
### [Справочник по API](xbox-apps/reference.md)
#### [/api/app/packagemanager/register](xbox-apps/wdp-loose-folder-register-api.md)
#### [/api/app/packagemanager/upload](xbox-apps/wdp-folder-upload.md)
#### [/ext/fiddler](xbox-apps/wdp-fiddler-api.md)
#### [/ext/screenshot](xbox-apps/wdp-media-capture-api.md)
#### [/ext/settings](xbox-apps/wdp-xboxsettings-api.md)
#### [/ext/smb/developerfolder](xbox-apps/wdp-smb-api.md)
#### [/ext/user](xbox-apps/wdp-user-management.md)
#### [/ext/xboxlive/sandbox](xbox-apps/wdp-sandbox-api.md)
## [Компоненты среды выполнения Windows](winrt-components/index.md)
### [Создание компонентов среды выполнения Windows на C++](winrt-components/creating-windows-runtime-components-in-cpp.md)
### [Пошаговое руководство: создание базового компонента среды выполнения Windows на C++ и его вызов из JavaScript или C#](winrt-components/walkthrough-creating-a-basic-windows-runtime-component-in-cpp-and-calling-it-from-javascript-or-csharp.md)
### [Создание компонентов среды выполнения Windows на C# и VisualBasic](winrt-components/creating-windows-runtime-components-in-csharp-and-visual-basic.md)
#### [Пользовательские события и методы доступа к событиям в компонентах среды выполнения Windows](winrt-components/custom-events-and-event-accessors-in-windows-runtime-components.md)
#### [Передача массивов компоненту среды выполнения Windows](winrt-components/passing-arrays-to-a-windows-runtime-component.md)
#### [Диагностика состояний ошибки компонентов среды выполнения Windows](winrt-components/diagnosing-windows-runtime-component-error-conditions.md)
#### [Распространение управляемого компонента среды выполнения Windows](winrt-components/distributing-a-managed-windows-runtime-component.md)
#### [Сопоставление типов .NET Framework с типами среды выполнения Windows](winrt-components/net-framework-mappings-of-windows-runtime-types.md)
### [Пошаговое руководство: создание простого компонента среды выполнения Windows и его вызов из JavaScript](winrt-components/walkthrough-creating-a-simple-windows-runtime-component-and-calling-it-from-javascript.md)
### [Создание событий в компонентах среды выполнения Windows](winrt-components/raising-events-in-windows-runtime-components.md)
### [Компоненты среды выполнения Windows для неопубликованных приложений Магазина Windows, выполняемые через посредника](winrt-components/brokered-windows-runtime-components-for-side-loaded-windows-store-apps.md)
## [Платформа XAML](xaml-platform/index.md)
### [Обзор языка XAML](xaml-platform/xaml-overview.md)
#### [Руководство по синтаксису XAML](xaml-platform/xaml-syntax-guide.md)
##### [Синтаксис Property-path](xaml-platform/property-path-syntax.md)
##### [Синтаксис команд перемещения и рисования](xaml-platform/move-draw-commands-syntax.md)
#### [XAML и пустое пространство](xaml-platform/xaml-and-whitespace.md)
#### [Области действия имен XAML](xaml-platform/xaml-namescopes.md)
#### [Пространства имен XAML и сопоставление пространств имен](xaml-platform/xaml-namespaces-and-namespace-mapping.md)
#### [Классы параметров шаблона](xaml-platform/template-settings-classes.md)
#### [Атрибут x:Class](xaml-platform/x-class-attribute.md)
#### [Атрибут x:FieldModifier](xaml-platform/x-fieldmodifier-attribute.md)
#### [Атрибут x:Key](xaml-platform/x-key-attribute.md)
#### [Атрибут x:Name](xaml-platform/x-name-attribute.md)
#### [Атрибут x:DeferLoadStrategy](xaml-platform/x-deferloadstrategy-attribute.md)
#### [Атрибут x:Phase](xaml-platform/x-phase-attribute.md)
#### [Директива x:Uid](xaml-platform/x-uid-directive.md)
#### [Расширение разметки {x:Bind}](xaml-platform/x-bind-markup-extension.md)
#### [Расширение разметки {Binding}](xaml-platform/binding-markup-extension.md)
#### [Расширение разметки {x:Null}](xaml-platform/x-null-markup-extension.md)
#### [Встроенные типы данных в языке XAML](xaml-platform/xaml-intrinsic-data-types.md)
#### [Расширение разметки {CustomResource}](xaml-platform/customresource-markup-extension.md)
#### [Расширение разметки {RelativeSource}](xaml-platform/relativesource-markup-extension.md)
#### [Расширение разметки {StaticResource}](xaml-platform/staticresource-markup-extension.md)
#### [Расширение разметки {TemplateBinding}](xaml-platform/templatebinding-markup-extension.md)
#### [Расширение разметки {ThemeResource}](xaml-platform/themeresource-markup-extension.md)
### [Общие сведения о свойствах зависимостей](xaml-platform/dependency-properties-overview.md)
### [Пользовательские свойства зависимостей](xaml-platform/custom-dependency-properties.md)
### [Общие сведения о присоединенных свойствах](xaml-platform/attached-properties-overview.md)
### [Пользовательские присоединенные свойства](xaml-platform/custom-attached-properties.md)
### [Общие сведения о событиях и перенаправленных событиях](xaml-platform/events-and-routed-events-overview.md)
## [Справка API](https://msdn.microsoft.com/library/windows/apps/bg124285.aspx)
## [Службы](https://developer.microsoft.com/windows/develop/microsoft-services)
## [Руководства по Windows8.x](https://msdn.microsoft.com/library/windows/apps/mt244353.aspx)
## [Руководства по Windows Phone Silverlight 8.x](https://msdn.microsoft.com/library/windows/apps/ff402535.aspx)
# [Публикация приложений для Windows](publish/index.md)
## [Использование информационной панели Центра разработки для Windows](publish/using-the-windows-dev-center-dashboard.md)
### [Типы, доступность и стоимость учетных записей](publish/account-types-locations-and-fees.md)
### [Создание учетной записи разработчика](publish/opening-a-developer-account.md)
### [Управление параметрами учетной записи и сведениями профиля](publish/managing-your-profile.md)
### [Как пользователи Windows 10 видят ваше приложение в Магазине](publish/how-your-app-appears-in-the-store-for-windows-10-customers.md)
### [Защита товарных знаков и авторских прав](publish/trademark-and-copyright-protection.md)
### [Программа предварительной оценки для Центра разработки](publish/dev-center-insider-program.md)
## [Управление пользователями учетных записей](publish/manage-account-users.md)
## [Создание приложения посредством резервирования имени](publish/create-your-app-by-reserving-a-name.md)
## [Отправка приложений](publish/app-submissions.md)
### [Настройки цен и доступности](publish/set-app-pricing-and-availability.md)
#### [Определение цены и выбор рынка](publish/define-pricing-and-market-selection.md)
#### [Параметры корпоративного лицензирования](publish/organizational-licensing.md)
### [Ввод свойств приложения](publish/enter-app-properties.md)
#### [Таблица категорий и подкатегорий](publish/category-and-subcategory-table.md)
#### [Объявления приложений](publish/app-declarations.md)
### [Возрастные категории](publish/age-ratings.md)
### [Отправка пакетов приложений](publish/upload-app-packages.md)
#### [Руководство по управлению пакетами приложения](publish/guidance-for-app-package-management.md)
#### [Требования к пакетам приложения](publish/app-package-requirements.md)
#### [Нумерация версий пакета](publish/package-version-numbering.md)
#### [Поддерживаемые языки](publish/supported-languages.md)
#### [Устранение ошибок, связанных с отправкой пакетов](publish/resolve-package-upload-errors.md)
### [Создание описаний приложений](publish/create-app-descriptions.md)
#### [Создание отличного описания приложения](publish/write-a-great-app-description.md)
#### [Снимки экрана и изображения приложения](publish/app-screenshots-and-images.md)
#### [Создайте описания для различных платформ](publish/create-platform-specific-descriptions.md)
### [Заметки по сертификации](publish/notes-for-certification.md)
### [Процесс сертификации приложения](publish/the-app-certification-process.md)
#### [Устранение ошибок отправки](publish/resolve-submission-errors.md)
#### [Недопущение распространенных ошибок при сертификации](publish/avoid-common-certification-failures.md)
### [Тестовые пакеты](publish/package-flights.md)
### [Бета-тестирование и целевое распространение](publish/beta-testing-and-targeted-distribution.md)
### [Распространение бизнес-приложений для предприятий](publish/distribute-lob-apps-to-enterprises.md)
## [Отправки IAP](publish/iap-submissions.md)
### [Определение кода продукта IAP](publish/set-your-iap-product-id.md)
### [Ввод свойств IAP](publish/enter-iap-properties.md)
### [Определение цен и доступности IAP](publish/set-iap-pricing-and-availability.md)
### [Создание описаний IAP](publish/create-iap-descriptions.md)
### [Пакетное управление IAP](publish/manage-iaps-in-bulk.md)
## [Монетизация с использованием объявлений](publish/monetize-with-ads.md)
### [О рекламе от аффилированных лиц Майкрософт](publish/about-affiliate-ads.md)
### [Интеграция Центра разработки и pubCenter](publish/pubcenter-dev-center-integration.md)
### [Консолидация учетных записей для выплат pubCenter-DevCenter и налогового профиля](publish/pubcenter-devcenter-payout-account-and-tax-profile-consolidation.md)
## [Управление приложениями и службы](publish/app-management-and-services.md)
### [Использование служб карт](publish/use-map-services.md)
### [Просмотр сведений идентификации приложений](publish/view-app-identity-details.md)
### [Управление именами приложений](publish/manage-app-names.md)
### [Создание пакетов предустановки для изготовителей оборудования](publish/generate-preinstall-packages-for-oems.md)
## [Аналитика](publish/analytics.md)
### [Отчет о приобретениях](publish/acquisitions-report.md)
### [Отчет о работоспособности](publish/health-report.md)
### [Отчет об оценках](publish/ratings-report.md)
### [Отчет об отзывах](publish/reviews-report.md)
### [Отчет об отзывах](publish/feedback-report.md)
### [Отчет об использовании](publish/usage-report.md)
### [Отчет о приобретениях IAP](publish/iap-acquisitions-report.md)
### [Отчет о рекламном посреднике](publish/ad-mediation-report.md)
### [Отчет об эффективности рекламы](publish/advertising-performance-report.md)
### [Отчет о результативности рекламы от аффилированных лиц Майкрософт](publish/affiliates-performance-report.md)
### [Отчет о рекламных объявлениях, которые привели к установке приложений](publish/app-install-ads-reports.md)
### [Отчет о каналах и преобразованиях](publish/channels-and-conversions-report.md)
### [Загрузка аналитических отчетов](publish/download-analytic-reports.md)
## [Повышение популярности приложения и информированности о нем пользователей](publish/app-promotion-and-customer-engagement.md)
### [Создание рекламной кампании для вашего приложения](publish/create-an-ad-campaign-for-your-app.md)
#### [Управление рекламной кампанией](publish/managing-your-ad-campaign.md)
#### [Сведения о собственной рекламе](publish/about-house-ads.md)
#### [О рекламе от сообщества](publish/about-community-ads.md)
#### [Общие вопросы о рекламных кампаниях](publish/common-questions.md)
### [Создание пользовательской кампании по продвижению приложения](publish/create-a-custom-app-promotion-campaign.md)
### [Создавайте промокоды](publish/generate-promotional-codes.md)
### [Выставление на продажу приложений и IAP](publish/put-apps-and-iaps-on-sale.md)
### [Реакция на отзывы пользователей](publish/respond-to-customer-reviews.md)
### [Рекомендации по маркетингу приложений](publish/app-marketing-guidelines.md)
### [Ссылка на ваше приложение](publish/link-to-your-app.md)
### [Повысьте популярность вашего приложения](publish/make-your-app-easier-to-promote.md)
## [Получение оплаты](publish/getting-paid-apps.md)
### [Настройка счета для выплат и налоговых форм](publish/setting-up-your-payout-account-and-tax-forms.md)
### [Сводка выплат](publish/payout-summary.md)
### [Процесс оплаты Microsoft Advertising](publish/microsoft-advertising-payment-process.md)
### [Налогообложение для платных приложений](publish/tax-details-for-paid-apps.md)
### [Биллинг мобильного оператора](publish/mobile-operator-billing.md)
### [Сведения об НДС](publish/vat-info.md)
## [Политики Магазина и правила поведения](https://msdn.microsoft.com/library/windows/apps/dn764939.aspx)
### [Политики Магазина Windows](https://msdn.microsoft.com/library/windows/apps/dn764944.aspx)
#### [Журнал изменений](https://msdn.microsoft.com/library/windows/apps/dn764945.aspx)
### [Качество приложения](https://msdn.microsoft.com/library/windows/apps/mt652261.aspx)
### [Правила поведения для разработчиков](https://msdn.microsoft.com/library/windows/apps/dn764941.aspx)


<!--HONumber=Jul16_HO2-->


