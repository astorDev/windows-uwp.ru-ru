...
description: "В данном техническом документе описаны техники переноса приложений .NET и Win32 с устаревших ресурсов Win32 в ресурсы MRT при упаковке в виде пакетов AppX."
ms.author: ptorr
ms.date: 03/09/2017
ms.topic: article
ms.prod: windows
...
---
# <a name="migrating-legacy-resources-to-the-windows-10-resource-management-platform"></a>Перенос устаревших ресурсов на платформу управления ресурсами Windows 10

## <a name="executive-summary"></a>Краткий обзор

Приложения Win32 часто локализованы на разных языках, расширяя свой общий адресуемый рынок<a name="footnote1_ref" href="#footnote1"><sup>1</sup></a>. Существует множество способов локализации традиционных приложений Win32, однако Windows 8 представила [новую систему управления ресурсами](https://msdn.microsoft.com/en-us/library/windows/apps/jj552947.aspx) (далее в данном документе "MRT" <a name="footnote2_ref" href="#footnote2"><sup>2</sup></a>), которая работает со всеми языками программирования, в различных типах приложений и предоставляет функциональные возможности, которые гораздо шире простой локализации. 

В сочетании с развертыванием на основе AppX (например, из Магазина Windows) MRT может автоматически предоставлять самые подходящие ресурсы для определенного пользователя или устройства, что сводит к минимуму размер загрузки и установки приложения. Это уменьшение размера может быть очень значительным для приложений с большим объемом локализованного содержимого. Оно может достигать даже порядка несколько *ГБ* для игр высочайшего класса. Дополнительные преимущества MRT включают локализованные описания в оболочке Windows и Магазине Windows, логику автоматического перехода к запасному варианту, если выбранный пользователем язык не соответствует доступным ресурсам.
...
<td>Требуется совсем немного усилий, чтобы ваше локализованное содержимое отобразилось в оболочке Windows и в Магазине Windows</td>
<td>Небольшое</td>
</tr>
<tr>
...
Вот простой пример приложения, которое содержит текстовые метки на двух кнопках (`openButton` и `saveButton`) и PNG-файл, используемый для логотипа (`logoImage`). Текстовые метки локализованы на английском и немецком языках, а логотип оптимизирован для обычных мониторов настольных компьютеров (коэффициент масштабирования 100%) и телефонов с высоким разрешением (коэффициент масштабирования 300%) <a name="footnote4_ref" href="#footnote4"><sup>4</sup></a>. Обратите внимание, что на этой схеме представлено высокоуровневое концептуальное представление модели; оно не совпадает в точности с реализацией:
...
0. В XML-файл добавьте следующее содержимое, где <span style="background-color: yellow">выделенный текст</span> необходимо заменить соответствующим текстом для вашего приложения на языке по умолчанию<a name="footnote5_ref" href="#footnote5"><sup>5</sup></a>
...
Если вы напрямую редактируете XML-файл, откройте файл `AppxManifest.xml` и внесите следующие изменения в <span style="background-color: lightgreen">выделенные значения</span>. Используйте этот *точный* текст, а не текст, относящийся к приложению<a name="footnote6_ref" href="#footnote6"><sup>6</sup></a>. Эти имена должны соответствовать `Names`, которые вы создали в файле `.resw`, с добавленной в префиксе схемой `ms-resource:` и пространством имен `Resources/`. 
...
0. Введите следующую команду, заменив "contoso_demo.xml" именем, соответствующим вашему проекту, и "en-US" на язык по умолчанию для вашего приложения (или оставьте его en-US, если необходимо). Обратите внимание, что XML-файл создается в родительском каталоге (а **не** в каталоге проекта), так как он не является частью приложения<a name="footnote7_ref" href="#footnote7"><sup>7</sup></a>:
 ...
 * `/o` Задает перезапись выходного файла, если он существует
0. После создания пакета его необходимо подписать. Самый простой способ получить сертификат подписи — создать пустой проект универсального приложения для Windows в Visual Studio и скопировать файл `.pfx`, который он создает. Можно также создать сертификат вручную с помощью служебных программ `MakeCert` и `Pvk2Pfx` как описано в разделе [ **Создание сертификата для подписи пакета приложения** на веб-сайте MSDN] (https://msdn.microsoft.com/en-us/library/windows/desktop/jj835832(v=vs.85).aspx). 
...
В качестве контрпримера: если приложение использует плоскую структуру файлов с одним каталогом `Resources`, содержащим файлы `english_strings.dll` и `french_strings.dll`, он **не** будет правильно сопоставляться с MRT. Лучшей структурой был бы каталог `Resources` с подкаталогами и файлами `en\strings.dll` и `fr\strings.dll`
<a name="footnote8_ref" href="#footnote8"><sup>8</sup></a>. Обратите внимание, что по-прежнему можно использовать MRT и преимущества пакетов AppX, даже если не удается выполнить требование, касающееся названий файла. Просто для этого потребуется больше усилий. Этот случай будет рассмотрен в следующей технической документации. 
...
Среда выполнения .NET будет вызывать событие `AssemblyResolve` всякий раз, когда не удается найти библиотеки DLL ресурсов. В этот момент обработчик событий обнаружит нужный файл через MRT и вернет сборку<a name="footnote9_ref" href="#footnote9"><sup>9</sup></a>.
...
Нажмите на номер, чтобы вернуться в исходное расположение в документе.

<a name="footnote1" href="#footnote1_ref">1.</a> В этой статье не приводится дальнейшая мотивация для локализации. Предполагается, что читатель уже понимает ее.

<a name="footnote2" href="#footnote2_ref">2.</a> В прошлом это называлось "Современная технология ресурсов", но термин "Современный" был отменен. Диспетчер ресурсов может быть также известен как MRM (диспетчер современных ресурсов) или PRI (индекс ресурсов пакета).

<a name="footnote3" href="#footnote3_ref">3.</a> В эту таблицу не включены задачи, не имеющие отношения к локализации, например, предоставление значков приложений с высоким разрешением или высокой контрастностью. Дополнительные сведения о предоставлении несколько ресурсов для плиток, значков, и т. д. см. в базе MSDN.

<a name="footnote4" href="#footnote4_ref">4.</a> Особенности коэффициента масштабирования здесь не рассматриваются

<a name="footnote5" href="#footnote5_ref">5.</a> Обратите внимание, что существует ограничение на длину некоторых из этих строк. Дополнительные сведения об этом см. в разделе [**VisualElements** на веб-сайте MSDN](https://msdn.microsoft.com/en-us/library/windows/apps/br211471.aspx) 

<a name="footnote6" href="#footnote6_ref">6.</a> Не требуется использовать именно эти имена ресурсов. Вы можете выбрать собственные имена, но выбранные вами имена должны точно совпадать с именами, содержащимися в файле `.resw`.

<a name="footnote7" href="#footnote7_ref">7.</a> Вы можете выбрать любой другой каталог по желанию, но не забудьте подставить имя каталога в команды на следующих этапах

<a name="footnote8" href="#footnote8_ref">8.</a> Можно также использовать такое же имя основного файла, но со встроенными квалификаторами, например `strings.lang-en.dll` и `strings.lang-fr.dll`, но использование каталогов с кодами языков концептуально проще, поэтому мы рассматриваем его подробно.

<a name="footnote9" href="#footnote9_ref">9.</a> Обратите внимание, что если ваше приложение уже содержит обработчик `AssemblyResolve` для других целей, необходимо будет интегрировать кода разрешения ресурсов с существующим кодом.
