...
Если вы используете C# или C + +/ CX, затем добавьте новый класс в свой проект и имя класса **записи**.
...
Если вы используете [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), вам следует удалить функцию **MainPage::ClickHandler** в порядке для сборки проекта.
...
Если вы используете [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), а затем Дополнительные сведения о привязке к отслеживаемой коллекции в [элементы управления XAML; привязка к C + +/ WinRT коллекции](/windows/uwp/cpp-and-winrt-apis/binding-collection). Если вы этого раздела сначала считать, затем намерение C + +/ WinRT код приведенных ниже будет более четкими.
...
> [!NOTE]
> Если вы используете C++, пользовательский Интерфейс не будет выглядеть так же, как на рисунке ниже: отрисовка свойство **ReleaseDateTime** отличается. В следующем разделе более подробное описание этого.
...
Как видно, выше, для форматирования гибкость мы используем разметки передавать строку формата в преобразователь через параметр преобразователя. В примерах кода, показанный в этом разделе, только C# преобразователь значений упрощает использование этого параметра. Но можно легко передавать строку формата стиле C++ в качестве параметра преобразователя и использовать этот символ в преобразователь значений с помощью функции форматирования, например **wprintf** или **swprintf**.
...
> [!NOTE]
> Начиная с Windows 10 версии 1607, платформа XAML предоставляет встроенный преобразователь Boolean в Visibility. Преобразователь сопоставляет **значение true,** значение перечисления **Visibility.Visible** и **значение false,** чтобы **Visibility.Collapsed** , вы можете привязать свойство Visibility к Boolean без создания преобразователя. Для использования встроенного преобразователя минимальная версия целевого пакета SDK вашего приложения должна быть 14393 или более поздней. Вы не сможете использовать преобразователь, если ваше приложение предназначено для более ранних версий Windows 10. Дополнительные сведения о целевой версии см. в разделе [версии адаптивного кода](https://msdn.microsoft.com/windows/uwp/debug-test-perf/version-adaptive-code).
