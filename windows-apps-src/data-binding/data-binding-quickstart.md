...
> [!NOTE]
> Если вы используете C++, пользовательский Интерфейс не будет выглядеть так же, как на рисунке ниже: отрисовки **ReleaseDateTime** свойства отличается. В следующем разделе более подробное описание этого.
...
Как видно выше, форматирование гибкость для мы используем разметку в преобразователь через параметр преобразователя передаем строку формата. В примерах кода, показанный в этом разделе, только C# преобразователь значений упрощает использование этого параметра. Но можно легко передавать строку формата стиле C++ в качестве параметра преобразователя и использовать этот символ в преобразователь значений с помощью функции форматирования, например **wprintf** или **swprintf**.
