---
description: Расширение разметки xBind позволяет функций, которые используются в разметке.
...
ms.locfileid: "57626219"
---
# <a name="functions-in-xbind"></a>Функции в x:Bind

> [!NOTE]
> Общие сведения об использовании привязки данных в приложении с помощью **{x: Bind}** (и для сравнения комплексная между **{x: Bind}** и **{Binding}**), см. в разделе [данных Привязка глубину](data-binding-in-depth.md).
...
Статические функции можно указывать с помощью синтаксиса XMLNamespace:ClassName.MethodName. Например, ниже синтаксис для привязки к статические функции в коде.
...
Также можно использовать системные функции непосредственно в разметке для выполнения простых сценариев, таких как форматирование даты, форматирование текста, объединение текста и др., например:
...
Обработчик привязки реагирует на изменение свойства уведомления инициируется именем функции и заново определить привязки при необходимости. Например:
...
> [!TIP]
> Для достижения сценариев, подобных тем, что поддерживались через преобразователи типов или MultiBinding в WPF, можно использовать функции в x: Bind.
...
При двусторонней привязке следует определить вторую функцию для обратного направления привязки. Это делается с помощью **BindBack** привязка свойства. В следующем примере функция должна принимает один аргумент, который является значением, который должен быть помещен обратно в модель.
