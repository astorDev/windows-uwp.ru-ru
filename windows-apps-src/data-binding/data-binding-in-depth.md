...
> [!NOTE]
> **{x: Bind}** не подходит для сценариев позднего связывания, таких как перемещение по структуре словаря объекта JSON, а также типизация. «Типизация» представляет собой слабую форму типизации на основе словарных соответствий имен свойств (в «проходит через, плавает и крякает как утка, при утка»). С утиной типизацией привязка к свойству **Age** одинаково удовлетворяется **человека** или **вино** объекта (при условии, что эти типы был свойство **Age** ). В этих сценариях использования расширения разметки **{Binding}** .
...
К примеру с книгами, группировании книг по имени автора приводит к коллекции, где каждая группа содержит групп
