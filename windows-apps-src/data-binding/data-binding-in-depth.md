...
-   Можно выполнить однократно для привязки [ **изображение** ](https://msdn.microsoft.com/library/windows/apps/BR242752) к фотографии текущего пользователя.
-   Односторонний режим можно использовать для привязки [ **ListView** ](https://msdn.microsoft.com/library/windows/apps/BR242878) к коллекции в режиме реального времени новостных статей, сгруппированные по газете раздела.
-   Можно использовать для привязки режим двустороннее [ **текстовое** ](https://msdn.microsoft.com/library/windows/apps/BR209683) имени клиента в форме.

Независимо от режима, существует два типа привязки, и они оба обычно объявляются в разметке пользовательского интерфейса. Вы можете использовать [расширение разметки {x:Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783) или [расширение разметки {Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782). Кроме того, можно одновременно использовать оба типа привязки в одном приложении и даже в одном элементе пользовательского интерфейса. {x: Bind} является новым для Windows 10, и он имеет более высокую производительность. Все сведения, описанные в этом разделе, касаются обоих типов привязки, если явно не указано иное.
...
При использовании C + +/ WinRT или Visual C++ расширения компонентов (C + +/ CX), то вам потребуется добавить [ **BindableAttribute** ](https://msdn.microsoft.com/library/windows/apps/Hh701872) к любому классу среды выполнения, который вы хотите использовать атрибут [{Binding} ](https://msdn.microsoft.com/library/windows/apps/Mt204782) расширения разметки с.

> [!IMPORTANT]
> Если вы используете [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt), а затем [ **BindableAttribute** ](https://msdn.microsoft.com/library/windows/apps/Hh701872) атрибут доступен, если вы установили пакет Windows SDK версии 10.0.17763.0 (Windows 10, версия 1809 ), или более поздней версии. Без этого атрибута необходимо реализовать [ICustomPropertyProvider](/uwp/api/windows.ui.xaml.data.icustompropertyprovider) и [ICustomProperty](/uwp/api/windows.ui.xaml.data.icustomproperty) интерфейсов, чтобы иметь возможность использовать [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782) разметки расширение.
...
> [!NOTE]
> Изменения в [ **TextBox.Text** ](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.textbox.text) отправляются двусторонней привязки источнику [ **TextBox** ](https://msdn.microsoft.com/library/windows/apps/BR209683) теряет фокус, а не после каждого нажатия пользователем клавиши.
...
При использовании C + +/ WinRT или Visual C++ расширения компонентов (C + +/ CX) выберите, чтобы использовать [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782) расширения разметки, вам потребуется добавить [ **BindableAttribute** ](https://msdn.microsoft.com/library/windows/apps/Hh701872) атрибут для любого класса среды выполнения, который вы хотите выполнить привязку к. Чтобы использовать [{x: Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783), этот атрибут не требуется.
...
У преобразователя также есть необязательные параметры: [**ConverterLanguage**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.data.binding.converterlanguage), который позволяет указать язык, который используется в преобразовании, и [ **ConverterParameter**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.data.binding.converterparameter), что позволяет передать параметр Логика преобразования. Пример использования параметра преобразователя см. в описании [**IValueConverter**](https://msdn.microsoft.com/library/windows/apps/BR209903).

> [!NOTE]
> Если возникает ошибка при преобразовании, не следует генерировать исключения. Вместо этого возвращайте свойство [**DependencyProperty.UnsetValue**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.dependencyproperty.unsetvalue), которое остановит передачу данных.
...
**Примечание**  этот раздел относится только к [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782), так как не удается создать [{x: Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783) привязками в коде. Однако некоторые из преимуществ {x:Bind} можно получить с помощью метода [**DependencyObject.RegisterPropertyChangedCallback**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.dependencyobject.registerpropertychangedcallback.aspx), что позволяет зарегистрироваться для получения уведомлений об изменениях любого свойства зависимости.
...
| Источник | Не требуется | `<ListView ItemsSource="{Binding Orders, Source={StaticResource MyData}}"/>` | Для {x: Bind} можно напрямую использовать именованный элемент, используйте свойство или статическим путем. | 
