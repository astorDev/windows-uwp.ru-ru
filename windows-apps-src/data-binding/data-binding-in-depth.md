...
Привязку данных можно использовать, чтобы просто отобразить значения из источника данных при первом отображении пользовательского интерфейса, не реагируя на изменения этих значений. Это режим привязки под названием *единовременной*, и он лучше всего подойдет для со значением, которое не изменяется во время выполнения. Кроме того вы можете «отслеживать» значения и обновлять пользовательский Интерфейс при их изменении. Этот режим называется *односторонней*, и он лучше всего подойдет для данных только для чтения. Наконец, вы можете как отслеживать, так и обновлять данные, чтобы изменения, которые пользователь вносит в значения в пользовательском интерфейсе, автоматически отправлялись в источник данных. Этот режим называется *двусторонней*, и он лучше всего подойдет для чтения и записи данных. Вот несколько примеров.
...
-   Можно использовать одностороннем режиме привязать [**ListView**](https://msdn.microsoft.com/library/windows/apps/BR242878) к коллекции актуальных новостных статей, сгруппированных по разделу газеты.
...
Таким образом, чтобы не требовалось реализовывать приведенный выше несколько раз, если вы используете C#, а затем можно просто шаблон выполнить наследование от базового класса **BindableBase** , который можно найти в образце [QuizGame](https://github.com/Microsoft/Windows-appsample-quizgame) (в папке «Common»). Пример приведен ниже.
...
Чтобы показать, что действительно отслеживаемое свойство **HostViewModel.NextButtonText** , добавьте обработчик событий **Click** к кнопке и обновите значение **HostViewModel.NextButtonText**. Сборка и запуск нажмите кнопку, чтобы увидеть значение кнопки **содержимого** обновления.
...
> [!NOTE]
> **{x: Bind}** не подходит для сценариев позднего связывания, таких как перемещение по структуре словаря объекта JSON, а также типизация. «Типизация» представляет собой слабую форму типизации на основе словарных соответствий имен свойств (в «проходит, плавает и крякает как утка, при утка»). С утиной типизацией привязка к свойству **Age** одинаково удовлетворяется **человека** или **вино** объекта (при условии, что эти типы было свойство **Age** ). В этих сценариях использования расширения разметки **{Binding}** .
