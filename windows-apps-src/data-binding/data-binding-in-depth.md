...
Привязку данных можно использовать, чтобы просто отобразить значения из источника данных при первом отображении пользовательского интерфейса, не реагируя на изменения этих значений. Это режим привязки под названием *одноразовый*, и он лучше всего подойдет для значения, которые не изменяются во время выполнения. Кроме того вы можете «отслеживать» значения и обновлять пользовательский Интерфейс при их изменении. Это более называется *односторонней*, и он лучше всего подойдет для данных только для чтения. Наконец, вы можете как отслеживать, так и обновлять данные, чтобы изменения, которые пользователь вносит в значения в пользовательском интерфейсе, автоматически отправлялись в источник данных. Этот режим называется *двусторонней*, и он лучше всего подойдет для чтения и записи данных. Вот несколько примеров.

-   Можно использовать режим одноразовый привязка [**изображения**](https://msdn.microsoft.com/library/windows/apps/BR242752) к фотографии текущего пользователя.
...
-   Двухсторонний режим можно использовать для привязки [**текстовое поле**](https://msdn.microsoft.com/library/windows/apps/BR209683) имени пользователя в форме.
...
Таким образом, чтобы не требовалось реализовывать приведенный выше несколько раз, если вы используете C#, а затем можно просто шаблон производным от базового класса **BindableBase** , который можно найти в образце [QuizGame](https://github.com/Microsoft/Windows-appsample-quizgame) (в папке «Общие»). Пример приведен ниже.
...
Чтобы показать, что свойство **HostViewModel.NextButtonText** действительно отслеживаемую, добавьте обработчик событий **Click** к кнопке и обновите значение **HostViewModel.NextButtonText**. Сборки, запустите и нажмите кнопку, чтобы просмотреть значение кнопки **содержимого** обновления.
...
> [!NOTE]
> Изменения [**TextBox.Text**](https://msdn.microsoft.com/library/windows/apps/windows.ui.xaml.controls.textbox.text) отправляются двусторонней присоединенного источника, когда [**TextBox**](https://msdn.microsoft.com/library/windows/apps/BR209683) теряет фокус и не после каждого нажатия клавиши пользователем.
...
> [!NOTE]
> **{x: Bind}** не подходит для сценариев, таких как перемещение по структуре словаря объекта JSON, ни типизация. «Типизация» представляет собой слабую форму типизации на основе словарных соответствий имен свойств (в «проходит, плавает и крякает как утка, при утка»). С утиной типизацией привязка к свойству **Age** одинаково удовлетворяется **человека** или **вин** объекта (при условии, что эти типы было свойство **Age** ). Для этих сценариев использование расширения разметки **{Binding}** .

### <a name="binding-object-declared-using-binding"></a>Объект привязки, объявленный с использованием расширения разметки {Binding}

Если вы используете C + +/ WinRT или Visual C++ расширения компонентов (C + +/ CX), чтобы использовать расширение разметки [{Binding}](https://msdn.microsoft.com/library/windows/apps/Mt204782) , необходимо добавить атрибут [**BindableAttribute**](https://msdn.microsoft.com/library/windows/apps/Hh701872) для любого класса среды выполнения, которое вы хотите выполнить привязку к. Чтобы использовать [{x: Bind}](https://msdn.microsoft.com/library/windows/apps/Mt204783), этот атрибут не нужен.
...
К примеру с книгами, группировании книг по имени автора приводит к коллекции групп где у каждой группы
