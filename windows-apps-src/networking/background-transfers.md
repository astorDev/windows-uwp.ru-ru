...
При запуске приложением фоновой передачи данных это приложение должно вызвать [**AttachAsync**](/uwp/api/windows.networking.backgroundtransfer.downloadoperation.AttachAsync) на всех существующих объектах [**DownloadOperation**](/uwp/api/windows.networking.backgroundtransfer.downloadoperation?branch=live). Невыполнение этого требования может привести к утечке таких событий, что делает функцию фоновой передачи бесполезной.
