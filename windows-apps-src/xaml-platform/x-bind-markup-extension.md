---
description: Расширение разметки xBind является альтернативой высокую производительность для привязки. xBind - новое в Windows 10 — выполняется быстрее и меньше памяти, чем привязка и поддерживает более отладки.
...
ms.locfileid: "67320276"
---
# <a name="xbind-markup-extension"></a>Расширение разметки x:Bind

**Примечание**  Общие сведения об использовании привязки данных в приложении с помощью **{x: Bind}** (и для сравнения комплексная между **{x: Bind}** и **{Binding}** ), см. в разделе [привязки данных в глубину](https://docs.microsoft.com/windows/uwp/data-binding/data-binding-in-depth).

**{X: Bind}** расширения разметки — новые возможности Windows 10 — представляет собой альтернативу **{Binding}** . **{x: Bind}**  выполняется быстрее и меньше памяти, чем **{Binding}** и поддерживает Улучшенная отладка.
...
-   [Образец {x: Bind}](https://go.microsoft.com/fwlink/p/?linkid=619989)
-   [QuizGame](https://github.com/microsoft/Windows-appsample-networkhelper)
-   [Пример основы пользовательского интерфейса XAML](https://go.microsoft.com/fwlink/p/?linkid=619992)
...
| _bindingProperties_ |
| _propName_=_значение_\[, _propName_=_значение_\]* | Одна или несколько привязок свойств, указанных с помощью синтаксиса пары "имя/значение". |
...
В случае использования языков C++/CX **{x:Bind}** нельзя привязать к частным полям и свойствам в модели страницы или данных — вам потребуется открытое свойство для выполнения привязки. Контактную зону для привязки необходимо предоставлять в качестве классов/интерфейсов CX, чтобы можно было получить соответствующие метаданные. **\[Bindable\]** атрибута не должно возникнуть необходимости.

При использовании **x:Bind** вам не нужно применять **ElementName=xxx** как часть выражения привязки. Вместо этого может служить имя элемента в первой части пути для привязки именованных элементов, вступают в поля внутри страницы или пользовательского элемента управления, представляющий источник привязки корневой. 


### <a name="collections"></a>Коллекции

Если источником данных выступает коллекция, то в пути свойства можно указывать элементы коллекции по их позиции или индексу. Например «команд\[0\]. Игроки», где литерала "\[\]" содержит «0», запрашивает первого элемента в коллекции с нулевым индексом.

Чтобы воспользоваться индексатором, модели необходимо реализовать **IList&lt;T&gt;** или **IVector&lt;T&gt;** в типе свойства, которое подлежит индексации. (Обратите внимание, IReadOnlyList&lt;T&gt; и IVectorView&lt;T&gt; не поддерживают синтаксис индексатора.) Если тип индексированного свойства поддерживает **INotifyCollectionChanged** или **IObservableVector**, а привязка относится к типу OneWay или TwoWay, то она будет принимать и регистрировать уведомления об изменениях на этих интерфейсах. Логика отслеживания изменений обновляется с учетом всех изменений коллекции, даже если эти изменения не влияют на конкретное индексированное значение. Это обусловлено тем, что логика ожидания передачи данных общая для всех экземпляров коллекции.

Если источником данных выступает словарь или карта, то в пути свойства можно указывать элементы коллекции по имени строки. Например **&lt;TextBlock Text = "{x: Bind игроков\[«Николай Новиков»\]" /&gt;** будет искать элемент в словарь с именем «Александр Иванов». Имя необходимо заключить в кавычки, которые могут быть как двойными, так и одинарными. Для экранирования кавычек в строках можно использовать символ карет (^). Зачастую проще всего использовать кавычки, отличные от тех, что используются для атрибута XAML. (Обратите внимание, IReadOnlyDictionary&lt;T&gt; и IMapView&lt;T&gt; не поддерживают синтаксис индексатора.)
...
**Groups3** в **Text = "{x: Bind ((data:SampleDataGroup) groups3\[0\]). Title}»** представляет собой словарь объектов, поэтому необходимо преобразовать его в **данных: SampleDataGroup**. Обратите внимание на то, как используется префикс пространства имен XML **data:** для сопоставления типа объектов с пространством имен кода, не являющимся частью пространства имен XAML по умолчанию.

_Примечание. C#-Синтаксиса приведения стиля является более гибким, чем синтаксис, присоединенное свойство, и рекомендованный синтаксис будет вперед._

## <a name="functions-in-binding-paths"></a>Функции в путях привязки

Начиная с Windows 10 версии 1607 **{x: Bind}** поддерживает использование функции на конечном этапе шаге пути привязки. Это мощная функция для привязки данных, который обеспечивает несколько сценариев в разметке. См. в разделе [функцию привязки](../data-binding/function-bindings.md) подробные сведения.

## <a name="event-binding"></a>Привязка события

Привязка события – это уникальная функция для компилированной привязки. Она позволяет определять обработчик для события с использованием привязки, а не применяется в качестве метода в коде программной части. Пример: **Нажмите кнопку «{x: Bind rootFrame.GoForward}» =** .
...
| **Преобразователь** | Указывает объект преобразователя, вызываемый механизмом привязки. Преобразователь можно задать в коде XAML, но только в случае, если вы ссылаетесь на экземпляр объекта, присвоенный в ссылке на [расширение разметки {StaticResource}](staticresource-markup-extension.md) этому объекту в словаре ресурсов. |
| **ConverterLanguage** | Указывает язык и региональные параметры, используемые преобразователем. (Если он установлен **ConverterLanguage** вы также должны параметр **преобразователь**.) Для культуры устанавливается в качестве идентификатора на основе стандартов. Подробнее: [**ConverterLanguage**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converterlanguage). |
| **converterParameter** | Указывает параметр преобразователя, который можно использовать в логике преобразователя. (Если он установлен **ConverterParameter** вы также должны параметр **преобразователь**.) Большинство преобразователи использовать простой логики, получения всех сведений, они должны из переданного значения для преобразования, и нет необходимости **ConverterParameter** значение. Параметр **ConverterParameter** используется для более расширенных преобразователей, у которых больше одной логики и которым недостаточно информации, переданной в **ConverterParameter**. Вы также можете написать преобразователь, который использует нестроковые значения, но это используется редко. Подробнее см. в разделе "Примечания" статьи [**ConverterParameter**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converterparameter). |
| **fallbackValue** | Задает значение, которое отображается, когда не удается разрешить источник или путь. |
| **Режим** | Указывает режим привязки, как одна из следующих строк: «OneTime», «OneWay» или «TwoWay». Значение по умолчанию — OneTime. Обратите внимание, что это поведение отличается от шаблона по умолчанию для привязки **{Binding}** , которая в большинстве случаев имеет значение OneWay. |
...
| **UpdateSourceTrigger** | Указывает, когда возвращать изменения в элементе управления модели в привязках TwoWay. Значение по умолчанию для всех свойств, за исключением TextBox.Text PropertyChanged; TextBox.Text — LostFocus.|
...
При использовании **{x: Bind}** с помощью шаблонов данных, необходимо указать тип, которая привязывается к, задав **x: DataType** значение, как показано в [примеры](#examples) раздел. Вы также можете задавать тип для интерфейса или базового класса, а затем использовать приведения, если понадобится сформулировать полное выражение.
...
**Совет**    Если вам нужно указать в одиночную фигурную скобку для значения, например в [ **путь** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.path) или [ **ConverterParameter** ](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converterparameter), укажите перед ним обратную косую черту: `\{`. Также можно включить всю строку, содержащую скобки, которые нужно преобразовать, в дополнительный набор кавычек, например: `ConverterParameter='{Mix}'`.
