...
Расширение разметки **{x:Bind}** — новая функция для Windows 10 — является альтернативой для **{Binding}**. Расширение разметки **{x:Bind}** не имеет некоторых функций **{Binding}**, но оно быстрее запускается, требует меньше памяти, чем **{Binding}**, и поддерживает более эффективную отладку.
...
**Примечание.** Если вы преобразуете разметку **{Binding}** в **{x:Bind}**, то следует учитывать различия между значениями по умолчанию для свойства **Mode**.
