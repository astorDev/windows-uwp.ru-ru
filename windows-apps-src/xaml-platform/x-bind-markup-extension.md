...
Если источником данных выступает коллекция, то в пути свойства можно указывать элементы коллекции по их позиции или индексу. Например, "Teams \[0 \]. Players", где литерал "\[ \]" заключает значение "0", которое запрашивает первый элемент в коллекции с нулевым индексом.
