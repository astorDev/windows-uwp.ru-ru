...
ms.locfileid: "70296990"
---
# <a name="xbind-markup-extension"></a>Расширение разметки x:Bind

Примечание  . Общие сведения об использовании привязки данных в приложении с **{КС:бинд}** (и при сравнении между **{КС:бинд}** и **{Binding}** ) см. в разделе о [привязке данных в подробном](https://docs.microsoft.com/windows/uwp/data-binding/data-binding-in-depth)виде.
...
| _биндингпропертиес_ |
| =_значение_пропнаме,=_значение_пропнаме\[\]* | Одна или несколько привязок свойств, указанных с помощью синтаксиса пары "имя/значение". |
...
Если источником данных выступает коллекция, то в пути свойства можно указывать элементы коллекции по их позиции или индексу. Например, "команды\[0.\] Players ", где литерал"\[\]"заключает" 0 ", который запрашивает первый элемент в коллекции с нулевым индексом.

Чтобы воспользоваться индексатором, модели необходимо реализовать **IList&lt;T&gt;** или **IVector&lt;T&gt;** в типе свойства, которое подлежит индексации. (Обратите внимание&lt;,&gt; что иреадонлилист&lt;t&gt; и IVectorView t не поддерживают синтаксис индексатора.) Если тип индексированного свойства поддерживает **INotifyCollectionChanged** или **IObservableVector**, а привязка относится к типу OneWay или TwoWay, то она будет принимать и регистрировать уведомления об изменениях на этих интерфейсах. Логика отслеживания изменений обновляется с учетом всех изменений коллекции, даже если эти изменения не влияют на конкретное индексированное значение. Это обусловлено тем, что логика ожидания передачи данных общая для всех экземпляров коллекции.
...
_Примечание. Синтаксис C#приведения в стиле является более гибким, чем синтаксис присоединяемых свойств, и является рекомендуемым синтаксисом._
...
Привязка события – это уникальная функция для компилированной привязки. Она позволяет определять обработчик для события с использованием привязки, а не применяется в качестве метода в коде программной части. Пример: **Щелкните = "{X:BIND rootFrame. GoForward}"** .
...
| **конвертерлангуаже** | Указывает язык и региональные параметры, используемые преобразователем. (Если вы настраиваете **конвертерлангуаже** , необходимо также задать **преобразователь**.) Язык и региональные параметры задаются в виде идентификатора на основе стандартов. Подробнее: [**ConverterLanguage**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converterlanguage). |
| **ConverterParameter** | Указывает параметр преобразователя, который можно использовать в логике преобразователя. (Если вы настраиваете **ConverterParameter** , необходимо также задать **преобразователь**.) Большинство преобразователей используют простую логику, которая получает все необходимые сведения из передаваемого значения и не требует значения **ConverterParameter** . Параметр **ConverterParameter** используется для более расширенных преобразователей, у которых больше одной логики и которым недостаточно информации, переданной в **ConverterParameter**. Вы также можете написать преобразователь, который использует нестроковые значения, но это используется редко. Подробнее см. в разделе "Примечания" статьи [**ConverterParameter**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.data.binding.converterparameter). |
| **фаллбакквалуе** | Задает значение, которое отображается, когда не удается разрешить источник или путь. |
| **Режим** | Задает режим привязки в виде одной из следующих строк: "OneTime", "OneWay" или "TwoWay". Значение по умолчанию — OneTime. Обратите внимание, что это поведение отличается от шаблона по умолчанию для привязки **{Binding}** , которая в большинстве случаев имеет значение OneWay. |
