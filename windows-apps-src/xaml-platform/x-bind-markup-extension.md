---
author: jwmsft
description: Расширение разметки xBind является альтернативой высокопроизводительной привязки. xBind - появившиеся в Windows 10 — работает быстрее и меньше памяти, чем Binding и поддерживает более отладки.
...
Расширение разметки **{x:Bind}**—новая функция для Windows10—является альтернативой для **{Binding}**. **{x: Bind}** работает быстрее и меньше памяти, чем **{Binding}** и поддерживает более эффективную отладку.
...
При использовании **x:Bind** вам не нужно применять **ElementName=xxx** как часть выражения привязки. Вместо вы можете использовать имя элемента в качестве первой части пути для привязки, поскольку именованные элементы становятся полями страницы или пользовательского элемента управления, представляющий собой источник привязки корня. 
