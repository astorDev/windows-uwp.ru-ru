...
В данном примере метод `showUpdatesButton_Click` основан на делегате [**RoutedEventHandler**](https://msdn.microsoft.com/library/windows/apps/br208812). Использовать нужно делегат, имя которого указано в синтаксисе для метода [**Click**](https://msdn.microsoft.com/library/windows/apps/br227737) на странице справки MSDN.
...
**Примечание.** В Visual Studio и ее рабочей области конструирования XAML вместо ключевого слова **Handles** обычно используется метод обработки экземпляра. Причина заключается в том, что установка подключения обработчика событий в XAML является частью стандартного рабочего процесса дизайнер-разработчик, а подход с использованием ключевого слова **Handles** несовместим с подключением обработчиков событий в XAML.
...
Также вы можете удалять обработчики, если событие было добавлено через атрибут XAML, что означает добавление события в сформированный код. Если у вас есть значение **Name** для элемента, для которого был прикреплен обработчик, то удалить его будет проще, так как в коде создается ссылка на объект. Если объект не имеет **Name**, возможен вариант обхода дерева объектов для обнаружения ссылки на объект.
...
-   Значение свойства элемента **Background** или **Fill** не равно **null**. Значение **null** свойства [**Brush**](https://msdn.microsoft.com/library/windows/apps/br228076) означает, что элемент прозрачен и проверка нажатия не может быть выполнена. (Чтобы сделать элемент прозрачным, но доступным для проверки нажатия, используйте значение свойства кисти [**Transparent**](https://msdn.microsoft.com/library/windows/apps/hh748061), а не **null**.)

**Примечание.**
            **Background** и **Fill** не определены элементом [**UIElement**](https://msdn.microsoft.com/library/windows/apps/br208911), а определены различными производными классами, например [**Control**](https://msdn.microsoft.com/library/windows/apps/br209390) и [**Shape**](https://msdn.microsoft.com/library/windows/apps/br243377). Но применения кистей, которые вы используете для свойств переднего и заднего планов, одинаковы для проверки нажатия и событий ввода, независимо от того, какой подкласс реализует свойства.
...
Некоторые элементы управления имеют особые правила для проверки нажатия. Например, [**TextBlock**](https://msdn.microsoft.com/library/windows/apps/br209652) не обладает свойством **Background**, но доступен для проверки нажатия в пределах границ его площади. [
              **Image**
            ...
            ](https://msdn.microsoft.com/library/windows/apps/br227702) располагают специальным поведением проверки нажатия, поскольку ввод может обрабатываться размещенным HTML и вызывать события сценария.
