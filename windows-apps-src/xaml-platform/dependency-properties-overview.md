...
1. **Анимированные значения:** активные анимации, анимации визуального состояния или анимации с поведением [**HoldEnd**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Media.Animation.FillBehavior). Чтобы получить практический результат, примененная к свойству анимация должна иметь приоритет перед основным (неанимированным) значением, даже если это значение было задано локально.
1. **Локальное значение:** локальное значение может быть задано через удобную оболочку свойства, что равнозначно заданию атрибута или элемента свойства в XAML либо путем вызова метода [**SetValue**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.dependencyobject.setvalue) с использованием свойства конкретного экземпляра. Если локальное значение задается с помощью привязки или статического ресурса, то каждое из этих действий имеет приоритет, равный приоритету локального значения, поэтому ссылки на ресурсы или привязки удаляются, если задается новое локальное значение.
1. **Шаблонные свойства:** элемент имеет такие свойства, если был создан как часть шаблона ([**ControlTemplate**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Controls.ControlTemplate) или [**DataTemplate**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.DataTemplate)).
1. **Методы установки значений для стиля:** значения из класса [**Setter**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.Setter) в стилях из ресурсов страницы или приложения.
...
## <a name="dependencyobject-and-threading"></a>**DependencyObject** и потоки
...
- [Раскадрованные анимации](https://docs.microsoft.com/windows/uwp/graphics/storyboarded-animations)
