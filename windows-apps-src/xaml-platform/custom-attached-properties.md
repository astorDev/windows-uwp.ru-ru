...
> [!WARNING]
> Существующие ограничения реализации XAML среды выполнения Windows — это, что нельзя анимации настраиваемое вложенное свойство.
...
Главной областью, в которой определение пользовательского присоединенного свойства отличается от пользовательского свойства зависимостей, является способ определения методов доступа или программ-оболочек. Вместо использования программы-оболочки, приведенными в [зависимостей настраиваемые свойства](custom-dependency-properties.md)необходимо также указать static **получения *** PropertyName* и **задать *** PropertyName* методы как методы доступа для вложенных свойств. Данные методы доступа используются в основном средством синтаксического анализа языка XAML, хотя для задания значений в сценариях без применения языка XAML их может использовать и любой другой абонент.

> [!IMPORTANT]
> Если не определить методы доступа правильно, обработчик XAML не может получить доступ к вложенное свойство и, возможно, кто-то пытается использовать его возникнет ошибка синтаксического анализа XAML. Кроме того, инструменты проектирования и программирования зачастую используют соглашения \*Property для присвоения имен идентификаторам при обнаружении пользовательских свойств зависимостей в сборке, на которую выполняется ссылка.
...
Подпись для **задать *** PropertyName* это должно быть доступа к данным.
...
Определение вложенное свойство в C + +/ CX немного сложнее. Необходимо определить отношения между заголовочным файлом и файлом кода. Кроме того, идентификатор следует представить как свойство с единственным методом доступа **get** по причинам, изложенным в разделе [Пользовательские свойства зависимостей](custom-dependency-properties.md). В C + +/ CX, необходимо определить в этом поле свойства отношения явно вместо того чтобы полагаться на языку .NET **только для чтения** и неявных резервное простых свойств. Также необходимо выполнить регистрацию присоединенного свойства во вспомогательной функции, которая выполняется только один раз, при первом запуске приложения, но до загрузки XAML-страниц, которым требуется присоединенное свойство. Вспомогательные функции, регистрирующие свойства, для всех свойств зависимостей и присоединенных свойств обычно вызываются из конструктора **App** / [**Application**](https://msdn.microsoft.com/library/windows/apps/br242325) в коде файла app.xaml.
...
> [!NOTE]
> При создании пользовательского интерфейса XAML с помощью C++, необходимо включить заголовок для пользовательского типа, который определяет вложенное свойство в любое время, что XAML-страницы с помощью этого типа. Каждая страница XAML имеет сопоставленный заголовок поддерживающего кода .xaml.h. Именно в него следует включить (с помощью **\#include**) заголовок для определения типа владельца присоединенного свойства.
...
Код выглядит примерно так в этом псевдокоде.
...
> [!NOTE]
> Дополнительные сведения об использовании панели в разделе [Общие сведения о настраиваемых панелях XAML](https://msdn.microsoft.com/library/windows/apps/mt228351).
