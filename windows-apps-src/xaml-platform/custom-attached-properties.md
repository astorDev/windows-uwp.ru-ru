...
> [!NOTE]
> При создании пользовательского интерфейса XAML с помощью C++ необходимо включать заголовок для настраиваемого типа, определяющего присоединенное свойство, при каждом использование этого типа в XAML-страницы. Каждая страница XAML имеет сопоставленный заголовок поддерживающего кода .xaml.h. Именно в него следует включить (с помощью **\#include**) заголовок для определения типа владельца присоединенного свойства.
