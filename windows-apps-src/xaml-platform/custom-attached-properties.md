...
> [!IMPORTANT]
> Если вы не указываете методы доступа определены неправильно, процессор XAML не имеют доступа к вашего присоединенного свойства, и любой пользователь, который пытается использовать его, скорее всего, возникнет ошибка средства синтаксического анализа XAML. Кроме того, инструменты проектирования и программирования зачастую используют соглашения \*Property для присвоения имен идентификаторам при обнаружении пользовательских свойств зависимостей в сборке, на которую выполняется ссылка.
