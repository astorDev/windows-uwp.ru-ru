...
Определите присоединенное свойство как свойство зависимостей, объявив **открытый** **статический** **readonly** свойство типа [  **DependencyProperty**](https://docs.microsoft.com/uwp/api/Windows.UI.Xaml.DependencyProperty). Данное свойство определяется при помощи возвращаемого значения метода [**RegisterAttached**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.dependencyproperty.registerattached). Имя свойства должно совпадать с именем присоединенного свойства, можно указать в качестве **RegisterAttached** *имя* параметр в строке, в конец которого добавлен «Property». Таково установленное соглашение для присвоения имен идентификаторам свойств зависимостей в отношении свойств, которые они представляют.
...
> [!IMPORTANT]
> Если методы доступа не был определен правильно, обработчик XAML не может получить доступ к присоединенное свойство, и всем, кто пытается использовать его, вероятно, будет получена ошибка средства синтаксического анализа XAML. Кроме того, разработки и инструменты программирования часто зависит от "\*свойство» соглашения об именовании идентификаторов, при обнаружении настраиваемого свойства зависимостей в связанной сборке.

## <a name="accessors"></a>Методы доступа

Подпись для метода доступа **Get**_PropertyName_ должна быть следующей.

`public static` _valueType_ **получить**_PropertyName_ `(DependencyObject target)`
...
Сигнатура для **задать *** PropertyName* доступа должно быть это.
...
> [!NOTE]
> Это также можно определить присоединенного свойства, где предполагаемое использование через синтаксис элемента свойства. В этом случае не требуется преобразование типов для значений, однако нужно обеспечить возможность создания запланированных значений в XAML. [**VisualStateManager.VisualStateGroups** ](https://docs.microsoft.com/dotnet/api/system.windows.visualstatemanager?view=netframework-4.8) является примером существующего присоединенного свойства, который поддерживает только использование элемента свойства.
...
Определяет присоединенное свойство в C++/CX немного сложнее. Необходимо определить отношения между заголовочным файлом и файлом кода. Кроме того, идентификатор следует представить как свойство с единственным методом доступа **get** по причинам, изложенным в разделе [Пользовательские свойства зависимостей](custom-dependency-properties.md). В C++/CX, необходимо определить эту связь ближнего действия свойство явно, а не полагается на .NET **readonly** языку и неявно заданное резервное простых свойств. Также необходимо выполнить регистрацию присоединенного свойства во вспомогательной функции, которая выполняется только один раз, при первом запуске приложения, но до загрузки XAML-страниц, которым требуется присоединенное свойство. Вспомогательные функции, регистрирующие свойства, для всех свойств зависимостей и присоединенных свойств обычно вызываются из конструктора **App** / [**Application**](https://docs.microsoft.com/uwp/api/windows.ui.xaml.application.) в коде файла app.xaml.
...
## <a name="setting-your-custom-attached-property-from-xaml-markup"></a>Задание пользовательских присоединенное свойство из разметки XAML

> [!NOTE]
> Если вы используете C++/WinRT, а затем перейдите к разделу ниже ([Задание настраиваемых присоединенное свойство императивно с C++/WinRT](#setting-your-custom-attached-property-imperatively-with-cwinrt)).
...
> [!NOTE]
> При создании пользовательского интерфейса XAML с C++/CX, то вы должны будут включать заголовок для пользовательского типа, который определяет присоединенное свойство, в любое время, страницы XAML использует этот тип. Каждая страница XAML имеет заголовок выделенным кодом (. xaml.h). Это происходит, когда следует включать (с помощью  **\#включают**) заголовок для определения тип владельца присоединенного свойства.

## <a name="setting-your-custom-attached-property-imperatively-with-cwinrt"></a>Задание пользовательских присоединенное свойство императивно с C++/WinRT

Если вы используете C++/WinRT, то можно получить доступ к настраиваемое присоединенное свойство из императивного кода, но не из разметки XAML. В коде ниже показан как.
...
Код выглядит примерно так это псевдокод.
...
> [!NOTE]
> Дополнительные сведения о работе панелей см. в разделе [Общие сведения о настраиваемых панелях XAML](https://docs.microsoft.com/windows/uwp/layout/custom-panels-overview).
