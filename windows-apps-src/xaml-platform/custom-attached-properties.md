---
description: В настоящем разделе разъясняются методики реализации присоединенного свойства на языке XAML как свойства зависимостей и определения соглашения о методе доступа, необходимого для использования присоединенного свойства в языке XAML.
...
**Внимание!** Ограничение реализации языка XAML в среде выполнения Windows заключается в отсутствии поддержки анимации пользовательских присоединенных свойств.
...
**Важно!** Если методы доступа не определены правильно, процессор XAML не сможет получить доступ к вашему присоединенному свойству, и при любой попытке его использования будет возникать ошибка средства синтаксического анализа языка XAML. Кроме того, инструменты проектирования и программирования зачастую используют соглашения \*Property для присвоения имен идентификаторам при обнаружении пользовательских свойств зависимостей в сборке, на которую выполняется ссылка.
