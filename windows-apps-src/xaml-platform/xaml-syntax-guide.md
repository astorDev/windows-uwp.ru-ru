...
Some properties on objects cannot be set in XAML by any means, and can only be set using code. Как правило, это свойства, более подходящие для работы с кодом, а не с языком XAML.
...
Синтаксис коллекций в языке XAML позволяет нам сделать вид, что мы задаем свойство для чтения, хотя на самом деле это не так. See "<bpt id="p1">[</bpt>Collection Syntax<ept id="p1">](#collection-syntax)</ept>" later in this topic.
...
Сначала присутствие объектного элемента укажет, что необходимо создать экземпляр нового объекта *objectName*. Only after such an instance exists can the instance property <bpt id="p1">*</bpt>propertyName<ept id="p1">*</ept> be set on it.
...
Язык XAML расширяет XML, добавляя элемент синтаксиса, известный как *присоединенное свойство*. Аналогично синтаксису элемента свойства синтаксис присоединенного свойства содержит точку, и эта точка имеет особое значение при анализе XAML. Specifically, the dot separates the provider of the attached property, and the property name.
...
<bpt id="p1">**</bpt>Note<ept id="p1">**</ept><ph id="ph1">  </ph>In reference topics for APIs that use XAML and use enumerations, click the link to the enumeration type in the <bpt id="p2">**</bpt>Property value<ept id="p2">**</ept> section of <bpt id="p3">**</bpt>Syntax<ept id="p3">**</ept>. Ссылка ведет на страницу перечисления, где вы найдете именованные константы для данного перечисления.
