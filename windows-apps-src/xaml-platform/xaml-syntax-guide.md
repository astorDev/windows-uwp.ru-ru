...
-   **Напрямую используя синтаксис объектных элементов:** Этот механизм использует открывающим и закрывающим тегами для создания экземпляра объекта, как элемент XML-форму. Вы можете применять этот синтаксис для объявления корневых объектов или создания вложенных объектов, задающих значения свойств.
-   **Косвенно с помощью синтаксиса атрибутов:** Это используется строковое значение встроенного, содержатся инструкции по созданию объекта. Средство синтаксического анализа XAML использует данную строку для задания значения свойства создаваемому опорному значению. Поддержка его ограничена определенными общими объектами и свойствами.
...
**Примечание**  некоторые структуры не могут объявляться как объектных элементов. Текст инициализации не поддерживается, и их нельзя использовать в качестве ресурсов. Чтобы задать такие значения для свойств в языке XAML, следует использовать синтаксис атрибута. Ниже приведены эти типы. [**Длительность**](https://msdn.microsoft.com/library/windows/apps/br242377), [ **RepeatBehavior**](https://msdn.microsoft.com/library/windows/apps/br210411), [ **точки**](https://msdn.microsoft.com/library/windows/apps/br225870), [ **Rect**  ](https://msdn.microsoft.com/library/windows/apps/br225994) и [ **размер**](https://msdn.microsoft.com/library/windows/apps/br225995).
...
Если свойство содержимого XAML существует для класса, это указывается в разделе справки для данного класса в подразделе "Атрибуты". Ищите значение [**ContentPropertyAttribute**](https://msdn.microsoft.com/library/windows/apps/br228011). Данный атрибут использует поле с именем Name. Значение Name — это имя свойства класса, являющееся свойством содержимого XAML. Например, на [ **границы** ](https://msdn.microsoft.com/library/windows/apps/br209250) справочной странице вы увидите следующее: ContentProperty("Name=Child").
...
**Примечание**  разработчики XAML часто выдвигают упорядочивания соглашения, если вы используете областей конструктора, отличного от редактора XML, но вы можете свободно изменять этот XAML в более поздней версии, чтобы изменить порядок атрибутов или добавлять новые.
...
-   Дочерние элементы могут информировать родительский контейнер панели, как они должны вести себя в макете: [**Холст**](https://msdn.microsoft.com/library/windows/apps/br209267), [ **сетки**](https://msdn.microsoft.com/library/windows/apps/br242704), [ **VariableSizedWrapGrid**](https://msdn.microsoft.com/library/windows/apps/br227651).
-   Использование элемента управления может влиять на поведение части важный элемент управления, которые поступают из шаблона элемента управления: [**ScrollViewer**](https://msdn.microsoft.com/library/windows/apps/br209527), [ **VirtualizingStackPanel**](https://msdn.microsoft.com/library/windows/apps/br227689).
-   С помощью службы, которая доступна в связанный класс, где служба и класс, который его использует не совместно используют наследования: [**Оформление**](https://msdn.microsoft.com/library/windows/apps/hh702143), [ **VisualStateManager**](https://msdn.microsoft.com/library/windows/apps/br209021), [ **AutomationProperties**](https://msdn.microsoft.com/library/windows/apps/br209081), [ **ToolTipService**](https://msdn.microsoft.com/library/windows/apps/br227609).
-   Анимация, предназначенные для: [**Раскадровки**](https://msdn.microsoft.com/library/windows/apps/br210490).
...
Так как выносить открывающую скобку символ \{ находится открывающий объектом последовательность расширения разметки, используйте escape-последовательности, чтобы указать значение строкового литерала, который начинается с "\{«. Escape-последовательность — "\{\}«. Например, чтобы указать строковое значение, — это единый открывающей фигурной скобкой, укажите значения атрибута в виде "\{\}\{«. Можно также использовать альтернативные кавычки (например, **"** в значении атрибута, разделяемых строками **«»**) для предоставления"\{"значение в виде строки.

**Примечание**  "\\}» также работает, если он находится внутри атрибута в кавычках.
 
...
**Примечание**  в справочные разделы по API, которые используют XAML и перечислений, щелкните ссылку, чтобы тип перечисления в **значение свойства** раздел **синтаксис**. Ссылка ведет на страницу перечисления, где вы найдете именованные константы для данного перечисления.
...
* [Пространства имен XAML и сопоставление пространств имен](xaml-namespaces-and-namespace-mapping.md)
