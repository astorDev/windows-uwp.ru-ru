...
Дополнительные сведения о отображение пространств имен XAML для пользовательских типов, включая специальные рекомендации для расширения компонентов Visual C++ (C + +/ CX), см. в разделе [пространства имен XAML и сопоставление пространств имен](xaml-namespaces-and-namespace-mapping.md).
...
-   Если вы часто используете в коде XAML одну и ту же цветную кисть, определите [**SolidColorBrush**](https://msdn.microsoft.com/library/windows/apps/br242962) в качестве ресурса, тогда вам не придется каждый раз указывать именованный цвет как значение атрибута.
