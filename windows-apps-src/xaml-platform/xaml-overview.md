...
Ниже приведен фрагмент кода, в котором показан созданный шаблоном <xref:Windows.UI.Xaml.Controls.Page> корень начальной страницы для приложения (в котором отображается только открывающий тег и упрощенный). Он объявляет пространство имен по умолчанию и пространство имен **x** (о котором пойдет речь ниже).
...
Например, ниже приведен синтаксис для ссылки на многократно используемый <xref:Windows.UI.Xaml.Style> из <xref:Windows.UI.Xaml.ResourceDictionary>: `<Button Style="{StaticResource SearchButtonStyle}"/>`. @No__t-0 — это ссылочный тип, а не простое значение, поэтому без использования `{StaticResource}` для установки свойства <xref:Windows.UI.Xaml.FrameworkElement.Style%2A?displayProperty=nameWithType> требуется элемент свойства `<Button.Style>` и определение `<Style>` в нем.
...
- Вы можете использовать стили и шаблоны для создания по шаблону существующих элементов управления или при определении элемента управления, поставляющего шаблон по умолчанию в составе своего пакета. При использовании для определения стилей и шаблонов соответствующий XAML часто объявляется как дискретный файл XAML с корнем <xref:Windows.UI.Xaml.ResourceDictionary>.
...
- Если в XAML часто используется та же цветовая кисть, определите <xref:Windows.UI.Xaml.Media.SolidColorBrush> как ресурс, а не используйте именованный цвет в качестве значения атрибута каждый раз.
...
- Если вы используете отдельные файлы XAML, предоставляющие ресурсы проектирования (<xref:Windows.UI.Xaml.ResourceDictionary.MergedDictionaries%2A>), рассмотрите возможность комментирования или удаления неиспользуемых ресурсов из этих файлов. Даже если у вас есть код XAML, который вы используете как отправную точку в нескольких приложениях или который предоставляет общие ресурсы для всех ваших приложений, именно приложение каждый раз упаковывает ресурсы XAML и в некоторых случаях вынуждено загружать их.
...
- Используйте контейнеры, такие как <xref:Windows.UI.Xaml.Controls.Border>, а не преднамеренные перерисовки элементов пользовательского интерфейса. В целом не следует рисовать один и тот же пиксель несколько раз. Дополнительные сведения о перерисовки и их проверке см. в разделе <xref:Windows.UI.Xaml.DebugSettings.IsOverdrawHeatMapEnabled?displayProperty=nameWithType>.
