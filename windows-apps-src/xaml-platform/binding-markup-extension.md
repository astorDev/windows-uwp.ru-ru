...
*PropertyPath* задает значение [**Path**](https://msdn.microsoft.com/library/windows/apps/br209830), который описывает свойство, привязываемое к свойству источника. PropertyPath — это позиционный параметр, то есть его можно задать в качестве первого безымянного параметра: `{Binding Path=...}`. Или можно явно использовать имя параметра: `{Binding Path=...}`.
