...
## <a name="why-doesnt-the-generated-implementation-function-for-a-read-only-property-have-the-const-qualifier"></a>Почему не созданный код реализации функции для только для чтения свойство `const` квалификатор?

При объявлении только для чтения свойство в [MIDL 3.0](/uwp/midl-3/)можно было бы ожидать `cppwinrt.exe` средство для создания функцию реализации, который является `const`-квалифицированного (const функция обрабатывает *Этот* указатель как const).

Конечно, рекомендуется использовать const, где это возможно, но `cppwinrt.exe` средству не пытаться причина о реализация функции можно представить могут быть const и которого может и не произойти. Можно внести какие-либо вашей реализации функций const, как показано в этом примере.
...
Можно удалить, `const` квалификатор на **ToString** следует решить, необходимо изменить некоторые состояние объекта в его реализации. Но сделать каждый член функции const или не const, но не оба. Другими словами, не перегрузок в реализации функции на `const`.

Помимо свои функции реализации других другой поместить где const переходит в изображение находится в проекции функции среды выполнения Windows. Рассмотрим следующий код.
...
Функции на проекции, const независимо от того, как выбор в рамках программы внедрения их. В фоновом проекции вызывает двоичный интерфейс приложений (ABI), какие суммы звонок через указатель интерфейса COM. Единственное состояние, которое планируемый **ToString** взаимодействует с является этот указатель интерфейса COM; и определенно не требуется настроить этот указатель таким образом, чтобы функция const. Это дает ссылки и гарантировать, что он не будет изменять что-либо о ссылке **IStringable** , вызове через и гарантирует, что можно вызвать **ToString** даже с полем const **IStringable**.

Понимать, что эти примеры `const` , сведения о реализации C + +/ WinRT проекции и реализации; они составляют нежелательных кода для справки. Нет нет такого понятия, как `const` на COM, ни ABI среды выполнения Windows (для функций-членов).

## <a name="do-you-have-any-recommendations-for-decreasing-the-code-size-for-cwinrt-binaries"></a>Имеется рекомендации для уменьшения размера кода C + +/ WinRT двоичных файлов?

При работе с объектами среды выполнения Windows, следует избегать шаблон кодирования, показанного ниже, потому что она может негативно сказывается на приложение, вызывая дополнительный код двоичные, чем необходимо создать.
...
В условиях среды выполнения Windows компилятору не удается для кэширования либо значение `c()` или интерфейсы для каждого метода, который вызывается через косвенное обращение ("."). Если ваше вмешательство, которая приводит к дополнительные виртуальные вызовы и дополнительную нагрузку подсчета ссылок. Шаблон выше легко может создать два раза больше код следующим строго необходимости. Вместо этого предпочитает использовать шаблон, показанный ниже там, где можно. Создает намного меньше кода и его можно также значительно повысить производительность время выполнения.
...
Рекомендуемый шаблон отображается над применяется не только к C + +/ WinRT, но для всех языковых проекций среды выполнения Windows.
