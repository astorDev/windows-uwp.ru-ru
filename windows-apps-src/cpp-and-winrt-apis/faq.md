...
При объявлении свойства только для чтения в [MIDL 3.0](/uwp/midl-3/)вы ожидаете `cppwinrt.exe` средство для создания в реализации функции для вас, `const`-полное (const функция обрабатывает *Этот* указатель как const).

Конечно, рекомендуется использовать const везде, где это возможно, но `cppwinrt.exe` само средство не пытаться причина, по которой о том, какую реализацию функции вполне может быть const, и который не может. Вы можете внести какие-либо из вашей реализации функции const, как показано в этом примере.
...
Вы можете удалить, `const` квалификатора для **ToString** следует решить, что необходимо изменить некоторые состояние объекта в своей реализации. Однако следует каждого из вашего члена функции const или не const, не оба сразу. Другими словами, не перегружать функцию реализации на `const`.
...
Функции в проекции являются const независимо от того как вы выбрали квалификация вашей реализации из них. Рендеринг проекции вызывает двоичный интерфейс приложения (ABI), какие суммы для вызова с помощью указатель интерфейса СОМ. Только состояние, в котором проецируемого **ToString** взаимодействует с является этот указатель COM-интерфейса; и не поддерживает определенно не нужно изменить этот указатель таким образом, чтобы функция const. Это дает гарантии, что оно не изменит ничего о **IStringable** ссылку, вызове через, и это гарантирует, что можно вызвать **ToString** даже с const ссылка на **IStringable**.
...
В мире среды выполнения Windows, компилятор не удалось кэшировать либо значение `c()` или интерфейсы для каждого метода, который будет вызываться через косвенного обращения ('. '). Если вы вмешиваться в процесс, приводит к дополнительные виртуальные вызовы и служебных данных подсчета ссылок. Шаблон выше может легко сформировать вдвое больше кода исключительно как требуется. Вместо этого предпочитают приведенный ниже там, где вы можете шаблон. Он создает гораздо меньше кода, и он может также существенно повысить производительность время выполнения.
