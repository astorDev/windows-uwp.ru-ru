...
Вы можете удалить, `const` квалификатора для **ToString** следует решить, что вам нужно изменить некоторые состояние объекта в своей реализации. Однако следует каждого из вашего член функции const или не const, не оба сразу. Другими словами, не перегружать функцию реализации на `const`.
...
Функции в проекции являются const независимо от того, как вы решили, какие реализацию их. Рендеринг проекции вызывает двоичный интерфейс приложения (ABI), какие суммы для вызова через указатель COM-интерфейса. Единственное состояние, которое проецируемого **ToString** взаимодействует с является этот указатель COM-интерфейса; и не поддерживает определенно не нужно изменять этот указатель, чтобы функция const. Это дает Software assurance, оно не изменит никаких сведений о **IStringable** ссылку, вызове через, что это гарантирует, что можно вызвать **ToString** даже при использовании const ссылка на **IStringable**.
