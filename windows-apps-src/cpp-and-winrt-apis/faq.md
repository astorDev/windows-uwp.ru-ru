...
ms.locfileid: "6463657"
---
# <a name="frequently-asked-questions-about-cwinrt"></a>Ответы на часто задаваемые вопросы о C++/WinRT
Ответы на вопросы, которые вы, скорее всего у о разработке и использовании API среды выполнения Windows с помощью [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt).
...
## <a name="why-wont-my-new-project-compile-im-using-visual-studio-2017-version-1580-or-higher-and-sdk-version-17134"></a>Почему не будет скомпилирован нового проекта Я использую Visual Studio 2017 (версии 15.8.0 или более поздней версии) и SDK версии 17134

Если вы используете Visual Studio 2017 (версии 15.8.0 или более поздней версии) и пакет Windows SDK версии 10.0.17134.0 (Windows 10, версия 1803), затем только что созданного C + +/ WinRT проект может не удаться компиляции с ошибкой «ошибка*C3861: «from_abi»: идентификатор не найти*» и другим ошибкам, рассчитанные в *base.h*. Решением является более поздней версии (Дополнительные совместимые) либо целевой версии Windows SDK, или задать свойство проекта **C/C++** > **язык** > **режим совместимости: нет** (Кроме того, если **/ permissive-** отображается в свойство проекта ** C/C++** > **командной строки** в столбце **Дополнительные параметры**, удалите его).
...
## <a name="why-doesnt-the-generated-implementation-function-for-a-read-only-property-have-the-const-qualifier"></a>Почему нет созданный реализации функции для свойства только для чтения `const` квалификатор?

При объявлении свойства только для чтения в [MIDL 3.0](/uwp/midl-3/)вы ожидаете `cppwinrt.exe` средство для создания в реализации функции для вас, которое соответствует `const`-полное (const функция обрабатывает *Этот* указатель как const).

Конечно, рекомендуется использовать const везде, где это возможно, но `cppwinrt.exe` сам инструмент не пытаться причина о том, какую реализацию функции вполне может быть const и который не может. Вы можете внести любые реализации функций const, как в этом примере.
...
Вы можете удалить, `const` квалификатора для **ToString** следует вы решите, что вам нужно изменить некоторые состояние объекта в своей реализации. Однако следует каждого из вашего члена функции const или не const, не оба сразу. Другими словами, не перегружать реализации функции на `const`.

Помимо вашей реализации функции, другой других разместить где const вступает в рисунка находится в проекции функции среды выполнения Windows. Рассмотрим этот код.
...
Для вызова **ToString** выше, команда **Перейти к объявлению** в Visual Studio показывает, что проекции **IStringable::ToString** среды выполнения Windows в C + +/ WinRT выглядит следующим образом.

```
winrt::hstring ToString() const;
```

Функции в проекции являются const независимо от того, как вы решили, какие из них вашей реализации. Рендеринг проекции вызывает двоичный интерфейс приложения (ABI), какие суммы для вызова через указатель COM-интерфейса. Только состояние, в котором проецируемого **ToString** взаимодействует с является этот указатель COM-интерфейса; и не поддерживает определенно не нужно изменять этот указатель, поэтому const функции. Это дает ссылка Software assurance, что оно не изменит никаких сведений о **IStringable** ссылку, которую вызова через, и это гарантирует, что можно вызвать **ToString** даже с const **IStringable**.

Понимать, что эти примеры `const` — подробные сведения о реализации C + +/ WinRT проекции и реализации; они составляют санации кода для свое преимущество. Такие понятия как не существует `const` на COM, ни ABI среды выполнения Windows (для функций-членов).

## <a name="do-you-have-any-recommendations-for-decreasing-the-code-size-for-cwinrt-binaries"></a>У вас есть все рекомендации для уменьшения размера кода для C + +/ WinRT двоичные файлы?
...
В мире среды выполнения Windows, компилятор не может кэшировать либо значение `c()` или интерфейсы для каждого метода, который будет вызываться через косвенного обращения ('. '). Если вы вмешиваться в процесс, приводит к более виртуальные вызовы и служебных данных подсчета ссылок. Шаблон выше может легко сформировать вдвое больше кода исключительно как требуется. Вместо этого предпочитают приведенный ниже там, где вы можете шаблон. Он создает намного меньше кода, и он может также существенно повысить производительность время выполнения.
