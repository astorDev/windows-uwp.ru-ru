...
При объявлении свойства только для чтения в [MIDL 3.0](/uwp/midl-3/)можно было ожидать `cppwinrt.exe` средство для создания в реализации функции для вас, которое соответствует `const`-квалифицированных (const функция обрабатывает *Этот* указатель как const).

Конечно, рекомендуется использовать константу везде, где это возможно, но `cppwinrt.exe` сам инструмент не пытаться причина о том, какую реализацию функции вполне может быть const и который не может. Вы можете внести какие-либо из вашей реализации функции const, как показано в этом примере.
...
Вы можете удалить, `const` квалификатора для **ToString** следует вы решите, что вам нужно изменить некоторые состояние объекта в своей реализации. Однако следует каждого из вашего член функции const или не const, не оба сразу. Другими словами, не перегружать реализации функции на `const`.
...
В мире среды выполнения Windows, компилятор не может кэшировать либо значение `c()` или интерфейсы для каждого метода, который будет вызываться через косвенного обращения ('. '). Если вы вмешаться, приводит к дополнительные виртуальные вызовы и служебных данных подсчета ссылок. Выше шаблон может легко сформировать вдвое больше кода исключительно как требуется. Вместо этого предпочитают приведенный ниже там, где вы можете шаблон. Он создает гораздо меньше код и он может также существенно повысить производительность время выполнения.
