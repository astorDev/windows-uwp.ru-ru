...
## <a name="why-wont-my-new-project-compile-im-using-visual-studio-2017-version-1580-or-higher-and-sdk-version-17134"></a>Почему не будет скомпилирован, новый проект? Я использую Visual Studio 2017 (версии 15.8.0 или более поздней версии) и пакет SDK версии 17134
...
## <a name="why-doesnt-the-generated-implementation-function-for-a-read-only-property-have-the-const-qualifier"></a>Почему не имеет функции созданных реализации для свойства только для чтения `const` квалификатор?

Если вы объявляете свойство только для чтения в [MIDL 3.0](/uwp/midl-3/), можно было ожидать `cppwinrt.exe` средство для создания в реализации функции, `const`-полное (const функция считает *это* указатель const).

Конечно, рекомендуется использовать const везде, где это возможно, но `cppwinrt.exe` средству не пытаться причина о том, какую реализацию функции вполне может быть const, и который не может. Вы можете внести какие-либо из вашей реализации функции const, как показано в этом примере.
...
Вы можете удалить, `const` квалификатора на **ToString** следует решить, необходимо изменить некоторые состояния объекта при его реализации. Однако следует каждый член функции const или не const, не оба сразу. Другими словами, не перегружать реализации функции на `const`.

Помимо реализации функции, другой других разместить где const вступает в изображение находится в проекции функции среды выполнения Windows. Рассмотрим этот код.
...
Для вызова **ToString** выше, команда **Перейти к объявлению** в Visual Studio показывает, что проекции среды выполнения Windows **IStringable::ToString** в C + +/ WinRT выглядит следующим образом.

```
winrt::hstring ToString() const;
```

Функции в проекции являются const независимо от того, как выбрать, какие реализацию их. Рендеринг проекции вызывает двоичный интерфейс приложения (ABI), какие суммы для вызова через указатель COM-интерфейса. Только состояние, в котором проекции **ToString** взаимодействует с — это указатель, COM-интерфейса; и Конечно не поддерживает не нужно изменить этот указатель, таким образом, чтобы функция const. Это дает гарантии, что оно не изменит ничего о **IStringable** ссылку, которую вызова через, и это гарантирует, что можно вызвать **ToString** даже с const ссылка на **IStringable**.

Понимать, что эти примеры `const` — подробные сведения о реализации C + +/ WinRT проекции и реализации; они составляют санации кода для свое преимущество. Такие понятия как не существует `const` на COM, ни ABI среды выполнения Windows (для функции-члена).
...
В мире среды выполнения Windows, компилятор не может кэшировать либо значение `c()` или интерфейсы для каждого метода, который будет вызываться через косвенного обращения ('. '). Если необходимо изменить, приводит к более виртуальные вызовы и служебных данных подсчета ссылок. Выше шаблон может легко создавать два раза больше кода, как строго. Вместо этого предпочитают приведенный ниже там, где вы можете шаблон. Он создает намного меньше кода, а также значительно повышает производительность время выполнения.
...
Рекомендуется приведенный выше шаблон применяется не только C + +/ WinRT, но все языковые проекции среды выполнения Windows.

> [!NOTE]
> Если в этом разделе не нашли ответы на вопросы, то может оказаться справки, перейдя к [сообществу разработчиков Visual Studio C++](https://developercommunity.visualstudio.com/spaces/62/index.html)или с помощью [ `c++-winrt` метки на Stack Overflow](https://stackoverflow.com/questions/tagged/c%2b%2b-winrt).
