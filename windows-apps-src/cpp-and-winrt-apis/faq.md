...
При объявлении свойства только для чтения в [MIDL 3.0](/uwp/midl-3/)вы ожидаете `cppwinrt.exe` средство для создания в реализации функции для вас, которое соответствует `const`-квалифицированных (const функция считает *это* указатель const).

Конечно, рекомендуется использовать const везде, где это возможно, но `cppwinrt.exe` сам инструмент не пытаться причина о том, какую реализацию функции вполне может быть const и который не может. Вы можете внести какие-либо из вашей реализации функции const, как в этом примере.
...
Функции, проекции являются const независимо от того, как вы решили, какие из них вашей реализации. Рендеринг проекции вызывает двоичный интерфейс приложения (ABI), какие суммы для вызова через указатель COM-интерфейса. Единственное состояние, которое проецируемого **ToString** взаимодействует с является этот указатель интерфейса COM; и Конечно не поддерживает не нужно изменять этот указатель, чтобы функция const. Это дает ссылка Software assurance, оно не изменит никаких сведений о **IStringable** ссылку, которую вы вызова через, что это гарантирует, что можно вызвать **ToString** даже с полем const **IStringable**.
