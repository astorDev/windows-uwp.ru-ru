...
Но что делать, если уничтожения экземпляра класса до завершения асинхронной операции? Существует множество возможностей, которые может проходить экземпляр класса за пределы области до завершения асинхронного метода. Но мы можете имитировать значение экземпляра класса `nullptr`.
...
После точки, где мы их уничтожения экземпляра класса выглядит так, будто не упоминаться непосредственно к нему еще раз. Но конечно *Этот* указатель к нему асинхронного объекта и пытается использовать, копировать значению, сохраненному в экземпляре класса. Сопрограммы функция-член, и он ожидает, что сможет использовать его *для этого* указателя с impunity.
...
## <a name="safely-accessing-the-this-pointer-with-an-event-handling-delegate"></a>Безопасного доступа к *Этот* указатель с помощью делегата обработки события
...
Предыдущий раздел выделенный потенциальных проблем с жизненным циклом в области сопрограммы с параллелизмом. Однако если обработки события с помощью функции-члена объекта, либо в лямбда-функции внутри функции-члена объекта, то вам необходимо подумать о относительный время существования получателя события (объект, обработав событие) и источник события (объект вызов события). Рассмотрим некоторые примеры кода.

Во-первых, пример кода ниже определяет простой **ИсточникСобытия** класс, который вызывает универсальный событие, которое обрабатывается любые делегаты, которые были добавлены к нему. Это пример событие происходит для использования типа делегата [**Windows::Foundation:: EventHandler**](/uwp/api/windows.foundation.eventhandler) , но проблем и способов их устранения ниже относятся к типам всех делегата.
...
Это следующей версии **Основная** функция имитирует что происходит при уничтожении получатель событий (возможно оно выходит за пределы области) во время на источник события — это по-прежнему создания событий.
...
Получатель событий удаляется, но лямбда-функции обработчика событий в ней по-прежнему подписан на событие **событий** . При возникновении этого события, лямбда-выражение пытается разыменования *Этот* указатель, который на данный момент является недопустимым. Таким образом, нарушение прав доступа в результате код в обработчике (или в продолжении сопрограммы, который) пытается использовать его.

> [!IMPORTANT]
> При возникновении такой ситуации, вам следует подумать о времени существования *данного* объекта. и ли записанный *Этот* объект время существования захвата. Если нет, записывайте его с помощью строгой или слабой ссылки, мы продемонстрируем ниже.
>
> Или&mdash;Если это имеет смысл для вашего сценария и рекомендации по позволяют даже&mdash;— вы можете отозвать обработчик после получатель завершил работу с событием, либо в деструкторе получателя. См. в разделе, [Отзыв зарегистрированного делегата](handle-events.md#revoke-a-registered-delegate).
...
В обоих случаях только что захват необработанных *Этот* указатель. И, не влияет на подсчет ссылок, поэтому ничего не позволяет текущего объекта от уничтожения.

### <a name="the-solution"></a>Решение

Решением является захват строгую ссылку. Строгую ссылку *делает* увеличить количество ссылок и оно *выполняет* сохранить текущий объект проверки активности. Просто объявите переменную захвата (под названием `strong_this` в этом примере) и инициализировать его с помощью вызова [**implements.get_strong**](/uwp/cpp-ref-for-winrt/implements#implementsgetstrong-function), который получает строгую ссылку на наших *Этот* указатель.
...
Даже можно опустить автоматической записи текущего объекта и обращаться захвата переменной вместо через неявный *Этот*элемент данных.
...
### <a name="if-you-use-a-member-function-as-a-delegate"></a>Если вы используете функцию-член представителем
