...
После точки, где мы уничтожить экземпляр класса выглядит так, будто не упоминаться непосредственно к нему еще раз. Но конечно *Этот* указатель к нему асинхронного объекта и пытается использовать, копировать значению, сохраненному в экземпляре класса. Сопрограммы функция-член, и он ожидает, что смогут использовать его *для этого* указателя с impunity.
...
В приведенном выше примере слабых ссылок не хранить экземпляр класса от уничтожения, когда строгой ссылок не останется. Однако он предоставляет способ проверки, можно ли получить строгую ссылку перед получением доступа к переменной-члена.
...
Даже можно опустить автоматического захват текущего объекта и обращаться захвата переменной вместо через неявное *Этот*элемент данных.
