...
Если сильная ссылка не подходит, то вместо этого вы можете вызвать [**implements::get_weak**](/uwp/cpp-ref-for-winrt/implements#implementsget_weak-function), чтобы получить слабую ссылку для *this*. Просто убедитесь, что можно все еще получить сильную ссылку перед доступом к элементам.
