...
3. Функция **Winrt::Windows::Foundation::IAsyncAction::get** заблокируется на несколько секунд, а затем возвращает результат **RetrieveValueAsync**.
4. **RetrieveValueAsync** возвращает значение `this->m_value`.

Шаг 4 безопасен, пока *this* допустимо.
