...
Таким образом другой вариант — явным образом запросить отмену из в вашем сопрограммы. Указывайте приведенный выше код в примере ниже. В этом примере новый **ExplicitCancellationAsync** получает объект, возвращенный функцией [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) и использует его для периодически проверять ли сопрограммы была отменена. До тех пор, пока не отменена, сопрограммы выполняется в бесконечном цикле; После отмены цикла и функции будут выхода обычным образом. Результат соответствует тому, как предыдущего примера, но здесь выхода из происходит явным образом и контроль.
