...
В этом разделе показано способами, которые можно выполнить оба Создание и использование асинхронного объектов среды выполнения Windows с помощью [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt).
...
Сопрограмма — это функция, как и любой другой, так как вызывающему объекту блокируется до момента функции выполнения к нему. И первой же возможности для сопрограммы для возврата — первый `co_await`, `co_return`, или `co_yield`.

Таким образом, перед передачей работы в сопрограмме, необходимо вернуть выполнение вызывающей стороне (приостановки сторону другими словами, создать) таким образом, чтобы заблокировать вызывающую. Если вы еще не делаете, `co_await`- применения другой операции, то можете `co_await` [**winrt::resume_background**](/uwp/cpp-ref-for-winrt/resume-background) функции. Это возвращает управление вызывающей стороне и затем сразу же возобновляет выполнение в потоке пула потоков.
...
Грубо говоря, после точки приостановки в сопрограмме, исходный поток выполнения может перейти от пользователя и возобновление могут возникать в любом потоке (другими словами, любой поток может вызвать метод **Completed** для асинхронной операции).

Однако, если вы `co_await` какие-либо из четырех типов среды выполнения Windows асинхронной операции (**IAsyncXxx**), затем C + +/ WinRT перехватывает контекст вызова в точке вы `co_await`. И это гарантирует, что вы по-прежнему на этом контексте при возобновлении задача-продолжение. C + +/ WinRT делает это, проверка, является ли вы уже на контекст вызова и, если это не так, переход к нему. Если вы решите потоке однопотоковом подразделении (STA), прежде чем `co_await`, а затем вы сможете на том же впоследствии; Если вы решите потоке многопотоковом подразделении (MTA), прежде чем `co_await`, а затем вы сможете на одном впоследствии.
...
Причина, по можно полагаться на это поведение заключается C + +/ WinRT предоставляет код для адаптации этих типов асинхронной операции среды выполнения Windows для поддержки языка сопрограммы C++ (эти фрагменты кода называются адаптеры ожидания). Оставшиеся ожидание типы в C + +/ WinRT — это просто программы-оболочки пула потоков и/или облегчающих; Поэтому он выполнить в пул потоков.
...
Если вы `co_await` другой тип&mdash;даже в C + +/ WinRT сопрограммы реализации&mdash;другой библиотека содержит адаптеров, а вам необходимо понять, выполните эти адаптеры возобновлением и контекстах.

Чтобы сохранить переключения контекста до как минимум, можно использовать некоторых методов, которые уже были в этом разделе. Давайте посмотрим, выполнив некоторые иллюстрации. В следующем примере псевдокода мы покажем контура обработчик событий, который вызывает API среды выполнения Windows для загрузки изображения, опускается на фоновый поток для обработки этого изображения, а затем возвращает в поток пользовательского интерфейса, чтобы показать изображения в пользовательском Интерфейсе.
...
В этом сценарии существует навыков ineffiency вокруг вызова **StorageFile::OpenAsync**. Нет необходимости контекстное переключение фона потоков (так, что обработчик можно вернуть выполнение вызывающей стороне), на возобновлением после какие C + +/ WinRT восстанавливает контекст потока пользовательского интерфейса. Но в этом случае нет необходимости быть в потоке пользовательского интерфейса, пока мы собираемся обновления пользовательского интерфейса. Дополнительные API среды выполнения Windows, мы вызываем метод *перед* наш вызов к **winrt::resume_background**более ненужные переключение вперед и назад контекста, которые мы взиматься. Решением является не перед затем вызвать *любой* API среды выполнения Windows. Переместите их все после **winrt::resume_background**.
...
Если вы хотите сделать что-то более сложных, а затем можно написать собственный await адаптеров. Например, если вы хотите, чтобы `co_await` для возобновления в том же потоке, которая async действие выполняется на (таким образом, нет без переключения контекста), то можно начать с написания await адаптеры, как показано ниже те.

> [!NOTE]
> В примере кода ниже предоставляется только; для образовательных целей Это для начала Общие сведения о том, как await рабочих адаптеров. Если вы хотите использовать метод, описанный в собственные базы кода, а затем мы рекомендуем Вам разрабатывать и тестировать собственные await struct(s) адаптер. Например можно написать **complete_on_any**, **complete_on_current**и **complete_on(dispatcher)**. Также учитывать, делая их шаблоны, которые принимают тип **IAsyncXxx** в качестве параметра шаблона.
...
Для использования **без_переключения** await адаптеры, просто измените тип, `co_await` выражение из **IAsyncXxx** **без_переключения**, следующим образом.
...
Компоненты среды выполнения Windows для асинхронное программирование позволяют отменить переадресации асинхронные действия или операции. Ниже приводится пример, который вызывает [**StorageFolder::GetFilesAsync**](/uwp/api/windows.storage.storagefolder.getfilesasync) для получения коллекции потенциально больших файлов, и он сохраняет полученный объект асинхронной операции в элементе данных. Пользователь имеет возможность отменить операцию.
...
Для реализации части отмены давайте начнем с простого примера.
...
При повторном запуске в примере выше, то увидите **ImplicitCancellationAsync** печати одно сообщение в секунду в течение трех секунд, после чего время автоматически завершает работу в результате отмены. Это работает, поскольку, на которых успешно выполняется действие `co_await` выражение, сопрограммы проверяет, является ли он был отменен. Если он достигнут, затем он игнорирует out; и если нет, затем она приостанавливает в обычном режиме.

Отмена Конечно, возможно, во время приостановки сопрограммы. Только в том случае, когда возобновления сопрограммы, или обращений к другой `co_await`, будет проверять для отмены. Проблемы — это один из потенциально слишком грубым точное задержка реагирования на отмену.

Таким образом другой вариант — явным образом запросить отмену из внутри ваша сопрограмма. Обновите приведенном выше примере с кодом в описании ниже. В этом примере новый **ExplicitCancellationAsync** получает объект, возвращенный функцией [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) и использует его для периодической проверки ли сопрограммы была отменена. До тех пор, пока не отменена, сопрограммы выполняется в бесконечном цикле; После отменяется, цикл и функции будут выхода обычным образом. Результат соответствует тому, как предыдущего примера, но здесь выхода из происходит явным образом и контроль.
...
Ожидание **winrt::get_cancellation_token** извлекает токен отмены изучения **IAsyncAction** , которая выдает сопрограммы от вашего имени. Оператора вызова функции на этот маркер можно использовать для запроса состояние отмены&mdash;по сути опрос для отмены. При выполнении определенных операций передачей или итерацию большой коллекции, то это разумный метод.

### <a name="register-a-cancellation-callback"></a>Зарегистрировать обратный вызов отмены

Отмена среды выполнения Windows не затрагивают автоматически другие асинхронные объекты. Но&mdash;появились в Windows SDK версии 10.0.17763.0 (Windows 10, версия 1809)&mdash;вы можете зарегистрировать обратный вызов отмены. Это вытеснением заинтересовать клиента, который может распространяться отмены и делает возможным для интеграции с существующих библиотек параллелизма.

В следующем примере кода выполняющий **NestedCoroutineAsync** , но не логики специальные отмены него есть в его. **CancellationPropagatorAsync** является по сути оболочкой на вложенных сопрограммы; оболочка pre-emptively пересылает отмены.
...
## <a name="reporting-progress"></a>Информирования о ходе выполнения

Если ваша сопрограмма возвращает [**IAsyncActionWithProgress**](/uwp/api/windows.foundation.iasyncactionwithprogress_tprogress_)или [**IAsyncOperationWithProgress**](/uwp/api/windows.foundation.iasyncoperationwithprogress_tresult_tprogress_), затем можно получить объект, возвращенный функцию [**winrt::get_progress_token**](/uwp/cpp-ref-for-winrt/get-progress-token) и использовать его для сообщения о ходе выполнения обратно на ход выполнения обработчик. Приведем пример кода.
...
> [!NOTE]
> Не является правильным для реализации более одного *обработчик завершения* асинхронные действия или операции. У вас есть только один делегат для события завершения, или вы можете `co_await` его. Если у вас есть оба, второй завершится ошибкой. Либо один из следующих двух видов обработчики завершения подходит; не оба сразу же async объекта.
...
Иногда имеется задачу, которая может выполняться параллельно другую работу, и вам не нужно ждать завершения этой задачи (никакой другой работы зависит от него), и вам требуется его, чтобы возвращать значение. В этом случае можно запускать задачу и его забудете. Для этого путем написания сопрограммы, чей тип возвращаемого значения является [**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget) (вместо одного из типов асинхронной операции среды выполнения Windows, или **concurrency::task**).
