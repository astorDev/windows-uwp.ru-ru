...
В этом разделе показано создание и использование асинхронных объектов среды выполнения Windows с помощью способа, которые можно выполнить оба [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt).
...
Таким образом, перед передачей работы в сопрограмме, необходимо вернуть выполнение вызывающей стороне (приостановки сторону другими словами, создать), чтобы заблокировать вызывающую. Если вы еще не делаете, `co_await`- применения другой операции, то можете `co_await` функцию [**winrt::resume_background**](/uwp/cpp-ref-for-winrt/resume-background) . Это возвращает управление вызывающей стороне и затем сразу же возобновляет выполнение в потоке пула потоков.
...
Грубо говоря, после приостановки точки в сопрограмме, исходный поток выполнения может перейти от пользователя и возобновление могут возникать в любом потоке (другими словами, любой поток может вызывать метод **Completed** для асинхронной операции).

Однако, если вы `co_await` какие-либо из четырех типов среды выполнения Windows асинхронной операции (**IAsyncXxx**), затем C + +/ WinRT перехватывает контекст вызова в момент вы `co_await`. И это гарантирует, что вы по-прежнему в этом контексте при возобновлении задача-продолжение. C + +/ WinRT осуществляется путем проверки ли вы уже на контекст вызова и, если это не так, переход к нему. Если вы решите потоке однопотоковом подразделении (STA), прежде чем `co_await`, а затем можно на том же впоследствии; Если вы решите потоке многопотоковом подразделении (MTA), прежде чем `co_await`, то можно на одном позже.
...
Причины можно полагаться на это поведение, так как C + +/ WinRT предоставляет код для адаптации этих типов асинхронной операции среды выполнения Windows для поддержки языка сопрограммы C++ (эти фрагменты кода называются адаптеры ожидания). Ожидание остальных в C + +/ WinRT — это просто программы-оболочки пула потоков и/или облегчающих; Поэтому он выполнить в пул потоков.
...
Если вы `co_await` другой тип&mdash;даже в C + +/ WinRT сопрограммы реализации&mdash;другой библиотека содержит адаптеров, и вам необходимо понять, возобновление и контексты предназначение эти адаптеры.

Чтобы сохранить переключения контекста до как минимум, можно использовать некоторых методов, которые уже были в этой статье. Рассмотрим некоторые иллюстрациях действий. В следующем примере псевдокода мы покажем краткие сведения о обработчик событий, который вызывает API-Интерфейс среды выполнения Windows для загрузки изображения, опускается на фоновый поток для обработки этого изображения, а затем возвращает в поток пользовательского интерфейса, чтобы показать изображения в пользовательском Интерфейсе.
...
В этом сценарии существует немного ineffiency вокруг вызова **StorageFile::OpenAsync**. Нет необходимости контекстное переключение фона поток (таким образом, обработчик можно вернуть выполнение вызывающей стороне), на возобновлением после какие C + +/ WinRT восстанавливает контекст потока пользовательского интерфейса. Однако в этом случае нет необходимости быть в потоке пользовательского интерфейса, пока мы все обновления пользовательского интерфейса. Дополнительные API среды выполнения Windows, мы вызываем метод *перед* наш вызов **winrt::resume_background**, более ненужные вперед и назад контекст параметров, которые мы взиматься. Решением является не до этого вызвать *любой* API среды выполнения Windows. Переместите их все после **winrt::resume_background**.
...
Если вы хотите сделать что-то более сложными, то можно написать собственный await адаптеров. Например, если вы хотите, чтобы `co_await` для возобновления в том же потоке, который выполнения асинхронного действия на (таким образом, нет без переключения контекста), то можно начать с написания await адаптеры, подобных тем ниже.

> [!NOTE]
> В примере кода ниже предоставляется только; для образовательных целей Это для начала Общие сведения о том, как await рабочих адаптеров. Если вы хотите использовать метод, описанный в собственный базу кода, рекомендуется разрабатывать и тестировать собственные await struct(s) адаптер. Например можно написать **complete_on_any**, **complete_on_current**и **complete_on(dispatcher)**. Также учитывать, что делает их шаблоны, которые принимают тип **IAsyncXxx** в качестве параметра шаблона.
...
Компоненты среды выполнения Windows для асинхронного программирования дают возможность отменить переадресации асинхронные действия или операции. Ниже приводится пример, который вызывает [**StorageFolder::GetFilesAsync**](/uwp/api/windows.storage.storagefolder.getfilesasync) для получения потенциально большое количество файлов, и сохраняется полученный объект асинхронной операции в элемента данных. Пользователь имеет возможность отменить операцию.
...
При запуске в примере выше, то увидите **ImplicitCancellationAsync** печати одно сообщение в секунду в течение трех секунд, после чего время автоматически завершает работу в результате отмены. Это работает, поскольку, на которых успешно выполняется действие `co_await` выражении сопрограммы проверяет ли она была отменена. Если Да, затем он игнорирует и если нет, затем она приостанавливает в обычном режиме.

Отмена Конечно, возможно, во время приостановки сопрограммы. Только в том случае, если возобновления сопрограммы, или обращений к другой `co_await`, будет проверять наличие отмены. Проблемы — это один из потенциально слишком грубым точное задержка реагирования на отмену.

Таким образом другой вариант — явным образом запросить отмену из в вашем сопрограммы. Указывайте приведенный выше код в примере ниже. В этом примере новый **ExplicitCancellationAsync** получает объект, возвращенный функцией [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) и использует его для периодической проверки ли сопрограммы была отменена. До тех пор, пока он не отменена, сопрограммы выполняется в бесконечном цикле; После отмены цикла и функции будут работу обычным образом. Результат соответствует тому, как предыдущего примера, но здесь выхода из происходит явным образом и полный контроль.
...
**CancellationPropagatorAsync** регистрирует лямбда-функции для собственной отмены обратного вызова, а затем он ждет (она приостанавливает) до завершения работы вложенной. Когда или **CancellationPropagatorAsync** отменяется, его можно распространять об отмене вложенных сопрограммы. Нет необходимости выполнять опрос для отмены; ни отмены блокируется неограниченное количество раз. Этот механизм отличается достаточной гибкостью для использования для взаимодействия с библиотекой сопрограммы или параллелизма, ничего не знает о C + +/ WinRT.

## <a name="reporting-progress"></a>Отчеты о ходе выполнения

Если ваша сопрограмма возвращает [**IAsyncActionWithProgress**](/uwp/api/windows.foundation.iasyncactionwithprogress_tprogress_), либо [**IAsyncOperationWithProgress**](/uwp/api/windows.foundation.iasyncoperationwithprogress_tresult_tprogress_), затем можно получить объект, возвращенный функцией [**winrt::get_progress_token**](/uwp/cpp-ref-for-winrt/get-progress-token) и использовать его для сообщения о ходе выполнения обратно на ход выполнения обработчик. Приведем пример кода.
...
> [!NOTE]
> Это не так для реализации более одного *обработчик завершения* асинхронные действия или операции. У вас есть только один делегат для события завершения, или вы можете `co_await` его. Если у вас есть оба, второй завершится ошибкой. Либо один из следующих двух видов обработчики завершения подходит; не оба сразу же async объекта.
...
В некоторых случаях у вас есть задачу, которая может выполняться параллельно другую работу, и вам не нужно ждать завершения этой задачи (никакой другой работы зависит от него), и вам требуется вернуть значение. В этом случае можно запускать задачу и его забудете. Что можно сделать путем написания сопрограммы, возвращаемый тип является [**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget) (вместо одного из типов асинхронной операции среды выполнения Windows, или **concurrency::task**).
