...
Однако, если вы `co_await` какие-либо из четырех типов среды выполнения Windows асинхронной операции (**IAsyncXxx**), затем C + +/ WinRT перехватывает контекст вызова в точке вы `co_await`. И это гарантирует, что вы еще этого контекста при возобновлении задача-продолжение. C + +/ WinRT делает это, проверка, является ли вы уже на контекст вызова и, если это не так, переход к нему. Если вы решите потоке однопотоковом подразделении (STA), прежде чем `co_await`, а затем можно на том же впоследствии; Если вы решите потоке многопотоковом подразделении (MTA), прежде чем `co_await`, то можно на одном впоследствии.
