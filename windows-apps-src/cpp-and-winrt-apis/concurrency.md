...
В этом разделе показаны способы, с помощью которых можно выполнить оба Создание и использование асинхронных объектов среды выполнения Windows с помощью [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt).
...
> [!NOTE]
> Функция **get** существует на C + +/ проекции WinRT введите **winrt::Windows::Foundation::IAsyncAction**, поэтому можно вызывать функцию в C + +/ WinRT проекта. Не позволит найти функцию в списке является членом интерфейса [**IAsyncAction**](/uwp/api/windows.foundation.iasyncaction) , так как **получить** не является частью поверхности приложения двоичный интерфейс (ABI) фактического типа среды выполнения Windows **IAsyncAction**.
...
При запуске в примере выше, то увидите **ImplicitCancellationAsync** печати одно сообщение в секунду в течение трех секунд, после чего время автоматически завершает работу в результате отмены. Это работает, поскольку, на которых успешно выполняется действие `co_await` выражение, сопрограммы проверяет, является ли он был отменен. Если Да, затем он игнорирует и если нет, затем она приостанавливает в обычном режиме.

Отмена Конечно, возможно, во время приостановки сопрограммы. Только в том случае, когда возобновления сопрограммы или другой `co_await`, будет проверять для отмены. Проблема — это один из потенциально слишком грубым точное задержка реагирования на отмену.

Таким образом другой вариант — явным образом запросить отмену из в вашем сопрограммы. Указывайте приведенном выше примере кода в примере ниже. В этом примере новый **ExplicitCancellationAsync** получает объект, возвращенный функцией [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) и использует его для периодической проверки ли сопрограммы была отменена. До тех пор, пока не отменена сопрограммы выполняется в бесконечном цикле; После отмены цикл и функции будут работу обычным образом. Результат — так же, как предыдущего примера, но здесь выхода из происходит явным образом и полный контроль.
...
**CancellationPropagatorAsync** регистрирует лямбда-функции для собственной отмены обратного вызова, а затем он ждет (приостанавливает) до завершения работы вложенной. Когда или **CancellationPropagatorAsync** отменяется, его можно распространять об отмене вложенных сопрограммы. Нет необходимости выполнять опрос для отмены; ни отмены блокируется неограниченное количество раз. Этот механизм отличается достаточной гибкостью, можно использовать для взаимодействия с библиотекой сопрограммы или параллелизма, ничего не знает о C + +/ WinRT.
