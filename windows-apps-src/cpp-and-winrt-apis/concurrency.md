...
> [!NOTE]
> Функция **get** существует на C + +/ проекции WinRT типа **winrt::Windows::Foundation::IAsyncAction**, поэтому можно вызывать функцию в C + +/ WinRT проекта. Вы найдете функции, перечисленные в качестве интерфейса [**IAsyncAction**](/uwp/api/windows.foundation.iasyncaction) , не так, как **получить** не является частью поверхности приложения двоичный интерфейс (ABI) фактического типа среды выполнения Windows **IAsyncAction**.
...
Компоненты среды выполнения Windows для асинхронного программирования позволяет отменить выполняемый асинхронные действия или операции. Начнем с простой пример.
...
При повторном запуске в примере выше, то увидите **ImplicitCancellationAsync** печати одно сообщение в секунду в течение трех секунд, после чего время автоматически завершает работу в результате отмены. Это работает, поскольку, на которых успешно выполняется действие `co_await` выражение, сопрограммы проверяет ли она была отменена. Если было, затем он игнорирует и если нет, затем она приостанавливает в обычном режиме.

Отмена Конечно, возможно, во время приостановки сопрограммы. Только в том случае, когда возобновления сопрограммы, или другой `co_await`, будет проверять для отмены. Проблемы — это один из потенциально слишком грубым точное задержка реагирования на отмену.

Таким образом другой вариант — явным образом запросить отмену из в вашем сопрограммы. Указывайте приведенный выше код в списке ниже. В этом примере новый **ExplicitCancellationAsync** возвращает объект, возвращенный функцию [**winrt::get_cancellation_token**](/uwp/cpp-ref-for-winrt/get-cancellation-token) и использует его для периодической проверки ли сопрограммы была отменена. До тех пор, пока не отменена сопрограммы выполняется в бесконечном цикле; После отмены цикла и функции будут выхода обычным образом. Результат соответствует тому, как предыдущего примера, но здесь выхода из происходит явным образом и контроль.
...
Ожидание **winrt::get_cancellation_token** извлекает токен отмены изучения **IAsyncAction** , которая выдает сопрограммы от вашего имени. Оператора вызова функции на этот маркер можно использовать для запроса состояние отмены&mdash;по сути опрос для отмены. При выполнении определенных операций передачей или выполните итерацию большой коллекции, то это разумный метод.

### <a name="register-a-cancellation-callback"></a>Зарегистрировать обратный вызов отмены
Отмены среды выполнения Windows не затрагивают автоматически другие асинхронные объекты. Но&mdash;в версии 10.0.17763.0 (Windows 10, версия 1809) пакета Windows SDK&mdash;вы можете зарегистрировать обратный вызов отмены. Это вытеснением заинтересовать клиента, который может распространяться отмены и делает возможным для интеграции с существующей библиотеки параллелизма.

В следующем примере кода выполняющий **NestedCoroutineAsync** , но него есть не логики специальные отмены в его. **CancellationPropagatorAsync** является по сути оболочкой для вложенных сопрограммы; оболочка pre-emptively перенаправляет отмены.
...
**CancellationPropagatorAsync** регистрирует лямбда-функции обратного вызова собственный отмены, а затем он ждет (приостанавливает) до завершения работы вложенной. Когда или **CancellationPropagatorAsync** отменяется, он распространяет об отмене вложенных сопрограммы. Нет необходимости выполнять опрос для отмены; ни отмены блокируется неограниченное количество раз. Этот механизм отличается достаточной гибкостью, можно использовать его для взаимодействия с библиотекой сопрограммы или блокировки, ничего не знает о C + +/ WinRT.
...
Иногда у вас есть задачи, которая может выполняться параллельно другую работу, и вам не нужно ждать завершения этой задачи (нет других заданий зависит от него), и вам требуется вернуть значение. В этом случае можно запускать задачу и его забудете. Для этого путем написания сопрограммы, чей тип возвращаемого значения является [**winrt::fire_and_forget**](/uwp/cpp-ref-for-winrt/fire-and-forget) (вместо одного из типов асинхронной операции среды выполнения Windows, или **concurrency::task**).
