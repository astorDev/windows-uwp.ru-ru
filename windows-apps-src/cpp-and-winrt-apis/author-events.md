...
Если события используется только для внутреннего использования в C + +/ WinRT проекта (а не в разных двоичных файлах), а затем по-прежнему использовать шаблон структуры [**winrt::event**](/uwp/cpp-ref-for-winrt/event) , но его параметризации с помощью C + +/ WinRT Windows Runtime [**winrt::delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) шаблон структуры, являющийся делегат эффективным, подсчетом ссылок. Он поддерживает любое число параметров, и они не ограничиваются типы среды выполнения Windows.

Во-первых, в примере ниже показано делегата подписи, которую не принимает никаких параметров (по сути сигнал простой), а затем тот, который принимает строку.
...
Обратите внимание на то, как добавить к событию столько мире делегаты по своему усмотрению. Но есть некоторые издержки, связанные с событием. Если все, что необходимо простой обратный вызов с помощью только один мире делегата, то вы можете использовать [**winrt::delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) самостоятельно.
