...
Если события используется только для внутреннего использования в C + +/ WinRT проекта (а не в разных двоичных файлах), а затем по-прежнему использовать шаблон структуры [**winrt::event**](/uwp/cpp-ref-for-winrt/event) , но он параметризации с помощью C + +/ WinRT Windows Runtime [**winrt::delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) шаблон структуры, который является эффективным, подсчетом ссылок делегата. Он поддерживает любое число параметров, и они не ограничены только типы среды выполнения Windows.
...
Мы рекомендуем пройти события и делегаты, в качестве параметров функции. **Добавить** функция [**winrt::event**](/uwp/cpp-ref-for-winrt/event) — одно исключение, так как в этом случае необходимо передать делегат. Причина Эта рекомендация связано делегаты выполнить различные формы для разных языков среды выполнения Windows (точки зрения ли они поддерживают регистрации клиента один или несколько). События, с помощью их несколько модель подписчиков составляют гораздо более прогнозируемым и согласованным параметр.
