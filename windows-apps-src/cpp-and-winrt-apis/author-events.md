...
Если события используется только для внутреннего использования в C + +/ WinRT проекта (а не в разных двоичных файлах), а затем по-прежнему использовать шаблон структуры [**winrt::event**](/uwp/cpp-ref-for-winrt/event) , но его параметризации с помощью C + +/ WinRT Windows Runtime [**winrt::delegate&lt;… T&gt; **](/uwp/cpp-ref-for-winrt/delegate) структура шаблона, который является эффективным, подсчетом ссылок делегата. Он поддерживает любое число параметров, и они не ограничиваются типы среды выполнения Windows.
