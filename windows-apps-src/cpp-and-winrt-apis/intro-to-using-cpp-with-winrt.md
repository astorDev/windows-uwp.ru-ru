...
C + +/ WinRT — Это рекомендуемая корпорацией Майкрософт замена для [C + +/ CX](/cpp/cppcx/visual-c-language-reference-c-cx?branch=live) языковой проекции и [Библиотеки шаблонов C++ (WRL) среды выполнения Windows](/cpp/windows/windows-runtime-cpp-template-library-wrl?branch=live). Полный список [разделы, посвященные C + +/ WinRT](index.md#topics-about-cwinrt) представлены сведения о взаимодействии с, а также перенос из C + +/ CX и WRL.
