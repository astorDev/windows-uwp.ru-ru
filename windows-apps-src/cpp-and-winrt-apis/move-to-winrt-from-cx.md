...
ms.locfileid: "8211188"
---
# <a name="move-to-cwinrt-from-ccx"></a>Переход на C++/WinRT с C++/CX

В этом разделе показано, как перенести код в [C + +/ CX](/cpp/cppcx/visual-c-language-reference-c-cx) проекта в его эквивалент на [C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt).

## <a name="porting-strategies"></a>Перенос стратегии

Если вы хотите перенести постепенно C + +/ CX до кода C + +/ WinRT, то можете. C + +/ CX и C + +/ WinRT кода могут сосуществовать в одном проекте, за исключением поддержку компилятора XAML и компоненты среды выполнения Windows. Для этих двух исключений, вам потребуется выбрать любой из C + +/ CX или C + +/ WinRT в том же проекте.

> [!IMPORTANT]
> Если ваш проект собирается приложения XAML, то одного рабочего процесса, который мы рекомендуем будет сначала создать новый проект в Visual Studio, с помощью одного из C + +/ WinRT шаблонов проекта (см. в разделе [Поддержка Visual Studio для C + +/ WinRT и VSIX](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-and-the-vsix)). Затем запустите копирования исходного кода и разметки из C + +/ CX проекта. Вы можете добавить новые страницы XAML с **проектом** \> **Добавить новый элемент**  \>  **Visual C++** > **пустая страница (C + +/ WinRT)**.
>
> Кроме того, можно использовать компонент среды выполнения Windows на коэффициент код из XAML C + +/ CX проекта, так как его переноса. Либо переместите столько C + +/ CX кода, как и в компоненте и затем измените проект XAML с C + +/ WinRT. Или в противном случае оставьте проектов XAML как C + +/ CX, создайте новый C + +/ компонента WinRT и переносом C + +/ CX код из проектов XAML в компоненте. Также возможны C + +/ CX проект компонента вместе с C + +/ WinRT проект компонента в одном решении оба ссылки из проекта приложения и постепенно порта от одного к другому. См. в разделе [взаимодействие между C + +/ WinRT и C + +/ CX](interop-winrt-cx.md) Дополнительные сведения об использовании эти две языковых проекции в том же проекте.
...
## <a name="converting-from-a-base-runtime-class-to-a-derived-one"></a>Преобразование из класса среды выполнения базовых извлеченных из них
Это обычно существует ссылка для базового, вы знаете, что ссылается на объект производного типа. В C + +/ CX, используйте `dynamic_cast` к *приведите* ссылку для базового в ссылки к производным. `dynamic_cast` Лишь это скрытого вызов [**QueryInterface**](https://msdn.microsoft.com/library/windows/desktop/ms682521). Ниже приведен стандартный пример&mdash;обрабатывать событие изменения свойства зависимостей, и вы хотите приводится от **DependencyObject** обратно к фактического типа, которому принадлежит свойство зависимостей.
...
Эквивалент C + +/ WinRT в коде заменяется `dynamic_cast` с помощью вызова функции [**IUnknown::try_as**](/uwp/cpp-ref-for-winrt/windows-foundation-iunknown#iunknowntryas-function) , который инкапсулирует **QueryInterface**. Также имеется возможность вместо этого вызовите метод [**IUnknown::as**](/uwp/cpp-ref-for-winrt/windows-foundation-iunknown#iunknownas-function), который создает исключение, если запрашиваются необходимый интерфейс (интерфейс по умолчанию для типа, который запрашивается) не возвращается. Вот C + +/ WinRT пример кода.
