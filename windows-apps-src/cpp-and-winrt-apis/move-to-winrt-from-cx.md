...
Если вы хотите перенести постепенно C + +/ CX до кода C + +/ WinRT, то можете. C + +/ CX и C + +/ WinRT кода могут сосуществовать в одном проекте, за исключением поддержку компилятора XAML и компоненты среды выполнения Windows. Для этих двух исключений, необходимо выбрать либо C + +/ CX или C + +/ WinRT в том же проекте.

> [!IMPORTANT]
> Если ваш проект собирается приложения XAML, то одного рабочего процесса, который мы рекомендуем будет сначала создать новый проект в Visual Studio, с помощью одного из C + +/ WinRT шаблонов проекта (см. в разделе [Поддержка Visual Studio для C + +/ WinRT](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)). Затем запустите копирования исходного кода и разметки из C + +/ CX проекта. Вы можете добавить новые страницы XAML с **проектом** \> **Добавить новый элемент**  \>  **Visual C++** > **пустая страница (C + +/ WinRT)**.
>
> Кроме того, можно использовать компонент среды выполнения Windows на коэффициент код из XAML C + +/ CX проекта, так как его переноса. Перенесите столько C + +/ CX кода, как и в компоненте и затем измените проект XAML с C + +/ WinRT. Или в противном случае оставьте проект XAML как C + +/ CX, создайте новый C + +/ компонента WinRT и переносом C + +/ CX код из проектов XAML в компоненте. Также возможны C + +/ CX проект компонента вместе с C + +/ WinRT проект компонента в одном решении, оба из них ссылки из проекта приложения и постепенно порта от одного к другому. См. в разделе [взаимодействие между C + +/ WinRT и C + +/ CX](interop-winrt-cx.md) Дополнительные сведения об использовании эти две языковых проекции в одном проекте.
...
| **Платформа:: Array\ ^** | См. в разделе [порт **платформы:: Array\ ^** ](#port-platformarray) |
...
### <a name="port-platformarray"></a>Порт **платформы:: Array\ ^**
Возможности включают в себя использование список инициализатора, **std::array**или **std::vector**. Дополнительные сведения и примеры кода см. в разделе [перечислены стандартные инициализатора](/windows/uwp/cpp-and-winrt-apis/std-cpp-data-types#standard-initializer-lists) и [Стандартные массивы и векторы](/windows/uwp/cpp-and-winrt-apis/std-cpp-data-types#standard-arrays-and-vectors).
