...
> [!IMPORTANT]
> Если ваш проект собирается приложения XAML, то один рабочий процесс, который мы рекомендуем будет сначала создать новый проект в Visual Studio, с помощью одного из C + +/ WinRT шаблонов проекта (см. в разделе [Поддержка Visual Studio для C + +/ WinRT и VSIX](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-and-the-vsix)). Затем запустите копирования исходный код и разметку из C + +/ CX проекта. Вы можете добавить новые страницы XAML с **проектом** \> **Добавить новый элемент**  \>  **Visual C++** > **пустая страница (C + +/ WinRT)**.
>
> Кроме того, можно использовать компонент среды выполнения Windows на коэффициент код из XAML C + +/ CX проекта, так как его переноса. Либо переместите ограничен C + +/ CX кода можно в компоненте, а затем измените проект XAML с C + +/ WinRT. Или в противном случае оставьте проект XAML как C + +/ CX, создайте новый C + +/ компонента WinRT и переносом C + +/ CX кода из проекта XAML и компонента. Также возможны C + +/ CX проект компонента вместе с C + +/ WinRT проект компонента, в том же решении оба ссылки из проекта приложения и постепенно порта от одного к другому. См. в разделе [взаимодействие между C + +/ WinRT и C + +/ CX](interop-winrt-cx.md) Дополнительные сведения об использовании эти две языковых проекции в одном проекте.
