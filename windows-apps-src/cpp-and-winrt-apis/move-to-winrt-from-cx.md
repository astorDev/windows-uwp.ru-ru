...
При переносе в эквивалентный C + +/ WinRT кода, по сути вы удаляете циркумфлексы и изменяете оператор косвенного (-&gt;) оператор-точку (.), так как C + +/ WinRT проецируемых типов являются значениями, а не указателями.
