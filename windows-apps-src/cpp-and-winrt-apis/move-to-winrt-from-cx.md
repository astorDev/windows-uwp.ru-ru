...
Если вы хотите перенести постепенно C + +/ CX кода на C + +/ WinRT, попробуйте выполнить. C + +/ CX и C + +/ WinRT кода могут сосуществовать в одном проекте, за исключением поддержки компилятора XAML и компоненты среды выполнения Windows. Для этих двух исключений, вам будет нужно выбрать C + +/ CX или C + +/ WinRT в том же проекте.

> [!IMPORTANT]
> Если проект создается в приложении XAML, один рабочий процесс, мы рекомендуем, то сначала создать новый проект в Visual Studio, с помощью одного из C + +/ WinRT шаблоны проектов (см. в разделе [поддержка Visual Studio для C + +/ WinRT](intro-to-using-cpp-with-winrt.md#visual-studio-support-for-cwinrt-xaml-the-vsix-extension-and-the-nuget-package)). Затем, с которого начинается копирование исходного кода и разметки над C + +/ CX проекта. Вы можете добавить новые страницы XAML с **проекта** \> **добавить новый элемент...** \> **Visual C++** > **пустая страница (C + +/ WinRT)**.
>
> Кроме того, можно использовать в компонент среды выполнения Windows для идентификации кода из XAML C + +/ CX проекта как портировать его. Либо переместите столько C + +/ CX кода, как и в компонент и затем изменим проект XAML к C + +/ WinRT. Или в противном случае оставьте проект XAML как C + +/ CX, создайте новый C + +/ компонент WinRT и начать перенос C + +/ CX код из проекта XAML в компонент. Также имеется C + +/ CX проект компонента вместе с C + +/ WinRT проект компонента, в том же решении, ссылаться на них из проекта приложения и постепенно порта от одного к другому. См. в разделе [взаимодействия между C + +/ WinRT и C + +/ CX](interop-winrt-cx.md) Дополнительные сведения об использовании двух языковых проекций в том же проекте.
...
Для написания C + +/ CX исходного кода, можно передать C + +/ CX типов как параметры функции как hat (\^) ссылки.
...
Для написания C + +/ CX исходного кода, используйте hat (\^) переменные для ссылки на объекты среды выполнения Windows и стрелку (-&gt;) оператор для разыменования hat переменной.
...
При переносе кода в эквивалентные C + +/ WinRT кода, можно получить долгий путь, удалив шляпы и изменение оператора стрелки (-&gt;) для оператора точки (.). C + +/ WinRT проецировать типы значений, а не указатель.
...
Конструктор по умолчанию для C + +/ CX hat указатель ей присваивается значение null. Вот C + +/ CX пример кода, в котором мы создадим переменную или поле правильного типа, но тот, который не инициализирована. Другими словами, он не относится изначально **TextBlock**; мы намерены ссылка будет назначить позже.
...
Для его эквивалента в C + +/ WinRT, см. в разделе [Delayed инициализации](consume-apis.md#delayed-initialization).
...
**Platform::Agile\^**  тип в C + +/ CX представляет класс среды выполнения Windows, который может осуществляться из любого потока. C + +/ WinRT эквивалентом является [ **winrt::agile_ref**](/uwp/cpp-ref-for-winrt/agile-ref).
...
**Platform::Exception\^**  типа создается в C + +/ CX, возвращаемые API среды выполнения Windows не S\_HRESULT ОК. Для C++/WinRT эквивалентом является [**winrt::hresult_error**](/uwp/cpp-ref-for-winrt/error-handling/hresult-error).

Порт для C + +/ WinRT, изменить любой код, использующий **Platform::Exception\^**  использовать **winrt::hresult_error**.
...
С использованием C + +/ CX, вы можете получить доступ к [ **Platform::String::Data** ](https://docs.microsoft.com/en-us/cpp/cppcx/platform-string-class#data) свойство для извлечения строки в стиле языка C **const wchar_t\***  массива (например, для передачи Чтобы **std::wcout**).
...
Когда дело доходит до реализации интерфейсов API, который принимает или возвращает строки, то обычно изменяете C + +/ CX кода, использующего **Platform::String\^**  использовать **winrt::hstring** вместо этого.
...
C + +/ CX предоставляет [Object::ToString](/cpp/cppcx/platform-object-class?view=vs-2017#tostring) метод.
...
* [Переместить в C + +/ WinRT из WRL](move-to-winrt-from-wrl.md)
