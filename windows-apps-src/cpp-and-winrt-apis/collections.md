...
Можно создать коллекцию среды выполнения Windows из данных и получить представление на нем готов к передаче API, без копирования никаких действий.
...
«Однопотоковом» в имена этих функций означает, что они не предоставляют все параллелизма&mdash;другими словами, они не являются потокобезопасными. Упоминание потоков связан с поворотом подразделениями, так как все гибкие объекты, возвращенные из этих функций (см. в разделе [гибкие объекты в C + +/ WinRT](agile-objects.md)). Это просто объекты, которые в одном потоке. И это полностью соответствующий, если вы просто хотите передавать данные один из способов или другой через двоичного интерфейса приложений (ABI).
