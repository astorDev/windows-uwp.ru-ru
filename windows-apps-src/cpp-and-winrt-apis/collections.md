...
Можно создать коллекцию среды выполнения Windows из данных и получить представление на ней готов к передаче API, без копирования никаких действий.
...
В примерах выше коллекции, мы создаем *можно* привязать к элементам управления XAML; Однако коллекция не доступным для отслеживания.
...
Объект возвращается в виде [**IObservableVector**](/uwp/api/windows.foundation.collections.iobservablevector_t_), и это интерфейса, через который вы (или элемент управления, к которому он привязан) вызывают функции и свойства возвращенный объект.
...
Существуют версии две функции, которые мы рассмотрели ассоциативность коллекции.
...
«Однопотоковом» в имена этих функций означает, что они не предоставляют все параллелизма&mdash;другими словами, они не являются потокобезопасными. Упоминание потоков связан с поворотом подразделениями, так как все гибкие объекты, возвращенные из этих функций (см. в разделе [гибкие объекты в C + +/ WinRT](agile-objects.md)). Это просто объекты, которые однопотоковом. И это полностью соответствующие в том случае, если вы просто хотите передачи данных один из способов или другой через двоичного интерфейса приложений (ABI).

## <a name="base-classes-for-collections"></a>Базовые классы для коллекций

Если для завершения гибкость, необходимо реализовать собственную пользовательскую коллекцию, вам потребуется избегать, опыте. Например, это представление пользовательского вектор будет выглядеть следующим образом *без помощи C + +/ WinRT базовых классов*.
...
Вместо этого он гораздо проще производным от шаблона структуры [**winrt::vector_view_base**](/uwp/cpp-ref-for-winrt/vector-view-base) представления пользовательского вектор и просто реализовать функцию **get_container** для предоставления контейнера, содержащего ваши данные.
...
Контейнер, возвращенный **get_container** необходимо предоставить интерфейс **начала** и **окончания** , **winrt::vector_view_base** ожидает, что. Как показано в примере выше, **std::vector** предоставляет. Но вы можете вернуть любого контейнера, удовлетворяющий тот же контракт, включая пользовательские контейнера.
...
Это базового класса C + +/ WinRT предоставляет для реализации пользовательских коллекций.
