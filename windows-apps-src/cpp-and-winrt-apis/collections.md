...
Вы можете избежать затрат вызовов в **Append** в предыдущем примере кода. Возможно, уже владеете исходным данным, или вы предпочитаете могут заполнить исходных данных до вступления в силу создания объекта коллекции среды выполнения Windows. Вот как это сделать.
...
Можно создать коллекцию среды выполнения Windows из данных и получить представление на нем готов к передаче API, все без копирования никаких действий.
...
Существует две функции, которые мы рассмотрели на версии ассоциативность коллекции.
...
«Однопотоковом» в имена этих функций означает, что они не предоставляют все параллелизма&mdash;другими словами, они не являются потокобезопасными. Упоминание потоков связан с поворотом подразделениями, так как все гибкие объекты, возвращенные из этих функций (см. в разделе [гибкие объекты в C + +/ WinRT](agile-objects.md)). Это просто объекты, которые однопотоковом. И это полностью соответствующий, если вы просто хотите передачи данных один из способов или другую через двоичного интерфейса приложений (ABI).

## <a name="base-classes-for-collections"></a>Базовые классы для коллекций

Если для завершения гибкости, необходимо реализовать собственные пользовательскую коллекцию, вам потребуется избегать, опыте. Например, это представление пользовательского вектор будет выглядеть следующим образом *без помощи C + +/ WinRT базовые классы*.
...
Контейнер, возвращенный **get_container** необходимо предоставить интерфейс **начала** и **окончания** этого **winrt::vector_view_base** ожидает. Как показано в примере выше, **std::vector** предоставляет. Но вы можете вернуть любые контейнера, удовлетворяющий тот же контракт, включая пользовательские контейнера.
...
Это базовый классы C + +/ WinRT предоставляет помогают реализовывать пользовательские коллекции.
