...
В приведенном выше примере кода вызов **winrt::init_apartment** инициализирует COM; по умолчанию в многопотокового подразделения.
...
Вы можете избежать затрат вызовов **Добавить** в предыдущем примере кода. Возможно, уже владеете исходным данным, или можно заполнить в источнике данных до вступления в силу создание объекта коллекции среды выполнения Windows. Вот как это сделать.
...
Можно создать коллекцию среды выполнения Windows из данных и получить представление на ней Готово к передаче API, без копирования никаких действий.
...
При необходимости можно простым этих коллекций с данными, передав функции *rvalue* тип **std::map** или **std::unordered_map**.
...
«Однопотоковом» в имена этих функций означает, что они не предоставляют все параллелизма&mdash;другими словами, они не являются потокобезопасными. Упоминание потоков связан с подразделениями, так как объект, возвращенный эти функции перемещаются (см. в разделе [гибкие объекты в C + +/ WinRT](agile-objects.md)). Это просто объекты, которые в одном потоке. И это полностью соответствующий, если требуется передавать данные один из способов или другой через двоичного интерфейса приложений (ABI).

## <a name="base-classes-for-collections"></a>Базовые классы для коллекций

Если для завершения гибкости необходимо реализовать свои собственные пользовательские коллекции, необходимо избегать, опыте. Например, это представление пользовательского вектор будет выглядеть следующим образом *без помощи C + +/ WinRT базовые классы*.
...
Вместо этого он гораздо проще производным от шаблона структуры [**winrt::vector_view_base**](/uwp/cpp-ref-for-winrt/vector-view-base) в представлении векторной и просто реализовать функцию **get_container** для предоставления доступа к контейнера, содержащего ваши данные.
