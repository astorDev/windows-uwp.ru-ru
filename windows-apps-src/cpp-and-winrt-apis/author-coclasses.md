...
ms.locfileid: "4389744"
---
# <a name="author-com-components-with-cwinrt"></a>Создание COM-компонентов с помощью C++/WinRT

[C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) помогут вам создать классической модели компонентных объектов (COM) компоненты (или классы), так же, как оно поможет вам создавать классы среды выполнения Windows. Вот простой иллюстрацию, на которой можно тестировать при вставке код в `pch.h` и `main.cpp` нового **Консольное приложение для Windows (C + +/ WinRT)** проекта.
...
В C + +/ WinRT реализовать класс фабрики и классы, производные от базовой структуры [**winrt::implements**](/uwp/cpp-ref-for-winrt/implements) . Сразу же после трех-директив using приведенный выше (и перед `main`), вставьте этот код для реализации компонента активатор уведомлений COM всплывающего уведомления.
...
Шаблон проекта создает `main` функции для вас. Удалите `main` функции, а вместо него вставьте этот список, код, который содержит код для регистрации вашего компонентного класса, а затем для доставки уведомления может обратного вызова приложения.
...
Если у вас уже есть библиотека DLL C + +/ классы среды выполнения Windows WinRT, будет уже есть функция **DllCanUnloadNow** , показано ниже. Если вы хотите добавить классы с этой библиотекой DLL, вы можете добавить функцию **DllGetClassObject** .
