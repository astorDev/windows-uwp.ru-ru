...
ms.locfileid: "7710769"
---
# <a name="author-com-components-with-cwinrt"></a>Создание COM-компонентов с помощью C++/WinRT

[C + +/ WinRT](/windows/uwp/cpp-and-winrt-apis/intro-to-using-cpp-with-winrt) помогут вам создать классической модели компонентных объектов (COM) компонентов (или классы), так же, как оно поможет вам создавать классы среды выполнения Windows. Вот простой иллюстрации можно проверить, вставьте код в `pch.h` и `main.cpp` нового **консольного приложения Windows (C + +/ WinRT)** проекта.
...
## <a name="a-more-realistic-and-interesting-example"></a>Пример более реалистичные и интересные
...
## <a name="implement-the-coclass-and-class-factory"></a>Реализовать фабрику компонентный класс и класс
...
В компонентный класс в приведенном выше коде мы реализуем метод **INotificationActivationCallback::Activate** , который функция, которая вызывается, когда пользователь нажимает кнопку обратного вызова на всплывающем уведомлении. Но прежде чем можно вызвать эту функцию, необходимо создать экземпляр компонентного, и это задание функцию **IClassFactory::CreateInstance** .

Компонентный класс, который мы только что реализован называется *активатор* уведомлений и его идентификатор класса (CLSID) имеет в виде `callback_guid` идентификатор ( **GUID**типа), вы см. в разделе выше. Мы будем использовать этот идентификатор позже, в виде главное меню и запись реестра Windows. Активатор COM CLSID и путь к его связанные COM-сервер (который является путь к исполняемому файлу, который мы создаем здесь) — это механизм, по которому всплывающее уведомление знает, что класса для создания экземпляра элемента при нажатии кнопки его обратного вызова (ли уведомление о нажатии в центре уведомлений или нет).

## <a name="best-practices-for-implementing-com-methods"></a>Советы и рекомендации по реализации методов COM

Методы для обработки ошибок и управление ресурсами можно перейти помощь в виде руки. Это более простой и удобной использовать исключения, чем коды ошибок. И если у вас имеется идиом ресурса приобретения есть инициализация (RAII), затем можно избежать, явно наличие коды ошибок и явное освобождение ресурсов. Такие явные проверки сделать код более замысловата, чем это необходимо, и дает ошибок немало места для скрытия. Вместо этого используйте RAII и генерации/catch исключения. Таким образом, ресурс распределения обработкой исключений, и ваш код прост.

Тем не менее не должна разрешать исключения для экранирования вашей реализации методов COM. Вы можете убедиться, что с помощью `noexcept` описатель на COM-методы. Это ОК для исключения в любом месте в граф вызова метода, до тех пор, пока их обработки перед выходом из метода. Если вы используете `noexcept`, но затем разрешите исключение для выхода вашего метода, а затем приложение будет завершено.
...
Шаблон проекта создает `main` функции для вас. Удалите `main` функцию, а вместо него вставьте этот код описания, который содержит код для регистрации вашего компонентный класс, а затем для доставки уведомления может обратного вызова вашего приложения.
...
### <a name="implement-the-coclass-class-factory-and-in-proc-server-exports"></a>Реализация компонентный класс, фабрики классов и экспорт в процессе сервера

Откройте `dllmain.cpp`и добавить в него текст кода, показано ниже.

Если у вас уже есть библиотеку DLL, которая реализует C + +/ WinRT среды выполнения Windows классы, то у вас будет уже есть функции **DllCanUnloadNow** , показанные ниже. Если вы хотите добавить компонентных классов с этой библиотекой DLL, можно добавить функцию **DllGetClassObject** .
