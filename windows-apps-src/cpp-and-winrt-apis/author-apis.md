...
В Visual Studio шаблон проекта **приложения основных компонентов Visual C++ (C++/WinRT)** иллюстрирует шаблон проекта **CoreApplication**. Шаблон начинается с передачи реализации [**Windows::ApplicationModel::Core::IFrameworkViewSource**](/uwp/api/windows.applicationmodel.core.iframeworkviewsource) в [**CoreApplication::Run**](/uwp/api/windows.applicationmodel.core.coreapplication.run).
...
Если ваш тип упакован в компонент среды выполнения Windows для использования из приложения, он должен представлять собой класс среды выполнения. Мы рекомендуем объявлять каждый класс среды выполнения в своем собственном файле языка описания интерфейса (IDL) (`.idl`). Вот пример.
...
- **MyRuntimeClass** — это имя класса среды выполнения. объявляемого в IDL и реализуемого в некоторых языках программирования.
...
- Независимо от наличия у вас объявленного в IDL конструктора для типа проекции создается перегрузка конструктора, принимающая `nullptr`. Вызов конструктора `nullptr` — это *первый из двух этапов* использования класс среды выполнения из *той же* единицы компиляции. Дополнительные сведения и пример кода см. в разделе [Использование API-интерфейсов в C++/WinRT](consume-apis.md#if-the-api-is-implemented-in-the-consuming-project).
...
Чтобы перейти от **MyType** к объекту **IStringable** или **IClosable**, который можно использовать или вернуть в качестве части вашей проекции, можно вызвать шаблон функции [**winrt::make**](/uwp/cpp-ref-for-winrt/make) шаблона функции. [**make**] возвращает интерфейс по умолчанию типа реализации.
...
> [!NOTE]
> Однако если вы ссылаетесь на тип из пользовательского интерфейса XAML, то в одном проекте будут и тип реализации, и тип проекции. В этом случае [**make**] возвращает экземпляр проецируемого типа. Пример кода для этого сценария см. в разделе [Элементы управления XAML; привязка к свойству C++/WinRT](binding-property.md#add-a-property-of-type-bookstoreviewmodel-to-mainpage).
