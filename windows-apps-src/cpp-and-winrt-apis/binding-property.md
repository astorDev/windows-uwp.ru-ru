...
>
> Модель представления является абстракцией представления, и поэтому привязаны непосредственно к представлению (разметку XAML). Модель данных — это абстракция данных, и оно было использоваться только из ваших моделей представления и не привязаны непосредственно к XAML. Таким образом вы можете объявить модели данных, не как классы среды выполнения, а в качестве структур C++ или классов. Они не нужно объявлять в MIDL, и вы будете использовать независимо от иерархии наследования, вы хотели.
...
Сохраните файл. Проект не будет создан до завершения в данный момент, но теперь построение является полезным очередь, необходимо сделать, так как она заново создает файлы исходного кода, в которых реализован класс **MainPage** (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h` и `MainPage.cpp`). Таким образом и перейти к построить сейчас. Ошибка сборки, можно ожидать на этом этапе **«MainViewModel»: не является членом «winrt::Bookstore::implementation::MainPage»**.

Если вы опустите включения из `BookstoreViewModel.idl` (см. список `MainPage.idl` выше), отобразится ошибка **ожидает \ < вблизи «MainViewModel»**. Еще один совет — убедиться, оставьте все типы в пространстве имен: пространство имен, которое отображается в примеры кода.
