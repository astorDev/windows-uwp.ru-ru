...
Начните с создания нового проекта в Microsoft Visual Studio. Создание **Visual C++** > **Универсальные** > **пустое приложение (C + +/ WinRT)** проект и назовите его в *магазине*.
...
>
> Любого класса среды выполнения, которые были объявлены в приложение, что *делает* являются производными от базового класса, называется *составные* класса. Также существуют ограничения по всему составные классы. Приложение могло пройти тесты [Комплект сертификации приложений для Windows](../debug-test-perf/windows-app-certification-kit.md) , используется в Visual Studio и в магазине Майкрософт для проверки отправки (и, следовательно, для приложения быть успешно добавлено в Microsoft Store), составные класса должны наследоваться от базового класса Windows. Это означает, что класс очень корне иерархии наследования должен являться типом из пространства имен Windows.*. Если вам нужно класс среды выполнения наследуется от базового класса&mdash;например, для реализации класса **BindableBase** для всех ваших моделей представления наследовать от&mdash;, а затем можно использовать наследование от [**Windows.UI.Xaml.DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject).
>
> Модель представления является абстракцией представления, и таким образом привязаны непосредственно к представлению (разметки XAML). Модель данных — это абстракция данных, и оно было использоваться только из ваших моделей представления и не привязаны непосредственно к XAML. Таким образом вы можете объявить модели данных, не как классы среды выполнения, но в качестве структур C++ или классов. Они не нужно объявлять в MIDL, и вы будете использовать любой иерархии наследования, вам.
...
Сохраните файл. Проект не будет создан до завершения на данный момент, но теперь построение является полезным аспектов, так как он обновляет файлы исходного кода, в которых реализован класс **MainPage** (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h` и `MainPage.cpp`). Таким образом и перейти к теперь сборки. Ошибка сборки, может ожидать на этом этапе **«MainViewModel»: не является членом «winrt::Bookstore::implementation::MainPage»**.

Если включить для `BookstoreViewModel.idl` (см. список `MainPage.idl` выше), отобразится ошибка **ожидает \ < около «MainViewModel»**. Еще один совет — убедиться, оставьте все типы в пространстве имен: пространство имен, которое отображается в описаниях кода.
...
## <a name="using-the-binding-markup-extension-with-cwinrt"></a>С помощью расширения разметки {Binding} в C + +/ WinRT
