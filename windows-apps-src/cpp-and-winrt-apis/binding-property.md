...
> [!NOTE]
> Классы модели представления&mdash;на самом деле, любого класса среды выполнения, которые были объявлены в приложении&mdash;требуется является производным от базового класса. Класс **BookSku** , объявили выше является примером этого. Он реализует интерфейс, но не является производным от базового класса.
>
...
>
> Модель представления является абстракцией представления, и поэтому привязаны непосредственно к представлению (разметки XAML). Модель данных — это абстракция данных, и оно было использоваться только из ваших моделей представления и не привязаны непосредственно к XAML. Таким образом вы можете объявить модели данных, не как классы среды выполнения, а в качестве структур C++ или классов. Их не нужно объявлять в MIDL, и вы можете использовать любой иерархии наследования, вы хотели.
...
Если вы опустите include из `BookstoreViewModel.idl` (см. список `MainPage.idl` выше), отобразится ошибка, **ожидает \< рядом с «MainViewModel»**. Еще один совет состоит в том, оставьте все типы в пространстве имен: пространство имен, которое отображается в описаниях кода.
