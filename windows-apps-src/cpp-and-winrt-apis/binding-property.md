...
Начните с создания нового проекта в Microsoft Visual Studio. Создание **Visual C++** > **Универсальные приложения для Windows** > **пустое приложение (C + +/ WinRT)** проект и назовите его *Bookstore*.
...
> [!NOTE]
> Классы модели представления&mdash;на самом деле, любого класса среды выполнения, которые были объявлены в приложении&mdash;требуется является производным от базового класса. Класс **BookSku** объявили выше является примером. Он реализует интерфейс, но он не является производным от базового класса.
>
> Любого класса среды выполнения, которые были объявлены в приложении, что *делает* являются производными от базового класса, называется *составные* класса. Также существуют ограничения по всему составные классы. Приложение может пройти тесты [Комплекта сертификации приложений для Windows](../debug-test-perf/windows-app-certification-kit.md) , используется в Visual Studio и через Microsoft Store для проверки отправок (и, следовательно, приложение могло быть успешно добавлено в Microsoft Store), необходимо составные класс наследоваться от базового класса Windows. Это означает, что класс очень корне иерархии наследования должен являться типом из пространства имен Windows.*. Если вам нужно создать среды выполнения класс, производный от базового класса&mdash;например, чтобы реализовать класс **BindableBase** для всех ваших моделей представления наследовать от&mdash;, а затем можно использовать наследование от [**Windows.UI.Xaml.DependencyObject**](/uwp/api/windows.ui.xaml.dependencyobject).
>
> Модель представления является абстракцией представления, и таким образом привязаны непосредственно к представлению (разметку XAML). Модель данных — это абстракция данных, и оно было потребить только из ваших моделей представления и не привязаны непосредственно к XAML. Таким образом вы можете объявить модели данных, не как классы среды выполнения, а в качестве структур C++ или классов. Они не нужно объявлять в MIDL и будете использовать независимо от иерархии наследования хотели.
...
Функция мутатора **названия** мы проверяем, ли значение устанавливается, отличающийся от текущего значения. И, если это так, мы обновляем название, а также вызывать событие [**INotifyPropertyChanged::PropertyChanged**](/uwp/api/windows.ui.xaml.data.inotifypropertychanged.PropertyChanged) с аргументом, равным имени свойства, которое было изменено. Это делается для того, чтобы пользовательскому интерфейсу было известно, значение какого свойства следует запрашивать повторно.
...
Сохраните файл. Проект не будет создан до завершения в данный момент, но создание теперь — это полезно объект, так как она заново создает файлы исходного кода, в которых реализован класс **MainPage** (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h` и `MainPage.cpp`). Таким образом и перейти к построить сейчас. Ошибка сборки можно ожидать на этом этапе **«MainViewModel»: не является членом «winrt::Bookstore::implementation::MainPage»**.

Если вы опустите включения из `BookstoreViewModel.idl` (см. список `MainPage.idl` выше), отобразится ошибка **ожидает \ < около «MainViewModel»**. Еще один совет имеет смысл оставлять все типы в одном пространстве имен: пространство имен, которое отображается в описаниях кода.

Чтобы устранить ошибки, мы ожидаем, что для просмотра, теперь необходимо скопируйте заглушки доступа для свойства **MainViewModel** из созданных файлов (`\Bookstore\Bookstore\Generated Files\sources\MainPage.h` и `MainPage.cpp`) и в `\Bookstore\Bookstore\MainPage.h` и `MainPage.cpp`.

В `\Bookstore\Bookstore\MainPage.h`, включают `BookstoreViewModel.h`, который объявляет тип реализации (**winrt::Bookstore::implementation::BookstoreViewModel**). Добавьте частный член для хранения модели представления. Обратите внимание, что функция доступа свойства (и член m_mainViewModel) реализуется относительно **Bookstore::BookstoreViewModel**, который является проецируемым типом. Тип реализации находится в том же проекте (единице компиляции) как приложения, поэтому мы создаем m_mainViewModel через перегрузку конструктора, который принимает `nullptr_t`. Также удалите свойство **MyProperty** .
...
В `\Bookstore\Bookstore\MainPage.cpp`, вызовите метод [**winrt::make**](/uwp/cpp-ref-for-winrt/make) (с типом реализации) для назначения нового экземпляра проецируемого типа для m_mainViewModel. Назначьте начальное значение для названия книги. Реализуйте метод доступа для свойства MainViewModel. И, наконец, обновите название книги в обработчике событий кнопки. Также удалите свойство **MyProperty** .
...
Откройте файл `MainPage.xaml`, который содержит разметку XAML для главной страницы пользовательского интерфейса. Как показано в следующем списке, удалите имя с кнопки и измените его **содержимое** значения из литерала на выражение привязки. Обратите внимание на свойство `Mode=OneWay` в выражении привязки (односторонняя из модели представления к пользовательскому интерфейсу). Без этого свойства пользовательский интерфейс не будет реагировать на события изменения свойств.
