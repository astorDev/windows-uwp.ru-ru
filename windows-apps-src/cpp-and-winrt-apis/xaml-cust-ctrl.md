---
author: stevewhims
description: В этом разделе описана этапы создания простого пользовательского элемента управления с помощью C + +/ WinRT. Вы можете создать данные для создания собственных функциональный и настраиваемых элементов управления пользовательского интерфейса.
...
Выше показан шаблон, который вы выполните при объявлении свойства зависимостей (DP). Существует два значения для каждого DP. Во-первых необходимо объявить статическое свойство только для чтения типа [**DependencyProperty**](/uwp/api/windows.ui.xaml.dependencyproperty). Он имеет имя вашего DP, а также *Свойства*. Вы используете этот статическое свойство в вашей реализации. Во-вторых вы объявляете свойство экземпляра чтения и записи с помощью типа и имя вашего DP.

> [!NOTE]
> Если вы хотите DP с типом с плавающей запятой, сделать ее `double` (`Double` в [MIDL 3.0](/uwp/midl-3/)). Объявления и реализации DP типа `float` (`Single` в MIDL), и затем задать значение для этого DP в разметке XAML, приводит к ошибке *не удалось создать «Windows.Foundation.Single» на основе текста "<NUMBER>"*.
...
В этом пошаговом руководстве мы не используем **OnLabelChanged**. Но есть таким образом, можно узнать, как зарегистрировать свойство зависимостей с помощью обратного вызова при изменении свойства. Реализация **OnLabelChanged** также показано, как получить производный тип проекции от базового типа проекции (базовый тип проекции — это **DependencyObject**, в данном случае). И показано, как затем получить указатель на тип, реализующий тип проекции. Эта операция второй естественным образом будет только в проекте, который реализует проецируемого типа (то есть проект, который реализует класс среды выполнения).
...
В его конструктор **BgLabelControl** устанавливает ключ стиля по умолчанию для себя. Но какие *— это* стиль по умолчанию? Пользовательского элемента управления (шаблона) необходимо иметь стиль по умолчанию&mdash;с шаблоном элемента управления по умолчанию&mdash;которой его можно использовать для отрисовки с на случай, если потребитель элемента управления не задает стиля и шаблона. В этом разделе мы добавим файла разметки в проект, содержащий нашем стиль по умолчанию.
...
В этом случае только свойства, который задает стиль по умолчанию — это шаблон элемента управления. Шаблон состоит из квадрат (которого фона привязывается к свойству **фона** , которой все экземпляры типа XAML [**элемента управления**](/uwp/api/windows.ui.xaml.controls.control) ) и текстовый элемент (текст которого привязано к свойству зависимостей **BgLabelControl::Label** ).
...
Кроме того, добавьте следующие директиву для `MainPage.h` так, чтобы информировать тип пользовательского элемента управления **BgLabelControl** типа **MainPage** (сочетание компиляции разметки XAML и императивном коде).
...
В этом пошаговом руководстве показали простой пример пользовательского элемента управления (шаблона) в C + +/ WinRT. Вы можете сделать собственных пользовательских элементов управления, произвольно полнофункциональной и полнофункциональным. Пример пользовательского элемента управления может занять форме что-то сложной, как редактируемых данных сетки, проигрыватель видео или визуализатор трехмерной геометрии.
