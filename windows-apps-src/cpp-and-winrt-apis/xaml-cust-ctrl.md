---
description: В этом разделе описана процедура этапы создания простого пользовательского элемента управления с помощью C + +/ WinRT. Вы можете создать на со статьей для создания собственных функциональных возможностей и настраиваемых элементов управления пользовательского интерфейса.
...
Выше показан шаблон, который вы выполните при объявлении свойства зависимостей (DP). Существует два значения для каждого DP. Во-первых можно объявить только для чтения статическое свойство типа [**DependencyProperty**](/uwp/api/windows.ui.xaml.dependencyproperty). Он имеет имя вашего DP, а также *Свойства*. Вы используете это статическое свойство в вашей реализации. Во-вторых вы объявляете свойство экземпляра чтения и записи с помощью типа и имя вашего DP. Если вы хотите создать *присоединенного свойства* (а не DP), затем см. в примерах кода в [пользовательские присоединенные свойства](/windows/uwp/xaml-platform/custom-attached-properties).

> [!NOTE]
> Если вы хотите DP с типом с плавающей точкой, сделать ее `double` (`Double` в [MIDL 3.0](/uwp/midl-3/)). Объявления и реализации DP типа `float` (`Single` в MIDL), а затем задать значение для этого DP в разметке XAML, приводит к ошибке *не удалось создать «Windows.Foundation.Single» на основе текста "<NUMBER>"*.
...
В его конструктор **BgLabelControl** устанавливает ключ стиля по умолчанию для себя. Но какие *— это* стиль по умолчанию? Необходимо иметь стиль по умолчанию пользовательского элемента управления (на основе шаблона)&mdash;с шаблоном элемента управления по умолчанию&mdash;которой его можно использовать для отрисовки с на случай, если потребитель элемента управления не задать стиль и шаблон. В этом разделе мы добавим файла разметки в проект, содержащий наших стиль по умолчанию.

В узле проекта создайте новую папку и назовите его «Темы». В разделе `Themes`, добавить новый элемент типа **Visual C++** > **XAML** > **Представление XAML**и назовите его «Generic.xaml». Имена папок и файлов должны быть следующим образом, чтобы платформа XAML для поиска стиль по умолчанию для пользовательского элемента управления. Удалите содержимое по умолчанию `Generic.xaml`и вставьте в разметке ниже.
...
Кроме того, добавьте следующие директиву для `MainPage.h` таким образом, учитывать тип пользовательского элемента управления **BgLabelControl** **MainPage** тип (сочетание компиляции разметки XAML и императивном коде). Если вы хотите использовать **BgLabelControl** из другой XAML-страницы, а затем добавьте этот же директивы #include в файл заголовка для этой страницы, слишком. Или, кроме того, просто поместите один директивы #include в вашем предварительно скомпилированный файл заголовка.
...
*Переопределяемые* функции имитируют по-разному в различных языковые проекции. В C#, например, переопределяемые функции обычно отображаются в виде защищенные виртуальные функции. В C + +/ WinRT, они не защищенные ни виртуальный, но по-прежнему можно переопределить их и предоставить собственные реализации, как показано выше.
