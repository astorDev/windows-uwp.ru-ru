...
> [!IMPORTANT]
> При регистрации делегата, приведенном выше примере кода передает необработанные *Этот* указатель (с указанием текущего объекта). Узнайте, как установить строгой или слабой ссылки на текущий объект, см. в разделе **при использовании функции-члена представителем** подразделе в разделе [безопасного доступа к *Этот* указатель с помощью делегата обработки события](weak-references.md#safely-accessing-the-this-pointer-with-an-event-handling-delegate).
