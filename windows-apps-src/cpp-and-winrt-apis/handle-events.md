...
> [!IMPORTANT]
> При регистрации делегата приведенный выше пример кода передает необработанный указатель *this* (указывающий на текущий объект). Чтобы узнать, как создавать строгие и слабые ссылки на текущий объект, ознакомьтесь с подразделом **Использование функции-члена в качестве делегата** раздела [Безопасный доступ к указателю *this* с помощью делегата, обрабатывающего события](weak-references.md#safely-accessing-the-this-pointer-with-an-event-handling-delegate).
...
Обычно при регистрации делегата возвращается маркер. Впоследствии можно использовать этот маркер для отзыва делегата. То есть регистрация делегата из события будет отозвана, и делегат не будет вызываться при следующем возникновении события. Для простоты ни один из приведенных выше примеров кода не показывает, как это можно сделать. Однако следующий пример сохраняет маркер в частном элементе данных структуры, а также отменяет его обработчик в деструкторе.
