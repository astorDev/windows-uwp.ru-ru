...
> [!IMPORTANT]
> При регистрации делегата, в примере выше передает необработанные *Этот* указатель (с указанием текущего объекта). Чтобы узнать, как установить строгой или слабой ссылки на текущий объект, см. в разделе **при использовании функции-члена представителем** вложенный раздела [безопасного доступа к *Этот* указатель с помощью делегата обработки события](weak-references.md#safely-accessing-the-this-pointer-with-an-event-handling-delegate).
...
Вместо строгой ссылки, как в примере выше, вы можете хранить слабую ссылку на кнопку (см. в разделе [надежную и слабые ссылки в C + +/ WinRT](weak-references.md)).
...
Если вы делегаты вместо сопрограммы, можно выбрать более простой синтаксис.
...
Если обработки события с помощью функции-члена объекта или из внутри лямбда-функции внутри функции-члена объекта, затем следует подумать о относительный время существования получателя события (объект, обработав событие) и источник события (объект вызов события). Дополнительные сведения и примеры кода см. в разделе [надежную и слабые ссылки в C + +/ WinRT](weak-references.md#safely-accessing-the-this-pointer-with-an-event-handling-delegate).
