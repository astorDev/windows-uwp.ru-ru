...
Вместо строгую ссылку, как в примере выше, вы можете хранить слабую ссылку на кнопку (см. в разделе [надежную и слабые ссылки в C + +/ WinRT](weak-references.md)).

Кроме того когда вы регистрируете делегата, можно указать **winrt::auto_revoke** (который является значением типа [**winrt::auto_revoke_t**](/uwp/cpp-ref-for-winrt/auto-revoke-t)) для запроса отзыва событий (от типа [**winrt::event_revoker**](/uwp/cpp-ref-for-winrt/event-revoker)). Отзыва событий содержит слабую ссылку на источник события (объект, который вызывает событие). Вы можете вручную отозвать событие, вызвав функцию-член **event_revoker::revoke**; но объект отзыва события вызывает эту функцию автоматически при выходе за пределы области действия. Функция **revoke** проверяет, существует ли источник события, и, если это так, отзывает ваш делегат. В этом примере нет необходимости хранить источник событий, а также не требуется деструктор.
