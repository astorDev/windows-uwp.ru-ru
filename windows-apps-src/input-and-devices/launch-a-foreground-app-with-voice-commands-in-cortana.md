...
Помимо использования в **Кортане** голосовых команд для доступа к системным функциям можно дополнить возможности **Кортаны** функциональностью вашего приложения. С помощью голосовых команд можно активировать приложение на переднем плане или выполнить в приложении определенное действие или команду. 
...
2.  В меню **Файл** выберите элемент **Создать** > **Проект**.
    ...
    Откроется диалоговое окно **Создание проекта**. В левой панели диалогового окна можно выбрать тип отображаемого шаблона.
...
1. В Visual Studio щелкните правой кнопкой мыши имя основного проекта и выберите элементы **Добавить > Новый элемент**. Добавление **XML-файла**
2. Введите имя [**VCD-файла**](https://msdn.microsoft.com/library/windows/apps/dn706593) (например, AdventureWorksCommands.xml) и щелкните элемент "Добавить". 
...
Добавьте элемент **VoiceCommands** с атрибутом **xmlns**, указывающим на
...
3. Добавьте элемент **Command** для каждой команды, поддержку которой необходимо обеспечить.
  ...
  > **Примечание.**  
Элементы  **ListenFor** невозможно изменить программно. Однако программно можно изменять элементы **PhraseList**, связанные с элементами **ListenFor**. Приложения должны изменять содержимое **PhraseList** во время выполнения на основе созданного набора данных, когда пользователь использует приложение. Ознакомьтесь со статьей [Динамическое изменение списков фраз для определения голосовых команд (VCD)](dynamically-modify-voice-command-definition--vcd--phrase-lists.md).
...
## <span id="Handle_activation_and_execute_voice_commands"></span><span id="handle_activation_and_execute_voice_commands"></span><span id="HANDLE_ACTIVATION_AND_EXECUTE_VOICE_COMMANDS"></span>Обработка активации и выполнение голосовых команд
