...
**Важно**  
Не рекомендуется менять реакцию на события взаимодействия встроенных жестов.
...
Ниже приведено несколько примеров встроенных зрительных образов контакта в Windows 8.
...
Тип информационного пользовательского интерфейса
Интервал времени
Активация
Использование
Всплывающая при заслонении подсказка (для функции "протаскивания" и мелких целевых объектов)
0 мс
Да
Для быстрого распознавания действий. Обычно используется для команд.
Всплывающая при заслонении подсказка (для выполнения действий)
200 мс
Да
Подробная подсказка
~2000 мс
Нет
Для неспешного, вдумчивого исследования и изучения. Обычно используется вместе с элементами семейства.
Взаимодействие самообнаружения
~2000 мс
Нет
Контекстное меню
~2000 мс
Нет
Предоставляет ограниченный набор команд, относящихся к выбранному объекту.
Всплывающие элементы
~2000 мс
Нет
Предоставляет ограниченный набор команд, относящихся к выбранному объекту.
...
**Интерфейс самообнаружения**

Взаимодействие самообнаружения — это визуальная информационная подсказка или анимация, демонстрирующая совершение какого-либо действия с целевым объектом и предоставляющая предварительный просмотр результата этого действия.

На изображениях ниже показано взаимодействие самообнаружения при выборе скольжением по диагонали на начальном экране. При прикосновении к плитке приложения (без перетаскивания) плитка скользит вниз (как при перетаскивании), открывая флажок выбора, который появится, если приложение будет действительно выбрано.

![Снимок экрана, показывающий состояние без выбора.](images/crossslide-selfreveal1.png)

*Нажмите пальцем элемент, чтобы начать взаимодействие самообнаружения для выбора. Взаимодействие самообнаружения показывает, какое действие будет выполнено для элемента.*

![Снимок экрана, показывающий анимацию выбора.](images/crossslide-selfreveal2.png)

*Не отрывая палец, проведите им по экрану, чтобы выбрать элемент.*

![Снимок экрана, показывающий анимацию перетаскивания.](images/crossslide-selfreveal3.png)

*Если пользователь продолжает перемещать палец по экрану, визуализация взаимодействия самообнаружения изменяется, показывая, что объект можно переместить.*

После отображения взаимодействия самообнаружения объект перестает быть активированным, как только пользователь отрывает от него палец.
