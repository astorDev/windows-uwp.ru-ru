...
:heavy_check_mark: SQLite — это облегченная и автономная база данных. Это библиотека кода без других зависимых компонентов. Ее не требуется настраивать.

:heavy_check_mark: Для работы с ней не требуется использовать сервер базы данных. Клиент и сервер работают в одном процессе.
...
Во-вторых приложение не нужно пакет библиотеки SQLite. Вместо этого ваше приложение может использовать версию SQLite, которая поставляется вместе с Windows. Это обеспечивает следующие преимущества.
...
:одно: Подготовка класса доступа к данным.
...
:five: Добавление базового интерфейса пользователя.
