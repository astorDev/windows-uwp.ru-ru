...
:heavy_check_mark: SQLite — упрощенная и автономные. Это библиотека кода без других зависимых компонентов. Ее не требуется настраивать.

:heavy_check_mark: Отсутствует сервер базы данных. Клиент и сервер работают в одном процессе.

:heavy_check_mark: SQLite является общедоступным, поэтому можно свободно использовать и распространять его вместе с вашим приложением.

:heavy_check_mark: SQLite работает на платформах и архитектур.
...
Библиотека [Microsoft.Data.Sqlite](https://docs.microsoft.com/dotnet/api/microsoft.data.sqlite?view=msdata-sqlite-2.0.0) реализует интерфейсы в пространстве имен [System.Data.Common](https://msdn.microsoft.com/library/system.data.common.aspx). Майкрософт активно поддерживает эти реализации и предоставляет интуитивно понятную оболочку для низкоуровневых собственных API-интерфейсов SQLite.
...
Во-вторых приложение не имеет для упаковки библиотеки SQLite. Вместо этого ваше приложение может использовать версию SQLite, которая поставляется вместе с Windows. Это обеспечивает следующие преимущества.

:heavy_check_mark: Уменьшает размер приложения, так как не нужно загрузить двоичный SQLite и затем упаковать его как часть приложения.

:heavy_check_mark: Позволяет избежать для принудительной отправки новой версии приложения для пользователей в случае недоступности SQLite публикует критически важные исправления ошибок и уязвимостей системы безопасности в SQLite. Версия Windows SQLite обслуживается корпорацией Майкрософт совместно с SQLite.org.

:heavy_check_mark: Время загрузки приложения может быть быстрее, так как, скорее всего, версию пакета SDK для SQLite будут уже загружены в память.

Давайте начнем с добавления библиотеки классов .NET Standard 2.0 в ваше решение. Использовать библиотеку классов для размещения вашего кода доступа к данным не обязательно, но мы используем ее в нашем примере. Мы назовем библиотеку **DataAccessLibrary**, а класс в библиотеке — **DataAccess**.
...
: один: Подготовьте класс доступа к данным.

: два: Инициализация базы данных SQLite.

: три: Вставка данных в базе данных SQLite.

: четыре: Получение данных из базы данных SQLite.

: пять: Добавление основной пользовательский интерфейс.
...
Добавьте следующие операторы using в верхней части этого файла.
...
Этот базовый пользовательский интерфейс предоставляет пользователю поле ``TextBox``, которое можно использовать для ввода строки, которую мы добавим в базу данных SQLite. Мы подключим ``Button``в этом пользовательском интерфейсе к обработчику событий, который будет получать данные из базы данных SQLite и затем отображать их в ``ListView``.

В файл **MainPage.xaml.cs** добавьте следующий обработчик. Это метод, который мы связали с событием ``Click`` кнопки ``Button`` в пользовательском интерфейсе.
...
**Подключение приложения непосредственно в базу данных SQL Server**

См. раздел [Использование базы данных SQL Server в приложении UWP](sql-server-databases.md).
