...
Службы приложений могут запускаться [вне процесса](/windows/uwp/launch-resume/how-to-create-and-consume-an-app-service) в фоновой задаче или [в процессе](/windows/uwp/launch-resume/convert-app-service-in-process) в рамках приложения переднего плана.
...
К папке общего хранилища предъявляются следующие требования и ограничения.

* Данные в папке общего хранилища не архивируются и не перемещаются. Кроме того, пользователь может очистить содержимое папки общего хранилища.
* Эту функцию нельзя использовать для обмена данными между приложениями разных издателей.
...
Для совместного использования файлов можно передать токены [шаредсторажеакцессманажер](/uwp/api/Windows.ApplicationModel.DataTransfer.SharedStorageAccessManager) в приложение через его значение.
...
Для обеспечения безопасности и сетевой изоляции подключения по обратной связи для IPC по умолчанию блокируются для упакованных приложений. Можно включить замыкание соединений между доверенными упакованными приложениями с помощью [возможностей](/previous-versions/windows/apps/hh770532(v=win.10)) и [свойств манифеста](/uwp/schemas/appxpackage/uapmanifestschema/element-uap4-loopbackaccessrules).

* Все Упакованные приложения, участвующие в подключениях замыкания на себя, должны будут объявить функцию `privateNetworkClientServer` в [манифестах пакетов](/uwp/schemas/appxpackage/uapmanifestschema/element-capability).
* Два упакованных приложения могут обмениваться данными через замыкание на себя, объявляя [лупбаккакцессрулес](/uwp/schemas/appxpackage/uapmanifestschema/element-uap4-loopbackaccessrules) в манифестах пакетов. Каждое приложение должно перечислить другое в его Лупбаккакцессрулес. Клиент объявляет правило "out" для сервера, а сервер объявляет правила "in" для поддерживаемых клиентов.

> [!NOTE]
> Имя семейства пакетов, необходимое для распознавания приложения в этих правилах, можно найти с помощью редактора манифеста пакета в Visual Studio во время разработки, через [Центр партнеров](/windows/uwp/publish/view-app-identity-details) для приложений, опубликованных с помощью Microsoft Store, или с помощью команды PowerShell [Get-AppxPackage](/powershell/module/appx/get-appxpackage?view=win10-ps) для уже установленных приложений.
...
* Если неупакованное приложение или служба подключаются к упакованному приложению, запустите `CheckNetIsolation.exe LoopbackExempt -is -n=<PACKAGEFAMILYNAME>`, чтобы пакетное приложение получало входящие подключения замыкания на себя. [Чеккнетисолатион. exe](/previous-versions/windows/apps/hh780593(v=win.10)) должен выполняться постоянно, пока упакованное приложение прослушивает подключения. Флаг `-is` появился в Windows 10, версия 1607 (10,0; Сборка 14393).
...
* Именованные каналы в упакованных приложениях поддерживаются только между процессами в одном пакете, если только процесс не имеет полного доверия.
...
[Сопоставление файлов](/windows/win32/memory/sharing-files-and-memory) можно использовать для совместного использования файла или памяти между двумя или более процессами в одном пакете.
